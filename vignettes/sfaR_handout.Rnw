...
Since its inception by \citet{aig77}, \citet{mee77}, and \citet{batt77} more than 40~years ago,
stochastic frontier analysis (SFA) has been used as a prominent tool for 
benchmarking many different performance aspects of various types of decision making units (DMUs). Since then 
several extensions have been developed to tackle different situations (e.g., data type, 
technological heterogeneity, endogeneity, \ldots). Overall, SFA can be used 
to evaluate different performance indicators for DMUs (e.g., technical and economic 
efficiencies, productivity, \ldots). By estimating the efficiency scores of 
DMUs, we can compare them, find out who is under-performing, and learn from the 
best practices. This can help make better policies to improve the 
...
SFA has been used in many different sectors including, e.g., agriculture 
\citep[e.g.,][]{latruffe23}, banking \citep[e.g.,][]{badu21}, education \citep[e.g., ][]{aga19}, 
energy demand or distribution \citep[e.g., ][]{filipp15, kop11}, health care \citep[e.g.,][]{ji20}, manufacturing \citep[e.g.,][]{boy19}, 
...
many of its variants are not covered by any of the aforementioned packages. For instance,
...
with each distribution can be found in Tables \ref{table:momentdesn1} and \ref{table:momentdesn2} in the 
...
are the standards in the stochastic frontier framework, other 
...
the relationship", which was later proved by \citet{wang02} to generate biased results.
...
$\exp{\left(-\hat{u}_i|\hat{\epsilon}_i\right)}$. Later, \citet{batt88} proposed to 
...
scores \citep{horrace96}. Tables~\ref{table:effcross1}, \ref{table:effcross2}, 
and \ref{table:effcross3}, in the Appendix, summarizes the main (in)efficiency measures. For the 
...
is solved using adaptive multidimensional integration over hypercubes\footnote{This 
is done using the \pkg{cubature} package \citep{cubature}}). For the
uniform distribution, different (in)efficiency scores can be derived when $\theta$
is finite or, $\theta \rightarrow \infty$.
...
cross-sectional models using the formulas in Tables~\ref{table:effcross1}, \ref{table:effcross2}, 
...
on a prior probability that can depend on some covariates. This probability can 
...
%
 ...
 in \ref{eq:3.2.6} contains the residuals from estimating the group frontiers. The robust
...
are presented in Table~\ref{table:gt} and can be chosen using the option
\code{modelType}. The same option is also available in the function \code{sfametacross}
to choose between the different metafrontiers as in Table~\ref{table:meta}.
...
supposed to be constant over time for each cross section. When this is not the case, three options 
...
production dataset. The estimation results can be found in Tables \ref{table:lcmgzisf}
and \ref{{table:lcmvariants}}.
