...
business applications and policy making. As such, the \pkg{sfaR} package intends to 
...
inefficiency term and a few panel-data models. For instance, the \pkg{frontier} package
implements only the half-normal and truncated normal distributions. Regarding the panel
models, \pkg{frontier} implements the time-invariant inefficiency model by 
\citet{pl81} and the time-varying efficiency model by \citet{batco92}. The package \pkg{sfaR} 
goes a step further and provides additional alternative specifications of time-varying inefficiency
...
found in the literature. For instance, the \pkg{Benchmarking} package \citep{bench} contains 
a very basic SFA model, the \pkg{sfadv} package \citep{sfadv} provides one methodology to 
deal with endogeneity in SFA, the packages \pkg{semsfa} \citep{semsfa} and \pkg{dsfa}  \citep{dsfa} offer semi- and 
non-parametric SFA models, and the \pkg{ssfa} package \citep{ssfa} allows for spatial 
stochastic frontiers. In addition to these packages, several R codes can be found
...
the \pkg{sfaR} package includes the standard latent-class SF, the zero-inefficiency SF, the 
contaminated-noise SF, the multi-modal inefficiency SF \citep{orea04, 
greene05, kum13, rho15, wheat19}, metafrontier estimations
\citep{batt04, huang14, am17}, and the sample-selection SF 
\citep{greene10}. 

While the \pkg{sfaR} package does not provide an exhaustive universe of the 
diverse stochastic frontier (SF) models that have been suggested in the previous four decades, 
it might be a first step into this direction. 
...
We consider a general stochastic frontier model:%
\footnote{For example, in case of a production frontier,
the dependent variable represents the output quantity,
while the explanatory variables represent the input quantities.
In case of a cost function, the dependent variable indicates the costs,
while the explanatory variables represent the input prices 
...
$\mathbf{x_i}$~is a vector of explanatory variables,
$\bm{\beta}$~is a vector of unknown coefficients,
$v_i$~is a two-sided (``noise'') error term that is usually assumed 
to follow a normal distribution $\mathcal{N}(0, \sigma_v^2)$,
$u_i$~is a one-sided (``inefficiency'') error term,
and $S = 1$ specifies a production frontier,
while $S = -1$ specifies a cost frontier. 

The spearhead for the SFA is the assumption regarding the distibution of
the inefficiency term~$u \geq 0$. 
Users of the \pkg{sfaR} package can choose among ten different distributions of~$u$ 
for standard SF models. Table~\ref{table:dens} presents the density 
functions of all the ten distributions implemented in the \pkg{sfaR} package.\footnote{Moments associated
with each distribution can be found in Table~\ref{table:momentdesn} in the 
Appendix.} For each of the distributions, the parameters can be obtained by 
maximizing the log-likelihood giving the convolution of $\epsilon_i=v_i - S \, u_i$. 
...
estimation for the rayleigh, generalized exponential, gamma, weibull, log-normal, 
truncated skewed laplace, and uniform distributions. For the gamma distribution, 
the hessian matrix is ill-conditionned leading to \code{NA} value in the standard
errors. As a solution to this problem, we re-run the estimation using the ML
parameters estimates as starting values, and a second order optimization 
alogorithm (here the Newton-Raphson). This strategy worked well for this case. The
estimation results are summarized in Tables \ref{table:repliotherdist1} and 
\ref{table:repliotherdist2}.
...
### ill-conditionned hessian matrix
...
## weibull distribution for u
we <- sfacross(formula = log(tc/wf) ~ log(y) + I(1/2 * (log(y))^2) +
 log(wl/wf) + log(wk/wf) + I(1/2 * (log(wl/wf))^2) + I(1/2 * (log(wk/wf))^2) +
 I(log(wl/wf) * log(wk/wf)) + I(log(y) * log(wl/wf)) + I(log(y) * log(wk/wf)),
 udist = 'weibull', uhet = ~ regu, data = utility, S = -1, Nsim = 100)
...
## truncated skewed laplace distribution
tsl <- sfacross(formula = log(tc/wf) ~ log(y) + I(1/2 * (log(y))^2) +
 log(wl/wf) + log(wk/wf) + I(1/2 * (log(wl/wf))^2) + I(1/2 * (log(wk/wf))^2) +
 I(log(wl/wf) * log(wk/wf)) + I(log(y) * log(wl/wf)) + I(log(y) * log(wk/wf)),
 udist = 'tslaplace', uhet = ~ regu, data = utility, S = -1)
## uniform distribution for u
uni <- sfacross(formula = log(tc/wf) ~ log(y) + I(1/2 * (log(y))^2) +
 log(wl/wf) + log(wk/wf) + I(1/2 * (log(wl/wf))^2) + I(1/2 * (log(wk/wf))^2) +
 I(log(wl/wf) * log(wk/wf)) + I(log(y) * log(wl/wf)) + I(log(y) * log(wk/wf)),
...
texreg(list(ray, ge, ga2, we), stars = c(0.01, 0.05, 0.1), 
       digits = 3, custom.model.names = c("rayleigh", "gexpo", "gamma", 
        "weibull"), table = FALSE)
@
%\end{longtable}
\end{adjustbox}
\caption{Other distributions using data from Table 4.1 in \citet[p.~119]{kum14}, Part 1}
\label{table:repliotherdist1}
\end{table}
...
%\begin{longtable}{l c c c c c}
<<table4.1newdist, echo=FALSE, results=tex>>=
texreg(list(lg, tsl, uni), stars = c(0.01, 0.05, 0.1), 
       digits = 3, custom.model.names = c("lognorm", "tsl", "uni"), table = FALSE)
@
%\end{longtable}
\end{adjustbox}
\caption{Other distributions using data from Table 4.1 in \citet[p.~119]{kum14}, Part 2}
\label{table:repliotherdist2}
\end{table}
...
weeff <- efficiencies(we)
wemarg <- marginal(we)
lgeff <- efficiencies(lg)
lgmarg <- marginal(lg)
tsleff <- efficiencies(tsl)
tslmarg <- marginal(tsl)
# unieff <- efficiencies(uni)
# unimarg <- marginal(uni)
...
dkum14 <- as.data.frame(rbind(c(apply(hlfeff, 2, mean)[nameselect], apply(hlfmarg, 2, mean)), 
                ...
                c(apply(weeff, 2, mean)[nameselect], apply(wemarg, 2, mean)), 
                c(apply(lgeff, 2, mean)[nameselect], apply(lgmarg, 2, mean)), 
                c(apply(tsleff, 2, mean)[nameselect], apply(tslmarg, 2, mean))))
dkum14 <- cbind(Distributions = c("Half-Normal", "Truncated Normal", "Scaling", "Exponential", 
                                  "Raleigh", "Gen. Exponential", "Gamma", "Weibull", 
                                  "Log-normal", "Trunc. Skewed Laplace"), 
                dkum14)
