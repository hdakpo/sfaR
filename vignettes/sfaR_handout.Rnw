...
\Abstract{
  This article gives an overview of the capabilities of the \proglang{R} package \pkg{sfaR}
  ...
  For many of these models, several distributions can be chosen for 
  the one-sided error term (e.g., half-normal, 
  ...
  advantage of the \pkg{sfaR} package is the availability of  different
  optimization algorithms. This article describes the main functions provided by the 
...
Since its inception by \citet{aig77}, \citet{mee77}, and \citet{batt77} more than 40~years ago, stochastic frontier analysis (SFA) has been used as a prominent tool for benchmarking many different performance aspects of various types of decision making units (DMUs). Since then several extensions have been developed to tackle different situations (e.g., data type, technological heterogeneity, endogeneity, \ldots). Overall, SFA can be used to evaluate different performance indicators for DMUs (e.g., technical and economic efficiencies, productivity, \ldots). By estimating the efficiency scores of DMUs, we can compare them, find out who is under-performing, and learn from the best practices. This can help make better policies to improve the efficiency of both private and public sectors \citep{par14}. 
...
\citep[e.g.,][]{mada23}, or to analyze the impact of environmental factors \citep[e.g.,][]{li19}. SFA has been used in many different sectors including, e.g., agriculture \citep[e.g.,][]{latruffe23}, banking \citep[e.g.,][]{badu21}, education \citep[e.g., ][]{aga19}, energy demand or distribution \citep[e.g., ][]{filipp15, kop11}, health care \citep[e.g.,][]{ji20}, manufacturing \citep[e.g.,][]{boy19}, 
museums \citep[e.g.,][]{price23}, and transportation \citep[e.g., ][]{coelli99, 
filipp16}. 

The scope of SFA spreads from pure research objectives to 
business applications and policy making. As such, the \pkg{sfaR} package intends to provide a platform for the basics but also for some of the recent advances in 
...
\proglang{Limdep}, \proglang{Stata}, \proglang{Ox}, \ldots}.
...
\footnote{The package \pkg{frontier} is based on the 
...
inefficiency term and a few panel-data models. For instance, the \pkg{frontier} package implements only the half-normal and truncated normal distributions. Regarding the panel models, \pkg{frontier} implements the time-invariant inefficiency model by \citet{pl81} and the time-varying efficiency model by \citet{batco92}. The package \pkg{sfaR} goes a step further and provides additional alternative specifications of time-varying inefficiency models \citep{kum90, batco92, lees93, cue00, cue02, feng09, kw05, al06}.
...
found in the literature. For instance, the \pkg{Benchmarking} package \citep{bench} contains a very basic SFA model, the \pkg{sfadv} package \citep{sfadv} provides one methodology to deal with endogeneity in SFA, the packages \pkg{semsfa} \citep{semsfa} and \pkg{dsfa}  \citep{dsfa} offer semi- and non-parametric SFA models, and the \pkg{ssfa} package \citep{ssfa} allows for spatial stochastic frontiers. In addition to these packages, several R codes can be found online to conduct some SFA \citep{si20, si22, nguyen22}.} Still, many of the recent developments are still lacking, especially regarding technological heterogeneity. For instance, the latent class stochastic frontier models, and many of its variants are not covered by any of the aforementioned packages. For instance, the \pkg{sfaR} package includes the standard latent-class SF, the zero-inefficiency SF, the 
contaminated-noise SF, the multi-modal inefficiency SF \citep{orea04, greene05, kum13, rho15, wheat19}, metafrontier estimations \citep{batt04, huang14, am17}, and the sample-selection SF \citep{greene10}. 

While the \pkg{sfaR} package does not provide an exhaustive universe of the 
diverse stochastic frontier (SF) models that have been suggested in the previous four decades, it might be a first step into this direction. As we expect the package to grow and even include other developers extension. As of now, the package, in summary, can handle cross-section but also panel data depending on the 
...
implemented and grouped into ten main functions. This article summarizes the main information about each group of models implemented and illustrate them using real life data. The structure of the paper is as follows. Section \ref{sec:modelstandar} gives a brief overview of the standard SF models in the literature for cross-section and panel data. Section \ref{sec:modelcm} describes the latent class SF in
addition to its variants. Section \ref{sec:other} presents the other 
SF models present in \pkg{sfaR}, namely the metafrontiers and the sample 
...
while the explanatory variables represent the input quantities. In case of a cost function, the dependent variable indicates the costs, while the explanatory variables represent the input prices and the output quantities. 
...
to follow a normal distribution $\mathcal{N}(0, \sigma_v^2)$, $u_i$~is a one-sided (``inefficiency'') error term, and $S = 1$ specifies a production frontier,
while $S = -1$ specifies a cost frontier. 

The spearhead for the SFA is the assumption regarding the distibution of
the inefficiency term~$u \geq 0$. Users of the \pkg{sfaR} package can choose among ten different distributions of~$u$ for standard SF models. Table~\ref{table:dens} presents the density functions of all the ten distributions implemented in the \pkg{sfaR} package.\footnote{Moments associated with each distribution can be found in Table~\ref{table:momentdesn1} and \ref{table:momentdesn2} in the Appendix.} For each of the distributions, the parameters can be obtained by maximizing the log-likelihood giving the convolution of $\epsilon_i=v_i - S \, u_i$. 

The most commonly used distributions are the one-parameter half-normal distribution
\citep{aig77} and the exponential distribution \citep{mee77}. Their large success 
...
truncated normal \citep{ste80, kum87}, and the gamma \citep{ste80, beckers87, greene90, greene03} were later introduced to shift away from the zero mode distributions. A particular feature of these two distributions is that, under specific conditions they nest the half-normal and the exponential distributions, 
...
are the standards in the stochastic frontier framework, other distributions are also introduced. For instance, the rayleigh \citep{oliv14, hajar15, wang20}, and the generalized exponential \citep{papa21} are one-parameter distributions, which have non-zero mode. Another alternative two-parameter distribution is the log-normal \citep{wang20}, which can be a good choice for certain type of datasets \citep{migon01, rama12}.

A common feature of all the previous distributions is their positive skewness, 
which supposed a negative third moment for the distribution of $\epsilon_i$ when $S=1$. However, in practice, the third moment of $\epsilon_i$ can be positive \citep{sw09}. This generate a type I error, which might prevent the identification of the inefficiency distribution parameters \citep{olson80}. According to \citet{wald82}, the "wrong" skewness implies that the maximum likelihood and the OLS estimates of equation~\ref{eq:1.1.1} will have the same slopes and there are no inefficiencies. A long debate has then emerged around the "wrong" skewness. An advantage of the weibull and the truncated skewed-laplace distributions is to allow positive or negative skeweness \citep{tsionas07, wang12}, and therefore identifying inefficiencies even when the OLS residuals has the "wrong" skewness. On another hand, the uniform distribution \citet{li96, nguyen10, lee14}, which has a zero skewness, has been introduced to handle situations where the inefficiency distribution is symmetric around the mean. In this latter case, the OLS residuals will also be symmetric, and based on the previous distributions, no inefficiencies can be measured. Another advantage of the uniform distribution is to set a threshold on the inefficiency, which can provide useful economic information \citep{lee14}.

\textbf{Do distributional assumptions even matter?} If as underlined by 
\citet[p.~16]{kum20} the "choice of $u$ is often driven through available statistical software to implement the method rather than an underlying theoretical link between a model of productive inefficiency and the exact shape of the 
...
also \citet[p.~214-216]{par14} for more discussion on the matter of the choice of $u$ distribution.} It is worth mentioning that if one is only interested on the features of the frontier, as long as inefficiency is not conditional on some variables, the OLS results should be sufficient.
...
component can lead to inconsistent results. As underlined in \citet[p.~115]{kum14}, "the early literature adopts a two-step procedure to investigate the relationship", which was later proved by \citet{wang02} to generate biased results. In the case of the truncated normal distribution, \citet{kum91, hua94, batt95} suggested to only parameterize the mean of the distribution as $\mu_i = \mathbf{z}'_{\bm\mu, \mathbf{i}}\bm\zeta$ when examining the inefficiency determinants. However, to account for non-monotonicity in the determinants of inefficiency, \citet{wang02b} suggested to parameterized both the mean and and the variance of the pre-truncated distribution. The same process can also be applied to the log-normal distribution.
...
inefficiency}, and its distribution the \textit{base distribution}
...
scaling function $h(\cdot)$ expands or contracts the scale of the 
...
experience, or measures of the environment in which the firm operates, for example."} When determinants of inefficiency are evaluated, so does the marginal impact of each $z$ variable on inefficiency. This marginal impact is estimated using the
...
one is interested in the average inefficiency ($E\left[u\right]=\sigma_u\sqrt{\frac{2}{\pi}}$). However, to obtain individual observation inefficiency, $\sigma_u^2$ is not anymore enough.\footnote{Unless inefficiency determinants are considered.} An early solution was proposed by \citet{jon82}
...
$u_i$ are obtained, technical efficiency can be computed as $\exp{\left(-\hat{u}_i|\hat{\epsilon}_i\right)}$. Later, \citet{batt88} proposed to 
derive the conditional mean efficiency using $E\left[\exp{\left(-u_i\right)|\hat{\epsilon}_i}\right]$. We went a step further in the \pkg{sfaR} package to also provide the reciprocal mean efficiency: $E\left[\exp{\left(u_i\right)|\hat{\epsilon}_i}\right]$. Also whenever possible we also provide the confidence intervall of the (in)efficiency scores \citep{horrace96}. Tables~\ref{table:effcross1}, \ref{table:effcross2}, and \ref{table:effcross3}, in the Appendix, summarizes the main (in)efficiency measures. For the weibull and log-normal distributions, all (in)efficiency measures are evaluated by solving a numerical integration (in \pkg{sfaR}, this is solved using adaptive multidimensional integration over hypercubes\footnote{This is done using the \pkg{cubature} package \citep{cubature}}). For the uniform distribution, different (in)efficiency scores can be derived when $\theta$ is finite or, $\theta \rightarrow \infty$.
...
of panel models consider the time invariant heterogeneity as if it were inefficiency, the second generation of models simultaneously consider firm heterogeneity in addition to inefficiency. Finally, the third generation model considers firm heterogeneity, and persistent and transient inefficiency components. All the panel models considered in \pkg{sfaR} are distribution-based models.
...
has only been developed for a few distributions for $u_i$, e.g., half-normal \citep{pl81}, truncated normal \citep{batt88}, rayleigh \citep{hajar15}, uniform \citep{nguyen10}, we extend it in the \pkg{sfaR} package to all the other distributions.\footnote{Proofs for the derivation of the density function
of $f(\epsilon)$ is available from the authors upon request.} 
An overview of the density of $\epsilon_i$ can be found in Table~\ref{table:mlpl81}, in the Appendix.\footnote{A distribution-free estimation of this model has been discussed in \citet{sl84}.}

Heterogeneity and heteroscedasticity can be handled in the \citet{pl81} model.
However, the determinant variables $\mathbf{z}$ have to be time invariant \citep{kum2000}. In the \pkg{sfaR}, we suggest several ways to obtain time invariant determinants (see section \ref{sec:prespackage}). Regarding the (in)efficiency scores, they are computed the same way as in the cross-sectional models using the formulas in Tables~\ref{table:effcross1}, \ref{table:effcross2}, 
and \ref{table:effcross3} in the Appendix.
...
first to suggest an ML estimation of time-varying inefficiency. Later \citet{batco92} developed the "time-decay" model along with two other alternatives. Many other models were suggested as documented in Table~\ref{table:gt}. 
...
%\hdashline 
\citet{pl81} & $G(t) = 1$ & \code{pl81} \\[1em]
\bottomrule
\end{tabular}
\end{adjustbox}
...
efficiency drivers is the one by \citet{al06}. For all the other models, efficiency
...
been extended to the SF framework by \citet{cau03, orea04}. In the next subsections, we describe the LCM for both pooled cross-section and panel data.
...
%
 ...
 \sigma_{v|j}^2}$$
%
The distribution of observations into classes is based on a prior probability that can depend on some covariates. This probability can be modelled using a logit distribution as
%
...
%
 ...
 estimation. It is worth noting that the \pkg{sfaR} package, currently, only considers the latent class SF model with the half-normal distribution.

\subsubsection{For panel data}

The difference with the pooled LCM is that the probability of belonging to 
a class is fixed over time. The logit distribution in \ref{eq:2.1.3} becomes
%
...
%
 ...
 \pkg{sfaR} package estimates the ZISF for cross-section or pooled data.} In 
 this case we have:
 %
...
\alpha + \mathbf{x_i^{\prime}}\bm{\beta} + 
 ...
 \alpha + \mathbf{x_i^{\prime}}\bm{\beta} +
  v_{2i} & \text{with probability} \quad 1-p
\end{cases}
\end{equation}
%
...
\end{equation}
%
All the derivations are similar to the ZISF case. The prior probability
...
between the two groups of observations. In this case we have:
%
\begin{equation}\label{eq:2.3.2}
y_i = \begin{cases}
\alpha + \mathbf{x_i^{\prime}}\bm{\beta} + 
 v_{1i} - Su_{1i} & \text{with probability} \quad p \\
 \alpha + \mathbf{x_i^{\prime}}\bm{\beta} + 
v_{2i} - Su_{2i} & \text{with probability} \quad 1-p
\end{cases}
\end{equation}
%
...
%
 ...
 likelihood can be estimated using five different approaches: Gauss-Kronrod quadrature, adaptive integration over hypercubes (hcubature and pcubature), Gauss-Hermite quadrature, and maximum simulated likelihood. We also use the BHHH estimator to obtain the asymptotic standard errors for the parameter estimates.
...
%
 ...
 in \ref{eq:3.2.6} contains the residuals from estimating the group frontiers. The  robust sandwich-form of the variance-covariance matrix estimator is used to 
 correct for the standard errors.

 A new stochastic metafrontier has also been suggested in  \citet{am17}. Based on the previous developments, The stochastic  frontiers corresponding to each group are defined as
 %
...
\begin{equation}\label{eq:3.2.14}
 ...
 assuming that the $v_{is}$ are not independent between groups.
...
specific class are included in the fomula based option \code{thet}. Heteroscedasticity in the variances of the one-sided and the two-sided error terms $u$ and $v$ are also defined using one-sided formulas (only right arguments e.g. \code{~x}) with \code{uhet} and \code{vhet}, respectively. Similarly, when some 
...
\code{sfalcmpanel} and \code{sfagzisfcross} functions, the probability of belonging to a class is parameterized as a logit model, for \code{sfacnsfcross}, \code{sfamisfcross}, and \code{sfazisfcross}, four models are possible through the \code{linkF} argument:
%
\begin{itemize} \itemsep 10pt
...
\end{itemize}
%
For the standard SF models using panel data, the different models in \code{sfapanel1} are presented in Table~\ref{table:gt} and can be chosen using the option \code{modelType}. The same option is also available in the function \code{sfametacross} to choose between the different metafrontiers as in Table~\ref{table:meta}.
...
supposed to be constant over time for each cross section. When this is not the case, three options are suggested to create constant variables: when \code{invariance = 1L}, the first period observation is used; when \code{invariance = 2L}, the last
period observation is used (this is the default option); and when 
\code{invariance = 3L}, the period mean is used.
...
starting values are obtained from the method of moments or from a simpler homoscedastic M(S)L estimation}, and to allow for a random start (\code{randStart} argument). The random starting values are obtained by taking the initial starting values and adding random numbers drawn from a normal distribution with zero mean and standard value of $0.01$.
...
\pkg{texreg} package \citep{texreg13}. The following code generates Table~\ref{table:replikum}.
...
\pkg{sfaR} allows us to test different distribution. The following codes run the
estimation for the rayleigh, generalized exponential, gamma, and log-normal 
distributions. For the gamma distribution, the hessian matrix is ill-conditionned 
leading to \code{NA} value in the standard errors. As a solution to this problem, 
we re-run the estimation using the ML parameters estimates as starting values, 
and a second order optimization algorithm (here the Newton-Raphson). This 
strategy worked well for this case. The estimation results are summarized in 
Tables \ref{table:repliotherdist}.
...
production dataset. The estimation results can be found in Tables \ref{table:lcmgzisf} and \ref{{table:lcmvariants}}.
...
(see Table \ref{table:lcmEff}). The command also provides posterior probabilities
for each class, which as suggested by \citet{greene05} can be used to compute 
individual specific parameters, in light of the random parameters model.
...
The results of the estimation can be found in Table \ref{table:panel1g}. The 
efficiency scores are displayed in Table \ref{table:panel1gEff}.
...
To illustrate the metafrontier estimation, we replicate Table 2 in \citep[pp. 1000-1001]{owu22}. In this article, the authors examined productivity differences
between male and female groundnut producers in Malawi. For the estimation, 
the authors assumed a Cobb-Douglas production function where \code{gnut} is 
the groudnut output production in kilograms. The inputs include land in ha
(\code{land}), labour in worker-days (\code{labor}), seed in kg (\code{seed}), 
and agro-chemicals in MWK (\code{agrochem}). Production practices variables are also included in the production technology: use of hired labour (\code{usehirlab}), use of improved seed (\code{imprseed}), planting time (\code{earlypln}, \code{latepln}), and soil type (\code{soil3}). Additional socioeconomic variables 
like exposure to agricultural extension (\code{extens}), social group membership
(\code{socnet}), and whether or not farmer is affiliated with an anchor farm (
\code{farmtyp2}), \code{farmtyp3}) are also considered. Finally, districts fixed
effects (\code{dist1}, \code{dist2}) are also controlled for the estimation.
...
We illustrate the LCM for panel data using the Spanish dairy farms data. The dataset is embedded in the \pkg{sfaR} package as \code{dairy}.
...
\begin{tablenotes}
\tiny
\item \textbf{Half-Normal}: $\mu_{*}= \frac{-S\sigma_u^2\sum_{t=1}^TG(t)\epsilon_{t}}{\sigma_u^2\sum_{t=1}^TG(t)^2 + \sigma_v^2}$ and 
$\sigma_*^2=\frac{\sigma_u^2\sigma_v^2}{\sigma_u^2\sum_{t=1}^TG(t)^2 + \sigma_v^2}$
\item \textbf{Truncated normal}: $\mu_{*}= \frac{\mu\sigma_v^2-S\sigma_u^2\sum_{t=1}^TG(t)\epsilon_t}{\sigma_u^2\sum_{t=1}G(t)^2+\sigma_v^2}$ and 
$\sigma_*^2=\frac{\sigma_u^2\sigma_v^2}{\sigma_u^2\sum_{t=1}^TG(t)^2 + \sigma_v^2}$
\item \textbf{Exponential}: $\mu_* = -\left(\frac{\sigma_v^2}{\sigma_u\sum_{t=1}G(t)^2}+\frac{S\sum_{t=1}^TG(t)\epsilon_t}{\sum_{t=1}G(t)^2}\right)$ and 
$\sigma_*^2 = \frac{\sigma_v^2}{\sum_{t=1}G(t)^2}$
\item \textbf{Rayleigh}: $\mu_* = -\frac{S\sigma_u^2\sum_{t=1}^TG(t)\epsilon_t}{\sigma_u^2\sum_{t=1}G(t)^2+\sigma_v^2}$ and $\sigma_*^2 = \frac{\sigma_u^2\sigma_v^2}{\sigma_u^2\sum_{t=1}G(t)^2+\sigma_v^2}$
...
$\sigma_*^2 = \frac{\sigma_v^2}{\sum_{t=1}G(t)^2}$, and $F_{r}$ is a pseudo/quasi random draw
\item \textbf{Log-normal}: $u_r=\exp{\left(\mu +\sigma_u\Phi^{-1}\left(h_r\right)\right)}$ and $h_r$ is a pseudo/quasi random draw
\item \textbf{Weibull}: $u_r = \sigma_u\left(-\ln{\left(1-h_r\right)}\right)^{1/k}$ and $h_r$ is a pseudo/quasi random draw
\item \textbf{Generalized exponential}: $\mu_* = -\left(\frac{\sigma_v^2}{\sigma_u\sum_{t=1}G(t)^2}+\frac{S\sum_{t=1}^TG(t)\epsilon_t}{\sum_{t=1}G(t)^2}\right)$ and 
$\mu_*' = -\left(\frac{2\sigma_v^2}{\sigma_u\sum_{t=1}G(t)^2}+\frac{S\sum_{t=1}^TG(t)\epsilon_t}{\sum_{t=1}G(t)^2}\right)$, \\ $\sigma_*^2 = \frac{\sigma_v^2}{\sum_{t=1}G(t)^2}$
\item \textbf{Truncated skewed-Laplace}: $\mu_* = -\left(\frac{\sigma_v^2}{\sigma_u\sum_{t=1}G(t)^2}+\frac{S\sum_{t=1}^TG(t)\epsilon_t}{\sum_{t=1}G(t)^2}\right)$ and $\mu_*' = -\left(\frac{\left(1+\lambda\right)\sigma_v^2}{\sigma_u\sum_{t=1}G(t)^2}+\frac{S\sum_{t=1}^TG(t)\epsilon_t}{\sum_{t=1}G(t)^2}\right)$, \\ $\sigma_*^2 = \frac{\sigma_v^2}{\sum_{t=1}G(t)^2}$
\item \textbf{Uniform}: $\mu_* = -\frac{S\sum_{t=1}^TG(t)\epsilon_t}{\sum_{t=1}G(t)^2}$ and $\sigma_*^2 = \frac{\sigma_v^2}{\sum_{t=1}G(t)^2}$
\end{tablenotes}
...
\begin{tabular}{@{}cc@{}}
\toprule
Densities & $E\left[\exp{\left(-G(t)u_i\right)|\epsilon_i}\right]$ \\
\midrule
...
%\hdashline 
Exponential & $\exp{\left(-G(t)\mu_{i*} + \frac{1}{2}G(t)^2\sigma_v^2\right)} \frac{\Phi\left(\frac{\mu_{i*}}{\sigma_v}-G(t)\sigma_v\right)}{\Phi\left(\frac{\mu_{i*}}{\sigma_v}\right)}$\\
%\hdashline
Rayleigh & $\exp{\left(-G(t)\mu_{i*} +\frac{G(t)^2\sigma_*^2}{2}\right)}\frac{\left[\left(G(t)\mu_{i*}-G(t)^2\sigma_*^2\right)\Phi\left(\frac{\mu_{i*}}{\sigma_*} - G(t)\sigma_*\right) + G(t)\sigma_*\phi\left(\frac{\mu_{i*}}{\sigma_*} - G(t)\sigma_*\right)\right]}{\left[\mu_{i*}\Phi\left(\frac{\mu_{i*}}{\sigma_*}\right) + \sigma_*\phi\left(\frac{\mu_{i*}}{\sigma_*}\right)\right]}$ \\
%\hdashline
Gamma & $\frac{\exp{\left(\frac{\sigma_v^2}{\sigma_u} + S\epsilon + \frac{\sigma_v^2}{2}\right)}\Phi\left(-\frac{\sigma_v}{\sigma_u} - \frac{S\epsilon}{\sigma_v} - \sigma_v\right)  \hat{g}(P-1, \epsilon)}{\Phi\left(-\frac{\sigma_v}{\sigma_u} - \frac{S\epsilon}{\sigma_v}\right)\hat{h}(P-1, \epsilon)}$ \\
%\hdashline
Log-Normal & $\frac{\frac{1}{\sigma_u\sigma_v}\int_0^\infty \frac{\exp{\left(-u\right)}}{u} \phi\left(\frac{\ln{u}-\mu}{\sigma_u}\right)\phi\left(\frac{\epsilon_i+Su}{\sigma_v}\right)du}{\frac{1}{R}\sum_{r = 1}^R\frac{1}{\sigma_v}\phi\left(\frac{\epsilon_i+Su_{ir}}{\sigma_v}\right)}$ \\
...
Truncated-Normal & $\exp{\left[\frac{1}{2}\sigma_*^2G(t)^2+\mu_*G(t)\right]\frac{\Phi\left(\frac{\mu_*}{\sigma_*}+G(t)\sigma_*\right)}{\Phi\left(\frac{\mu_*}{\sigma_*}\right)}}$  \\
%\hdashline 
Exponential & $\exp{\left(G(t)\mu_* + \frac{1}{2}G(t)^2\sigma_v^2\right)} \frac{\Phi\left(\frac{\mu_*}{\sigma_v}+G(t)\sigma_v\right)}{\Phi\left(\frac{\mu_*}{\sigma_v}\right)}$\\
%\hdashline
Rayleigh & $\exp{\left(G(t)\mu_* +\frac{G(t)^2\sigma_*^2}{2}\right)}\frac{\left[\left(G(t)\mu_*+G(t)^2\sigma_*^2\right)\Phi\left(\frac{\mu_*}{\sigma_*} + \sigma_*\right) + \sigma_*\phi\left(\frac{\mu_*}{\sigma_*} + \sigma_*\right)\right]}{\left[\mu_*\Phi\left(\frac{\mu_*}{\sigma_*}\right) + \sigma_*\phi\left(\frac{\mu_*}{\sigma_*}\right)\right]}$\\
%\hdashline
Gamma & $\frac{\exp{\left(-\frac{\sigma_v^2}{\sigma_u} - S\epsilon + \frac{\sigma_v^2}{2}\right)}\Phi\left(-\frac{\sigma_v}{\sigma_u} - \frac{S\epsilon}{\sigma_v} + \sigma_v\right)  \hat{k}(P-1, \epsilon)}{\Phi\left(-\frac{\sigma_v}{\sigma_u} - \frac{S\epsilon}{\sigma_v}\right)\hat{h}(P-1, \epsilon)}$\\
%\hdashline
Log-Normal & $\frac{\frac{1}{\sigma_u\sigma_v}\int_0^\infty \frac{\exp{\left(u\right)}}{u} \phi\left(\frac{\ln{u}-\mu}{\sigma_u}\right)\phi\left(\frac{\epsilon_i+Su}{\sigma_v}\right)du}{\frac{1}{R}\sum_{r = 1}^R\frac{1}{\sigma_v}\phi\left(\frac{\epsilon_i+Su_{ir}}{\sigma_v}\right)}$\\
