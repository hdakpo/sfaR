...
SFA has been used in many different sectors including, e.g., agriculture \citep[e.g.,][]{latruffe23}, banking \citep[e.g.,][]{badu21}, 
health care \citep[e.g.,][]{ji20}, manufacturing \citep[e.g.,][]{boy19}, 
and museums \citep[e.g.,][]{price23}. 
...
is done using the \pgk{cubature} package \citep{cubature}). For the
...
estimation results are summarized in Table \ref{table:repliotherdist}.
...
texreg(list(ray, ge, ga2, we, lg, tsl), stars = c(0.01, 0.05, 0.1), 
       digits = 3, custom.model.names = c("rayleigh", "gexpo", "gamma", 
        "weibull", "lognorm", "tsl"), table = FALSE)
@
%\end{longtable}
\end{adjustbox}
\caption{Other distributions using data from Table 4.1 in \citet[p.~119]{kum14}}
