...
#' @noRd
pLCMhalfnormlike3C_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
psLCMfhalfnorm3C_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike3C_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  gradll <- cbind(sweep((0.5 * Xsig5 + Xsig3), MARGIN = 1, STATS = -(2 * (sigx5_1 *
    ...
    MARGIN = 1, STATS = 2 * (sigx17_1 * sigx5_1 * ewz1/(sigx7bis * (1 + ewz1 +
      ewz2))), FUN = "*"), 2 * (sigx5_1 * ewu1 * ewz1 * sigx21_1/sigx8), 2 *
    (sigx5_1 * ewu1 * ewz1 * sigx24_1/sigx8), sweep((0.5 * Xsig6 + Xsig4), MARGIN = 1,
    ...
    ewu2 * ewz2 * sigx21_2/sigx25), 2 * (sigx5_2 * ewu2 * ewz2 * sigx24_2/sigx25),
    sweep((0.5 * Xsig9 + Xsig8), MARGIN = 1, STATS = -(2 * (wzdeno * sigx5_3 *
      ...
      sigx5_3 * ewu3 * sigx21_3/sigx26), 2 * (wzdeno * sigx5_3 * ewu3 * sigx24_3/sigx26),
...
#' @noRd
LCM3ChnormAlgOpt_k90 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm3C_k90(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike3C_k90(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike3C_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, gr = function(parm) {
    -colSums(pgradLCMhalfnormlike3C_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike3C_k90,
    grad = pgradLCMhalfnormlike3C_k90, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike3C_k90(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike3C_k90(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike3C_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike3C_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike3C_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar,
    nZHvar = nZHvar)
  if (is.null(start)) {
...
#' @noRd
pLCM3Chalfnormeff_k90 <- function(object, level) {
