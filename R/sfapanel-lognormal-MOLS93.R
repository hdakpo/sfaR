...
#' @noRd
plognormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar, vHvar,
  muHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar, gHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
...
#' @noRd
pstlognorm_mols93 <- function(olsObj, epsiRes, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, ngZGvar, gHvar, Yvar, Xvar, S, wHvar, whichStart, printInfo,
  tol, N, FiMat, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradlognormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar,
  vHvar, muHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar, gHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  delta <- parm[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)]
  phi <- parm[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar)]
  ...
  sigFi5 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*") *
    (sweep(sweep(EqFi, MARGIN = 1, STATS = 2 * (gisq), FUN = "*"), MARGIN = 1,
      STATS = 2 * (S * giepsi), FUN = "+"))
  ZZ1 <- list()
  for (i in 1:ngZGvar) {
    ZZ1[[i]] <- sweep(EqFi, MARGIN = 1, STATS = Zigisq[, i], FUN = "*")
  }
  ZZ2 <- list()
  for (i in 1:ngZGvar) {
    ZZ2[[i]] <- sweep(ZZ1[[i]], MARGIN = 1, STATS = 2 * (S * Zigiepsi[, i]),
      FUN = "+")
  }
  ZZ3 <- list()
  for (i in 1:ngZGvar) {
    ZZ3[[i]] <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*") *
      ZZ2[[i]]
  }
  sigFi6 <- sapply(ZZ3, FUN = function(x) apply(x, 1, sum))
  ...
  for (k in 1:nXvar) {
    gx[, k] <- -apply(sweep(2 * (S * EqFi) * sigFi7, MARGIN = 1, STATS = Xgi[,
      k], FUN = "*") + sweep(2 * sigFi7, MARGIN = 1, STATS = Xepsi_i[, k],
      FUN = "*"), 1, sum)/sigDiv
  }
  gmu <- matrix(nrow = N, ncol = nmuZUvar)
  for (k in 1:nmuZUvar) {
    gmu[, k] <- apply(sweep(sigFi5, MARGIN = 1, STATS = -muHvar[, k], FUN = "*"),
      1, sum)/sigDiv
  }
  ...
  gradll <- cbind(gx, gmu, gu, gv - sweep(vHvar, MARGIN = 1, STATS = 0.5 * TT,
    FUN = "*"), sweep(-sigFi6, MARGIN = 1, STATS = 1/sigDiv, FUN = "*"))
...
#' @noRd
lognormAlgOpt_mols93 <- function(start, randStart, sdStart, olsParam, dataTable,
  S, gHvar, ngZGvar, nXvar, muHvar_c, muHvar_p, nmuZUvar, N, NT, FiMat_N, FiMat_NT,
  uHvar_c, uHvar_p, nuZUvar, vHvar_c, vHvar_p, nvZVvar, pindex, TT, Yvar, Xvar,
  wHvar_c, wHvar_p, method, printInfo, itermax, stepmax, tol, gradtol, hessianType,
  whichStart, initIter, initAlg, qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
    start_st <- pstlognorm_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      nmuZUvar = nmuZUvar, muHvar = muHvar_c, N = NT, FiMat = FiMat_NT, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, gHvar = gHvar, ngZGvar = ngZGvar,
      uHvar = uHvar_c, vHvar = vHvar_c, Yvar = Yvar, Xvar = Xvar, S = S, initIter = initIter,
      initAlg = initAlg, wHvar = wHvar_c, whichStart = whichStart, tol = tol,
      printInfo = printInfo)
  ...
  startLoglik <- sum(plognormlike_mols93(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(plognormlike_mols93(parm,
    ...
    TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_mols93(parm,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    N = N, FiMat = FiMat_N)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = plognormlike_mols93,
    grad = pgradlognormlike_mols93, start = startVal, finalHessian = if (hessianType ==
    ...
    wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(plognormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_mols93(parm,
      ...
      fn = function(parm) -sum(plognormlike_mols93(parm, nXvar = nXvar, gHvar = gHvar,
        ...
        FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_mols93(parm,
        ...
        muHvar = muHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradlognormlike_mols93(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
        muHvar = muHvar_p, N = N, FiMat = FiMat_N)), unname(parm)), "dgCMatrix"),
      method = "Sparse", control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
        ...
        preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal, fn = function(parm) -sum(plognormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_mols93(parm,
      ...
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), print.info = printInfo, maxiter = itermax,
      epsa = gradtol, epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(plognormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradlognormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), control = list(iter.max = itermax, trace = if (printInfo) 1 else 0,
      eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradlognormlike_mols93(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradlognormlike_mols93(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
        muHvar = muHvar_p, N = N, FiMat = FiMat_N)), mleObj$par)
    if (method == "sr1")
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradlognormlike_mols93(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
        muHvar = muHvar_p, N = N, FiMat = FiMat_N)), mleObj$solution)
  }
  mleObj$logL_OBS <- plognormlike_mols93(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradlognormlike_mols93(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffLogNorm_mols93 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq, giepsi,
  gisq) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffLogNorm_mols93 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq,
  giepsi, gisq, git) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffLogNorm_mols93 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq,
  giepsi, gisq, git) {
...
#' @noRd
plognormeff_mols93 <- function(object, level) {
  ...
  for (i in 1:object$Nid) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * giepsi[i] + ur^2 * gisq[i])/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffLogNorm_mols93, lowerLimit = 0, upperLimit = Inf,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in 1:object$Nobs) {
      res$teBC[i] <- hcubature(f = fnCondBCEffLogNorm_mols93, lowerLimit = 0,
      ...
      res$teBC_reciprocal[i] <- hcubature(f = fnCondBCreciprocalEffLogNorm_mols93,
