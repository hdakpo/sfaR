...
#' Multi-modal inefficiency stochastic frontier estimation using cross-sectional 
#' data
#'
#' @description
#' \code{\link{sfamisfcross}} is a symbolic formula-based function for the
...
#'
#' @aliases sfamisfcross bread.sfamisfcross estfun.sfamisfcross 
#' print.sfamisfcross
#'
...
#' @param x an object of class sfamisfcross (returned by the function 
#' \code{\link{sfamisfcross}}).
#' @param ... additional arguments of frontier are passed to sfamisfcross; 
...
#'
#' \deqn{f(\epsilon_i)=\frac{p_i}{\sqrt{\sigma_{u_1}^2 + 
...
#' where
#'
#' \deqn{\mu_{1i*}=\frac{\mu_1\sigma_v^2 - 
#' S\epsilon_i\sigma_{u_1}^2}{\sigma_{u_1}^2 + \sigma_v^2}}
#'
#' and
#'
#' \deqn{\sigma_{1*}^2 = \frac{\sigma_{u_1}^2 
...
#' 
#' \code{sfamisfcross} allows for the maximization of weighted log-likelihood.
...
#' objects of class \code{'sfamisfcross'} including methods to the generic 
#' functions \code{\link[=print.sfamisfcross]{print}},
#' \code{\link[=summary.sfamisfcross]{summary}}, 
#' \code{\link[=coef.sfamisfcross]{coef}}, 
#' \code{\link[=fitted.sfamisfcross]{fitted}}, 
#' \code{\link[=logLik.sfamisfcross]{logLik}}, 
#' \code{\link[=residuals.sfamisfcross]{residuals}}, 
#' \code{\link[=vcov.sfamisfcross]{vcov}}, 
#' \code{\link[=efficiencies.sfamisfcross]{efficiencies}}, 
#' \code{\link[=ic.sfamisfcross]{ic}}, 
#' \code{\link[=marginal.sfamisfcross]{marginal}},
#' \code{\link[=estfun.sfamisfcross]{estfun}} and 
#' \code{\link[=bread.sfamisfcross]{bread}} (from the \CRANpkg{sandwich} 
#' package), [lmtest::coeftest()] (from the \CRANpkg{lmtest} package).
#' 
#' @return \code{\link{sfamisfcross}} returns a list of class 
#' \code{'sfamisfcross'} containing the following elements:
#'
...
#'
#' @seealso \code{\link[=print.sfamisfcross]{print}} for printing 
#' \code{sfamisfcross} object.
#' 
#' \code{\link[=summary.sfamisfcross]{summary}} for creating and printing
#' summary results.
#'
#' \code{\link[=coef.sfamisfcross]{coef}} for extracting coefficients of the
#' estimation.
#'
#' \code{\link[=efficiencies.sfamisfcross]{efficiencies}} for computing
#' (in-)efficiency estimates.
#'
#' \code{\link[=fitted.sfamisfcross]{fitted}} for extracting the fitted frontier
#' values.
#'
#' \code{\link[=ic.sfamisfcross]{ic}} for extracting information criteria.
#'
#' \code{\link[=logLik.sfamisfcross]{logLik}} for extracting log-likelihood
#' value(s) of the estimation.
#'
#' \code{\link[=marginal.sfamisfcross]{marginal}} for computing marginal effects 
#' of inefficiency drivers.
#'
#' \code{\link[=residuals.sfamisfcross]{residuals}} for extracting residuals of 
#' the estimation.
#'
#' \code{\link[=vcov.sfamisfcross]{vcov}} for computing the variance-covariance
#' matrix of the coefficients.
#' 
#' \code{\link[=bread.sfamisfcross]{bread}} for bread for sandwich estimator.
#' 
#' \code{\link[=estfun.sfamisfcross]{estfun}} for gradient extraction for each 
#' observation.
#'
...
#' 
#' cb_misf_h <- sfamisfcross(formula = YIT ~ X1 + X2 + X3 + X4, udist = 'hnormal', 
#' data = dairyspain, S = 1, method = 'bfgs')
#' 
#' summary(cb_misf_h)
#' 
#' @export 
sfamisfcross <- function(formula, muhet, uhet, vhet, thet, logDepVar = TRUE, data,
  subset, weights, wscale = TRUE, S = 1L, udist = "hnormal", linkF = "logit", start = NULL,
  randStart = FALSE, whichStart = 2L, initAlg = "nm", initIter = 100, method = "bfgs",
  hessianType = 1, simType = "halton", Nsim = 100, prime = 2L, burn = 10, antithetics = FALSE,
  seed = 12345, itermax = 2000L, printInfo = FALSE, tol = 1e-12, gradtol = 1e-06,
  ...
  formula <- formDist_sfamisfcross(udist = udist, formula = formula, muhet = muhet,
  ...
  nParm <- if (udist %in% c("tnormal", "lognormal")) {
    ...
    if (udist %in% c("gamma", "weibull", "tslaplace")) {
      nXvar + 2 * nuZUvar + nvZVvar + nZHvar + 2
    } else {
  ...
  if (linkF == "logit") {
    mleList <- tryCatch(switch(udist, hnormal = do.call(misfhalfnormAlgOpt_logit,
      FunArgs), exponential = do.call(misfexponormAlgOpt_logit, FunArgs), tnormal = do.call(misftruncnormAlgOpt_logit,
      FunArgs), rayleigh = do.call(misfraynormAlgOpt_logit, FunArgs), gamma = do.call(misfgammanormAlgOpt_logit,
      FunArgs), uniform = do.call(misfuninormAlgOpt_logit, FunArgs), lognormal = do.call(misflognormAlgOpt_logit,
      FunArgs), weibull = do.call(misfweibullnormAlgOpt_logit, FunArgs), genexponential = do.call(misfgenexponormAlgOpt_logit,
      FunArgs), tslaplace = do.call(misftslnormAlgOpt_logit, FunArgs)), error = function(e) print(e))
  } else {
    if (linkF == "probit") {
      mleList <- tryCatch(switch(udist, hnormal = do.call(misfhalfnormAlgOpt_probit,
        FunArgs), exponential = do.call(misfexponormAlgOpt_probit, FunArgs),
        tnormal = do.call(misftruncnormAlgOpt_probit, FunArgs), rayleigh = do.call(misfraynormAlgOpt_probit,
          FunArgs), gamma = do.call(misfgammanormAlgOpt_probit, FunArgs),
        uniform = do.call(misfuninormAlgOpt_probit, FunArgs), lognormal = do.call(misflognormAlgOpt_probit,
          FunArgs), weibull = do.call(misfweibullnormAlgOpt_probit, FunArgs),
        genexponential = do.call(misfgenexponormAlgOpt_probit, FunArgs),
        tslaplace = do.call(misftslnormAlgOpt_probit, FunArgs)), error = function(e) print(e))
    } else {
      if (linkF == "cauchit") {
        mleList <- tryCatch(switch(udist, hnormal = do.call(misfhalfnormAlgOpt_cauchit,
          FunArgs), exponential = do.call(misfexponormAlgOpt_cauchit, FunArgs),
          tnormal = do.call(misftruncnormAlgOpt_cauchit, FunArgs), rayleigh = do.call(misfraynormAlgOpt_cauchit,
          FunArgs), gamma = do.call(misfgammanormAlgOpt_cauchit, FunArgs),
          uniform = do.call(misfuninormAlgOpt_cauchit, FunArgs), lognormal = do.call(misflognormAlgOpt_cauchit,
          FunArgs), weibull = do.call(misfweibullnormAlgOpt_cauchit, FunArgs),
          genexponential = do.call(misfgenexponormAlgOpt_cauchit, FunArgs),
          tslaplace = do.call(misftslnormAlgOpt_cauchit, FunArgs)), error = function(e) print(e))
      } else {
        if (linkF == "cloglog") {
          mleList <- tryCatch(switch(udist, hnormal = do.call(misfhalfnormAlgOpt_cloglog,
          FunArgs), exponential = do.call(misfexponormAlgOpt_cloglog, FunArgs),
          tnormal = do.call(misftruncnormAlgOpt_cloglog, FunArgs), rayleigh = do.call(misfraynormAlgOpt_cloglog,
            FunArgs), gamma = do.call(misfgammanormAlgOpt_cloglog, FunArgs),
          uniform = do.call(misfuninormAlgOpt_cloglog, FunArgs), lognormal = do.call(misflognormAlgOpt_cloglog,
            FunArgs), weibull = do.call(misfweibullnormAlgOpt_cloglog,
            FunArgs), genexponential = do.call(misfgenexponormAlgOpt_cloglog,
            FunArgs), tslaplace = do.call(misftslnormAlgOpt_cloglog, FunArgs)),
  ...
  # quick renaming -------
  if (udist %in% c("tnormal", "lognormal")) {
    names(mleList$startVal) <- fName_mu_sfamisfcross(Xvar = Xvar, muHvar = muHvar,
      uHvar = uHvar, vHvar = vHvar, Zvar = Zvar)
  } else {
    names(mleList$startVal) <- fName_uv_sfamisfcross(Xvar = Xvar, udist = udist,
      uHvar = uHvar, vHvar = vHvar, Zvar = Zvar)
  }
  ...
  rm(mleList)
  class(returnObj) <- "sfamisfcross"
  return(returnObj)
}

# print for sfamisfcross ----------
#' @rdname sfamisfcross
#' @exportS3Method print sfamisfcross
print.sfamisfcross <- function(x, ...) {
  cat("Call:\n")
...
#' @rdname sfamisfcross
#' @exportS3Method sandwich::bread sfamisfcross
bread.sfamisfcross <- function(x, ...) {
  if (x$hessianType == "Analytic/Numeric Hessian") {
    ...
    if (x$udist == "hnormal") {
      hessAnalytical <- eval(parse(text = paste0("chessmisfhalfnormlike_",
      ...
      if (x$udist == "exponential") {
        hessAnalytical <- eval(parse(text = paste0("chessmisfexponormlike_",
          x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
        ...
        if (x$udist == "tnormal") {
          hessAnalytical <- eval(parse(text = paste0("chessmisftruncnormlike_",
          x$linkF, "(parm = x$mlParam,
          ...
          if (x$udist == "rayleigh") {
          hessAnalytical <- eval(parse(text = paste0("chessmisfraynormlike_",
            x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          ...
          if (x$udist == "uniform") {
            hessAnalytical <- eval(parse(text = paste0("chessmisfuninormlike_",
            x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
            ...
            hessAnalytical <- eval(parse(text = paste0("chessmisfgenexponormlike_",
              x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
            ...
            if (x$udist == "tslaplace") {
              hessAnalytical <- eval(parse(text = paste0("chessmisftslnormlike_",
              x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
              ...
              hessAnalytical <- eval(parse(text = paste0("calculus::jacobian(function(parm) 
                      colSums(cgradmisfgammanormlike_",
                x$linkF, "(parm,
                nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
              ...
              if (x$udist == "weibull") {
                hessAnalytical <- eval(parse(text = paste0("calculus::jacobian(function(parm) 
                      colSums(cgradmisfweibullnormlike_",
                x$linkF, "(parm,
                nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
                nvZVvar = ncol(vHvar), uHvar = uHvar,
                vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
...
#' @rdname sfamisfcross
#' @exportS3Method sandwich::estfun sfamisfcross
estfun.sfamisfcross <- function(x, ...) {
  return(x$gradL_OBS)
}
