...
#' @noRd
ptslnormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
psttslnorm_gzit <- function(olsObj, epsiRes, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, ngZGvar, gHvar, S, wHvar,
  initIter, initAlg, modelType, whichStart, printInfo, tol) {
  if (whichStart == 1L) {
...
#' @param TT vector of time of presence
#' @noRd
pgradtslnormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar,
  gHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  ...
  sigx5 <- (0.5 * (dmusig1 * ewv/sqrt(ewv/gisq)) - 0.5 * (mustar1 *
    pmusig1))
  ...
  sigZ3 <- sweep(sigZ1, MARGIN = 1, STATS = 2 * (sigx16 * expo2),
    FUN = "*") - sweep(sigZ2, MARGIN = 1, STATS = sigx19 *
    expo1, FUN = "*")
  Xsig1 <- sweep(Xgi, MARGIN = 1, STATS = (S * mustar1/gisq),
    FUN = "*")
  Xsig2 <- sweep(Xgi, MARGIN = 1, STATS = (S * mustar2/gisq),
    FUN = "*")
  Xsig3 <- sweep(2 * Xepsi_i - 2 * Xsig1, MARGIN = 1, STATS = (pmusig1/ewv),
    FUN = "*")
  ...
  Xsig6 <- sweep(Xgi, MARGIN = 1, STATS = S * dmusig2/sigmastar,
    FUN = "*")
  Xsig7 <- sweep((0.5 * Xsig3 + Xsig5), MARGIN = 1, STATS = expo1,
    FUN = "*")
  Xsig8 <- sweep((0.5 * Xsig4 + Xsig6), MARGIN = 1, STATS = expo2,
    FUN = "*")
  gradll <- cbind(sweep((Xsig7 - 2 * (Xsig8)), MARGIN = 1,
    STATS = 1/sigx4, FUN = "*"), sweep(uHvar, MARGIN = 1,
    STATS = (sigx6/sigx7 - 0.5), FUN = "*"), sweep(vHvar,
    MARGIN = 1, STATS = (sigx15/sigx4 - 0.5 * (TT - 1)),
    FUN = "*"), 1/(1 + lambda) - (sigx3 * expo1/sigx7 + 2/(1 +
    2 * lambda)), sweep(sigZ3, MARGIN = 1, STATS = 1/sigx4,
...
#' @param TT vector of time of presence
#' @noRd
phesstslnormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar,
  gHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  ...
  sigZ5 <- sweep(Zigiepsi, MARGIN = 1, STATS = S * pmusig2,
    FUN = "*") - sweep(sigZ1, MARGIN = 1, STATS = dmusig2 *
    mustar2, FUN = "*")
  sigx45 <- (0.5 * (sigx17/sigmastar^2) - 0.5/(wvgi * gisq^2))
  sigZ6 <- sweep(Zigiepsi, MARGIN = 1, STATS = S * pmusig1,
    FUN = "*") - sweep(sigZ2, MARGIN = 1, STATS = mustar1 *
    dmusig1, FUN = "*")
  Xsig9 <- sweep((2 * Xepsi_i - 2 * Xsig1), MARGIN = 1, STATS = (sigx3/ewv),
    FUN = "*")
  Xsig10 <- sweep((Xsig7 - 2 * (Xsig8)), MARGIN = 1, STATS = sigx3 *
    ewu_h * gisq/sigx7^2, FUN = "*")
  Xsig11 <- sweep(Xgi, MARGIN = 1, STATS = S * (sigx22/wvgi +
    dmusig2 * mustar2/ewv)/gisq, FUN = "*")
  Xsig12 <- sweep((2 * Xepsi_i - 2 * Xsig2), MARGIN = 1, STATS = (sigx16/ewv),
    FUN = "*")
  ZZ1 <- list()
  for (i in 1:ngZGvar) {
    ZZ1[[i]] <- sweep(Xgi, MARGIN = 1, STATS = sigx17 * Zigisq[,
      i]/sigmastar^2, FUN = "*")
  }
  ZZ2 <- list()
  for (i in 1:ngZGvar) {
    ZZ2[[i]] <- sweep(Xzigi[[i]], MARGIN = 1, STATS = 1/sigmastar,
      FUN = "*")
  }
  Xsig15 <- sweep(Xgi, MARGIN = 1, STATS = S * (mustar1 * dmusig1/ewv +
    sigx23/wvgi)/gisq, FUN = "*")
  Xsig16 <- sweep((2 * Xepsi_i - 2 * Xsig1), MARGIN = 1, STATS = (sigx19/ewv),
    FUN = "*")
  ZZ3 <- list()
  for (i in 1:ngZGvar) {
    ZZ3[[i]] <- sweep((ZZ2[[i]] - ZZ1[[i]]), MARGIN = 1,
      STATS = S * sigx19, FUN = "*")
  }
  ...
  Xsig21 <- sweep(Xgi, MARGIN = 1, STATS = S * sigx9 * dmusig2/sigmastar,
    FUN = "*")
  Xsig22 <- sweep(Xgi, MARGIN = 1, STATS = S * (sigx8 * mustar2/(ewv *
    gisq) + 0.5/sigmastar^2) * dmusig2 * ewv/wvgi, FUN = "*")
  Xsig23 <- sweep((2 * Xepsi_i - 2 * Xsig2), MARGIN = 1, STATS = (sigx10/ewv),
    FUN = "*")
  Xsig24 <- sweep((Xsig7 - 2 * (Xsig8)), MARGIN = 1, STATS = sigx15/sigx4,
    FUN = "*")
  Xsig25 <- sweep(Xgi, MARGIN = 1, STATS = (S * ((1 + lambda)/wugi -
    mustar1/sigmastar^2)), FUN = "*")
  Xsig26 <- sweep(Xgi, MARGIN = 1, STATS = S * sigx35 * dmusig1/sigmastar,
    FUN = "*")
  Xsig27 <- sweep(Xgi, MARGIN = 1, STATS = S * (mustar1 * sigx13/(ewv *
    gisq) + 0.5/sigmastar^2) * dmusig1 * ewv/wvgi, FUN = "*")
  Xsig28 <- sweep((2 * Xepsi_i - 2 * Xsig1), MARGIN = 1, STATS = (sigx14/ewv),
    FUN = "*")
  Xsig29 <- sweep((2 * Xepsi_i - 2 * Xsig1), MARGIN = 1, STATS = (sigx5/ewv),
    FUN = "*")
  Xsig30 <- sweep(Xgi, MARGIN = 1, STATS = S * (0.5 * sigx24 +
    0.5 * sigx23), FUN = "*")
  Xsig31 <- sweep((2 * Xepsi_i - 2 * Xsig2), MARGIN = 1, STATS = (sigx2/ewv),
    FUN = "*")
  Xsig32 <- sweep(Xgi, MARGIN = 1, STATS = S * (0.5 * sigx27 +
    0.5 * sigx22), FUN = "*")
  Xsig33 <- sweep((Xsig7 - 2 * (Xsig8)), MARGIN = 1, STATS = sigx6 *
    ewu_h * gisq/sigx7^2, FUN = "*")
  Xsig34 <- sweep((0.5 * Xsig29 + Xsig30), MARGIN = 1, STATS = (1 +
    lambda) * expo1, FUN = "*")
  Xsig35 <- sweep((0.5 * Xsig31 + Xsig32), MARGIN = 1, STATS = (expo2),
    FUN = "*")
  Xsig36 <- sweep((Xsig34 - 2 * Xsig35), MARGIN = 1, STATS = 1/sigx7,
    FUN = "*")
  Xsig37 <- sweep((2 * Xsig19 + 2 * Xsig20), MARGIN = 1, STATS = pmusig2,
    FUN = "*")
  Xsig38 <- sweep((0.5 * (Xsig37 - Xsig21) + Xsig22 - 0.5 *
    Xsig23), MARGIN = 1, STATS = 2 * (expo2), FUN = "*")
  Xsig39 <- sweep((2 * Xsig25 + 2 * Xsig20), MARGIN = 1, STATS = pmusig1,
    FUN = "*")
  Xsig40 <- sweep((0.5 * (Xsig39 - Xsig26) + Xsig27 - 0.5 *
    Xsig28), MARGIN = 1, STATS = expo1, FUN = "*")
  Xsig41 <- sweep((Xsig38 - (Xsig24 + Xsig40)), MARGIN = 1,
    STATS = 1/sigx4, FUN = "*")
  Xsig42 <- sweep((Xsig18 - 0.5 * Xsig9), MARGIN = 1, STATS = 1/sigx7,
    FUN = "*")
  Xsig43 <- sweep((Xsig42 - Xsig10), MARGIN = 1, STATS = expo1,
    FUN = "*")
  ZZ4 <- list()
  for (i in 1:ngZGvar) {
    ZZ4[[i]] <- sweep((Xsig11 - 0.5 * Xsig12), MARGIN = 1,
      STATS = sigZ1[, i], FUN = "*")
  }
  ZZ5 <- list()
  for (i in 1:ngZGvar) {
    ZZ5[[i]] <- sweep((ZZ2[[i]] - ZZ1[[i]]), MARGIN = 1,
      STATS = S * sigx16, FUN = "*")
  }
  ZZ6 <- list()
  for (i in 1:ngZGvar) {
    ZZ6[[i]] <- sweep((ZZ4[[i]] + ZZ5[[i]]), MARGIN = 1,
      STATS = 2 * (expo2), FUN = "*")
  }
  ZZ7 <- list()
  for (i in 1:ngZGvar) {
    ZZ7[[i]] <- sweep((Xsig7 - 2 * (Xsig8)), MARGIN = 1,
      STATS = sigZ3[, i]/sigx4, FUN = "*")
  }
  ZZ8 <- list()
  for (i in 1:ngZGvar) {
    ZZ8[[i]] <- sweep((Xsig15 - 0.5 * Xsig16), MARGIN = 1,
      STATS = sigZ2[, i], FUN = "*")
  }
  ZZ9 <- list()
  for (i in 1:ngZGvar) {
    ZZ9[[i]] <- sweep((ZZ8[[i]] + ZZ3[[i]]), MARGIN = 1,
      STATS = expo1, FUN = "*")
  }
  ZZ10 <- list()
  for (i in 1:ngZGvar) {
    ZZ10[[i]] <- sweep((ZZ6[[i]] - (ZZ7[[i]] + ZZ9[[i]])),
      MARGIN = 1, STATS = wHvar/sigx4, FUN = "*")
  }
  hessll <- matrix(nrow = nXvar + nuZUvar + nvZVvar + ngZGvar +
    1, ncol = nXvar + nuZUvar + nvZVvar + ngZGvar + 1)
  hessll[1:nXvar, 1:nXvar] <- 0.5 * (sapply(1:nXvar, function(x) crossprod(2 *
    Xsq[[x]], as.matrix(wHvar * pmusig1 * expo1/ewv/sigx4))) -
    crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar * 2 *
      (S^2/gisq) * pmusig1 * expo1/ewv/sigx4, FUN = "*"),
      Xgi) - crossprod(sweep((2 * Xepsi_i - 2 * Xsig1),
    MARGIN = 1, STATS = wHvar * S * dmusig1/sigmastar * expo1/ewv/sigx4,
    FUN = "*"), Xgi)) - (0.5 * crossprod(sweep((0.5 * Xsig3 +
    Xsig5), MARGIN = 1, STATS = wHvar * expo1/ewv/sigx4,
    FUN = "*"), (2 * Xepsi_i - 2 * Xsig1)) + crossprod(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S^2 * mustar1 * dmusig1/(wvgi *
      gisq^2) * expo1/ewv/sigx4, FUN = "*"), Xgi)) - 2 *
    (0.5 * (sapply(1:nXvar, function(x) crossprod(2 * Xsq[[x]],
      as.matrix(wHvar * pmusig2 * expo2/ewv/sigx4))) -
      crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
        2 * (S^2/gisq) * pmusig2 * expo2/ewv/sigx4, FUN = "*"),
        Xgi) - crossprod(sweep((2 * Xepsi_i - 2 * Xsig2),
      MARGIN = 1, STATS = wHvar * S * dmusig2/sigmastar *
        expo2/ewv/sigx4, FUN = "*"), Xgi)) - (0.5 * crossprod(sweep((0.5 *
      Xsig4 + Xsig6), MARGIN = 1, STATS = wHvar * expo2/ewv/sigx4,
      FUN = "*"), (2 * Xepsi_i - 2 * Xsig2)) + crossprod(sweep(Xgi,
      MARGIN = 1, STATS = wHvar * S^2 * mustar2 * dmusig2/(wvgi *
        gisq^2) * expo2/ewv/sigx4, FUN = "*"), Xgi))) -
    crossprod(sweep(Xsig7 - 2 * (Xsig8), MARGIN = 1, STATS = wHvar/sigx4^2,
      FUN = "*"), Xsig7 - 2 * (Xsig8))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod((Xsig36 -
    Xsig33), sweep(uHvar, MARGIN = 1, STATS = wHvar, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(Xsig41, sweep(vHvar, MARGIN = 1,
    STATS = wHvar, FUN = "*"))
  hessll[1:nXvar, nXvar + nuZUvar + nvZVvar + 1] <- colSums(-(sweep(Xsig43,
    MARGIN = 1, STATS = wHvar, FUN = "*")))
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 2):(nXvar +
    nuZUvar + nvZVvar + ngZGvar + 1)] <- sapply(ZZ10, colSums)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((2 * (((0.25 * sigx27 - 0.5 * sigx29) * ewv - 0.5 *
      (sigx2 * mustar2/gisq)) * expo2) - ((0.25 * sigx24 -
      0.5 * sigx25) * ewv - 0.5 * (mustar1 * sigx5/gisq)) *
      (1 + lambda)^2 * expo1)/sigx28 - (sigx6/gisq + 0.5 *
      sigwu) * sigx6 * gisq/sigx7^2), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (2 * ((0.5 * (0.5 * (sigx9 *
      dmusig2 * ewv/sigx30) + 2 * ((sigx32 * mustar2 -
      0.5 * (sigx8 * ewv/ewu_h)) * pmusig2)) - (((0.25 *
      sigx33 + 0.5 * (sigx8 * mustar2/gisq))/ewu_h - 0.5 *
      sigx34) * dmusig2 * ewv/wvgi + 0.5 * (sigx10 * mustar2/wugi))) *
      expo2) - ((0.5 * (0.5 * (sigx35 * dmusig1 * ewv/sigx30) +
      2 * ((mustar1 * sigx32 - 0.5 * (sigx13 * ewv/ewu_h)) *
        pmusig1)) - (((0.25 * sigx33 + 0.5 * (mustar1 *
      sigx13/gisq))/ewu_h - 0.5 * sigx34) * dmusig1 * ewv/wvgi +
      0.5 * (mustar1 * sigx14/wugi))) * (1 + lambda) *
      expo1 + sigx15 * sigx6/sigx7))/sigx4, FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), nXvar + nuZUvar + nvZVvar +
    1] <- (wHvar * ((sigx3 * (sigx6/gisq + 0.5 * sigwu) *
    gisq/sigx7^2 + (0.5 * (sigx3 * mustar1/gisq) + 0.5 *
    (ewv * pmusig1)) * (1 + lambda)/sigx28) * expo1)) %*%
    uHvar
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 2):(nXvar + nuZUvar + nvZVvar + ngZGvar + 1)] <- 2 *
    (crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (sigx29 *
      ewv/wvgi - (0.5 * sigx16 + 0.5 * dmusig2) * mustar2)/gisq *
      expo2/sigwu, FUN = "*"), sigZ1) + crossprod(sweep(uHvar,
      MARGIN = 1, STATS = wHvar * 0.5 * (sigx16 * ewv *
        sigx17/sigmastar^2) * expo2/sigwu, FUN = "*"),
      Zigisq)) - (crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    (sigx25 * ewv/wvgi - mustar1 * (0.5 * sigx19 + 0.5 *
      dmusig1))/gisq * (1 + lambda) * expo1/sigwu, FUN = "*"),
    sigZ2) + crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    0.5 * (sigx19 * ewv * sigx17/sigmastar^2) * (1 + lambda) *
    expo1/sigwu, FUN = "*"), Zigisq) + crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * sigx6/(sigx4 * gisq)/sigwu,
    FUN = "*"), sigZ3))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (2 * ((0.5 * (sigx10 * sigx9) +
      0.5 * ((2 * ((sigx8/ewu_h - 0.5 * (sigx36 * mustar2/sigmastar^2)) *
        ewv) - epsilon_isq/ewv) * pmusig2 - sigx8 * sigx9 *
        dmusig2 * ewv/wvgi) - (1/wugi - ((sigx8^2 + 0.5/sigmastar^2) *
      mustar2 + 0.5 * (sigx36 * ewv/sigmastar^2) + 0.5 *
      sigx8)) * dmusig2 * ewv/wvgi) * expo2) - ((0.5 *
      (sigx14 * sigx35) + 0.5 * ((2 * ((sigx13 * (1 + lambda)/ewu_h -
      0.5 * (mustar1 * sigx37/sigmastar^2)) * ewv) - epsilon_isq/ewv) *
      pmusig1 - sigx13 * sigx35 * dmusig1 * ewv/wvgi) -
      ((1 + lambda)/wugi - ((sigx13^2 + 0.5/sigmastar^2) *
        mustar1 + 0.5 * (sigx37 * ewv/sigmastar^2) +
        0.5 * sigx13)) * dmusig1 * ewv/wvgi) * expo1 +
      sigx15^2/sigx4))/sigx4, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    nXvar + nuZUvar + nvZVvar + 1] <- (wHvar * (-((((lpwu -
    0.5 * (dmusig1/wvgi)) * ewv + 0.5 * (sigx3 * sigx35))/sigx7 -
    sigx3 * sigx15 * ewu_h * gisq/sigx7^2) * expo1))) %*%
    vHvar
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 2):(nXvar + nuZUvar + nvZVvar +
      ngZGvar + 1)] <- 2 * (crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * ((sigx38/gisq - 0.5 * (mustar2 * pmusig2/sigmastar^2)) *
      ewv/wvgi + sigx8 * dmusig2 * mustar2 + 0.5 * (sigx16 *
      sigx9)) * expo2/sigx4, FUN = "*"), sigZ1) - (crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (sigx42 * mustar2 + sigx17/ewu_h) *
      sigx16 * ewv/sigmastar^2 * expo2/sigx4, FUN = "*"),
    Zigisq) + crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    0.5 * sigx16 * ewv/sigmastar^2 * expo2/sigx4, FUN = "*"),
    sigZ4))) - (crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    ((sigx26/gisq - 0.5 * (mustar1 * pmusig1/sigmastar^2)) *
      ewv/wvgi + mustar1 * sigx13 * dmusig1 + 0.5 * (sigx19 *
      sigx35)) * expo1/sigx4, FUN = "*"), sigZ2) - (crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (sigx42 * mustar1 + (1 +
      lambda) * sigx17/ewu_h) * sigx19 * ewv/sigmastar^2 *
      expo1/sigx4, FUN = "*"), Zigisq) + crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * 0.5 * sigx19 * ewv/sigmastar^2 *
      expo1/sigx4, FUN = "*"), sigZ4)) + crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx15/sigx4/sigx4, FUN = "*"),
    sigZ3))
  hessll[nXvar + nuZUvar + nvZVvar + 1, nXvar + nuZUvar + nvZVvar +
    1] <- sum(wHvar * (4/(1 + 2 * lambda)^2 - (((sigx3 *
    mustar1/gisq + ewv * pmusig1)/sigx28 + sigx3^2 * expo1/sigx7^2) *
    expo1 + 1/(1 + lambda)^2)))
  hessll[nXvar + nuZUvar + nvZVvar + 1, (nXvar + nuZUvar +
    nvZVvar + 2):(nXvar + nuZUvar + nvZVvar + ngZGvar + 1)] <- colSums(sweep(sigZ3,
    MARGIN = 1, STATS = wHvar * sigx3/sigx4/gisq * expo1/sigwu,
    FUN = "*") - sweep(sigZ2, MARGIN = 1, STATS = wHvar *
    (mustar1^2 * pmusig1/gisq + ewv * sigx23)/wvgi/gisq *
    expo1/sigwu, FUN = "*") + sweep(Zigisq, MARGIN = 1, STATS = wHvar *
    sigx19 * ewv * sigx17/sigmastar^2 * expo1/sigwu, FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 2):(nXvar + nuZUvar +
    nvZVvar + ngZGvar + 1), (nXvar + nuZUvar + nvZVvar +
    2):(nXvar + nuZUvar + nvZVvar + ngZGvar + 1)] <- 2 *
    (crossprod(sweep(sigZ1, MARGIN = 1, STATS = wHvar * mustar2/(ewv *
      gisq) * mustar2 * pmusig2 * expo2/sigx4, FUN = "*"),
      sigZ1) - crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar *
      sigx17/sigmastar^2 * mustar2 * pmusig2 * expo2/sigx4,
      FUN = "*"), sigZ1) + crossprod(sweep(sigZ5, MARGIN = 1,
      STATS = wHvar/sigmastar * expo2/sigx4, FUN = "*"),
      sigZ1) + S * (sapply(1:ngZGvar, function(x) crossprod(Zisqgiepsi[[x]],
      as.matrix(wHvar * sigx16/sigmastar * expo2/sigx4))) -
      crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar *
        sigx16 * sigx17/sigmastar^2 * expo2/sigx4, FUN = "*"),
        Zigiepsi)) - (crossprod(sweep(Zigisq, MARGIN = 1,
      STATS = wHvar * sigx16 * sigx45 * ewv * mustar2/sigmastar^2 *
        expo2/sigx4, FUN = "*"), Zigisq) + crossprod(sweep(Zigiepsi,
      MARGIN = 1, STATS = wHvar * sigx16 * S * sigx17/sigmastar^2 *
        expo2/sigx4, FUN = "*"), Zigisq) + (sapply(1:ngZGvar,
      function(x) crossprod(Zisqgisq[[x]], as.matrix(wHvar *
        sigx16 * mustar2 * sigx17/sigmastar^2 * expo2/sigx4))) -
      crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar *
        sigx16 * 2 * (wvgi * sigx17 * gisq/sigmastar^2) *
        mustar2 * sigx17/sigmastar^2 * expo2/sigx4, FUN = "*"),
        Zigisq)))) - (crossprod(sweep(sigZ2, MARGIN = 1,
    STATS = wHvar * mustar1/(ewv * gisq) * mustar1 * pmusig1 *
      expo1/sigx4, FUN = "*"), sigZ2) - crossprod(sweep(Zigisq,
    MARGIN = 1, STATS = wHvar * sigx17/sigmastar^2 * mustar1 *
      pmusig1 * expo1/sigx4, FUN = "*"), sigZ2) + crossprod(sweep(sigZ6,
    MARGIN = 1, STATS = wHvar/sigmastar * expo1/sigx4, FUN = "*"),
    sigZ2) + S * (sapply(1:ngZGvar, function(x) crossprod(Zisqgiepsi[[x]],
    as.matrix(wHvar * sigx19 * expo1/sigmastar/sigx4))) -
    crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar * sigx19 *
      expo1 * sigx17/sigmastar^2/sigx4, FUN = "*"), Zigiepsi)) -
    (crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar *
      sigx19 * expo1 * mustar1 * sigx45 * ewv/sigmastar^2/sigx4,
      FUN = "*"), Zigisq) + crossprod(sweep(Zigiepsi, MARGIN = 1,
      STATS = wHvar * sigx19 * expo1 * S * sigx17/sigmastar^2/sigx4,
      FUN = "*"), Zigisq) + (sapply(1:ngZGvar, function(x) crossprod(Zisqgisq[[x]],
      as.matrix(wHvar * sigx19 * expo1 * mustar1 * sigx17/sigmastar^2/sigx4))) -
      crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar *
        sigx19 * expo1 * mustar1 * sigx17 * 2 * (wvgi *
        sigx17 * gisq/sigmastar^2/sigmastar^2/sigx4),
        FUN = "*"), Zigisq))) + crossprod(sweep(sigZ3,
    MARGIN = 1, STATS = wHvar/sigx4/sigx4, FUN = "*"), sigZ3)) -
    0.5 * (sapply(1:ngZGvar, function(x) crossprod(Zisqgisq[[x]],
      as.matrix(wHvar/gisq))) - crossprod(sweep(Zigisq,
      MARGIN = 1, STATS = wHvar/gisq/gisq, FUN = "*"),
      Zigisq))
...
#' @noRd
tslnormAlgOpt_gzit <- function(start, olsParam, dataTable, S,
  nXvar, uHvar_c, uHvar_p, nuZUvar, vHvar_c, vHvar_p, nvZVvar,
  gHvar, modelType, ngZGvar, Yvar, Xvar, pindex, TT, wHvar_c,
  wHvar_p, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
    start_st <- psttslnorm_gzit(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      ...
      initIter = initIter, initAlg = initAlg, gHvar = gHvar,
      whichStart = whichStart, tol = tol, printInfo = printInfo)
    InitTsl <- start_st$initTsl
    startVal <- start_st$StartVal
  }
  startLoglik <- sum(ptslnormlike_gzit(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptslnormlike_gzit,
    grad = pgradtslnormlike_gzit, hess = phesstslnormlike_gzit,
    ...
    fn = function(parm) {
      ...
      as(-phesstslnormlike_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar,
        ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    ...
    fn = function(parm) {
      ...
      -phesstslnormlike_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar,
        ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
    ...
    -colSums(pgradtslnormlike_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar,
      ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
      Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p))
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtslnormlike_gzit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstslnormlike_gzit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    ...
    if (method == "sr1") {
      mleObj$hessian <- phesstslnormlike_gzit(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- ptslnormlike_gzit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtslnormlike_gzit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p)
...
#' @noRd
ptslnormeff_gzit <- function(object, level) {
