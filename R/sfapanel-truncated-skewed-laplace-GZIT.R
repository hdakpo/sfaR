...
#' @noRd
ptslnormlike_bc92III <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
psttslnorm_bc92III <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradtslnormlike_bc92III <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar,
  gHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
...
#' @param TT vector of time of presence
#' @noRd
phesstslnormlike_bc92III <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar,
  gHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 2):(nXvar + nuZUvar +
    ...
    (crossprod(sweep(sigZ1, MARGIN = 1, STATS = wHvar * mustar2/(ewv *
      ...
      MARGIN = 1, STATS = wHvar * sigx16 * sigx17/sigmastar^2 *
        expo2/sigx4, FUN = "*"), Zigiepsi)) - (crossprod(sweep(Zigisq,
      MARGIN = 1, STATS = wHvar * sigx16 * sigx45 * ewv *
        mustar2/sigmastar^2 * expo2/sigx4, FUN = "*"),
      Zigisq) + crossprod(sweep(Zigiepsi, MARGIN = 1, STATS = wHvar *
      sigx16 * S * sigx17/sigmastar^2 * expo2/sigx4, FUN = "*"),
      Zigisq) + (sapply(1:ngZGvar, function(x) crossprod(Zisqgisq[[x]],
      as.matrix(wHvar * sigx16 * mustar2 * sigx17/sigmastar^2 *
        expo2/sigx4))) - crossprod(sweep(Zigisq, MARGIN = 1,
      STATS = wHvar * sigx16 * 2 * (wvgi * sigx17 * gisq/sigmastar^2) *
...
#' @noRd
tslnormAlgOpt_bc92III <- function(start, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psttslnorm_bc92III(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(ptslnormlike_bc92III(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal,
    fn = function(parm) {
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptslnormlike_bc92III,
    grad = pgradtslnormlike_bc92III, hess = phesstslnormlike_bc92III,
    ...
    fn = function(parm) {
      ...
      as(-phesstslnormlike_bc92III(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
    ...
    -phesstslnormlike_bc92III(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar,
      ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
      Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtslnormlike_bc92III(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstslnormlike_bc92III(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    ...
    if (method == "sr1") {
      mleObj$hessian <- phesstslnormlike_bc92III(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- ptslnormlike_bc92III(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtslnormlike_bc92III(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p)
...
#' @noRd
ptslnormeff_bc92III <- function(object, level) {
