...
#' @noRd
ptruncnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
psttruncnorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradtruncnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  nmuZUvar, uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S,
  wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  ...
  gradll <- cbind(-(sweep(Xgi, MARGIN = 1, STATS = S * sigx1/sigmasq,
    ...
    (dmu/(ewu_h * pmu) + mu/ewu)), FUN = "*"), sweep(uHvar,
    MARGIN = 1, STATS = -(((sigx5 * sigx6 + 0.5 * (gisq/sigmasq)) *
      ewu - (0.5 * ((mu)^2/ewu) + 0.5 * (mu * dmu/(ewu_h *
      pmu))))), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = ((sigx6 *
    sigx8 + 2 * (epsilon_isq/(2 * ewv)^2) - 0.5/sigmasq) *
    ewv - 0.5 * (TT - 1)), FUN = "*"), -(((mug * sigx3 *
    giZi/ssq + S * Zi_epsi/(sigmasq * sigmastar)) * sigx6 +
    0.5 * (giZi/sigmasq)) * ewu), -(((mug * sigx3 * giZisq/ssq +
    S * Zisq_epsi/(sigmasq * sigmastar)) * sigx6 + 0.5 *
    (giZisq/sigmasq)) * ewu))
...
#' @param TT vector of time of presence
#' @noRd
phesstruncnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  nmuZUvar, uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S,
  wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  ...
  sigx12 <- (mug * sigx3 * giZi/ssq + S * Zi_epsi/(sigmasq *
    sigmastar))
  sigx13 <- (mug * sigx3 * giZisq/ssq + S * Zisq_epsi/(sigmasq *
    sigmastar))
  sigx14 <- (ewv/sigmastar - sigx2 * dmusig/pmusig)
  sigx15 <- (1/(sigmasq * sigmastar) - sigx7 * ewv/ssq)
  sigx16 <- ((0.5 * (sigx3/ssq) - 0.5/(sigmasq^2 * sigmastar)) *
    ewu * ewv - 2 * (sigmasq * sigmastar * sigx3^2/ssq))
  sigx17 <- (((0.5 * sigx9 - 0.5 * sigx15) * ewu - 2 * (sigx7 *
    sigmasq * sigmastar * sigx3/ssq)) * mug + mu * sigx3)
  sigx18 <- (1 - sigx6 * dmusig/pmusig)
  sigx19 <- (sigx6 * dmusig/pmusig - 1)
  sigx20 <- (((0.5 * (wu2sq/(sigmasq * sigmastar)) - 0.5 *
    sigx11) * ewv - 2 * (sigx4 * sigmasq * sigmastar * sigx3/ssq)) *
    mug - S * sigx3 * giepsi)
  ...
  hessll[1:nXvar, 1:nXvar] <- -(crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * S^2 * ((mug/(ewv * pmusig * sigmastar) +
      dmusig * ewu/(pmusig * sigmastar)^2) * dmusig/sigmasq -
      1/ewv) * ewu/sigmasq, FUN = "*"), Xgi) + sapply(1:nXvar,
    function(x) crossprod(Xsq[[x]], as.matrix(wHvar/(2 *
      ewv)))))
  ...
  hessll[1:nXvar, (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar +
    ...
    (S * (sigx10 * sigx5/sigmasq + sigx6 * sigx11) * ewu),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + 1):(nXvar +
    ...
    sweep(Xgi, MARGIN = 1, STATS = wHvar * S * (sigx10 *
      sigx8/sigmasq + sigx6 * sigx7 * ewu/ssq) * ewv, FUN = "*"),
    vHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + nvZVvar + 1)] <- -colSums(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * sigx10 * sigx12/sigmasq *
      S * ewu, FUN = "*") + sweep(XiZi, MARGIN = 1, STATS = wHvar/(sigmasq *
    sigmastar) * sigx6 * S * ewu, FUN = "*") - sweep(Xgi,
    MARGIN = 1, STATS = wHvar * ewu * sigx3 * giZi/ssq *
      sigx6 * S * ewu, FUN = "*"))
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + nvZVvar + 2)] <- -colSums(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * sigx10 * sigx13/sigmasq *
      S * ewu, FUN = "*") + sweep(XiZisq, MARGIN = 1, STATS = wHvar/(sigmasq *
    sigmastar) * sigx6 * S * ewu, FUN = "*") - sweep(Xgi,
    MARGIN = 1, STATS = wHvar * ewu * sigx3 * giZisq/ssq *
      sigx6 * S * ewu, FUN = "*"))
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    ...
    MARGIN = 1, STATS = -wHvar * (((sigx5 * sigx14/sigmasq +
      sigx6 * sigx4 * ewv/ssq) * ewu - (0.5 * (((1 - mu^2/ewu_h^2)/(ewu_h *
      pmu) - mu * dmu/(ewu_h * pmu)^2) * dmu) + mu/ewu))),
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * (sigx6 * sigx15 + sigx14 *
      sigx8/sigmasq) * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    nuZUvar + nvZVvar + 1)] <- colSums(sweep(muHvar, MARGIN = 1,
    STATS = -wHvar * ((sigx12 * sigx14/sigmasq + sigx6 *
      ewv * sigx3 * giZi/ssq) * ewu), FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    nuZUvar + nvZVvar + 2)] <- colSums(sweep(muHvar, MARGIN = 1,
    STATS = -wHvar * ((sigx13 * sigx14/sigmasq + sigx6 *
      ewv * sigx3 * giZisq/ssq) * ewu), FUN = "*"))
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * -((((sigx19 * sigx5^2 - 0.5 *
      (gisq^2/sigmasq^2)) * ewu + ((sigx4 * (mu * ewv -
      (2 * (sigx4 * sigmasq * mug * sigmastar/ssq) + 3 *
        (S * giepsi)) * ewu) + (0.5 * (ewu * gisq/sigmasq) -
      0.5 * (0.5 * wu2sq + ewu * gisq/sigmasq)) * wu2sq *
      ewv * mug/sigmastar)/ssq + S * giepsi/(sigmasq *
      sigmastar)) * sigx6 + 0.5 * (gisq/sigmasq)) * ewu +
      0.5 * ((mu)^2/ewu) - 0.5 * (mu * (0.5 * (mu^2/(ewu_h^3 *
      pmu)) - (0.5 * (ewu_h * pmu) - 0.5 * (mu * dmu))/(ewu_h *
      pmu)^2) * dmu))), FUN = "*"), uHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
      nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = wHvar * (sigx19 * sigx5 * sigx8 + 0.5 * (gisq/sigmasq^2) -
      ((0.5 * (wu2sq * ewv/sigmasq) + 0.5 * ((ewu * gisq/sigmasq -
        1) * ewv/sigmasq + 1 - 0.5 * (wu2sq * (1 - ewv/sigmasq)))) *
        mug/sigmastar + mu * sigx4 - sigx7 * (2 * (sigx4 *
        sigmasq * mug * sigmastar/ssq) + S * giepsi)) *
        sigx6/ssq) * ewu * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * ((((sigx20 * giZi - S *
      sigx4 * Zi_epsi) * sigx6/ssq + sigx19 * sigx5 * sigx12 -
      0.5 * (gisq * giZi/sigmasq^2)) * ewu + sigx12 * sigx6 +
      0.5 * (giZi/sigmasq)) * ewu), FUN = "*"))
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * ((((sigx20 * giZisq - S *
      sigx4 * Zisq_epsi) * sigx6/ssq + sigx19 * sigx5 *
      sigx13 - 0.5 * (gisq * giZisq/sigmasq^2)) * ewu +
      sigx13 * sigx6 + 0.5 * (giZisq/sigmasq)) * ewu),
    FUN = "*"))
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * ((sigx18 * sigx8^2 + 0.5/sigmasq^2 -
      16 * (ewv * epsilon_isq/(2 * ewv)^4)) * ewv + sigx6 *
      (mu/(sigmasq * sigmastar) - (((3 * (mu) - 2 * (sigx7 *
        sigmasq * mug * sigmastar/ssq)) * ewv - S * ewu *
        giepsi) * sigx7 + (0.5 * (ewv/sigmasq) - 0.5 *
        (0.5 * (1 - ewv/sigmasq) + ewv/sigmasq)) * (1 -
        ewv/sigmasq) * ewu * mug/sigmastar)/ssq) + 2 *
      (epsilon_isq/(2 * ewv)^2) - 0.5/sigmasq) * ewv, FUN = "*"),
    vHvar)
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar + nvZVvar), (nXvar + nmuZUvar + nuZUvar + nvZVvar +
    1)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (((sigx17 * giZi - S * sigx7 * Zi_epsi) * sigx6/ssq +
      sigx12 * sigx18 * sigx8 - 0.5 * (giZi/sigmasq^2)) *
      ewu * ewv), FUN = "*"))
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar + nvZVvar), (nXvar + nmuZUvar + nuZUvar + nvZVvar +
    2)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (((sigx17 * giZisq - S * sigx7 * Zisq_epsi) * sigx6/ssq +
      sigx13 * sigx18 * sigx8 - 0.5 * (giZisq/sigmasq^2)) *
      ewu * ewv), FUN = "*"))
  hessll[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 1), (nXvar +
    nmuZUvar + nuZUvar + nvZVvar + 1)] <- sum(wHvar * (-((((sigx16 *
    mug * giZi - 2 * (S * sigx3 * Zi_epsi)) * ewu * giZi +
    mug * sigx3 * Zisq) * sigx6/ssq + sigx19 * sigx12^2 *
    ewu + 0.5 * ((Zisq - ewu * giZi^2/sigmasq)/sigmasq)) *
    ewu)))
  hessll[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 1), (nXvar +
    nmuZUvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar * (-(((((sigx16 *
    mug * giZi - S * sigx3 * Zi_epsi) * giZisq - S * sigx3 *
    giZi * Zisq_epsi) * ewu + mug * sigx3 * Zicub) * sigx6/ssq +
    sigx19 * sigx12 * sigx13 * ewu + 0.5 * ((Zicub - ewu *
    giZi * giZisq/sigmasq)/sigmasq)) * ewu)))
  hessll[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 2), (nXvar +
    nmuZUvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar * (-((((sigx16 *
    mug * giZisq - 2 * (S * sigx3 * Zisq_epsi)) * ewu * giZisq +
    mug * sigx3 * Zifour) * sigx6/ssq + sigx19 * sigx13^2 *
    ewu + 0.5 * ((Zifour - ewu * giZisq^2/sigmasq)/sigmasq)) *
    ewu)))
...
#' @noRd
truncnormAlgOpt_mbc92 <- function(start, olsParam, dataTable,
  S, nXvar, muHvar_c, muHvar_p, nmuZUvar, uHvar_c, uHvar_p,
  nuZUvar, vHvar_c, vHvar_p, nvZVvar, Yvar, Xvar, wHvar_c,
  wHvar_p, pindex, TT, method, printInfo, itermax, stepmax,
  tol, gradtol, whichStart, initIter, initAlg, hessianType,
  qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
  ...
  startLoglik <- sum(ptruncnormlike_mbc92(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptruncnormlike_mbc92,
    grad = pgradtruncnormlike_mbc92, hess = phesstruncnormlike_mbc92,
    ...
    fn = function(parm) {
      ...
      as(-phesstruncnormlike_mbc92(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar_p, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
    ...
    fn = function(parm) {
      ...
      -phesstruncnormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtruncnormlike_mbc92(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstruncnormlike_mbc92(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    ...
    if (method == "sr1") {
      mleObj$hessian <- phesstruncnormlike_mbc92(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- ptruncnormlike_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtruncnormlike_mbc92(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
