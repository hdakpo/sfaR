...
#' @noRd
pgammanormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, ngZGvar, gHvar, N, FiMat, wHvar) {
  beta <- parm[1:(nXvar)]
...
#' @noRd
pstgammanorm_gzit <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, ngZGvar, gHvar, S, printInfo, tol, N, FiMat, wHvar, modelType, whichStart,
  initIter, initAlg) {
  if (whichStart == 1L) {
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar)), paste0("Zv_",
    colnames(vHvar)), "P", if (modelType %in% c("bc92a", "kw05")) {
    "eta"
  } else {
    if (modelType == "bc92b") {
      c("eta1", "eta2")
...
#' @param FiMat matrix of random draws
#' @noRd
pgradgammanormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, ngZGvar, gHvar, N, FiMat, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  P <- parm[nXvar + nuZUvar + nvZVvar + 1]
  ...
  sigZ1 <- sweep(Zigisq, MARGIN = 1, STATS = 0.5 * (epsi_uv/sqrt(gisq))/sigsta,
    FUN = "*") + sweep(Zigiepsi, MARGIN = 1, STATS = S * sqrt(gisq)/sigsta, FUN = "*") -
    sweep(Zigisq, MARGIN = 1, STATS = epsi_uv * ewv_h * sqrt(gisq)/sigsta^2,
      FUN = "*")
  sigZ2 <- sweep(Zigiepsi, MARGIN = 1, STATS = S/gisq, FUN = "*") - sweep(Zigisq,
    MARGIN = 1, STATS = epsi_uv/gisq/gisq, FUN = "*")
  ...
  ZZ1 <- list()
  for (i in 1:ngZGvar) {
    ZZ1[[i]] <- sweep(sigFi5, MARGIN = 1, STATS = sigZ1[, i] * ewv_h/sqrt(gisq),
      FUN = "*") - sweep(sigFi1, MARGIN = 1, STATS = 0.5 * (Zigisq[, i]/gisq) *
      ewv_h/sqrt(gisq), FUN = "*")
  }
  ZZ2 <- list()
  for (i in 1:ngZGvar) {
    ZZ2[[i]] <- sweep(ZZ1[[i]], MARGIN = 1, STATS = sigZ2[, i], FUN = "-")
  }
  ZZ3 <- list()
  for (i in 1:ngZGvar) {
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * sigx4/sigsta, FUN = "*") +
    ...
    (Wu) + digamma(P)), sweep(sigZ1, MARGIN = 1, STATS = sigx8, FUN = "*") +
    sweep(sigFi11, MARGIN = 1, STATS = 1/sdDiv, FUN = "*") - 0.5 * (Zigisq/gisq))
...
#' @noRd
gammanormAlgOpt_gzit <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- pstgammanorm_gzit(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      N = NT, FiMat = FiMat_NT, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      gHvar = gHvar, ngZGvar = ngZGvar, uHvar = uHvar_c, vHvar = vHvar_c, Yvar = Yvar,
      Xvar = Xvar, S = S, modelType = modelType, wHvar = wHvar_c, whichStart = whichStart,
      initIter = initIter, initAlg = initAlg, tol = tol, printInfo = printInfo)
    initGamma <- start_st$initGamma
    startVal <- start_st$StartVal
  }
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(pgammanormlike_gzit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pgammanormlike_gzit(parm,
    ...
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_gzit(parm,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgammanormlike_gzit,
    grad = pgradgammanormlike_gzit, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) -sum(pgammanormlike_gzit(parm, nXvar = nXvar, gHvar = gHvar,
      ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pgammanormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradgammanormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(pgammanormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradgammanormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgammanormlike_gzit(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradgammanormlike_gzit(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
  ...
  mleObj$logL_OBS <- pgammanormlike_gzit(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradgammanormlike_gzit(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
pgammanormeff_gzit <- function(object, level) {
