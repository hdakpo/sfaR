...
ccnsfraynormlike_logit <- function(parm, nXvar, nuZUvar, nvZVvar,
...
ccnsfraynormlike_cauchit <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
ccnsfraynormlike_probit <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
ccnsfraynormlike_cloglog <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## logit specification class membership
cmcesfraynormlike_logit <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
...
## cauchit specification class membership
cmcesfraynormlike_cauchit <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
...
## probit specification class membership
cmcesfraynormlike_probit <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
...
## cloglog specification class membership
cmcesfraynormlike_cloglog <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
...
## logit specification class membership
cgradcnsfraynormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## cauchit specification class membership
cgradcnsfraynormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx4_1 <- (ewu * sigx3_1/sigma_sq1 + S * usq1 * sigx2_1 *
    (epsilon)/ewv1)
  sigx4_2 <- (ewu * sigx3_2/sigma_sq2 + S * usq2 * sigx2_2 *
    (epsilon)/ewv2)
  ...
  sigx7_1 <- (dpsv1 * usq1 + S^2 * sigx6_1 * sigx2_1 * ewu *
    (epsilon)^2/sigmastar1)
  sigx7_2 <- (dpsv2 * usq2 + S^2 * sigx6_2 * sigx2_2 * ewu *
    (epsilon)^2/sigmastar2)
  ...
  sigx18 <- (sigx2_1 * sigx1_1 * sigmastar1/ewvsr1 - sigx2_2 *
    sigx1_2 * sigmastar2/ewvsr2)
  sigx19 <- (ewz2 * sigx1_2 * sigx4_2 * sigmastar2/ewvsr2 +
    ewz1 * sigx1_1 * sigx4_1 * sigmastar1/ewvsr1)
  sigx20 <- (ewz2 * sigx2_2 * sigx1_2 * sigmastar2/ewvsr2 +
    ewz1 * sigx2_1 * sigx1_1 * sigmastar1/ewvsr1)
  sigx21 <- (sigx9_1 * ewz1 * sigx1_1/(sigma_sq1 * ewvsr1) +
    sigx9_2 * ewz2 * sigx1_2/(sigma_sq2 * ewvsr2))
  sigx22 <- (pi * sigx20 * ((Wz)^2 + 1))
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx19/sigx20,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = (sigx21 *
    ewu/sigx20 - 1), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = sigx17_1 * ewz1 * sigx1_1/(sigx20 * ewvsr1),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx17_2 *
    ewz2 * sigx1_2/(sigx20 * ewvsr2), FUN = "*"), sweep(Zvar,
    MARGIN = 1, STATS = sigx18/sigx22, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

## probit specification class membership
cgradcnsfraynormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx18 <- (sigx2_1 * sigx1_1 * sigmastar1/ewvsr1 - sigx2_2 *
    sigx1_2 * sigmastar2/ewvsr2)
  sigx19 <- ((1 - pwZ) * sigx1_2 * sigx4_2 * sigmastar2/ewvsr2 +
    sigx1_1 * sigx4_1 * pwZ * sigmastar1/ewvsr1)
  sigx20 <- ((1 - pwZ) * sigx2_2 * sigx1_2 * sigmastar2/ewvsr2 +
    sigx2_1 * sigx1_1 * pwZ * sigmastar1/ewvsr1)
  sigx21 <- (sigx9_1 * sigx1_1 * pwZ/(sigma_sq1 * ewvsr1) +
    sigx9_2 * (1 - pwZ) * sigx1_2/(sigma_sq2 * ewvsr2))
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx19/sigx20,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = (sigx21 *
    ewu/sigx20 - 1), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = sigx17_1 * pwZ * sigx1_1/(sigx20 * ewvsr1), FUN = "*"),
    sweep(vHvar, MARGIN = 1, STATS = sigx17_2 * (1 - pwZ) *
      sigx1_2/(sigx20 * ewvsr2), FUN = "*"), sweep(Zvar,
      MARGIN = 1, STATS = sigx18 * dwZ/sigx20, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

## cloglog specification class membership
cgradcnsfraynormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx18 <- (sigx2_1 * sigx1_1 * sigmastar1/ewvsr1 - sigx2_2 *
    sigx1_2 * sigmastar2/ewvsr2)
  sigx19 <- ((1 - prZ) * sigx1_1 * sigx4_1 * sigmastar1/ewvsr1 +
    prZ * sigx1_2 * sigx4_2 * sigmastar2/ewvsr2)
  sigx20 <- ((1 - prZ) * sigx2_1 * sigx1_1 * sigmastar1/ewvsr1 +
    sigx2_2 * prZ * sigx1_2 * sigmastar2/ewvsr2)
  sigx21 <- (sigx9_1 * (1 - prZ) * sigx1_1/(sigma_sq1 * ewvsr1) +
    sigx9_2 * prZ * sigx1_2/(sigma_sq2 * ewvsr2))
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx19/sigx20,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = (sigx21 *
    ewu/sigx20 - 1), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = sigx17_1 * (1 - prZ) * sigx1_1/(sigx20 * ewvsr1),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx17_2 *
    prZ * sigx1_2/(sigx20 * ewvsr2), FUN = "*"), sweep(Zvar,
    MARGIN = 1, STATS = sigx18 * prZ * ewz/sigx20, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}
...
## logit specification class membership
cgradmcesfraynormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
...
## cauchit specification class membership
cgradmcesfraynormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
  ...
  sigx18 <- (sigx2_1 * sigx1_1 * sigmastar1/(ewu1 * ewvsr1) -
    sigx2_2 * sigx1_2 * sigmastar2/(ewu2 * ewvsr2))
  sigx19 <- (ewz2 * sigx1_2 * sigx4_2 * sigmastar2/(ewu2 *
    ewvsr2) + ewz1 * sigx1_1 * sigx4_1 * sigmastar1/(ewu1 *
    ewvsr1))
  sigx20 <- (ewz2 * sigx2_2 * sigx1_2 * sigmastar2/(ewu2 *
    ewvsr2) + ewz1 * sigx2_1 * sigx1_1 * sigmastar1/(ewu1 *
    ewvsr1))
  sigx21_1 <- (sigx9_1/(sigma_sq1 * ewvsr1) - sigx2_1 * ewu1 *
    ewvsr1 * sigmastar1/(ewu1 * ewvsr1)^2)
  sigx21_2 <- (sigx9_2/(sigma_sq2 * ewvsr2) - sigx2_2 * ewu2 *
    ewvsr2 * sigmastar2/(ewu2 * ewvsr2)^2)
  sigx22 <- (pi * sigx20 * ((Wz)^2 + 1))
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx19/sigx20,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = sigx21_1 *
    ewz1 * sigx1_1/sigx20, FUN = "*"), sweep(uHvar, MARGIN = 1,
    STATS = sigx21_2 * ewz2 * sigx1_2/sigx20, FUN = "*"),
    sweep(vHvar, MARGIN = 1, STATS = sigx17_1 * ewz1 * sigx1_1/sigx20,
      FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx17_2 *
      ewz2 * sigx1_2/sigx20, FUN = "*"), sweep(Zvar, MARGIN = 1,
      STATS = sigx18/sigx22, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

## probit specification class membership
cgradmcesfraynormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
  ...
  sigx18 <- (sigx2_1 * sigx1_1 * sigmastar1/(ewu1 * ewvsr1) -
    sigx2_2 * sigx1_2 * sigmastar2/(ewu2 * ewvsr2))
  sigx19 <- ((1 - pwZ) * sigx1_2 * sigx4_2 * sigmastar2/(ewu2 *
    ewvsr2) + sigx1_1 * sigx4_1 * pwZ * sigmastar1/(ewu1 *
    ewvsr1))
  sigx20 <- ((1 - pwZ) * sigx2_2 * sigx1_2 * sigmastar2/(ewu2 *
    ewvsr2) + sigx2_1 * sigx1_1 * pwZ * sigmastar1/(ewu1 *
    ewvsr1))
  sigx21_1 <- (sigx9_1/(sigma_sq1 * ewvsr1) - sigx2_1 * ewu1 *
    ewvsr1 * sigmastar1/(ewu1 * ewvsr1)^2)
  sigx21_2 <- (sigx9_2/(sigma_sq2 * ewvsr2) - sigx2_2 * ewu2 *
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx19/sigx20,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = sigx21_1 *
    pwZ * sigx1_1/sigx20, FUN = "*"), sweep(uHvar, MARGIN = 1,
    STATS = sigx21_2 * (1 - pwZ) * sigx1_2/sigx20, FUN = "*"),
    sweep(vHvar, MARGIN = 1, STATS = sigx17_1 * pwZ * sigx1_1/sigx20,
      FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx17_2 *
      (1 - pwZ) * sigx1_2/sigx20, FUN = "*"), sweep(Zvar,
      MARGIN = 1, STATS = sigx18 * dwZ/sigx20, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

## cloglog specification class membership
cgradmcesfraynormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
  ...
  sigx18 <- (sigx2_1 * sigx1_1 * sigmastar1/(ewu1 * ewvsr1) -
    sigx2_2 * sigx1_2 * sigmastar2/(ewu2 * ewvsr2))
  sigx19 <- ((1 - prZ) * sigx1_1 * sigx4_1 * sigmastar1/(ewu1 *
    ewvsr1) + prZ * sigx1_2 * sigx4_2 * sigmastar2/(ewu2 *
    ewvsr2))
  sigx20 <- ((1 - prZ) * sigx2_1 * sigx1_1 * sigmastar1/(ewu1 *
    ewvsr1) + sigx2_2 * prZ * sigx1_2 * sigmastar2/(ewu2 *
    ewvsr2))
  sigx21_1 <- (sigx9_1/(sigma_sq1 * ewvsr1) - sigx2_1 * ewu1 *
    ewvsr1 * sigmastar1/(ewu1 * ewvsr1)^2)
  sigx21_2 <- (sigx9_2/(sigma_sq2 * ewvsr2) - sigx2_2 * ewu2 *
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx19/sigx20,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = sigx21_1 *
    (1 - prZ) * sigx1_1/sigx20, FUN = "*"), sweep(uHvar,
    MARGIN = 1, STATS = sigx21_2 * prZ * sigx1_2/sigx20,
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx17_1 *
    (1 - prZ) * sigx1_1/sigx20, FUN = "*"), sweep(vHvar,
    MARGIN = 1, STATS = sigx17_2 * prZ * sigx1_2/sigx20,
    FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx18 *
    prZ * ewz/sigx20, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}
...
## logit specification class membership
chesscnsfraynormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## cauchit specification class membership
chesscnsfraynormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll <- matrix(nrow = nXvar + nuZUvar + 2 * nvZVvar + nZHvar,
    ncol = nXvar + nuZUvar + 2 * nvZVvar + nZHvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((((S^2 * ewu * (epsilon)^2/(sigma_sq1 *
      ewv1) - 1) * wuv1 + ewu/starsq1) * dmusig1 * ewu/sigma_sq1 +
      usq1 * (S * ((2 * (S * dmusig1 * wuv1 * (epsilon)) +
        3 * pmusig1) * ewu/sigma_sq1 + S * usq1 * sigx2_1 *
        (epsilon)/ewv1) * (epsilon) - dmusig1 * sigmastar1)/ewv1) *
      ewz1 * sigx1_1 * sigmastar1/ewvsr1 + (((S^2 * ewu *
      (epsilon)^2/(sigma_sq2 * ewv2) - 1) * wuv2 + ewu/starsq2) *
      dmusig2 * ewu/sigma_sq2 + usq2 * (S * ((2 * (S *
      dmusig2 * wuv2 * (epsilon)) + 3 * pmusig2) * ewu/sigma_sq2 +
      S * usq2 * sigx2_2 * (epsilon)/ewv2) * (epsilon) -
      dmusig2 * sigmastar2)/ewv2) * ewz2 * sigx1_2 * sigmastar2/ewvsr2 -
      sigx19^2/sigx20)/sigx20, FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (((usq1 * (pmusig1 -
      0.5 * (S * dmusig1 * ewu * (epsilon)/starsq1)) +
      S * sigx6_1 * ewu * (S * ewu * sigx3_1 * (epsilon)/sigma_sq1 -
        2 * sigx2_1) * (epsilon)/sigmastar1) * sigmastar1 +
      (0.5 * (ewu * ewv1 * sigx3_1/starsq1) + S * sigx9_1 *
        (epsilon)/ewv1) * usq1) * ewz1 * sigx1_1/(sigma_sq1 *
      ewvsr1) + ((usq2 * (pmusig2 - 0.5 * (S * dmusig2 *
      ewu * (epsilon)/starsq2)) + S * sigx6_2 * ewu * (S *
      ewu * sigx3_2 * (epsilon)/sigma_sq2 - 2 * sigx2_2) *
      (epsilon)/sigmastar2) * sigmastar2 + (0.5 * (ewu *
      ewv2 * sigx3_2/starsq2) + S * sigx9_2 * (epsilon)/ewv2) *
      usq2) * ewz2 * sigx1_2/(sigma_sq2 * ewvsr2) - sigx21 *
      sigx19/sigx20) * ewu/sigx20, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (((((sigx14_1 * sigx3_1 + (S * (0.5 * (vsq1/ewv1) +
    1/sigma_sq1) * dmusig1 * ewu * (epsilon)/sigmastar1 -
    pmusig1)/sigma_sq1) * ewu/sigma_sq1 + S * (2 * (sigx12_1 *
    ewu^2/sigx13_1) - 4/(2 * ewv1)^2) * sigx2_1 * (epsilon)) *
    sigmastar1 + 0.5 * (vsq1 * ewu^2 * sigx3_1/(sigma_sq1^2 *
    sigmastar1))) * ewv1 + S * sigx17_1 * usq1 * (epsilon)/ewv1 -
    0.5 * (ewu * sigx3_1 * sigmastar1/sigma_sq1))/(sigx20 *
    ewvsr1) - sigx17_1 * sigx19 * ewvsr1/(sigx20 * ewvsr1)^2) *
    ewz1 * sigx1_1, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * (((((sigx14_2 * sigx3_2 + (S * (0.5 *
      (vsq2/ewv2) + 1/sigma_sq2) * dmusig2 * ewu * (epsilon)/sigmastar2 -
      pmusig2)/sigma_sq2) * ewu/sigma_sq2 + S * (2 * (sigx12_2 *
      ewu^2/sigx13_2) - 4/(2 * ewv2)^2) * sigx2_2 * (epsilon)) *
      sigmastar2 + 0.5 * (vsq2 * ewu^2 * sigx3_2/(sigma_sq2^2 *
      sigmastar2))) * ewv2 + S * sigx17_2 * usq2 * (epsilon)/ewv2 -
      0.5 * (ewu * sigx3_2 * sigmastar2/sigma_sq2))/(sigx20 *
      ewvsr2) - sigx17_2 * sigx19 * ewvsr2/(sigx20 * ewvsr2)^2) *
      ewz2 * sigx1_2, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * ((sigx1_1 * sigx4_1 *
      sigmastar1/ewvsr1 - sigx1_2 * sigx4_2 * sigmastar2/ewvsr2)/sigx22 -
      pi * sigx19 * sigx18 * ((Wz)^2 + 1)/sigx22^2), FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    (((((0.5 * ((dpsv1 * usq1 + S * ewu * pmusig1 * (epsilon)/sigma_sq1 -
      dmusig1 * sigmastar1) * ewu/sigma_sq1 - 0.5 * (usq1 *
      sigx2_1)) + 0.5 * (sigx7_1 * ewu/sigma_sq1)) * usq1 *
      ewv1 + S^2 * sigx9_1 * sigx6_1 * ewu^2 * (epsilon)^2/sigma_sq1)/sigmastar1 +
      (usq1 * (ewu * (S^2 * sigx6_1 * dmusig1 * (epsilon)^2 -
        dpsv1/sigma_sq1) - 0.5 * (0.5 * (usq1 * dmusig1 *
        ewv1/sigmastar1) + S^2 * sigx6_1 * dmusig1 *
        ewu * (epsilon)^2)) + S^2 * ((dpsv1 * usq1 *
        sigx6_1/sigma_sq1 - ((0.5 * (ewu/sigma_sq1) +
        1 - 0.5 * (0.5 * usq1 + ewu/sigma_sq1)) * usq1 *
        ewv1/sigmastar1 + (2 - 2 * (sigx5_1^2 * ewu *
        sigma_sq1/starsq1^2)) * sigmastar1) * sigx2_1/starsq1^2) *
        ewu + (1 - 0.5 * usq1) * sigx6_1 * sigx2_1) *
        ewu * (epsilon)^2/sigmastar1) * sigmastar1)/(sigma_sq1 *
      ewvsr1) + sigx9_1 * (1/(sigma_sq1 * ewvsr1) - ewu *
      ewvsr1/(sigma_sq1 * ewvsr1)^2)) * ewz1 * sigx1_1 +
      ((((0.5 * ((dpsv2 * usq2 + S * ewu * pmusig2 * (epsilon)/sigma_sq2 -
        dmusig2 * sigmastar2) * ewu/sigma_sq2 - 0.5 *
        (usq2 * sigx2_2)) + 0.5 * (sigx7_2 * ewu/sigma_sq2)) *
        usq2 * ewv2 + S^2 * sigx9_2 * sigx6_2 * ewu^2 *
        (epsilon)^2/sigma_sq2)/sigmastar2 + (usq2 * (ewu *
        (S^2 * sigx6_2 * dmusig2 * (epsilon)^2 - dpsv2/sigma_sq2) -
        0.5 * (0.5 * (usq2 * dmusig2 * ewv2/sigmastar2) +
          S^2 * sigx6_2 * dmusig2 * ewu * (epsilon)^2)) +
        S^2 * ((dpsv2 * usq2 * sigx6_2/sigma_sq2 - ((0.5 *
          (ewu/sigma_sq2) + 1 - 0.5 * (0.5 * usq2 + ewu/sigma_sq2)) *
          usq2 * ewv2/sigmastar2 + (2 - 2 * (sigx5_2^2 *
          ewu * sigma_sq2/starsq2^2)) * sigmastar2) *
          sigx2_2/starsq2^2) * ewu + (1 - 0.5 * usq2) *
          sigx6_2 * sigx2_2) * ewu * (epsilon)^2/sigmastar2) *
        sigmastar2)/(sigma_sq2 * ewvsr2) + sigx9_2 *
        (1/(sigma_sq2 * ewvsr2) - ewu * ewvsr2/(sigma_sq2 *
          ewvsr2)^2)) * ewz2 * sigx1_2 - sigx21^2 * ewu/sigx20) *
    ewu/sigx20, FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq1 * dmusig1) +
      0.5 * ((dmusig1 * ewv1/sigma_sq1 - S^2 * vsq1 * sigx6_1 *
        dmusig1 * ewu * (epsilon)^2/sigmastar1) * ewu/sigma_sq1 -
        0.5 * (usq1 * vsq1 * dmusig1)))/sigmastar1 +
      dpsv1 * usq1 * sigx14_1 + (S * (pmusig1 - ewu * (pmusig1/sigma_sq1 +
      S * sigx6_1 * dmusig1 * (epsilon))) * (epsilon) -
      sigx11_1 * ewu)/sigma_sq1)/sigma_sq1 - S^2 * (((0.5 *
      (usq1 * ewv1/sigma_sq1) + 0.5 * ((ewu/sigma_sq1 -
      1) * ewv1/sigma_sq1 + 1 - 0.5 * (usq1 * vsq1))) *
      ewu/sigmastar1 + 2 * sigx12_1)/sigx13_1 - ((starsq1^2 +
      2 * (sigx5_1 * sigma_sq1^2 * sigmastar1)) * sigmastar1 +
      0.5 * (starsq1^2 * usq1 * ewv1/sigmastar1)) * sigx12_1 *
      ewu/sigx13_1^2) * sigx2_1 * ewu * (epsilon)^2) *
      sigmastar1 + 0.5 * (((dpsv1 * usq1 * vsq1 + sigx2_1 *
      ewv1/sigma_sq1) * ewu/sigma_sq1 + vsq1 * sigx2_1)/starsq1 -
      sigx5_1 * vsq1 * sigx2_1 * ewu/starsq1^2) + 0.5 *
      (sigx15_1 * usq1 * ewv1/starsq1)) * ewv1 + (S^2 *
      sigx17_1 * sigx6_1 * ewu * (epsilon)^2/sigmastar1 -
      0.5 * ((dpsv1 * sigmastar1 + 0.5 * (sigx2_1 * ewv1/sigmastar1)) *
        usq1))/sigma_sq1)/(sigx20 * ewvsr1) - sigx17_1 *
      sigx21 * ewvsr1/(sigx20 * ewvsr1)^2) * ewz1 * sigx1_1 *
      ewu, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq2 * dmusig2) +
      0.5 * ((dmusig2 * ewv2/sigma_sq2 - S^2 * vsq2 * sigx6_2 *
        dmusig2 * ewu * (epsilon)^2/sigmastar2) * ewu/sigma_sq2 -
        0.5 * (usq2 * vsq2 * dmusig2)))/sigmastar2 +
      dpsv2 * usq2 * sigx14_2 + (S * (pmusig2 - ewu * (pmusig2/sigma_sq2 +
      S * sigx6_2 * dmusig2 * (epsilon))) * (epsilon) -
      sigx11_2 * ewu)/sigma_sq2)/sigma_sq2 - S^2 * (((0.5 *
      (usq2 * ewv2/sigma_sq2) + 0.5 * ((ewu/sigma_sq2 -
      1) * ewv2/sigma_sq2 + 1 - 0.5 * (usq2 * vsq2))) *
      ewu/sigmastar2 + 2 * sigx12_2)/sigx13_2 - ((starsq2^2 +
      2 * (sigx5_2 * sigma_sq2^2 * sigmastar2)) * sigmastar2 +
      0.5 * (starsq2^2 * usq2 * ewv2/sigmastar2)) * sigx12_2 *
      ewu/sigx13_2^2) * sigx2_2 * ewu * (epsilon)^2) *
      sigmastar2 + 0.5 * (((dpsv2 * usq2 * vsq2 + sigx2_2 *
      ewv2/sigma_sq2) * ewu/sigma_sq2 + vsq2 * sigx2_2)/starsq2 -
      sigx5_2 * vsq2 * sigx2_2 * ewu/starsq2^2) + 0.5 *
      (sigx15_2 * usq2 * ewv2/starsq2)) * ewv2 + (S^2 *
      sigx17_2 * sigx6_2 * ewu * (epsilon)^2/sigmastar2 -
      0.5 * ((dpsv2 * sigmastar2 + 0.5 * (sigx2_2 * ewv2/sigmastar2)) *
        usq2))/sigma_sq2)/(sigx20 * ewvsr2) - sigx17_2 *
      sigx21 * ewvsr2/(sigx20 * ewvsr2)^2) * ewz2 * sigx1_2 *
      ewu, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    2 * nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((sigx9_1 * sigx1_1/(sigma_sq1 *
      ewvsr1) - sigx9_2 * sigx1_2/(sigma_sq2 * ewvsr2))/sigx22 -
      pi * sigx21 * sigx18 * ((Wz)^2 + 1)/sigx22^2) * ewu,
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (((((sigx11_1 * sigx14_1 +
      (S * (S * sigx12_1 * dmusig1 * ewu * (epsilon)/starsq1^2 -
        2 * (pmusig1/sigma_sq1)) * (epsilon) - 0.5 *
        sigx10_1)/sigma_sq1) * ewv1 + (0.5 * (ewv1 *
      (S^2 * sigx12_1 * dmusig1 * ewu^2 * (epsilon)^2/(starsq1^2 *
        sigmastar1) - dmusig1)/sigma_sq1 - 0.5 * (vsq1 *
      dmusig1)) + 0.5 * dmusig1) * vsq1/sigmastar1 + S *
      pmusig1 * (epsilon)/sigma_sq1) * ewu/sigma_sq1 +
      ((2 - 16 * (ewv1^2/(2 * ewv1)^2)) * (S * (epsilon))^2/(2 *
        ewv1)^2 - S^2 * (sigx12_1 * (1/sigx13_1 - ((starsq1^2 +
        2 * (sigx12_1 * sigma_sq1^2 * sigmastar1)) *
        sigmastar1 + 0.5 * (starsq1^2 * vsq1 * ewu/sigmastar1)) *
        ewv1/sigx13_1^2) + (0.5 * (ewv1/sigma_sq1) -
        0.5 * (0.5 * vsq1 + ewv1/sigma_sq1)) * vsq1/(starsq1^2 *
        ewv1)) * ewu^2 * (epsilon)^2) * sigx2_1) * sigmastar1 +
      sigx17_1 * sigx14_1 + (((0.5 * (((0.5 * sigx10_1 +
      2 * (S * pmusig1 * (epsilon)/sigma_sq1)) * ewu -
      dmusig1 * sigmastar1)/(sigma_sq1^2 * sigmastar1) -
      sigx12_1 * sigx2_1/starsq1^2) + 0.5 * (sigx15_1/starsq1)) *
      ewv1 + 0.5 * (sigx2_1/starsq1)) * vsq1 - 0.5 * ((sigx11_1 *
      sigmastar1 + 0.5 * (vsq1 * sigx2_1/sigmastar1))/sigma_sq1)) *
      ewu) * ewv1/(sigx20 * ewvsr1) - (sigx17_1 * ewz1 *
      sigx1_1 + 0.5 * (sigx20 * ewvsr1)) * sigx17_1/(sigx20 *
      ewvsr1)^2) * ewz1 * sigx1_1, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (sigx17_1 * sigx17_2 * ewz2 * ewz1 * sigx1_1 * sigx1_2 *
      ewvsr2/((sigx20 * ewvsr2)^2 * ewvsr1)), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
      2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx17_1 * (1/sigx22 - pi *
      sigx18 * ((Wz)^2 + 1) * ewz1/sigx22^2) * sigx1_1/ewvsr1,
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * (((((sigx11_2 * sigx14_2 + (S * (S *
      sigx12_2 * dmusig2 * ewu * (epsilon)/starsq2^2 -
      2 * (pmusig2/sigma_sq2)) * (epsilon) - 0.5 * sigx10_2)/sigma_sq2) *
      ewv2 + (0.5 * (ewv2 * (S^2 * sigx12_2 * dmusig2 *
      ewu^2 * (epsilon)^2/(starsq2^2 * sigmastar2) - dmusig2)/sigma_sq2 -
      0.5 * (vsq2 * dmusig2)) + 0.5 * dmusig2) * vsq2/sigmastar2 +
      S * pmusig2 * (epsilon)/sigma_sq2) * ewu/sigma_sq2 +
      ((2 - 16 * (ewv2^2/(2 * ewv2)^2)) * (S * (epsilon))^2/(2 *
        ewv2)^2 - S^2 * (sigx12_2 * (1/sigx13_2 - ((starsq2^2 +
        2 * (sigx12_2 * sigma_sq2^2 * sigmastar2)) *
        sigmastar2 + 0.5 * (starsq2^2 * vsq2 * ewu/sigmastar2)) *
        ewv2/sigx13_2^2) + (0.5 * (ewv2/sigma_sq2) -
        0.5 * (0.5 * vsq2 + ewv2/sigma_sq2)) * vsq2/(starsq2^2 *
        ewv2)) * ewu^2 * (epsilon)^2) * sigx2_2) * sigmastar2 +
      sigx17_2 * sigx14_2 + (((0.5 * (((0.5 * sigx10_2 +
      2 * (S * pmusig2 * (epsilon)/sigma_sq2)) * ewu -
      dmusig2 * sigmastar2)/(sigma_sq2^2 * sigmastar2) -
      sigx12_2 * sigx2_2/starsq2^2) + 0.5 * (sigx15_2/starsq2)) *
      ewv2 + 0.5 * (sigx2_2/starsq2)) * vsq2 - 0.5 * ((sigx11_2 *
      sigmastar2 + 0.5 * (vsq2 * sigx2_2/sigmastar2))/sigma_sq2)) *
      ewu) * ewv2/(sigx20 * ewvsr2) - (sigx17_2 * ewz2 *
      sigx1_2 + 0.5 * (sigx20 * ewvsr2)) * sigx17_2/(sigx20 *
      ewvsr2)^2) * ewz2 * sigx1_2, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * (1/sigx22 +
      pi * sigx18 * ((Wz)^2 + 1) * ewz2/sigx22^2) * sigx1_2/ewvsr2),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar + nZHvar), (nXvar + nuZUvar + 2 * nvZVvar +
    1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * (sigx18 * (sigx2_1 * sigx1_1 *
      sigmastar1/ewvsr1 + 2 * (pi * Wz * sigx20) - sigx2_2 *
      sigx1_2 * sigmastar2/ewvsr2)/sigx22^2), FUN = "*"),
    Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
...
## probit specification class membership
chesscnsfraynormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((((S^2 * ewu * (epsilon)^2/(sigma_sq1 *
      ...
      usq1 * (S * ((2 * (S * dmusig1 * wuv1 * (epsilon)) +
        3 * pmusig1) * ewu/sigma_sq1 + S * usq1 * sigx2_1 *
        (epsilon)/ewv1) * (epsilon) - dmusig1 * sigmastar1)/ewv1) *
      pwZ * sigx1_1 * sigmastar1/ewvsr1 + (((S^2 * ewu *
      (epsilon)^2/(sigma_sq2 * ewv2) - 1) * wuv2 + ewu/starsq2) *
      dmusig2 * ewu/sigma_sq2 + usq2 * (S * ((2 * (S *
      dmusig2 * wuv2 * (epsilon)) + 3 * pmusig2) * ewu/sigma_sq2 +
      S * usq2 * sigx2_2 * (epsilon)/ewv2) * (epsilon) -
      dmusig2 * sigmastar2)/ewv2) * (1 - pwZ) * sigx1_2 *
      sigmastar2/ewvsr2 - sigx19^2/sigx20)/sigx20, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (((usq1 * (pmusig1 -
      0.5 * (S * dmusig1 * ewu * (epsilon)/starsq1)) +
      S * sigx6_1 * ewu * (S * ewu * sigx3_1 * (epsilon)/sigma_sq1 -
        2 * sigx2_1) * (epsilon)/sigmastar1) * sigmastar1 +
      (0.5 * (ewu * ewv1 * sigx3_1/starsq1) + S * sigx9_1 *
        (epsilon)/ewv1) * usq1) * pwZ * sigx1_1/(sigma_sq1 *
      ewvsr1) + ((usq2 * (pmusig2 - 0.5 * (S * dmusig2 *
      ewu * (epsilon)/starsq2)) + S * sigx6_2 * ewu * (S *
      ewu * sigx3_2 * (epsilon)/sigma_sq2 - 2 * sigx2_2) *
      (epsilon)/sigmastar2) * sigmastar2 + (0.5 * (ewu *
      ewv2 * sigx3_2/starsq2) + S * sigx9_2 * (epsilon)/ewv2) *
      usq2) * (1 - pwZ) * sigx1_2/(sigma_sq2 * ewvsr2) -
      sigx21 * sigx19/sigx20) * ewu/sigx20, FUN = "*"),
    uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (((((sigx14_1 * sigx3_1 + (S * (0.5 * (vsq1/ewv1) +
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    ...
    STATS = wHvar * S * (((((sigx14_2 * sigx3_2 + (S * (0.5 *
      ...
      0.5 * (ewu * sigx3_2 * sigmastar2/sigma_sq2))/(sigx20 *
      ewvsr2) - sigx17_2 * sigx19 * ewvsr2/(sigx20 * ewvsr2)^2) *
      (1 - pwZ) * sigx1_2, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * dwZ * (sigx1_1 * sigx4_1 *
      sigmastar1/ewvsr1 - (sigx19 * sigx18/sigx20 + sigx1_2 *
      sigx4_2 * sigmastar2/ewvsr2))/sigx20, FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    (((((0.5 * ((dpsv1 * usq1 + S * ewu * pmusig1 * (epsilon)/sigma_sq1 -
      ...
      (usq1 * (ewu * (S^2 * sigx6_1 * dmusig1 * (epsilon)^2 -
        ...
        ewu * (epsilon)^2/sigmastar1) * sigmastar1)/(sigma_sq1 *
      ewvsr1) + sigx9_1 * (1/(sigma_sq1 * ewvsr1) - ewu *
      ewvsr1/(sigma_sq1 * ewvsr1)^2)) * pwZ * sigx1_1 +
      ((((0.5 * ((dpsv2 * usq2 + S * ewu * pmusig2 * (epsilon)/sigma_sq2 -
        dmusig2 * sigmastar2) * ewu/sigma_sq2 - 0.5 *
        (usq2 * sigx2_2)) + 0.5 * (sigx7_2 * ewu/sigma_sq2)) *
        usq2 * ewv2 + S^2 * sigx9_2 * sigx6_2 * ewu^2 *
        ...
        sigmastar2)/(sigma_sq2 * ewvsr2) + sigx9_2 *
        (1/(sigma_sq2 * ewvsr2) - ewu * ewvsr2/(sigma_sq2 *
          ewvsr2)^2)) * (1 - pwZ) * sigx1_2 - sigx21^2 *
      ewu/sigx20) * ewu/sigx20, FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq1 * dmusig1) +
      0.5 * ((dmusig1 * ewv1/sigma_sq1 - S^2 * vsq1 * sigx6_1 *
      ...
      0.5 * ((dpsv1 * sigmastar1 + 0.5 * (sigx2_1 * ewv1/sigmastar1)) *
        usq1))/sigma_sq1)/(sigx20 * ewvsr1) - sigx17_1 *
      sigx21 * ewvsr1/(sigx20 * ewvsr1)^2) * pwZ * sigx1_1 *
      ewu, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq2 * dmusig2) +
      ...
      0.5 * ((dpsv2 * sigmastar2 + 0.5 * (sigx2_2 * ewv2/sigmastar2)) *
        usq2))/sigma_sq2)/(sigx20 * ewvsr2) - sigx17_2 *
      sigx21 * ewvsr2/(sigx20 * ewvsr2)^2) * (1 - pwZ) *
      sigx1_2 * ewu, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    2 * nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (sigx9_1 * sigx1_1/(sigma_sq1 *
      ewvsr1) - (sigx21 * sigx18/sigx20 + sigx9_2 * sigx1_2/(sigma_sq2 *
      ewvsr2))) * dwZ * ewu/sigx20, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (((((sigx11_1 * sigx14_1 +
      (S * (S * sigx12_1 * dmusig1 * ewu * (epsilon)/starsq1^2 -
      ...
      ewv1 + 0.5 * (sigx2_1/starsq1)) * vsq1 - 0.5 * ((sigx11_1 *
      sigmastar1 + 0.5 * (vsq1 * sigx2_1/sigmastar1))/sigma_sq1)) *
      ewu) * ewv1/(sigx20 * ewvsr1) - (sigx17_1 * pwZ *
      sigx1_1 + 0.5 * (sigx20 * ewvsr1)) * sigx17_1/(sigx20 *
      ewvsr1)^2) * pwZ * sigx1_1, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (sigx17_1 * sigx17_2 * (1 - pwZ) * pwZ * sigx1_1 * sigx1_2 *
      ewvsr2/((sigx20 * ewvsr2)^2 * ewvsr1)), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
      2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx17_1 * (1 - sigx18 *
      pwZ/sigx20) * dwZ * sigx1_1/(sigx20 * ewvsr1), FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * (((((sigx11_2 * sigx14_2 + (S * (S *
      ...
      ewv2 + 0.5 * (sigx2_2/starsq2)) * vsq2 - 0.5 * ((sigx11_2 *
      sigmastar2 + 0.5 * (vsq2 * sigx2_2/sigmastar2))/sigma_sq2)) *
      ewu) * ewv2/(sigx20 * ewvsr2) - (sigx17_2 * (1 -
      pwZ) * sigx1_2 + 0.5 * (sigx20 * ewvsr2)) * sigx17_2/(sigx20 *
      ewvsr2)^2) * (1 - pwZ) * sigx1_2, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * (sigx18 * (1 -
      pwZ)/sigx20 + 1) * dwZ * sigx1_2/(sigx20 * ewvsr2)),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar + nZHvar), (nXvar + nuZUvar + 2 * nvZVvar +
    1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * ((sigx18 * dwZ/sigx20 +
      Wz) * sigx18 * dwZ/sigx20), FUN = "*"), Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
}
...
## cloglog specification class membership
chesscnsfraynormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((((S^2 * ewu * (epsilon)^2/(sigma_sq1 *
      ...
      usq1 * (S * ((2 * (S * dmusig1 * wuv1 * (epsilon)) +
        3 * pmusig1) * ewu/sigma_sq1 + S * usq1 * sigx2_1 *
        (epsilon)/ewv1) * (epsilon) - dmusig1 * sigmastar1)/ewv1) *
      (1 - prZ) * sigx1_1 * sigmastar1/ewvsr1 + (((S^2 *
      ewu * (epsilon)^2/(sigma_sq2 * ewv2) - 1) * wuv2 +
      ewu/starsq2) * dmusig2 * ewu/sigma_sq2 + usq2 * (S *
      ((2 * (S * dmusig2 * wuv2 * (epsilon)) + 3 * pmusig2) *
        ewu/sigma_sq2 + S * usq2 * sigx2_2 * (epsilon)/ewv2) *
      (epsilon) - dmusig2 * sigmastar2)/ewv2) * prZ * sigx1_2 *
      sigmastar2/ewvsr2 - sigx19^2/sigx20)/sigx20, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (((usq1 * (pmusig1 -
      ...
      S * sigx6_1 * ewu * (S * ewu * sigx3_1 * (epsilon)/sigma_sq1 -
        2 * sigx2_1) * (epsilon)/sigmastar1) * sigmastar1 +
      (0.5 * (ewu * ewv1 * sigx3_1/starsq1) + S * sigx9_1 *
        (epsilon)/ewv1) * usq1) * (1 - prZ) * sigx1_1/(sigma_sq1 *
      ewvsr1) + ((usq2 * (pmusig2 - 0.5 * (S * dmusig2 *
      ewu * (epsilon)/starsq2)) + S * sigx6_2 * ewu * (S *
      ewu * sigx3_2 * (epsilon)/sigma_sq2 - 2 * sigx2_2) *
      (epsilon)/sigmastar2) * sigmastar2 + (0.5 * (ewu *
      ewv2 * sigx3_2/starsq2) + S * sigx9_2 * (epsilon)/ewv2) *
      usq2) * prZ * sigx1_2/(sigma_sq2 * ewvsr2) - sigx21 *
      sigx19/sigx20) * ewu/sigx20, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (((((sigx14_1 * sigx3_1 + (S * (0.5 * (vsq1/ewv1) +
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    ...
    STATS = wHvar * S * (((((sigx14_2 * sigx3_2 + (S * (0.5 *
      ...
      0.5 * (ewu * sigx3_2 * sigmastar2/sigma_sq2))/(sigx20 *
      ewvsr2) - sigx17_2 * sigx19 * ewvsr2/(sigx20 * ewvsr2)^2) *
      prZ * sigx1_2, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * prZ * (sigx1_1 * sigx4_1 *
      sigmastar1/ewvsr1 - (sigx19 * sigx18/sigx20 + sigx1_2 *
      sigx4_2 * sigmastar2/ewvsr2)) * ewz/sigx20, FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    (((((0.5 * ((dpsv1 * usq1 + S * ewu * pmusig1 * (epsilon)/sigma_sq1 -
      ...
      (usq1 * (ewu * (S^2 * sigx6_1 * dmusig1 * (epsilon)^2 -
        ...
        ewu * (epsilon)^2/sigmastar1) * sigmastar1)/(sigma_sq1 *
      ewvsr1) + sigx9_1 * (1/(sigma_sq1 * ewvsr1) - ewu *
      ewvsr1/(sigma_sq1 * ewvsr1)^2)) * (1 - prZ) * sigx1_1 +
      ((((0.5 * ((dpsv2 * usq2 + S * ewu * pmusig2 * (epsilon)/sigma_sq2 -
        dmusig2 * sigmastar2) * ewu/sigma_sq2 - 0.5 *
        (usq2 * sigx2_2)) + 0.5 * (sigx7_2 * ewu/sigma_sq2)) *
        usq2 * ewv2 + S^2 * sigx9_2 * sigx6_2 * ewu^2 *
        ...
        sigmastar2)/(sigma_sq2 * ewvsr2) + sigx9_2 *
        (1/(sigma_sq2 * ewvsr2) - ewu * ewvsr2/(sigma_sq2 *
          ewvsr2)^2)) * prZ * sigx1_2 - sigx21^2 * ewu/sigx20) *
    ewu/sigx20, FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq1 * dmusig1) +
      ...
      0.5 * ((dpsv1 * sigmastar1 + 0.5 * (sigx2_1 * ewv1/sigmastar1)) *
        usq1))/sigma_sq1)/(sigx20 * ewvsr1) - sigx17_1 *
      sigx21 * ewvsr1/(sigx20 * ewvsr1)^2) * (1 - prZ) *
      sigx1_1 * ewu, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq2 * dmusig2) +
      0.5 * ((dmusig2 * ewv2/sigma_sq2 - S^2 * vsq2 * sigx6_2 *
      ...
      0.5 * ((dpsv2 * sigmastar2 + 0.5 * (sigx2_2 * ewv2/sigmastar2)) *
        usq2))/sigma_sq2)/(sigx20 * ewvsr2) - sigx17_2 *
      sigx21 * ewvsr2/(sigx20 * ewvsr2)^2) * prZ * sigx1_2 *
      ewu, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    2 * nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (sigx9_1 * sigx1_1/(sigma_sq1 *
      ewvsr1) - (sigx21 * sigx18/sigx20 + sigx9_2 * sigx1_2/(sigma_sq2 *
      ewvsr2))) * prZ * ewu * ewz/sigx20, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (((((sigx11_1 * sigx14_1 +
      (S * (S * sigx12_1 * dmusig1 * ewu * (epsilon)/starsq1^2 -
      ...
      ewv1 + 0.5 * (sigx2_1/starsq1)) * vsq1 - 0.5 * ((sigx11_1 *
      sigmastar1 + 0.5 * (vsq1 * sigx2_1/sigmastar1))/sigma_sq1)) *
      ewu) * ewv1/(sigx20 * ewvsr1) - (sigx17_1 * (1 -
      prZ) * sigx1_1 + 0.5 * (sigx20 * ewvsr1)) * sigx17_1/(sigx20 *
      ewvsr1)^2) * (1 - prZ) * sigx1_1, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (sigx17_1 * sigx17_2 * prZ * (1 - prZ) * sigx1_1 * sigx1_2 *
      ewvsr2/((sigx20 * ewvsr2)^2 * ewvsr1)), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
      2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx17_1 * (1 - sigx18 *
      (1 - prZ)/sigx20) * prZ * sigx1_1 * ewz/(sigx20 *
      ewvsr1), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * (((((sigx11_2 * sigx14_2 + (S * (S *
      ...
      ewv2 + 0.5 * (sigx2_2/starsq2)) * vsq2 - 0.5 * ((sigx11_2 *
      sigmastar2 + 0.5 * (vsq2 * sigx2_2/sigmastar2))/sigma_sq2)) *
      ewu) * ewv2/(sigx20 * ewvsr2) - (sigx17_2 * prZ *
      sigx1_2 + 0.5 * (sigx20 * ewvsr2)) * sigx17_2/(sigx20 *
      ewvsr2)^2) * prZ * sigx1_2, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * (sigx18 * prZ/sigx20 +
      1) * prZ * sigx1_2 * ewz/(sigx20 * ewvsr2)), FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar + nZHvar), (nXvar + nuZUvar + 2 * nvZVvar +
    1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = wHvar * sigx18 * (1 - (sigx18 * prZ/sigx20 +
      1) * ewz) * prZ * ewz/sigx20, FUN = "*"), Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
}
...
## logit specification class membership
chessmcesfraynormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
...
## cauchit specification class membership
chessmcesfraynormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((((S^2 * ewu1 * (epsilon)^2/(sigma_sq1 *
      ewv1) - 1) * wuv1 + ewu1/starsq1) * dmusig1 * ewu1/sigma_sq1 +
      usq1 * (S * ((2 * (S * dmusig1 * wuv1 * (epsilon)) +
        3 * pmusig1) * ewu1/sigma_sq1 + S * usq1 * sigx2_1 *
        (epsilon)/ewv1) * (epsilon) - dmusig1 * sigmastar1)/ewv1) *
      ewz1 * sigx1_1 * sigmastar1/(ewu1 * ewvsr1) + (((S^2 *
      ewu2 * (epsilon)^2/(sigma_sq2 * ewv2) - 1) * wuv2 +
      ewu2/starsq2) * dmusig2 * ewu2/sigma_sq2 + usq2 *
      (S * ((2 * (S * dmusig2 * wuv2 * (epsilon)) + 3 *
        pmusig2) * ewu2/sigma_sq2 + S * usq2 * sigx2_2 *
        (epsilon)/ewv2) * (epsilon) - dmusig2 * sigmastar2)/ewv2) *
      ewz2 * sigx1_2 * sigmastar2/(ewu2 * ewvsr2) - sigx19^2/sigx20)/sigx20,
    FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx21_1 * (S * usq1 *
      (epsilon)/ewv1 - sigx19/sigx20) + (((usq1 * (pmusig1 -
      0.5 * (S * dmusig1 * ewu1 * (epsilon)/starsq1)) +
      S * sigx6_1 * ewu1 * (S * ewu1 * sigx3_1 * (epsilon)/sigma_sq1 -
        2 * sigx2_1) * (epsilon)/sigmastar1) * sigmastar1 +
      0.5 * (usq1 * ewu1 * ewv1 * sigx3_1/starsq1))/ewvsr1 -
      ewu1^2 * ewvsr1 * sigx3_1 * sigmastar1/(ewu1 * ewvsr1)^2)/sigma_sq1) *
      ewz1 * sigx1_1/sigx20, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx21_2 * (S * usq2 *
      (epsilon)/ewv2 - sigx19/sigx20) + (((usq2 * (pmusig2 -
      0.5 * (S * dmusig2 * ewu2 * (epsilon)/starsq2)) +
      S * sigx6_2 * ewu2 * (S * ewu2 * sigx3_2 * (epsilon)/sigma_sq2 -
        2 * sigx2_2) * (epsilon)/sigmastar2) * sigmastar2 +
      0.5 * (usq2 * ewu2 * ewv2 * sigx3_2/starsq2))/ewvsr2 -
      ewu2^2 * ewvsr2 * sigx3_2 * sigmastar2/(ewu2 * ewvsr2)^2)/sigma_sq2) *
      ewz2 * sigx1_2/sigx20, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (sigx17_1 * (S * usq1 * (epsilon)/ewv1 - sigx19/sigx20) +
    (((sigx14_1 * sigx3_1 + (S * (0.5 * (vsq1/ewv1) + 1/sigma_sq1) *
      dmusig1 * ewu1 * (epsilon)/sigmastar1 - pmusig1)/sigma_sq1) *
      ewu1/sigma_sq1 + S * (2 * (sigx12_1 * ewu1^2/sigx13_1) -
      4/(2 * ewv1)^2) * sigx2_1 * (epsilon)) * sigmastar1 +
      0.5 * (vsq1 * ewu1^2 * sigx3_1/(sigma_sq1^2 * sigmastar1))) *
      ewv1/(ewu1 * ewvsr1) - 0.5 * (ewu1^2 * ewvsr1 * sigx3_1 *
    sigmastar1/((ewu1 * ewvsr1)^2 * sigma_sq1))) * ewz1 *
    sigx1_1/sigx20, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx17_2 * (S * usq2 *
      (epsilon)/ewv2 - sigx19/sigx20) + (((sigx14_2 * sigx3_2 +
      (S * (0.5 * (vsq2/ewv2) + 1/sigma_sq2) * dmusig2 *
        ewu2 * (epsilon)/sigmastar2 - pmusig2)/sigma_sq2) *
      ewu2/sigma_sq2 + S * (2 * (sigx12_2 * ewu2^2/sigx13_2) -
      4/(2 * ewv2)^2) * sigx2_2 * (epsilon)) * sigmastar2 +
      0.5 * (vsq2 * ewu2^2 * sigx3_2/(sigma_sq2^2 * sigmastar2))) *
      ewv2/(ewu2 * ewvsr2) - 0.5 * (ewu2^2 * ewvsr2 * sigx3_2 *
      sigmastar2/((ewu2 * ewvsr2)^2 * sigma_sq2))) * ewz2 *
      sigx1_2/sigx20, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * ((sigx1_1 * sigx4_1 *
      sigmastar1/(ewu1 * ewvsr1) - sigx1_2 * sigx4_2 *
      sigmastar2/(ewu2 * ewvsr2))/sigx22 - pi * sigx19 *
      sigx18 * ((Wz)^2 + 1)/sigx22^2), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    (((usq1 * (ewu1 * (S^2 * sigx6_1 * dmusig1 * (epsilon)^2 -
      dpsv1/sigma_sq1) - 0.5 * (0.5 * (usq1 * dmusig1 *
      ewv1/sigmastar1) + S^2 * sigx6_1 * dmusig1 * ewu1 *
      (epsilon)^2)) + S^2 * ((dpsv1 * usq1 * sigx6_1/sigma_sq1 -
      ((0.5 * (ewu1/sigma_sq1) + 1 - 0.5 * (0.5 * usq1 +
        ewu1/sigma_sq1)) * usq1 * ewv1/sigmastar1 + (2 -
      ...
      sigmastar1 + (0.5 * ((dpsv1 * usq1 + S * ewu1 * pmusig1 *
      (epsilon)/sigma_sq1 - dmusig1 * sigmastar1) * ewu1/sigma_sq1 -
      0.5 * (usq1 * sigx2_1)) + 0.5 * (sigx7_1 * ewu1/sigma_sq1)) *
      usq1 * ewv1/sigmastar1)/(sigma_sq1 * ewvsr1) + ewu1 *
      (S^2 * sigx21_1 * sigx6_1 * ewu1 * (epsilon)^2/starsq1 -
        ((((dpsv1 * usq1 - S * pmusig1 * (epsilon)) *
          ewu1/sigma_sq1 + dmusig1 * sigmastar1) * sigmastar1 +
          (0.5 * (usq1 * ewv1/starsq1) - 2 * (ewu1 *
          ewvsr1^2 * sigmastar1/(ewu1 * ewvsr1)^2)) *
          sigx2_1 * ewu1)/(ewu1 * ewvsr1)^2 + sigx9_1/(sigma_sq1 *
          ewvsr1)^2) * ewvsr1) - sigx21_1^2 * ewz1 *
      sigx1_1/sigx20) * ewz1 * sigx1_1/sigx20, FUN = "*"),
    uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -wHvar * (sigx21_1 * sigx21_2 * ewz2 * ewz1 *
      sigx1_1 * sigx1_2/sigx20^2), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq1 * dmusig1) +
      0.5 * ((dmusig1 * ewv1/sigma_sq1 - S^2 * vsq1 * sigx6_1 *
      ...
      S^2 * (((0.5 * (usq1 * ewv1/sigma_sq1) + 0.5 * ((ewu1/sigma_sq1 -
        1) * ewv1/sigma_sq1 + 1 - 0.5 * (usq1 * vsq1))) *
        ewu1/sigmastar1 + 2 * sigx12_1)/sigx13_1 - ((starsq1^2 +
        2 * (sigx5_1 * sigma_sq1^2 * sigmastar1)) * sigmastar1 +
        0.5 * (starsq1^2 * usq1 * ewv1/sigmastar1)) *
        sigx12_1 * ewu1/sigx13_1^2) * sigx2_1 * ewu1 *
        (epsilon)^2) * sigmastar1 + 0.5 * (((dpsv1 *
      usq1 * vsq1 + sigx2_1 * ewv1/sigma_sq1) * ewu1/sigma_sq1 +
      vsq1 * sigx2_1)/starsq1 - sigx5_1 * vsq1 * sigx2_1 *
      ewu1/starsq1^2) + 0.5 * (sigx15_1 * usq1 * ewv1/starsq1))/ewvsr1 -
      sigx16_1 * ewu1 * ewvsr1/(ewu1 * ewvsr1)^2) * ewv1 +
      ewu1 * (S^2 * sigx17_1 * sigx6_1 * ewu1 * (epsilon)^2/starsq1 -
        0.5 * ((((dpsv1 * usq1 - S * pmusig1 * (epsilon)) *
          ewu1/sigma_sq1 + dmusig1 * sigmastar1) * sigmastar1 +
          (0.5 * (usq1 * ewv1/starsq1) - 2 * (ewu1 *
          ewvsr1^2 * sigmastar1/(ewu1 * ewvsr1)^2)) *
          sigx2_1 * ewu1) * ewvsr1/(ewu1 * ewvsr1)^2)) -
      sigx17_1 * sigx21_1 * ewz1 * sigx1_1/sigx20) * ewz1 *
      sigx1_1/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * sigx21_1 * ewz2 *
      ewz1 * sigx1_1 * sigx1_2/sigx20^2), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    sigx21_1 * (1/sigx22 - pi * sigx18 * ((Wz)^2 + 1) * ewz1/sigx22^2) *
    sigx1_1, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    nuZUvar + 1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((usq2 * (ewu2 * (S^2 *
      sigx6_2 * dmusig2 * (epsilon)^2 - dpsv2/sigma_sq2) -
      0.5 * (0.5 * (usq2 * dmusig2 * ewv2/sigmastar2) +
        S^2 * sigx6_2 * dmusig2 * ewu2 * (epsilon)^2)) +
      S^2 * ((dpsv2 * usq2 * sigx6_2/sigma_sq2 - ((0.5 *
        (ewu2/sigma_sq2) + 1 - 0.5 * (0.5 * usq2 + ewu2/sigma_sq2)) *
        usq2 * ewv2/sigmastar2 + (2 - 2 * (sigx5_2^2 *
        ewu2 * sigma_sq2/starsq2^2)) * sigmastar2) *
        sigx2_2/starsq2^2) * ewu2 + (1 - 0.5 * usq2) *
        sigx6_2 * sigx2_2) * ewu2 * (epsilon)^2/sigmastar2) *
      sigmastar2 + (0.5 * ((dpsv2 * usq2 + S * ewu2 * pmusig2 *
      (epsilon)/sigma_sq2 - dmusig2 * sigmastar2) * ewu2/sigma_sq2 -
      0.5 * (usq2 * sigx2_2)) + 0.5 * (sigx7_2 * ewu2/sigma_sq2)) *
      usq2 * ewv2/sigmastar2)/(sigma_sq2 * ewvsr2) + ewu2 *
      (S^2 * sigx21_2 * sigx6_2 * ewu2 * (epsilon)^2/starsq2 -
        ((((dpsv2 * usq2 - S * pmusig2 * (epsilon)) *
          ewu2/sigma_sq2 + dmusig2 * sigmastar2) * sigmastar2 +
          (0.5 * (usq2 * ewv2/starsq2) - 2 * (ewu2 *
          ewvsr2^2 * sigmastar2/(ewu2 * ewvsr2)^2)) *
          sigx2_2 * ewu2)/(ewu2 * ewvsr2)^2 + sigx9_2/(sigma_sq2 *
          ewvsr2)^2) * ewvsr2) - sigx21_2^2 * ewz2 *
      sigx1_2/sigx20) * ewz2 * sigx1_2/sigx20, FUN = "*"),
    uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_1 * sigx21_2 * ewz2 *
      ewz1 * sigx1_1 * sigx1_2/sigx20^2), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 *
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((((((0.5 * (vsq2 * dmusig2) + 0.5 * ((dmusig2 * ewv2/sigma_sq2 -
      S^2 * vsq2 * sigx6_2 * dmusig2 * ewu2 * (epsilon)^2/sigmastar2) *
      ewu2/sigma_sq2 - 0.5 * (usq2 * vsq2 * dmusig2)))/sigmastar2 +
      dpsv2 * usq2 * sigx14_2 + (S * (pmusig2 - ewu2 *
      (pmusig2/sigma_sq2 + S * sigx6_2 * dmusig2 * (epsilon))) *
      (epsilon) - sigx11_2 * ewu2)/sigma_sq2)/sigma_sq2 -
      S^2 * (((0.5 * (usq2 * ewv2/sigma_sq2) + 0.5 * ((ewu2/sigma_sq2 -
        1) * ewv2/sigma_sq2 + 1 - 0.5 * (usq2 * vsq2))) *
        ewu2/sigmastar2 + 2 * sigx12_2)/sigx13_2 - ((starsq2^2 +
        2 * (sigx5_2 * sigma_sq2^2 * sigmastar2)) * sigmastar2 +
        0.5 * (starsq2^2 * usq2 * ewv2/sigmastar2)) *
        sigx12_2 * ewu2/sigx13_2^2) * sigx2_2 * ewu2 *
        (epsilon)^2) * sigmastar2 + 0.5 * (((dpsv2 *
      usq2 * vsq2 + sigx2_2 * ewv2/sigma_sq2) * ewu2/sigma_sq2 +
      vsq2 * sigx2_2)/starsq2 - sigx5_2 * vsq2 * sigx2_2 *
      ewu2/starsq2^2) + 0.5 * (sigx15_2 * usq2 * ewv2/starsq2))/ewvsr2 -
      sigx16_2 * ewu2 * ewvsr2/(ewu2 * ewvsr2)^2) * ewv2 +
      ewu2 * (S^2 * sigx17_2 * sigx6_2 * ewu2 * (epsilon)^2/starsq2 -
        0.5 * ((((dpsv2 * usq2 - S * pmusig2 * (epsilon)) *
          ewu2/sigma_sq2 + dmusig2 * sigmastar2) * sigmastar2 +
          (0.5 * (usq2 * ewv2/starsq2) - 2 * (ewu2 *
          ewvsr2^2 * sigmastar2/(ewu2 * ewvsr2)^2)) *
          sigx2_2 * ewu2) * ewvsr2/(ewu2 * ewvsr2)^2)) -
      sigx17_2 * sigx21_2 * ewz2 * sigx1_2/sigx20) * ewz2 *
    sigx1_2/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -wHvar * (sigx21_2 * (1/sigx22 + pi * sigx18 *
      ((Wz)^2 + 1) * ewz2/sigx22^2) * sigx1_2), FUN = "*"),
    Zvar)
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((((((sigx11_1 * sigx14_1 +
      (S * (S * sigx12_1 * dmusig1 * ewu1 * (epsilon)/starsq1^2 -
        2 * (pmusig1/sigma_sq1)) * (epsilon) - 0.5 *
        sigx10_1)/sigma_sq1) * ewv1 + (0.5 * (ewv1 *
      (S^2 * sigx12_1 * dmusig1 * ewu1^2 * (epsilon)^2/(starsq1^2 *
        sigmastar1) - dmusig1)/sigma_sq1 - 0.5 * (vsq1 *
      dmusig1)) + 0.5 * dmusig1) * vsq1/sigmastar1 + S *
      pmusig1 * (epsilon)/sigma_sq1) * ewu1/sigma_sq1 +
      ((2 - 16 * (ewv1^2/(2 * ewv1)^2)) * (S * (epsilon))^2/(2 *
        ...
        ewv1/sigx13_1^2) + (0.5 * (ewv1/sigma_sq1) -
        0.5 * (0.5 * vsq1 + ewv1/sigma_sq1)) * vsq1/(starsq1^2 *
        ewv1)) * ewu1^2 * (epsilon)^2) * sigx2_1) * sigmastar1 +
      ((0.5 * (((0.5 * sigx10_1 + 2 * (S * pmusig1 * (epsilon)/sigma_sq1)) *
        ewu1 - dmusig1 * sigmastar1)/(sigma_sq1^2 * sigmastar1) -
        sigx12_1 * sigx2_1/starsq1^2) + 0.5 * (sigx15_1/starsq1)) *
        ewv1 + 0.5 * (sigx2_1/starsq1)) * vsq1 * ewu1)/(ewu1 *
      ewvsr1) + sigx17_1 * sigx14_1 - 0.5 * (sigx16_1 *
      ewu1 * ewvsr1/(ewu1 * ewvsr1)^2)) * ewv1 - (sigx17_1^2 *
      ewz1 * sigx1_1/sigx20 + 0.5 * (((sigx11_1 * ewu1 *
      ewv1/sigma_sq1 + 0.5 * sigx2_1) * sigmastar1 + (0.5 *
      (vsq1 * ewv1/starsq1) - ewu1 * ewvsr1^2 * sigmastar1/(ewu1 *
      ewvsr1)^2) * sigx2_1 * ewu1) * ewu1 * ewvsr1/(ewu1 *
      ewvsr1)^2))) * ewz1 * sigx1_1/sigx20, FUN = "*"),
    vHvar)
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
      2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -wHvar * (sigx17_1 * sigx17_2 * ewz2 * ewz1 *
      sigx1_1 * sigx1_2/sigx20^2), FUN = "*"), vHvar)
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 *
      nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx17_1 * (1/sigx22 - pi *
      sigx18 * ((Wz)^2 + 1) * ewz1/sigx22^2) * sigx1_1,
    FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar), (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((((((sigx11_2 * sigx14_2 +
      (S * (S * sigx12_2 * dmusig2 * ewu2 * (epsilon)/starsq2^2 -
        2 * (pmusig2/sigma_sq2)) * (epsilon) - 0.5 *
        sigx10_2)/sigma_sq2) * ewv2 + (0.5 * (ewv2 *
      (S^2 * sigx12_2 * dmusig2 * ewu2^2 * (epsilon)^2/(starsq2^2 *
        sigmastar2) - dmusig2)/sigma_sq2 - 0.5 * (vsq2 *
      dmusig2)) + 0.5 * dmusig2) * vsq2/sigmastar2 + S *
      pmusig2 * (epsilon)/sigma_sq2) * ewu2/sigma_sq2 +
      ((2 - 16 * (ewv2^2/(2 * ewv2)^2)) * (S * (epsilon))^2/(2 *
        ewv2)^2 - S^2 * (sigx12_2 * (1/sigx13_2 - ((starsq2^2 +
        2 * (sigx12_2 * sigma_sq2^2 * sigmastar2)) *
        ...
        ewv2/sigx13_2^2) + (0.5 * (ewv2/sigma_sq2) -
        0.5 * (0.5 * vsq2 + ewv2/sigma_sq2)) * vsq2/(starsq2^2 *
        ewv2)) * ewu2^2 * (epsilon)^2) * sigx2_2) * sigmastar2 +
      ((0.5 * (((0.5 * sigx10_2 + 2 * (S * pmusig2 * (epsilon)/sigma_sq2)) *
        ewu2 - dmusig2 * sigmastar2)/(sigma_sq2^2 * sigmastar2) -
        sigx12_2 * sigx2_2/starsq2^2) + 0.5 * (sigx15_2/starsq2)) *
        ewv2 + 0.5 * (sigx2_2/starsq2)) * vsq2 * ewu2)/(ewu2 *
      ewvsr2) + sigx17_2 * sigx14_2 - 0.5 * (sigx16_2 *
      ewu2 * ewvsr2/(ewu2 * ewvsr2)^2)) * ewv2 - (sigx17_2^2 *
      ewz2 * sigx1_2/sigx20 + 0.5 * (((sigx11_2 * ewu2 *
      ewv2/sigma_sq2 + 0.5 * sigx2_2) * sigmastar2 + (0.5 *
      (vsq2 * ewv2/starsq2) - ewu2 * ewvsr2^2 * sigmastar2/(ewu2 *
      ewvsr2)^2) * sigx2_2 * ewu2) * ewu2 * ewvsr2/(ewu2 *
      ewvsr2)^2))) * ewz2 * sigx1_2/sigx20, FUN = "*"),
    vHvar)
  hessll[(nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar), (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * (1/sigx22 +
      pi * sigx18 * ((Wz)^2 + 1) * ewz2/sigx22^2) * sigx1_2),
    FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 *
    nuZUvar + 2 * nvZVvar + nZHvar), (nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1, STATS = -wHvar *
    (sigx18 * (sigx2_1 * sigx1_1 * sigmastar1/(ewu1 * ewvsr1) +
      2 * (pi * Wz * sigx20) - sigx2_2 * sigx1_2 * sigmastar2/(ewu2 *
      ewvsr2))/sigx22^2), FUN = "*"), Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
}
...
## probit specification class membership
chessmcesfraynormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((((S^2 * ewu1 * (epsilon)^2/(sigma_sq1 *
      ...
      usq1 * (S * ((2 * (S * dmusig1 * wuv1 * (epsilon)) +
        3 * pmusig1) * ewu1/sigma_sq1 + S * usq1 * sigx2_1 *
        (epsilon)/ewv1) * (epsilon) - dmusig1 * sigmastar1)/ewv1) *
      pwZ * sigx1_1 * sigmastar1/(ewu1 * ewvsr1) + (((S^2 *
      ewu2 * (epsilon)^2/(sigma_sq2 * ewv2) - 1) * wuv2 +
      ewu2/starsq2) * dmusig2 * ewu2/sigma_sq2 + usq2 *
      (S * ((2 * (S * dmusig2 * wuv2 * (epsilon)) + 3 *
        pmusig2) * ewu2/sigma_sq2 + S * usq2 * sigx2_2 *
        (epsilon)/ewv2) * (epsilon) - dmusig2 * sigmastar2)/ewv2) *
      (1 - pwZ) * sigx1_2 * sigmastar2/(ewu2 * ewvsr2) -
      sigx19^2/sigx20)/sigx20, FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx21_1 * (S * usq1 *
      (epsilon)/ewv1 - sigx19/sigx20) + (((usq1 * (pmusig1 -
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx21_2 * (S * usq2 *
      ...
      0.5 * (usq2 * ewu2 * ewv2 * sigx3_2/starsq2))/ewvsr2 -
      ewu2^2 * ewvsr2 * sigx3_2 * sigmastar2/(ewu2 * ewvsr2)^2)/sigma_sq2) *
      (1 - pwZ) * sigx1_2/sigx20, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (sigx17_1 * (S * usq1 * (epsilon)/ewv1 - sigx19/sigx20) +
    (((sigx14_1 * sigx3_1 + (S * (0.5 * (vsq1/ewv1) + 1/sigma_sq1) *
      ...
      0.5 * (vsq1 * ewu1^2 * sigx3_1/(sigma_sq1^2 * sigmastar1))) *
      ewv1/(ewu1 * ewvsr1) - 0.5 * (ewu1^2 * ewvsr1 * sigx3_1 *
    sigmastar1/((ewu1 * ewvsr1)^2 * sigma_sq1))) * pwZ *
    sigx1_1/sigx20, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx17_2 * (S * usq2 *
      ...
      0.5 * (vsq2 * ewu2^2 * sigx3_2/(sigma_sq2^2 * sigmastar2))) *
      ewv2/(ewu2 * ewvsr2) - 0.5 * (ewu2^2 * ewvsr2 * sigx3_2 *
      sigmastar2/((ewu2 * ewvsr2)^2 * sigma_sq2))) * (1 -
      pwZ) * sigx1_2/sigx20, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * dwZ * (sigx1_1 * sigx4_1 *
      sigmastar1/(ewu1 * ewvsr1) - (sigx19 * sigx18/sigx20 +
      sigx1_2 * sigx4_2 * sigmastar2/(ewu2 * ewvsr2)))/sigx20,
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    (((usq1 * (ewu1 * (S^2 * sigx6_1 * dmusig1 * (epsilon)^2 -
      ...
      (S^2 * sigx21_1 * sigx6_1 * ewu1 * (epsilon)^2/starsq1 -
        ((((dpsv1 * usq1 - S * pmusig1 * (epsilon)) *
          ...
          ewvsr1^2 * sigmastar1/(ewu1 * ewvsr1)^2)) *
          sigx2_1 * ewu1)/(ewu1 * ewvsr1)^2 + sigx9_1/(sigma_sq1 *
          ewvsr1)^2) * ewvsr1) - sigx21_1^2 * pwZ * sigx1_1/sigx20) *
    pwZ * sigx1_1/sigx20, FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -wHvar * (sigx21_1 * sigx21_2 * (1 - pwZ) * pwZ *
      sigx1_1 * sigx1_2/sigx20^2), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq1 * dmusig1) +
      ...
      ewu1 * (S^2 * sigx17_1 * sigx6_1 * ewu1 * (epsilon)^2/starsq1 -
        0.5 * ((((dpsv1 * usq1 - S * pmusig1 * (epsilon)) *
          ...
          ewvsr1^2 * sigmastar1/(ewu1 * ewvsr1)^2)) *
          sigx2_1 * ewu1) * ewvsr1/(ewu1 * ewvsr1)^2)) -
      sigx17_1 * sigx21_1 * pwZ * sigx1_1/sigx20) * pwZ *
      sigx1_1/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * sigx21_1 * (1 -
      pwZ) * pwZ * sigx1_1 * sigx1_2/sigx20^2), FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    sigx21_1 * (1 - sigx18 * pwZ/sigx20) * dwZ * sigx1_1/sigx20,
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    nuZUvar + 1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((usq2 * (ewu2 * (S^2 *
      ...
      (S^2 * sigx21_2 * sigx6_2 * ewu2 * (epsilon)^2/starsq2 -
        ((((dpsv2 * usq2 - S * pmusig2 * (epsilon)) *
          ...
          ewvsr2^2 * sigmastar2/(ewu2 * ewvsr2)^2)) *
          sigx2_2 * ewu2)/(ewu2 * ewvsr2)^2 + sigx9_2/(sigma_sq2 *
          ewvsr2)^2) * ewvsr2) - sigx21_2^2 * (1 - pwZ) *
      sigx1_2/sigx20) * (1 - pwZ) * sigx1_2/sigx20, FUN = "*"),
    uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_1 * sigx21_2 * (1 -
      pwZ) * pwZ * sigx1_1 * sigx1_2/sigx20^2), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 *
    ...
    ((((((0.5 * (vsq2 * dmusig2) + 0.5 * ((dmusig2 * ewv2/sigma_sq2 -
      ...
      ewu2 * (S^2 * sigx17_2 * sigx6_2 * ewu2 * (epsilon)^2/starsq2 -
        0.5 * ((((dpsv2 * usq2 - S * pmusig2 * (epsilon)) *
          ...
          ewvsr2^2 * sigmastar2/(ewu2 * ewvsr2)^2)) *
          sigx2_2 * ewu2) * ewvsr2/(ewu2 * ewvsr2)^2)) -
      sigx17_2 * sigx21_2 * (1 - pwZ) * sigx1_2/sigx20) *
    (1 - pwZ) * sigx1_2/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -wHvar * (sigx21_2 * (sigx18 * (1 - pwZ)/sigx20 +
      1) * dwZ * sigx1_2/sigx20), FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((((((sigx11_1 * sigx14_1 +
      (S * (S * sigx12_1 * dmusig1 * ewu1 * (epsilon)/starsq1^2 -
      ...
      ewvsr1) + sigx17_1 * sigx14_1 - 0.5 * (sigx16_1 *
      ewu1 * ewvsr1/(ewu1 * ewvsr1)^2)) * ewv1 - (sigx17_1^2 *
      pwZ * sigx1_1/sigx20 + 0.5 * (((sigx11_1 * ewu1 *
      ewv1/sigma_sq1 + 0.5 * sigx2_1) * sigmastar1 + (0.5 *
      (vsq1 * ewv1/starsq1) - ewu1 * ewvsr1^2 * sigmastar1/(ewu1 *
      ewvsr1)^2) * sigx2_1 * ewu1) * ewu1 * ewvsr1/(ewu1 *
      ewvsr1)^2))) * pwZ * sigx1_1/sigx20, FUN = "*"),
    vHvar)
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
      2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
  ...
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 *
      nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx17_1 * (1 - sigx18 *
      pwZ/sigx20) * dwZ * sigx1_1/sigx20, FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar), (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((((((sigx11_2 * sigx14_2 +
      ...
      ewvsr2) + sigx17_2 * sigx14_2 - 0.5 * (sigx16_2 *
      ewu2 * ewvsr2/(ewu2 * ewvsr2)^2)) * ewv2 - (sigx17_2^2 *
      (1 - pwZ) * sigx1_2/sigx20 + 0.5 * (((sigx11_2 *
      ewu2 * ewv2/sigma_sq2 + 0.5 * sigx2_2) * sigmastar2 +
      (0.5 * (vsq2 * ewv2/starsq2) - ewu2 * ewvsr2^2 *
        sigmastar2/(ewu2 * ewvsr2)^2) * sigx2_2 * ewu2) *
      ewu2 * ewvsr2/(ewu2 * ewvsr2)^2))) * (1 - pwZ) *
      sigx1_2/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar), (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * (sigx18 * (1 -
      pwZ)/sigx20 + 1) * dwZ * sigx1_2/sigx20), FUN = "*"),
    Zvar)
  hessll[(nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 *
    nuZUvar + 2 * nvZVvar + nZHvar), (nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1, STATS = -wHvar *
    ((sigx18 * dwZ/sigx20 + Wz) * sigx18 * dwZ/sigx20), FUN = "*"),
    Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
...
## cloglog specification class membership
chessmcesfraynormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  delta2 <- parm[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((((S^2 * ewu1 * (epsilon)^2/(sigma_sq1 *
      ...
      usq1 * (S * ((2 * (S * dmusig1 * wuv1 * (epsilon)) +
        3 * pmusig1) * ewu1/sigma_sq1 + S * usq1 * sigx2_1 *
        (epsilon)/ewv1) * (epsilon) - dmusig1 * sigmastar1)/ewv1) *
      (1 - prZ) * sigx1_1 * sigmastar1/(ewu1 * ewvsr1) +
      (((S^2 * ewu2 * (epsilon)^2/(sigma_sq2 * ewv2) -
        1) * wuv2 + ewu2/starsq2) * dmusig2 * ewu2/sigma_sq2 +
        usq2 * (S * ((2 * (S * dmusig2 * wuv2 * (epsilon)) +
          3 * pmusig2) * ewu2/sigma_sq2 + S * usq2 *
          sigx2_2 * (epsilon)/ewv2) * (epsilon) - dmusig2 *
          sigmastar2)/ewv2) * prZ * sigx1_2 * sigmastar2/(ewu2 *
        ewvsr2) - sigx19^2/sigx20)/sigx20, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx21_1 * (S * usq1 *
      (epsilon)/ewv1 - sigx19/sigx20) + (((usq1 * (pmusig1 -
      ...
      0.5 * (usq1 * ewu1 * ewv1 * sigx3_1/starsq1))/ewvsr1 -
      ewu1^2 * ewvsr1 * sigx3_1 * sigmastar1/(ewu1 * ewvsr1)^2)/sigma_sq1) *
      (1 - prZ) * sigx1_1/sigx20, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx21_2 * (S * usq2 *
      (epsilon)/ewv2 - sigx19/sigx20) + (((usq2 * (pmusig2 -
      0.5 * (S * dmusig2 * ewu2 * (epsilon)/starsq2)) +
  ...
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar +
    ...
    (((sigx14_1 * sigx3_1 + (S * (0.5 * (vsq1/ewv1) + 1/sigma_sq1) *
      ...
      0.5 * (vsq1 * ewu1^2 * sigx3_1/(sigma_sq1^2 * sigmastar1))) *
      ewv1/(ewu1 * ewvsr1) - 0.5 * (ewu1^2 * ewvsr1 * sigx3_1 *
    sigmastar1/((ewu1 * ewvsr1)^2 * sigma_sq1))) * (1 - prZ) *
    sigx1_1/sigx20, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (sigx17_2 * (S * usq2 *
      ...
      0.5 * (vsq2 * ewu2^2 * sigx3_2/(sigma_sq2^2 * sigmastar2))) *
      ewv2/(ewu2 * ewvsr2) - 0.5 * (ewu2^2 * ewvsr2 * sigx3_2 *
      sigmastar2/((ewu2 * ewvsr2)^2 * sigma_sq2))) * prZ *
      sigx1_2/sigx20, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * prZ * (sigx1_1 * sigx4_1 *
    ...
    (((usq1 * (ewu1 * (S^2 * sigx6_1 * dmusig1 * (epsilon)^2 -
      ...
      (S^2 * sigx21_1 * sigx6_1 * ewu1 * (epsilon)^2/starsq1 -
        ((((dpsv1 * usq1 - S * pmusig1 * (epsilon)) *
          ...
          ewvsr1^2 * sigmastar1/(ewu1 * ewvsr1)^2)) *
          sigx2_1 * ewu1)/(ewu1 * ewvsr1)^2 + sigx9_1/(sigma_sq1 *
          ewvsr1)^2) * ewvsr1) - sigx21_1^2 * (1 - prZ) *
      sigx1_1/sigx20) * (1 - prZ) * sigx1_1/sigx20, FUN = "*"),
    uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -wHvar * (sigx21_1 * sigx21_2 * prZ * (1 - prZ) *
      sigx1_1 * sigx1_2/sigx20^2), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((((0.5 * (vsq1 * dmusig1) +
      ...
      ewu1 * (S^2 * sigx17_1 * sigx6_1 * ewu1 * (epsilon)^2/starsq1 -
        0.5 * ((((dpsv1 * usq1 - S * pmusig1 * (epsilon)) *
          ...
          ewvsr1^2 * sigmastar1/(ewu1 * ewvsr1)^2)) *
          sigx2_1 * ewu1) * ewvsr1/(ewu1 * ewvsr1)^2)) -
      sigx17_1 * sigx21_1 * (1 - prZ) * sigx1_1/sigx20) *
      (1 - prZ) * sigx1_1/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * sigx21_1 * prZ *
      (1 - prZ) * sigx1_1 * sigx1_2/sigx20^2), FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    sigx21_1 * (1 - sigx18 * (1 - prZ)/sigx20) * prZ * sigx1_1 *
    ewz/sigx20, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    nuZUvar + 1):(nXvar + 2 * nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((usq2 * (ewu2 * (S^2 *
      ...
      (S^2 * sigx21_2 * sigx6_2 * ewu2 * (epsilon)^2/starsq2 -
        ((((dpsv2 * usq2 - S * pmusig2 * (epsilon)) *
          ...
          ewvsr2^2 * sigmastar2/(ewu2 * ewvsr2)^2)) *
          sigx2_2 * ewu2)/(ewu2 * ewvsr2)^2 + sigx9_2/(sigma_sq2 *
          ewvsr2)^2) * ewvsr2) - sigx21_2^2 * prZ * sigx1_2/sigx20) *
      prZ * sigx1_2/sigx20, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_1 * sigx21_2 * prZ *
      (1 - prZ) * sigx1_1 * sigx1_2/sigx20^2), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 *
    ...
    ((((((0.5 * (vsq2 * dmusig2) + 0.5 * ((dmusig2 * ewv2/sigma_sq2 -
      ...
      ewu2 * (S^2 * sigx17_2 * sigx6_2 * ewu2 * (epsilon)^2/starsq2 -
        0.5 * ((((dpsv2 * usq2 - S * pmusig2 * (epsilon)) *
          ...
          ewvsr2^2 * sigmastar2/(ewu2 * ewvsr2)^2)) *
          sigx2_2 * ewu2) * ewvsr2/(ewu2 * ewvsr2)^2)) -
      sigx17_2 * sigx21_2 * prZ * sigx1_2/sigx20) * prZ *
    sigx1_2/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + 2 * nuZUvar), (nXvar +
    2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -wHvar * (sigx21_2 * (sigx18 * prZ/sigx20 + 1) *
      prZ * sigx1_2 * ewz/sigx20), FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((((((sigx11_1 * sigx14_1 +
      (S * (S * sigx12_1 * dmusig1 * ewu1 * (epsilon)/starsq1^2 -
      ...
      ewvsr1) + sigx17_1 * sigx14_1 - 0.5 * (sigx16_1 *
      ewu1 * ewvsr1/(ewu1 * ewvsr1)^2)) * ewv1 - (sigx17_1^2 *
      (1 - prZ) * sigx1_1/sigx20 + 0.5 * (((sigx11_1 *
      ewu1 * ewv1/sigma_sq1 + 0.5 * sigx2_1) * sigmastar1 +
      (0.5 * (vsq1 * ewv1/starsq1) - ewu1 * ewvsr1^2 *
        sigmastar1/(ewu1 * ewvsr1)^2) * sigx2_1 * ewu1) *
      ewu1 * ewvsr1/(ewu1 * ewvsr1)^2))) * (1 - prZ) *
      sigx1_1/sigx20, FUN = "*"), vHvar)
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
      2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
  ...
  hessll[(nXvar + 2 * nuZUvar + 1):(nXvar + 2 * nuZUvar + nvZVvar),
    (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 *
      nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx17_1 * (1 - sigx18 *
      (1 - prZ)/sigx20) * prZ * sigx1_1 * ewz/sigx20, FUN = "*"),
    Zvar)
  hessll[(nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar), (nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((((((sigx11_2 * sigx14_2 +
      ...
      ewvsr2) + sigx17_2 * sigx14_2 - 0.5 * (sigx16_2 *
      ewu2 * ewvsr2/(ewu2 * ewvsr2)^2)) * ewv2 - (sigx17_2^2 *
      prZ * sigx1_2/sigx20 + 0.5 * (((sigx11_2 * ewu2 *
      ewv2/sigma_sq2 + 0.5 * sigx2_2) * sigmastar2 + (0.5 *
      (vsq2 * ewv2/starsq2) - ewu2 * ewvsr2^2 * sigmastar2/(ewu2 *
      ewvsr2)^2) * sigx2_2 * ewu2) * ewu2 * ewvsr2/(ewu2 *
      ewvsr2)^2))) * prZ * sigx1_2/sigx20, FUN = "*"),
    vHvar)
  hessll[(nXvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nuZUvar +
    2 * nvZVvar), (nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar +
    2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx17_2 * (sigx18 * prZ/sigx20 +
      1) * prZ * sigx1_2 * ewz/sigx20), FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(nXvar + 2 *
    nuZUvar + 2 * nvZVvar + nZHvar), (nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(nXvar + 2 * nuZUvar + 2 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1, STATS = wHvar *
    sigx18 * (1 - (sigx18 * prZ/sigx20 + 1) * ewz) * prZ *
    ewz/sigx20, FUN = "*"), Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
...
## logit specification class membership
cnsfraynormAlgOpt_logit <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(ccnsfraynormlike_logit(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(ccnsfraynormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradcnsfraynormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = ccnsfraynormlike_logit,
      grad = cgradcnsfraynormlike_logit, hess = chesscnsfraynormlike_logit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(ccnsfraynormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradcnsfraynormlike_logit(parm,
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesscnsfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesscnsfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(ccnsfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradcnsfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesscnsfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradcnsfraynormlike_logit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chesscnsfraynormlike_logit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- ccnsfraynormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradcnsfraynormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
## cauchit specification class membership
cnsfraynormAlgOpt_cauchit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initRay <- start_st$initRay
  startVal <- start_st$StartVal
  startLoglik <- sum(ccnsfraynormlike_cauchit(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("CNSF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(ccnsfraynormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradcnsfraynormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = ccnsfraynormlike_cauchit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(ccnsfraynormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradcnsfraynormlike_cauchit(parm,
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesscnsfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesscnsfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(ccnsfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradcnsfraynormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesscnsfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradcnsfraynormlike_cauchit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chesscnsfraynormlike_cauchit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- ccnsfraynormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradcnsfraynormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## probit specification class membership
cnsfraynormAlgOpt_probit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initRay <- start_st$initRay
  startVal <- start_st$StartVal
  startLoglik <- sum(ccnsfraynormlike_probit(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("CNSF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(ccnsfraynormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradcnsfraynormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = ccnsfraynormlike_probit,
      grad = cgradcnsfraynormlike_probit, hess = chesscnsfraynormlike_probit,
      start = startVal, finalHessian = if (hessianType ==
        2) "bhhh" else TRUE, control = list(printLevel = if (printInfo) 2 else 0,
        iterlim = itermax, reltol = tol, tol = tol, qac = qac),
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(ccnsfraynormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradcnsfraynormlike_probit(parm,
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesscnsfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesscnsfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(ccnsfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradcnsfraynormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesscnsfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradcnsfraynormlike_probit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chesscnsfraynormlike_probit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- ccnsfraynormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradcnsfraynormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## cloglog specification class membership
cnsfraynormAlgOpt_cloglog <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initRay <- start_st$initRay
  startVal <- start_st$StartVal
  startLoglik <- sum(ccnsfraynormlike_cloglog(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("CNSF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(ccnsfraynormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradcnsfraynormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = ccnsfraynormlike_cloglog,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(ccnsfraynormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradcnsfraynormlike_cloglog(parm,
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesscnsfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradcnsfraynormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesscnsfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(ccnsfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradcnsfraynormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesscnsfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradcnsfraynormlike_cloglog(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chesscnsfraynormlike_cloglog(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- ccnsfraynormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradcnsfraynormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## logit specification class membership
mcesfraynormAlgOpt_logit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initRay <- start_st$initRay
  startVal <- start_st$StartVal
  startLoglik <- sum(cmcesfraynormlike_logit(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmcesfraynormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmcesfraynormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmcesfraynormlike_logit,
      grad = cgradmcesfraynormlike_logit, hess = chessmcesfraynormlike_logit,
      start = startVal, finalHessian = if (hessianType ==
        2) "bhhh" else TRUE, control = list(printLevel = if (printInfo) 2 else 0,
        iterlim = itermax, reltol = tol, tol = tol, qac = qac),
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmcesfraynormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmcesfraynormlike_logit(parm,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmcesfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmcesfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmcesfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmcesfraynormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmcesfraynormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmcesfraynormlike_logit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmcesfraynormlike_logit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmcesfraynormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmcesfraynormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## cauchit specification class membership
mcesfraynormAlgOpt_cauchit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initRay <- start_st$initRay
  startVal <- start_st$StartVal
  startLoglik <- sum(cmcesfraynormlike_cauchit(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MCESF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmcesfraynormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmcesfraynormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmcesfraynormlike_cauchit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmcesfraynormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmcesfraynormlike_cauchit(parm,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmcesfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmcesfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmcesfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmcesfraynormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmcesfraynormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmcesfraynormlike_cauchit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmcesfraynormlike_cauchit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmcesfraynormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmcesfraynormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## probit specification class membership
mcesfraynormAlgOpt_probit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initRay <- start_st$initRay
  startVal <- start_st$StartVal
  startLoglik <- sum(cmcesfraynormlike_probit(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MCESF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmcesfraynormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmcesfraynormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmcesfraynormlike_probit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmcesfraynormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmcesfraynormlike_probit(parm,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmcesfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmcesfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmcesfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmcesfraynormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmcesfraynormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmcesfraynormlike_probit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmcesfraynormlike_probit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmcesfraynormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmcesfraynormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## cloglog specification class membership
mcesfraynormAlgOpt_cloglog <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initRay <- start_st$initRay
  startVal <- start_st$StartVal
  startLoglik <- sum(cmcesfraynormlike_cloglog(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MCESF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmcesfraynormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmcesfraynormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmcesfraynormlike_cloglog,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmcesfraynormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmcesfraynormlike_cloglog(parm,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmcesfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmcesfraynormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmcesfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmcesfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmcesfraynormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmcesfraynormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmcesfraynormlike_cloglog(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmcesfraynormlike_cloglog(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmcesfraynormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmcesfraynormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
