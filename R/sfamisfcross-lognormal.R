...
cmisflognormlike_logit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
...
cmisflognormlike_cauchit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
cmisflognormlike_probit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
cmisflognormlike_cloglog <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
#' @noRd
cstmisflognorm <- function(olsObj, epsiRes, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
...
#' @noRd
# logit specification class membership
cgradmisflognormlike_logit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  sDiv1 <- apply(sweep(F3_1, MARGIN = 1, STATS = 1/ewv_h, FUN = "*"), 1, sum)
  sDiv2 <- apply(sweep(F3_2, MARGIN = 1, STATS = 1/ewv_h, FUN = "*"), 1, sum)
  F5_1 <- sweep(F3_1 * F2_1, MARGIN = 1, STATS = 1/ewv_h^3, FUN = "*")
  F5_2 <- sweep(F3_2 * F2_2, MARGIN = 1, STATS = 1/ewv_h^3, FUN = "*")
  sigx1 <- (prC * sDiv2 + ewz * sDiv1/wzdeno)
  F6_1 <- sweep(F3_1 * F1_1 * F2_1, MARGIN = 1, STATS = 1/ewv_h^3, FUN = "*")
  F6_2 <- sweep(F3_2 * F1_2 * F2_2, MARGIN = 1, STATS = 1/ewv_h^3, FUN = "*")
  F7_1 <- sweep(F3_1 * F1_1 * qnorm(FiMat) * F2_1, MARGIN = 1, STATS = ewu1_h/ewv_h^3,
    FUN = "*")
  F7_2 <- sweep(F3_2 * F1_2 * qnorm(FiMat) * F2_2, MARGIN = 1, STATS = ewu2_h/ewv_h^3,
    FUN = "*")
  F8_1 <- sweep(sweep(F3_1 * F2_1^2, MARGIN = 1, STATS = (0.5 * (1/ewv_h^2)), FUN = "*") -
    0.5 * F3_1, MARGIN = 1, STATS = 1/ewv_h, FUN = "*")
  F8_2 <- sweep(sweep(F3_2 * F2_2^2, MARGIN = 1, STATS = (0.5 * (1/ewv_h^2)), FUN = "*") -
    0.5 * F3_2, MARGIN = 1, STATS = 1/ewv_h, FUN = "*")
  ...
  gradll <- cbind(gx, gmu1, gmu2, gu1, gu2, gv1 + gv2, sweep(Zvar, MARGIN = 1,
    STATS = prC * ewz * (sDiv1 - sDiv2)/(sigx1 * wzdeno), FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

# cauchit specification class membership
cgradmisflognormlike_cauchit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  ...
  sDiv1 <- apply(sweep(F3_1, MARGIN = 1, STATS = 1/ewvsr, FUN = "*"), 1, sum)
  sDiv2 <- apply(sweep(F3_2, MARGIN = 1, STATS = 1/ewvsr, FUN = "*"), 1, sum)
  F5_1 <- sweep(F3_1 * F2_1, MARGIN = 1, STATS = 1/ewvsr^3, FUN = "*")
  F5_2 <- sweep(F3_2 * F2_2, MARGIN = 1, STATS = 1/ewvsr^3, FUN = "*")
  sigx1 <- (ewz2 * sDiv2 + ewz1 * sDiv1)
  F6_1 <- sweep(F3_1 * F1_1 * F2_1, MARGIN = 1, STATS = 1/ewvsr^3, FUN = "*")
  F6_2 <- sweep(F3_2 * F1_2 * F2_2, MARGIN = 1, STATS = 1/ewvsr^3, FUN = "*")
  F7_1 <- sweep(F3_1 * F1_1 * qnorm(FiMat) * F2_1, MARGIN = 1, STATS = ewusr1/ewvsr^3,
    FUN = "*")
  F7_2 <- sweep(F3_2 * F1_2 * qnorm(FiMat) * F2_2, MARGIN = 1, STATS = ewusr2/ewvsr^3,
    FUN = "*")
  F8_1 <- sweep(sweep(F3_1 * F2_1^2, MARGIN = 1, STATS = (0.5 * (1/ewvsr^2)), FUN = "*") -
    0.5 * F3_1, MARGIN = 1, STATS = 1/ewvsr, FUN = "*")
  F8_2 <- sweep(sweep(F3_2 * F2_2^2, MARGIN = 1, STATS = (0.5 * (1/ewvsr^2)), FUN = "*") -
    0.5 * F3_2, MARGIN = 1, STATS = 1/ewvsr, FUN = "*")
  gx1 <- matrix(nrow = N, ncol = nXvar)
  gx2 <- matrix(nrow = N, ncol = nXvar)
  for (k in 1:nXvar) {
    gx1[, k] <- apply(sweep(F5_1, MARGIN = 1, STATS = Xvar[, k], FUN = "*"),
  ...
  gradll <- cbind(gx, gmu1, gmu2, gu1, gu2, gv1 + gv2, sweep(Zvar, MARGIN = 1,
    STATS = (sDiv1 - sDiv2)/(pi * sigx1 * ((Wz)^2 + 1)), FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

# probit specification class membership
cgradmisflognormlike_probit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
# cloglog specification class membership
cgradmisflognormlike_cloglog <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  gradll <- cbind(gx, gmu1, gmu2, gu1, gu2, gv1 + gv2, sweep(Zvar, MARGIN = 1,
    STATS = prZ * ewz * (sDiv1 - sDiv2)/((1 - prZ) * sDiv1 + prZ * sDiv2), FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}
...
## logit specification class membership
misflognormAlgOpt_logit <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  startLoglik <- sum(cmisflognormlike_logit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisflognormlike_logit(parm,
    ...
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_logit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    ...
    xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisflognormlike_logit,
    grad = cgradmisflognormlike_logit, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) -sum(cmisflognormlike_logit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar,
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N,
      FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmisflognormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(cgradmisflognormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      ...
      fn = function(parm) -sum(cmisflognormlike_logit(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmisflognormlike_logit(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      print.info = printInfo, maxiter = itermax, epsa = gradtol, epsb = gradtol),
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisflognormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gradient = function(parm) -colSums(cgradmisflognormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisflognormlike_logit(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
  ...
  if (hessianType != 2) {
    ...
    if (method == "nlminb")
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(cgradmisflognormlike_logit(parm,
  ...
  mleObj$logL_OBS <- cmisflognormlike_logit(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
    nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisflognormlike_logit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)
...
## cauchit specification class membership
misflognormAlgOpt_cauchit <- function(start, randStart, sdStart, olsParam, dataTable,
  S, wHvar, nXvar, N, FiMat, uHvar, nuZUvar, vHvar, nvZVvar, muHvar, nmuZUvar,
  Zvar, nZHvar, Yvar, Xvar, method, printInfo, itermax, stepmax, whichStart, initIter,
  initAlg, tol, gradtol, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(cmisflognormlike_cauchit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
    nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisflognormlike_cauchit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_cauchit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), hessian = if (hessianType !=
    2) 1 else 0, control = list(trace = printInfo, maxeval = itermax, stepmax = stepmax,
    xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisflognormlike_cauchit,
    grad = cgradmisflognormlike_cauchit, start = startVal, finalHessian = if (hessianType ==
      2) "bhhh" else TRUE, control = list(printLevel = if (printInfo) 2 else 0,
      iterlim = itermax, reltol = tol, tol = tol, qac = qac), nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar), sr1 = trustOptim::trust.optim(x = startVal,
    fn = function(parm) -sum(cmisflognormlike_cauchit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar,
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N,
      FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), method = "SR1",
    control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
      prec = tol, report.level = if (printInfo) 4L else 0, report.precision = 1L)),
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmisflognormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(cgradmisflognormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), unname(parm)),
      "dgCMatrix"), method = "Sparse", control = list(maxit = itermax, cgtol = gradtol,
      stop.trust.radius = tol, prec = tol, report.level = if (printInfo) 4L else 0,
      report.precision = 1L, preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal,
      fn = function(parm) -sum(cmisflognormlike_cauchit(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmisflognormlike_cauchit(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      print.info = printInfo, maxiter = itermax, epsa = gradtol, epsb = gradtol),
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisflognormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gradient = function(parm) -colSums(cgradmisflognormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), control = list(iter.max = itermax,
      trace = printInfo, eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisflognormlike_cauchit(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method == "ucminf")
      mleObj$hessian <- mleObj$hessian
    if (method == "nlminb")
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(cgradmisflognormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar, Yvar = Yvar,
        Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
        nZHvar = nZHvar)), unname(mleObj$par))
  ...
  mleObj$logL_OBS <- cmisflognormlike_cauchit(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
    nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisflognormlike_cauchit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik, mleObj = mleObj,
...
## probit specification class membership
misflognormAlgOpt_probit <- function(start, randStart, sdStart, olsParam, dataTable,
  S, wHvar, nXvar, N, FiMat, uHvar, nuZUvar, vHvar, nvZVvar, muHvar, nmuZUvar,
  Zvar, nZHvar, Yvar, Xvar, method, printInfo, itermax, stepmax, whichStart, initIter,
  initAlg, tol, gradtol, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(cmisflognormlike_probit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
    nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisflognormlike_probit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_probit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), hessian = if (hessianType !=
    2) 1 else 0, control = list(trace = printInfo, maxeval = itermax, stepmax = stepmax,
    xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisflognormlike_probit,
    grad = cgradmisflognormlike_probit, start = startVal, finalHessian = if (hessianType ==
      2) "bhhh" else TRUE, control = list(printLevel = if (printInfo) 2 else 0,
      iterlim = itermax, reltol = tol, tol = tol, qac = qac), nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar), sr1 = trustOptim::trust.optim(x = startVal,
    fn = function(parm) -sum(cmisflognormlike_probit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar,
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N,
      FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), method = "SR1",
    control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
      prec = tol, report.level = if (printInfo) 4L else 0, report.precision = 1L)),
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmisflognormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(cgradmisflognormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), unname(parm)),
      "dgCMatrix"), method = "Sparse", control = list(maxit = itermax, cgtol = gradtol,
      stop.trust.radius = tol, prec = tol, report.level = if (printInfo) 4L else 0,
      report.precision = 1L, preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal,
      fn = function(parm) -sum(cmisflognormlike_probit(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmisflognormlike_probit(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      print.info = printInfo, maxiter = itermax, epsa = gradtol, epsb = gradtol),
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisflognormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gradient = function(parm) -colSums(cgradmisflognormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), control = list(iter.max = itermax,
      trace = printInfo, eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisflognormlike_probit(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- cmisflognormlike_probit(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
    nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisflognormlike_probit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik, mleObj = mleObj,
...
## cloglog specification class membership
misflognormAlgOpt_cloglog <- function(start, randStart, sdStart, olsParam, dataTable,
  S, wHvar, nXvar, N, FiMat, uHvar, nuZUvar, vHvar, nvZVvar, muHvar, nmuZUvar,
  Zvar, nZHvar, Yvar, Xvar, method, printInfo, itermax, stepmax, whichStart, initIter,
  initAlg, tol, gradtol, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(cmisflognormlike_cloglog(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
    nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisflognormlike_cloglog(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_cloglog(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), hessian = if (hessianType !=
    2) 1 else 0, control = list(trace = printInfo, maxeval = itermax, stepmax = stepmax,
    xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisflognormlike_cloglog,
    grad = cgradmisflognormlike_cloglog, start = startVal, finalHessian = if (hessianType ==
      2) "bhhh" else TRUE, control = list(printLevel = if (printInfo) 2 else 0,
      iterlim = itermax, reltol = tol, tol = tol, qac = qac), nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar), sr1 = trustOptim::trust.optim(x = startVal,
    fn = function(parm) -sum(cmisflognormlike_cloglog(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar,
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N,
      FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), method = "SR1",
    control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
      prec = tol, report.level = if (printInfo) 4L else 0, report.precision = 1L)),
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmisflognormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisflognormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(cgradmisflognormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), unname(parm)),
      "dgCMatrix"), method = "Sparse", control = list(maxit = itermax, cgtol = gradtol,
      stop.trust.radius = tol, prec = tol, report.level = if (printInfo) 4L else 0,
      report.precision = 1L, preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal,
      fn = function(parm) -sum(cmisflognormlike_cloglog(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmisflognormlike_cloglog(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S,
        wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)),
      print.info = printInfo, maxiter = itermax, epsa = gradtol, epsb = gradtol),
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisflognormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), gradient = function(parm) -colSums(cgradmisflognormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)), control = list(iter.max = itermax,
      trace = printInfo, eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisflognormlike_cloglog(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- cmisflognormlike_cloglog(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar, muHvar = muHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, N = N, FiMat = FiMat, Zvar = Zvar,
    nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisflognormlike_cloglog(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    N = N, FiMat = FiMat, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik, mleObj = mleObj,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in 1:object$Nobs) {
      ...
      teBC_reciprocal_c1[i] <- hcubature(f = fnCondBCreciprocalEffLogNorm,
        ...
        sigmaV = exp(Wv[i]/2), mu = mu1[i], epsilon = epsilon[i], S = object$S,
      ...
      teBC_reciprocal_c2[i] <- hcubature(f = fnCondBCreciprocalEffLogNorm,
        ...
        sigmaV = exp(Wv[i]/2), mu = mu2[i], epsilon = epsilon[i], S = object$S,
