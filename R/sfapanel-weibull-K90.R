...
#' @noRd
pweibullnormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstweibullnorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param FiMat matrix of random draws
#' @noRd
pgradweibullnormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  lFi1 <- sweep(lFimat, MARGIN = 1, STATS = (S * ewu_h * gid1_epsi_gisq),
    FUN = "*")
  lFi2 <- sweep(lFimat, MARGIN = 1, STATS = (S * ewu_h * gid1sq_epsi_gisq),
    FUN = "*")
  ...
  lFi7 <- sweep((lFiepsi * lFiuv), MARGIN = 1, STATS = 2 *
    (ewv/(2 * ewv)^2), FUN = "*")
  lFi8 <- sweep(lFimat2, MARGIN = 1, STATS = ewu_h * gisq,
    FUN = "*")
  lFi9 <- sweep(((lFi8 + lFi5) * lFiuv), MARGIN = 1, STATS = (ewu_h/(2 *
    ewv)), FUN = "*")
...
#' @noRd
weibullnormAlgOpt_k90 <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(pweibullnormlike_k90(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(pweibullnormlike_k90(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)),
    gr = function(parm) -colSums(pgradweibullnormlike_k90(parm,
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pweibullnormlike_k90,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pweibullnormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      ...
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradweibullnormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      ...
      fn = function(parm) -sum(pweibullnormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p, N = N, FiMat = FiMat_N)), print.info = printInfo,
      maxiter = itermax, epsa = gradtol, epsb = gradtol),
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(pweibullnormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradweibullnormlike_k90(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  ...
  mleObj$logL_OBS <- pweibullnormlike_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradweibullnormlike_k90(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
    FiMat = FiMat_N)
...
#' @noRd
fnCondEffWeibull_k90 <- function(u, sigmaU, sigmaV, k, S, TT,
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffWeibull_k90 <- function(u, sigmaU, sigmaV, k, S, TT,
  epsilon_isq, giepsi, gisq, git) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffWeibull_k90 <- function(u, sigmaU, sigmaV,
...
pweibullnormeff_k90 <- function(object, level) {
  ...
  for (i in seq_along(1:object$Nid)) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) *
      ...
      exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffWeibull_k90, lowerLimit = 0,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in seq_along(1:object$Nid)) {
      res$teBC[i] <- hcubature(f = fnCondBCEffWeibull_k90,
