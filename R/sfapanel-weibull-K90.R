...
#' @noRd
pweibullnormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
pstweibullnorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, S, wHvar, printInfo, tol, N, FiMat, whichStart, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradweibullnormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, wHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  k <- parm[nXvar + nuZUvar + nvZVvar + 1]
  ...
  lFi1 <- sweep(lFimat, MARGIN = 1, STATS = (S * ewu_h * gid1_epsi_gisq), FUN = "*")
  lFi2 <- sweep(lFimat, MARGIN = 1, STATS = (S * ewu_h * gid1sq_epsi_gisq), FUN = "*")
  lFi3 <- sweep(lFiuv, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*")
  lFi4 <- sweep(lFimat2, MARGIN = 1, STATS = ewu_h * gisq, FUN = "*")
  lFi5 <- sweep(lFimat, MARGIN = 1, STATS = S * giepsi, FUN = "*")
  lFi6 <- sweep((2 * lFi4 + 2 * (lFi5)) * lFiuv * lFimat3, MARGIN = 1, STATS = ewu_h/(2 *
    (k^2 * ewv)), FUN = "*")
  lFi7 <- sweep((lFiepsi * lFiuv), MARGIN = 1, STATS = 2 * (ewv/(2 * ewv)^2), FUN = "*")
  lFi8 <- sweep(lFimat2, MARGIN = 1, STATS = ewu_h * gisq, FUN = "*")
  lFi9 <- sweep(((lFi8 + lFi5) * lFiuv), MARGIN = 1, STATS = (ewu_h/(2 * ewv)),
    FUN = "*")
  lFi10 <- sweep((lFimat)^2, MARGIN = 1, STATS = ewu_h^2 * gid2_gicub, FUN = "*")
  lFi11 <- sweep((lFimat)^2, MARGIN = 1, STATS = ewu_h^2 * gid2sq_gicub, FUN = "*")
  lFi12 <- sweep(S * lFimat, MARGIN = 1, STATS = ewu_h, FUN = "*")
  gx <- matrix(nrow = N, ncol = nXvar)
  for (k in 1:nXvar) {
    gx[, k] <- -apply(sweep(2 * (lFi12) * lFi3, MARGIN = 1, STATS = Xgi[, k],
      FUN = "*") + sweep(lFi3, MARGIN = 1, STATS = Xepsi_i[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gu <- matrix(nrow = N, ncol = nuZUvar)
  for (k in 1:nuZUvar) {
    gu[, k] <- apply(sweep(lFi9, MARGIN = 1, STATS = -uHvar[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gv <- matrix(nrow = N, ncol = nvZVvar)
  for (k in 1:nvZVvar) {
    gv[, k] <- apply(sweep(lFi7, MARGIN = 1, STATS = vHvar[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gradll <- cbind(gx, gu, gv - sweep(vHvar, MARGIN = 1, STATS = 0.5 * TT, FUN = "*"),
    apply(lFi6, 1, sum)/sdFiv, apply(-((lFi10 + 2 * lFi1) * lFi3), 1, sum)/sdFiv,
    apply(-((lFi11 + 2 * lFi2) * lFi3), 1, sum)/sdFiv)
...
#' @noRd
weibullnormAlgOpt_k90 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- pstweibullnorm_k90(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pweibullnormlike_k90(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pweibullnormlike_k90(parm,
    ...
    N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_k90(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
    N = N, FiMat = FiMat_N)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pweibullnormlike_k90,
    grad = pgradweibullnormlike_k90, start = startVal, finalHessian = if (hessianType ==
    ...
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pweibullnormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      fn = function(parm) -sum(pweibullnormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
        FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradweibullnormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      objective = function(parm) -sum(pweibullnormlike_k90(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
        N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradweibullnormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradweibullnormlike_k90(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
      Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradweibullnormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- pweibullnormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradweibullnormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffWeibull_k90 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffWeibull_k90 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq, git) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffWeibull_k90 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq,
  giepsi, gisq, git) {
  exp(git * u) * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) *
    exp(-(u/sigmaU)^k) * exp(-(epsilon_isq + 2 * S * u * giepsi + u^2 * gisq)/(2 *
    sigmaV^2))
...
#' @noRd
pweibullnormeff_k90 <- function(object, level) {
  ...
  for (i in seq_along(1:object$Nid)) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * giepsi[i] + ur^2 * gisq[i])/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffWeibull_k90, lowerLimit = 0, upperLimit = Inf,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in seq_along(1:object$Nid)) {
      res$teBC[i] <- hcubature(f = fnCondBCEffWeibull_k90, lowerLimit = 0,
      ...
      res$teBC_reciprocal[i] <- hcubature(f = fnCondBCreciprocalEffWeibull_k90,
