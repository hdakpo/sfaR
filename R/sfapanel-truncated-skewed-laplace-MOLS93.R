...
#' @noRd
ptslnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
psttslnorm_mols93 <- function(olsObj, epsiRes, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, ngZGvar, gHvar, S, wHvar,
  initIter, initAlg, whichStart, printInfo, tol) {
  if (whichStart == 1L) {
...
#' @param TT vector of time of presence
#' @noRd
pgradtslnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar,
  gHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
...
#' @param TT vector of time of presence
#' @noRd
phesstslnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar,
  gHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  ...
  return(hessll[-(nXvar + nuZUvar + nvZVvar + ngZGvar + 1),
    -(nXvar + nuZUvar + nvZVvar + ngZGvar + 1)])
}
...
#' @noRd
tslnormAlgOpt_mols93 <- function(start, olsParam, dataTable,
  S, nXvar, uHvar_c, uHvar_p, nuZUvar, vHvar_c, vHvar_p, nvZVvar,
  gHvar, ngZGvar, Yvar, Xvar, pindex, TT, wHvar_c, wHvar_p,
  method, printInfo, itermax, stepmax, tol, gradtol, whichStart,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psttslnorm_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      ...
      wHvar = wHvar_c, ngZGvar = ngZGvar, initIter = initIter,
      initAlg = initAlg, gHvar = gHvar, whichStart = whichStart,
      tol = tol, printInfo = printInfo)
    InitTsl <- start_st$initTsl
    startVal <- start_st$StartVal
  }
  startLoglik <- sum(ptslnormlike_mols93(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptslnormlike_mols93,
    grad = pgradtslnormlike_mols93, hess = phesstslnormlike_mols93,
    ...
    fn = function(parm) {
      ...
      as(-phesstslnormlike_mols93(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
    ...
    fn = function(parm) {
      ...
      -phesstslnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar,
        ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtslnormlike_mols93(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstslnormlike_mols93(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    ...
    if (method == "sr1") {
      mleObj$hessian <- phesstslnormlike_mols93(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- ptslnormlike_mols93(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtslnormlike_mols93(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p)
