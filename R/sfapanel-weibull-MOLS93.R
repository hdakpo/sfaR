...
#' @noRd
pweibullnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar, gHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
...
#' @noRd
pstweibullnorm_mols93 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar,
  vHvar, Yvar, Xvar, ngZGvar, gHvar, S, wHvar, initIter, initAlg, whichStart, printInfo,
  tol, N, FiMat) {
  if (whichStart == 1L) {
    ...
    initWeibull <- maxLik::maxLik(logLik = cweibullnormlike, start = cstweibullnorm(olsObj = olsObj,
      ...
      hess = chessweibullnormlike, method = initAlg, control = list(iterlim = initIter,
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar)), paste0("Zv_",
    colnames(vHvar)), "k", paste0("eta_", colnames(gHvar[, -ngZGvar])))
...
#' @param FiMat matrix of random draws
#' @noRd
pgradweibullnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar, gHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  lFisq <- sweep(lFimat, MARGIN = 1, STATS = S * ewu_h * giepsi, FUN = "*")
  lFiu <- sweep(lFimat^2, MARGIN = 1, STATS = (ewu_h)^2 * gisq, FUN = "*")
  lFiepsi <- sweep(lFiu + 2 * (lFisq), MARGIN = 1, STATS = (epsilon_isq), FUN = "+")
  lFiuv <- exp(-sweep(lFiepsi, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*"))
  sdFiv <- apply(lFiuv, 1, sum)
  lFi1 <- sweep(lFimat, MARGIN = 1, STATS = S * ewu_h, FUN = "*")
  lFi2 <- sweep(lFiuv, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*")
  lFi3 <- sweep(lFimat2, MARGIN = 1, STATS = ewu_h * gisq, FUN = "*")
  lFi4 <- sweep(lFimat, MARGIN = 1, STATS = S * giepsi, FUN = "*")
  lFi5 <- sweep(((lFi3 + lFi4) * lFiuv), MARGIN = 1, STATS = ewu_h/(2 * ewv), FUN = "*")
  lFi6 <- sweep(lFiepsi * lFiuv, MARGIN = 1, STATS = 2 * (ewv/(2 * ewv)^2), FUN = "*")
  lFi7 <- sweep((2 * (lFi3) + 2 * (lFi4)) * lFiuv * lFimat3, MARGIN = 1, STATS = ewu_h/(2 *
    (k^2 * ewv)), FUN = "*")
  ZZ1 <- list()
  for (i in 1:ngZGvar) {
    ZZ1[[i]] <- sweep(lFimat^2, MARGIN = 1, STATS = (ewu_h)^2 * Zigisq[, i],
      FUN = "*")
  }
  ZZ2 <- list()
  for (i in 1:ngZGvar) {
    ZZ2[[i]] <- sweep(lFi1, MARGIN = 1, STATS = 2 * (Zigiepsi[, i]), FUN = "*")
  }
  ZZ3 <- list()
  for (i in 1:ngZGvar) {
  ...
  for (k in 1:nXvar) {
    gx[, k] <- -apply(sweep(2 * (lFi1) * lFi2, MARGIN = 1, STATS = Xgi[, k],
      FUN = "*") + sweep(lFi2, MARGIN = 1, STATS = 2 * Xepsi_i[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gu <- matrix(nrow = N, ncol = nuZUvar)
  for (k in 1:nuZUvar) {
    gu[, k] <- apply(sweep(lFi5, MARGIN = 1, STATS = -uHvar[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gv <- matrix(nrow = N, ncol = nvZVvar)
  for (k in 1:nvZVvar) {
    gv[, k] <- apply(sweep(lFi6, MARGIN = 1, STATS = vHvar[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gradll <- cbind(gx, gu, gv - sweep(vHvar, MARGIN = 1, STATS = 0.5 * TT, FUN = "*"),
    apply(lFi7, 1, sum)/sdFiv, sweep(-lFi8, MARGIN = 1, STATS = 1/sdFiv, FUN = "*"))
...
#' @noRd
weibullnormAlgOpt_mols93 <- function(start, randStart, sdStart, olsParam, dataTable,
  S, gHvar, ngZGvar, nXvar, N, NT, FiMat_N, FiMat_NT, uHvar_c, uHvar_p, nuZUvar,
  vHvar_c, vHvar_p, nvZVvar, pindex, TT, Yvar, Xvar, wHvar_c, wHvar_p, method,
  printInfo, itermax, whichStart, initIter, initAlg, stepmax, tol, gradtol, hessianType,
  qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
    start_st <- pstweibullnorm_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      N = NT, FiMat = FiMat_NT, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      gHvar = gHvar, ngZGvar = ngZGvar, uHvar = uHvar_c, initIter = initIter,
      initAlg = initAlg, vHvar = vHvar_c, Yvar = Yvar, Xvar = Xvar, S = S,
      wHvar = wHvar_c, whichStart = whichStart, tol = tol, printInfo = printInfo)
    initWeibull <- start_st$initWeibull
    startVal <- start_st$StartVal
  }
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(pweibullnormlike_mols93(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pweibullnormlike_mols93(parm,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pweibullnormlike_mols93,
    grad = pgradweibullnormlike_mols93, start = startVal, finalHessian = if (hessianType ==
      2) "bhhh" else TRUE, control = list(printLevel = if (printInfo) 2 else 0,
      iterlim = itermax, reltol = tol, tol = tol, qac = qac), nXvar = nXvar,
    gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    ...
    fn = function(parm) -sum(pweibullnormlike_mols93(parm, nXvar = nXvar, gHvar = gHvar,
      ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), method = "SR1",
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), unname(parm)),
      ...
      fn = function(parm) -sum(pweibullnormlike_mols93(parm, nXvar = nXvar,
        gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      ...
      epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(pweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), control = list(iter.max = itermax,
      trace = if (printInfo) 1 else 0, eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradweibullnormlike_mols93(mleObj$par, nXvar = nXvar,
      gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradweibullnormlike_mols93(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
        FiMat = FiMat_N)), mleObj$par)
    if (method == "sr1")
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradweibullnormlike_mols93(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
        FiMat = FiMat_N)), mleObj$solution)
  }
  mleObj$logL_OBS <- pweibullnormlike_mols93(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradweibullnormlike_mols93(parm = mlParam, nXvar = nXvar,
    gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffWeibull_mols93 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq) {
  u * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) * exp(-(u/sigmaU)^k) *
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffWeibull_mols93 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq, git) {
  exp(-git * u) * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) *
    exp(-(u/sigmaU)^k) * exp(-(epsilon_isq + 2 * S * u * giepsi + u^2 * gisq)/(2 *
    sigmaV^2))
}
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffWeibull_mols93 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq,
  giepsi, gisq, git) {
  exp(git * u) * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) *
    exp(-(u/sigmaU)^k) * exp(-(epsilon_isq + 2 * S * u * giepsi + u^2 * gisq)/(2 *
    sigmaV^2))
...
#' @noRd
pweibullnormeff_mols93 <- function(object, level) {
  ...
  for (i in seq_along(1:object$Nid)) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * giepsi[i] + ur^2 * gisq[i])/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffWeibull_mols93, lowerLimit = 0, upperLimit = Inf,
      maxEval = 100, fDim = 1, sigmaU = exp(Wu[i]/2), sigmaV = exp(Wv[i]/2),
      k = k, epsilon_isq = epsilon_isq[i], giepsi = giepsi[i], gisq = gisq[i],
      TT = TT[i], S = object$S, vectorInterface = FALSE, tol = 1e-15)$integral/density_epsilon_vec[i]
  }
  res <- data.frame(levels(pindex[, 1]), u = u, giepsi = giepsi, gisq = gisq, epsilon_isq = epsilon_isq,
    density_epsilon_vec = density_epsilon_vec, Wu = Wu, Wv = Wv, TT = TT)
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in seq_along(1:object$Nid)) {
      res$teBC[i] <- hcubature(f = fnCondBCEffWeibull_mols93, lowerLimit = 0,
        upperLimit = Inf, maxEval = 100, fDim = 1, sigmaU = exp(res$Wu[i]/2),
        sigmaV = exp(res$Wv[i]/2), k = k, epsilon_isq = res$epsilon_isq[i],
        giepsi = res$giepsi[i], gisq = res$gisq[i], TT = res$TT[i], S = object$S,
        git = git[i], vectorInterface = FALSE, tol = 1e-15)$integral/density_epsilon_vec[i]
      res$teBC_reciprocal[i] <- hcubature(f = fnCondBCreciprocalEffWeibull_mols93,
