...
#' @noRd
pweibullnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstweibullnorm_mols93 <- function(olsObj, epsiRes, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, ngZGvar, gHvar, S, wHvar,
  initIter, initAlg, whichStart, printInfo, tol, N, FiMat) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradweibullnormlike_mols93 <- function(parm, nXvar, nuZUvar,
...
#' @noRd
weibullnormAlgOpt_mols93 <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(pweibullnormlike_mols93(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(pweibullnormlike_mols93(parm,
    ...
    gr = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)),
    hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pweibullnormlike_mols93,
    grad = pgradweibullnormlike_mols93, start = startVal,
    ...
    gr = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      ...
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)),
    hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradweibullnormlike_mols93(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)),
    ...
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradweibullnormlike_mols93(parm,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradweibullnormlike_mols93(mleObj$par,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N))
  ...
  mleObj$logL_OBS <- pweibullnormlike_mols93(parm = mlParam,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradweibullnormlike_mols93(parm = mlParam,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
...
#' @noRd
fnCondEffWeibull_mols93 <- function(u, sigmaU, sigmaV, k, S,
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffWeibull_mols93 <- function(u, sigmaU, sigmaV, k, S,
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffWeibull_mols93 <- function(u, sigmaU, sigmaV,
...
pweibullnormeff_mols93 <- function(object, level) {
  ...
  for (i in seq_along(1:object$Nid)) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) *
      ...
      exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffWeibull_mols93, lowerLimit = 0,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in seq_along(1:object$Nid)) {
      res$teBC[i] <- hcubature(f = fnCondBCEffWeibull_mols93,
