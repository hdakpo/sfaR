...
#' @noRd
pgenexponormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
pstgenexponorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar,
  vHvar, Yvar, Xvar, S, wHvar, printInfo, tol, whichStart, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param TT vector of time of presence
#' @noRd
pgradgenexponormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  sigx15 <- (S * Zi_epsi/sgsq - (ewv/ewu_h + S * giepsi) * (sqrt(ewv/gisq) - 0.5 *
    (ewv/sgsq)) * giZi/sgsq^2)
  sigx16 <- ((2 * (ewv/ewu_h) + S * giepsi) * pmusig2/sgsq - dmusig2)
  sigx17 <- (S * Zisq_epsi/sgsq - (ewv/ewu_h + S * giepsi) * (sqrt(ewv/gisq) -
    0.5 * (ewv/sgsq)) * giZisq/sgsq^2)
  sigx18 <- (S * Zi_epsi/sgsq - (2 * (ewv/ewu_h) + S * giepsi) * (sqrt(ewv/gisq) -
    0.5 * (ewv/sgsq)) * giZi/sgsq^2)
  sigx19 <- (S * Zisq_epsi/sgsq - (2 * (ewv/ewu_h) + S * giepsi) * (sqrt(ewv/gisq) -
    0.5 * (ewv/sgsq)) * giZisq/sgsq^2)
  ...
  gradll <- cbind(sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = 1/sigx4, FUN = "*"),
    sweep(uHvar, MARGIN = 1, STATS = ((sigx5 * sigx3 - sigx6 * sigx2)/(sigx4 *
      ...
      0.5 * (TT - 1)), FUN = "*"), sigx20/sigx4 - 0.5 * (giZi/gisq), sigx21/sigx4 -
      0.5 * (giZisq/gisq))
...
#' @param TT vector of time of presence
#' @noRd
phessgenexponormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx30 <- ((0.5 * (dmusig1 * sigx22/sgsq) - 0.5 * pmusig1) * ewv/sqrt(ewv/gisq) -
  ...
  sigx36 <- ((sigx22 * sigx15/(ewv * gisq) - sigx24 * giZi/sgsq^2) * sigx22 * pmusig1 +
    (S * pmusig1 * Zi_epsi - dmusig1 * sigx22 * sigx15)/sgsq)
  sigx37 <- ((0.5 * (sigx24/sgsq^2) - 0.5/sigx27) * ewv * sigx22 * giZi + S * sigx24 *
    Zi_epsi)
  sigx38 <- ((0.5 * (sigx24/sgsq^2) - 0.5/sigx27) * sigx26 * ewv * giZi + S * sigx24 *
    Zi_epsi)
  sigx39 <- (Zisq - 2 * (sqrt(ewv/gisq) * sigx24 * gisq * giZi^2/sgsq^2))
  sigx40 <- ((sigx26 * sigx18/(ewv * gisq) - sigx24 * giZi/sgsq^2) * sigx26 * pmusig2 +
    (S * pmusig2 * Zi_epsi - sigx26 * dmusig2 * sigx18)/sgsq)
  Xsig0 <- sweep(Xgi, MARGIN = 1, STATS = (S * (2 * (ewv/ewu_h) + S * giepsi)/gisq),
    FUN = "*")
  Xsig1 <- sweep(Xgi, MARGIN = 1, STATS = (S * (ewv/ewu_h + S * giepsi)/gisq),
    FUN = "*")
  ...
  Xsig12 <- sweep((Xsig9 - 0.5 * Xsig10), MARGIN = 1, STATS = sigx17, FUN = "*")
  Xsig13 <- sweep(XiZisq, MARGIN = 1, STATS = 1/sgsq, FUN = "*")
  Xsig14 <- sweep(Xgi, MARGIN = 1, STATS = sigx24 * giZi/sgsq^2, FUN = "*")
  Xsig15 <- sweep(Xgi, MARGIN = 1, STATS = sigx24 * giZisq/sgsq^2, FUN = "*")
  Xsig16 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = sigx20/sigx4, FUN = "*")
  ...
  Xsig22 <- sweep(Xgi, MARGIN = 1, STATS = (0.5 * (dmusig1 * sigx22/sgsq) + 0.5 *
  ...
  Xsig25 <- sweep((Xsig23 - 0.5 * Xsig24), MARGIN = 1, STATS = sigx18, FUN = "*")
  Xsig26 <- sweep((Xsig23 - 0.5 * Xsig24), MARGIN = 1, STATS = sigx19, FUN = "*")
  Xsig27 <- sweep((0.5 * Xsig20 + Xsig21), MARGIN = 1, STATS = sigx2, FUN = "*")
  Xsig28 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = sigx21/sigx4, FUN = "*")
  Xsig29 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = sigx13/sigx4, FUN = "*")
  Xsig30 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = (sigx5 * sigx3 - sigx6 *
    sigx2) * ewu_h * gisq/(sigx4 * ewu_h * gisq)^2, FUN = "*")
  Xsig31 <- sweep(XiZi, MARGIN = 1, STATS = 1/sgsq, FUN = "*")
  Xsig33 <- sweep((Xsig31 - Xsig14), MARGIN = 1, STATS = S * sigx14, FUN = "*")
  Xsig34 <- sweep((Xsig31 - Xsig14), MARGIN = 1, STATS = S * sigx16, FUN = "*")
  Xsig35 <- sweep((Xsig25 + Xsig34), MARGIN = 1, STATS = sigx2, FUN = "*")
  Xsig36 <- sweep((Xsig11 + Xsig33), MARGIN = 1, STATS = sigx3, FUN = "*")
  ...
  Xsig40 <- sweep(((Xepsi_i - 2 * Xsig1)), MARGIN = 1, STATS = sigx5/ewv, FUN = "*")
  Xsig41 <- sweep(Xgi, MARGIN = 1, STATS = (sigx26 * sigx12/(ewv * gisq) + 0.5/sgsq^2) *
    dmusig2 * ewv/sqrt(ewv/gisq), FUN = "*")
  Xsig42 <- sweep(Xgi, MARGIN = 1, STATS = (S * (2/(ewu_h * gisq) - sigx26/sgsq^2)),
    FUN = "*")
  Xsig43 <- sweep((2 * Xsig42 + Xsig37), MARGIN = 1, STATS = pmusig2, FUN = "*")
  Xsig44 <- sweep(Xgi, MARGIN = 1, STATS = S * sigx25 * dmusig2/sgsq, FUN = "*")
  Xsig45 <- sweep((Xepsi_i - 2 * Xsig0), MARGIN = 1, STATS = (sigx29/ewv), FUN = "*")
  Xsig46 <- sweep((0.5 * (Xsig43 - Xsig44) + S * Xsig41 - 0.5 * Xsig45), MARGIN = 1,
    STATS = sigx2, FUN = "*")
  Xsig47 <- sweep((0.5 * (Xsig38 - Xsig39) + S * Xsig18 - 0.5 * Xsig17), MARGIN = 1,
    STATS = sigx3, FUN = "*")
  Xsig48 <- sweep((0.5 * Xsig40 + S * Xsig22), MARGIN = 1, STATS = sigx3, FUN = "*")
  Xsig49 <- sweep((Xsig13 - Xsig15), MARGIN = 1, STATS = S * sigx14, FUN = "*")
  Xsig50 <- sweep((Xsig26 + S * sigx16 * (Xsig13 - Xsig15)), MARGIN = 1, STATS = sigx2,
    FUN = "*")
  Xsig51 <- sweep((Xsig12 + Xsig49), MARGIN = 1, STATS = sigx3, FUN = "*")
  ...
  hessll[1:nXvar, 1:nXvar] <- 0.5 * (sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
    ...
    crossprod(sweep((Xepsi_i - 2 * Xsig0), MARGIN = 1, STATS = wHvar * S * dmusig2/sgsq *
      sigx2/ewv/sigx4, FUN = "*"), Xgi)) - (crossprod(sweep(0.5 * ((0.5 * Xsig4 +
    ...
    crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar * 2 * (S^2/gisq) * pmusig1 *
      sigx3/ewv/sigx4, FUN = "*"), Xgi) - crossprod(sweep((Xepsi_i - 2 * Xsig1),
    MARGIN = 1, STATS = wHvar * S * dmusig1/sgsq * sigx3/ewv/sigx4, FUN = "*"),
    Xgi)) - (crossprod(sweep(0.5 * ((0.5 * Xsig5 + Xsig6)), MARGIN = 1, STATS = wHvar *
    sigx3/ewv/sigx4, FUN = "*"), (Xepsi_i - 2 * Xsig1)) + crossprod(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S^2 * dmusig1 * sigx22/sigx27 * sigx3/ewv/sigx4,
    FUN = "*"), Xgi))) - crossprod(sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = wHvar/sigx4/sigx4,
    FUN = "*"), (Xsig7 - Xsig8))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep((Xsig27 - Xsig48),
    MARGIN = 1, STATS = wHvar/(sigx4 * ewu_h * gisq), FUN = "*") - sweep(Xsig30,
    MARGIN = 1, STATS = wHvar, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep((Xsig47 -
    (Xsig29 + Xsig46)), MARGIN = 1, STATS = wHvar/sigx4, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep((Xsig36 - (Xsig16 +
    Xsig35)), MARGIN = 1, STATS = wHvar/sigx4, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep((Xsig51 - (Xsig28 +
    Xsig50)), MARGIN = 1, STATS = wHvar/sigx4, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((0.25 * (dmusig1 * sigx22/sgsq) - 0.5 * (0.5 *
      (dmusig1 * sigx22/sgsq) - 0.5 * pmusig1)) * ewv - 0.5 * (sigx5 * sigx22/gisq)) *
      sigx3 - sigx2 * (ewv * pmusig2 - sigx26 * sigx6/gisq))/(sigx4 * ewu_h^2 *
      gisq) - ((sigx5 * sigx3 - sigx6 * sigx2)/gisq + 0.5 * (sigx4 * ewu_h)) *
      (sigx5 * sigx3 - sigx6 * sigx2) * gisq/(sigx4 * ewu_h * gisq)^2), FUN = "*"),
    uHvar)
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((sigx30 * sigx15/gisq + 0.5 * (sigx14 * ewv *
      sigx24 * giZi/sgsq^2)) * sigx3 - ((sigx31 * sigx18/sgsq + sigx16 * ewv *
      sigx24 * giZi/sgsq^2) * sigx2 + sigx20 * (sigx5 * sigx3 - sigx6 * sigx2)/(sigx4 *
      gisq)))/(sigx4 * ewu_h), FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((sigx30 * sigx17/gisq + 0.5 * (sigx14 * ewv *
      sigx24 * giZisq/sgsq^2)) * sigx3 - ((sigx31 * sigx19/sgsq + sigx16 *
      ewv * sigx24 * giZisq/sgsq^2) * sigx2 + sigx21 * (sigx5 * sigx3 - sigx6 *
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    sigx15 - (sigx33 * giZi + 0.5 * (S * Zi_epsi/sqrt(ewv/gisq))) * sigx14 *
    ewv/sgsq^2) * sigx3 - ((sigx34 * sigx18 - (sigx35 * giZi + 0.5 * (S * Zi_epsi/sqrt(ewv/gisq))) *
    sigx16 * ewv/sgsq^2) * sigx2 + sigx20 * sigx13/sigx4))/sigx4, FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 2)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = wHvar * ((sigx32 *
    sigx17 - (sigx33 * giZisq + 0.5 * (S * Zisq_epsi/sqrt(ewv/gisq))) * sigx14 *
    ewv/sgsq^2) * sigx3 - ((sigx34 * sigx19 - (sigx35 * giZisq + 0.5 * (S * Zisq_epsi/sqrt(ewv/gisq))) *
    sigx16 * ewv/sgsq^2) * sigx2 + sigx21 * sigx13/sigx4))/sigx4, FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 1)] <- sum(wHvar *
    (((sigx36 * sigx15 - (sigx37 * giZi + (sigx22 * sigx39 + S * giZi * Zi_epsi) *
      sigx24) * sigx14/sgsq^2) * sigx3 - ((sigx40 * sigx18 - (sigx38 * giZi +
      (sigx26 * sigx39 + S * giZi * Zi_epsi) * sigx24) * sigx16/sgsq^2) * sigx2 +
      sigx20^2/sigx4))/sigx4 - 0.5 * ((Zisq - giZi^2/gisq)/gisq)))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    (((sigx36 * sigx17 - (sigx37 * giZisq + (sigx22 * (Zicub - 2 * (sqrt(ewv/gisq) *
      sigx24 * gisq * giZi * giZisq/sgsq^2)) + S * giZi * Zisq_epsi) * sigx24) *
      sigx14/sgsq^2) * sigx3 - ((sigx40 * sigx19 - (sigx38 * giZisq + (sigx26 *
      (Zicub - 2 * (sqrt(ewv/gisq) * sigx24 * gisq * giZi * giZisq/sgsq^2)) +
      S * giZi * Zisq_epsi) * sigx24) * sigx16/sgsq^2) * sigx2 + sigx20 * sigx21/sigx4))/sigx4 -
      0.5 * ((Zicub - giZi * giZisq/gisq)/gisq)))
  hessll[(nXvar + nuZUvar + nvZVvar + 2), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    (((((sigx22 * sigx17/(ewv * gisq) - sigx24 * giZisq/sgsq^2) * sigx22 * pmusig1 +
      (S * pmusig1 * Zisq_epsi - dmusig1 * sigx22 * sigx17)/sgsq) * sigx17 -
      (((0.5 * (sigx24/sgsq^2) - 0.5/sigx27) * ewv * sigx22 * giZisq + S *
        sigx24 * Zisq_epsi) * giZisq + (sigx22 * (Zifour - 2 * (sqrt(ewv/gisq) *
        sigx24 * gisq * giZisq^2/sgsq^2)) + S * giZisq * Zisq_epsi) * sigx24) *
        sigx14/sgsq^2) * sigx3 - ((((sigx26 * sigx19/(ewv * gisq) - sigx24 *
      giZisq/sgsq^2) * sigx26 * pmusig2 + (S * pmusig2 * Zisq_epsi - sigx26 *
      dmusig2 * sigx19)/sgsq) * sigx19 - (((0.5 * (sigx24/sgsq^2) - 0.5/sigx27) *
      sigx26 * ewv * giZisq + S * sigx24 * Zisq_epsi) * giZisq + (sigx26 *
      (Zifour - 2 * (sqrt(ewv/gisq) * sigx24 * gisq * giZisq^2/sgsq^2)) + S *
      giZisq * Zisq_epsi) * sigx24) * sigx16/sgsq^2) * sigx2 + sigx21^2/sigx4))/sigx4 -
...
#' @noRd
genexponormAlgOpt_mbc92 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  startLoglik <- sum(pgenexponormlike_mbc92(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pgenexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradgenexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgenexponormlike_mbc92,
    grad = pgradgenexponormlike_mbc92, hess = phessgenexponormlike_mbc92, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phessgenexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
      ...
      -phessgenexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgenexponormlike_mbc92(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessgenexponormlike_mbc92(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
  ...
  mleObj$logL_OBS <- pgenexponormlike_mbc92(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradgenexponormlike_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
