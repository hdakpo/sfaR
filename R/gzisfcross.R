...
#' Generalized zero inefficiency stochastic frontier using cross-section data
#'
#' @description
...
#'
#' @aliases gzisfcross print.gzisfcross
#'
...
#' @param x an object of class gzisfcross (returned by the function \code{\link{gzisfcross}}).
#' @param ... additional arguments of frontier are passed to gzisfcross; 
...
#' 
#' \Sexpr[results=rd, stage=build]{
#' katex::math_to_rd('\\\pi(i,j) = \\\frac{\\\exp{(\\\theta_j \\\prime Z_{hi})}}{\\\sum_{m=1}^{J}\\\exp{(\\\theta_m \\\prime Z_{hi})}}')
#' }
#'
...
#'
#' @return \code{\link{gzisfcross}} returns a list of class \code{'gzisfcross'}
...
#'
#' @note In the case of panel data, \code{\link{gzisfcross}} estimates a pooled
...
#' summary results.
#'
#' \code{\link[=coef.gzisfcross]{coef}} for extracting coefficients of the
#' estimation.
#'
#' \code{\link[=efficiencies.gzisfcross]{efficiencies}} for computing
#' (in-)efficiency estimates.
#'
#' \code{\link[=fitted.gzisfcross]{fitted}} for extracting the fitted frontier
#' values.
#'
#' \code{\link[=ic.gzisfcross]{ic}} for extracting information criteria.
#'
...
#' 
#' \code{\link[=bread.gzisfcross]{bread}} for bread for sandwich estimator.
#' 
#' \code{\link[=estfun.gzisfcross]{estfun}} for gradient extraction for each 
#' observation.
#'
...
gzisfcross <- function(formula, uhet, vhet, thet, logDepVar = TRUE,
  ...
  formula <- formDist_gzisfcross(formula = formula, uhet = uhet,
    vhet = vhet, thet = thet)
  ...
  if (!(gzisfClasses %in% 2:5)) {
    stop("argument 'gzisfClasses' must be comprised between 2 and 5",
      call. = FALSE)
  }
  ...
  # quick renaming -------
  names(mleList$startVal) <- fName_gzisfcross(Xvar = Xvar,
    uHvar = uHvar, vHvar = vHvar, Zvar = Zvar, nZHvar = nZHvar,
    gzisfClasses = gzisfClasses)
  ...
  returnObj$mlParamMatrix <- rbind(matrix(mleList$mlParam[1:(gzisfClasses *
    (nXvar + nuZUvar + nvZVvar))], ncol = gzisfClasses),
    cbind(matrix(mleList$mlParam[(gzisfClasses * (nXvar +
      nuZUvar + nvZVvar) + 1):(gzisfClasses * (nXvar +
      nuZUvar + nvZVvar) + (gzisfClasses - 1) * nZHvar)],
      ncol = gzisfClasses - 1), NA))
  colnames(returnObj$mlParamMatrix) <- paste0("Class", 1:gzisfClasses)
  d1names <- names(mleList$mlParam)[c(1:(nXvar + nuZUvar +
    nvZVvar), (gzisfClasses * (nXvar + nuZUvar + nvZVvar) +
    1):(gzisfClasses * (nXvar + nuZUvar + nvZVvar) + nZHvar))]
  ...
  rm(mleList)
  class(returnObj) <- "gzisfcross"
  return(returnObj)
}

# print for gzisfcross ----------
#' @rdname gzisfcross
#' @export
print.gzisfcross <- function(x, ...) {
  cat("Call:\n")
