...
#' @noRd
pLCMhalfnormlike5C_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  beta3 <- parm[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 2 * ngZGvar - 1):(3 *
    nXvar + 2 * nuZUvar + 2 * nvZVvar + 2 * ngZGvar - 2)]
  delta3 <- parm[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 2 * ngZGvar - 1):(3 *
    nXvar + 3 * nuZUvar + 2 * nvZVvar + 2 * ngZGvar - 2)]
  phi3 <- parm[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 2 * ngZGvar - 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar + 2 * ngZGvar - 2)]
  eta3 <- parm[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 2 * ngZGvar - 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar - 3)]
  eta3[ngZGvar] <- 1
  beta4 <- parm[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar - 2):(4 *
    nXvar + 3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar - 3)]
  delta4 <- parm[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar - 2):(4 *
    nXvar + 4 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar - 3)]
  phi4 <- parm[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar - 2):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 3 * ngZGvar - 3)]
  eta4 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * ngZGvar - 2):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar - 4)]
  eta4[ngZGvar] <- 1
  beta5 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar - 3):(5 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar - 4)]
  delta5 <- parm[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar - 3):(5 *
    nXvar + 5 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar - 4)]
  phi5 <- parm[(5 * nXvar + 5 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar - 3):(5 * nXvar +
    5 * nuZUvar + 5 * nvZVvar + 4 * ngZGvar - 4)]
  eta5 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 4 * ngZGvar - 3):(5 * nXvar +
    5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar - 5)]
  eta5[ngZGvar] <- 1
  theta1 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar - 4):(5 *
    nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + nZHvar - 5)]
  theta2 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + nZHvar -
    4):(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 2 * nZHvar - 5)]
  theta3 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 2 * nZHvar -
    4):(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 3 * nZHvar - 5)]
  theta4 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 3 * nZHvar -
    4):(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 4 * nZHvar - 5)]
  ...
  git5 <- exp(as.numeric(crossprod(matrix(eta5), t(gHvar))))
...
#' @noRd
psLCMfhalfnorm5C_mols93 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
  ...
  StartVal <- c(Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar -
    ...
    1), Esti[(nXvar + 3):(nXvar + ngZGvar + 1)], 0.98 * Esti[1:(nXvar)], Esti[nXvar +
    1], if (nuZUvar > 1) rep(0, nuZUvar - 1), Esti[nXvar + 2], if (nvZVvar >
    1) rep(0, nvZVvar - 1), Esti[(nXvar + 3):(nXvar + ngZGvar + 1)], 0.98 * Esti[1:(nXvar)],
    Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar - 1), Esti[nXvar + 2], if (nvZVvar >
      1) rep(0, nvZVvar - 1), Esti[(nXvar + 3):(nXvar + ngZGvar + 1)], 0.98 *
      Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar - 1),
    Esti[nXvar + 2], if (nvZVvar > 1) rep(0, nvZVvar - 1), Esti[(nXvar + 3):(nXvar +
      ngZGvar + 1)], rep(0, 4 * nZHvar))
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar_p)),
    ...
    paste0("eta_", colnames(gHvar[, -ngZGvar])), names(Esti)[1:nXvar], paste0("Zu_",
      colnames(uHvar_p)), paste0("Zv_", colnames(vHvar_p)), paste0("eta_",
      colnames(gHvar[, -ngZGvar])), names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar_p)),
    paste0("Zv_", colnames(vHvar_p)), paste0("eta_", colnames(gHvar[, -ngZGvar])),
    names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar_p)), paste0("Zv_", colnames(vHvar_p)),
    paste0("eta_", colnames(gHvar[, -ngZGvar])), paste0("Cl1_", colnames(Zvar)),
    paste0("Cl2_", colnames(Zvar)), paste0("Cl3_", colnames(Zvar)), paste0("Cl4_",
      colnames(Zvar)))
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike5C_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  Zitgitepsit5 <- sweep(gHvar, MARGIN = 1, STATS = git5 * epsilon_it5, FUN = "*")
  Zigiepsi5 <- apply(Zitgitepsit5, 2, function(x) tapply(x, pindex[, 1], sum))
  Zitgitsq5 <- sweep(gHvar, MARGIN = 1, STATS = 2 * git5^2, FUN = "*")
  Zigisq5 <- apply(Zitgitsq5, 2, function(x) tapply(x, pindex[, 1], sum))
  ewu1 <- exp(Wu1)
  ewu2 <- exp(Wu2)
  ewu3 <- exp(Wu3)
  ewu4 <- exp(Wu4)
  ...
  sigx1_5 <- exp(-(0.5 * (epsilon_isq5/ewv5 - (-musig5)^2)))
  ...
  sigx2_4 <- (sigx1_4 * ewz4 * pmusig4 * sigmastar4/wzT4)
  sigx2_5 <- (wzlogit * sigx1_5 * pmusig5 * sigmastar5/wzT5)
  ...
  sigx3_5 <- (TT * Tpi * ewusqrt5 * ewvTT5 * pmusig5 * sigmastar5/wzT5^2)
  sigx4 <- ((2 * sigx2_1 + 2 * sigx2_2 + 2 * sigx2_3 + 2 * sigx2_4)/wzdeno + 2 *
    sigx2_5)
  sigx5_1 <- (sigx4 * wzdeno * Tpi * ewusqrt1 * ewvTT1)
  sigx5_2 <- (sigx4 * wzdeno * Tpi * ewusqrt2 * ewvTT2)
  sigx5_3 <- (sigx4 * wzdeno * Tpi * ewusqrt3 * ewvTT3)
  sigx5_4 <- (sigx4 * wzdeno * Tpi * ewusqrt4 * ewvTT4)
  sigx5_5 <- (sigx4 * Tpi * ewusqrt5 * ewvTT5)
  ...
  sigx6_5 <- (S * ewu5 * pmusig5 * giepsi5/ssqx2_5 - dmusig5)
  ...
  sigx7_5 <- (0.5 * (wvsq5 * ewu5/sigmastar5) + sigmastar5)
  ...
  Xgsq4 <- Xepsi_i4 - sweep(Xgi4, MARGIN = 1, STATS = 2 * (S^2 * ewu4 * giepsi4/sigmasq4),
    FUN = "*")
  Xgsq5 <- Xepsi_i5 - sweep(Xgi5, MARGIN = 1, STATS = 2 * (S^2 * ewu5 * giepsi5/sigmasq5),
    FUN = "*")
  ...
  Xsig5 <- sweep(Xgsq5, MARGIN = 1, STATS = 0.5 * (pmusig5/ewv5), FUN = "*") +
    sweep(Xgi5, MARGIN = 1, STATS = S * dmusig5 * ewu5/ssqx2_5, FUN = "*")
  ...
  Zsig4 <- sweep(Zigiepsi4, MARGIN = 1, STATS = S * sigx6_4 * sigmastar4/ssqx2_4,
    ...
    sweep(Zigisq4, MARGIN = 1, STATS = 0.5 * (ewu4 * ewv4 * pmusig4/(sigmasq4^2 *
      sigmastar4)), FUN = "*")
  Zsig5 <- sweep(Zigiepsi5, MARGIN = 1, STATS = S * sigx6_5 * sigmastar5/ssqx2_5,
    FUN = "*") - sweep(Zigisq5, MARGIN = 1, STATS = S * sigx6_5 * sigmastar5 *
    ewu5 * (sigmastar5 - 0.5 * (ewu5 * ewv5/ssqx2_5)) * giepsi5/ssqx2_5^2, FUN = "*") -
    sweep(Zigisq5, MARGIN = 1, STATS = 0.5 * (ewu5 * ewv5 * pmusig5/(sigmasq5^2 *
      sigmastar5)), FUN = "*")
  gradll <- cbind(sweep(Xsig1, MARGIN = 1, STATS = -(2 * (sigx1_1 * ewz1 * sigmastar1/sigx5_1)),
    ...
    FUN = "*"), sweep(Xsig4, MARGIN = 1, STATS = -(2 * (sigx1_4 * ewz4 * sigmastar4/sigx5_4)),
    ...
    0.5 * (Tpi * ewusqrt4 * ewvTT4 * pmusig4 * sigmastar4/wzT4^2)) * sigx1_4 *
    ewz4/(sigx4 * wzdeno)), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 *
    ((((S * sigx7_4 * dmusig4 * ewu4 * ewv4 * giepsi4/ssqx2_4^2 - 0.5 * ((2 *
      (S^2 * sigx7_4 * ewu4^2 * ewv4 * giepsi4^2/(ssqx2_4^2 * sigmasq4 * sigmastar4)) -
      epsilon_isq4/ewv4) * pmusig4)) * sigmastar4 + 0.5 * (wvsq4 * ewu4 * ewv4 *
      pmusig4/ssqx2_4))/wzT4 - 0.5 * sigx3_4) * sigx1_4 * ewz4/(sigx4 * wzdeno)),
    FUN = "*"), sweep(Zsig4, MARGIN = 1, STATS = 2 * (sigx1_4 * ewu4 * ewz4/sigx5_4),
    FUN = "*"), sweep(Xsig5, MARGIN = 1, STATS = -(2 * (wzlogit * sigx1_5 * sigmastar5/sigx5_5)),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (((0.5 * (wusq5 * ewv5 *
    pmusig5/ssqx2_5) + S * (1/ssqx2_5 - (0.5 * (wusq5 * ewv5/sigmastar5) + sigmastar5 *
    gisq5) * ewu5/ssqx2_5^2) * sigx6_5 * sigmastar5 * giepsi5) * ewu5/wzT5 -
    0.5 * (Tpi * ewusqrt5 * ewvTT5 * pmusig5 * sigmastar5/wzT5^2)) * wzlogit *
    sigx1_5/sigx4), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * ((((S *
    sigx7_5 * dmusig5 * ewu5 * ewv5 * giepsi5/ssqx2_5^2 - 0.5 * ((2 * (S^2 *
    sigx7_5 * ewu5^2 * ewv5 * giepsi5^2/(ssqx2_5^2 * sigmasq5 * sigmastar5)) -
    epsilon_isq5/ewv5) * pmusig5)) * sigmastar5 + 0.5 * (wvsq5 * ewu5 * ewv5 *
    pmusig5/ssqx2_5))/wzT5 - 0.5 * sigx3_5) * wzlogit * sigx1_5/sigx4), FUN = "*"),
    sweep(Zsig5, MARGIN = 1, STATS = 2 * (wzlogit * sigx1_5 * ewu5/sigx5_5),
      ...
      FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = (2 * (sigx1_4 * pmusig3 *
      sigmastar4/wzT4) - sigx4) * ewz4/(sigx4 * wzdeno), FUN = "*"))
  return(sweep(gradll[, -c((nXvar + nuZUvar + nvZVvar + ngZGvar), 2 * (nXvar +
    nuZUvar + nvZVvar + ngZGvar), 3 * (nXvar + nuZUvar + nvZVvar + ngZGvar),
    4 * (nXvar + nuZUvar + nvZVvar + ngZGvar), 5 * (nXvar + nuZUvar + nvZVvar +
      ngZGvar))], MARGIN = 1, STATS = wHvar, FUN = "*"))
}
...
#' @noRd
LCM5ChnormAlgOpt_mols93 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm5C_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike5C_mols93(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike5C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    -colSums(pgradLCMhalfnormlike5C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike5C_mols93,
    grad = pgradLCMhalfnormlike5C_mols93, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      -sum(pLCMhalfnormlike5C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
    }, gr = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike5C_mols93(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike5C_mols93(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike5C_mols93(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
        pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)),
        unname(mleObj$par))
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike5C_mols93(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike5C_mols93(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
...
#' @noRd
pLCM5Chalfnormeff_mols93 <- function(object, level) {
  ...
  beta3 <- object$mlParam[(2 * object$nXvar + 2 * object$nuZUvar + 2 * object$nvZVvar +
    ...
    object$nvZVvar + 2 * ncol(object$gHvar) - 2)]
  ...
  eta3 <- object$mlParam[(3 * object$nXvar + 3 * object$nuZUvar + 3 * object$nvZVvar +
    2 * ncol(object$gHvar) - 1):(3 * object$nXvar + 3 * object$nuZUvar + 3 *
    object$nvZVvar + 3 * ncol(object$gHvar) - 3)]
  eta3[ncol(object$gHvar)] <- 1
  beta4 <- object$mlParam[(3 * object$nXvar + 3 * object$nuZUvar + 3 * object$nvZVvar +
    3 * ncol(object$gHvar) - 2):(4 * object$nXvar + 3 * object$nuZUvar + 3 *
    object$nvZVvar + 3 * ncol(object$gHvar) - 3)]
  delta4 <- object$mlParam[(4 * object$nXvar + 3 * object$nuZUvar + 3 * object$nvZVvar +
    3 * ncol(object$gHvar) - 2):(4 * object$nXvar + 4 * object$nuZUvar + 3 *
    object$nvZVvar + 3 * ncol(object$gHvar) - 3)]
  phi4 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 3 * object$nvZVvar +
    3 * ncol(object$gHvar) - 2):(4 * object$nXvar + 4 * object$nuZUvar + 4 *
    object$nvZVvar + 3 * ncol(object$gHvar) - 3)]
  eta4 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    3 * ncol(object$gHvar) - 2):(4 * object$nXvar + 4 * object$nuZUvar + 4 *
    object$nvZVvar + 4 * ncol(object$gHvar) - 4)]
  eta4[ncol(object$gHvar)] <- 1
  beta5 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) - 3):(5 * object$nXvar + 4 * object$nuZUvar + 4 *
    object$nvZVvar + 4 * ncol(object$gHvar) - 4)]
  delta5 <- object$mlParam[(5 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) - 3):(5 * object$nXvar + 5 * object$nuZUvar + 4 *
    object$nvZVvar + 4 * ncol(object$gHvar) - 4)]
  phi5 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) - 3):(5 * object$nXvar + 5 * object$nuZUvar + 5 *
    object$nvZVvar + 4 * ncol(object$gHvar) - 4)]
  eta5 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    4 * ncol(object$gHvar) - 3):(5 * object$nXvar + 5 * object$nuZUvar + 5 *
    object$nvZVvar + 5 * ncol(object$gHvar) - 5)]
  eta5[ncol(object$gHvar)] <- 1
  theta1 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) - 4):(5 * object$nXvar + 5 * object$nuZUvar + 5 *
    object$nvZVvar + 5 * ncol(object$gHvar) + object$nZHvar - 5)]
  theta2 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) + object$nZHvar - 4):(5 * object$nXvar + 5 * object$nuZUvar +
    5 * object$nvZVvar + 5 * ncol(object$gHvar) + 2 * object$nZHvar - 5)]
  theta3 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) + 2 * object$nZHvar - 4):(5 * object$nXvar + 5 * object$nuZUvar +
    5 * object$nvZVvar + 5 * ncol(object$gHvar) + 3 * object$nZHvar - 5)]
  theta4 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) + 3 * object$nZHvar - 4):(5 * object$nXvar + 5 * object$nuZUvar +
    5 * object$nvZVvar + 5 * ncol(object$gHvar) + 4 * object$nZHvar - 5)]
  ...
  epsilon_isq5 <- as.numeric(tapply(epsilon_it5^2, pindex[, 1], sum))
  git5 <- exp(as.numeric(crossprod(matrix(eta5), t(object$gHvar))))
  git_epsit5 <- epsilon_it5 * git5
  giepsi5 <- as.numeric(tapply(git_epsit5, pindex[, 1], sum))
  gisq5 <- as.numeric(tapply(git5^2, pindex[, 1], sum))
