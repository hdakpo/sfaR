...
#' @noRd
ptslnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  mustar2 <- -((1 + lambda) * exp(Wv)/(TT * exp(Wu/2)) + S * epsilon_i/TT)
  sigmastar <- sqrt(exp(Wv)/TT)
  ll <- log(1 + lambda) - 1/2 * Wu - (TT - 1)/2 * Wv - 1/2 * log(TT) - (TT - 1)/2 *
    log(2 * pi) - log(2 * lambda + 1) + log(2 * exp(-1/2 * (epsilon_isq/exp(Wv) -
...
#' @noRd
psttslnorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradtslnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx6 <- (0.5 * (dmusig * ewv/sqrt(wvtt)) - 0.5 * (epsi_vu * pmusig))
  sigx7 <- (2 * sigx5 - sigx6 * (1 + lambda) * sigx3_1)
  ttsigu <- (TT * sigx4 * ewu_h)
  sigx8 <- ((2 * (sigx2_2 * wvwu_epsi) + epsilon_isq/ewv) * pepsi)
  sigx9 <- (0.5 * sigx8 - sigx2_2 * depsi * ewv/sqrt(wvtt))
  epsi_vusq <- (2 * (epsi_vu * sigx2_1) + epsilon_isq/ewv)
  sigx10 <- (0.5 * (epsi_vusq * pmusig) - sigx2_1 * dmusig * ewv/sqrt(wvtt))
  sigx11 <- (2 * (sigx9 * sigx3_2) - sigx10 * sigx3_1)
  sigx12 <- (epsi_vu * pmusig - dmusig * ewv/sqrt(wvtt))
  Xsig1 <- (sweep(2 * Xepsi_i, MARGIN = 1, STATS = pmusig/ewv, FUN = "*") - sweep(X_iM,
    MARGIN = 1, STATS = 2 * (S * epsi_vu/TT) * pmusig/ewv, FUN = "*"))
  Xsig2 <- sweep(X_iM, MARGIN = 1, STATS = S * dmusig/ssq_v, FUN = "*")
  Xsig3 <- sweep(2 * Xepsi_i, MARGIN = 1, STATS = pepsi/ewv, FUN = "*") - sweep(X_iM,
    MARGIN = 1, STATS = 2 * (S * wvwu_epsi/TT) * pepsi/ewv, FUN = "*")
  Xsig4 <- sweep(X_iM, MARGIN = 1, STATS = S * depsi/ssq_v, FUN = "*")
  Xsig5 <- sweep(0.5 * Xsig1 + Xsig2, MARGIN = 1, STATS = sigx3_1, FUN = "*")
  Xsig6 <- sweep((0.5 * Xsig3 + Xsig4), MARGIN = 1, STATS = sigx3_2, FUN = "*")
  gradll <- cbind(sweep((Xsig5 - 2 * Xsig6), MARGIN = 1, STATS = 1/sigx4, FUN = "*"),
    sweep(uHvar, MARGIN = 1, STATS = (sigx7/ttsigu - 0.5), FUN = "*"), sweep(vHvar,
      MARGIN = 1, STATS = (sigx11/sigx4 - 0.5 * (TT - 1)), FUN = "*"), 1/(1 +
      lambda) - (sigx12 * sigx3_1/ttsigu + 2/(1 + 2 * lambda)))
...
#' @param TT vector of time of presence
#' @noRd
phesstslnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  Xsig5 <- sweep(0.5 * Xsig1 + Xsig2, MARGIN = 1, STATS = sigx3_1, FUN = "*")
  Xsig6 <- sweep((0.5 * Xsig3 + Xsig4), MARGIN = 1, STATS = sigx3_2, FUN = "*")
  Xsig7 <- sweep((Xsig5 - 2 * Xsig6), MARGIN = 1, STATS = sigx7/sigx4, FUN = "*")
  ...
  Xsig8 <- sweep(X_iM, MARGIN = 1, STATS = (S * wvwu_epsi/TT), FUN = "*")
  Xsig9 <- sweep(X_iM, MARGIN = 1, STATS = (S * epsi_vu/TT), FUN = "*")
  ttvq <- (TT^2 * sqrt(wvtt))
  sigx17 <- (epsi_vu * sigx2_1/(TT * ewv) + 0.5/ssq_v^2)
  Xsig10 <- sweep((2 * Xepsi_i - 2 * Xsig9), MARGIN = 1, STATS = 0.5 * (sigx6/ewv) *
    (1 + lambda) * sigx3_1, FUN = "*") + sweep(X_iM, MARGIN = 1, STATS = S *
    sigx14 * (1 + lambda) * sigx3_1, FUN = "*")
  Xsig11 <- sweep((2 * Xepsi_i - 2 * Xsig8), MARGIN = 1, STATS = 0.5 * (sigx13/ewv) *
    sigx3_2, FUN = "*") + sweep(X_iM, MARGIN = 1, STATS = S * (0.5 * ddsq + 0.5 *
    (pepsi - depsi * wvwu_epsi/ssq_v)) * sigx3_2, FUN = "*")
  Xsig12 <- sweep((Xsig10 - 2 * Xsig11), MARGIN = 1, STATS = 1/ttsigu, FUN = "*") -
    sweep((Xsig5 - 2 * Xsig6), MARGIN = 1, STATS = TT * sigx7 * ewu_h/ttsigu^2,
      FUN = "*")
  Xsig13 <- 0.5 * (sweep(X_iM, MARGIN = 1, STATS = 2 * (S * (1/(wutt) - wvwu_epsi/ssq_v^2)) *
    pepsi, FUN = "*") + sweep(Xepsi_i, MARGIN = 1, STATS = 2/ewv * pepsi, FUN = "*") -
    sweep(X_iM, MARGIN = 1, STATS = S * sigx15 * depsi/ssq_v, FUN = "*"))
  Xsig14 <- sweep(X_iM, MARGIN = 1, STATS = S * (sigx2_2 * wvwu_epsi/(TT * ewv) +
    0.5/ssq_v^2) * depsi * ewv/sqrt(wvtt), FUN = "*")
  Xsig15 <- sweep((2 * Xepsi_i - 2 * Xsig8), MARGIN = 1, STATS = 0.5 * (sigx9/ewv),
    FUN = "*")
  Xsig16 <- sweep((Xsig5 - 2 * Xsig6), MARGIN = 1, STATS = sigx11/sigx4, FUN = "*")
  Xsig17 <- sweep(X_iM, MARGIN = 1, STATS = 2 * (S * ((1 + lambda)/(wutt) - epsi_vu/ssq_v^2)) *
    pmusig, FUN = "*") + sweep(Xepsi_i, MARGIN = 1, STATS = 2/ewv * pmusig, FUN = "*")
  Xsig18 <- sweep(X_iM, MARGIN = 1, STATS = S * epsi_vusq * dmusig/ssq_v, FUN = "*")
  Xsig19 <- sweep(X_iM, MARGIN = 1, STATS = S * sigx17 * dmusig * ewv/sqrt(wvtt),
    FUN = "*") - sweep((2 * Xepsi_i - 2 * Xsig9), MARGIN = 1, STATS = 0.5 * (sigx10/ewv),
    FUN = "*")
  Xsig20 <- sweep(2 * ((Xsig13 + Xsig14 - Xsig15)), MARGIN = 1, STATS = sigx3_2/sigx4,
    FUN = "*") - (sweep(Xsig16, MARGIN = 1, STATS = 1/sigx4, FUN = "*") + sweep((0.5 *
    (Xsig17 - Xsig18) + Xsig19), MARGIN = 1, STATS = sigx3_1/sigx4, FUN = "*"))
  ...
  hessll[1:nXvar, 1:nXvar] <- 0.5 * (sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
    as.matrix(wHvar * 2 * pmusig * sigx3_1/ewv/sigx4))) - 2 * crossprod(sweep(X_iM,
    MARGIN = 1, STATS = (wHvar * S^2 * pmusig/TT) * sigx3_1/ewv/sigx4, FUN = "*"),
    X_iM) - crossprod(sweep((2 * Xepsi_i - 2 * Xsig9), MARGIN = 1, STATS = wHvar *
    S * dmusig/ssq_v * sigx3_1/ewv/sigx4, FUN = "*"), X_iM)) - (0.5 * crossprod(sweep((0.5 *
    Xsig1 + Xsig2), MARGIN = 1, STATS = wHvar * sigx3_1/ewv/sigx4, FUN = "*"),
    (2 * Xepsi_i - 2 * Xsig9)) + crossprod(sweep(X_iM, MARGIN = 1, STATS = wHvar *
    S^2 * epsi_vu * dmusig/ttvq * sigx3_1/ewv/sigx4, FUN = "*"), X_iM)) - 2 *
    (0.5 * (sapply(1:nXvar, function(x) crossprod(Xsq[[x]], as.matrix(wHvar *
      2 * pepsi * sigx3_2/ewv/sigx4))) - 2 * crossprod(sweep(X_iM, MARGIN = 1,
      STATS = wHvar * (S^2 * pepsi/TT) * sigx3_2/ewv/sigx4, FUN = "*"), X_iM) -
      crossprod(sweep((2 * Xepsi_i - 2 * Xsig8), MARGIN = 1, STATS = wHvar *
        S * depsi/ssq_v * sigx3_2/ewv/sigx4, FUN = "*"), X_iM)) - (0.5 *
      crossprod(sweep((0.5 * Xsig3 + Xsig4), MARGIN = 1, STATS = wHvar * sigx3_2/ewv/sigx4,
        FUN = "*"), (2 * Xepsi_i - 2 * Xsig8)) + crossprod(sweep(X_iM, MARGIN = 1,
      STATS = wHvar * S^2 * depsi * wvwu_epsi/ttvq * sigx3_2/ewv/sigx4, FUN = "*"),
      X_iM))) - crossprod(sweep((Xsig5 - 2 * Xsig6), MARGIN = 1, STATS = wHvar/sigx4^2,
    FUN = "*"), (Xsig5 - 2 * Xsig6))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(Xsig12, sweep(uHvar,
    MARGIN = 1, STATS = wHvar, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(Xsig20,
    sweep(vHvar, MARGIN = 1, STATS = wHvar, FUN = "*"))
  hessll[1:nXvar, nXvar + nuZUvar + nvZVvar + 1] <- colSums(-(sweep(X_iM, MARGIN = 1,
    STATS = wHvar * S * pmusig/ttsigu * sigx3_1, FUN = "*") - sweep((2 * Xepsi_i -
    2 * Xsig9), MARGIN = 1, STATS = wHvar * 0.5 * (sigx12/ewv)/ttsigu * sigx3_1,
    FUN = "*") - sweep((Xsig5 - 2 * Xsig6), MARGIN = 1, STATS = wHvar * TT *
    sigx12 * ewu_h/ttsigu^2 * sigx3_1, FUN = "*")))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((2 * (((0.25 * ddsq - 0.5 * (0.5 * ddsq - 0.5 *
      pepsi)) * ewv - 0.5 * (sigx13 * wvwu_epsi/TT)) * sigx3_2) - ((0.25 *
      dmusq - 0.5 * (0.5 * dmusq - 0.5 * pmusig)) * ewv - 0.5 * (epsi_vu *
      sigx6/TT)) * (1 + lambda)^2 * sigx3_1)/ttusq - (0.5 * ttsigu + 2 * sigx5 -
      sigx6 * (1 + lambda) * sigx3_1) * sigx7/ttsigu^2), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (2 * ((0.5 *
    (0.5 * (sigx15 * depsi * ewv/(wutt * sqrt(wvtt))) + 2 * ((sigx16 * wvwu_epsi -
      0.5 * (sigx2_2 * ewv/ewu_h)) * pepsi)) - (((0.25 * (ewv/ssq_v^2) + 0.5 *
    (sigx2_2 * wvwu_epsi/TT))/ewu_h - 0.5 * (wutt/(wutt)^2)) * depsi * ewv/sqrt(wvtt) +
    0.5 * (sigx9 * wvwu_epsi/(wutt)))) * sigx3_2) - ((0.5 * (0.5 * (epsi_vusq *
    dmusig * ewv/(wutt * sqrt(wvtt))) + 2 * ((epsi_vu * sigx16 - 0.5 * (sigx2_1 *
    ewv/ewu_h)) * pmusig)) - (((0.25 * (ewv/ssq_v^2) + 0.5 * (epsi_vu * sigx2_1/TT))/ewu_h -
    0.5 * (wutt/(wutt)^2)) * dmusig * ewv/sqrt(wvtt) + 0.5 * (epsi_vu * sigx10/(wutt)))) *
    (1 + lambda) * sigx3_1 + sigx11 * sigx7/ttsigu))/sigx4, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), nXvar + nuZUvar + nvZVvar + 1] <- (wHvar *
    (sigx12 * (0.5 * ttsigu + 2 * sigx5 - sigx6 * (1 + lambda) * sigx3_1)/ttsigu^2 +
      (0.5 * (sigx12 * epsi_vu/TT) + 0.5 * (ewv * pmusig)) * (1 + lambda)/ttusq) *
    sigx3_1) %*% uHvar
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    (2 * ((0.5 * (sigx9 * sigx15) + 0.5 * ((2 * ((sigx2_2/ewu_h - 0.5 * ((1/ewu_h -
      TT * wvwu_epsi/ssq_v^2) * wvwu_epsi/ssq_v^2)) * ewv) - epsilon_isq/ewv) *
      pepsi - sigx2_2 * sigx15 * depsi * ewv/sqrt(wvtt)) - (1/(wutt) - ((sigx2_2^2 +
      0.5/ssq_v^2) * wvwu_epsi + 0.5 * ((1/ewu_h - TT * wvwu_epsi/ssq_v^2) *
      ewv/ssq_v^2) + 0.5 * sigx2_2)) * depsi * ewv/sqrt(wvtt)) * sigx3_2) -
      ((0.5 * (sigx10 * epsi_vusq) + 0.5 * ((2 * ((sigx2_1 * (1 + lambda)/ewu_h -
        0.5 * (epsi_vu * ((1 + lambda)/ewu_h - TT * epsi_vu/ssq_v^2)/ssq_v^2)) *
        ewv) - epsilon_isq/ewv) * pmusig - sigx2_1 * epsi_vusq * dmusig *
        ewv/sqrt(wvtt)) - ((1 + lambda)/(wutt) - ((sigx2_1^2 + 0.5/ssq_v^2) *
        epsi_vu + 0.5 * (((1 + lambda)/ewu_h - TT * epsi_vu/ssq_v^2) * ewv/ssq_v^2) +
        0.5 * sigx2_1)) * dmusig * ewv/sqrt(wvtt)) * sigx3_1 + sigx11^2/sigx4))/sigx4,
    FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), nXvar + nuZUvar + nvZVvar +
    1] <- (wHvar * (-(((((1 + lambda) * pmusig/ewu_h - 0.5 * (dmusig/sqrt(wvtt))) *
    ewv + 0.5 * (sigx12 * epsi_vusq))/ttsigu - TT * sigx12 * sigx11 * ewu_h/ttsigu^2) *
    sigx3_1))) %*% vHvar
  hessll[nXvar + nuZUvar + nvZVvar + 1, nXvar + nuZUvar + nvZVvar + 1] <- sum(wHvar *
    (4/(1 + 2 * lambda)^2 - (((sigx12 * epsi_vu/TT + ewv * pmusig)/ttusq + sigx12^2 *
      sigx3_1/ttsigu^2) * sigx3_1 + 1/(1 + lambda)^2)))
...
#' @noRd
tslnormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(ptslnormlike_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(ptslnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradtslnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptslnormlike_pl81,
    grad = pgradtslnormlike_pl81, hess = phesstslnormlike_pl81, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phesstslnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      -phesstslnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtslnormlike_pl81(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstslnormlike_pl81(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phesstslnormlike_pl81(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- ptslnormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtslnormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
...
ptslnormeff_pl81 <- function(object, level) {
  ...
  mustar2 <- -((1 + lambda) * exp(Wv)/(TT * exp(Wu/2)) + object$S * epsilon_i/TT)
  sigmastar <- sqrt(exp(Wv)/TT)
  a <- mustar1/sigmastar
  b <- mustar2/sigmastar
