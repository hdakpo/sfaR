...
#' @noRd
pgammanormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstgammanorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradgammanormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  ...
  sigx9 <- (0.5 * (epsi_uv * giZi/sqrt(gisq)) + S * sqrt(gisq) *
    Zi_epsi)
  sigx10 <- (sigx9/sigsta - epsi_uv * ewv_h * sqrt(gisq) *
    giZi/sigsta^2)
  sigx11 <- (S * Zi_epsi - epsi_uv * giZi/gisq)
  sigx12 <- (0.5 * (epsi_uv * giZisq/sqrt(gisq)) + S * sqrt(gisq) *
    Zisq_epsi)
  ...
  sigFi11 <- sweep(sigFi1, MARGIN = 1, STATS = (giZi/gisq),
    FUN = "*")
  ...
  sigFi14 <- sweep(sigFi13, MARGIN = 1, STATS = sigx11/gisq,
    FUN = "-")
  sigFi15 <- sweep(sigFi1, MARGIN = 1, STATS = (giZisq/gisq),
    FUN = "*")
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * sigx4/sigsta,
    ...
    0.5 * (giZi/gisq), sigx13 * sigx8 + apply((sigFi18) *
...
#' @noRd
gammanormAlgOpt_mbc92 <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(pgammanormlike_mbc92(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(pgammanormlike_mbc92(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)),
    gr = function(parm) -colSums(pgradgammanormlike_mbc92(parm,
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgammanormlike_mbc92,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pgammanormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      ...
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradgammanormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      ...
      fn = function(parm) -sum(pgammanormlike_mbc92(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_mbc92(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p, N = N, FiMat = FiMat_N)), print.info = printInfo,
      maxiter = itermax, epsa = gradtol, epsb = gradtol),
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(pgammanormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgammanormlike_mbc92(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  ...
  mleObj$logL_OBS <- pgammanormlike_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradgammanormlike_mbc92(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
    FiMat = FiMat_N)
