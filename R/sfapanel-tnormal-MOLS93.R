...
#' @noRd
ptruncnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar,
  vHvar, muHvar, Yvar, Xvar, pindex, TT, S, ngZGvar, gHvar, wHvar) {
  beta <- parm[1:(nXvar)]
  ...
  eta <- parm[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 1):(nXvar + nmuZUvar + nuZUvar +
    nvZVvar + ngZGvar - 1)]
  eta[ngZGvar] <- 1
...
#' @noRd
psttruncnorm_mols93 <- function(olsObj, epsiRes, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, ngZGvar, gHvar, Yvar, Xvar, S, wHvar, printInfo, tol, whichStart,
  initIter, initAlg) {
  if (whichStart == 1L) {
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zmu_", colnames(muHvar)),
    paste0("Zu_", colnames(uHvar)), paste0("Zv_", colnames(vHvar)), paste0("eta_",
...
#' @param TT vector of time of presence
#' @noRd
pgradtruncnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar,
  vHvar, muHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar, gHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  delta <- parm[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)]
  phi <- parm[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar)]
  ...
  sigx4 <- ((0.5 * sigx3 + sigmastar * gisq) * muepsi/ssq + S * giepsi/(ssqx2))
  sigx5 <- (0.5 * (rwv * ewu/sigmastar) + sigmastar)
  sigx6 <- (mu/(ssqx2) - sigx5 * muepsi/ssq)
  sigx7 <- (epsilon_isq/(2 * ewv)^2)
  sigZ1 <- sweep(szsq, MARGIN = 1, STATS = muepsi, FUN = "*") + sweep(Zigiepsi,
    MARGIN = 1, STATS = S/(ssqx2), FUN = "*")
  gradll <- cbind((sweep(Xgi, MARGIN = 1, STATS = -S * sigx1/sigmasq, FUN = "*") -
    sweep(Xepsi_i, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*")), sweep(muHvar,
    MARGIN = 1, STATS = (sigx2/sigmasq - (dmuu/wup + mu/ewu)), FUN = "*"), sweep(uHvar,
    MARGIN = 1, STATS = -((sigx4 * mup + 0.5 * gsig) * ewu - (0.5 * muwu + 0.5 *
      (mu * dmuu/wup))), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = ((mup *
    sigx6 + 2 * sigx7 - 0.5/sigmasq) * ewv - 0.5 * (TT - 1)), FUN = "*"), -sweep(sweep(sigZ1,
    MARGIN = 1, STATS = mup, FUN = "*") + 0.5 * zsig, MARGIN = 1, STATS = ewu,
    FUN = "*"))
  return(sweep(gradll[, -(nXvar + nmuZUvar + nuZUvar + nvZVvar + ngZGvar)], MARGIN = 1,
    STATS = wHvar, FUN = "*"))
}
...
#' @param TT vector of time of presence
#' @noRd
phesstruncnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar,
  vHvar, muHvar, Yvar, Xvar, pindex, TT, S, wHvar, ngZGvar, gHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  delta <- parm[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)]
  ...
  hessll <- matrix(nrow = nXvar + nmuZUvar + nuZUvar + nvZVvar + ngZGvar, ncol = nXvar +
    nmuZUvar + nuZUvar + nvZVvar + ngZGvar)
  hessll[1:nXvar, 1:nXvar] <- -(crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    S^2 * ((muepsi/(ewv * pmustar) + dmusig * ewu/(pmustar)^2) * dmusig/sigmasq -
    1/ewv) * ewu/sigmasq, FUN = "*"), Xgi) + sapply(1:nXvar, function(x) {
    crossprod(Xsq[[x]], as.matrix(wHvar/ewv))
  }))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * S * (sigx9 * dmusig/sigmasq - 1)/sigmasq, FUN = "*"), muHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)] <- crossprod(sweep(Xgi,
    MARGIN = 1, STATS = -wHvar * (S * (sigx10 * sigx4/sigmasq + mup * (1/(ssqx2) -
      sigx12 * ewu/ssq)) * ewu), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xepsi_i, MARGIN = 1, STATS = wHvar * 2 * (ewv/(2 *
    ewv)^2), FUN = "*") + sweep(Xgi, MARGIN = 1, STATS = wHvar * S * (sigx10 *
    sigx6/sigmasq + mup * sigx5 * ewu/ssq) * ewv, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + nvZVvar + 1):(nXvar + nmuZUvar +
    nuZUvar + nvZVvar + ngZGvar)] <- -(crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    (sigx10/sigmasq * S * ewu), FUN = "*"), sigZ1) + sapply(1:ngZGvar, function(x) crossprod(Xzigi[[x]],
    as.matrix(wHvar * (mup/(ssqx2) * S * ewu)))) - crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * (mup * ewu * S * ewu), FUN = "*"), szsq))
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * ((1/ewu - (muepsi/(ewu * pmustar) + dmusig *
      ewv/(pmustar)^2) * dmusig/sigmasq) * ewv/sigmasq + dmuu * (dmuu/wup^2 +
      mu/(ewu_h^3 * pmuu)) - 1/ewu), FUN = "*"), muHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = -wHvar * (((sigx4 *
    sigx11/sigmasq + mup * sigx12 * ewv/ssq) * ewu - (0.5 * (((1 - mu^2/ewu_h^2)/wup -
    mu * dmuu/wup^2) * dmuu) + mu/ewu))), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar + nuZUvar + 1):(nXvar +
    ...
    (mup * (1/(ssqx2) - sigx5 * ewv/ssq) + sigx11 * sigx6/sigmasq) * ewv, FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar + nuZUvar + nvZVvar +
    1):(nXvar + nmuZUvar + nuZUvar + nvZVvar + ngZGvar)] <- -(crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * sigx11/sigmasq * ewu, FUN = "*"), sigZ1) + crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * mup * ewv * ewu, FUN = "*"), szsq))
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar), (nXvar + 1):(nXvar +
    nmuZUvar + nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar *
    (((((mup * dpmu - 1) * sigx4^2 - 0.5 * (gisq^2/sigmasq^2)) * ewu + ((sigx12 *
      (mu * ewv - (2 * sigx13 + 3 * (S * giepsi)) * ewu) + (0.5 * wugsig -
      0.5 * (0.5 * wugsig2 + ewu * gisq/sigmasq)) * wugsig2 * ewv * muepsi/sigmastar)/ssq +
      S * giepsi/(ssqx2)) * mup + 0.5 * gsig) * ewu + 0.5 * muwu - 0.5 * (mu *
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar), (nXvar + nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * ((mup * dpmu - 1) * sigx4 * sigx6 + 0.5 * sqsig -
      ((0.5 * (wugsig2 * ewv/sigmasq) + 0.5 * ((ewu * gisq/sigmasq - 1) * ewv/sigmasq +
        1 - 0.5 * (wugsig2 * rwv))) * muepsi/sigmastar + mu * sigx12 - sigx5 *
        (2 * sigx13 + S * giepsi)) * mup/ssq) * ewu * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar), (nXvar + nmuZUvar +
    nuZUvar + nvZVvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar + ngZGvar)] <- -(crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((0.5 * (wugsig2/(ssqx2)) - 0.5 * (1/(ssqx2) -
      sigx12 * ewu/ssq)) * ewv - 2 * (sigx12 * ssqx2 * ssuv/ssq)) * muepsi -
      S * ssuv * giepsi) * mup/ssq * ewu * ewu, FUN = "*"), Zigisq) - crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * S * sigx12 * mup/ssq * ewu * ewu, FUN = "*"),
    Zigiepsi) + crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (mup * dpmu -
    1) * sigx4 * ewu * ewu, FUN = "*"), sigZ1) - crossprod(sweep(uHvar, MARGIN = 1,
    STATS = wHvar * 0.5 * (gisq/sigmasq^2) * ewu * ewu, FUN = "*"), Zigisq) +
    crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * mup * ewu, FUN = "*"),
      sigZ1) + crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * 0.5 * ewu,
    FUN = "*"), zsig))
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * (((1 - mup * dpmu) * sigx6^2 + 0.5/sigmasq^2 -
      16 * (ewv * epsilon_isq/(2 * ewv)^4)) * ewv + mup * (mu/(ssqx2) - (((3 *
      (mu) - 2 * (sigx5 * sigmasq * muepsi * sigmastar/ssq)) * ewv - S * ewu *
      giepsi) * sigx5 + (0.5 * (ewv/sigmasq) - 0.5 * (0.5 * rwv + ewv/sigmasq)) *
      rwv * ewu * muepsi/sigmastar)/ssq) + 2 * sigx7 - 0.5/sigmasq) * ewv,
    FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar),
    (nXvar + nmuZUvar + nuZUvar + nvZVvar + 1):(nXvar + nmuZUvar + nuZUvar +
      nvZVvar + ngZGvar)] <- -(crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    (((0.5 * (rwv/(ssqx2)) - 0.5 * (1/(ssqx2) - sigx5 * ewv/ssq)) * ewu - 2 *
      (sigx5 * ssqx2 * ssuv/ssq)) * muepsi + mu * ssuv) * mup/ssq * ewu * ewv,
    FUN = "*"), Zigisq) - crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    S * sigx5 * mup/ssq * ewu * ewv, FUN = "*"), Zigiepsi) + crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (1 - mup * dpmu) * sigx6 * ewu * ewv, FUN = "*"),
    sigZ1) - crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar * 0.5/sigmasq^2 *
    ewu * ewv, FUN = "*"), Zigisq))
  hessll[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 1):(nXvar + nmuZUvar + nuZUvar +
    nvZVvar + ngZGvar), (nXvar + nmuZUvar + nuZUvar + nvZVvar + 1):(nXvar + nmuZUvar +
    nuZUvar + nvZVvar + ngZGvar)] <- -(crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar *
    ((0.5 * (ssuv/ssq) - 0.5/(sigmasq^2 * sigmastar)) * ewu * ewv - 2 * (ssqx2 *
      ssuv^2/ssq)) * muepsi * ewu/ssq * mup * ewu, FUN = "*"), Zigisq) - crossprod(sweep(Zigiepsi,
    MARGIN = 1, STATS = wHvar * S * ssuv * ewu/ssq * mup * ewu, FUN = "*"), Zigisq) +
    sapply(1:ngZGvar, function(x) crossprod(Zisqgisq[[x]], as.matrix(wHvar *
      muepsi * ssuv/ssq * mup * ewu))) + S * (sapply(1:ngZGvar, function(x) crossprod(Zisqgiepsi[[x]],
    as.matrix(wHvar * mup/(ssqx2) * ewu))) - crossprod(sweep(Zigisq, MARGIN = 1,
    STATS = wHvar * ewu * ssuv/ssq * mup * ewu, FUN = "*"), Zigiepsi)) + crossprod(sweep(sigZ1,
    MARGIN = 1, STATS = wHvar * (mup * dpmu - 1) * ewu * ewu, FUN = "*"), sigZ1) +
    0.5 * (sapply(1:ngZGvar, function(x) crossprod(Zisqgisq[[x]], as.matrix(wHvar *
      ewu/sigmasq))) - crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar *
      ewu/sigmasq/sigmasq * ewu, FUN = "*"), Zigisq)))
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll <- (hessll + (hessll))/2
  return(hessll[-(nXvar + nmuZUvar + nuZUvar + nvZVvar + ngZGvar), -(nXvar + nmuZUvar +
...
#' @noRd
truncnormAlgOpt_mols93 <- function(start, randStart, sdStart, olsParam, dataTable,
  S, nXvar, muHvar_c, muHvar_p, nmuZUvar, uHvar_c, uHvar_p, gHvar, ngZGvar, nuZUvar,
  vHvar_c, vHvar_p, nvZVvar, Yvar, Xvar, wHvar_c, wHvar_p, pindex, TT, method,
  printInfo, itermax, whichStart, initIter, initAlg, stepmax, tol, gradtol, hessianType,
  qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
    start_st <- psttruncnorm_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, gHvar = gHvar, ngZGvar = ngZGvar,
      nmuZUvar = nmuZUvar, muHvar = muHvar_c, uHvar = uHvar_c, vHvar = vHvar_c,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar_c, initIter = initIter,
      initAlg = initAlg, whichStart = whichStart, tol = tol, printInfo = printInfo)
    Inittrunc <- start_st$inittrunc
    startVal <- start_st$StartVal
  }
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(ptruncnormlike_mols93(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(ptruncnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradtruncnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptruncnormlike_mols93,
    grad = pgradtruncnormlike_mols93, hess = phesstruncnormlike_mols93, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phesstruncnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
        pindex = pindex, TT = TT, S = S, wHvar = wHvar_p), "dgCMatrix")
      ...
      -phesstruncnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p, gHvar = gHvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtruncnormlike_mols93(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstruncnormlike_mols93(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phesstruncnormlike_mols93(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- ptruncnormlike_mols93(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtruncnormlike_mols93(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
...
#' @noRd
ptruncnormeff_mols93 <- function(object, level) {
  ...
  eta <- object$mlParam[(object$nXvar + object$nmuZUvar + object$nuZUvar + object$nvZVvar +
    1):(object$nXvar + object$nmuZUvar + object$nuZUvar + object$nvZVvar + object$ngZGvar -
    1)]
