...
#' @noRd
ptruncnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
psttruncnorm_mols93 <- function(olsObj, epsiRes, nXvar, nmuZUvar,
  nuZUvar, nvZVvar, muHvar, uHvar, vHvar, ngZGvar, gHvar, Yvar,
  Xvar, S, wHvar, printInfo, tol, whichStart, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param TT vector of time of presence
#' @noRd
pgradtruncnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar,
  nmuZUvar, uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S,
  wHvar, ngZGvar, gHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
...
#' @param TT vector of time of presence
#' @noRd
phesstruncnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar,
  nmuZUvar, uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S,
  wHvar, ngZGvar, gHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  ...
  return(hessll[-(nXvar + nmuZUvar + nuZUvar + nvZVvar + ngZGvar),
    -(nXvar + nmuZUvar + nuZUvar + nvZVvar + ngZGvar)])
}
...
#' @noRd
truncnormAlgOpt_mols93 <- function(start, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psttruncnorm_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      ...
      initIter = initIter, initAlg = initAlg, whichStart = whichStart,
      tol = tol, printInfo = printInfo)
    Inittrunc <- start_st$inittrunc
    startVal <- start_st$StartVal
  }
  startLoglik <- sum(ptruncnormlike_mols93(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      -sum(ptruncnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      -colSums(pgradtruncnormlike_mols93(parm, nXvar = nXvar,
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptruncnormlike_mols93,
    grad = pgradtruncnormlike_mols93, hess = phesstruncnormlike_mols93,
    ...
    fn = function(parm) {
      ...
      as(-phesstruncnormlike_mols93(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
        muHvar = muHvar_p, uHvar = uHvar_p, gHvar = gHvar,
        ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    ...
    fn = function(parm) {
      ...
      -phesstruncnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtruncnormlike_mols93(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstruncnormlike_mols93(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
    ...
    if (method == "sr1") {
      mleObj$hessian <- phesstruncnormlike_mols93(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- ptruncnormlike_mols93(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtruncnormlike_mols93(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
