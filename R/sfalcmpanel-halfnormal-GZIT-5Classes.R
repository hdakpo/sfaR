...
#' @noRd
pLCMhalfnormlike5C_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  beta5 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + 1):(5 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar)]
  delta5 <- parm[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + 1):(5 *
    nXvar + 5 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar)]
  phi5 <- parm[(5 * nXvar + 5 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + 1):(5 * nXvar +
    5 * nuZUvar + 5 * nvZVvar + 4 * ngZGvar)]
  eta5 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 4 * ngZGvar + 1):(5 * nXvar +
    5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar)]
  theta1 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 1):(5 *
    nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + nZHvar)]
  theta2 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + nZHvar +
    1):(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 2 * nZHvar)]
  theta3 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 2 * nZHvar +
    1):(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 3 * nZHvar)]
  theta4 <- parm[(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 3 * nZHvar +
    1):(5 * nXvar + 5 * nuZUvar + 5 * nvZVvar + 5 * ngZGvar + 4 * nZHvar)]
...
#' @noRd
psLCMfhalfnorm5C_gzit <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike5C_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @noRd
LCM5ChnormAlgOpt_gzit <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm5C_gzit(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike5C_gzit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike5C_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    -colSums(pgradLCMhalfnormlike5C_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike5C_gzit,
    grad = pgradLCMhalfnormlike5C_gzit, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      -sum(pLCMhalfnormlike5C_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
    }, gr = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike5C_gzit(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike5C_gzit(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike5C_gzit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
        pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)),
        unname(mleObj$par))
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike5C_gzit(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike5C_gzit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
...
#' @noRd
pLCM5Chalfnormeff_gzit <- function(object, level) {
  ...
  beta5 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) + 1):(5 * object$nXvar + 4 * object$nuZUvar + 4 *
    object$nvZVvar + 4 * ncol(object$gHvar))]
  delta5 <- object$mlParam[(5 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) + 1):(5 * object$nXvar + 5 * object$nuZUvar + 4 *
    object$nvZVvar + 4 * ncol(object$gHvar))]
  phi5 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) + 1):(5 * object$nXvar + 5 * object$nuZUvar + 5 *
    object$nvZVvar + 4 * ncol(object$gHvar))]
  eta5 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    4 * ncol(object$gHvar) + 1):(5 * object$nXvar + 5 * object$nuZUvar + 5 *
    object$nvZVvar + 5 * ncol(object$gHvar))]
  theta1 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) + 1):(5 * object$nXvar + 5 * object$nuZUvar + 5 *
    object$nvZVvar + 5 * ncol(object$gHvar) + object$nZHvar)]
  theta2 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) + object$nZHvar + 1):(5 * object$nXvar + 5 * object$nuZUvar +
    5 * object$nvZVvar + 5 * ncol(object$gHvar) + 2 * object$nZHvar)]
  theta3 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) + 2 * object$nZHvar + 1):(5 * object$nXvar + 5 * object$nuZUvar +
    5 * object$nvZVvar + 5 * ncol(object$gHvar) + 3 * object$nZHvar)]
  theta4 <- object$mlParam[(5 * object$nXvar + 5 * object$nuZUvar + 5 * object$nvZVvar +
    5 * ncol(object$gHvar) + 3 * object$nZHvar + 1):(5 * object$nXvar + 5 * object$nuZUvar +
    5 * object$nvZVvar + 5 * ncol(object$gHvar) + 4 * object$nZHvar)]
