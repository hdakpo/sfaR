...
#' @noRd
puninormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstuninorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgraduninormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  epsilon_isq <- as.numeric(tapply(epsilon_it^2, pindex[, 1],
    sum))
  Xepsi_it <- sweep(-Xvar, MARGIN = 1, STATS = 2 * epsilon_it,
    FUN = "*")
  ...
  gradll <- cbind(sweep(X_iM, MARGIN = 1, STATS = S * ddmu/sigx5,
    FUN = "*") - 0.5 * (sweep(Xepsi_i, MARGIN = 1, STATS = 1/ewv,
    FUN = "*") - sweep(X_iM, MARGIN = 1, STATS = 2 * (S^2 *
    epsilon_i/TT)/ewv, FUN = "*")), sweep(uHvar, MARGIN = 1,
    STATS = (sqrt(12)/2 * (dmusig1 * ewu_h/sigx1) - 0.5),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = (sigx4/sigx1 +
    0.5 + 0.5 * sigx2 - 0.5 * TT), FUN = "*"))
...
#' @param TT vector of time of presence
#' @noRd
phessuninormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  Xsq <- list()
  for (i in 1:nXvar) {
    Xsq[[i]] <- apply(sweep(Xvar, MARGIN = 1, STATS = 2 *
      Xvar[, i], FUN = "*"), 2, function(x) tapply(x, pindex[,
      1], sum))
  }
  ewu_h <- exp(Wu/2)
  ewv <- exp(Wv)
  ...
  sigx4 <- (0.5 * sigx3 - 0.5 * (musig1 * dmusig1))
  sigx5 <- (TT * ppmu * sqrt(ewv/TT))
  sigF1 <- sweep(X_iM, MARGIN = 1, STATS = (S^2 * epsilon_i/TT),
    FUN = "*")
  sigF2 <- sweep((Xepsi_i - 2 * sigF1), MARGIN = 1, STATS = 1/ewv,
    FUN = "*")
  sigF3 <- sweep(X_iM, MARGIN = 1, STATS = S * ((0.5 * (dmusig2 *
    (ewv - S^2 * epsilon_i^2/TT)/ttsq^2) - 0.5 * ((1/TT -
    musig1^2/ewv) * dmusig1))/sigx1 - sigx4 * ddmu/(TT *
    sigx1^2)), FUN = "*")
  hessll <- matrix(nrow = nXvar + nuZUvar + nvZVvar, ncol = nXvar +
    nuZUvar + nvZVvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(X_iM, MARGIN = 1,
    STATS = wHvar * ((S * dmusig2 * epsilon_i/TT - musig1 *
      dmusig1)/(TT * ewv * ppmu * sqrt(ewv/TT)) - ddmu^2/sigx5^2),
    FUN = "*"), X_iM) - 0.5 * (sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
    as.matrix(wHvar/(ewv)))) - crossprod(sweep(X_iM, MARGIN = 1,
    STATS = wHvar * 2 * (S^2/TT)/ewv, FUN = "*"), X_iM))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(X_iM,
    MARGIN = 1, STATS = -wHvar * (sqrt(12)/2 * (S * (musig1/(ewv *
      ppmu * sqrt(ewv/TT)) + ddmu/(TT * sigx1^2)) * dmusig1 *
      ewu_h)), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep((0.5 * sigF2 + sigF3), MARGIN = 1,
    STATS = wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    sqrt(12)/2 * (((0.5 - sqrt(12)/2 * (TT * musig1 * ewu_h/ewv))/sigx1 -
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    ...
    MARGIN = 1, STATS = -wHvar * ((0.5 * ((sqrt(12)/2 - sqrt(12)/2 *
      (TT * musig1^2/ewv))/sigx1) + sqrt(12)/2 * (sigx4/sigx1^2)) *
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * ((0.5 * (S * ((0.5 * (S^2 *
      epsilon_i^2) - TT * ewv)/ttsq^2 + 1) * dmusig2 *
      ewv * epsilon_i/ttsq^2) - 0.25 * (TT * musig1^3 *
      dmusig1/ewv))/sigx1 - (((0.5 * (ppmu/ttsq) + 0.5 *
      (S * dmusig2 * epsilon_i/ttsq^2)) * ewv - 0.5 * (musig1 *
      dmusig1)) * sigx4/sigx1^2 + 0.5 * sigx2)), FUN = "*"),
    vHvar)
...
#' @noRd
uninormAlgOpt_pl81 <- function(start, olsParam, dataTable, S,
  nXvar, uHvar_c, uHvar_p, nuZUvar, vHvar_c, vHvar_p, nvZVvar,
  Yvar, Xvar, wHvar_c, wHvar_p, pindex, TT, method, printInfo,
  itermax, stepmax, tol, whichStart, initIter, initAlg, gradtol,
  hessianType, qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
  ...
  startLoglik <- sum(puninormlike_pl81(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      -sum(puninormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p))
    }, gr = function(parm) {
      -colSums(pgraduninormlike_pl81(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p))
    }, hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      maxeval = itermax, stepmax = stepmax, xtol = tol,
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = puninormlike_pl81,
    grad = pgraduninormlike_pl81, hess = phessuninormlike_pl81,
    ...
    fn = function(parm) {
      ...
      as(-phessuninormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
    ...
    fn = function(parm) {
      ...
      -phessuninormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgraduninormlike_pl81(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessuninormlike_pl81(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (method == "sr1") {
      mleObj$hessian <- phessuninormlike_pl81(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- puninormlike_pl81(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgraduninormlike_pl81(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
...
puninormeff_pl81 <- function(object, level) {
  ...
  if (object$logDepVar == TRUE) {
    ...
    teJLMS2 <- exp(-u2)
    teMO <- exp(-m)
    teBC1 <- exp(-mustar + sigmastar^2/2) * (pnorm((-mustar +
      theta)/sigmastar + sigmastar) - pnorm(-mustar/sigmastar +
      sigmastar))/(pnorm((theta - mustar)/sigmastar) -
      pnorm(-mustar/sigmastar))
    teBC2 <- exp(-mustar + sigmastar^2/2) * (1 - pnorm(-mustar/sigmastar +
      sigmastar))/(1 - pnorm(-mustar/sigmastar))
    teBCLB <- exp(-uUB)
    teBCUB <- exp(-uLB)
    teBC1_reciprocal <- exp(-mustar + sigmastar^2/2) * (pnorm((-mustar +
      theta)/sigmastar - sigmastar) - pnorm(-mustar/sigmastar -
      sigmastar))/(pnorm((theta - mustar)/sigmastar) -
      pnorm(-mustar/sigmastar))
    teBC2_reciprocal <- exp(-mustar + sigmastar^2/2) * (1 -
      pnorm(-mustar/sigmastar - sigmastar))/(1 - pnorm(-mustar/sigmastar))
    res <- data.frame(levels(pindex[, 1]), u1 = u1, u2 = u2,
      uLB = uLB, uUB = uUB, teJLMS1 = teJLMS1, teJLMS2 = teJLMS2,
      m = m, teMO = teMO, teBC1 = teBC1, teBC2 = teBC2,
      teBCLB = teBCLB, teBCUB = teBCUB, teBC1_reciprocal = teBC1_reciprocal,
      teBC2_reciprocal = teBC2_reciprocal, theta = theta)
  } else {
    res <- data.frame(levels(pindex[, 1]), u1 = u1, u2 = u2,
      uLB = uLB, uUB = uUB, m = m, theta = theta)
  }
