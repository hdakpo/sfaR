...
#' @noRd
pLCMhalfnormlike3C_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  eta3[ngZGvar] <- 1
  theta1 <- parm[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar - 2):(3 *
    nXvar + 3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar + nZHvar - 3)]
  theta2 <- parm[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar + nZHvar -
    2):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 3 * ngZGvar + 2 * nZHvar - 3)]
...
#' @noRd
psLCMfhalfnorm3C_mols93 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
  ...
  StartVal <- c(Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar -
    ...
    1) rep(0, nvZVvar - 1), Esti[(nXvar + 3):(nXvar + ngZGvar + 1)], rep(0, 2 *
    nZHvar))
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar_p)),
    ...
    paste0("eta_", colnames(gHvar[, -ngZGvar])), names(Esti)[1:nXvar], paste0("Zu_",
      ...
      colnames(gHvar[, -ngZGvar])), paste0("Cl1_", colnames(Zvar)), paste0("Cl2_",
      colnames(Zvar)))
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike3C_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  sigx2_3 <- (wzlogit * sigx1_3 * pmusig3 * sigmastar3/wzT3)
  ...
  gradll <- cbind(sweep(Xsig1, MARGIN = 1, STATS = -(2 * (sigx1_1 * ewz1 * sigmastar1/sigx5_1)),
    ...
    FUN = "*"), sweep(Xsig3, MARGIN = 1, STATS = -(2 * (wzlogit * sigx1_3 * sigmastar3/sigx5_3)),
    ...
    0.5 * (Tpi * ewusqrt3 * ewvTT3 * pmusig3 * sigmastar3/wzT3^2)) * wzlogit *
    sigx1_3/sigx4), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * ((((S *
    sigx7_3 * dmusig3 * ewu3 * ewv3 * giepsi3/ssqx2_3^2 - 0.5 * ((2 * (S^2 *
    sigx7_3 * ewu3^2 * ewv3 * giepsi3^2/(ssqx2_3^2 * sigmasq3 * sigmastar3)) -
    epsilon_isq3/ewv3) * pmusig3)) * sigmastar3 + 0.5 * (wvsq3 * ewu3 * ewv3 *
    pmusig3/ssqx2_3))/wzT3 - 0.5 * sigx3_3) * wzlogit * sigx1_3/sigx4), FUN = "*"),
    sweep(Zsig3, MARGIN = 1, STATS = 2 * (wzlogit * sigx1_3 * ewu3/sigx5_3),
...
#' @noRd
LCM3ChnormAlgOpt_mols93 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm3C_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike3C_mols93(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike3C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    -colSums(pgradLCMhalfnormlike3C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike3C_mols93,
    grad = pgradLCMhalfnormlike3C_mols93, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      -sum(pLCMhalfnormlike3C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
    }, gr = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike3C_mols93(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike3C_mols93(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike3C_mols93(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
        pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)),
        unname(mleObj$par))
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike3C_mols93(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike3C_mols93(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
...
#' @noRd
pLCM3Chalfnormeff_mols93 <- function(object, level) {
  ...
  theta1 <- object$mlParam[(3 * object$nXvar + 3 * object$nuZUvar + 3 * object$nvZVvar +
    3 * ncol(object$gHvar) - 2):(3 * object$nXvar + 3 * object$nuZUvar + 3 *
    object$nvZVvar + 3 * ncol(object$gHvar) + object$nZHvar - 3)]
  theta2 <- object$mlParam[(3 * object$nXvar + 3 * object$nuZUvar + 3 * object$nvZVvar +
    3 * ncol(object$gHvar) + object$nZHvar - 2):(3 * object$nXvar + 3 * object$nuZUvar +
    3 * object$nvZVvar + 3 * ncol(object$gHvar) + 2 * object$nZHvar - 3)]
