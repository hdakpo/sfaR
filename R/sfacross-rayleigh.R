...
#' @noRd
craynormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  wHvar, S) {
  beta <- parm[1:(nXvar)]
  ...
  ll <- (-Wu - 1/2 * Wv - (S * epsilon)^2/(2 * exp(Wv)) + 1/2 * (mustar/sigmastar)^2 +
    1/2 * log(sigmastar^2) + log(sigmastar * dnorm(mustar/sigmastar) + mustar *
...
#' @param vHvar matrix of Zv variables
#' @noRd
cstraynorm <- function(olsObj, epsiRes, S, nuZUvar, uHvar, nvZVvar, vHvar) {
...
#' @param S integer for cost/prod estimation
#' @noRd
cgradraynormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  wHvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx3 <- (1/(sigx2) - (0.5 * ((1 - exp(Wu)/(sigma_sq)) * exp(Wv)/sigmastar) +
    sigmastar) * exp(Wu)/(sigx2)^2)
  ...
  sigx6 <- ((0.5 * ((1 - exp(Wv)/(sigma_sq)) * dmusig2) + pmusigepsisq)/(pdmusig) -
    S^2 * (0.5 * ((1 - exp(Wv)/(sigma_sq)) * exp(Wu)/sigmastar) + sigmastar) *
      wuepsisq/sigx5)
  ...
  gradll <- (cbind(sweep(Xvar, MARGIN = 1, STATS = S * (((pdmusigepsi)/(pdmusig) -
    S * (epsilon)/exp(Wv)) * wusq + S * (epsilon)/exp(Wv)), FUN = "*"), sweep(uHvar,
    MARGIN = 1, STATS = (((0.5 * (dwsig) - pmusigepsi) * exp(Wu)/(pdmusigsq) +
      0.5) * (1 - wusq) + S^2 * (sigx3) * exp(Wu)^2 * (epsilon)^2/(sigx2) -
      1), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = (((sigx6) * wusq + 2 *
    ((S * (epsilon))^2/(2 * exp(Wv))^2)) * exp(Wv) + 0.5 * (wvsigmasq) - 0.5),
    FUN = "*")))
...
#' @param S integer for cost/prod estimation
#' @noRd
chessraynormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  wHvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S^2 * wHvar *
    (((((S^2 * wuepsisq/((sigma_sq) * exp(Wv)) - 1) * (sigx4) + exp(Wu)/(sigx2)) *
      dmusig - exp(Wu) * (pdmusigepsi)^2/(pdmusigsq))/(pdmusig) + 1/exp(Wv)) *
      wusq - 1/exp(Wv)), FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = S * wHvar * (((pmusig - 0.5 * (S * dmusigwuepsi/(sigx2)))/(pdmusigsq) -
      (0.5 * (dwsig) - pmusigepsi) * exp(Wu) * (pdmusigepsi)/(pdmusigsq)^2) *
      (1 - wusq) - 2 * (S * (sigx3) * wuepsi/(sigx2))) * exp(Wu), FUN = "*"),
    uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * wHvar * (((exp(Wu) * (S * (0.5 * ((wvsigmasq)/exp(Wv)) +
      1/(sigma_sq)) * dmusigepsi/sigmastar - (0.5 * (wdmu2sig) + pmusigepsisq) *
      (pdmusigepsi)/(pdmusig)) - pmusig)/(pdmusigsq) + 2 * (S * (wvsigmasq2wu) *
      wuepsi/(sigx5))) * wusq - 4 * (S * (epsilon)/(2 * exp(Wv))^2)) * exp(Wv),
    FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((0.5 * (dwsig) + S * (S * (sigx3) * dmusigwuepsi -
      pmusig) * (epsilon) - ((0.5 * (dwsig) - pmusigepsi) * wusq + 0.5 * (0.5 *
      ((1 - wusq) * dwsig) + S^2 * dmusig3epsisq)))/(pdmusig) - 0.5)/(sigma_sq) -
      ((0.5 * (dwsig) - pmusigepsi) * (1 - wusq) + pdmusig) * (0.5 * (dwsig) -
        pmusigepsi) * exp(Wu)/(pdmusigsq)^2) * (1 - wusq) + S^2 * ((2 * (sigx3) -
      ((0.5 * (wusq) + 1 - 0.5 * (0.5 * (1 - wusq) + wusq)) * wuwvsig + (2 -
        2 * ((wuwvsig2)^2 * exp(Wu) * (sigma_sq)/(sigx2)^2)) * sigmastar) *
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    (((((0.5 * (exp(Wv) * (S^2 * (wvsigmasq2wu) * dmusigwu * wuepsisq/(sigx5) -
      dmusig)/(sigma_sq) - 0.5 * (wdmu1sig)) + 0.5 * dmusig) * (wvsigmasq)/sigmastar +
      (exp(Wv) * (S * (S * (wvsigmasq2wu) * dmusigwuepsi/(sigx2)^2 - pmusig/(sigma_sq)) *
        (epsilon) - (0.5 * (wdmu2sig) + pmusigepsisq)^2 * exp(Wu)/(pdmusig)) +
        pmusigepsi)/(sigma_sq))/(pdmusig) - ((sigx6) * wvsigmasq2 + S^2 *
      ((wvsigmasq2wu) * (1/(sigx5) - (0.5 * ((sigx2)^2 * (wvsigmasq)/(sigx2)) +
        2 * ((wvsigmasq2wu) * exp(Wv))) * exp(Wu) * exp(Wv)/(sigx5)^2) +
        (0.5 * (wvsigmasq2) - 0.5 * (0.5 * (wvsigmasq) + wvsigmasq2)) * (wvsigmasq) *
          (sigma_sq)/((sigx2)^2 * exp(Wv))) * wuepsisq)) * exp(Wu) - 0.5 *
      (wvsigmasq))/(sigma_sq) + (2 - 16 * (exp(Wv)^2/(2 * exp(Wv))^2)) * (S *
      (epsilon))^2/(2 * exp(Wv))^2) * exp(Wv), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (((0.5 *
    (wdmu1sig) + 0.5 * ((dmusig * wvsigmasq2 - S^2 * (wvsigmasq) * dmusig3epsisq/sigmastar) *
    wusq - 0.5 * ((1 - wusq) * wdmu1sig)))/sigmastar + (pmusigepsi - ((0.5 *
    (wdmu2sig) + pmusigepsisq) * (0.5 * (dwsig) - pmusigepsi) * (1 - wusq)/(pdmusig) +
    S * (pmusig/(sigma_sq) + S * (sigx3) * dmusigepsi) * (epsilon)) * exp(Wu))/(sigma_sq))/(pdmusig) +
    0.5/(sigma_sq) - ((sigx6)/(sigma_sq) + S^2 * (((0.5 * ((1 - wusq) * wvsigmasq2) +
    0.5 * ((wusq - 1) * wvsigmasq2 + 1 - 0.5 * ((1 - wusq) * (wvsigmasq))) +
    0.5 * (wvsigmasq)) * exp(Wu)/sigmastar + sigmastar)/(sigx5) + (wvsigmasq2wu) *
    (1/(sigx5) - (0.5 * ((sigx2)^2 * (1 - wusq)/(sigx2)) + 2 * ((wuwvsig2) *
      exp(Wu))) * exp(Wu) * exp(Wv)/(sigx5)^2)) * (epsilon)^2) * exp(Wu)) *
    exp(Wu) * wvsigmasq2, FUN = "*"), vHvar)
...
raynormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S, nXvar,
  ...
  startLoglik <- sum(craynormlike(startVal, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S))
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(craynormlike(parm,
    ...
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradraynormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = craynormlike,
    grad = cgradraynormlike, hess = chessraynormlike, start = startVal, finalHessian = if (hessianType ==
    ...
    Xvar = Xvar, wHvar = wHvar, S = S), sr1 = trustOptim::trust.optim(x = startVal,
    fn = function(parm) -sum(craynormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), method = "SR1", control = list(maxit = itermax,
    ...
    fn = function(parm) -sum(craynormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradraynormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hs = function(parm) as(-chessraynormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S), "dgCMatrix"), method = "Sparse",
    ...
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradraynormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hess = function(parm) -chessraynormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S), print.info = printInfo,
    maxiter = itermax, epsa = gradtol, epsb = gradtol), nlminb = nlminb(start = startVal,
    objective = function(parm) -sum(craynormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      wHvar = wHvar, S = S)), gradient = function(parm) -colSums(cgradraynormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hessian = function(parm) -chessraynormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradraynormlike(mleObj$par, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      wHvar = wHvar, S = S))
  }
  ...
  mleObj$logL_OBS <- craynormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    wHvar = wHvar, S = S)
  mleObj$gradL_OBS <- cgradraynormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    wHvar = wHvar, S = S)
...
craynormeff <- function(object, level) {
  ...
  if (object$logDepVar == TRUE) {
    ...
    teBC_reciprocal <- exp(mustar + sigmastar^2/2) * (sigmastar * dnorm(mustar/sigmastar +
      ...
      dnorm(mustar/sigmastar) + mustar * pnorm(mustar/sigmastar))
    teMO <- exp(-m)
    res <- data.frame(u = u, teJLMS = teJLMS, teBC = teBC, m = m, teMO = teMO,
