...
#' @noRd
ptruncnormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
psttruncnorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar,
  ...
  initTrunc <- maxLik(logLik = ctruncnormlike, start = csttruncnorm(olsObj = olsObj,
    epsiRes = epsiRes, S = S, nmuZUvar = nmuZUvar, nuZUvar = nuZUvar,
    muHvar = muHvar, uHvar = uHvar, nvZVvar = nvZVvar, vHvar = vHvar),
    grad = cgradtruncnormlike, method = "BHHH", control = list(iterlim = itermax,
...
#' @param TT vector of time of presence
#' @noRd
pgradtruncnormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
  nmuZUvar, uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S,
  wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  ...
  gradll <- cbind(sweep(Xepsi_i, MARGIN = 1, STATS = -1/(2 *
    ewv), FUN = "*") + sweep(Xgi, MARGIN = 1, STATS = -S *
    sigx1/sigmasq, FUN = "*"), sweep(muHvar, MARGIN = 1,
    STATS = (sigx2/sigmasq - (dmu/(ewu_h * pmu) + mu/ewu)),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = -(((sigx6 *
    sigx5 + 0.5 * (gisq/sigmasq)) * ewu - (0.5 * ((mu)^2/ewu) +
    0.5 * (mu * dmu/(ewu_h * pmu))))), FUN = "*"), sweep(vHvar,
    MARGIN = 1, STATS = ((sigx5 * sigx8 + 2 * (epsilon_isq/(2 *
      ewv)^2) - 0.5/sigmasq) * ewv - 0.5 * (TT - 1)), FUN = "*"),
    -(((mug * sigx9 * gid2_gicub/ssq + S * gid1_epsi_gisq/(ssqx2)) *
      sigx5 + 0.5 * (gid2_gicub/sigmasq)) * ewu), -(((mug *
      sigx9 * gid2sq_gicub/ssq + S * gid1sq_epsi_gisq/(ssqx2)) *
      sigx5 + 0.5 * (gid2sq_gicub/sigmasq)) * ewu))
...
#' @param TT vector of time of presence
#' @noRd
phesstruncnormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
  nmuZUvar, uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S,
  wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  ...
  sigx18 <- (0.5 * (mu^2/(ewu_h^3 * pmu)) - (0.5 * (ewu_h *
    pmu) - 0.5 * (mu * dmu))/(ewu_h * pmu)^2)
  sigx19 <- (((0.5 * (sigx3/(ssqx2)) - 0.5 * sigx11) * ewv -
    2 * (sigx4 * ssqx2 * sigx9/ssq)) * mug - S * sigx9 *
    giepsi)
  sigx20 <- (((0.5 * (prV/(ssqx2)) - 0.5 * sigx15) * ewu -
    2 * (sigx7 * ssqx2 * sigx9/ssq)) * mug + mu * sigx9)
  sigx21 <- (1 - sigx5 * dmusig/pmusig)
  sigx22 <- ((0.5 * (sigx9/ssq) - 0.5/(sigmasq^2 * sigmastar)) *
    ewu * ewv - 2 * (ssqx2 * sigx9^2/ssq))
  hessll <- matrix(nrow = nXvar + nmuZUvar + nuZUvar + nvZVvar +
    2, ncol = nXvar + nmuZUvar + nuZUvar + nvZVvar + 2)
  hessll[1:nXvar, 1:nXvar] <- sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
    as.matrix(-wHvar/(2 * ewv)))) + crossprod(sweep(Xgi,
    MARGIN = 1, STATS = -wHvar * S^2 * ((mug/(ewv * pmusig *
      sigmastar) + dmusig * ewu/(pmusig * sigmastar)^2) *
      dmusig/sigmasq - 1/ewv) * ewu/sigmasq, FUN = "*"),
    Xgi)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S * ((mug/(pmusig * sigmastar) +
      dmusig * ewu * ewv/(pmusig * sigmastar)^2) * dmusig/sigmasq -
      1)/sigmasq, FUN = "*"), muHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar)] <- crossprod(sweep(Xgi, MARGIN = 1, STATS = -wHvar *
    (S * (sigx10 * sigx6/sigmasq + sigx5 * sigx11) * ewu),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + 1):(nXvar +
    nmuZUvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xepsi_i,
    MARGIN = 1, STATS = wHvar * 2/(2 * ewv)^2 * ewv, FUN = "*") +
    sweep(Xgi, MARGIN = 1, STATS = wHvar * S * (sigx10 *
      sigx8/sigmasq + sigx5 * sigx7 * ewu/ssq) * ewv, FUN = "*"),
    vHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + nvZVvar + 1)] <- -colSums(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S * sigx10 * sigx12/sigmasq *
      ewu, FUN = "*") + sweep(Xi_gid1_gisq, MARGIN = 1,
    STATS = wHvar * S * sigx5/(ssqx2) * ewu, FUN = "*") -
    sweep(Xgi, MARGIN = 1, STATS = wHvar * S * sigx5 * ewu *
      sigx9 * gid2_gicub/ssq * ewu, FUN = "*"))
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + nvZVvar + 2)] <- -colSums(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S * sigx10 * sigx13/sigmasq *
      ewu, FUN = "*") + sweep(Xi_gid1sq_gisq, MARGIN = 1,
    STATS = wHvar * S * sigx5/(ssqx2) * ewu, FUN = "*") -
    sweep(Xgi, MARGIN = 1, STATS = wHvar * S * sigx5 * ewu *
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 1):(nXvar +
    ...
    ((1/ewu - (mug/(ewu * pmusig * sigmastar) + dmusig *
      ewv/(pmusig * sigmastar)^2) * dmusig/sigmasq) * ewv/sigmasq +
      dmu * (dmu/(ewu_h * pmu)^2 + mu/(ewu_h^3 * pmu)) -
      1/ewu), FUN = "*"), muHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    1):(nXvar + nmuZUvar + nuZUvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = -wHvar * (((sigx6 * sigx14/sigmasq +
      sigx5 * sigx4 * ewv/ssq) * ewu - (0.5 * (((1 - mu^2/ewu_h^2)/(ewu_h *
      pmu) - mu * dmu/(ewu_h * pmu)^2) * dmu) + mu/ewu))),
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * (sigx5 * sigx15 + sigx14 *
      sigx8/sigmasq) * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    nuZUvar + nvZVvar + 1)] <- -colSums(sweep(muHvar, MARGIN = 1,
    STATS = wHvar * (sigx12 * sigx14/sigmasq + sigx5 * ewv *
      sigx9 * gid2_gicub/ssq) * ewu, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar +
    nuZUvar + nvZVvar + 2)] <- -colSums(sweep(muHvar, MARGIN = 1,
    STATS = wHvar * (sigx13 * sigx14/sigmasq + sigx5 * ewv *
      sigx9 * gid2sq_gicub/ssq) * ewu, FUN = "*"))
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * ((((sigx16 * sigx6^2 - 0.5 *
      (gisq^2/sigmasq^2)) * ewu + ((sigx4 * (mu * ewv -
      (2 * sigx17 + 3 * (S * giepsi)) * ewu) + (0.5 * (ewu *
      gisq/sigmasq) - 0.5 * (0.5 * sigx3 + ewu * gisq/sigmasq)) *
      sigx3 * ewv * mug/sigmastar)/ssq + S * giepsi/(ssqx2)) *
      sigx5 + 0.5 * (gisq/sigmasq)) * ewu + 0.5 * ((mu)^2/ewu) -
      0.5 * (mu * sigx18 * dmu))), FUN = "*"), uHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
      nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = wHvar * (sigx16 * sigx6 * sigx8 + 0.5 * (gisq/sigmasq^2) -
      ((0.5 * (sigx3 * ewv/sigmasq) + 0.5 * ((ewu * gisq/sigmasq -
        1) * ewv/sigmasq + 1 - 0.5 * (sigx3 * prV))) *
        mug/sigmastar + mu * sigx4 - sigx7 * (2 * sigx17 +
        S * giepsi)) * sigx5/ssq) * ewu * ewv, FUN = "*"),
    vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + nuZUvar + nvZVvar + 1)] <- -colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((sigx19 * gid2_gicub -
      S * sigx4 * gid1_epsi_gisq) * sigx5/ssq + sigx16 *
      sigx6 * sigx12 - 0.5 * (gid2_gicub * gisq/sigmasq^2)) *
      ewu + sigx12 * sigx5 + 0.5 * (gid2_gicub/sigmasq)) *
      ewu, FUN = "*"))
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar),
    (nXvar + nmuZUvar + nuZUvar + nvZVvar + 2)] <- -colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((sigx19 * gid2sq_gicub -
      S * sigx4 * gid1sq_epsi_gisq) * sigx5/ssq + sigx16 *
      sigx6 * sigx13 - 0.5 * (gid2sq_gicub * gisq/sigmasq^2)) *
      ewu + sigx13 * sigx5 + 0.5 * (gid2sq_gicub/sigmasq)) *
      ewu, FUN = "*"))
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * ((sigx21 * sigx8^2 + 0.5/sigmasq^2 -
      16 * (ewv * epsilon_isq/(2 * ewv)^4)) * ewv + sigx5 *
      (mu/(ssqx2) - (((3 * (mu) - 2 * (sigx7 * sigmasq *
        mug * sigmastar/ssq)) * ewv - S * ewu * giepsi) *
        sigx7 + (0.5 * (ewv/sigmasq) - 0.5 * (0.5 * prV +
        ewv/sigmasq)) * prV * ewu * mug/sigmastar)/ssq) +
      2 * (epsilon_isq/(2 * ewv)^2) - 0.5/sigmasq) * ewv,
    FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar + nvZVvar), (nXvar + nmuZUvar + nuZUvar + nvZVvar +
    1)] <- -colSums(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    ((sigx20 * gid2_gicub - S * sigx7 * gid1_epsi_gisq) *
      sigx5/ssq + sigx12 * sigx21 * sigx8 - 0.5 * (gid2_gicub/sigmasq^2)) *
    ewu * ewv, FUN = "*"))
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar + nvZVvar), (nXvar + nmuZUvar + nuZUvar + nvZVvar +
    2)] <- -colSums(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    ((sigx20 * gid2sq_gicub - S * sigx7 * gid1sq_epsi_gisq) *
      sigx5/ssq + sigx13 * sigx21 * sigx8 - 0.5 * (gid2sq_gicub/sigmasq^2)) *
    ewu * ewv, FUN = "*"))
  hessll[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 1), (nXvar +
    nmuZUvar + nuZUvar + nvZVvar + 1)] <- sum(wHvar * (-(((((sigx22 *
    mug * gid2_gicub - S * sigx9 * gid1_epsi_gisq) * ewu *
    gid2_gicub + mug * sigx9 * gd2Zsq_gd1_gicub)/ssq + S *
    (gd1Zsq_gd1_epsi_gisq/(ssqx2) - ewu * sigx9 * gid2_gicub *
      gid1_epsi_gisq/ssq)) * sigx5 + sigx16 * sigx12^2 *
    ewu + 0.5 * ((gd2Zsq_gd1_gicub - ewu * gid2_gicub^2/sigmasq)/sigmasq)) *
    ewu)))
  hessll[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 1), (nXvar +
    nmuZUvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar * (-(((((sigx22 *
    mug * gid2_gicub - S * sigx9 * gid1_epsi_gisq) * ewu *
    gid2sq_gicub + mug * sigx9 * gd2Zcub_gd1_gicub)/ssq +
    S * (gd1Zcub_gd1_epsi_gisq/(ssqx2) - ewu * sigx9 * gid2_gicub *
      gid1sq_epsi_gisq/ssq)) * sigx5 + sigx16 * sigx12 *
    sigx13 * ewu + 0.5 * ((gd2Zcub_gd1_gicub - ewu * gid2_gicub *
    gid2sq_gicub/sigmasq)/sigmasq)) * ewu)))
  hessll[(nXvar + nmuZUvar + nuZUvar + nvZVvar + 2), (nXvar +
    nmuZUvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar * (-(((((sigx22 *
    mug * gid2sq_gicub - S * sigx9 * gid1sq_epsi_gisq) *
    ewu * gid2sq_gicub + mug * sigx9 * gd2Zfour_gd1_gicub)/ssq +
    S * (gd1Zfour_gd1_epsi_gisq/(ssqx2) - ewu * sigx9 * gid2sq_gicub *
      gid1sq_epsi_gisq/ssq)) * sigx5 + sigx16 * sigx13^2 *
    ewu + 0.5 * ((gd2Zfour_gd1_gicub - ewu * gid2sq_gicub^2/sigmasq)/sigmasq)) *
    ewu)))
...
#' @noRd
truncnormAlgOpt_k90 <- function(start, olsParam, dataTable, S,
  nXvar, muHvar_c, muHvar_p, nmuZUvar, uHvar_c, uHvar_p, nuZUvar,
  ...
  startLoglik <- sum(ptruncnormlike_k90(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal,
    fn = function(parm) {
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptruncnormlike_k90,
    grad = pgradtruncnormlike_k90, hess = phesstruncnormlike_k90,
    ...
    fn = function(parm) {
      ...
      as(-phesstruncnormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    -phesstruncnormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtruncnormlike_k90(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstruncnormlike_k90(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    ...
    if (method == "sr1") {
      mleObj$hessian <- phesstruncnormlike_k90(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- ptruncnormlike_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtruncnormlike_k90(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
...
ptruncnormeff_k90 <- function(object, level) {
  ...
  phi <- object$mlParam[(object$nXvar + object$nmuZUvar + object$nuZUvar +
    1):(object$nXvar + object$nmuZUvar + object$nuZUvar +
    object$nvZVvar)]
  eta1 <- object$mlParam[object$nXvar + object$nmuZUvar + object$nuZUvar +
    object$nvZVvar + 1]
  eta2 <- object$mlParam[object$nXvar + object$nmuZUvar + object$nuZUvar +
    object$nvZVvar + 2]
