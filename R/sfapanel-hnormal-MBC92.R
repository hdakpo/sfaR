...
#' @noRd
phalfnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
psthalfnorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradhalfnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx7 <- (Zi_epsi/(sigmasq * sigmastar) - ewu * sigx3 * giepsi *
    giZi/ssq)
  sigx8 <- (Zisq_epsi/(sigmasq * sigmastar) - ewu * sigx3 *
    giepsi * giZisq/ssq)
  ...
  gradll <- cbind(-(sweep(Xepsi_i, MARGIN = 1, STATS = 0.5/ewv,
    ...
    STATS = (0.5 * wvsq + S * sigx9 * dmusig * ewu * ewv *
      giepsi/sqspmu - (0.5 * (2 * (S^2 * sigx9 * ewu^2 *
      ewv * giepsi^2/sigx6) - epsilon_isq/ewv) + 0.5 *
      TT)), FUN = "*"), ewu * (S * sigx1 * sigx7 - 0.5 *
    (giZi/sigmasq)), ewu * (S * sigx1 * sigx8 - 0.5 * (giZisq/sigmasq)))
...
#' @param TT vector of time of presence
#' @noRd
phesshalfnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx19 <- (1/sigmastar - dmusig * (dmusig/(pmusig * sigmastar) -
    S * giepsi/ewv)/pmusig)
  ...
  hessll[1:nXvar, 1:nXvar] <- -(0.5 * (sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
    ...
    STATS = wHvar * 2 * (S^2 * ewu/sigmasq)/ewv, FUN = "*"),
    Xgi)) + crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    S^2 * dmusig * (dmusig/sps^2 - S * giepsi/(sigmasq^2 *
    ewv * pmusig * sigmastar)) * ewu^2, FUN = "*"), Xgi))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S * sigx5 * ewu * (S * (2/sigmastar -
      dmusig * (dmusig/(pmusig * sigmastar) - S * giepsi/ewv)/pmusig) *
      ewu * giepsi/sigmasq - dmusig/pmusig), FUN = "*"),
    uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    ...
    ssq * dmusig * ewu * ewv * giepsi/(sqspmu^2 * sigmasq *
    sigmastar)) * sigx9 * dmusig * ewu, FUN = "*") - 0.5 *
    (sweep(Xgi, MARGIN = 1, STATS = wHvar * 4 * (S^2 * sigx9 *
      ewu^2 * ewv * giepsi/sigx6), FUN = "*") - sweep(Xepsi_i,
      MARGIN = 1, STATS = wHvar/ewv, FUN = "*")), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(XiZi,
    MARGIN = 1, STATS = wHvar * sigx1/(sigmasq * sigmastar) *
      S * ewu, FUN = "*") - sweep(Xgi, MARGIN = 1, STATS = wHvar *
    sigx1 * ewu * sigx3 * giZi/ssq * S * ewu, FUN = "*") +
    sweep(Xgi, MARGIN = 1, STATS = wHvar * S * sigx19 * ewu *
      sigx7/sigmasq * S * ewu, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(XiZisq,
    MARGIN = 1, STATS = wHvar * sigx1/(sigmasq * sigmastar) *
      S * ewu, FUN = "*") - sweep(Xgi, MARGIN = 1, STATS = wHvar *
    sigx1 * ewu * sigx3 * giZisq/ssq * S * ewu, FUN = "*") +
    sweep(Xgi, MARGIN = 1, STATS = wHvar * S * sigx19 * ewu *
      sigx8/sigmasq * S * ewu, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ewu * (S * ((1/(sigmasq * sigmastar) - ((0.5 * sigx11 +
    1.5 - 0.5 * (0.5 * wu2sq + ewu * gisq/sigmasq)) * wu2sq *
    ewv/sigmastar + (3 * gisq - 2 * (sigx4^2 * ewu * sigmasq/ssq)) *
    sigmastar) * ewu/ssq) * sigx1 + S * sigx17 * sigx5^2 *
    ewu * giepsi) * giepsi - 0.5 * (wu2sq * gisq/sigmasq)),
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (0.5 * (gisq/sigmasq^2) +
      S * (((((0.5 * (wu2sq * ewv) - S^2 * sigx9 * sigx5 *
        ewu * giepsi^2)/sigmasq + 0.5 * sigx12 + 0.5 *
        wvsq) * ewu/sigmastar + sigmastar)/sqspmu - sigx9 *
        (2 * (sigx4 * sigmasq * pmusig * sigmastar) -
          S * ssq * sigx5 * dmusig * giepsi) * ewu/sqspmu^2) *
        dmusig - S * (((0.5 * (wu2sq * ewv/sigmasq) +
        0.5 * sigx12) * ewu/sigmastar + 2 * sigx9)/sigx6 -
        ((ssq * gisq + 2 * (sigx4 * sigmasq^2 * sigmastar)) *
          sigmastar + 0.5 * (ssq * wu2sq * ewv/sigmastar)) *
          sigx9 * ewu/sigx6^2) * ewu * giepsi) * giepsi) *
      ewu * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1)] <- colSums(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((0.5 * (gisq * giZi/sigmasq^2) + S * (S * sigx17 * sigx5 *
      giepsi * sigx7 - (((((0.5 * wu2sq - 0.5)/(sigmasq *
      sigmastar) - 0.5 * sigx5) * ewv - 2 * (sigx4 * sigmasq *
      sigmastar * sigx3/ssq)) * ewu + sigmastar) * giepsi *
      giZi + sigx4 * Zi_epsi) * sigx1/ssq)) * ewu + S *
      sigx1 * sigx7 - 0.5 * (giZi/sigmasq)) * ewu, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 2)] <- colSums(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((0.5 * (gisq * giZisq/sigmasq^2) + S * (S * sigx17 *
      sigx5 * giepsi * sigx8 - (((((0.5 * wu2sq - 0.5)/(sigmasq *
      sigmastar) - 0.5 * sigx5) * ewv - 2 * (sigx4 * sigmasq *
      sigmastar * sigx3/ssq)) * ewu + sigmastar) * giepsi *
      giZisq + sigx4 * Zisq_epsi) * sigx1/ssq)) * ewu +
      S * sigx1 * sigx8 - 0.5 * (giZisq/sigmasq)) * ewu,
    FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * (ewv * (S * (((sigx10 * wvsq +
      S^2 * sigx9^2 * ewu * ewv * giepsi^2/(ssq * sigmasq)) *
      ewu/sigmastar + sigmastar)/sqspmu - sigx9^2 * (2 *
      ...
      (2 * (S^2 * ((sigx10 * wvsq * ewu/sigmastar + sigmastar)/sigx6 -
        ((ssq + 2 * (sigx9 * sigmasq^2 * sigmastar)) *
          sigmastar + 0.5 * (ssq * wvsq * ewu/sigmastar)) *
          sigx9 * ewv/sigx6^2) * ewu^2 * ewv * giepsi^2) +
        epsilon_isq/ewv)), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (0.5 * (giZi/sigmasq^2) -
      S * (((sigx14 - 2 * sigx15) * ewu * giepsi * giZi +
        sigx9 * Zi_epsi) * sigx1 + S * sigx9 * sigx18 *
        ewu * giepsi * sigx7)/ssq) * ewu * ewv, FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (0.5 * (giZisq/sigmasq^2) -
      S * (((sigx14 - 2 * sigx15) * ewu * giepsi * giZisq +
        sigx9 * Zisq_epsi) * sigx1 + S * sigx9 * sigx18 *
        ewu * giepsi * sigx8)/ssq) * ewu * ewv, FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar +
    nvZVvar + 1)] <- sum(wHvar * (ewu * (S * ewu * (S * sigx17 *
    sigx7^2 - (((0.5 * (sigx3/ssq) - 0.5/(sigmasq^2 * sigmastar)) *
    ewu^2 * ewv * giepsi * giZi + sigx3 * Zi_epsi) * giZi +
    sigx3 * (giepsi * (Zisq - 2 * (ewu * sigmasq * sigmastar *
      sigx3 * giZi^2/ssq)) + giZi * Zi_epsi)) * sigx1/ssq) -
    0.5 * ((Zisq - ewu * giZi^2/sigmasq)/sigmasq))))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar +
    nvZVvar + 2)] <- sum(wHvar * (ewu * (S * ewu * (S * sigx17 *
    sigx7 * sigx8 - (((0.5 * (sigx3/ssq) - 0.5/(sigmasq^2 *
    sigmastar)) * ewu^2 * ewv * giepsi * giZi + sigx3 * Zi_epsi) *
    giZisq + sigx3 * (giepsi * (Zicub - 2 * (ewu * sigmasq *
    sigmastar * sigx3 * giZi * giZisq/ssq)) + giZi * Zisq_epsi)) *
    sigx1/ssq) - 0.5 * ((Zicub - ewu * giZi * giZisq/sigmasq)/sigmasq))))
  hessll[(nXvar + nuZUvar + nvZVvar + 2), (nXvar + nuZUvar +
    nvZVvar + 2)] <- sum(wHvar * (ewu * (S * ewu * (S * sigx17 *
    sigx8^2 - (((0.5 * (sigx3/ssq) - 0.5/(sigmasq^2 * sigmastar)) *
    ewu^2 * ewv * giepsi * giZisq + sigx3 * Zisq_epsi) *
    giZisq + sigx3 * (giepsi * (Zifour - 2 * (ewu * sigmasq *
    sigmastar * sigx3 * giZisq^2/ssq)) + giZisq * Zisq_epsi)) *
    sigx1/ssq) - 0.5 * ((Zifour - ewu * giZisq^2/sigmasq)/sigmasq))))
...
#' @noRd
halfnormAlgOpt_mbc92 <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(phalfnormlike_mbc92(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      -sum(phalfnormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p))
    }, gr = function(parm) {
      -colSums(pgradhalfnormlike_mbc92(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p))
    }, hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      maxeval = itermax, stepmax = stepmax, xtol = tol,
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = phalfnormlike_mbc92,
    grad = pgradhalfnormlike_mbc92, hess = phesshalfnormlike_mbc92,
    ...
    fn = function(parm) {
      ...
      as(-phesshalfnormlike_mbc92(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    ...
    fn = function(parm) {
      ...
      -phesshalfnormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradhalfnormlike_mbc92(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesshalfnormlike_mbc92(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (method == "sr1") {
      mleObj$hessian <- phesshalfnormlike_mbc92(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- phalfnormlike_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradhalfnormlike_mbc92(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
