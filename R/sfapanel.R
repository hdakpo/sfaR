...
#' 
#' @description
#' \code{\link{sfapanel}} is a symbolic formula-based function for the
...
#'
#' @aliases sfapanel print.sfapanel
#'
...
#' @param idVar When \code{'data'} is not an object of class 
#' \code{'pdata.frame'}, \code{'idVar'} defines the cross sections 
#' identification. Default = \code{NULL}.
#' @param timeVar When \code{'data'} is not an object of class 
...
#' \item \code{'cu00'} for the model in Cuesta (2000).
...
#' are suggested to create constant variables: when \code{'invariance = 1L'}, the
#' first period observation is used; when \code{'invariance = 2L'}, the last
...
#' invariant inefficiency model (Pitt and Lee (1981): \code{'pl81'}) is defined as: 
#' 
...
#'
#' where \eqn{i} is the cross section and \eqn{t} the period the cross section is
#' observed, \eqn{y} is the output (cost, revenue, profit), \eqn{\mathbf{x}} is 
#' the vector of main explanatory variables (inputs and other control variables), 
#' \eqn{u} is the one-sided error term with variance \eqn{\sigma_{u}^2}, and 
#' \eqn{v} is the two-sided error term with variance \eqn{\sigma_{v}^2}.
#'
...
#' \frac{\mu^2}{\sigma_u^2}\right)\right]}\Phi\left(\frac{\mu_{i*}}{\sigma_*}\right)
#' }
#' 
#' where
#' 
#' \deqn{\mu_{i*}=\frac{\mu\sigma_v^2-S\sigma_u^2\sum_t\epsilon_{it}}{T\sigma_u^2+\sigma_v^2}}
#' 
...
#' \eqn{\sigma^2_{ui} = \exp{(\bm{\delta}'\mathbf{Z}_{ui})}} or \eqn{\sigma^2_{vi} =
#' \exp{(\bm{\phi}'\mathbf{Z}_{vi})}}, where \eqn{\mathbf{Z}_{ui}} and \eqn{\mathbf{Z}_{vi}} are the heteroscedasticity
#' variables (inefficiency drivers in the case of \eqn{\mathbf{Z}_{ui}}) and \eqn{\bm{\delta}}
#' and \eqn{\bm{\phi}} the coefficients. In the case of heterogeneity in the
#' truncated mean \eqn{\mu_i}, it is modelled as \eqn{\mu_i=\bm{\omega}'\mathbf{Z}_{\mu_i}}.
#' The \eqn{\mathbf{Z}} variables are such that they are constant over time for each 
#' cross section \eqn{i}. When this is not the case, one of the 
#' \code{'invariance'} option is activated.
#' 
#' In the case of the time varying inefficiency, the model writes as follows:
#' 
#' \deqn{y_{it} = \alpha + \mathbf{x_{it}^{\prime}}\bm{\beta} +  v_{it} - SG(t)u_i}
#' 
...
#' \text{Kumbhakar (1990)} & \text{\code{'modelType = 'k90''}} & G(t)=\left[1+\exp{\left(\eta_1t + \eta_2t^2\right)}\right]^{-1}\\ 
#' \text{Battese and Coelli (1992)} & \text{\code{'modelType = 'bc92a''}} & G(t)=\exp{\left[-\eta(t-T)\right]} \\
#' \text{Cuesta and Orea (2002),  Feng and Serletis (2009)} & \text{\code{'modelType = 'bc92b''}} & G(t)=\exp{\left[-\eta_1(t-T)-\eta_2(t-T)^2\right]} \\
#' \text{Alvarez et al.  (2006)} & \text{\code{'modelType = 'bc92c''}} & G(t)=\exp{\left(\bm{\eta}'\mathbf{Z}_{git}\right)} \\
#' \text{Battese and Coelli (1992)} & \text{\code{'modelType = 'mbc92''}} & G(t)= 1 + \eta_1(t-T) + \eta_2(t-T)^2\\
#' \text{Cuesta (2000)} & \text{\code{'modelType = 'cu00''}} & G(t)=\exp{\left[-\eta_i(t-T)\right]} \\
#' \text{Kumbhakar and Wang (2005)} & \text{\code{'modelType = 'kw05''}} & G(t)=\exp{\left[-\eta(t-t_1)\right]} \\
#' \text{Modified Lee and Schmidt (1993)} & \text{\code{'modelType = 'mols93''}} & G(t)=\exp{\left[-\eta_t(t-T)\right]}
#' \end{array}
#' }
#' 
...
#' 
#' \code{sfapanel} allows for the maximization of weighted log-likelihood.
...
#' objects of class \code{'sfapanel'} including methods to the generic functions 
#' \code{\link[=print.sfapanel]{print}}
#'
#' @return \code{\link{sfapanel}} returns a list of class \code{'sfapanel'}
...
#'
#'@seealso \code{\link[=print.sfapanel]{print}} for printing \code{sfapanel} 
#' object.
#' 
#' @export
#' 
...
#' res_pl81 <- sfapanel(formula = LNCT ~ LNQ2 + LNQ3 + LNNET + LNPK + LNPL, 
#' modelType = 'pl81', S = -1)
sfapanel <- function(formula, muhet, uhet, vhet, logDepVar = TRUE,
  ...
  whichStart = 2L, initAlg = "nm", initIter = 100, invariance = 2L,
  method = "bfgs", hessianType = 2L, simType = "halton", Nsim = 100,
  prime = 2L, burn = 10, antithetics = FALSE, seed = 12345,
  ...
  if (!(modelType %in% c("pl81", "bc92a", "bc92b", "bc92c",
    "mbc92", "k90", "kw05", "cu00", "mols93", "tfe"))) {
    # .tre tfe wangHo..other panel.
  ...
  if (modelType %in% c("bc92a", "bc92b", "k90", "kw05", "cu00",
    ...
    if (modelType == "pl81") {
      ...
      if (modelType == "bc92c") {
        ...
        if (modelType == "tfe") {
          nXold <- length(attr(terms(formula), "term.labels"))
          formula <- update(formula, ~0 + . + as.factor(data[,
          names(attr(data, "index"))[1]]))
        }
      }
    }
  }
  if (modelType == "bc92c") {
    formula <- formDist_sfapanel_bc92c(udist = udist, formula = formula,
      muhet = muhet, uhet = uhet, vhet = vhet, ghet = ghet)
  } else {
  ...
  pindex <- index(data)
  if (modelType == "tfe") {
    colnames(Xvar)[(nXold):(nXvar)] <- levels(data[, names(pindex)[1]])
  }
  ...
  if (udist %in% c("tnormal", "lognormal")) {
    ...
    if (modelType == "bc92c") {
      mtgH <- delete.response(terms(formula, data = data,
        rhs = 5))
    }
    if (invariance == 1) {
  ...
  if (modelType == "bc92a") {
    gHvar <- cbind(eta = unlist(lapply(TT, FUN = function(x) -rev(-(0:(x -
      1))))))
    ngZGvar <- dim(gHvar)[2]
  } else {
    if (modelType == "bc92b") {
    ...
    FunArgs <- if (udist == "tnormal") {
      ...
      if (udist == "lognormal") {
        ...
        if (udist %in% c("gamma", "weibull")) {
          ...
          hessianType = hessianType, initAlg = initAlg,
          initIter = initIter, whichStart = whichStart,
          qac = qac)
        } else {
          list(start = start, olsParam = olsParam, dataTable = dataTable,
          nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (modelType %in% c("bc92a", "bc92b", "bc92c", "kw05",
      ...
      FunArgs <- if (udist == "tnormal") {
        ...
        if (udist == "lognormal") {
          ...
          list(start = start, olsParam = olsParam,
            ...
            itermax = itermax, stepmax = stepmax, tol = tol,
            gradtol = gradtol, hessianType = hessianType,
            initAlg = initAlg, initIter = initIter,
      ...
      if (modelType == "tfe") {
        FunArgs <- if (udist == "tnormal") {
          list(start = start, olsParam = olsParam, dataTable = dataTable,
          nXvar = nXvar, nmuZUvar = nmuZUvar, nuZUvar = nuZUvar,
          nvZVvar = nvZVvar, muHvar = muHvar_c, uHvar = uHvar_c,
          vHvar = vHvar_c, Yvar = Yvar, Xvar = Xvar,
          ...
          if (udist == "lognormal") {
          list(start = start, olsParam = olsParam,
            dataTable = dataTable, nXvar = nXvar, nmuZUvar = nmuZUvar,
            nuZUvar = nuZUvar, nvZVvar = nvZVvar, muHvar = muHvar_c,
            uHvar = uHvar_c, vHvar = vHvar_c, Yvar = Yvar,
            Xvar = Xvar, S = S, wHvar = wHvar, N = NT,
            FiMat = FiMat_NT, method = method, printInfo = printInfo,
            itermax = itermax, stepmax = stepmax, tol = tol,
            gradtol = gradtol, hessianType = hessianType,
            qac = qac)
          } else {
          if (udist %in% c("gamma", "weibull")) {
            list(start = start, olsParam = olsParam,
            dataTable = dataTable, nXvar = nXvar,
            nuZUvar = nuZUvar, nvZVvar = nvZVvar,
            uHvar = uHvar_c, vHvar = vHvar_c, Yvar = Yvar,
            Xvar = Xvar, S = S, wHvar = wHvar, N = NT,
            FiMat = FiMat_NT, method = method, printInfo = printInfo,
  ...
  if (modelType == "pl81") {
    ...
    if (modelType %in% c("bc92a", "bc92b", "bc92c", "kw05",
      ...
      mleList <- tryCatch(switch(udist, hnormal = do.call(halfnormAlgOpt_gzit,
        FunArgs), tnormal = do.call(truncnormAlgOpt_gzit,
        FunArgs), exponential = do.call(exponormAlgOpt_gzit,
        FunArgs), rayleigh = do.call(raynormAlgOpt_gzit,
        FunArgs), genexponential = do.call(genexponormAlgOpt_gzit,
        FunArgs), tslaplace = do.call(tslnormAlgOpt_gzit,
        FunArgs)), error = function(e) e)
    } else {
      if (modelType == "k90") {
        ...
        if (modelType == "mbc92") {
          ...
          FunArgs), tnormal = do.call(truncnormAlgOpt_mbc92,
          FunArgs), exponential = do.call(exponormAlgOpt_mbc92,
          FunArgs), rayleigh = do.call(raynormAlgOpt_mbc92,
          FunArgs), genexponential = do.call(genexponormAlgOpt_mbc92,
          FunArgs), tslaplace = do.call(tslnormAlgOpt_mbc92,
          FunArgs)), error = function(e) e)
        } else {
          if (modelType == "mols93") {
          mleList <- tryCatch(switch(udist, hnormal = do.call(halfnormAlgOpt_mols93,
            FunArgs), tnormal = do.call(truncnormAlgOpt_mols93,
            FunArgs), exponential = do.call(exponormAlgOpt_mols93,
            FunArgs), rayleigh = do.call(raynormAlgOpt_mols93,
            FunArgs), genexponential = do.call(genexponormAlgOpt_mols93,
            FunArgs), tslaplace = do.call(tslnormAlgOpt_mols93,
            FunArgs)), error = function(e) e)
          } else {
          if (modelType == "tfe") {
            tryCatch(switch(udist, hnormal = do.call(halfnormAlgOpt,
            FunArgs), exponential = do.call(exponormAlgOpt,
            FunArgs), tnormal = do.call(truncnormAlgOpt,
            FunArgs), rayleigh = do.call(raynormAlgOpt,
            FunArgs), gamma = do.call(gammanormAlgOpt,
            FunArgs), uniform = do.call(uninormAlgOpt,
            FunArgs), lognormal = do.call(lognormAlgOpt,
            FunArgs), weibull = do.call(weibullnormAlgOpt,
            FunArgs), genexponential = do.call(genexponormAlgOpt,
            FunArgs), tslaplace = do.call(tslnormAlgOpt,
            FunArgs)), error = function(e) e)
          }
          }
        }
  ...
  if (!is.null(start)) {
    if (udist %in% c("tnormal", "lognormal")) {
      names(mleList$startVal) <- fName_mu_sfapanel(Xvar = Xvar,
        ...
        vHvar = vHvar_p, modelType = modelType, gHvar = if (modelType %in%
          c("cu00", "bc92c", "mols93"))
          gHvar else NULL)
    } else {
      names(mleList$startVal) <- fName_uv_sfapanel(Xvar = Xvar,
        udist = udist, uHvar = uHvar_p, vHvar = vHvar_p,
        modelType = modelType, gHvar = if (modelType %in%
          c("cu00", "bc92c", "mols93"))
          gHvar else NULL)
    }
  ...
  if (modelType %in% c("bc92a", "bc92b", "bc92c", "kw05", "cu00",
    "mols93")) {
  ...
  if (modelType == "tfe") {
    ...
    datGradL <- cbind(pindex, datGradL)
  } else {
    datGradL <- mleList$mleObj$gradL_OBS
...
bread.sfapanel <- function(x, ...) {
  if (x$hessianType == "Analytic/Numeric Hessian") {
    ...
    if (x$modelType %in% c("pl81", "mbc92", "k90")) {
      ...
      if (x$modelType %in% c("bc92a", "bc92b", "bc92c",
        "kw05", "cu00", "mols93")) {
        if (x$udist == "hnormal") {
          hessAnalytical <- eval(parse(text = paste0("phesshalfnormlike_",
          x$modelType, "(parm = x$mlParam, nXvar = nXvar, nuZUvar = nuZUvar, 
          ...
          ngZGvar = ngZGvar, gHvar = gHvar)")))
                ...
                if (x$udist == "weibull") {
                  ...
                  hessAnalytical <- eval(parse(text = paste0("calculus::jacobian(function(parm) -colSums(pgradlognormlike_",
                    x$modelType, "(parm, nXvar = ncol(Xvar), 
        ...
        if (x$modelType == "tfe") {
          ...
          hessAnalytical <- chesshalfnormlike(parm = x$mlParam,
            nXvar = ncol(Xvar), nuZUvar = ncol(uHvar_c),
            nvZVvar = ncol(vHvar_c), uHvar = uHvar_c,
            vHvar = vHvar_c, Yvar = Yvar, Xvar = Xvar,
          ...
          if (x$udist == "exponential") {
            ...
            hessAnalytical <- chesstruncnormlike(parm = x$mlParam,
              nXvar = ncol(Xvar), nmuZUvar = ncol(muHvar_c),
              nuZUvar = ncol(uHvar_c), nvZVvar = ncol(vHvar_c),
              muHvar = muHvar_c, uHvar = uHvar_c,
              vHvar = vHvar_c, Yvar = Yvar, Xvar = Xvar,
              wHvar = x$dataTable$weights, S = x$S)
            } else {
            if (x$udist == "rayleigh") {
              ...
              if (x$udist == "genexponential") {
                ...
                if (x$udist == "gamma") {
                  ...
                  nXvar = ncol(Xvar), nuZUvar = ncol(uHvar_c),
                  nvZVvar = ncol(vHvar_c),
                  uHvar = uHvar_c, vHvar = vHvar_c,
                  Yvar = Yvar, Xvar = Xvar,
                  wHvar = x$dataTable$weights,
                  S = x$S, N = x$Nobs, FiMat = x$FiMat_NT)
                } else {
                  if (x$udist == "weibull") {
