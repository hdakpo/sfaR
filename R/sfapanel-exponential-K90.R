...
#' @noRd
pexponormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
pstexponorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradexponormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * sigx1/gisq, FUN = "*") - sweep(Xepsi_i,
    MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = ((0.5 *
    sigx2 - 0.5 * euvg)/(ewu_h * gisq) + 0.5), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = (sigx5 * sigx4 + 2 * (ewv * epsilon_isq/(2 * ewv)^2) - 0.5 * (TT -
      1)), FUN = "*"), sigx6 * (S * gid1_epsi_gisq/gisig - euvg * sigx7 * gid2_gicub/gisig^2) -
    0.5 * (gid2_gicub/gisq), sigx6 * (S * gid1sq_epsi_gisq/gisig - euvg * sigx7 *
...
#' @param TT vector of time of presence
#' @noRd
phessexponormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx6 <- (euvg/gisig - dmusig/pmusig)
  sigx7 <- (sigmastar - 0.5 * (ewv/gisig))
  sigx8 <- (1/sigmastar - dmusig * (dmusig/(pmusig * sigmastar) - euvg/ewv)/pmusig)
  sigx9 <- (dmusig/(pmusig * sigmastar)^2 - euvg/(ewv * pmusig * sigmastar))
  sigx10 <- ((0.5 * euvg - 0.5 * sigx2) * dmusig/pmusig + 0.5 * (ewv/sigmastar))
  sigx11 <- (0.5 * (euvg/(pmusig * sigmastar)) - 0.5 * (dmusig * ewv/(pmusig *
    sigmastar)^2))
  sigx12 <- (ewv/sigmastar - ((dmusig/(pmusig * sigmastar) - 1/ewu_h) * ewv - S *
    giepsi) * dmusig/pmusig)
  sigx13 <- (((0.5/gisq - 0.5 * (1/gisq - 0.5 * (ewv/gisig^2)))/sigmastar - sigx7 *
    gisq/gisig^2) * euvg + sigx7/ewu_h)
  sigx14 <- (0.5 * (sigx7/gisig^2) - 0.5/(sigmastar * gisq^2))
  sigx15 <- (sigx14 * ewv * euvg * gid2_gicub + S * sigx7 * gid1_epsi_gisq)
  sigx16 <- (dmusig/pmusig - euvg/gisig)
  sigx17 <- (S * gid1_epsi_gisq/gisig - euvg * sigx7 * gid2_gicub/gisig^2)
  sigx18 <- (euvg * sigx7 * gid2_gicub/gisig^2 + dmusig * sigx16 * sigx17/pmusig)
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    S^2 * (1/ewv - dmusig * sigx9/gisq)/gisq, FUN = "*"), Xgi) - sapply(1:nXvar,
    function(x) crossprod(Xsq[[x]], as.matrix(wHvar/(2 * ewv))))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * S * (0.5 * (dmusig * (dmusig * ewv/(pmusig * sigmastar)^2 -
      euvg/(pmusig * sigmastar))/gisq) - 0.5)/(ewu_h * gisq), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xepsi_i,
    ...
    MARGIN = 1, STATS = wHvar * S * ((1 - dmusig * sigx9 * ewv/gisq) * sigx4 -
      0.5 * (sigx5/gisig^2)), FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(Xi_gid1_gisq,
    MARGIN = 1, STATS = S * wHvar * sigx6/gisig, FUN = "*") - sweep(Xgi, MARGIN = 1,
    STATS = S * wHvar * sigx7 * gid2_gicub/gisig^2 * sigx6, FUN = "*") + sweep(Xgi,
    MARGIN = 1, STATS = S * wHvar * sigx8 * sigx17/gisq, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(Xi_gid1sq_gisq,
    MARGIN = 1, STATS = S * wHvar * sigx6/gisig, FUN = "*") - sweep(Xgi, MARGIN = 1,
    STATS = S * wHvar * sigx7 * gid2sq_gicub/gisig^2 * sigx6, FUN = "*") + sweep(Xgi,
    MARGIN = 1, STATS = S * wHvar * sigx8 * (S * gid1sq_epsi_gisq/gisig - euvg *
      sigx7 * gid2sq_gicub/gisig^2)/gisq, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((0.25 + 0.5 * (sigx11 * dmusig/gisq)) * ewv/(ewu_h^2 *
      gisq) - 0.5 * ((0.5 * sigx2 - 0.5 * euvg) * ewu_h * gisq/(ewu_h * gisq)^2)),
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (sigx5 *
    (0.25 * (ewv/(ewu_h * gisig^2)) - 0.5 * (ewu_h * gisq/(ewu_h * gisq)^2)) -
    (sigx11 * dmusig/gisq + 0.5) * sigx4 * ewv/ewu_h), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (0.5 * (sigx6 * ewv * sigx7 * gid2_gicub/gisig^2) -
      sigx10 * sigx17/gisq)/ewu_h, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (0.5 * (sigx6 * ewv * sigx7 * gid2sq_gicub/gisig^2) -
      sigx10 * (S * gid1sq_epsi_gisq/gisig - euvg * sigx7 * gid2sq_gicub/gisig^2)/gisq)/ewu_h,
    FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    ((((sigx4 * euvg - 1)/(pmusig * sigmastar) + (0.5 * (pmusig/gisig) - sigx4 *
      dmusig) * ewv/(pmusig * sigmastar)^2) * dmusig + 1/ewu_h) * sigx4 + 2 *
      ((1 - 8 * (ewv^2/(2 * ewv)^2)) * epsilon_isq/(2 * ewv)^2) - 0.5 * (sigx5 *
      (1/ewu_h - euvg * gisq/gisig^2)/gisig^2)) * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 1)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = wHvar * (sigx4 *
    sigx12 * sigx17 - (sigx13 * gid2_gicub + 0.5 * (S * gid1_epsi_gisq/sigmastar)) *
    sigx6 * ewv/gisig^2), FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 2)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = wHvar * (sigx4 *
    sigx12 * (S * gid1sq_epsi_gisq/gisig - euvg * sigx7 * gid2sq_gicub/gisig^2) -
    (sigx13 * gid2sq_gicub + 0.5 * (S * gid1sq_epsi_gisq/sigmastar)) * sigx6 *
      ewv/gisig^2), FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 1)] <- sum(wHvar *
    (sigx6 * (S * (gd1Zsq_gd1_epsi_gisq/gisig - sigx7 * gid2_gicub * gid1_epsi_gisq/gisig^2) -
      (sigx15 * gid2_gicub + euvg * sigx7 * (gd2Zsq_gd1_gicub - 2 * (sigmastar *
        sigx7 * gid2_gicub^2 * gisq/gisig^2)))/gisig^2) + sigx19 * sigx17 -
      0.5 * ((gd2Zsq_gd1_gicub - gid2_gicub^2/gisq)/gisq)))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    (sigx6 * (S * (gd1Zcub_gd1_epsi_gisq/gisig - sigx7 * gid2_gicub * gid1sq_epsi_gisq/gisig^2) -
      (sigx15 * gid2sq_gicub + euvg * sigx7 * (gd2Zcub_gd1_gicub - 2 * (sigmastar *
        sigx7 * gid2_gicub * gid2sq_gicub * gisq/gisig^2)))/gisig^2) + sigx19 *
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 2), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    (sigx6 * (S * (gd1Zfour_gd1_epsi_gisq/gisig - sigx7 * gid2sq_gicub * gid1sq_epsi_gisq/gisig^2) -
      ((sigx14 * ewv * euvg * gid2sq_gicub + S * sigx7 * gid1sq_epsi_gisq) *
        gid2sq_gicub + euvg * sigx7 * (gd2Zfour_gd1_gicub - 2 * (sigmastar *
        sigx7 * gid2sq_gicub^2 * gisq/gisig^2)))/gisig^2) + (S * gid1sq_epsi_gisq/gisig -
      (euvg * sigx7 * gid2sq_gicub/gisig^2 + dmusig * sigx16 * (S * gid1sq_epsi_gisq/gisig -
        euvg * sigx7 * gid2sq_gicub/gisig^2)/pmusig)) * (S * gid1sq_epsi_gisq/gisig -
      euvg * sigx7 * gid2sq_gicub/gisig^2) - 0.5 * ((gd2Zfour_gd1_gicub - gid2sq_gicub^2/gisq)/gisq)))
...
#' @noRd
exponormAlgOpt_k90 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(pexponormlike_k90(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pexponormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradexponormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pexponormlike_k90,
    grad = pgradexponormlike_k90, hess = phessexponormlike_k90, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phessexponormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      -phessexponormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradexponormlike_k90(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessexponormlike_k90(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phessexponormlike_k90(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- pexponormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradexponormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
