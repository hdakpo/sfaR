...
#' @noRd
cGZISFhalfnormlike2C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  phi2 <- parm[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar)]
  theta <- parm[(2 * nXvar + nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + nuZUvar +
    2 * nvZVvar + nZHvar)]
...
#' @noRd
csGZISFfhalfnorm2C <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param nZHvar number of separating variables
#' @noRd
cgradGZISFhalfnormlike2C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  depsi2 <- dnorm(epsi2)
  sigx1 <- (dmusig1 * depsi1 * ewu1/sigmastar1 + S * depsi1 * pmusig1 * (epsilon1))
  ...
  sigx12 <- (sigx11 * dmusig1 * depsi1 * ewu1/ssq1^2 + 0.5 * sigx7)
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = 2 * (S * sigx1 * ewz/sigx4),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (ewu1 * ewz * sigx10/s3sq1),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (ewv1 * ewz * sigx13/s3sq1),
    FUN = "*"), sweep(Xvar, MARGIN = 1, STATS = S^2 * prC * depsi2 * (epsilon2)/sigx14,
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx16 * prC/sigx17, FUN = "*"),
...
#' @param nZHvar number of separating variables
#' @noRd
chessGZISFhalfnormlike2C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx19 <- (sigx18 * depsi1 * pmusig1)
  sigx20 <- (2 * sigx19 - prC * depsi2/(wzdeno * ewv2_h))
  sigx21 <- (S * (dmusig1 * ewu1/sigmastar1 + S * pmusig1 * (epsilon1)) * (epsilon1)/(sigma_sq1) -
    pmusig1)
  ...
  sigx29 <- (0.5 * sigx28 - ((0.5/sqrt(sigma_sq1) - wzdeno^2 * sqrt(sigma_sq1)/wzsq1^2) *
    ewz + 0.5 * (wzdeno/sqrt(sigma_sq1))) * depsi1/wzsq1^2)
  hessll <- matrix(nrow = 2 * nXvar + nuZUvar + 2 * nvZVvar + nZHvar, ncol = 2 *
    nXvar + nuZUvar + 2 * nvZVvar + nZHvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    2 * (S^2 * ((depsi1 * sigx21 + S * dmusig1 * sigx22 * ewu1 * (epsilon1)/ssq1)/sigx4 -
    2 * (sigx1^2 * ewz/sigx4^2)) * ewz), FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * 2 * (S * (((sigx6 * dmusig1 * depsi1 + S * ((0.5 * sigx21 -
      0.5 * pmusig1) * depsi1/(sigma_sq1) - S * sigx6 * dmusig1 * sigx22 *
      (epsilon1)) * (epsilon1)/(sigma_sq1))/wzdeno - 0.5 * sigx23)/s3sq1 -
      2 * (sigx1 * ewz * sigx10/sigx24)) * ewu1 * ewz), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((S * ((0.5 * sigx21 - 0.5 * pmusig1) *
      depsi1/(sigma_sq1) + S * sigx11 * dmusig1 * sigx22 * ewu1 * (epsilon1)/ssq1^2) *
      (epsilon1)/(sigma_sq1) - sigx11 * dmusig1 * depsi1 * ewu1/ssq1^2)/wzdeno -
      0.5 * sigx23)/s3sq1 - 2 * (sigx1 * ewz * sigx13/sigx24)) * ewv1 * ewz),
    FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S^3 * prC * sigx1 * depsi2 * ewv2_h^3 *
      ewz * (epsilon2)/(sigx14^2 * wzdeno * (sigma_sq1) * sqrt(sigma_sq1)))),
    FUN = "*"), Xvar)
  hessll[1:nXvar, (2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + 2 *
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar * (2 * (S *
    sigx16 * prC * sigx1 * ewv2_h * ewz/(sigx17^2 * wzdeno * (sigma_sq1) * sqrt(sigma_sq1)))),
    FUN = "*"), vHvar)
  hessll[1:nXvar, (2 * nXvar + nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + nuZUvar +
    2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (2 * sigx18 - 2 * (sigx20 * ewz/(sigx3 * wzdeno * sqrt(sigma_sq1)))) *
    sigx1 * ewz/(sigx3 * (sigma_sq1)), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((ewu1 * (S * (0.5 * sigx27 - (0.5 * (S^2 *
      sigx6 * depsi1 * (epsilon1)^2/(sigma_sq1)^2) - (((0.5 * (ewu1/(sigma_sq1)) +
      1 - 0.5 * (0.5 * prU1 + ewu1/(sigma_sq1))) * prU1 * ewv1/sigmastar1 +
      (2 - 2 * (sigx5^2 * ewu1 * (sigma_sq1)/ssq1^2)) * sigmastar1)/ssq1^2 +
      S^2 * sigx6^2 * ewu1 * (epsilon1)^2/ssq1) * depsi1) * dmusig1) * (epsilon1)/wzdeno -
      0.5 * (wzdeno * (S * sigx8 * (epsilon1) - wzdeno^2 * depsi1 * pmusig1/wzsq1^2)/wzsq1^2)) +
      S * sigx8 * (epsilon1)/wzdeno - 0.5 * sigx9)/s3sq1 - (0.5 * (sigx3/sqrt(sigma_sq1)) +
      2 * (ewz * sigx10)) * ewu1 * sigx10/s3sq1^2) * ewu1 * ewz), FUN = "*"),
    uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * 2 * (((S *
    (((((0.5 * (prU1 * ewv1) - S^2 * sigx11 * sigx6 * ewu1 * (epsilon1)^2)/(sigma_sq1) +
      0.5 * ((ewu1/(sigma_sq1) - 1) * ewv1/(sigma_sq1) + 1 - 0.5 * (prU1 *
        prV1))) * depsi1/sigmastar1 + 0.5 * (S^2 * sigx11 * depsi1 * (epsilon1)^2/(sigma_sq1)^2)) *
      ewu1 + sigx11 * (1 - 2 * (sigx5 * ewu1 * (sigma_sq1) * sigmastar1/ssq1^2)) *
      depsi1) * dmusig1/ssq1^2 + 0.5 * sigx27) * (epsilon1)/wzdeno - 0.5 *
    (wzdeno * (S * sigx8 * (epsilon1) - wzdeno^2 * depsi1 * pmusig1/wzsq1^2)/wzsq1^2))/s3sq1 -
    (0.5 * (sigx3/sqrt(sigma_sq1)) + 2 * (ewz * sigx10)) * sigx13/s3sq1^2) *
    ewu1 * ewv1 * ewz), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar +
    nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar *
    (2 * (S^2 * prC * depsi2 * ewu1 * ewv2_h^3 * ewz * sigx10 * (epsilon2)/(sigx14^2 *
      sqrt(sigma_sq1)))), FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + nuZUvar + nvZVvar + 1):(2 *
    nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar *
    (2 * (sigx16 * prC * ewu1 * ewv2_h * ewz * sigx10/(sigx17^2 * sqrt(sigma_sq1)))),
    FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + nuZUvar + 2 * nvZVvar + 1):(2 *
    nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = wHvar * (2 * (sigx29 * pmusig1 - S * sigx18 * sigx6 * dmusig1 * depsi1 *
      (epsilon1)) - 2 * (sigx20 * ewz * sigx10/s3sq1)) * ewu1 * ewz/sigx3,
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    prV1 + S^2 * sigx11^2 * ewu1 * ewv1 * (epsilon1)^2/(ssq1^2 * (sigma_sq1))) *
    depsi1 * ewu1/sigmastar1 + ((0.5 * (S^2 * depsi1 * (epsilon1)^2/(sigma_sq1)^2) -
    2 * (sigx11 * depsi1 * (sigma_sq1) * sigmastar1/ssq1^2)) * ewv1 + depsi1) *
    sigx11) * dmusig1 * ewu1/ssq1^2 + S * (0.5 * (ewv1 * (S * (sigx11 * dmusig1 *
    ewu1/ssq1^2 + 0.5 * sigx25) * (epsilon1) - 2 * (pmusig1/(sigma_sq1)))) +
    0.5 * pmusig1) * depsi1 * (epsilon1)/(sigma_sq1)^2) * (epsilon1)/wzdeno -
    (0.5 * (depsi1 * pmusig1) + 0.5 * (ewv1 * (S * sigx12 * (epsilon1) - wzdeno^2 *
      depsi1 * pmusig1/wzsq1^2))) * wzdeno/wzsq1^2)/s3sq1 - (0.5 * (sigx3/sqrt(sigma_sq1)) +
    2 * (ewz * sigx13)) * ewv1 * sigx13/s3sq1^2) * ewv1 * ewz), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -wHvar * (2 * (S^2 * prC * depsi2 * ewv1 * ewv2_h^3 * ewz * sigx13 *
      (epsilon2)/(sigx14^2 * sqrt(sigma_sq1)))), FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (2 * nXvar + nuZUvar +
    nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (sigx16 * prC * ewv1 * ewv2_h * ewz * sigx13/(sigx17^2 *
      sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (2 * nXvar + nuZUvar +
    2 * nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (2 * (sigx29 * pmusig1 + S * sigx11 * sigx18 *
      dmusig1 * depsi1 * ewu1 * (epsilon1)/ssq1^2) - 2 * (sigx20 * ewz * sigx13/s3sq1)) *
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (nXvar +
    ...
    MARGIN = 1, STATS = wHvar * S^2 * ((S^2 * (epsilon2)^2/ewv2_h^2 - 1)/sigx14 -
      S^2 * prC * depsi2 * (epsilon2)^2/sigx14^2) * prC * depsi2, FUN = "*"),
    Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (2 *
    nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S^2 * ((0.5 * (S^2 * (epsilon2)^2/ewv2_h^2 -
      2) - 0.5)/sigx17 - sigx16 * prC/sigx17^2) * prC * depsi2 * (epsilon2)/ewv2_h^2,
    FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (2 *
    nXvar + nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (S^2 * (sigx20/sigx3 + 1/wzdeno) * prC * depsi2 *
      ewz * (epsilon2)/sigx14), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar),
    (2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * prC * (S^2 * (0.5 * (0.5 * (S^2 * (epsilon2)^2/ewv2_h^2) -
      1) - 0.25) * depsi2 * (epsilon2)^2/sigx14 - (sigx16 * prC + 0.5 * sigx17) *
      sigx16/sigx17^2), FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar),
    (2 * nXvar + nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar +
      nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar * (((sigx16 *
    sigx20/sigx3 + 0.5 * (S^2 * depsi2 * (epsilon2)^2/(wzdeno * ewv2_h^2)))/ewv2_h -
    0.5 * (wzdeno * depsi2 * ewv2_h/(wzdeno * ewv2_h)^2)) * prC * ewz/sigx3),
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + nuZUvar + 2 * nvZVvar +
    nZHvar), (2 * nXvar + nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + nuZUvar + 2 *
    nvZVvar + nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1, STATS = wHvar * ((prC *
    (1/(wzdeno^2 * ewv2_h) + ewv2_h/(wzdeno * ewv2_h)^2) * depsi2 - (sigx20^2/sigx3 +
    2 * ((2 - 2 * (wzdeno * (sigma_sq1) * ewz/wzsq1^2)) * depsi1 * pmusig1 *
      sqrt(sigma_sq1)/wzsq1^2))) * ewz + 2 * sigx19 - prC * depsi2/(wzdeno *
    ewv2_h)) * ewz/sigx3, FUN = "*"), Zvar)
...
#' @noRd
GZISF2ChnormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(cGZISFhalfnormlike2C(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cGZISFhalfnormlike2C(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradGZISFhalfnormlike2C(parm, nXvar = nXvar,
    ...
    control = list(trace = if (printInfo) 1 else 0, maxeval = itermax, stepmax = stepmax,
      xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cGZISFhalfnormlike2C,
    grad = cgradGZISFhalfnormlike2C, hess = chessGZISFhalfnormlike2C, start = startVal,
    ...
    fn = function(parm) -sum(cGZISFhalfnormlike2C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradGZISFhalfnormlike2C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cGZISFhalfnormlike2C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradGZISFhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessGZISFhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      ...
      fn = function(parm) -sum(cGZISFhalfnormlike2C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        ...
        nZHvar = nZHvar)), hess = function(parm) -chessGZISFhalfnormlike2C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
        vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar,
        nZHvar = nZHvar), print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(cGZISFhalfnormlike2C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradGZISFhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessGZISFhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = if (printInfo) 1 else 0, eval.max = itermax,
        rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradGZISFhalfnormlike2C(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessGZISFhalfnormlike2C(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
      mleObj$hessian <- chessGZISFhalfnormlike2C(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  }
  mleObj$logL_OBS <- cGZISFhalfnormlike2C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradGZISFhalfnormlike2C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
#' post. prob. and efficiencies for gzisfcross 2 classes halfnormal-normal distribution
...
cGZISF2Chalfnormeff <- function(object, level) {
  ...
  phi2 <- object$mlParam[(2 * object$nXvar + object$nuZUvar + object$nvZVvar +
    1):(2 * object$nXvar + object$nuZUvar + 2 * object$nvZVvar)]
  theta <- object$mlParam[(2 * object$nXvar + object$nuZUvar + 2 * object$nvZVvar +
    1):(2 * object$nXvar + object$nuZUvar + 2 * object$nvZVvar + object$nZHvar)]
...
#' marginal effects for for gzisfcross 2 classes halfnormal-normal distribution
#' @param object object of class gzisfcross
#' @noRd
cmargGZISF2Chalfnorm_Eu <- function(object) {
  ...
  margEff_c2 <- matrix(0, nrow = object$Nobs, ncol = object$nuZUvar - 1)
