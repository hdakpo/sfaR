...
#' @noRd
phalfnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  ll <- log(2) - TT/2 * log(2 * pi) - (TT - 1)/2 * Wv - 1/2 * log(exp(Wv) + TT *
    exp(Wu)) + pnorm(mustar/sigmastar, log.p = TRUE) - epsilon_isq/(2 * exp(Wv)) +
    1/2 * (mustar/sigmastar)^2
  return(wHvar * ll)
}
...
#' @noRd
psthalfnorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  if (whichStart == 1L) {
    ...
    initHalf <- maxLik::maxLik(logLik = chalfnormlike, start = csthalfnorm(olsObj = olsObj,
      ...
      method = initAlg, control = list(iterlim = initIter, printLevel = if (printInfo) 2 else 0,
        reltol = tol), nXvar = nXvar, nuZUvar = 1, nvZVvar = 1, uHvar = as.matrix(uHvar[,
        1]), vHvar = vHvar[, 1, drop = FALSE], Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar)
...
#' @param TT vector of time of presence
#' @noRd
pgradhalfnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  gradll <- cbind(sweep(X_iM, MARGIN = 1, STATS = S * ewu * (S * epsilon_i/ewv -
    dmusig/(pmustar))/sigmasq, FUN = "*") - sweep(Xepsi_i, MARGIN = 1, STATS = 2/(2 *
    ewv), FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = ewu * (S * sigx5 * sigx1 *
    epsilon_i - 0.5 * ttsq), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = ((2 *
    epsivsq + S * sigx6 * sigx3 * epsiusq - 0.5/sigmasq) * ewv - 0.5 * (TT -
    1)), FUN = "*"))
...
#' @param TT vector of time of presence
#' @noRd
phesshalfnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(X_iM, MARGIN = 1, STATS = S^2 * wHvar *
    (1/ewv - dmusig * (dmusig/(pmustar)^2 - S * epsilon_i/(ewv * pmustar)) *
      wusq) * wusq, FUN = "*"), X_iM) - sapply(1:nXvar, function(x) {
    crossprod(Xsq[[x]], as.matrix(wHvar * 2/(2 * ewv)))
  })
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(X_iM, MARGIN = 1,
    STATS = S * sigx5 * ewu * wHvar * (S * (2/sigmastar - sigx7) * ewu * epsilon_i/sigmasq -
      dpmu), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xepsi_i,
    MARGIN = 1, STATS = 2 * wHvar * (2/(2 * ewv)^2) * ewv, FUN = "*") + sweep(X_iM,
    MARGIN = 1, STATS = wHvar * (S * sigx6 * (dpmu + S * (sigx7 - 2/sigmastar) *
      ewu * epsilon_i/sigmasq) * ewu/ssq) * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((0.5 * (TT^2/sigmasq^2) + S * (S * (1 - dmusig *
      sigx3/pmusig) * sigx5^2 * epsilon_i - ((0.5 * (ttusq) + 1 - 0.5 * (0.5 *
      wu2sq + ttusq)) * wu2sq * ewv/sigmastar + (2 * TT - 2 * (sigx4^2 * ewu *
      sigmasq/ssq)) * sigmastar) * sigx1/ssq) * epsilon_i) * ewu + S * sigx5 *
      sigx1 * epsilon_i - 0.5 * ttsq) * ewu, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    ((0.5/sigmasq^2 - 16 * (ewv * epsilon_isq/(2 * ewv)^4)) * ewv + 2 * epsivsq +
      S * (((0.5 * (ewv/sigmasq) - 0.5 * (0.5 * wvsq + ewv/sigmasq)) * wvsq *
        sigx3/sigmastar + S * sigx6^2 * (1 + dmusig * sigx1/pmusig) * ewv *
        epsilon_i/ssq) * ewu + sigx6 * (1 - 2 * (sigx6 * ewv * ssqx2/ssq)) *
        sigx3) * epsiusq - 0.5/sigmasq) * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (0.5 * (TT/sigmasq^2) +
    S * (((0.5 * (wu2sq * ewv/sigmasq) + 0.5 * (1 + ewv * (ttusq - 1)/sigmasq -
      0.5 * (wvsq * wu2sq))) * sigx3/sigmastar + S * sigx6 * sigx5 * (dmusig *
      sigx3/pmusig - 1) * epsilon_i) * ewu + sigx6 * (1 - 2 * (sigx4 * ewu *
      ssqx2/ssq)) * sigx3) * epsilon_i/ssq) * ewu * ewv, FUN = "*"), vHvar)
...
#' @noRd
halfnormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(phalfnormlike_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(phalfnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradhalfnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = phalfnormlike_pl81,
    grad = pgradhalfnormlike_pl81, hess = phesshalfnormlike_pl81, start = startVal,
    ...
    fn = function(parm) {
      ...
      -colSums(pgradhalfnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phesshalfnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      -phesshalfnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradhalfnormlike_pl81(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesshalfnormlike_pl81(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phesshalfnormlike_pl81(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- phalfnormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradhalfnormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
