...
#' @noRd
praynormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  ...
  ll <- 1/2 * (mustar/sigmastar)^2 - epsilon_isq/(2 * exp(Wv)) -
    ...
    1/2 * log(exp(Wv) + TT * exp(Wu)) + log(sigmastar * dnorm(mustar/sigmastar) +
...
#' @noRd
pstraynorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradraynormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx7 <- (sigx4 * wu2sq/sigx1 + S^2 * sigx6 * ewu * epsilon_i^2/sigmastar -
    0.5 * TT)
  sigx8 <- (0.5 * wvdsig + S * pmusig * epsilon_i/sigmasq)
  sigx9 <- (0.5 * (wvsq * ewu/sigmastar) + sigmastar)
  sigx10 <- (sigx8/sigx1 - S^2 * sigx9 * ewu * epsilon_i^2/(ssq *
    sigmastar))
  sigx11 <- (sigx10 * ewu - 0.5)/sigmasq
  gradll <- cbind(sweep(X_iM, MARGIN = 1, STATS = S * ewu *
    sigx3/sigmasq, FUN = "*") - sweep(Xepsi_i, MARGIN = 1,
    STATS = 1/ewv, FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = (sigx7 *
    ewu/sigmasq - 0.5), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = ((sigx11 + 2 * (epsilon_isq/(2 * ewv)^2)) * ewv -
      0.5 * (TT - 1)), FUN = "*"))
...
#' @param TT vector of time of presence
#' @noRd
phessraynormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(X_iM, MARGIN = 1,
    STATS = S^2 * wHvar * (1/ewv - (((1 - S^2 * ewu * epsilon_i^2/(ewv *
      sigmasq)) * squv - ewu/(ssqx2)) * dmusig + ewu *
      sigx2^2/(sigx1 * sigmasq))/sigx1) * ewu/sigmasq,
    FUN = "*"), X_iM) - sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
    as.matrix(wHvar/ewv)))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(X_iM,
    MARGIN = 1, STATS = S * wHvar * (((sigx4 * sigx2/sigx1 +
      0.5 * (S * sigx12)) * ewu/sigmasq - pmusig) * wu2sq/sigx1 +
      2 * (S * sigx6 * ewu * epsilon_i/sigmastar)) * ewu/sigmasq,
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xepsi_i, MARGIN = 1, STATS = wHvar *
    2 * (2/(2 * ewv)^2) * ewv, FUN = "*") + sweep(X_iM, MARGIN = 1,
    STATS = S * wHvar * (((sigx8 * sigx2/sigx1 - S * (0.5 *
      (wvsq/ewv) + 1/sigmasq) * sigx12) * ewu + pmusig)/(sigx1 *
      sigmasq) - 2 * (S * sigx9 * ewu * epsilon_i/sigx14)) *
      ewu/sigmasq * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((0.5 * dmuvsig + ewu * (S^2 * sigx6 * dmusig * epsilon_i^2 -
      (sigx4 * wu2sq/sigx1 + TT) * sigx4/sigmasq) - (0.5 *
      (0.5 * (wu2sq * dmusig * ewv/sigmastar) + S^2 * sigx6 *
        dmusig * ewu * epsilon_i^2) + S * pmusig * epsilon_i)) *
      wu2sq/sigx1 + ewu * (S^2 * (2/(ssqx2) - (((0.5 *
      sigx13 + 2 - 0.5 * (0.5 * wu2sq + TT * ewu/sigmasq)) *
      wu2sq * ewv/sigmastar + (4 * TT - 2 * (sigx5^2 *
      ewu * sigmasq/ssq)) * sigmastar) * ewu/ssq + 0.5 *
      (wu2sq * sigx6))) * epsilon_i^2/sigmastar - TT *
      sigx7/sigmasq) - 0.5 * TT) * ewu/sigmasq, FUN = "*"),
    uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((0.5 * (wvsq * dmusig) +
      0.5 * (ewu * (TT * dmusig * ewv/sigmasq - S^2 * wvsq *
        sigx6 * dmusig * ewu * epsilon_i^2/sigmastar)/sigmasq -
        0.5 * (wvsq * wu2sq * dmusig)))/sigmastar + (S *
      pmusig * epsilon_i - (sigx8 * sigx4 * wu2sq/sigx1 +
      S * (S * sigx6 * dmusig * epsilon_i + TT * pmusig/sigmasq) *
        epsilon_i) * ewu)/sigmasq)/sigx1 - (S^2 * (((0.5 *
      (wu2sq * ewv/sigmasq) + 0.5 * wvsq + 0.5 * (1 + ewv *
      (TT * ewu/sigmasq - 1)/sigmasq - 0.5 * (wvsq * wu2sq))) *
      ewu/sigmastar + sigmastar)/sigx14 + sigx9 * (1/sigx14 -
      (0.5 * (ssq * wu2sq/(ssqx2)) + 2 * (sigx5 * ewu)) *
        ewu * ewv/sigx14^2)) * ewu * epsilon_i^2 + TT *
      sigx11)) * ewu * ewv/sigmasq, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * (((((0.5 * (ewv * (S^2 *
      sigx9 * dmusig * ewu^2 * epsilon_i^2/sigx14 - dmusig)/sigmasq -
      0.5 * (wvsq * dmusig)) + 0.5 * dmusig) * wvsq/sigmastar +
      (ewv * (S * (S * sigx9 * dmusig * ewu * epsilon_i/ssq -
        pmusig/sigmasq) * epsilon_i - sigx8^2 * ewu/sigx1) +
        S * pmusig * epsilon_i)/sigmasq)/sigx1 - S^2 *
      (sigx9 * (1/sigx14 - (0.5 * (ssq * wvsq/(ssqx2)) +
        2 * (sigx9 * ewv)) * ewu * ewv/sigx14^2) + (0.5 *
        (ewv/sigmasq) - 0.5 * (0.5 * wvsq + ewv/sigmasq)) *
        wvsq * sigmasq/(ssq * ewv)) * ewu * epsilon_i^2) *
      ewu - ((sigx10 * ewu - 0.5) * ewv/sigmasq + 0.5))/sigmasq +
      (2 - 16 * (ewv^2/(2 * ewv)^2)) * epsilon_isq/(2 *
        ewv)^2) * ewv, FUN = "*"), vHvar)
...
#' @noRd
raynormAlgOpt_pl81 <- function(start, olsParam, dataTable, S,
  ...
  startLoglik <- sum(praynormlike_pl81(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal,
    fn = function(parm) {
      -sum(praynormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p))
    }, gr = function(parm) {
      -colSums(pgradraynormlike_pl81(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p))
    }, hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      maxeval = itermax, stepmax = stepmax, xtol = tol,
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = praynormlike_pl81,
    grad = pgradraynormlike_pl81, hess = phessraynormlike_pl81,
    ...
    fn = function(parm) {
      ...
      as(-phessraynormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
    ...
    -phessraynormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
      Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradraynormlike_pl81(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessraynormlike_pl81(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (method == "sr1") {
      mleObj$hessian <- phessraynormlike_pl81(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- praynormlike_pl81(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradraynormlike_pl81(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
...
praynormeff_pl81 <- function(object, level) {
  ...
  if (object$logDepVar == TRUE) {
    ...
    res <- data.frame(levels(pindex[, 1]), u = u, teJLMS = teJLMS,
      teBC = teBC, m = m, teMO = teMO, teBC_reciprocal = teBC_reciprocal)
  } else {
