...
#' @noRd
cLCMhalfnormlike3C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
csLCMfhalfnorm3C <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param nZHvar number of separating variables
#' @noRd
cgradLCMhalfnormlike3C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  dmusig1 <- dnorm(-(S * exp(Wu1) * (epsilon1)/((sigma_sq1) * sigmastar1)))
  dmusig2 <- dnorm(-(S * exp(Wu2) * (epsilon2)/((sigma_sq2) * sigmastar2)))
  dmusig3 <- dnorm(-(S * exp(Wu3) * (epsilon3)/((sigma_sq3) * sigmastar3)))
  pmusig1 <- pnorm(-(S * exp(Wu1) * (epsilon1)/((sigma_sq1) * sigmastar1)))
  pmusig2 <- pnorm(-(S * exp(Wu2) * (epsilon2)/((sigma_sq2) * sigmastar2)))
  pmusig3 <- pnorm(-(S * exp(Wu3) * (epsilon3)/((sigma_sq3) * sigmastar3)))
  depsisq1 <- dnorm(S * (epsilon1)/sqrt(sigma_sq1))
  depsisq2 <- dnorm(S * (epsilon2)/sqrt(sigma_sq2))
  depsisq3 <- dnorm(S * (epsilon3)/sqrt(sigma_sq3))
  sigx1_1 <- (dmusig1 * depsisq1 * exp(Wu1)/sigmastar1 + S * depsisq1 * pmusig1 *
    (epsilon1))
  sigx1_2 <- (dmusig2 * depsisq2 * exp(Wu2)/sigmastar2 + S * depsisq2 * pmusig2 *
    (epsilon2))
  sigx1_3 <- (dmusig3 * depsisq3 * exp(Wu3)/sigmastar3 + S * depsisq3 * pmusig3 *
    (epsilon3))
  sqsq1 <- ((sigma_sq1) * sigmastar1)
  sqsq2 <- ((sigma_sq2) * sigmastar2)
  sqsq3 <- ((sigma_sq3) * sigmastar3)
  sigx2_1 <- (0.5 * ((1 - exp(Wu1)/(sigma_sq1)) * exp(Wv1)/sigmastar1) + sigmastar1)
  sigx2_2 <- (0.5 * ((1 - exp(Wu2)/(sigma_sq2)) * exp(Wv2)/sigmastar2) + sigmastar2)
  sigx2_3 <- (0.5 * ((1 - exp(Wu3)/(sigma_sq3)) * exp(Wv3)/sigmastar3) + sigmastar3)
  sigx3_1 <- (0.5 * ((1 - exp(Wv1)/(sigma_sq1)) * exp(Wu1)/sigmastar1) + sigmastar1)
  sigx3_2 <- (0.5 * ((1 - exp(Wv2)/(sigma_sq2)) * exp(Wu2)/sigmastar2) + sigmastar2)
  sigx3_3 <- (0.5 * ((1 - exp(Wv3)/(sigma_sq3)) * exp(Wu3)/sigmastar3) + sigmastar3)
  ...
  sigx6_1 <- (S * (dpepsisq1 - (1/sqsq1 - sigx2_1 * exp(Wu1)/sqsq1^2) * dmusig1 *
    depsisq1) * (epsilon1) - sigx5_1)
  sigx6_2 <- (S * (dpepsisq2 - (1/sqsq2 - sigx2_2 * exp(Wu2)/sqsq2^2) * dmusig2 *
    depsisq2) * (epsilon2) - sigx5_2)
  sigx6_3 <- (S * (dpepsisq3 - (1/sqsq3 - sigx2_3 * exp(Wu3)/sqsq3^2) * dmusig3 *
    depsisq3) * (epsilon3) - 0.5 * dpsq3)
  sigx7_1 <- (S * (sigx3_1 * dmusig1 * depsisq1 * exp(Wu1)/sqsq1^2 + dpepsisq1) *
    (epsilon1) - sigx5_1)
  sigx7_2 <- (S * (sigx3_2 * dmusig2 * depsisq2 * exp(Wu2)/sqsq2^2 + dpepsisq2) *
    (epsilon2) - sigx5_2)
  sigx7_3 <- (S * (sigx3_3 * dmusig3 * depsisq3 * exp(Wu3)/sqsq3^2 + dpepsisq3) *
    (epsilon3) - 0.5 * dpsq3)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = 2 * (S * sigx1_1 * exp(Wz1)/sigsq_1),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (exp(Wu1) * exp(Wz1) * sigx6_1/(sigwz1)),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (exp(Wv1) * exp(Wz1) * sigx7_1/(sigwz1)),
    FUN = "*"), sweep(Xvar, MARGIN = 1, STATS = 2 * (S * sigx1_2 * exp(Wz2)/sigsq_2),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (exp(Wu2) * exp(Wz2) * sigx6_2/(sigwz2)),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (exp(Wv2) * exp(Wz2) * sigx7_2/(sigwz2)),
    FUN = "*"), sweep(Xvar, MARGIN = 1, STATS = 2 * (S * prC * sigx1_3/sigsq_3),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (prC * exp(Wu3) * sigx6_3/(sigwz3)),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (prC * exp(Wv3) * sigx7_3/(sigwz3)),
    FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = (2 * dpsq1 - sigx4) * exp(Wz1)/wzdsig,
    FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = (2 * dpsq2 - sigx4) * exp(Wz2)/wzdsig,
    FUN = "*"))
...
#' @param nZHvar number of separating variables
#' @noRd
chessLCMhalfnormlike3C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx6_3 <- (S * (dpepsisq3 - sqewu3 * dmusig3 * depsisq3) * (epsilon3) - 0.5 *
    dpsq3)
  ...
  sigx4w7z <- ((2 * dpsq1 - sigx4) * sqrt(sigma_sq3)/(sigwz3)^2 + 1/(sigwz3))
  sigx4w8z <- ((2 * dpsq2 - sigx4) * sqrt(sigma_sq3)/(sigwz3)^2 + 1/(sigwz3))
  sigx4w9z <- (0.5 * (sigx4/sqrt(sigma_sq3)) + 2 * (prC * sigx6_3))
  sigx4w10z <- (sigx4 * (sigma_sq3)/sqrt(sigma_sq3))
  sigx4w11z <- (0.5 * (sigx4/sqrt(sigma_sq3)) + 2 * (prC * sigx7_3))
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    ...
    STATS = -(4 * (S * prC * sigx1_3 * ewu1 * (sigma_sq3) * ewz1 * sigx6_1 *
      sqrt(sigma_sq3)/(sigsq_3^2 * wzdsq1))) * wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    1):(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewu1 * ewu3 * ewz1 * sigx6_1 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 *
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar +
    ...
    STATS = -(4 * (prC * ewu1 * ewv3 * ewz1 * sigx7_3 * sigx6_1 * sqrt(sigma_sq3)/((sigwz3)^2 *
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    ...
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    nZHvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    (((S * ((((0.5 * (ewv1/(sigma_sq1)) - 0.5 * (0.5 * (wvsq1) + ewv1/(sigma_sq1))) *
      ...
      ewu1/sqsq1^2 + S * (0.5 * (ewv1 * (S * sigx16_1 * (epsilon1) - 2 * (pmusig1/(sigma_sq1)))) +
      0.5 * pmusig1) * depsisq1 * (epsilon1)/(sigma_sq1)^2) * (epsilon1) -
      (0.5 * (depsisq1 * pmusig1) + 0.5 * (ewv1 * sigx13_1))/(sigma_sq1))/(sigwz1) -
      sigx18_1 * ewv1 * sigx7_1/(sigwz1)^2) * ewv1 * ewz1) * wHvar, FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (2 * nXvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_3 * (sigma_sq3) * ewv1 * ewz1 *
      sigx7_1 * sqrt(sigma_sq3)/(sigsq_3^2 * wzdsq1))) * wHvar, FUN = "*"),
    Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (prC * ewu3 * ewv1 * ewz1 * sigx7_1 * sigx6_3 *
      sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq1))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 3 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (prC * ewv1 * ewv3 * ewz1 * sigx7_1 * sigx7_3 *
      sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq1))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx4w2z * ewv1 * ewz1 * sigx7_1/sqrt(sigma_sq1) * wHvar,
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + nZHvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (2 *
    ...
    MARGIN = 1, STATS = -(4 * (S^2 * prC * sigx1_2 * sigx1_3 * (sigma_sq3) *
      ewz2 * sqrt(sigma_sq3)/(sigsq_3^2 * wzdeno * (sigma_sq2)^(3/2)))) * wHvar,
    FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
    ...
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_2 * ewu3 * ewz2 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 *
      wzdeno * (sigma_sq2)^(3/2)))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
    nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_2 * ewv3 * ewz2 * sigx7_3 * sqrt(sigma_sq3)/((sigwz3)^2 *
      wzdeno * (sigma_sq2)^(3/2)))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
    ...
    3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S *
  ...
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    ...
    sigx1_3 * ewu2 * (sigma_sq3) * ewz2 * sigx6_2 * sqrt(sigma_sq3)/(sigsq_3^2 *
    wzdsq2))) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(4 * (prC *
    ewu2 * ewu3 * ewz2 * sigx6_2 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq2))) *
    wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 *
      nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(4 * (prC *
    ewu2 * ewv3 * ewz2 * sigx7_3 * sigx6_2 * sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq2))) *
    wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
  ...
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar + 3 * nuZUvar +
      3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = sigx4w4z *
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    ...
    ewu2/sqsq2^2 + S * (0.5 * (ewv2 * (S * sigx16_2 * (epsilon2) - 2 * (pmusig2/(sigma_sq2)))) +
    0.5 * pmusig2) * depsisq2 * (epsilon2)/(sigma_sq2)^2) * (epsilon2) - (0.5 *
    (depsisq2 * pmusig2) + 0.5 * (ewv2 * sigx13_2))/(sigma_sq2))/(sigwz2) - sigx18_2 *
    ewv2 * sigx7_2/(sigwz2)^2) * ewv2 * ewz2) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(4 * (S * prC *
    sigx1_3 * (sigma_sq3) * ewv2 * ewz2 * sigx7_2 * sqrt(sigma_sq3)/(sigsq_3^2 *
    wzdsq2))) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(4 * (prC *
    ewu3 * ewv2 * ewz2 * sigx7_2 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq2))) *
    wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(4 * (prC *
    ewv2 * ewv3 * ewz2 * sigx7_2 * sigx7_3 * sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq2))) *
    wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx4w3z *
    ewv2 * ewz1 * ewz2 * sigx7_2/sqrt(sigma_sq2)) * wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    ...
    (S^2 * ((depsisq3 * (S * (dmusig3 * ewu3/sigmastar3 + S * pmusig3 * (epsilon3)) *
      ...
      2 * (prC * sigx1_3^2/sigsq_3^2)) * prC) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    ...
    (S * ((sqewu3 * dmusig3 * depsisq3 + (S * (sigx10_3 - S * sqewu3 * dmusig3 *
      ...
      2 * (prC * sigx1_3 * sigx6_3/((sigwz3)^2 * (sigma_sq3)))) * prC * ewu3) *
    wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    ...
    (S * (((S * (sigx10_3 + S * sigx3_3 * dmusig3 * (duv3) * ewu3 * (epsilon3)/sqsq3^2) *
      ...
      depsisq3 * ewu3/sqsq3^2)/(sigwz3) - 2 * (prC * sigx1_3 * sigx7_3/((sigwz3)^2 *
      (sigma_sq3)))) * prC * ewv3) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 3 *
    nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    prC * sigx4w5z * sigx1_3 * ewz1/((sigma_sq3)^(3/2))) * wHvar, FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * prC * sigx4w6z * sigx1_3 * ewz2/((sigma_sq3)^(3/2))) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    ...
    (((ewu3 * (S * (0.5 * sigx15_3 - (0.5 * (S^2 * sqewu3 * depsisq3 * (epsilon3)^2/(sigma_sq3)^2) -
      ...
      (epsilon3) - 0.5 * dpsq3)/(sigwz3) - sigx4w9z * ewu3 * sigx6_3/(sigwz3)^2) *
  ...
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    ...
    (((S * (((((0.5 * (wusq3 * ewv3) - S^2 * sigx3_3 * sqewu3 * ewu3 * (epsilon3)^2)/(sigma_sq3) +
      ...
      0.5 * sigx12_3)/(sigwz3) - sigx4w9z * sigx7_3/(sigwz3)^2) * prC * ewu3 *
      ewv3) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 3 *
    nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(prC *
    sigx4w5z * ewu3 * ewz1 * sigx6_3/sqrt(sigma_sq3)) * wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(prC * sigx4w6z * ewu3 * ewz2 * sigx6_3/sqrt(sigma_sq3)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    ...
    (((S * ((((0.5 * (ewv3/(sigma_sq3)) - 0.5 * (0.5 * wvsq3 + ewv3/(sigma_sq3))) *
      ...
      ewu3/sqsq3^2 + S * (0.5 * (ewv3 * (S * sigx16_3 * (epsilon3) - 2 * (pmusig3/(sigma_sq3)))) +
      0.5 * pmusig3) * depsisq3 * (epsilon3)/(sigma_sq3)^2) * (epsilon3) -
      (0.5 * (depsisq3 * pmusig3) + 0.5 * (ewv3 * sigx13_3))/(sigma_sq3))/(sigwz3) -
      sigx4w11z * ewv3 * sigx7_3/(sigwz3)^2) * prC * ewv3) * wHvar, FUN = "*"),
    vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 3 *
    nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(prC *
    sigx4w5z * ewv3 * ewz1 * sigx7_3/sqrt(sigma_sq3)) * wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(prC * sigx4w6z * ewv3 * ewz2 * sigx7_3/sqrt(sigma_sq3)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + nZHvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
  ...
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    ...
    1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
  ...
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar + 3 *
    ...
    nZHvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
...
#' @noRd
LCM3ChnormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S, wHvar,
  ...
  startLoglik <- sum(cLCMhalfnormlike3C(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cLCMhalfnormlike3C(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradLCMhalfnormlike3C(parm, nXvar = nXvar,
    ...
    control = list(trace = if (printInfo) 1 else 0, maxeval = itermax, stepmax = stepmax,
      xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cLCMhalfnormlike3C,
    grad = cgradLCMhalfnormlike3C, hess = chessLCMhalfnormlike3C, start = startVal,
    ...
    fn = function(parm) -sum(cLCMhalfnormlike3C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike3C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cLCMhalfnormlike3C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradLCMhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessLCMhalfnormlike3C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      method = "Sparse", control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
        prec = tol, report.level = if (printInfo) 2 else 0, report.precision = 1L,
        preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal, fn = function(parm) -sum(cLCMhalfnormlike3C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradLCMhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessLCMhalfnormlike3C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      objective = function(parm) -sum(cLCMhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradLCMhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessLCMhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = if (printInfo) 1 else 0, eval.max = itermax,
        rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradLCMhalfnormlike3C(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessLCMhalfnormlike3C(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
      mleObj$hessian <- chessLCMhalfnormlike3C(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  }
  mleObj$logL_OBS <- cLCMhalfnormlike3C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradLCMhalfnormlike3C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
