...
#' @noRd
pgenexponormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstgenexponorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradgenexponormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx16 <- ((2 * (ewv/ewu_h) + S * giepsi) * pmusig2/sgsq -
    dmusig2)
  sigx17 <- (S * gid1_epsi_gisq/sgsq - (2 * (ewv/ewu_h) + S *
    giepsi) * (sqrt(ewv/gisq) - 0.5 * (ewv/sgsq)) * gid2_gicub/sgsq^2)
  ...
  sigx20 <- (S * gid1sq_epsi_gisq/sgsq - (2 * (ewv/ewu_h) +
...
#' @param TT vector of time of presence
#' @noRd
phessgenexponormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx30 <- ((0.5 * ddgq - 0.5 * pmusig1) * ewv/sqrt(ewv/gisq) -
    (0.5 * sigx14 + 0.5 * dmusig1) * sigx22)
  ...
  sigx36 <- ((sigx22 * sigx15/(ewv * gisq) - sigx24 * gid2_gicub/sgsq^2) *
    sigx22 * pmusig1 + (S * pmusig1 * gid1_epsi_gisq - dmusig1 *
    sigx22 * sigx15)/sgsq)
  sigx43 <- (0.5 * (sigx24/sgsq^2) - 0.5/sigx27)
  sigx37 <- (sigx43 * ewv * sigx22 * gid2_gicub + S * sigx24 *
    gid1_epsi_gisq)
  sigx38 <- (sqrt(ewv/gisq) * sigx24 * gid2_gicub * gid2sq_gicub *
    gisq/sgsq^2)
  sigx39 <- (sqrt(ewv/gisq) * sigx24 * gid2_gicub^2 * gisq/sgsq^2)
  sigx40 <- ((sigx26 * sigx17/(ewv * gisq) - sigx24 * gid2_gicub/sgsq^2) *
    sigx26 * pmusig2 + (S * pmusig2 * gid1_epsi_gisq - sigx26 *
    dmusig2 * sigx17)/sgsq)
  sigx41 <- (sigx43 * sigx26 * ewv * gid2_gicub + S * sigx24 *
    gid1_epsi_gisq)
  sigx42 <- (gd1Zfour_gd1_epsi_gisq/sgsq - sigx24 * gid2sq_gicub *
    gid1sq_epsi_gisq/sgsq^2)
  ...
  Xsig11 <- sweep((Xsig9 - 0.5 * Xsig10), MARGIN = 1, STATS = sigx15,
    FUN = "*")
  Xsig12 <- sweep((Xsig9 - 0.5 * Xsig10), MARGIN = 1, STATS = sigx19,
    FUN = "*")
  Xsig13 <- sweep(Xi_gid1_gisq, MARGIN = 1, STATS = 1/sgsq,
    FUN = "*")
  Xsig14 <- sweep(Xgi, MARGIN = 1, STATS = sigx24 * gid2_gicub/sgsq^2,
    FUN = "*")
  Xsig15 <- sweep((Xsig13 - Xsig14), MARGIN = 1, STATS = S *
    sigx14, FUN = "*")
  Xsig16 <- sweep((Xsig13 - Xsig14), MARGIN = 1, STATS = S *
    sigx16, FUN = "*")
  ...
  Xsig22 <- sweep(Xgi, MARGIN = 1, STATS = (0.5 * ddgq + 0.5 *
    (pmusig1 - dmusig1 * sigx22/sgsq)), FUN = "*")
  Xsig23 <- sweep(Xgi, MARGIN = 1, STATS = S * sigx28/gisq,
    FUN = "*")
  Xsig24 <- sweep((Xepsi_i - 2 * Xsig0), MARGIN = 1, STATS = (sigx16/ewv),
    FUN = "*")
  Xsig25 <- sweep((Xsig23 - 0.5 * Xsig24), MARGIN = 1, STATS = sigx17,
    FUN = "*")
  Xsig26 <- sweep((Xsig23 - 0.5 * Xsig24), MARGIN = 1, STATS = sigx20,
    FUN = "*")
  Xsig27 <- sweep((Xsig25 + Xsig16), MARGIN = 1, STATS = sigx2,
    FUN = "*")
  Xsig28 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = sigx21/sigx4,
    FUN = "*")
  Xsig29 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = sigx18/sigx4,
    FUN = "*")
  Xsig30 <- sweep((Xsig11 + Xsig15), MARGIN = 1, STATS = sigx3,
    FUN = "*")
  Xsig31 <- sweep(Xi_gid1sq_gisq, MARGIN = 1, STATS = 1/sgsq,
    FUN = "*")
  Xsig32 <- sweep(Xgi, MARGIN = 1, STATS = sigx24 * gid2sq_gicub/sgsq^2,
    FUN = "*")
  Xsig33 <- sweep((Xsig31 - Xsig32), MARGIN = 1, STATS = S *
    sigx14, FUN = "*")
  Xsig34 <- sweep((Xsig31 - Xsig32), MARGIN = 1, STATS = S *
    sigx16, FUN = "*")
  Xsig35 <- sweep((Xsig26 + Xsig34), MARGIN = 1, STATS = sigx2,
    FUN = "*")
  Xsig36 <- sweep((Xsig12 + Xsig33), MARGIN = 1, STATS = sigx3,
    FUN = "*")
  ...
  Xsig39 <- sweep(Xgi, MARGIN = 1, STATS = S * sigx8 * dmusig1/sgsq,
    FUN = "*")
  Xsig40 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = sigx13/sigx4,
    FUN = "*")
  Xsig41 <- sweep(Xgi, MARGIN = 1, STATS = (S * (2/(ewu_h *
    gisq) - sigx26/sgsq^2)), FUN = "*")
  Xsig42 <- sweep(Xgi, MARGIN = 1, STATS = S * sigx25 * dmusig2/sgsq,
    FUN = "*")
  Xsig43 <- sweep((2 * Xsig41 + Xsig37), MARGIN = 1, STATS = pmusig2,
    FUN = "*")
  Xsig44 <- sweep(Xgi, MARGIN = 1, STATS = (sigx26 * sigx12/(ewv *
    gisq) + 0.5/sgsq^2) * dmusig2 * ewv/sqrt(ewv/gisq), FUN = "*")
  Xsig45 <- sweep((Xepsi_i - 2 * Xsig0), MARGIN = 1, STATS = (sigx29/ewv),
    FUN = "*")
  Xsig46 <- sweep((0.5 * (Xsig43 - Xsig42) + S * Xsig44 - 0.5 *
    Xsig45), MARGIN = 1, STATS = sigx2, FUN = "*")
  Xsig47 <- sweep((0.5 * (Xsig38 - Xsig39) + S * Xsig18 - 0.5 *
    Xsig17), MARGIN = 1, STATS = sigx3, FUN = "*")
  Xsig48 <- sweep((Xsig7 - Xsig8), MARGIN = 1, STATS = (sigx5 *
    sigx3 - sigx6 * sigx2) * ewu_h * gisq/(sigx4 * ewu_h *
    gisq)^2, FUN = "*")
  Xsig49 <- sweep((Xepsi_i - 2 * Xsig1), MARGIN = 1, STATS = (sigx5/ewv),
    FUN = "*")
  Xsig50 <- sweep((0.5 * Xsig20 + Xsig21), MARGIN = 1, STATS = sigx2,
    FUN = "*")
  Xsig51 <- sweep((0.5 * Xsig49 + S * Xsig22), MARGIN = 1,
    STATS = sigx3, FUN = "*")
  ...
  hessll[1:nXvar, 1:nXvar] <- 0.5 * (sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
    ...
    MARGIN = 1, STATS = wHvar * pmusig2 * 2 * (S^2/gisq) *
    ...
    MARGIN = 1, STATS = wHvar * S^2 * sigx26 * dmusig2/sigx27 *
      sigx2/ewv/sigx4, FUN = "*"), Xgi)) - (0.5 * (sapply(1:nXvar,
    function(x) crossprod(Xsq[[x]], as.matrix(wHvar * pmusig1 *
      sigx3/ewv/sigx4))) - crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * pmusig1 * 2 * (S^2/gisq) * sigx3/ewv/sigx4,
    FUN = "*"), Xgi) - crossprod(sweep((Xepsi_i - 2 * Xsig1),
    MARGIN = 1, STATS = wHvar * S * dmusig1/sgsq * sigx3/ewv/sigx4,
    FUN = "*"), Xgi)) - (0.5 * crossprod(sweep(((0.5 * Xsig5 +
    Xsig6)), MARGIN = 1, STATS = wHvar * sigx3/ewv/sigx4,
    FUN = "*"), (Xepsi_i - 2 * Xsig1)) + crossprod(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S^2 * dmusig1 * sigx22/sigx27 *
      sigx3/ewv/sigx4, FUN = "*"), Xgi))) - crossprod(sweep((Xsig7 -
    Xsig8), MARGIN = 1, STATS = wHvar/sigx4^2, FUN = "*"),
    (Xsig7 - Xsig8))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep((Xsig50 -
    Xsig51), MARGIN = 1, STATS = wHvar/(sigx4 * ewu_h * gisq),
    FUN = "*") - sweep(Xsig48, MARGIN = 1, STATS = wHvar,
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep((Xsig47 - (Xsig40 + Xsig46)),
    MARGIN = 1, STATS = wHvar/sigx4, FUN = "*"), vHvar)
  hessll[1:nXvar, nXvar + nuZUvar + nvZVvar + 1] <- colSums(sweep((Xsig30 -
    (Xsig29 + Xsig27)), MARGIN = 1, STATS = wHvar/sigx4,
    FUN = "*"))
  hessll[1:nXvar, nXvar + nuZUvar + nvZVvar + 2] <- colSums(sweep((Xsig36 -
    (Xsig28 + Xsig35)), MARGIN = 1, STATS = wHvar/sigx4,
    FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((((0.25 * ddgq - 0.5 * (0.5 * ddgq - 0.5 * pmusig1)) *
      ewv - 0.5 * (sigx5 * sigx22/gisq)) * sigx3 - sigx2 *
      (ewv * pmusig2 - sigx26 * sigx6/gisq))/(sigx4 * ewu_h^2 *
      gisq) - ((sigx5 * sigx3 - sigx6 * sigx2)/gisq + 0.5 *
      (sigx4 * ewu_h)) * (sigx5 * sigx3 - sigx6 * sigx2) *
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    ...
    ((sigx30 * sigx15/gisq + 0.5 * (sigx14 * ewv * sigx24 *
      gid2_gicub/sgsq^2)) * sigx3 - ((sigx31 * sigx17/sgsq +
      sigx16 * ewv * sigx24 * gid2_gicub/sgsq^2) * sigx2 +
      sigx18 * (sigx5 * sigx3 - sigx6 * sigx2)/(sigx4 *
        gisq)))/(sigx4 * ewu_h), FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 2)] <- colSums(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((sigx30 * sigx19/gisq + 0.5 * (sigx14 * ewv * sigx24 *
      gid2sq_gicub/sgsq^2)) * sigx3 - ((sigx31 * sigx20/sgsq +
      sigx16 * ewv * sigx24 * gid2sq_gicub/sgsq^2) * sigx2 +
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * ((sigx32 * sigx15 - (sigx33 *
      gid2_gicub + 0.5 * (S * gid1_epsi_gisq/sqrt(ewv/gisq))) *
      sigx14 * ewv/sgsq^2) * sigx3 - ((sigx34 * sigx17 -
      (sigx35 * gid2_gicub + 0.5 * (S * gid1_epsi_gisq/sqrt(ewv/gisq))) *
        sigx16 * ewv/sgsq^2) * sigx2 + sigx18 * sigx13/sigx4))/sigx4,
    FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((sigx32 * sigx19 - (sigx33 *
      gid2sq_gicub + 0.5 * (S * gid1sq_epsi_gisq/sqrt(ewv/gisq))) *
      sigx14 * ewv/sgsq^2) * sigx3 - ((sigx34 * sigx20 -
      (sigx35 * gid2sq_gicub + 0.5 * (S * gid1sq_epsi_gisq/sqrt(ewv/gisq))) *
        sigx16 * ewv/sgsq^2) * sigx2 + sigx21 * sigx13/sigx4))/sigx4,
    FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar +
    nvZVvar + 1)] <- sum(wHvar * (((sigx36 * sigx15 + sigx14 *
    (S * (gd1Zsq_gd1_epsi_gisq/sgsq - sigx24 * gid2_gicub *
      gid1_epsi_gisq/sgsq^2) - (sigx37 * gid2_gicub + sigx22 *
      sigx24 * (gd2Zsq_gd1_gicub - 2 * sigx39))/sgsq^2)) *
    sigx3 - ((sigx40 * sigx17 + sigx16 * (S * (gd1Zsq_gd1_epsi_gisq/sgsq -
    sigx24 * gid2_gicub * gid1_epsi_gisq/sgsq^2) - (sigx41 *
    gid2_gicub + sigx26 * sigx24 * (gd2Zsq_gd1_gicub - 2 *
    sigx39))/sgsq^2)) * sigx2 + sigx18^2/sigx4))/sigx4 -
    0.5 * ((gd2Zsq_gd1_gicub - gid2_gicub^2/gisq)/gisq)))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar +
    nvZVvar + 2)] <- sum(wHvar * (((sigx36 * sigx19 + sigx14 *
    (S * (gd1Zcub_gd1_epsi_gisq/sgsq - sigx24 * gid2_gicub *
      gid1sq_epsi_gisq/sgsq^2) - (sigx37 * gid2sq_gicub +
      sigx22 * sigx24 * (gd2Zcub_gd1_gicub - 2 * sigx38))/sgsq^2)) *
    sigx3 - ((sigx40 * sigx20 + sigx16 * (S * (gd1Zcub_gd1_epsi_gisq/sgsq -
    sigx24 * gid2_gicub * gid1sq_epsi_gisq/sgsq^2) - (sigx41 *
    gid2sq_gicub + sigx26 * sigx24 * (gd2Zcub_gd1_gicub -
    2 * sigx38))/sgsq^2)) * sigx2 + sigx18 * sigx21/sigx4))/sigx4 -
    0.5 * ((gd2Zcub_gd1_gicub - gid2_gicub * gid2sq_gicub/gisq)/gisq)))
  hessll[(nXvar + nuZUvar + nvZVvar + 2), (nXvar + nuZUvar +
    nvZVvar + 2)] <- sum(wHvar * (((((sigx22 * sigx19/(ewv *
    gisq) - sigx24 * gid2sq_gicub/sgsq^2) * sigx22 * pmusig1 +
    (S * pmusig1 * gid1sq_epsi_gisq - dmusig1 * sigx22 *
      sigx19)/sgsq) * sigx19 + sigx14 * (S * sigx42 - ((sigx43 *
    ewv * sigx22 * gid2sq_gicub + S * sigx24 * gid1sq_epsi_gisq) *
    gid2sq_gicub + sigx22 * sigx24 * (gd2Zfour_gd1_gicub -
    2 * sigx44))/sgsq^2)) * sigx3 - ((((sigx26 * sigx20/(ewv *
    gisq) - sigx24 * gid2sq_gicub/sgsq^2) * sigx26 * pmusig2 +
    (S * pmusig2 * gid1sq_epsi_gisq - sigx26 * dmusig2 *
      sigx20)/sgsq) * sigx20 + sigx16 * (S * sigx42 - ((sigx43 *
    sigx26 * ewv * gid2sq_gicub + S * sigx24 * gid1sq_epsi_gisq) *
    gid2sq_gicub + sigx26 * sigx24 * (gd2Zfour_gd1_gicub -
    2 * sigx44))/sgsq^2)) * sigx2 + sigx21^2/sigx4))/sigx4 -
...
#' @noRd
genexponormAlgOpt_k90 <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(pgenexponormlike_k90(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      -sum(pgenexponormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p))
    }, gr = function(parm) {
      -colSums(pgradgenexponormlike_k90(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p))
    }, hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      maxeval = itermax, stepmax = stepmax, xtol = tol,
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgenexponormlike_k90,
    grad = pgradgenexponormlike_k90, hess = phessgenexponormlike_k90,
    ...
    fn = function(parm) {
      ...
      as(-phessgenexponormlike_k90(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    ...
    fn = function(parm) {
      ...
      -phessgenexponormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgenexponormlike_k90(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessgenexponormlike_k90(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (method == "sr1") {
      mleObj$hessian <- phessgenexponormlike_k90(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- pgenexponormlike_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradgenexponormlike_k90(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
