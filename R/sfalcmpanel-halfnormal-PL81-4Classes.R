...
#' @noRd
pLCMhalfnormlike4C_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @noRd
psLCMfhalfnorm4C_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
  ...
  StartVal <- c(Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar -
    ...
    0.98 * Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar - 1), 0.98 * Esti[nXvar +
      ...
      2], if (nvZVvar > 1) rep(0, nvZVvar - 1), 0.98 * Esti[1:(nXvar)], 0.98 *
      Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar - 1), 0.98 * Esti[nXvar +
      2], if (nvZVvar > 1) rep(0, nvZVvar - 1), rep(0, 3 * nZHvar))
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike4C_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  sigx4_4 <- (wzdeno * sigx3_4 * pmu4 * sigmasq4/piuv4)
  sigx5 <- ((2 * sigx4_1 + 2 * sigx4_2 + 2 * sigx4_3)/(1 + ewz1 + ewz2 + ewz3) +
    2 * sigx4_4)
  ...
  gradll <- cbind(sweep((0.5 * Xsig3 + Xsig2), MARGIN = 1, STATS = -(2 * (sigx3_1 *
    ewz1 * sigmasq1/(sigx5 * (1 + ewz1 + ewz2 + ewz3) * (2 * pi)^(TT/2) * ewu1_h *
    ewv1_h))), FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx11_1 * sigx3_1 *
    ewz1/(sigx5 * (1 + ewz1 + ewz2 + ewz3))), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = 2 * (sigx18_1 * sigx3_1 * ewz1/(sigx5 * (1 + ewz1 + ewz2 + ewz3))),
    FUN = "*"), sweep((0.5 * Xsig6 + Xsig5), MARGIN = 1, STATS = -(2 * (sigx3_2 *
    ewz2 * sigmasq2/(sigx5 * (1 + ewz1 + ewz2 + ewz3) * (2 * pi)^(TT/2) * ewu2_h *
    ewv2_h))), FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx11_2 * sigx3_2 *
    ewz2/(sigx5 * (1 + ewz1 + ewz2 + ewz3))), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = 2 * (sigx18_2 * sigx3_2 * ewz2/(sigx5 * (1 + ewz1 + ewz2 + ewz3))),
    FUN = "*"), sweep((0.5 * Xsig9 + Xsig8), MARGIN = 1, STATS = -(2 * (sigx3_3 *
    ewz3 * sigmasq3/(sigx5 * (1 + ewz1 + ewz2 + ewz3) * (2 * pi)^(TT/2) * ewu3_h *
    ewv3_h))), FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx11_3 * sigx3_3 *
    ewz3/(sigx5 * (1 + ewz1 + ewz2 + ewz3))), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = 2 * (sigx18_3 * sigx3_3 * ewz3/(sigx5 * (1 + ewz1 + ewz2 + ewz3))),
    FUN = "*"), sweep((0.5 * Xsig12 + Xsig11), MARGIN = 1, STATS = -(2 * (wzdeno *
    sigx3_4 * sigmasq4/(sigx5 * (2 * pi)^(TT/2) * ewu4_h * ewv4_h))), FUN = "*"),
    sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx11_4 * wzdeno * sigx3_4/sigx5),
      FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (sigx18_4 * wzdeno *
      sigx3_4/sigx5), FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = (2 * (sigx3_1 *
      pmu1 * sigmasq1/piuv1) - sigx5) * ewz1/(sigx5 * (1 + ewz1 + ewz2 + ewz3)),
      FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = (2 * (sigx3_2 * pmu2 * sigmasq2/piuv2) -
      sigx5) * ewz2/(sigx5 * (1 + ewz1 + ewz2 + ewz3)), FUN = "*"), sweep(Zvar,
      MARGIN = 1, STATS = (2 * (sigx3_3 * pmu3 * sigmasq3/piuv3) - sigx5) *
        ewz3/(sigx5 * (1 + ewz1 + ewz2 + ewz3)), FUN = "*"))
...
#' @noRd
LCM4ChnormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm4C_pl81(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike4C_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike4C_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, gr = function(parm) {
    -colSums(pgradLCMhalfnormlike4C_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike4C_pl81,
    grad = pgradLCMhalfnormlike4C_pl81, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) {
      -sum(pLCMhalfnormlike4C_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike4C_pl81(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike4C_pl81(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike4C_pl81(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike4C_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike4C_pl81(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar,
    nZHvar = nZHvar)
  if (is.null(start)) {
...
#' @noRd
pLCM4Chalfnormeff_pl81 <- function(object, level) {
