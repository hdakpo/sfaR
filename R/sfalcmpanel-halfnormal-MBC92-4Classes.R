...
#' @noRd
pLCMhalfnormlike4C_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @noRd
psLCMfhalfnorm4C_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike4C_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  gradll <- cbind(sweep((0.5 * Xsig5 + Xsig3), MARGIN = 1, STATS = -(2 * (sigx5_1 *
    ...
    sweep(vHvar, MARGIN = 1, STATS = 2 * (sigx17_1 * sigx5_1 * ewz1/(sigx7bis *
      ...
      MARGIN = 1, STATS = 2 * (sigx17_2 * sigx5_2 * ewz2/(sigx7bis * (1 + ewz1 +
        ewz2 + ewz3))), FUN = "*"), 2 * (sigx5_2 * ewu2 * ewz2 * sigx24_2/sigx25),
    2 * (sigx5_2 * ewu2 * ewz2 * sigx21_2/sigx25), sweep((0.5 * Xsig9 + Xsig8),
    ...
    2 * (sigx5_3 * ewu3 * ewz3 * sigx24_3/sigx25), 2 * (sigx5_3 * ewu3 * ewz3 *
      sigx21_3/sigx25), sweep((0.5 * Xsig12 + Xsig11), MARGIN = 1, STATS = -(2 *
    ...
    2 * (wzdeno * sigx5_4 * ewu4 * sigx24_4/sigx26), 2 * (wzdeno * sigx5_4 *
      ewu4 * sigx21_4/sigx26), sweep(Zvar, MARGIN = 1, STATS = (2 * sigx27 -
...
#' @noRd
LCM4ChnormAlgOpt_mbc92 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm4C_mbc92(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike4C_mbc92(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike4C_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, gr = function(parm) {
    -colSums(pgradLCMhalfnormlike4C_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike4C_mbc92,
    grad = pgradLCMhalfnormlike4C_mbc92, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) {
      -sum(pLCMhalfnormlike4C_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike4C_mbc92(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike4C_mbc92(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike4C_mbc92(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike4C_mbc92(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike4C_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar,
    nZHvar = nZHvar)
  if (is.null(start)) {
...
#' @noRd
pLCM4Chalfnormeff_mbc92 <- function(object, level) {
