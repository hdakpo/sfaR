...
#' @noRd
pweibullnormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar, ngZGvar, gHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
...
#' @noRd
pstweibullnorm_gzit <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar,
  vHvar, Yvar, Xvar, ngZGvar, gHvar, S, wHvar, modelType, initIter, initAlg, whichStart,
  printInfo, tol, N, FiMat) {
  if (whichStart == 1L) {
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar)), paste0("Zv_",
    colnames(vHvar)), "k", if (modelType %in% c("bc92a", "kw05")) {
    "eta"
  } else {
    if (modelType == "bc92b") {
      c("eta1", "eta2")
...
#' @param FiMat matrix of random draws
#' @noRd
pgradweibullnormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @noRd
weibullnormAlgOpt_gzit <- function(start, randStart, sdStart, olsParam, dataTable,
  S, modelType, gHvar, ngZGvar, nXvar, N, NT, FiMat_N, FiMat_NT, uHvar_c, uHvar_p,
  ...
  if (!is.null(start)) {
    ...
    start_st <- pstweibullnorm_gzit(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      ...
      modelType = modelType, wHvar = wHvar_c, whichStart = whichStart, tol = tol,
      printInfo = printInfo)
  ...
  startLoglik <- sum(pweibullnormlike_gzit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pweibullnormlike_gzit(parm,
    ...
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_gzit(parm,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pweibullnormlike_gzit,
    ...
    fn = function(parm) -sum(pweibullnormlike_gzit(parm, nXvar = nXvar, gHvar = gHvar,
      ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pweibullnormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradweibullnormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradweibullnormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradweibullnormlike_gzit(mleObj$par, nXvar = nXvar,
  ...
  mleObj$logL_OBS <- pweibullnormlike_gzit(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradweibullnormlike_gzit(parm = mlParam, nXvar = nXvar,
    gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffWeibull_gzit <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffWeibull_gzit <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq, git) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffWeibull_gzit <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq,
  giepsi, gisq, git) {
  exp(git * u) * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) *
    exp(-(u/sigmaU)^k) * exp(-(epsilon_isq + 2 * S * u * giepsi + u^2 * gisq)/(2 *
    sigmaV^2))
...
pweibullnormeff_gzit <- function(object, level) {
  ...
  for (i in seq_along(1:object$Nid)) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * giepsi[i] + ur^2 * gisq[i])/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffWeibull_gzit, lowerLimit = 0, upperLimit = Inf,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in seq_along(1:object$Nid)) {
      res$teBC[i] <- hcubature(f = fnCondBCEffWeibull_gzit, lowerLimit = 0,
