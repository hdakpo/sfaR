...
#' @noRd
plognormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, muHvar, uHvar,
  vHvar, Yvar, Xvar, pindex, TT, S, wHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
...
#' @noRd
pstlognorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, nmuZUvar, muHvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, whichStart, initIter, initAlg, printInfo,
  tol, N, FiMat) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradlognormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, muHvar,
  ...
  EqFi <- exp(sweep(sweep(qFimat, MARGIN = 1, STATS = ewu_h, FUN = "*"), MARGIN = 1,
    STATS = mu, FUN = "+"))
  EqFi_epsi <- sweep(sweep(EqFi, MARGIN = 1, STATS = TT, FUN = "*"), MARGIN = 1,
    STATS = 2 * (S * epsilon_i), FUN = "+")
  ...
  sigFi4 <- sweep(sigFi2 * EqFi * qFimat, MARGIN = 1, STATS = ewu_h/(2 * ewv),
    FUN = "*") * (0.5 * EqFi_epsi + sweep(EqFi, MARGIN = 1, STATS = 0.5 * (TT),
    FUN = "*"))
  sigFi5 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*") *
    (sweep(sweep(EqFi, MARGIN = 1, STATS = 2 * (TT), FUN = "*"), MARGIN = 1,
      STATS = 2 * (S * epsilon_i), FUN = "+"))
  sigFi6 <- sweep(sigFi2, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*")
  gx <- matrix(nrow = N, ncol = nXvar)
  for (k in 1:nXvar) {
    gx[, k] <- -apply(sweep(2 * (S * EqFi) * sigFi6, MARGIN = 1, STATS = X_iM[,
      k], FUN = "*") + sweep(2 * sigFi6, MARGIN = 1, STATS = Xepsi_i[, k],
      FUN = "*"), 1, sum)/sigDiv
  }
...
#' @noRd
lognormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  if (!is.null(start)) {
    ...
    start_st <- pstlognorm_pl81(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(plognormlike_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(plognormlike_pl81(parm,
    ...
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_pl81(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
    ...
    control = list(trace = if (printInfo) 1 else 0, maxeval = itermax, stepmax = stepmax,
      xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = plognormlike_pl81,
    grad = pgradlognormlike_pl81, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) -sum(plognormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    ...
    fn = function(parm) -sum(plognormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N,
      FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradlognormlike_pl81(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N,
    ...
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradlognormlike_pl81(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradlognormlike_pl81(mleObj$par, nXvar = nXvar,
  ...
  mleObj$logL_OBS <- plognormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradlognormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffLogNorm_pl81 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_i, epsilon_isq) {
  1/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * dnorm((log(u) - mu)/sigmaU) * exp(-(epsilon_isq +
    2 * S * u * epsilon_i + u^2)/(2 * sigmaV^2))
}
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffLogNorm_pl81 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_i, epsilon_isq) {
  exp(-u)/u * 1/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * dnorm((log(u) - mu)/sigmaU) *
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffLogNorm_pl81 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_i,
  epsilon_isq) {
  exp(u)/u * 1/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * dnorm((log(u) - mu)/sigmaU) *
    exp(-(epsilon_isq + 2 * S * u * epsilon_i + u^2)/(2 * sigmaV^2))
}
...
#' @noRd
plognormeff_pl81 <- function(object, level) {
  ...
  for (i in 1:object$Nid) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * epsilon_i[i] + ur^2)/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffLogNorm_pl81, lowerLimit = 0, upperLimit = Inf,
      maxEval = 100, fDim = 1, sigmaU = exp(Wu[i]/2), sigmaV = exp(Wv[i]/2),
      mu = mu[i], epsilon_i = epsilon_i[i], epsilon_isq = epsilon_isq[i], TT = TT[i],
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in 1:object$Nid) {
      teBC[i] <- hcubature(f = fnCondBCEffLogNorm_pl81, lowerLimit = 0, upperLimit = Inf,
        maxEval = 100, fDim = 1, sigmaU = exp(Wu[i]/2), sigmaV = exp(Wv[i]/2),
      ...
      teBC_reciprocal[i] <- hcubature(f = fnCondBCreciprocalEffLogNorm_pl81,
        ...
        sigmaV = exp(Wv[i]/2), mu = mu[i], epsilon_i = epsilon_i[i], epsilon_isq = epsilon_isq[i],
        TT = TT[i], S = object$S, vectorInterface = FALSE, tol = 1e-15)$integral/density_epsilon_vec[i]
    }
