...
#' @noRd
pexponormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  ll <- -1/2 * Wu - (TT - 1)/2 * Wv - (TT - 1)/2 * log(2 * pi) - 1/2 * log(TT) -
...
#' @noRd
pstexponorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradexponormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  gradll <- cbind(sweep(X_iM, MARGIN = 1, STATS = S * (epsi_vu/ewv - dmusig/sigx3)/TT,
    FUN = "*") - sweep(Xepsi_i, MARGIN = 1, STATS = 1/ewv, FUN = "*"), sweep(uHvar,
    MARGIN = 1, STATS = ((0.5 * sigx4 - 0.5 * epsi_vu)/(wutt) - 0.5), FUN = "*"),
    sweep(vHvar, MARGIN = 1, STATS = (sigx5 * sigx2 + 2 * (ewv * epsilon_isq/(2 *
      ewv)^2) - 0.5 * (TT - 1)), FUN = "*"))
...
#' @param TT vector of time of presence
#' @noRd
phessexponormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx10 <- ((0.5 * (epsi_sig3) - 0.5 * (sigx9)) * dmusig/TT + 0.5)
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(X_iM, MARGIN = 1, STATS = wHvar *
    S^2 * (1/ewv - dmusig * sigx8/TT)/TT, FUN = "*"), X_iM) - sapply(1:nXvar,
    function(x) {
      crossprod(Xsq[[x]], as.matrix(wHvar/ewv))
    })
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(X_iM, MARGIN = 1,
    STATS = wHvar * S * (0.5 * (dmusig * (sigx9 - epsi_sig3)/TT) - 0.5)/(wutt),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xepsi_i,
    MARGIN = 1, STATS = 2 * wHvar * (ewv * 2/(2 * ewv)^2), FUN = "*"), vHvar) +
    crossprod(sweep(X_iM, MARGIN = 1, STATS = wHvar * S * ((1 - dmusig * sigx8 *
      wvtt) * sigx2 - 0.5 * (sigx5/ssq_v^2)), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((0.25 + 0.5 * ((0.5 * (epsi_sig3) - 0.5 * (sigx9)) *
      dmusig/TT)) * ewv/(wuttsq) - 0.5 * (TT * (0.5 * sigx4 - 0.5 * epsi_vu) *
      ewu_h/(wutt)^2)), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (sigx5 *
    (0.25 * (ewv/(ewu_h * ssq_v^2)) - 0.5 * (wutt/(wutt)^2)) - sigx10 * sigx2 *
    ewv/ewu_h), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    ((((sigx2 * epsi_vu - 1)/sigx3 + sigx11) * dmusig + 1/ewu_h) * sigx2 + 2 *
      ((1 - 8 * (ewv^2/(2 * ewv)^2)) * epsilon_isq/(2 * ewv)^2) - 0.5 * (sigx5 *
      (1/ewu_h - TT * epsi_vu/ssq_v^2)/ssq_v^2)) * ewv, FUN = "*"), vHvar)
...
#' @noRd
exponormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(pexponormlike_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pexponormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradexponormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pexponormlike_pl81,
    grad = pgradexponormlike_pl81, hess = phessexponormlike_pl81, start = startVal,
    ...
    fn = function(parm) {
      ...
      -colSums(pgradexponormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phessexponormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      -phessexponormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradexponormlike_pl81(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessexponormlike_pl81(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phessexponormlike_pl81(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- pexponormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradexponormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
