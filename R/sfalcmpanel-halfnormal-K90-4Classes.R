...
#' @noRd
pLCMhalfnormlike4C_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  theta1 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 9):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + nZHvar + 8)]
  theta2 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 9):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 8)]
  theta3 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 9):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar + 8)]
  ...
  git3 <- 1/unlist(lapply(TT, FUN = function(x) 1 + exp(eta3_1 * seq(1:x) + eta3_2 *
    (seq(1:x))^2)))
  ...
  git4 <- 1/unlist(lapply(TT, FUN = function(x) 1 + exp(eta4_1 * seq(1:x) + eta4_2 *
    (seq(1:x))^2)))
...
#' @noRd
psLCMfhalfnorm4C_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar_p)),
    ...
    "eta1", "eta2", names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar_p)), paste0("Zv_",
      colnames(vHvar_p)), "eta1", "eta2", paste0("Cl1_", colnames(Zvar)), paste0("Cl2_",
      colnames(Zvar)), paste0("Cl3_", colnames(Zvar)))
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike4C_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  gitd1_3 <- Zit * exp(Zit * (eta3_1 + eta3_2 * Zit))
  gitd1_epsit_gitsq3 <- -gitd1_3 * epsilon_it3 * git3^2
  gid1_epsi_gisq3 <- as.numeric(tapply(gitd1_epsit_gitsq3, pindex[, 1], sum))
  gitd1_4 <- Zit * exp(Zit * (eta4_1 + eta4_2 * Zit))
  gitd1_epsit_gitsq4 <- -gitd1_4 * epsilon_it4 * git4^2
  gid1_epsi_gisq4 <- as.numeric(tapply(gitd1_epsit_gitsq4, pindex[, 1], sum))
  ...
  gitd2_3 <- 2 * Zit * exp(Zit * (eta3_1 + eta3_2 * Zit))
  gitd2_gitcub3 <- -gitd1_3 * git3^3
  gid2_gicub3 <- as.numeric(tapply(gitd2_gitcub3, pindex[, 1], sum))
  gitd2_4 <- 2 * Zit * exp(Zit * (eta4_1 + eta4_2 * Zit))
  gitd2_gitcub4 <- -gitd1_4 * git4^3
  gid2_gicub4 <- as.numeric(tapply(gitd2_gitcub4, pindex[, 1], sum))
  ...
  gitd1sq_epsit_gitsq3 <- -Zit^2 * exp(Zit * (eta3_1 + eta3_2 * Zit)) * epsilon_it3 *
    git3^2
  gid1sq_epsi_gisq3 <- as.numeric(tapply(gitd1sq_epsit_gitsq3, pindex[, 1], sum))
  gitd1sq_epsit_gitsq4 <- -Zit^2 * exp(Zit * (eta4_1 + eta4_2 * Zit)) * epsilon_it4 *
    git4^2
  gid1sq_epsi_gisq4 <- as.numeric(tapply(gitd1sq_epsit_gitsq4, pindex[, 1], sum))
  ...
  gitd2sq_gitcub3 <- -2 * Zit^2 * exp(Zit * (eta3_1 + eta3_2 * Zit)) * git3^3
  gid2sq_gicub3 <- as.numeric(tapply(gitd2sq_gitcub3, pindex[, 1], sum))
  gitd2sq_gitcub4 <- -2 * Zit^2 * exp(Zit * (eta4_1 + eta4_2 * Zit)) * git4^3
  gid2sq_gicub4 <- as.numeric(tapply(gitd2sq_gitcub4, pindex[, 1], sum))
  ewu1 <- exp(Wu1)
  ewu2 <- exp(Wu2)
  ewu3 <- exp(Wu3)
  ewu4 <- exp(Wu4)
  ...
  wzdeno <- (1 - (ewz1 + ewz2 + ewz3)/(1 + ewz1 + ewz2 + ewz3))
  ...
  sigx7_4 <- (wzdeno * sigx5_4 * pmusig4 * sigmastar4/sigx6_4)
  sigx7bis <- ((2 * sigx7_1 + 2 * sigx7_2 + 2 * sigx7_3)/(1 + ewz1 + ewz2 + ewz3) +
    2 * sigx7_4)
  sigx8 <- (sigx7bis * (1 + ewz1 + ewz2 + ewz3) * (2 * pi)^(TT/2) * ewu1_h * ewv1_h)
  ...
  sigx19_3 <- (gid1_epsi_gisq3/ssq3 - ewu3 * sigx18_3 * gid2_gicub3 * giepsi3/ssq3^2)
  sigx19_4 <- (gid1_epsi_gisq4/ssq4 - ewu4 * sigx18_4 * gid2_gicub4 * giepsi4/ssq4^2)
  sigx20_1 <- (ewu1 * ewv1 * pmusig1 * gid2_gicub1/(sigmasq1^2 * sigmastar1))
  sigx20_2 <- (ewu2 * ewv2 * pmusig2 * gid2_gicub2/(sigmasq2^2 * sigmastar2))
  sigx20_3 <- (ewu3 * ewv3 * pmusig3 * gid2_gicub3/(sigmasq3^2 * sigmastar3))
  sigx20_4 <- (ewu4 * ewv4 * pmusig4 * gid2_gicub4/(sigmasq4^2 * sigmastar4))
  ...
  sigx22_3 <- (gid1sq_epsi_gisq3/ssq3 - ewu3 * sigx18_3 * gid2sq_gicub3 * giepsi3/ssq3^2)
  sigx22_4 <- (gid1sq_epsi_gisq4/ssq4 - ewu4 * sigx18_4 * gid2sq_gicub4 * giepsi4/ssq4^2)
  ...
  sigx23_3 <- (ewu3 * ewv3 * pmusig3 * gid2sq_gicub3/(sigmasq3^2 * sigmastar3))
  sigx23_4 <- (ewu4 * ewv4 * pmusig4 * gid2sq_gicub4/(sigmasq4^2 * sigmastar4))
  ...
  sigx25 <- (sigx7bis * (1 + ewz1 + ewz2 + ewz3) * (2 * pi)^(TT/2) * ewu2_h * ewv2_h)
  sigx26 <- (sigx7bis * (2 * pi)^(TT/2) * ewu4_h * ewv4_h)
  ...
  gradll <- cbind(sweep((0.5 * Xsig5 + Xsig3), MARGIN = 1, STATS = -(2 * (sigx5_1 *
    ...
    (sigx11_1 * sigx5_1 * ewz1/(sigx7bis * (1 + ewz1 + ewz2 + ewz3))), FUN = "*"),
    sweep(vHvar, MARGIN = 1, STATS = 2 * (sigx17_1 * sigx5_1 * ewz1/(sigx7bis *
      (1 + ewz1 + ewz2 + ewz3))), FUN = "*"), 2 * (sigx5_1 * ewu1 * ewz1 *
      sigx21_1/sigx8), 2 * (sigx5_1 * ewu1 * ewz1 * sigx24_1/sigx8), sweep((0.5 *
      Xsig6 + Xsig4), MARGIN = 1, STATS = -(2 * (sigx5_2 * ewz2 * sigmastar2/sigx25)),
      FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx11_2 * sigx5_2 *
      ewz2/(sigx7bis * (1 + ewz1 + ewz2 + ewz3))), FUN = "*"), sweep(vHvar,
      MARGIN = 1, STATS = 2 * (sigx17_2 * sigx5_2 * ewz2/(sigx7bis * (1 + ewz1 +
        ewz2 + ewz3))), FUN = "*"), 2 * (sigx5_2 * ewu2 * ewz2 * sigx21_2/sigx25),
    2 * (sigx5_2 * ewu2 * ewz2 * sigx24_2/sigx25), sweep((0.5 * Xsig9 + Xsig8),
      MARGIN = 1, STATS = -(2 * (sigx5_3 * ewz3 * sigmastar3/sigx25)), FUN = "*"),
    sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx11_3 * sigx5_3 * ewz3/(sigx7bis *
      (1 + ewz1 + ewz2 + ewz3))), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 *
      (sigx17_3 * sigx5_3 * ewz3/(sigx7bis * (1 + ewz1 + ewz2 + ewz3))), FUN = "*"),
    2 * (sigx5_3 * ewu3 * ewz3 * sigx21_3/sigx25), 2 * (sigx5_3 * ewu3 * ewz3 *
      sigx24_3/sigx25), sweep((0.5 * Xsig12 + Xsig11), MARGIN = 1, STATS = -(2 *
      (wzdeno * sigx5_4 * sigmastar4/sigx26)), FUN = "*"), sweep(uHvar, MARGIN = 1,
      STATS = 2 * (sigx11_4 * wzdeno * sigx5_4/sigx7bis), FUN = "*"), sweep(vHvar,
      MARGIN = 1, STATS = 2 * (sigx17_4 * wzdeno * sigx5_4/sigx7bis), FUN = "*"),
    2 * (wzdeno * sigx5_4 * ewu4 * sigx21_4/sigx26), 2 * (wzdeno * sigx5_4 *
      ewu4 * sigx24_4/sigx26), sweep(Zvar, MARGIN = 1, STATS = (2 * sigx27 -
      sigx7bis) * ewz1/(sigx7bis * (1 + ewz1 + ewz2 + ewz3)), FUN = "*"), sweep(Zvar,
      MARGIN = 1, STATS = (2 * sigx28 - sigx7bis) * ewz2/(sigx7bis * (1 + ewz1 +
        ewz2 + ewz3)), FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = (2 * sigx29 -
      sigx7bis) * ewz3/(sigx7bis * (1 + ewz1 + ewz2 + ewz3)), FUN = "*"))
...
#' @noRd
LCM4ChnormAlgOpt_k90 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm4C_k90(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike4C_k90(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike4C_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, gr = function(parm) {
    -colSums(pgradLCMhalfnormlike4C_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike4C_k90,
    grad = pgradLCMhalfnormlike4C_k90, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike4C_k90(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike4C_k90(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike4C_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike4C_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike4C_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar,
    nZHvar = nZHvar)
  if (is.null(start)) {
...
#' @noRd
pLCM4Chalfnormeff_k90 <- function(object, level) {
  ...
  theta1 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    9):(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar + object$nZHvar +
    8)]
  theta2 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    object$nZHvar + 9):(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    2 * object$nZHvar + 8)]
  theta3 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    2 * object$nZHvar + 9):(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    3 * object$nZHvar + 8)]
