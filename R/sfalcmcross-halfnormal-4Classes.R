...
#' @noRd
cLCMhalfnormlike4C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  phi4 <- parm[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar)]
  theta1 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + nZHvar)]
  theta2 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)]
  theta3 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)]
...
#' @noRd
csLCMfhalfnorm4C <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  StartVal <- c(Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar -
    ...
    Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar - 1), Esti[nXvar + 2], if (nvZVvar >
      ...
      1), 0.98 * Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0,
      nuZUvar - 1), Esti[nXvar + 2], if (nvZVvar > 1) rep(0, nvZVvar - 1),
    rep(0, 3 * nZHvar))
...
#' @param nZHvar number of separating variables
#' @noRd
cgradLCMhalfnormlike4C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx4 <- ((2 * wzlogit1 + 2 * wzlogit2 + 2 * wzlogit3)/wzdeno + 2 * wzlogit4)
  ...
  sigx6_4 <- (S * (dpepsisq4 - sqewu4 * dmusig4 * depsisq4) * (epsilon4) - 0.5 *
    dpsq4)
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = 2 * (S * sigx1_1 * ewz1/sigsq_1),
    ...
    FUN = "*"), sweep(Xvar, MARGIN = 1, STATS = 2 * (S * prC * sigx1_4/sigsq_4),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (prC * ewu4 * sigx6_4/(sigwz4)),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (prC * ewv4 * sigx7_4/(sigwz4)),
...
#' @param nZHvar number of separating variables
#' @noRd
chessLCMhalfnormlike4C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx10_2 <- (0.5 * (S * (dmusig2 * ewu2/sigmastar2 + S * pmusig2 * (epsilon2)) *
    (epsilon2)/(sigma_sq2) - pmusig2) - 0.5 * pmusig2) * dep2sq2
  sigx10_3 <- (0.5 * (S * (dmusig3 * ewu3/sigmastar3 + S * pmusig3 * (epsilon3)) *
    (epsilon3)/(sigma_sq3) - pmusig3) - 0.5 * pmusig3) * dep3sq3
  sigx10_4 <- (0.5 * (S * (dmusig4 * ewu4/sigmastar4 + S * pmusig4 * (epsilon4)) *
    (epsilon4)/(sigma_sq4) - pmusig4) - 0.5 * pmusig4) * dep4sq4
  ...
  sigx4w2z <- ((2 - 2 * (ewz1/wzdeno))/wzdsig - 2 * ((2 * dpsq1 - sigx4) * ewz1/wzdsig^2))
  sigx4w3z <- (2 * ((2 * dpsq1 - sigx4)/wzdsig^2) + 2/(sigx4wzsq))
  sigx4w4z <- ((2 - 2 * (ewz2/wzdeno))/wzdsig - 2 * ((2 * dpsq2 - sigx4) * ewz2/wzdsig^2))
  sigx4w5z <- (2 * (wzdeno * (2 * dpsq1 - sigx4)/wzdsig^2) + 2/wzdsig)
  sigx4w6z <- (2 * (wzdeno * (2 * dpsq2 - sigx4)/wzdsig^2) + 2/wzdsig)
  sigx4w7z <- ((2 * dpsq1 - sigx4) * sqrt(sigma_sq4)/(sigwz4)^2 + 1/(sigwz4))
  sigx4w8z <- ((2 * dpsq2 - sigx4) * sqrt(sigma_sq4)/(sigwz4)^2 + 1/(sigwz4))
  sigx4w9z <- (0.5 * (sigx4/sqrt(sigma_sq4)) + 2 * (prC * sigx6_4))
  sigx4w10z <- (sigx4 * (sigma_sq4)/sqrt(sigma_sq4))
  sigx4w11z <- (0.5 * (sigx4/sqrt(sigma_sq4)) + 2 * (prC * sigx7_4))
  sigx4w12z <- (2 * ((2 * dpsq3 - sigx4)/wzdsig^2) + 2/(sigx4wzsq))
  sigx4w13z <- ((2 - 2 * (ewz3/wzdeno))/wzdsig - 2 * ((2 * dpsq3 - sigx4) * ewz3/wzdsig^2))
  sigx4w14z <- (2 * (wzdeno * (2 * dpsq3 - sigx4)/wzdsig^2) + 2/wzdsig)
  ...
  hessll[1:nXvar, (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 3 *
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 *
    (S^2 * prC * sigx1_1 * sigx1_4 * (sigma_sq4) * ewz1 * sqrt(sigma_sq4)/(sigsq_4^2 *
      wzdeno * (sigma_sq1)^(3/2)))) * wHvar, FUN = "*"), Xvar)
  hessll[1:nXvar, (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 *
    (S * prC * sigx1_1 * ewu4 * ewz1 * sigx6_4 * sqrt(sigma_sq4)/wzsigx4_1)) *
    wHvar, FUN = "*"), uHvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 *
    (S * prC * sigx1_1 * ewv4 * ewz1 * sigx7_4 * sqrt(sigma_sq4)/wzsigx4_1)) *
    wHvar, FUN = "*"), vHvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S *
    sigx4w2z * sigx1_1 * ewz1/((sigma_sq1)^(3/2)) * wHvar, FUN = "*"), Zvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * sigx4wz * sigx1_1 * ewz1 * ewz2/((sigma_sq1)^(3/2))) * wHvar,
    FUN = "*"), Zvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w12z * sigx1_1 * ewz1 * ewz3/((sigma_sq1)^(3/2))) *
      wHvar, FUN = "*"), Zvar)
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    ...
    STATS = -(4 * (S * prC * sigx1_4 * ewu1 * (sigma_sq4) * ewz1 * sigx6_1 *
      sqrt(sigma_sq4)/(sigsq_4^2 * wzdsq1))) * wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    1):(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewu1 * ewu4 * ewz1 * sigx6_1 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 4 * nuZUvar + 3 * nvZVvar +
    ...
    STATS = -(4 * (prC * ewu1 * ewv4 * ewz1 * sigx7_4 * sigx6_1 * sqrt(sigma_sq4)/((sigwz4)^2 *
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    ...
    MARGIN = 1, STATS = sigx4w2z * ewu1 * ewz1 * sigx6_1/sqrt(sigma_sq1) * wHvar,
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    nZHvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4wz * ewu1 * ewz1 * ewz2 * sigx6_1/sqrt(sigma_sq1)) *
      wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    2 * nZHvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4w12z * ewu1 * ewz1 * ewz3 * sigx6_1/sqrt(sigma_sq1)) *
      wHvar, FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 3 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_4 * (sigma_sq4) * ewv1 * ewz1 *
      sigx7_1 * sqrt(sigma_sq4)/(sigsq_4^2 * wzdsq1))) * wHvar, FUN = "*"),
    Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (4 * nXvar + 3 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (prC * ewu4 * ewv1 * ewz1 * sigx7_1 * sigx6_4 *
      sqrt(sigma_sq4)/((sigwz4)^2 * wzdsq1))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (4 * nXvar + 4 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (prC * ewv1 * ewv4 * ewz1 * sigx7_1 * sigx7_4 *
      sqrt(sigma_sq4)/((sigwz4)^2 * wzdsq1))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx4w2z * ewv1 * ewz1 * sigx7_1/sqrt(sigma_sq1) * wHvar,
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + nZHvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4wz * ewv1 * ewz1 * ewz2 * sigx7_1/sqrt(sigma_sq1)) *
      wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 *
    nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx4w12z * ewv1 *
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
    ...
    MARGIN = 1, STATS = -(4 * (S^2 * prC * sigx1_2 * sigx1_4 * (sigma_sq4) *
      ewz2 * sqrt(sigma_sq4)/(sigsq_4^2 * wzdeno * (sigma_sq2)^(3/2)))) * wHvar,
    FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (4 *
    ...
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_2 * ewu4 * ewz2 * sigx6_4 * sqrt(sigma_sq4)/wzsigx4_2)) *
      wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (4 *
    nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_2 * ewv4 * ewz2 * sigx7_4 * sqrt(sigma_sq4)/wzsigx4_2)) *
      wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S * sigx4w3z * sigx1_2 *
    ewz1 * ewz2/((sigma_sq2)^(3/2))) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S *
    sigx4w4z * sigx1_2 * ewz2/((sigma_sq2)^(3/2)) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx4w12z * sigx1_2 * ewz2 * ewz3/((sigma_sq2)^(3/2))) * wHvar, FUN = "*"),
    Zvar)
  ...
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    ...
    STATS = -(sigx4w12z * ewu2 * ewz2 * ewz3 * sigx6_2/sqrt(sigma_sq2)) * wHvar,
    FUN = "*"), Zvar)
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    ...
    3 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(4 * (S * prC *
    sigx1_4 * (sigma_sq4) * ewv2 * ewz2 * sigx7_2 * sqrt(sigma_sq4)/(sigsq_4^2 *
    wzdsq2))) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(4 * (prC *
    ewu4 * ewv2 * ewz2 * sigx7_2 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 * wzdsq2))) *
    wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(4 * (prC *
    ewv2 * ewv4 * ewz2 * sigx7_2 * sigx7_4 * sqrt(sigma_sq4)/((sigwz4)^2 * wzdsq2))) *
    wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx4w3z *
    ewv2 * ewz1 * ewz2 * sigx7_2/sqrt(sigma_sq2)) * wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = sigx4w4z * ewv2 * ewz2 * sigx7_2/sqrt(sigma_sq2) * wHvar, FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(sigx4w12z * ewv2 * ewz2 * ewz3 * sigx7_2/sqrt(sigma_sq2)) * wHvar,
    FUN = "*"), Zvar)
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    ...
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 *
    (S^2 * prC * sigx1_3 * sigx1_4 * (sigma_sq4) * ewz3 * sqrt(sigma_sq4)/(sigsq_4^2 *
      wzdeno * (sigma_sq3)^(3/2)))) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 *
    (S * prC * sigx1_3 * ewu4 * ewz3 * sigx6_4 * sqrt(sigma_sq4)/wzsigx4_3)) *
    wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 *
    (S * prC * sigx1_3 * ewv4 * ewz3 * sigx7_4 * sqrt(sigma_sq4)/wzsigx4_3)) *
    wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx4w3z * sigx1_3 * ewz1 * ewz3/((sigma_sq3)^(3/2))) * wHvar, FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * sigx4wz * sigx1_3 * ewz2 * ewz3/((sigma_sq3)^(3/2))) * wHvar,
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * sigx4w13z * sigx1_3 * ewz3/((sigma_sq3)^(3/2)) *
      wHvar, FUN = "*"), Zvar)
  ...
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    ...
    (S * prC * sigx1_4 * ewu3 * (sigma_sq4) * ewz3 * sigx6_3 * sqrt(sigma_sq4)/(sigsq_4^2 *
      wzdsq3))) * wHvar, FUN = "*"), Xvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(4 *
    (prC * ewu3 * ewu4 * ewz3 * sigx6_3 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      wzdsq3))) * wHvar, FUN = "*"), uHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(4 *
    (prC * ewu3 * ewv4 * ewz3 * sigx7_4 * sigx6_3 * sqrt(sigma_sq4)/((sigwz4)^2 *
      wzdsq3))) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx4w3z *
    ewu3 * ewz1 * ewz3 * sigx6_3/sqrt(sigma_sq3)) * wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4wz * ewu3 * ewz2 * ewz3 * sigx6_3/sqrt(sigma_sq3)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = sigx4w13z * ewu3 * ewz3 * sigx6_3/sqrt(sigma_sq3) * wHvar,
    FUN = "*"), Zvar)
  ...
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar +
    ...
    (S * (((S * (sigx10_4 + S * sigx3_4 * dmusig4 * (duv4) * ewu4 * (epsilon4)/sqsq4^2) *
      ...
      depsisq4 * ewu4/sqsq4^2)/(sigwz4) - 2 * (prC * sigx1_4 * sigx7_4/((sigwz4)^2 *
      (sigma_sq4)))) * prC * ewv4) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    prC * sigx4w5z * sigx1_4 * ewz1/((sigma_sq4)^(3/2))) * wHvar, FUN = "*"),
    Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * prC * sigx4w6z * sigx1_4 * ewz2/((sigma_sq4)^(3/2))) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * prC * sigx4w14z * sigx1_4 * ewz3/((sigma_sq4)^(3/2))) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    ...
    (((ewu4 * (S * (0.5 * sigx15_4 - (0.5 * (S^2 * sqewu4 * depsisq4 * (epsilon4)^2/(sigma_sq4)^2) -
      ...
      (epsilon4) - 0.5 * dpsq4)/(sigwz4) - sigx4w9z * ewu4 * sigx6_4/(sigwz4)^2) *
  ...
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    ...
    (((S * (((((0.5 * ((wusq4) * ewv4) - S^2 * sigx3_4 * sqewu4 * ewu4 * (epsilon4)^2)/(sigma_sq4) +
      ...
      0.5 * sigx12_4)/(sigwz4) - sigx4w9z * sigx7_4/(sigwz4)^2) * prC * ewu4 *
      ewv4) * wHvar, FUN = "*"), vHvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(prC *
    sigx4w5z * ewu4 * ewz1 * sigx6_4/sqrt(sigma_sq4)) * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(prC * sigx4w6z * ewu4 * ewz2 * sigx6_4/sqrt(sigma_sq4)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(prC * sigx4w14z * ewu4 * ewz3 * sigx6_4/sqrt(sigma_sq4)) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    ...
    (((S * ((((0.5 * (ewv4/(sigma_sq4)) - 0.5 * (0.5 * (wvsq4) + ewv4/(sigma_sq4))) *
      ...
      0.5 * (ewv4 * sigx13_4))/(sigma_sq4))/(sigwz4) - sigx4w11z * ewv4 * sigx7_4/(sigwz4)^2) *
      prC * ewv4) * wHvar, FUN = "*"), vHvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(prC *
    sigx4w5z * ewv4 * ewz1 * sigx7_4/sqrt(sigma_sq4)) * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(prC * sigx4w6z * ewv4 * ewz2 * sigx7_4/sqrt(sigma_sq4)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(prC * sigx4w14z * ewv4 * ewz3 * sigx7_4/sqrt(sigma_sq4)) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + nZHvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = ((1 - ewz1/wzdeno)/wzdsig - 2 * (dwp1/(wzdsig^2 * sqrt(sigma_sq1)))) *
      (2 * dpsq1 - sigx4) * ewz1 * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + nZHvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar +
    1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -(((2 * dpsq1 - sigx4)/(sigx4wzsq) + 2 * ((2 * dpsq2 -
      sigx4) * depsisq1 * pmusig1/(wzdsig^2 * sqrt(sigma_sq1)))) * ewz1 * ewz2) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + nZHvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar +
    1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -(((2 * dpsq1 - sigx4)/(sigx4wzsq) + 2 * ((2 * dpsq3 -
      sigx4) * depsisq1 * pmusig1/(wzdsig^2 * sqrt(sigma_sq1)))) * ewz1 * ewz3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + 2 * nZHvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    nZHvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz2/wzdeno)/wzdsig - 2 * (dwp2/(wzdsig^2 * sqrt(sigma_sq2)))) *
      (2 * dpsq2 - sigx4) * ewz2 * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar + 4 *
    nuZUvar + 4 * nvZVvar + 2 * nZHvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    2 * nZHvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -(((2 * dpsq2 - sigx4)/(sigx4wzsq) + 2 * ((2 * dpsq3 -
      sigx4) * depsisq2 * pmusig2/(wzdsig^2 * sqrt(sigma_sq2)))) * ewz2 * ewz3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    2 * nZHvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz3/wzdeno)/wzdsig - 2 * (dwp3/(wzdsig^2 * sqrt(sigma_sq3)))) *
      (2 * dpsq3 - sigx4) * ewz3 * wHvar, FUN = "*"), Zvar)
...
#' @noRd
LCM4ChnormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S, wHvar,
  ...
  startLoglik <- sum(cLCMhalfnormlike4C(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cLCMhalfnormlike4C(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradLCMhalfnormlike4C(parm, nXvar = nXvar,
    ...
    control = list(trace = if (printInfo) 1 else 0, maxeval = itermax, stepmax = stepmax,
      xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cLCMhalfnormlike4C,
    grad = cgradLCMhalfnormlike4C, hess = chessLCMhalfnormlike4C, start = startVal,
    ...
    fn = function(parm) -sum(cLCMhalfnormlike4C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike4C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cLCMhalfnormlike4C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradLCMhalfnormlike4C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessLCMhalfnormlike4C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      method = "Sparse", control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
        prec = tol, report.level = if (printInfo) 2 else 0, report.precision = 1L,
        preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal, fn = function(parm) -sum(cLCMhalfnormlike4C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradLCMhalfnormlike4C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessLCMhalfnormlike4C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      objective = function(parm) -sum(cLCMhalfnormlike4C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradLCMhalfnormlike4C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessLCMhalfnormlike4C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = if (printInfo) 1 else 0, eval.max = itermax,
        rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradLCMhalfnormlike4C(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessLCMhalfnormlike4C(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
      mleObj$hessian <- chessLCMhalfnormlike4C(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  }
  mleObj$logL_OBS <- cLCMhalfnormlike4C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradLCMhalfnormlike4C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
cLCM4Chalfnormeff <- function(object, level) {
  ...
  theta1 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    1):(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar + object$nZHvar)]
  theta2 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    object$nZHvar + 1):(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    2 * object$nZHvar)]
  theta3 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    2 * object$nZHvar + 1):(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    3 * object$nZHvar)]
