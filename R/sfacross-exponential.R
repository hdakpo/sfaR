...
#' @noRd
cexponormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  wHvar, S) {
  beta <- parm[1:(nXvar)]
...
#' @param vHvar matrix of Zv variables
#' @noRd
cstexponorm <- function(olsObj, epsiRes, S, nuZUvar, uHvar, nvZVvar, vHvar) {
...
#' @param S integer for cost/prod estimation
#' @noRd
cgradexponormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, wHvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * (sigx2 - 1/exp(Wu/2)), FUN = "*"),
    sweep(uHvar, MARGIN = 1, STATS = ((0.5 * (dmustar/(sigx)) - 1/(2 * exp(Wu))) *
      exp(Wv) - (0.5 + 0.5 * (su_epsi))), FUN = "*"), sweep(vHvar, MARGIN = 1,
      STATS = (exp(Wv)/(2 * exp(Wu)) - (sigx3) * pdmustar), FUN = "*"))
...
#' @param S integer for cost/prod estimation
#' @noRd
chessexponormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, wHvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S^2 * wHvar *
    ((sv_epsi + su_sv)/(exp(Wv/2)^2 * pmustar) - sigx2/(exp(Wv/2) * pmustar)) *
    dmustar, FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = S * wHvar * (0.5 * (((sv_epsi + su_sv)/(sigx) - dmustar * exp(Wu) *
      su_sv/(sigx)^2) * dmustar * exp(Wv/2)) + 0.5/exp(Wu/2)), FUN = "*"),
    uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (S * ((sigx3) * (sv_epsi + su_sv - pdmustar) +
      0.5) * sigx2), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((0.5 * ((0.5 * ((sv_epsi + su_sv)/(exp(Wu) *
      pmustar)) - ((0.5 * (dmustar * exp(Wv)/su_sv) + exp(Wu) * pmustar) *
      su_sv - 0.5 * (exp(Wv) * pmustar/su_sv))/(sigx)^2) * dmustar) + (1/(2 *
      exp(Wu)))) * exp(Wv) + 0.25 * (su_epsi)), FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    (exp(Wv)/(2 * exp(Wu)) - ((sigx3)^2 * (pdmustar + mustar) + 0.25 * (sv_epsi) +
      0.5 * ((1/exp(Wu) - 0.5 * (exp(Wv)/(exp(Wu) * su_sv)^2)) * exp(Wv)/su_sv)) *
      pdmustar), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar * ((((sigx3) *
    (0.5 * (sv_epsi + su_sv) - 0.5 * (pdmustar))/(exp(Wu) * su_sv) - 0.5 * ((exp(Wu) *
    su_sv - 0.5 * (exp(Wv)/su_sv))/(exp(Wu) * su_sv)^2)) * pdmustar + 1/(2 *
...
exponormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S, nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cexponormlike(parm,
    ...
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradexponormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cexponormlike,
    grad = cgradexponormlike, hess = chessexponormlike, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) -sum(cexponormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hs = function(parm) as(-chessexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradexponormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hess = function(parm) -chessexponormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    objective = function(parm) -sum(cexponormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      wHvar = wHvar, S = S)), gradient = function(parm) -colSums(cgradexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hessian = function(parm) -chessexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradexponormlike(mleObj$par, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      wHvar = wHvar, S = S))
  }
  ...
  mleObj$logL_OBS <- cexponormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    wHvar = wHvar, S = S)
  mleObj$gradL_OBS <- cgradexponormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    wHvar = wHvar, S = S)
...
cexponormeff <- function(object, level) {
  ...
  if (object$logDepVar == TRUE) {
    teJLMS <- exp(-u)
    teMO <- exp(-m)
    teBC <- exp(-mustar + 1/2 * exp(Wv)) * pnorm(mustar/sqrt(exp(Wv)) - sqrt(exp(Wv)))/pnorm(mustar/sqrt(exp(Wv)))
    teBCLB <- exp(-uUB)
    teBCUB <- exp(-uLB)
