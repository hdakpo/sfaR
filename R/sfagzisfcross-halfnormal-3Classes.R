...
#' @noRd
cGZISFhalfnormlike3C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  beta3 <- parm[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar)]
  phi3 <- parm[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar)]
  theta1 <- parm[(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar + nZHvar)]
  theta2 <- parm[(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)]
  ...
  ifelse(Probc1 * Pi1 + Probc2 * Pi2 + Probc3 * Pi3 <= 0, return(-.Machine$double.xmax), return(wHvar *
    log(Probc1 * Pi1 + Probc2 * Pi2 + Probc3 * Pi3)))
}
...
#' @noRd
csGZISFfhalfnorm3C <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  StartVal <- c(Esti[1:(nXvar + 1)], if (nuZUvar > 1) rep(0, nuZUvar - 1), Esti[nXvar +
    ...
    1) rep(0, nvZVvar - 1), 0.98 * Esti[1:nXvar], Esti[nXvar + 2], if (nvZVvar >
    1) rep(0, nvZVvar - 1), rep(0, 2 * nZHvar))
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar)), paste0("Zv_",
    ...
    colnames(Zvar)), paste0("Cl2_", colnames(Zvar)))
...
#' @param nZHvar number of separating variables
#' @noRd
cgradGZISFhalfnormlike3C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  sigx13 <- (0.5 * (S^2 * depsi3 * (epsilon3)^2/ewv3_h^2) - 0.5 * depsi3)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = 2 * (S * sigx1_1 * ewz1/sigx4_1),
    ...
    FUN = "*"), sweep(Xvar, MARGIN = 1, STATS = S^2 * prC * depsi3 * (epsilon3)/(sigx3 *
    ewv3_h^3), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx13 * prC/(sigx3 *
    ewv3_h), FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx12_1 * ewz1/(sigx3 *
...
#' @param nZHvar number of separating variables
#' @noRd
chessGZISFhalfnormlike3C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx18_1 <- ((2 - 2 * (ewz1/wzdeno))/(sigx3 * wzdeno) - 2 * (sigx12_1 * ewz1/(sigx3 *
    wzdeno)^2))
  sigx18_2 <- (2 * (sigx12_2/(sigx3 * wzdeno)^2) + 2/(sigx3 * wzdeno^2))
  ...
  sigx27_2 <- ((2 - 2 * (ewz2/wzdeno))/(sigx3 * wzdeno) - 2 * (sigx12_2 * ewz2/(sigx3 *
    wzdeno)^2))
  hessll <- matrix(nrow = 3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar, ncol = 3 *
    nXvar + 2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)
  ...
  hessll[1:nXvar, (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 *
    ...
    (2 * (S^3 * prC * sigx1_1 * depsi3 * ewv3_h^3 * ewz1 * (epsilon3)/((sigx3 *
      ewv3_h^3)^2 * wzdeno * (sigma_sq1) * sqrt(sigma_sq1)))), FUN = "*"),
    Xvar)
  hessll[1:nXvar, (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 *
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar *
    (2 * (S * sigx13 * prC * sigx1_1 * ewv3_h * ewz1/((sigx3 * ewv3_h)^2 * wzdeno *
      (sigma_sq1) * sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[1:nXvar, (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 *
    nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * sigx18_1 * sigx1_1 * ewz1/sigx21_1, FUN = "*"), Zvar)
  hessll[1:nXvar, (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -wHvar * (S * sigx18_2 * sigx1_1 * ewz1 * ewz2/sigx21_1), FUN = "*"),
    Zvar)
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    ...
    STATS = -wHvar * (2 * (S^2 * prC * depsi3 * ewu1 * ewv3_h^3 * ewz1 * sigx8_1 *
      (epsilon3)/sigx26_1)), FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -wHvar * (2 * (sigx13 * prC * ewu1 * ewv3_h * ewz1 * sigx8_1/sigx19_1)),
    FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar +
    1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * sigx18_1 * ewu1 * ewz1 * sigx8_1/sqrt(sigma_sq1),
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar +
    nZHvar + 1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx18_2 * ewu1 * ewz1 * ewz2 * sigx8_1/sqrt(sigma_sq1)),
    FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (2 * nXvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = -wHvar * (2 * (S^2 * prC * depsi3 * ewv1 * ewv3_h^3 *
      ewz1 * sigx11_1 * (epsilon3)/sigx26_1)), FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = -wHvar * (2 * (sigx13 * prC * ewv1 * ewv3_h * ewz1 *
      sigx11_1/sigx19_1)), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx18_1 * ewv1 * ewz1 * sigx11_1/sqrt(sigma_sq1),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar + nZHvar + 1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx18_2 * ewv1 * ewz1 * ewz2 * sigx11_1/sqrt(sigma_sq1)),
    FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (2 *
    ...
    MARGIN = 1, STATS = -wHvar * (2 * (S^3 * prC * sigx1_2 * depsi3 * ewv3_h^3 *
      ewz2 * (epsilon3)/((sigx3 * ewv3_h^3)^2 * wzdeno * sigx21_2))), FUN = "*"),
    Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
    nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S * sigx13 * prC * sigx1_2 * ewv3_h *
      ewz2/((sigx3 * ewv3_h)^2 * wzdeno * sigx21_2))), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
    nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar * (S * sigx27_1 *
    sigx1_2 * ewz1 * ewz2/(sigx21_2)), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (3 *
    nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (sigx27_2 * sigx1_2 * ewz2/(sigx21_2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * 2 * (((ewu2 * (S * (0.5 * sigx24_2 - (0.5 * (S^2 *
      ...
      0.5 * dpesq2)/sigx9_2 - (0.5 * sigx22_2 + 2 * (ewz2 * sigx8_2)) * ewu2 *
      sigx8_2/sigx9_2^2) * ewu2 * ewz2), FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * 2 * (((S * (((((0.5 * (prU2 * ewv2) - S^2 * (0.5 *
      ...
      (0.5 * sigx22_2 + 2 * (ewz2 * sigx8_2)) * sigx11_2/sigx9_2^2) * ewu2 *
      ewv2 * ewz2), FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar * (2 *
    (S^2 * prC * depsi3 * ewu2 * ewv3_h^3 * ewz2 * sigx8_2 * (epsilon3)/sigx25_2)),
    FUN = "*"), Xvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 3 *
      nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar * (2 *
    (sigx13 * prC * ewu2 * ewv3_h * ewz2 * sigx8_2/sigx19_2)), FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 3 *
      nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar *
    (sigx27_1 * ewu2 * ewz1 * ewz2 * sigx8_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar + 2 * nuZUvar +
      3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    (sigx27_2 * ewu2 * ewz2 * sigx8_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    ...
    2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar * (2 *
    (S^2 * prC * depsi3 * ewv2 * ewv3_h^3 * ewz2 * sigx11_2 * (epsilon3)/sigx25_2)),
    FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar * (2 *
    (sigx13 * prC * ewv2 * ewv3_h * ewz2 * sigx11_2/sigx19_2)), FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (sigx27_1 * ewv2 * ewz1 * ewz2 * sigx11_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * sigx27_2 * ewv2 * ewz2 * sigx11_2/sqrt(sigma_sq2), FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 *
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S^2 * ((S^2 * (epsilon3)^2/ewv3_h^2 - 1)/(sigx3 * ewv3_h^3) - S^2 * prC *
    depsi3 * (epsilon3)^2/(sigx3 * ewv3_h^3)^2) * prC * depsi3, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 *
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S^2 * ((0.5 * (S^2 * (epsilon3)^2/ewv3_h^2 - 2) - 0.5)/(sigx3 * ewv3_h) -
    sigx13 * prC/(sigx3 * ewv3_h)^2) * prC * depsi3 * (epsilon3)/ewv3_h^2, FUN = "*"),
    vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 *
    nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar *
    (S^2 * (wzdeno * sigx12_1/(sigx3 * wzdeno)^2 + 1/(sigx3 * wzdeno)) * prC *
      depsi3 * ewz1 * (epsilon3)/ewv3_h^3), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -wHvar * (S^2 * (wzdeno * sigx12_2/(sigx3 * wzdeno)^2 + 1/(sigx3 *
      wzdeno)) * prC * depsi3 * ewz2 * (epsilon3)/ewv3_h^3), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar), (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 *
    nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    prC * (S^2 * (0.5 * (0.5 * (S^2 * (epsilon3)^2/ewv3_h^2) - 1) - 0.25) * depsi3 *
    (epsilon3)^2/(sigx3 * ewv3_h^3) - (sigx13 * prC + 0.5 * (sigx3 * ewv3_h)) *
    sigx13/(sigx3 * ewv3_h)^2), FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 *
    nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    ((wzdeno * sigx12_1/(sigx3 * wzdeno)^2 + 1/(sigx3 * wzdeno)) * sigx13 * prC *
      ewz1/ewv3_h), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar +
    2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -wHvar * ((wzdeno * sigx12_2/(sigx3 * wzdeno)^2 + 1/(sigx3 * wzdeno)) *
      sigx13 * prC * ewz2/ewv3_h), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar + nZHvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 3 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = wHvar * ((1 - ewz1/wzdeno)/(sigx3 * wzdeno) - 2 * (depsi1 * ewz1 *
      pmusig1/sigx25_1)) * sigx12_1 * ewz1, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar +
    3 * nvZVvar + nZHvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar +
    1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * ((sigx12_1/(sigx3 * wzdeno^2) + 2 * (sigx12_2 *
      depsi1 * pmusig1/sigx25_1)) * ewz1 * ewz2), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + nZHvar + 1):(3 * nXvar + 2 *
    nuZUvar + 3 * nvZVvar + 2 * nZHvar), (3 * nXvar + 2 * nuZUvar + 3 * nvZVvar +
    nZHvar + 1):(3 * nXvar + 2 * nuZUvar + 3 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = wHvar * ((1 - ewz2/wzdeno)/(sigx3 * wzdeno) - 2 * (depsi2 *
      ewz2 * pmusig2/((sigx3 * wzdeno)^2 * sqrt(sigma_sq2)))) * sigx12_2 *
      ewz2, FUN = "*"), Zvar)
...
#' @noRd
GZISF3ChnormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(cGZISFhalfnormlike3C(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cGZISFhalfnormlike3C(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradGZISFhalfnormlike3C(parm, nXvar = nXvar,
    ...
    control = list(trace = if (printInfo) 1 else 0, maxeval = itermax, stepmax = stepmax,
      xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cGZISFhalfnormlike3C,
    grad = cgradGZISFhalfnormlike3C, hess = chessGZISFhalfnormlike3C, start = startVal,
    ...
    fn = function(parm) -sum(cGZISFhalfnormlike3C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradGZISFhalfnormlike3C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cGZISFhalfnormlike3C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradGZISFhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessGZISFhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      ...
      fn = function(parm) -sum(cGZISFhalfnormlike3C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        ...
        nZHvar = nZHvar)), hess = function(parm) -chessGZISFhalfnormlike3C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
        vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar,
        nZHvar = nZHvar), print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(cGZISFhalfnormlike3C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradGZISFhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessGZISFhalfnormlike3C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = if (printInfo) 1 else 0, eval.max = itermax,
        rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradGZISFhalfnormlike3C(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessGZISFhalfnormlike3C(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
      mleObj$hessian <- chessGZISFhalfnormlike3C(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  }
  mleObj$logL_OBS <- cGZISFhalfnormlike3C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradGZISFhalfnormlike3C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
#' post. prob. and efficiencies for gzisfcross 3 classes halfnormal-normal distribution
...
cGZISF3Chalfnormeff <- function(object, level) {
  ...
  phi3 <- object$mlParam[(3 * object$nXvar + 2 * object$nuZUvar + 2 * object$nvZVvar +
    1):(3 * object$nXvar + 2 * object$nuZUvar + 3 * object$nvZVvar)]
  theta1 <- object$mlParam[(3 * object$nXvar + 2 * object$nuZUvar + 3 * object$nvZVvar +
    1):(3 * object$nXvar + 2 * object$nuZUvar + 3 * object$nvZVvar + object$nZHvar)]
  theta2 <- object$mlParam[(3 * object$nXvar + 2 * object$nuZUvar + 3 * object$nvZVvar +
    object$nZHvar + 1):(3 * object$nXvar + 2 * object$nuZUvar + 3 * object$nvZVvar +
    2 * object$nZHvar)]
...
#' marginal effects for for gzisfcross 3 classes halfnormal-normal distribution
#' @param object object of class gzisfcross
#' @noRd
cmargGZISF3Chalfnorm_Eu <- function(object) {
  ...
  margEff_c3 <- matrix(0, nrow = object$Nobs, ncol = object$nuZUvar - 1)
