########################################################
#                                                      #
# Zero Inefficiency (ZISF) uniform                     #
#                                                      #
########################################################
...
cstzisfuninorm <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, 
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", 
    ...
    colnames(Zvar)))
names(initUni$estimate) <- c(names(Esti)[1:nXvar], paste0(
    "Zu_",
    colnames(uHvar)[1]
...
chesszisfuninormlike <- function(parm, nXvar, nuZUvar, nvZVvar, 
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, 
    STATS = wHvar*((prC * depsi * sigx11 + ewz * (S * depsi * (epsilon) - 
      eeusq * dmusig)/ezusq)/(sigx2 * ewvsr^3) - sigx1^2/(sigx2 * 
      ewvsr)^2), FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, 
    MARGIN = 1, STATS = wHvar*S * (eeusq * dmusig/(2 * (wzdeno * 
      ewvsr^2)) - (sigx1 * sigx3/sigx2 + sqrt(12)/2 * (wzdeno * 
      ddue * ewusr/wzsq))) * ewz/(sigx2 * ewvsr), FUN = "*"), 
    uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + 
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar*S * 
    (((0.5 * (depsi * sigx11) - 0.5 * ((eeusq^2/ewvsr^2 - 
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar + 
    ...
    MARGIN = 1, STATS = wHvar*S * (sigx9 * ddue - (sigx10 * sigx1/sigx2 + 
      S * prC * depsi * (epsilon)/(wzdeno * ewvsr^2))) * 
      ewz/(sigx2 * ewvsr), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + 
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar*(((sqrt(12)/2 * 
    (((sqrt(12)/2 * (dmusig/ewvsr) - 12 * (wzdeno^2 * ewusr * 
      (pmusig - pepsi)/wzsq)) * ewusr + 0.5 * (pmusig - 
      pepsi)) * wzdeno/wzsq) + sqrt(12)/2 * (eeusq * dmusig)/(2 * 
    (wzdeno * ewvsr^3))) * ewusr + sigx3^2 * ewz/sigx2) * 
    ewz/sigx2), FUN = "*"), uHvar)
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), 
    ...
    MARGIN = 1, STATS = wHvar*(((0.25 * (S^3 * depsi * (epsilon)^3) - 
      0.25 * (eeusq^3 * dmusig)) * ewz/ezusq + S^2 * (0.5 * 
      (0.5 * (S^2 * (epsilon)^2/ewvsr^2) - 1) - 0.25) * 
      prC * depsi * (epsilon)^2)/(sigx2 * ewvsr^3) - sigx7 * 
      (sigx5 * ewz/ezusq + sigx6 * prC + 0.5 * (sigx2 * 
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), 
    ...
    sigx9 - sigx7 * sigx10/sigx2)/ewvsr - (0.5 * (S^2 * depsi * 
    (epsilon)^2/(wzdeno * ewvsr^3)) - 0.5 * (wzdeno * depsi * 
    ewvsr/(wzdeno * ewvsr)^2)) * prC) * ewz/sigx2, FUN = "*"), 
    Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 
    ...
    MARGIN = 1, STATS = wHvar*((prC * (1/(wzdeno^2 * ewvsr) + ewvsr/(wzdeno * 
      ewvsr)^2) * depsi - (sigx10^2/sigx2 + (sqrt(12) * 
      (1 - 24 * (wzdeno * ewusr^2 * ewz/wzsq)) + sqrt(12)) * 
      ewusr * (pmusig - pepsi)/wzsq)) * ewz + sigx9 * (pmusig - 
      pepsi) - sigx4) * ewz/sigx2, FUN = "*"), Zvar)
...
#' @noRd
zisfuninormAlgOpt <- function(start, olsParam, dataTable, S, wHvar,
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal, 
    fn = function(parm) -sum(czisfuninormlike(parm, nXvar = nXvar, 
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, 
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, 
      nZHvar = nZHvar)), gr = function(parm) -colSums(cgradzisfuninormlike(parm, 
    ...
    maxLikAlgo = maxRoutine(fn = czisfuninormlike, grad = cgradzisfuninormlike, 
      ...
      fn = function(parm) -sum(czisfuninormlike(parm, nXvar = nXvar, 
        ...
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), hs = function(parm) as(-chesszisfuninormlike(parm, 
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
      ...
      fn = function(parm) -sum(czisfuninormlike(parm, nXvar = nXvar, 
        ...
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), hess = function(parm) -chesszisfuninormlike(parm, 
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar), print.info = printInfo, 
      maxiter = itermax, epsa = gradtol, epsb = gradtol), 
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(czisfuninormlike(parm, 
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gradient = function(parm) -colSums(cgradzisfuninormlike(parm, 
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), hessian = function(parm) -chesszisfuninormlike(parm, 
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
...
czisfuninormeff <- function(parm) {
  ...
  Wz <- as.numeric(crossprod(matrix(theta), t(Zvar)))
  epsilon <- Yvar - as.numeric(crossprod(matrix(beta), t(Xvar)))
  Pi1 <- 1/exp(Wu) * (pnorm((exp(Wu) + S * epsilon)/exp(Wv/2)) - 
  ...
  teBC_Wc <- Pcond_c1 * Pcond_c1 * teBC_c1 + Pcond_c2
  res <- bind_cols(Group_c = Group_c, u1_c1 = u1_c1, u1_c2 = u1_c2, 
    u1_c = u1_c, u1_Wc = u1_Wc, teJLMS1_c1 = teJLMS1_c1, 
    teJLMS1_c2 = teJLMS1_c2, teJLMS1_c = teJLMS1_c, teJLMS1_Wc = teJLMS1_Wc, 
    u2_c1 = u2_c1, u2_c2 = u2_c2, u2_c = u2_c, u2_Wc = u2_Wc, 
    teJLMS2_c1 = teJLMS2_c1, teJLMS2_c2 = teJLMS2_c2, teJLMS2_c = teJLMS2_c, 
    teJLMS2_Wc = teJLMS2_Wc, teBC_c1 = teBC_c1, teBC_c2 = teBC_c2, 
    teBC_c = teBC_c, teBC_Wc = teBC_Wc, Pcond_c1 = Pcond_c1, 
    Pcond_c2 = Pcond_c2)
  return(res)
}
...
czisfmarguninorm_VuI <- function(object) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  theta <- parm[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 
