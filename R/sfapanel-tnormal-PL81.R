...
#' @noRd
ptruncnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, muHvar, nmuZUvar,
  ...
  mustar <- (mu * exp(Wv) - exp(Wu) * S * epsilon_i)/(exp(Wv) + TT * exp(Wu))
  sigmastar <- sqrt(exp(Wu) * exp(Wv)/(exp(Wv) + TT * exp(Wu)))
  ll <- -TT/2 * log(2 * pi) - (TT - 1)/2 * Wv - 1/2 * log(exp(Wv) + TT * exp(Wu)) +
...
#' @noRd
psttruncnorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, muHvar, nmuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, whichStart, initIter, initAlg, printInfo,
  tol) {
  if (whichStart == 1L) {
...
#' @param TT vector of time of presence
#' @noRd
pgradtruncnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, muHvar, nmuZUvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  delta <- parm[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)]
  ...
  gradll <- cbind(sweep(X_iM, MARGIN = 1, STATS = -S * sigx2/sigmasq, FUN = "*") -
    sweep(Xepsi_i, MARGIN = 1, STATS = 2/(2 * ewv), FUN = "*"), sweep(muHvar,
    MARGIN = 1, STATS = (sigx6/sigmasq - (dmu/(ewup) + muwu)), FUN = "*"), sweep(uHvar,
    MARGIN = 1, STATS = -(((sigx3 * musq + 0.5 * ttsq) * ewu - (0.5 * muwusq +
      0.5 * (mu * dmu/(ewup))))), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = ((musq *
    sigx5 + 2 * epsivsq - 0.5/sigmasq) * ewv - 0.5 * (TT - 1)), FUN = "*"))
...
#' @param TT vector of time of presence
#' @noRd
phesstruncnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, muHvar, nmuZUvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  delta <- parm[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(X_iM, MARGIN = 1, STATS = -S^2 *
    wHvar * ((sigx1/(ewv * pmustar) + dmusig * ewu/(pmustar)^2) * dmusig/sigmasq -
    1/ewv) * ewu/sigmasq, FUN = "*"), X_iM) - sapply(1:nXvar, function(x) {
    crossprod(Xsq[[x]], as.matrix(wHvar * 2/(2 * ewv)))
  })
  hessll[1:nXvar, (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(X_iM, MARGIN = 1,
    STATS = wHvar * S * (sigx8 * dmusig/sigmasq - 1)/sigmasq, FUN = "*"), muHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)] <- crossprod(sweep(X_iM,
    MARGIN = 1, STATS = -wHvar * (S * (sigx7 * sigx3/sigmasq + musq * (1/(ssqx2) -
      sigx0 * ewu/ssq)) * ewu), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xepsi_i, MARGIN = 1, STATS = wHvar * 2 * (ewv *
    2/(2 * ewv)^2), FUN = "*"), vHvar) + crossprod(sweep(X_iM, MARGIN = 1, STATS = wHvar *
    S * ewv * (sigx7 * sigx5/sigmasq + musq * sigx4 * ewu/ssq), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * ((1/ewu - (sigx1/(ewu * pmustar) + dmusig * ewv/(pmustar)^2) *
      dmusig/sigmasq) * ewv/sigmasq + dmu * (dmu/(ewup)^2 + mu/(ewuh^3 * pmu)) -
      1/ewu), FUN = "*"), muHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = -wHvar * (((sigx3 *
    (ewv/sigmastar - sigx6 * dpmu)/sigmasq + musq * sigx0 * ewv/ssq) * ewu -
    (0.5 * (((1 - mu^2/ewuh^2)/(ewup) - mu * dmu/(ewup)^2) * dmu) + muwu))),
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar + nuZUvar + 1):(nXvar +
    ...
    (musq * (1/(ssqx2) - sigx4 * ewv/ssq) + (ewv/sigmastar - sigx6 * dpmu) *
      sigx5/sigmasq) * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar), (nXvar + nmuZUvar +
    ...
    (((((musq * dpmu - 1) * sigx3^2 - 0.5 * (TT^2/sigmasq^2)) * ewu + ((sigx0 *
      (mu * ewv - (2 * sigx9 + 3 * (S * epsilon_i)) * ewu) + (0.5 * (TT * ewu/sigmasq) -
      0.5 * (0.5 * wu2sq + TT * ewu/sigmasq)) * wu2sq * ewv * sigx1/sigmastar)/ssq +
      epsi_ss) * musq + 0.5 * ttsq) * ewu + 0.5 * muwusq - 0.5 * (mu * (0.5 *
      (mu^2/(ewuh^3 * pmu)) - (0.5 * (ewup) - 0.5 * (mu * dmu))/(ewup)^2) *
      dmu))), FUN = "*"), uHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar), (nXvar + nmuZUvar +
    nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((musq * dpmu - 1) * sigx3 * sigx5 + 0.5 * (TT/sigmasq^2) -
      ((0.5 * (wu2sq * ewv/sigmasq) + 0.5 * (1 + ewv * (TT * ewu/sigmasq -
        1)/sigmasq - 0.5 * (wvsq * wu2sq))) * sigx1/sigmastar + mu * sigx0 -
        sigx4 * (2 * sigx9 + S * epsilon_i)) * musq/ssq) * ewu * ewv, FUN = "*"),
    vHvar)
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * (((1 - musq * dpmu) * sigx5^2 + 0.5/sigmasq^2 -
      16 * (ewv * epsilon_isq/(2 * ewv)^4)) * ewv + musq * (mu/(ssqx2) - (((3 *
      (mu) - 2 * sigx10) * ewv - S * ewu * epsilon_i) * sigx4 + (0.5 * (ewv/sigmasq) -
      0.5 * (0.5 * wvsq + ewv/sigmasq)) * wvsq * ewu * sigx1/sigmastar)/ssq) +
      2 * epsivsq - 0.5/sigmasq) * ewv, FUN = "*"), vHvar)
...
#' @noRd
truncnormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable,
  S, nXvar, muHvar_c, muHvar_p, nmuZUvar, uHvar_c, uHvar_p, nuZUvar, vHvar_c, vHvar_p,
  nvZVvar, Yvar, Xvar, wHvar_c = wHvar_c, wHvar_p = wHvar_p, pindex, TT, method,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psttruncnorm_pl81(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      ...
      S = S, wHvar = wHvar_c, tol = tol, whichStart = whichStart, initIter = initIter,
      initAlg = initAlg, printInfo = printInfo)
    Inittrunc <- start_st$inittrunc
    startVal <- start_st$StartVal
  }
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(ptruncnormlike_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(ptruncnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, vHvar = vHvar_p,
      Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradtruncnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ptruncnormlike_pl81,
    grad = pgradtruncnormlike_pl81, hess = phesstruncnormlike_pl81, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phesstruncnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      ...
      -phesstruncnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradtruncnormlike_pl81(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesstruncnormlike_pl81(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phesstruncnormlike_pl81(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- ptruncnormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradtruncnormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar_p, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p)
  if (is.null(start)) {
...
ptruncnormeff_pl81 <- function(object, level) {
  ...
  epsilon_i <- as.numeric(tapply(epsilon_it, pindex[, 1], sum))
  mustar <- (mu * exp(Wv) - exp(Wu) * object$S * epsilon_i)/(exp(Wv) + TT * exp(Wu))
  sigmastar <- sqrt(exp(Wu) * exp(Wv)/(exp(Wv) + TT * exp(Wu)))
  ...
  if (object$logDepVar == TRUE) {
    ...
    res <- data.frame(levels(pindex[, 1]), u = u, uLB = uLB, uUB = uUB, teJLMS = teJLMS,
      m = m, teMO = teMO, teBC = teBC, teBCLB = teBCLB, teBCUB = teBCUB, teBC_reciprocal = teBC_reciprocal)
  } else {
    res <- data.frame(levels(pindex[, 1]), u = u, uLB = uLB, uUB = uUB, m = m)
  }
