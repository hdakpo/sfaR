...
#' @noRd
ctslnormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  wHvar, S) {
  beta <- parm[1:(nXvar)]
  ...
  if (lambda < 0) {
    ...
    ll <- log(1 + lambda) - log(2 * lambda + 1) - 1/2 * Wu + log(2 * exp(A) *
...
#' @param vHvar matrix of Zv variables
#' @noRd
csttslnorm <- function(olsObj, epsiRes, S, nuZUvar, uHvar, nvZVvar, vHvar) {
...
#' @param wHvar vector of weights (weighted likelihood)
#' @noRd
cgradtslnormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx1 <- 0.5 * (S * (epsilon)/exp(Wu/2)) + 2 * (exp(Wu) * exp(Wv)/(2 * exp(Wu))^2)
  papb <- 2 * (eA * pa) - eB * pb
  epsivl <- 0.5 * ((1 + lambda) * wvwu) - epsiv
  sigx2 <- 2 * (eA * (exp(Wv) * pa/(2 * exp(Wu)) - (epsiuv) * da)) - ((1 + lambda)^2 *
    exp(Wv) * pb/(2 * exp(Wu)) - (epsivl) * db) * eB
  epsiuvl <- (1 + lambda) * exp(Wu) * exp(Wv)/(2 * exp(Wu))^2
  epsiuvlx2 <- (0.5 * (S * (epsilon)/exp(Wu/2)) + 2 * (epsiuvl))
  sigx3 <- 2 * ((0.5 * (da * wvwu) - (sigx1) * pa) * eA) - (0.5 * (db * wvwu) -
    epsiuvlx2 * pb) * (1 + lambda) * eB
  sigx4 <- db/exp(Wv/2) - (1 + lambda) * pb/exp(Wu/2)
  dpa <- da/exp(Wv/2) - pa/exp(Wu/2)
  sigx5 <- 2 * ((dpa) * eA) - (sigx4) * eB
  pdbuv <- (eC) * pb - db * wvwu
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * (sigx5)/(papb), FUN = "*"),
    sweep(uHvar, MARGIN = 1, STATS = ((sigx3)/(papb) - 0.5), FUN = "*"), sweep(vHvar,
      MARGIN = 1, STATS = (sigx2)/(papb), FUN = "*"), (1/(1 + lambda) - ((pdbuv) *
      eB/(papb) + 2/(1 + 2 * lambda))))
...
#' @param wHvar vector of weights (weighted likelihood)
#' @noRd
chesstslnormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S^2 * wHvar *
    (2 * ((((wvwu + S * (epsilon)/exp(Wv/2))/exp(Wv/2) - 1/exp(Wu/2)) * da/exp(Wv/2) -
      (dpa)/exp(Wu/2)) * eA) - ((((d)/exp(Wv/2) - (1 + lambda)/exp(Wu/2)) *
      db/exp(Wv/2) - (1 + lambda) * (sigx4)/exp(Wu/2)) * eB + (sigx5)^2/(papb)))/(papb),
    FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = S * wHvar * (2 * ((((0.5 + sigx1) * pa - 0.5 * (da * wvwu))/exp(Wu/2) +
      (0.5 * ((wvwu + S * (epsilon)/exp(Wv/2))/exp(Wu/2)) - (sigx1)/exp(Wv/2)) *
        da) * eA) - (((0.5 * ((d)/exp(Wu/2)) - epsiuvlx2/exp(Wv/2)) * db +
      (0.5 * pb - (0.5 * (db * wvwu) - epsiuvlx2 * pb) * (1 + lambda))/exp(Wu/2)) *
      (1 + lambda) * eB + (sigx3) * (sigx5)/(papb)))/(papb), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * wHvar * (2 * ((da * (exp(Wv)/(2 * exp(Wu)) - ((epsiuv) *
      (wvwu + S * (epsilon)/exp(Wv/2)) + 0.5))/exp(Wv/2) - (exp(Wv) * pa/(2 *
      exp(Wu)) - (epsiuv) * da)/exp(Wu/2)) * eA) - ((((1 + lambda)^2 * exp(Wv)/(2 *
      exp(Wu)) - ((d) * (epsivl) + 0.5)) * db/exp(Wv/2) - ((1 + lambda)^2 *
      exp(Wv) * pb/(2 * exp(Wu)) - (epsivl) * db) * (1 + lambda)/exp(Wu/2)) *
      eB + (sigx5) * (sigx2)/(papb)))/(papb), FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1)] <- crossprod(Xvar, -wHvar *
    (S * (((eC)/exp(Wv/2) - (d)/exp(Wu/2)) * db - (((pdbuv) * (1 + lambda) +
      pb)/exp(Wu/2) + (pdbuv) * (sigx5)/(papb))) * eB/(papb)))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (2 * (((0.5 * (0.5 * (exp(Wv/2) * (wvwu + S *
      (epsilon)/exp(Wv/2))/exp(Wu/2)) - 0.5) - 0.5 * (sigx1)) * da * wvwu -
      ((0.5 * (da * wvwu) - (sigx1) * pa) * (sigx1) + (2 * ((1 - 8 * (exp(Wu)^2/(2 *
        exp(Wu))^2)) * exp(Wu) * exp(Wv)/(2 * exp(Wu))^2) - 0.25 * (S * (epsilon)/exp(Wu/2))) *
        pa)) * eA) - (((0.5 * (0.5 * ((d) * (1 + lambda) * wvwu) - 0.5) -
      0.5 * (epsiuvlx2 * (1 + lambda))) * db * wvwu - ((0.5 * (db * wvwu) -
      epsiuvlx2 * pb) * epsiuvlx2 * (1 + lambda) + (2 * ((1 - 8 * (exp(Wu)^2/(2 *
      exp(Wu))^2)) * epsiuvl) - 0.25 * (S * (epsilon)/exp(Wu/2))) * pb)) *
      (1 + lambda) * eB + (sigx3)^2/(papb)))/(papb), FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    (2 * ((((exp(Wv) * pa/(2 * exp(Wu)) - (epsiuv) * da)/2 + (pa - (epsiuv) *
      da)/2) * exp(Wv)/exp(Wu) - (0.25 * (wvwu) + 0.25 * (S * (epsilon)/exp(Wv/2)) -
      (epsiuv)^2 * (wvwu + S * (epsilon)/exp(Wv/2))) * da) * eA) - (((((1 +
      lambda)^2 * exp(Wv) * pb/(2 * exp(Wu)) - (epsivl) * db)/2 + (pb - (epsivl) *
      db)/2) * (1 + lambda)^2 * exp(Wv)/exp(Wu) - (0.25 * ((1 + lambda) * wvwu) +
      0.25 * (S * (epsilon)/exp(Wv/2)) - (d) * (epsivl)^2) * db) * eB + (sigx2)^2/(papb)))/(papb),
    FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (2 * ((((0.5 *
    (da * wvwu) - (sigx1) * pa)/(2 * exp(Wu)) - 2 * (exp(Wu) * pa/(2 * exp(Wu))^2)) *
    exp(Wv) + ((epsiuv) * (sigx1) + 0.5 * ((0.5 - (epsiuv) * (wvwu + S * (epsilon)/exp(Wv/2))) *
    wvwu)) * da) * eA) - ((((epsivl) * epsiuvlx2 + 0.5 * ((0.5 - (d) * (epsivl)) *
    wvwu)) * db + ((0.5 * (db * wvwu) - epsiuvlx2 * pb) * (1 + lambda)/(2 * exp(Wu)) -
    2 * (exp(Wu) * pb/(2 * exp(Wu))^2)) * (1 + lambda) * exp(Wv)) * (1 + lambda) *
    eB + (sigx3) * (sigx2)/(papb)))/(papb), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1)] <- crossprod(uHvar,
    -wHvar * ((((0.5 * (eC) - 0.5 * ((d) * wvwu)) * (1 + lambda) + 0.5) * db *
      wvwu - ((pdbuv) * (epsiuvlx2 * (1 + lambda) + (sigx3)/(papb)) + ((1 +
      lambda) * exp(Wv)/exp(Wu) + 0.5 * (S * (epsilon)/exp(Wu/2))) * pb)) *
      eB/(papb)))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 1)] <- crossprod(vHvar, -wHvar * ((((pdbuv) * (1 + lambda)/2 +
    pb) * (1 + lambda) * exp(Wv)/exp(Wu) - (((eC) * (epsivl) + (0.5 - (d) * (epsivl)) *
    wvwu) * db + (pdbuv) * (sigx2)/(papb))) * eB/(papb)))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 1)] <- sum(wHvar *
    (4/(1 + 2 * lambda)^2 - ((((pdbuv) * eB/(papb) + eC) * (pdbuv) + (((d) *
      exp(Wv/2) - S * (epsilon))/exp(Wu/2) - (1 + lambda) * exp(Wv)/exp(Wu)) *
      db * wvwu + exp(Wv) * pb/exp(Wu)) * eB/(papb) + 1/(1 + lambda)^2)))
...
tslnormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S, nXvar,
  uHvar, nuZUvar, vHvar, nvZVvar, Yvar, Xvar, wHvar, method, printInfo, itermax,
  ...
  startLoglik <- sum(ctslnormlike(startVal, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(ctslnormlike(parm,
    ...
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)), gr = function(parm) -colSums(cgradtslnormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ctslnormlike,
    grad = cgradtslnormlike, hess = chesstslnormlike, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) -sum(ctslnormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      S = S, wHvar = wHvar)), gr = function(parm) -colSums(cgradtslnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)), hs = function(parm) as(-chesstslnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)), gr = function(parm) -colSums(cgradtslnormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)), hess = function(parm) -chesstslnormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    objective = function(parm) -sum(ctslnormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar)), gradient = function(parm) -colSums(cgradtslnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)), hessian = function(parm) -chesstslnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradtslnormlike(mleObj$par, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar))
  }
  ...
  mleObj$logL_OBS <- ctslnormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar)
  mleObj$gradL_OBS <- cgradtslnormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar)
...
ctslnormeff <- function(object, level) {
  ...
  u <- exp(Wv/2) * (2 * exp(A) * (dnorm(a) + a * pnorm(a)) - exp(B) * (dnorm(b) +
    b * pnorm(b)))/(2 * exp(A) * pnorm(a) - exp(B) * pnorm(b))
  if (object$logDepVar == TRUE) {
    teJLMS <- exp(-u)
    teBC <- (2 * exp(A) * exp(-a * exp(Wv/2) + exp(Wv)/2) * pnorm(a - exp(Wv/2)) -
      exp(B) * exp(-b * exp(Wv/2) + exp(Wv)/2) * pnorm(b - exp(Wv/2)))/(2 *
      exp(A) * pnorm(a) - exp(B) * pnorm(b))
    teBC_reciprocal <- (2 * exp(A) * exp(a * exp(Wv/2) + exp(Wv)/2) * pnorm(a +
...
cmargtslnorm_Eu <- function(object) {
  ...
  lambda <- object$mlParam[object$nXvar + object$nuZUvar + object$nvZVvar + 1]
  Wu <- as.numeric(crossprod(matrix(delta), t(uHvar)))
  margEff <- kronecker(matrix(delta[2:object$nuZUvar] * (1 + 4 * lambda + 2 * lambda^2)/((1 +
    lambda) * (1 + 2 * lambda)), nrow = 1), matrix(exp(Wu/2), ncol = 1))
...
cmargtslnorm_Vu <- function(object) {
  ...
  lambda <- object$mlParam[object$nXvar + object$nuZUvar + object$nvZVvar + 1]
  Wu <- as.numeric(crossprod(matrix(delta), t(uHvar)))
  margEff <- kronecker(matrix(delta[2:object$nuZUvar] * (1 + 8 * lambda + 16 *
