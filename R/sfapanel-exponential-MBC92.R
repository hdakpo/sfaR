...
#' @noRd
pexponormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
pstexponorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradexponormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * sigx1/gisq, FUN = "*") - sweep(Xepsi_i,
    ...
    STATS = (sigx5 * sigx4 + 2 * (ewv * epsilon_isq/(2 * ewv)^2) - 0.5 * (TT -
      1)), FUN = "*"), sigx6 * sigx8 - 0.5 * (giZi/gisq), sigx6 * sigx9 - 0.5 *
    (giZisq/gisq))
...
#' @param TT vector of time of presence
#' @noRd
phessexponormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx18 <- (1/sigmastar - dmusig * (dmusig/(pmusig * sigmastar) - euvg/ewv)/pmusig)
  sigx19 <- (dmusig/(pmusig * sigmastar)^2 - euvg/(ewv * pmusig * sigmastar))
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    (1/ewv - dmusig * sigx19/gisq)/gisq, FUN = "*"), Xgi) - sapply(1:nXvar, function(x) crossprod(Xsq[[x]],
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xepsi_i,
    ...
    MARGIN = 1, STATS = wHvar * S * ((1 - dmusig * sigx19 * ewv/gisq) * sigx4 -
      0.5 * (sigx5/gisig^2)), FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1)] <- S * colSums(sweep(XiZi, MARGIN = 1,
    STATS = wHvar/gisig * sigx6, FUN = "*") - sweep(Xgi, MARGIN = 1, STATS = wHvar *
    sigx7 * giZi/gisig^2 * sigx6, FUN = "*") + sweep(Xgi, MARGIN = 1, STATS = wHvar *
    sigx18 * sigx8/gisq, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 2)] <- S * colSums(sweep(XiZisq,
    MARGIN = 1, STATS = wHvar/gisig * sigx6, FUN = "*") - sweep(Xgi, MARGIN = 1,
    STATS = wHvar * sigx7 * giZisq/gisig^2 * sigx6, FUN = "*") + sweep(Xgi, MARGIN = 1,
    STATS = wHvar * sigx18 * sigx9/gisq, FUN = "*"))
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (0.5 * (sigx6 * ewv * sigx7 * giZi/gisig^2) -
      sigx10 * sigx8/gisq)/ewu_h, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (0.5 * (sigx6 * ewv * sigx7 * giZisq/gisig^2) -
      sigx10 * sigx9/gisq)/ewu_h, FUN = "*"))
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    sigx12 * sigx8 - (sigx13 * giZi + 0.5 * (S * Zi_epsi/sigmastar)) * sigx6 *
    ewv/gisig^2), FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 2)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = wHvar * (sigx4 *
    sigx12 * sigx9 - (sigx13 * giZisq + 0.5 * (S * Zisq_epsi/sigmastar)) * sigx6 *
    ewv/gisig^2), FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 1)] <- sum(wHvar *
    ((S * Zi_epsi/gisig - (euvg * sigx7 * giZi/gisig^2 + dmusig * sigx17 * sigx8/pmusig)) *
      sigx8 - ((sigx15 * giZi + (euvg * (Zisq - 2 * (sigmastar * sigx7 * gisq *
      giZi^2/gisig^2)) + S * giZi * Zi_epsi) * sigx7) * sigx6/gisig^2 + 0.5 *
      ((Zisq - giZi^2/gisq)/gisq))))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    ((S * Zi_epsi/gisig - (euvg * sigx7 * giZi/gisig^2 + dmusig * sigx17 * sigx8/pmusig)) *
      sigx9 - ((sigx15 * giZisq + (euvg * (Zicub - 2 * (sigmastar * sigx7 *
      gisq * giZi * giZisq/gisig^2)) + S * giZi * Zisq_epsi) * sigx7) * sigx6/gisig^2 +
      0.5 * ((Zicub - giZi * giZisq/gisq)/gisq))))
  hessll[(nXvar + nuZUvar + nvZVvar + 2), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    ((S * Zisq_epsi/gisig - (euvg * sigx7 * giZisq/gisig^2 + dmusig * sigx17 *
      sigx9/pmusig)) * sigx9 - ((sigx16 * giZisq + (euvg * (Zifour - 2 * (sigmastar *
      sigx7 * gisq * giZisq^2/gisig^2)) + S * giZisq * Zisq_epsi) * sigx7) *
      sigx6/gisig^2 + 0.5 * ((Zifour - giZisq^2/gisq)/gisq))))
...
#' @noRd
exponormAlgOpt_mbc92 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  startLoglik <- sum(pexponormlike_mbc92(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pexponormlike_mbc92,
    grad = pgradexponormlike_mbc92, hess = phessexponormlike_mbc92, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phessexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      -phessexponormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradexponormlike_mbc92(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessexponormlike_mbc92(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phessexponormlike_mbc92(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- pexponormlike_mbc92(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradexponormlike_mbc92(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
