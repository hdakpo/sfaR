...
#' @noRd
ctruncnormlike <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar, uHvar,
  ...
  ll <- (-1/2 * log(exp(Wu) + exp(Wv)) + dnorm((mu + S * epsilon)/sqrt(exp(Wu) +
    exp(Wv)), log = TRUE) + pnorm(mustar/sigmastar, log.p = TRUE) - pnorm(mu/sqrt(exp(Wu)),
    log.p = TRUE))
  return(ll * wHvar)
}
...
#' @param vHvar matrix of Zv variables
#' @noRd
csttruncnorm <- function(olsObj, epsiRes, S, nmuZUvar, nuZUvar, uHvar, muHvar, nvZVvar,
  vHvar) {
...
#' @param S integer for cost/prod estimation
#' @noRd
cgradtruncnormlike <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar, uHvar,
  vHvar, Yvar, Xvar, S, wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  delta <- parm[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)]
  ...
  sigx3 <- 0.5 * ((1 - exp(Wv)/(sigma_sq)) * exp(Wu)/sigmastar) + sigmastar
  wusq <- exp(Wu)/(sigma_sq)
  sigx4 <- (mu * exp(Wv) - S * exp(Wu) * (epsilon))/(sigx2)^2
  sigx5 <- (0.5 * ((1 - wusq) * exp(Wv)/sigmastar) + sigmastar) * sigx4
  ...
  pmuwu <- exp(Wu/2) * pmu
  gradll <- (cbind(sweep(Xvar, MARGIN = 1, STATS = S * (dmusig * exp(Wu)/(pmusigx2) +
    (mustar3))/(sigma_sq), FUN = "*"), sweep(muHvar, MARGIN = 1, STATS = ((dmusig *
    exp(Wv)/(pmusigx2) - (mustar3))/(sigma_sq) - dmu/(pmuwu)), FUN = "*"), sweep(uHvar,
    MARGIN = 1, STATS = ((dmustar2epsix3 - (sigx6) * dmusig/pmusig) * exp(Wu) +
      0.5 * (mu * dmu/(pmuwu))), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = (dmustar2epsix3 +
...
#' @param S integer for cost/prod estimation
#' @noRd
chesstruncnormlike <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar, uHvar,
  vHvar, Yvar, Xvar, S, wHvar) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  delta <- parm[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)]
  ...
  sigx7 <- 0.5 * ((((mustar3)^2/(sigma_sq) - 2)/(dmustar2 * (sigma_sq)) - dmustar2epsix2sq) *
    dmustar2 * (mustar3)/(sigma_sq))
  sigx8 <- 0.5 * (((2 - (mustar3)^2/(sigma_sq))/(dmustar2 * (sigma_sq)) + dmustar2epsix2sq) *
    dmustar2 * (mustar3)/(sigma_sq))
  ...
  sigx13 <- ((0.5 * ((0.5 * ((mustar3)^2/(dmustar2 * (sigma_sq)^3)) - (sigx12)/dmusqx2) *
    dmustar2epsi) - dmustar2epsix3) * exp(Wu) + 0.5 * (dmustar2epsix2) - 0.5)/(sigma_sq)
  sigx14 <- ((sigx3)/(sigx2)^2 + dmusig * (sigmu)/((sigma_sq) * pmusigx2))
  sigx15 <- ((0.5 * ((0.5 * ((mustar3)^2/(dmustar2 * (sigma_sq)^3)) - (sigx12)/dmusqx2) *
    dmustar2epsi) - dmustar2epsix3) * exp(Wv) + 0.5 * (dmustar2epsix2) - 0.5)/(sigma_sq)
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S^2 * wHvar *
    ((0 - 1) - ((muwuepsi)/(exp(Wv) * pmusigx2) + dmusig * exp(Wu)/(pmusigx2)^2) *
      dmusig * wusq)/(sigma_sq), FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -wHvar * (S * ((0 - 1) + ((muwuepsi)/(pmusigx2) + dmusig * exp(Wu) *
      exp(Wv)/(pmusigx2)^2) * dmusig/(sigma_sq))/(sigma_sq)), FUN = "*"), muHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * wHvar * (sigx7 - (((sigx10)/(sigx2)^2 - (sigx6) *
      dmusig/((sigma_sq) * pmusigx2)) * exp(Wu) - ((sigx6) * (muwuepsi)/exp(Wv) +
      1/sigmastar)/(sigma_sq)) * dmusig/pmusig) * exp(Wu), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S * wHvar * (sigx7 -
    (sigx14 * exp(Wu) + (muwuepsi) * (sigmu)/((sigma_sq) * exp(Wv))) * dmusig/pmusig) *
    exp(Wv), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * (dmu * (dmu/(pmuwu)^2 + mu/(exp(Wu/2)^3 * pmu)) -
      ((0 + 1) + ((muwuepsi)/(exp(Wu) * pmusigx2) + dmusig * exp(Wv)/(pmusigx2)^2) *
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar), (nXvar + nmuZUvar +
    ...
    ((sigx13 - (((sigx10) * (mu * exp(Wv) - (2 * ((sigx10) * sigx11) + 3 * (S *
      (epsilon))) * exp(Wu)) + (0.5 * (wusq) - 0.5 * (0.5 * (1 - wusq) + wusq)) *
      (1 - wusq) * exp(Wv) * sigmustar)/(sigx2)^2 + (sigx6)^2 * (sigx9) * exp(Wu) +
      S * (epsilon)/(sigx2)) * dmusig/pmusig) * exp(Wu) + 0.5 * (mu * (0.5 *
      (mu^2/(exp(Wu/2)^3 * pmu)) - (0.5 * (pmuwu) - 0.5 * (mu * dmu))/(pmuwu)^2) *
      dmu)), FUN = "*"), uHvar)
  hessll[(nXvar + nmuZUvar + nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * (sigx15 + dmusig * (mu/(sigx2) - ((((3 * (mu) -
      2 * ((sigx3) * (sigma_sq) * (muwuepsi) * sigmastar/(sigx2)^2)) * exp(Wv) -
      S * exp(Wu) * (epsilon)) * (sigx3) + (0.5 * (exp(Wv)/(sigma_sq)) - 0.5 *
      (0.5 * (1 - exp(Wv)/(sigma_sq)) + exp(Wv)/(sigma_sq))) * (1 - exp(Wv)/(sigma_sq)) *
      exp(Wu) * (muwuepsi)/sigmastar)/(sigx2)^2 + (sigx9) * exp(Wv) * (mu/(sigx2) -
      (sigx3) * (muwuepsi)/(sigx2)^2)^2))/pmusig) * exp(Wv), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar + 1):(nXvar + nmuZUvar +
    nuZUvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = wHvar * ((sigx8 -
    ((sigx10) * exp(Wv)/(sigx2)^2 - (sigx6) * ((muwuepsi)/exp(Wu) + dmusig *
      exp(Wv)/(pmusigx2))/(sigma_sq)) * dmusig/pmusig) * exp(Wu) + 0.5 * (((1 -
    mu^2/exp(Wu/2)^2)/(pmuwu) - mu * dmu/(pmuwu)^2) * dmu)), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + nmuZUvar + nuZUvar + 1):(nXvar +
    ...
    (((1/sigmastar - (muwuepsi) * (sigmu)/exp(Wu))/(sigma_sq) - sigx14 * exp(Wv)) *
      dmusig/pmusig + sigx8) * exp(Wv), FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + nmuZUvar + nuZUvar), (nXvar + nmuZUvar +
    nuZUvar + 1):(nXvar + nmuZUvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((sigx6) * (sigx9) * (sigmu) - ((0.5 * ((1 -
      wusq) * wvsq) + 0.5 * ((wusq - 1) * wvsq + 1 - 0.5 * ((1 - wusq) * (1 -
      wvsq)))) * sigmustar + mu * (sigx10) - (sigx3) * (2 * ((sigx10) * sigx11) +
      S * (epsilon)))/(sigx2)^2) * dmusig/pmusig + (0.5 * ((0.5 * ((mustar3)^2/(dmustar2 *
      (sigma_sq)^3)) - (sigx12)/dmusqx2) * dmustar2epsi) - dmustar2epsix3)/(sigma_sq)) *
...
truncnormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S, nXvar,
  ...
  startLoglik <- sum(ctruncnormlike(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(ctruncnormlike(parm,
    ...
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = ctruncnormlike,
    grad = cgradtruncnormlike, hess = chesstruncnormlike, start = startVal, finalHessian = if (hessianType ==
    ...
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(ctruncnormlike(parm,
      ...
      gr = function(parm) -colSums(cgradtruncnormlike(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)),
      hs = function(parm) as(-chesstruncnormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar, uHvar = uHvar,
        vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar), "dgCMatrix"),
      method = "Sparse", control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
        ...
        preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal, fn = function(parm) -sum(ctruncnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar)), gr = function(parm) -colSums(cgradtruncnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar)), hess = function(parm) -chesstruncnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar), print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(ctruncnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar)), gradient = function(parm) -colSums(cgradtruncnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar)), hessian = function(parm) -chesstruncnormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, nmuZUvar = nmuZUvar,
      muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
  ...
  mleObj$logL_OBS <- ctruncnormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)
  mleObj$gradL_OBS <- cgradtruncnormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, nmuZUvar = nmuZUvar, muHvar = muHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar)
