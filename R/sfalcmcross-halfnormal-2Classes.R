...
#' @noRd
cLCMhalfnormlike2C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
csLCMfhalfnorm2C <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", colnames(uHvar)), paste0("Zv_",
    ...
    colnames(vHvar)), paste0("Cl1_", colnames(Zvar)))
...
#' @param nZHvar number of separating variables
#' @noRd
cgradLCMhalfnormlike2C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx6 <- (S * (0.5 * dpepsisq - (1/sqsq1 - sigx2_1 * exp(Wu1)/sqsq1^2) * dmusig1 *
    depsisq1) * (epsilon1)/wzdeno - 0.5 * wdpdsq)
  sigx7 <- (sigx3_1 * dmusig1 * depsisq1 * exp(Wu1)/sqsq1^2 + 0.5 * dpepsisq) *
    (epsilon1)
  ...
  sigx12 <- (S * (0.5 * sigx11 - sigx10 * dmusig2 * depsisq2) * (epsilon2) - 0.5 *
    (dpsq2))
  sigx13 <- (S * (sigx3_2 * dmusig2 * depsisq2 * exp(Wu2)/sqsq2^2 + 0.5 * sigx11) *
    (epsilon2) - 0.5 * (dpsq2))
  sigx14 <- (prC * depsisq2 * pmusig2/wzdsq2)
  sigx15 <- (2 * (sigx9) - 2 * sigx14) * exp(Wz)
  sigx26 <- (S * sigx7/wzdeno - 0.5 * wdpdsq)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = 2 * (S * sigx1_1 * exp(Wz)/sigx5),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (exp(Wu1) * exp(Wz) * sigx6/sigsq_1),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (exp(Wv1) * exp(Wz) * sigx26/sigsq_1),
    FUN = "*"), sweep(Xvar, MARGIN = 1, STATS = 2 * (S * prC * sigx1_2/(s3q)),
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (prC * exp(Wu2) * sigx12/sigsq_2),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 * (prC * exp(Wv2) * sigx13/sigsq_2),
    FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx15/sigx4, FUN = "*"))
...
#' @param nZHvar number of separating variables
#' @noRd
chessLCMhalfnormlike2C <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx14 <- (prC * depsisq2 * pmusig2/wzdsq2)
  sigx15 <- (2 * (sigx9) - 2 * sigx14) * exp(Wz)
  sigx16 <- (S * (dmusig1 * exp(Wu1)/sigmastar1 + S * pmusig1 * (epsilon1)) * (epsilon1)/(sigma_sq1) -
    pmusig1)
  ...
  sigx20 <- 0.5 * (S * depsisq1 * (S * (0.5 * sigx17 - (1/sqsq1 - sigx2_1 * exp(Wu1)/sqsq1^2) *
    dmusig1) * (epsilon1) - 2 * (pmusig1/(sigma_sq1))) * (epsilon1)/(sigma_sq1)^2)
  sigx21 <- 0.5 * (wzdeno * depsisq2 * pmusig2/wzdsq2^2)
  sigx22 <- (sigx3_2 * dmusig2 * depsisq2 * exp(Wu2)/sqsq2^2 + 0.5 * sigx11)
  sigx23 <- (0.5 * sigx11 - sigx10 * dmusig2 * depsisq2)
  sigx24 <- 0.5 * ((S * sigx23 * (epsilon2) - dpsq2)/(sigma_sq2))
  sigx25 <- (S * (dmusig2 * exp(Wu2)/sigmastar2 + S * pmusig2 * (epsilon2)) * (epsilon2)/(sigma_sq2) -
    pmusig2)
  sigx26 <- (S * sigx7/wzdeno - 0.5 * wdpdsq)
  wusq2 <- exp(Wu2)/(sigma_sq2)
  sigx27 <- depsisq2 * (epsilon2)^2/(sigma_sq2)^2
  sigx28 <- 0.5 * (wzdeno * (S * (0.5 * dpepsisq - (1/sqsq1 - sigx2_1 * exp(Wu1)/sqsq1^2) *
    dmusig1 * depsisq1) * (epsilon1) - wzdeno^2 * depsisq1 * pmusig1/wzdsq1^2)/wzdsq1^2)
  sigx29 <- dmusig1 * (depsisq1 * exp(Wu1)/exp(Wv1) + depsisq1) * exp(Wu1) * (epsilon1)
  wusqx2 <- exp(Wu1)/sqsq1^2
  sigx30 <- 1/sqsq1 - sigx2_1 * wusqx2
  sigx31 <- (sigx30) * dmusig1 * depsisq1
  sigx32 <- dmusig2 * (depsisq2 * exp(Wu2)/exp(Wv2) + depsisq2) * exp(Wu2) * (epsilon2)
  sigx33 <- (0.5 * (sigx4/sqrt(sigma_sq2)) + 2 * (prC * sigx12))
  sigx34 <- 0.5 * (S * pmusig2 * (epsilon2)/(sigma_sq2)^2)
  sigx35 <- 0.5 * (S * depsisq2 * (S * (sigx34 - sigx10 * dmusig2) * (epsilon2) -
    2 * (pmusig2/(sigma_sq2))) * (epsilon2)/(sigma_sq2)^2)
  sigx36 <- (0.5 * (sigx4/sqrt(sigma_sq1)) + 2 * (exp(Wz) * sigx6))
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    2 * (S^2 * ((depsisq1 * sigx16 + S * sigx29/sqsq1)/sigx5 - 2 * (sigx1_1^2 *
    exp(Wz)/sigx5^2)) * exp(Wz)), FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * 2 * (S * (((sigx31 + S * (sigx18 - S * (sigx30) * dmusig1 *
      (depsisq1 * exp(Wu1)/exp(Wv1) + depsisq1) * (epsilon1)) * (epsilon1)/(sigma_sq1))/wzdeno -
      sigx19)/sigsq_1 - 2 * (sigx1_1 * exp(Wz) * sigx6/(sigsq_1^2 * wzdeno *
      (sigma_sq1)))) * exp(Wu1) * exp(Wz)), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((S * (sigx18 + S * sigx3_1 * sigx29/sqsq1^2) *
      (epsilon1)/(sigma_sq1) - sigx3_1 * dmusig1 * depsisq1 * wusqx2)/wzdeno -
      sigx19)/sigsq_1 - 2 * (sigx1_1 * exp(Wz) * sigx26/(sigsq_1^2 * wzdeno *
      (sigma_sq1)))) * exp(Wv1) * exp(Wz)), FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S^2 * prC * sigx1_1 * sigx1_2 * (sigma_sq2) *
      exp(Wz) * sqrt(sigma_sq2)/((s3q)^2 * wzdeno * (sigma_sq1) * sqrt(sigma_sq1)))) *
      wHvar, FUN = "*"), Xvar)
  hessll[1:nXvar, (2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 * (S * prC * sigx1_1 *
    exp(Wu2) * exp(Wz) * sigx12 * sqrt(sigma_sq2)/(sigsq_2^2 * wzdeno * (sigma_sq1) *
    sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  hessll[1:nXvar, (2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar +
    2 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(4 * (S * prC *
    sigx1_1 * exp(Wv2) * exp(Wz) * sigx13 * sqrt(sigma_sq2)/(sigsq_2^2 * wzdeno *
  ...
  hessll[1:nXvar, (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + 2 *
    ...
    S * (2 * sigx8 - 2 * (sigx15/(wzdeno * sigx4 * sqrt(sigma_sq1)))) * sigx1_1 *
    exp(Wz)/(sigx4 * (sigma_sq1)), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((exp(Wu1) * (S * (sigx20 - (0.5 * (S^2 *
      (sigx30) * depsisq1 * (epsilon1)^2/(sigma_sq1)^2) - (((0.5 * (wusq1) +
      1 - 0.5 * (0.5 * (1 - wusq1) + wusq1)) * (1 - wusq1) * exp(Wv1)/sigmastar1 +
      (2 - 2 * (sigx2_1^2 * exp(Wu1) * (sigma_sq1)/sqsq1^2)) * sigmastar1)/sqsq1^2 +
      S^2 * (sigx30)^2 * exp(Wu1) * (epsilon1)^2/sqsq1) * depsisq1) * dmusig1) *
      (epsilon1)/wzdeno - sigx28) + S * (0.5 * dpepsisq - sigx31) * (epsilon1)/wzdeno -
      0.5 * wdpdsq)/sigsq_1 - sigx36 * exp(Wu1) * sigx6/sigsq_1^2) * exp(Wu1) *
      exp(Wz)), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * 2 * (((S *
    (((((0.5 * ((1 - wusq1) * exp(Wv1)) - S^2 * sigx3_1 * (sigx30) * exp(Wu1) *
      (epsilon1)^2)/(sigma_sq1) + 0.5 * ((wusq1 - 1) * wvsq1 + 1 - 0.5 * ((1 -
      wusq1) * (1 - wvsq1)))) * depsisq1/sigmastar1 + 0.5 * (S^2 * sigx3_1 *
      depsisq1 * (epsilon1)^2/(sigma_sq1)^2)) * exp(Wu1) + sigx3_1 * (1 - 2 *
      (sigx2_1 * exp(Wu1) * (sigma_sq1) * sigmastar1/sqsq1^2)) * depsisq1) *
      dmusig1/sqsq1^2 + sigx20) * (epsilon1)/wzdeno - sigx28)/sigsq_1 - sigx36 *
    sigx26/sigsq_1^2) * exp(Wu1) * exp(Wv1) * exp(Wz)), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar +
    nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(4 * (S *
    prC * sigx1_2 * exp(Wu1) * (sigma_sq2) * exp(Wz) * sigx6 * sqrt(sigma_sq2)/((s3q)^2 *
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + nuZUvar + nvZVvar + 1):(2 *
    ...
    (prC * exp(Wu1) * exp(Wu2) * exp(Wz) * sigx6 * sigx12 * sqrt(sigma_sq2)/(sigsq_2^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 *
    nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(4 * (prC * exp(Wu1) * exp(Wv2) * exp(Wz) * sigx13 * sigx6 * sqrt(sigma_sq2)/(sigsq_2^2 *
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    ...
    MARGIN = 1, STATS = wHvar * (2 * ((0.5 * (S^2 * sigx8 * depsisq1 * (epsilon1)^2/(sigma_sq1)^2) -
      ((0.5/sqrt(sigma_sq1) - wzdeno^2 * sqrt(sigma_sq1)/wzdsq1^2) * exp(Wz) +
        0.5 * (wzdeno/sqrt(sigma_sq1))) * depsisq1/wzdsq1^2) * pmusig1 -
      S * sigx8 * sigx31 * (epsilon1)) - 2 * (sigx15 * sigx6/sigsq_1)) * exp(Wu1) *
      exp(Wz)/sigx4, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    2 * (((S * ((((0.5 * (wvsq1) - 0.5 * (0.5 * (1 - wvsq1) + wvsq1)) * (1 -
    wvsq1) + S^2 * sigx3_1^2 * exp(Wu1) * exp(Wv1) * (epsilon1)^2/(sqsq1^2 *
    (sigma_sq1))) * depsisq1 * exp(Wu1)/sigmastar1 + ((0.5 * (S^2 * depsisq1 *
    (epsilon1)^2/(sigma_sq1)^2) - 2 * (sigx3_1 * depsisq1 * (sigma_sq1) * sigmastar1/sqsq1^2)) *
    exp(Wv1) + depsisq1) * sigx3_1) * dmusig1 * wusqx2 + S * (0.5 * (exp(Wv1) *
    (S * (sigx3_1 * dmusig1 * wusqx2 + 0.5 * sigx17) * (epsilon1) - 2 * (pmusig1/(sigma_sq1)))) +
    0.5 * pmusig1) * depsisq1 * (epsilon1)/(sigma_sq1)^2) * (epsilon1)/wzdeno -
    (0.5 * (depsisq1 * pmusig1) + 0.5 * (exp(Wv1) * (S * sigx7 - wzdeno^2 * depsisq1 *
      pmusig1/wzdsq1^2))) * wzdeno/wzdsq1^2)/sigsq_1 - (0.5 * (sigx4/sqrt(sigma_sq1)) +
    2 * (exp(Wz) * sigx26)) * exp(Wv1) * sigx26/sigsq_1^2) * exp(Wv1) * exp(Wz)),
    FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (S * prC * sigx1_2 * (sigma_sq2) * exp(Wv1) * exp(Wz) * sigx26 *
      sqrt(sigma_sq2)/((s3q)^2 * sqrt(sigma_sq1)))) * wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (2 * nXvar + nuZUvar +
    nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (prC * exp(Wu2) * exp(Wv1) * exp(Wz) * sigx26 *
      sigx12 * sqrt(sigma_sq2)/(sigsq_2^2 * sqrt(sigma_sq1)))) * wHvar, FUN = "*"),
    uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (2 * nXvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (prC * exp(Wv1) * exp(Wv2) * exp(Wz) * sigx26 *
      sigx13 * sqrt(sigma_sq2)/(sigsq_2^2 * sqrt(sigma_sq1)))) * wHvar, FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (2 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (2 * ((0.5 * (S^2 * sigx8 * depsisq1 * (epsilon1)^2/(sigma_sq1)^2) -
      ((0.5/sqrt(sigma_sq1) - wzdeno^2 * sqrt(sigma_sq1)/wzdsq1^2) * exp(Wz) +
        0.5 * (wzdeno/sqrt(sigma_sq1))) * depsisq1/wzdsq1^2) * pmusig1 +
      S * sigx3_1 * sigx8 * dmusig1 * depsisq1 * exp(Wu1) * (epsilon1)/sqsq1^2) -
      2 * (sigx15 * sigx26/sigsq_1)) * exp(Wv1) * exp(Wz)/sigx4, FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (nXvar +
    ...
    MARGIN = 1, STATS = wHvar * 2 * (S^2 * ((depsisq2 * sigx25 + S * sigx32/sqsq2)/(s3q) -
      2 * (prC * sigx1_2^2/(s3q)^2)) * prC), FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (2 *
    ...
    MARGIN = 1, STATS = wHvar * 2 * (S * ((sigx10 * dmusig2 * depsisq2 + (S *
      ((0.5 * sigx25 - 0.5 * pmusig2) * depsisq2/(sigma_sq2) - S * sigx10 *
        dmusig2 * (depsisq2 * exp(Wu2)/exp(Wv2) + depsisq2) * (epsilon2)) *
      (epsilon2) - 0.5 * (sigx1_2/(sigma_sq2)))/(sigma_sq2))/sigsq_2 - 2 *
      (prC * sigx1_2 * sigx12/(sigsq_2^2 * (sigma_sq2)))) * prC * exp(Wu2)),
    FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (2 *
    nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((S * ((0.5 * sigx25 - 0.5 * pmusig2) *
      depsisq2/(sigma_sq2) + S * sigx3_2 * sigx32/sqsq2^2) * (epsilon2) - 0.5 *
      (sigx1_2/(sigma_sq2)))/(sigma_sq2) - sigx3_2 * dmusig2 * depsisq2 * exp(Wu2)/sqsq2^2)/sigsq_2 -
      2 * (prC * sigx1_2 * sigx13/(sigsq_2^2 * (sigma_sq2)))) * prC * exp(Wv2)),
    FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar + nvZVvar), (2 *
    nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S * prC * (2 * ((2 *
    (sigx9) - 2 * sigx14)/sigx4) + 2/wzdeno) * sigx1_2 * exp(Wz)/(s3q)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * 2 * (((exp(Wu2) * (S * (sigx35 - (0.5 * (S^2 *
      sigx10 * sigx27) - (((0.5 * (wusq2) + 1 - 0.5 * (0.5 * (1 - wusq2) +
      wusq2)) * (1 - wusq2) * exp(Wv2)/sigmastar2 + (2 - 2 * (sigx2_2^2 * exp(Wu2) *
      (sigma_sq2)/sqsq2^2)) * sigmastar2)/sqsq2^2 + S^2 * sigx10^2 * exp(Wu2) *
      (epsilon2)^2/sqsq2) * depsisq2) * dmusig2) * (epsilon2) - sigx24) + S *
      sigx23 * (epsilon2) - 0.5 * (dpsq2))/sigsq_2 - sigx33 * exp(Wu2) * sigx12/sigsq_2^2) *
      prC * exp(Wu2)), FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((S * (((((0.5 * ((1 - wusq2) * exp(Wv2)) -
      S^2 * sigx3_2 * sigx10 * exp(Wu2) * (epsilon2)^2)/(sigma_sq2) + 0.5 *
      ((wusq2 - 1) * wvsq2 + 1 - 0.5 * ((1 - wusq2) * (1 - wvsq2)))) * depsisq2/sigmastar2 +
      0.5 * (S^2 * sigx3_2 * sigx27)) * exp(Wu2) + sigx3_2 * (1 - 2 * (sigx2_2 *
      exp(Wu2) * (sigma_sq2) * sigmastar2/sqsq2^2)) * depsisq2) * dmusig2/sqsq2^2 +
      sigx35) * (epsilon2) - sigx24)/sigsq_2 - sigx33 * sigx13/sigsq_2^2) *
      prC * exp(Wu2) * exp(Wv2)), FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + nvZVvar),
    (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
      nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(prC *
    (2 * ((2 * (sigx9) - 2 * sigx14) * sigx12/sigx4) + 2 * (S * sigx23 * (epsilon2)/wzdeno -
      sigx21)) * exp(Wu2) * exp(Wz)/sigsq_2) * wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    ...
    ((((0.5 * (wvsq2) - 0.5 * (0.5 * (1 - wvsq2) + wvsq2)) * (1 - wvsq2) + S^2 *
      sigx3_2^2 * exp(Wu2) * exp(Wv2) * (epsilon2)^2/(sqsq2^2 * (sigma_sq2))) *
      depsisq2 * exp(Wu2)/sigmastar2 + ((0.5 * (S^2 * sigx27) - 2 * (sigx3_2 *
      depsisq2 * (sigma_sq2) * sigmastar2/sqsq2^2)) * exp(Wv2) + depsisq2) *
      sigx3_2) * dmusig2 * exp(Wu2)/sqsq2^2 + S * (0.5 * (exp(Wv2) * (S * (sigx3_2 *
      dmusig2 * exp(Wu2)/sqsq2^2 + sigx34) * (epsilon2) - 2 * (pmusig2/(sigma_sq2)))) +
      0.5 * pmusig2) * depsisq2 * (epsilon2)/(sigma_sq2)^2) * (epsilon2) -
    (0.5 * (depsisq2 * pmusig2) + 0.5 * (exp(Wv2) * (S * sigx22 * (epsilon2) -
      dpsq2)))/(sigma_sq2))/sigsq_2 - (0.5 * (sigx4/sqrt(sigma_sq2)) + 2 *
    (prC * sigx13)) * exp(Wv2) * sigx13/sigsq_2^2) * prC * exp(Wv2)), FUN = "*"),
    vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar + 2 * nuZUvar + 2 *
    nvZVvar), (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(prC *
    (2 * ((2 * (sigx9) - 2 * sigx14) * sigx13/sigx4) + 2 * (S * sigx22 * (epsilon2)/wzdeno -
      sigx21)) * exp(Wv2) * exp(Wz)/sigsq_2) * wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(2 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + nZHvar), (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = wHvar * ((2 * (prC * (1/(wzdeno^2 * sqrt(sigma_sq2)) + sqrt(sigma_sq2)/wzdsq2^2) *
      depsisq2 * pmusig2) - ((2 * (sigx9) - 2 * sigx14)^2/sigx4 + 2 * ((2 -
      2 * (wzdeno * (sigma_sq1) * exp(Wz)/wzdsq1^2)) * depsisq1 * pmusig1 *
      sqrt(sigma_sq1)/wzdsq1^2))) * exp(Wz) + 2 * (sigx9) - 2 * sigx14) * exp(Wz)/sigx4,
    FUN = "*"), Zvar)
...
#' @noRd
LCM2ChnormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S, wHvar,
  ...
  startLoglik <- sum(cLCMhalfnormlike2C(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cLCMhalfnormlike2C(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradLCMhalfnormlike2C(parm, nXvar = nXvar,
    ...
    control = list(trace = if (printInfo) 1 else 0, maxeval = itermax, stepmax = stepmax,
      xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cLCMhalfnormlike2C,
    grad = cgradLCMhalfnormlike2C, hess = chessLCMhalfnormlike2C, start = startVal,
    ...
    fn = function(parm) -sum(cLCMhalfnormlike2C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike2C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cLCMhalfnormlike2C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradLCMhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessLCMhalfnormlike2C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      method = "Sparse", control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
        prec = tol, report.level = if (printInfo) 2 else 0, report.precision = 1L,
        preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal, fn = function(parm) -sum(cLCMhalfnormlike2C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradLCMhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessLCMhalfnormlike2C(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      objective = function(parm) -sum(cLCMhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradLCMhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessLCMhalfnormlike2C(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = if (printInfo) 1 else 0, eval.max = itermax,
        rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradLCMhalfnormlike2C(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessLCMhalfnormlike2C(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
      mleObj$hessian <- chessLCMhalfnormlike2C(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  }
  mleObj$logL_OBS <- cLCMhalfnormlike2C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradLCMhalfnormlike2C(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
cmargLCM2Chalfnorm_Eu <- function(object) {
  ...
  for (c in seq_len(ncol(margEff_c1))) {
    margEff_c[, c] <- ifelse(Group_c == 1, margEff_c1[, c], margEff_c2[, c])
  }
