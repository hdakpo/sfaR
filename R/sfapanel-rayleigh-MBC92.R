...
#' @noRd
praynormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstraynorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradraynormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx7 <- (1/(sigmasq * sigmastar) - (0.5 * sigx6 + sigmastar *
    gisq) * ewu/ssq)
  ...
  sigx14 <- (Zisq_epsi/(sigmasq * sigmastar) - ewu * sigx13 *
    giepsi * giZisq/ssq)
  sigx15 <- (0.5 * (dmusig * ewv * giZisq/(sigmasq * sigmastar)) +
    S^2 * dmusig * giepsi * sigx14)
  sigx16 <- (pmusig * giZisq/sigmasq + S * dmusig * sigx14)
  sigx17 <- (sigx15 * ewu + S * (pmusig * Zisq_epsi - ewu *
    sigx16 * giepsi))
  sigx18 <- (dmusig * ewv * giZi/(sigmasq * sigmastar))
  sigx19 <- (Zi_epsi/(sigmasq * sigmastar) - ewu * sigx13 *
    giepsi * giZi/ssq)
  sigx20 <- (pmusig * Zi_epsi - ewu * (pmusig * giZi/sigmasq +
    S * dmusig * sigx19) * giepsi)
  sigx21 <- (((0.5 * sigx18 + S^2 * dmusig * giepsi * sigx19) *
    ewu + S * sigx20)/sigx3 + 0.5 * giZi)
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * ewu *
    ...
    (S^2 * ewu * giepsi * sigx19/sigmastar - sigx21)/sigmasq,
    ewu * (S^2 * ewu * giepsi * sigx14/sigmastar - sigx22)/sigmasq)
...
#' @param TT vector of time of presence
#' @noRd
phessraynormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx25 <- ((0.5 * ewusq - 1.5)/(sigmasq * sigmastar) - 0.5 *
    sigx7)
  sigx26 <- ((sigx25 * ewv - 2 * (sigx24 * sigmasq * sigmastar *
    sigx13/ssq)) * ewu + 3 * sigmastar)
  sigx27 <- ((0.5 * ewusq - 0.5)/(sigmasq * sigmastar) - 0.5 *
    sigx7)
  sigx28 <- ((sigx27 * ewv - 2 * (sigx24 * sigmasq * sigmastar *
    sigx13/ssq)) * ewu + sigmastar)
  sigx29 <- (sigx28 * giepsi * giZi + sigx24 * Zi_epsi)
  sigx30 <- (sigx28 * giepsi * giZisq + sigx24 * Zisq_epsi)
  sigx31 <- (sigx30/ssq + S^2 * sigx7 * ewu * giepsi^2 * sigx14/(sigmasq *
    sigmastar))
  sigx32 <- (sigx29/ssq + S^2 * sigx7 * ewu * giepsi^2 * sigx19/(sigmasq *
    sigmastar))
  sigx33 <- (sigx24 * ewu/ssq - 1/(sigmasq * sigmastar))
  sigx34 <- (sigx24 * dmusig * ewv/ssq + S^2 * sigx7 * dmusig *
    giepsi^2/sigmasq)
  sigx35 <- ((dmusig + S^2 * sigx11 * dmusig * ewu^2 * ewv *
    giepsi^2/(ssq * sigmasq * sigmastar))/(sigmasq * sigmastar) -
    sigx11 * dmusig * ewv/ssq)
  sigx36 <- (S * sigx11 * dmusig * ewu * giepsi/ssq - pmusig/sigmasq)
  sigx37 <- ((0.5 * (wvsq/(sigmasq * sigmastar)) - 0.5 * (1/(sigmasq *
    sigmastar) - sigx11 * ewv/ssq)) * ewu - 2 * (sigx11 *
    sigmasq * sigmastar * sigx13/ssq))
  sigx38 <- ((0.5 * (sigx13/ssq) - 0.5/(sigmasq^2 * sigmastar)) *
    ewu^2 * ewv * giepsi * giZi + sigx13 * Zi_epsi)
  sigx39 <- (sigx38 * giZi + sigx13 * (giepsi * (Zisq - 2 *
    (ewu * sigmasq * sigmastar * sigx13 * giZi^2/ssq)) +
    giZi * Zi_epsi))
  sigx40 <- (sigx38 * giZisq + sigx13 * (giepsi * (Zicub -
    2 * (ewu * sigmasq * sigmastar * sigx13 * giZi * giZisq/ssq)) +
    giZi * Zisq_epsi))
  sigx41 <- ((0.5 * (sigx13/ssq) - 0.5/(sigmasq^2 * sigmastar)) *
    ewu^2 * ewv * giepsi * giZisq + sigx13 * Zisq_epsi)
  sigx42 <- (sigx41 * giZisq + sigx13 * (giepsi * (Zifour -
    2 * (ewu * sigmasq * sigmastar * sigx13 * giZisq^2/ssq)) +
    giZisq * Zisq_epsi))
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * S^2 * (1/ewv - (((1 - S^2 * ewu * giepsi^2/(sigmasq *
      ewv)) * sigx1 - ewu/(sigmasq * sigmastar)) * dmusig +
      ewu * sigx2^2/(sigx3 * sigmasq))/sigx3) * ewu/sigmasq,
  ...
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S * (((sigx5 * sigx2/sigx3 +
      ...
      pmusig) * ewusq/sigx3 + 2 * (S * sigx7 * ewu * giepsi/sigmastar)) *
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    ...
    STATS = wHvar * S * (((sigx10 * sigx2/sigx3 - S * (0.5 *
      (wvsq/ewv) + 1/sigmasq) * dmusig * giepsi/sigmastar) *
      ewu + pmusig)/(sigx3 * sigmasq) - 2 * (S * sigx11 *
      ewu * giepsi/(ssq * sigmastar))) * ewu/sigmasq *
      ewv, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(XiZi,
    MARGIN = 1, STATS = wHvar/(sigmasq * sigmastar) * giepsi *
      S * ewu/sigmastar * S * ewu/sigmasq, FUN = "*") -
    sweep(Xgi, MARGIN = 1, STATS = wHvar * ewu * sigx13 *
      giZi/ssq * giepsi * S * ewu/sigmastar * S * ewu/sigmasq,
      FUN = "*") + sweep(Xgi, MARGIN = 1, STATS = wHvar *
    sigx19 * S * ewu/sigmastar * S * ewu/sigmasq, FUN = "*") -
    (sweep(Xgi, MARGIN = 1, STATS = wHvar * ((((0.5 * sigx18 +
      S^2 * dmusig * giepsi * sigx19) * ewu + S * sigx20) *
      sigx2/sigx3 - pmusig * giZi)/sigmasq + S * ((1 -
      S^2 * ewu * giepsi^2/(sigmasq * ewv)) * sigx19 +
      (ewu * (S^2 * giepsi * sigx19/ewv + giZi/(sigmasq *
        sigmastar)) * giepsi - (0.5 * (ewu * giepsi *
        giZi/sigmasq) + 2 * Zi_epsi)/sigmastar)/sigmasq +
      ewu * sigx13 * giepsi * giZi/ssq) * dmusig) * ewu/sigx3 *
      S * ewu/sigmasq, FUN = "*") + sweep(XiZi, MARGIN = 1,
      STATS = wHvar * pmusig/sigx3 * S * ewu/sigmasq, FUN = "*")))
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(XiZisq,
    MARGIN = 1, STATS = wHvar/(sigmasq * sigmastar) * giepsi *
      S * ewu/sigmastar * S * ewu/sigmasq, FUN = "*") -
    sweep(Xgi, MARGIN = 1, STATS = wHvar * ewu * sigx13 *
      giZisq/ssq * giepsi * S * ewu/sigmastar * S * ewu/sigmasq,
      FUN = "*") + sweep(Xgi, MARGIN = 1, STATS = wHvar *
    sigx14 * S * ewu/sigmastar * S * ewu/sigmasq, FUN = "*") -
    (sweep(Xgi, MARGIN = 1, STATS = wHvar * ((sigx17 * sigx2/sigx3 -
      pmusig * giZisq)/sigmasq + S * ((1 - S^2 * ewu *
      giepsi^2/(sigmasq * ewv)) * sigx14 + (ewu * (S^2 *
      giepsi * sigx14/ewv + giZisq/(sigmasq * sigmastar)) *
      giepsi - (0.5 * (ewu * giepsi * giZisq/sigmasq) +
      2 * Zisq_epsi)/sigmastar)/sigmasq + ewu * sigx13 *
      giepsi * giZisq/ssq) * dmusig) * ewu/sigx3 * S *
      ewu/sigmasq, FUN = "*") + sweep(XiZisq, MARGIN = 1,
      STATS = wHvar * pmusig/sigx3 * S * ewu/sigmasq, FUN = "*")))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((0.5 * (dmusig * ewv/sigmastar) + ewu * (S^2 * sigx7 *
      dmusig * giepsi^2 - (sigx5 * ewusq/sigx3 + gisq) *
      sigx5/sigmasq) - (0.5 * (0.5 * (ewusq * dmusig *
      ewv/sigmastar) + S^2 * sigx7 * dmusig * ewu * giepsi^2) +
      S * pmusig * giepsi)) * ewusq/sigx3 + ewu * (S^2 *
      (2/(sigmasq * sigmastar) - (((0.5 * (ewu * gisq/sigmasq) +
        2 - 0.5 * (0.5 * ewusq + ewu * gisq/sigmasq)) *
        ewusq * ewv/sigmastar + (4 * gisq - 2 * (sigx24^2 *
        ewu * sigmasq/ssq)) * sigmastar) * ewu/ssq +
        0.5 * (ewusq * sigx7))) * giepsi^2/sigmastar -
      sigx8 * gisq/sigmasq) - 0.5 * gisq) * ewu/sigmasq,
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((0.5 * (wvsq * dmusig) +
      0.5 * ((dmusig * ewv * gisq/sigmasq - S^2 * wvsq *
        sigx7 * dmusig * ewu * giepsi^2/sigmastar) *
        ewu/sigmasq - 0.5 * (ewusq * wvsq * dmusig)))/sigmastar +
      (S * pmusig * giepsi - (sigx10 * sigx5 * ewusq/sigx3 +
        S * (pmusig * gisq/sigmasq + S * sigx7 * dmusig *
          giepsi) * giepsi) * ewu)/sigmasq)/sigx3 - (sigx12 *
      gisq/sigmasq + S^2 * (((0.5 * (ewusq * ewv/sigmasq) +
      0.5 * ((ewu * gisq/sigmasq - 1) * ewv/sigmasq + 1 -
        0.5 * (ewusq * wvsq)) + 0.5 * wvsq) * ewu/sigmastar +
      sigmastar)/(ssq * sigmastar) + sigx11 * (1/(ssq *
      sigmastar) - (0.5 * (ssq * ewusq/(sigmasq * sigmastar)) +
      2 * (sigx24 * ewu)) * ewu * ewv/(ssq * sigmastar)^2)) *
      ewu * giepsi^2)) * ewu * ewv/sigmasq, FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1)] <- colSums(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ewu * (ewu * (S^2 * (2 * (Zi_epsi/(sigmasq * sigmastar)) -
    ((sigx26 * giepsi * giZi + sigx24 * Zi_epsi) * ewu/ssq +
      0.5 * (ewusq * sigx19))) * giepsi/sigmastar - (((0.5 *
    (dmusig * ewv * giZi/sigmastar) - ((0.5 * sigx18 + S^2 *
    dmusig * giepsi * sigx19) * ewu + S * sigx20) * sigx5 *
    ewusq/sigx3)/sigmasq + S * (((pmusig * gisq/sigmasq +
    S * sigx7 * dmusig * giepsi) * giZi/sigmasq + S * sigx32 *
    dmusig) * ewu + S * sigx33 * dmusig * Zi_epsi - pmusig *
    giZi/sigmasq) * giepsi - (0.5 * (sigx34 * giZi) + S^2 *
    sigx32 * dmusig * giepsi) * ewu)/sigx3 + (S^2 * ewu *
    giepsi * sigx19/sigmastar - sigx21) * gisq/sigmasq)) -
    sigx21)/sigmasq, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 2)] <- colSums(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ewu * (ewu * (S^2 * (2 * (Zisq_epsi/(sigmasq * sigmastar)) -
    ((sigx26 * giepsi * giZisq + sigx24 * Zisq_epsi) * ewu/ssq +
      0.5 * (ewusq * sigx14))) * giepsi/sigmastar - (((0.5 *
    (dmusig * ewv * giZisq/sigmastar) - sigx17 * sigx5 *
    ewusq/sigx3)/sigmasq + S * (((pmusig * gisq/sigmasq +
    S * sigx7 * dmusig * giepsi) * giZisq/sigmasq + S * sigx31 *
    dmusig) * ewu + S * sigx33 * dmusig * Zisq_epsi - pmusig *
    giZisq/sigmasq) * giepsi - (0.5 * (sigx34 * giZisq) +
    S^2 * sigx31 * dmusig * giepsi) * ewu)/sigx3 + sigx23 *
    gisq/sigmasq)) - sigx22)/sigmasq, FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = wHvar * (((((0.5 * (ewv * (S^2 *
      ...
      dmusig)/sigmasq - 0.5 * (wvsq * dmusig)) + 0.5 *
      dmusig) * wvsq/sigmastar + (ewv * (S * sigx36 * giepsi -
      sigx10^2 * ewu/sigx3) + S * pmusig * giepsi)/sigmasq)/sigx3 -
      S^2 * (sigx11 * (1/(ssq * sigmastar) - (0.5 * (ssq *
        wvsq/(sigmasq * sigmastar)) + 2 * (sigx11 * ewv)) *
        ewu * ewv/(ssq * sigmastar)^2) + (0.5 * (ewv/sigmasq) -
        0.5 * (0.5 * wvsq + ewv/sigmasq)) * wvsq * sigmasq/(ssq *
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = -wHvar * ((((0.5 * (sigx35 * giZi) +
      S * (S * sigx11 * dmusig * Zi_epsi/ssq - sigx36 *
        giZi/sigmasq) * giepsi - ((0.5 * sigx18 + S^2 *
      dmusig * giepsi * sigx19) * ewu + S * sigx20) * sigx10/(sigx3 *
      sigmasq)) * ewv/sigx3 + S^2 * ((sigx37 * ewu * giepsi *
      giZi + sigx11 * Zi_epsi) * ewv/ssq + 0.5 * (wvsq *
      sigx19)) * giepsi/sigmastar) * ewu + ewv * (S^2 *
      ewu * giepsi * sigx19/sigmastar - sigx21)/sigmasq) *
      ewu/sigmasq), FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * ((((0.5 * (sigx35 * giZisq) +
      S * (S * sigx11 * dmusig * Zisq_epsi/ssq - sigx36 *
        giZisq/sigmasq) * giepsi - sigx17 * sigx10/(sigx3 *
      sigmasq)) * ewv/sigx3 + S^2 * ((sigx37 * ewu * giepsi *
      giZisq + sigx11 * Zisq_epsi) * ewv/ssq + 0.5 * (wvsq *
      sigx14)) * giepsi/sigmastar) * ewu + ewv * sigx23/sigmasq) *
      ewu/sigmasq), FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar +
    nvZVvar + 1)] <- sum(wHvar * (ewu * (ewu * (S^2 * ((0.5 *
    (giZi * sigx19/sigmasq) - sigx39/ssq) * ewu * giepsi +
    Zi_epsi * sigx19)/sigmastar - ((((0.5 * sigx18 + S^2 *
    dmusig * giepsi * sigx19) * ewu + S * sigx20)^2/(sigx3 *
    sigmasq) + 0.5 * (((dmusig * Zisq - S^2 * dmusig * ewu^2 *
    giepsi * giZi * sigx19/(sigmasq * sigmastar))/(sigmasq *
    sigmastar) - dmusig * ewu * sigx13 * giZi^2/ssq) * ewv) +
    S * (S * ((Zi_epsi - S^2 * ewu^2 * giepsi^2 * sigx19/(sigmasq *
      sigmastar)) * sigx19 - sigx39 * ewu * giepsi/ssq) *
      dmusig - (((pmusig * Zisq - ewu * (pmusig * giZi/sigmasq +
      S * dmusig * sigx19) * giZi)/sigmasq - S * (sigx39/ssq +
      S^2 * ewu * giepsi * sigx19^2/(sigmasq * sigmastar)) *
      dmusig * ewu) * giepsi + (2 * (S * dmusig * sigx19) +
      pmusig * giZi/sigmasq) * Zi_epsi)))/sigx3 + (S^2 *
    ewu * giepsi * sigx19/sigmastar - sigx21) * giZi/sigmasq)) -
    0.5 * Zisq)/sigmasq))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar +
    nvZVvar + 2)] <- sum(wHvar * (ewu * (ewu * (S^2 * ((0.5 *
    (giZi * sigx14/sigmasq) - sigx40/ssq) * ewu * giepsi +
    Zi_epsi * sigx14)/sigmastar - ((((0.5 * sigx18 + S^2 *
    dmusig * giepsi * sigx19) * ewu + S * sigx20) * sigx17/(sigx3 *
    sigmasq) + 0.5 * (((dmusig * Zicub - S^2 * dmusig * ewu^2 *
    giepsi * giZisq * sigx19/(sigmasq * sigmastar))/(sigmasq *
    sigmastar) - dmusig * ewu * sigx13 * giZi * giZisq/ssq) *
    ewv) + S * (S * ((Zi_epsi - S^2 * ewu^2 * giepsi^2 *
    sigx19/(sigmasq * sigmastar)) * sigx14 - (sigx40 * ewu *
    giepsi/ssq + sigx19 * Zisq_epsi)) * dmusig - (((pmusig *
    Zicub - ewu * (pmusig * giZi/sigmasq + S * dmusig * sigx19) *
    giZisq)/sigmasq - S * (sigx40/ssq + S^2 * ewu * giepsi *
    sigx19 * sigx14/(sigmasq * sigmastar)) * dmusig * ewu) *
    giepsi + sigx16 * Zi_epsi)))/sigx3 + sigx23 * giZi/sigmasq)) -
    0.5 * Zicub)/sigmasq))
  hessll[(nXvar + nuZUvar + nvZVvar + 2), (nXvar + nuZUvar +
    nvZVvar + 2)] <- sum(wHvar * (ewu * (ewu * (S^2 * ((0.5 *
    (giZisq * sigx14/sigmasq) - sigx42/ssq) * ewu * giepsi +
    Zisq_epsi * sigx14)/sigmastar - ((sigx17^2/(sigx3 * sigmasq) +
    0.5 * (((dmusig * Zifour - S^2 * dmusig * ewu^2 * giepsi *
      giZisq * sigx14/(sigmasq * sigmastar))/(sigmasq *
      sigmastar) - dmusig * ewu * sigx13 * giZisq^2/ssq) *
      ewv) + S * (S * ((Zisq_epsi - S^2 * ewu^2 * giepsi^2 *
    sigx14/(sigmasq * sigmastar)) * sigx14 - sigx42 * ewu *
    giepsi/ssq) * dmusig - (((pmusig * Zifour - ewu * sigx16 *
    giZisq)/sigmasq - S * (sigx42/ssq + S^2 * ewu * giepsi *
    sigx14^2/(sigmasq * sigmastar)) * dmusig * ewu) * giepsi +
    (2 * (S * dmusig * sigx14) + pmusig * giZisq/sigmasq) *
      Zisq_epsi)))/sigx3 + sigx23 * giZisq/sigmasq)) -
    0.5 * Zifour)/sigmasq))
...
#' @noRd
raynormAlgOpt_mbc92 <- function(start, olsParam, dataTable, S,
  ...
  startLoglik <- sum(praynormlike_mbc92(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      -sum(praynormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p))
    }, gr = function(parm) {
      -colSums(pgradraynormlike_mbc92(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p))
    }, hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      maxeval = itermax, stepmax = stepmax, xtol = tol,
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = praynormlike_mbc92,
    grad = pgradraynormlike_mbc92, hess = phessraynormlike_mbc92,
    ...
    fn = function(parm) {
      ...
      as(-phessraynormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
    ...
    fn = function(parm) {
      ...
      -phessraynormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradraynormlike_mbc92(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessraynormlike_mbc92(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (method == "sr1") {
      mleObj$hessian <- phessraynormlike_mbc92(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- praynormlike_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradraynormlike_mbc92(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
