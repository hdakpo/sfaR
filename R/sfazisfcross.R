...
#' Zero inefficiency stochastic frontier estimation using cross-sectional data
#'
#' @description
#' \code{\link{sfazisfcross}} is a symbolic formula-based function for the
...
#'
#' @aliases sfazisfcross bread.sfazisfcross estfun.sfazisfcross 
#' print.sfazisfcross
#'
...
#' @param sigmavType Character string. Default = \code{'common'}. Nature of the 
...
#' @param x an object of class sfazisfcross (returned by the function 
#' \code{\link{sfazisfcross}}).
#' @param ... additional arguments of frontier are passed to sfazisfcross; 
...
#' 
#' \code{sfazisfcross} allows for the maximization of weighted log-likelihood.
...
#' objects of class \code{'sfazisfcross'} including methods to the generic 
#' functions \code{\link[=print.sfazisfcross]{print}},
#' \code{\link[=summary.sfazisfcross]{summary}}, 
#' \code{\link[=coef.sfazisfcross]{coef}}, 
#' \code{\link[=fitted.sfazisfcross]{fitted}}, 
#' \code{\link[=logLik.sfazisfcross]{logLik}}, 
#' \code{\link[=residuals.sfazisfcross]{residuals}}, 
#' \code{\link[=vcov.sfazisfcross]{vcov}}, 
#' \code{\link[=efficiencies.sfazisfcross]{efficiencies}}, 
#' \code{\link[=ic.sfazisfcross]{ic}}, 
#' \code{\link[=marginal.sfazisfcross]{marginal}},
#' \code{\link[=estfun.sfazisfcross]{estfun}} and 
#' \code{\link[=bread.sfazisfcross]{bread}} (from the \CRANpkg{sandwich} 
#' package), [lmtest::coeftest()] (from the \CRANpkg{lmtest} package).
#' 
#' @return \code{\link{sfazisfcross}} returns a list of class 
#' \code{'sfazisfcross'} containing the following elements:
#'
...
#'
#' @seealso \code{\link[=print.sfazisfcross]{print}} for printing 
#' \code{sfazisfcross} object.
#' 
#' \code{\link[=summary.sfazisfcross]{summary}} for creating and printing
#' summary results.
#'
#' \code{\link[=coef.sfazisfcross]{coef}} for extracting coefficients of the
#' estimation.
#'
#' \code{\link[=efficiencies.sfazisfcross]{efficiencies}} for computing
#' (in-)efficiency estimates.
#'
#' \code{\link[=fitted.sfazisfcross]{fitted}} for extracting the fitted frontier
#' values.
#'
#' \code{\link[=ic.sfazisfcross]{ic}} for extracting information criteria.
#'
#' \code{\link[=logLik.sfazisfcross]{logLik}} for extracting log-likelihood
#' value(s) of the estimation.
#'
#' \code{\link[=marginal.sfazisfcross]{marginal}} for computing marginal effects 
#' of inefficiency drivers.
#'
#' \code{\link[=residuals.sfazisfcross]{residuals}} for extracting residuals of 
#' the estimation.
#'
#' \code{\link[=vcov.sfazisfcross]{vcov}} for computing the variance-covariance
#' matrix of the coefficients.
#' 
#' \code{\link[=bread.sfazisfcross]{bread}} for bread for sandwich estimator.
#' 
#' \code{\link[=estfun.sfazisfcross]{estfun}} for gradient extraction for each 
#' observation.
#'
...
#' 
#' cb_zisf_h <- sfazisfcross(formula = YIT ~ X1 + X2 + X3 + X4, 
...
sfazisfcross <- function(formula, muhet, uhet, vhet, thet, logDepVar = TRUE, data,
  ...
  formula <- formDist_sfazisfcross(udist = udist, formula = formula, muhet = muhet,
  ...
  if (linkF == "logit") {
    if (sigmavType == "common") {
      mleList <- tryCatch(switch(udist, hnormal = do.call(zisfhalfnormAlgOpt_logit,
        FunArgs), exponential = do.call(zisfexponormAlgOpt_logit, FunArgs),
        tnormal = do.call(zisftruncnormAlgOpt_logit, FunArgs), rayleigh = do.call(zisfraynormAlgOpt_logit,
          FunArgs), gamma = do.call(zisfgammanormAlgOpt_logit, FunArgs),
        uniform = do.call(zisfuninormAlgOpt_logit, FunArgs), lognormal = do.call(zisflognormAlgOpt_logit,
          FunArgs), weibull = do.call(zisfweibullnormAlgOpt_logit, FunArgs),
        genexponential = do.call(zisfgenexponormAlgOpt_logit, FunArgs), tslaplace = do.call(zisftslnormAlgOpt_logit,
          FunArgs)), error = function(e) print(e))
    } else {
      if (sigmavType == "different") {
        mleList <- tryCatch(switch(udist, hnormal = do.call(mnsfhalfnormAlgOpt_logit,
          FunArgs), exponential = do.call(mnsfexponormAlgOpt_logit, FunArgs),
          tnormal = do.call(mnsftruncnormAlgOpt_logit, FunArgs), rayleigh = do.call(mnsfraynormAlgOpt_logit,
          FunArgs), gamma = do.call(mnsfgammanormAlgOpt_logit, FunArgs),
          uniform = do.call(mnsfuninormAlgOpt_logit, FunArgs), lognormal = do.call(mnsflognormAlgOpt_logit,
          FunArgs), weibull = do.call(mnsfweibullnormAlgOpt_logit, FunArgs),
          genexponential = do.call(mnsfgenexponormAlgOpt_logit, FunArgs),
          tslaplace = do.call(mnsftslnormAlgOpt_logit, FunArgs)), error = function(e) print(e))
      }
    }
  } else {
    if (linkF == "probit") {
      if (sigmavType == "common") {
        mleList <- tryCatch(switch(udist, hnormal = do.call(zisfhalfnormAlgOpt_probit,
          FunArgs), exponential = do.call(zisfexponormAlgOpt_probit, FunArgs),
          tnormal = do.call(zisftruncnormAlgOpt_probit, FunArgs), rayleigh = do.call(zisfraynormAlgOpt_probit,
          FunArgs), gamma = do.call(zisfgammanormAlgOpt_probit, FunArgs),
          uniform = do.call(zisfuninormAlgOpt_probit, FunArgs), lognormal = do.call(zisflognormAlgOpt_probit,
          FunArgs), weibull = do.call(zisfweibullnormAlgOpt_probit, FunArgs),
          genexponential = do.call(zisfgenexponormAlgOpt_probit, FunArgs),
          tslaplace = do.call(zisftslnormAlgOpt_probit, FunArgs)), error = function(e) print(e))
      } else {
        if (sigmavType == "different") {
          mleList <- tryCatch(switch(udist, hnormal = do.call(mnsfhalfnormAlgOpt_probit,
          FunArgs), exponential = do.call(mnsfexponormAlgOpt_probit, FunArgs),
          tnormal = do.call(mnsftruncnormAlgOpt_probit, FunArgs), rayleigh = do.call(mnsfraynormAlgOpt_probit,
            FunArgs), gamma = do.call(mnsfgammanormAlgOpt_probit, FunArgs),
          uniform = do.call(mnsfuninormAlgOpt_probit, FunArgs), lognormal = do.call(mnsflognormAlgOpt_probit,
            FunArgs), weibull = do.call(mnsfweibullnormAlgOpt_probit, FunArgs),
          genexponential = do.call(mnsfgenexponormAlgOpt_probit, FunArgs),
          tslaplace = do.call(mnsftslnormAlgOpt_probit, FunArgs)), error = function(e) print(e))
        }
      }
    } else {
      if (linkF == "cauchit") {
        if (sigmavType == "common") {
          ...
          FunArgs), exponential = do.call(zisfexponormAlgOpt_cauchit, FunArgs),
          tnormal = do.call(zisftruncnormAlgOpt_cauchit, FunArgs), rayleigh = do.call(zisfraynormAlgOpt_cauchit,
            FunArgs), gamma = do.call(zisfgammanormAlgOpt_cauchit, FunArgs),
          uniform = do.call(zisfuninormAlgOpt_cauchit, FunArgs), lognormal = do.call(zisflognormAlgOpt_cauchit,
            FunArgs), weibull = do.call(zisfweibullnormAlgOpt_cauchit,
            FunArgs), genexponential = do.call(zisfgenexponormAlgOpt_cauchit,
            FunArgs), tslaplace = do.call(zisftslnormAlgOpt_cauchit, FunArgs)),
          ...
          mleList <- tryCatch(switch(udist, hnormal = do.call(mnsfhalfnormAlgOpt_cauchit,
            ...
            FunArgs), tnormal = do.call(mnsftruncnormAlgOpt_cauchit, FunArgs),
            rayleigh = do.call(mnsfraynormAlgOpt_cauchit, FunArgs), gamma = do.call(mnsfgammanormAlgOpt_cauchit,
            FunArgs), uniform = do.call(mnsfuninormAlgOpt_cauchit, FunArgs),
            lognormal = do.call(mnsflognormAlgOpt_cauchit, FunArgs), weibull = do.call(mnsfweibullnormAlgOpt_cauchit,
        ...
        if (linkF == "cloglog") {
          ...
          mleList <- tryCatch(switch(udist, hnormal = do.call(zisfhalfnormAlgOpt_cloglog,
            ...
            FunArgs), tnormal = do.call(zisftruncnormAlgOpt_cloglog, FunArgs),
            rayleigh = do.call(zisfraynormAlgOpt_cloglog, FunArgs), gamma = do.call(zisfgammanormAlgOpt_cloglog,
            FunArgs), uniform = do.call(zisfuninormAlgOpt_cloglog, FunArgs),
            lognormal = do.call(zisflognormAlgOpt_cloglog, FunArgs), weibull = do.call(zisfweibullnormAlgOpt_cloglog,
          ...
          if (sigmavType == "different") {
            ...
            FunArgs), rayleigh = do.call(mnsfraynormAlgOpt_cloglog, FunArgs),
            gamma = do.call(mnsfgammanormAlgOpt_cloglog, FunArgs), uniform = do.call(mnsfuninormAlgOpt_cloglog,
  ...
  # quick renaming -------
  if (udist %in% c("tnormal", "lognormal")) {
    names(mleList$startVal) <- fName_mu_sfazisfcross(Xvar = Xvar, muHvar = muHvar,
      uHvar = uHvar, vHvar = vHvar, Zvar = Zvar, sigmavType)
  } else {
    names(mleList$startVal) <- fName_uv_sfazisfcross(Xvar = Xvar, udist = udist,
      uHvar = uHvar, vHvar = vHvar, Zvar = Zvar, sigmavType)
  }
  ...
  rm(mleList)
  class(returnObj) <- "sfazisfcross"
  return(returnObj)
}

# print for sfazisfcross ----------
#' @rdname sfazisfcross
#' @exportS3Method print sfazisfcross
print.sfazisfcross <- function(x, ...) {
  cat("Call:\n")
...
#' @rdname sfazisfcross
#' @exportS3Method sandwich::bread sfazisfcross
bread.sfazisfcross <- function(x, ...) {
  if (x$hessianType == "Analytic/Numeric Hessian") {
    ...
    if (x$sigmavType == "common") {
      if (x$udist == "hnormal") {
        hessAnalytical <- eval(parse(text = paste0("chesszisfhalfnormlike_",
        ...
        if (x$udist == "exponential") {
          hessAnalytical <- eval(parse(text = paste0("chesszisfexponormlike_",
          x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          ...
          if (x$udist == "tnormal") {
          hessAnalytical <- eval(parse(text = paste0("chesszisftruncnormlike_",
            x$linkF, "(parm = x$mlParam,
          ...
          if (x$udist == "rayleigh") {
            hessAnalytical <- eval(parse(text = paste0("chesszisfraynormlike_",
            x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
            ...
            if (x$udist == "uniform") {
            hessAnalytical <- eval(parse(text = paste0("chesszisfuninormlike_",
              x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
            ...
            if (x$udist == "genexponential") {
              hessAnalytical <- eval(parse(text = paste0("chesszisfgenexponormlike_",
              x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
              ...
              hessAnalytical <- eval(parse(text = paste0("chesszisftslnormlike_",
                x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
              ...
              if (x$udist == "gamma") {
                hessAnalytical <- eval(parse(text = paste0("calculus::jacobian(function(parm) 
                      colSums(cgradzisfgammanormlike_",
                x$linkF, "(parm,
                nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
                ...
                if (x$udist == "weibull") {
                hessAnalytical <- eval(parse(text = paste0("calculus::jacobian(function(parm) 
                      colSums(cgradzisfweibullnormlike_",
                  x$linkF, "(parm,
                nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
                nvZVvar = ncol(vHvar), uHvar = uHvar,
                vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      if (x$sigmavType == "different") {
        if (x$udist == "hnormal") {
          hessAnalytical <- eval(parse(text = paste0("chessmnsfhalfnormlike_",
          x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          ...
          if (x$udist == "exponential") {
          hessAnalytical <- eval(parse(text = paste0("chessmnsfexponormlike_",
            x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          ...
          if (x$udist == "tnormal") {
            hessAnalytical <- eval(parse(text = paste0("chessmnsftruncnormlike_",
            x$linkF, "(parm = x$mlParam,
            nXvar = ncol(Xvar), nmuZUvar = ncol(muHvar),
            nuZUvar = ncol(uHvar), nvZVvar = ncol(vHvar),
            muHvar = muHvar, uHvar = uHvar, vHvar = vHvar,
            ...
            if (x$udist == "rayleigh") {
            hessAnalytical <- eval(parse(text = paste0("chessmnsfraynormlike_",
              x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
            ...
            if (x$udist == "uniform") {
              hessAnalytical <- eval(parse(text = paste0("chessmnsfuninormlike_",
              x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
              ...
              if (x$udist == "genexponential") {
              hessAnalytical <- eval(parse(text = paste0("chessmnsfgenexponormlike_",
                x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
              ...
              if (x$udist == "tslaplace") {
                hessAnalytical <- eval(parse(text = paste0("chessmnsftslnormlike_",
                x$linkF, "(parm = x$mlParam,
          nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
          nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, Zvar = Zvar, nZHvar = ncol(Zvar),
          wHvar = x$dataTable$weights, S = x$S)")))
                ...
                if (x$udist == "gamma") {
                hessAnalytical <- eval(parse(text = paste0("calculus::jacobian(function(parm) 
                      colSums(cgradmnsfgammanormlike_",
                  x$linkF, "(parm,
                nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
                nvZVvar = ncol(vHvar), uHvar = uHvar,
                vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
                ...
                if (x$udist == "weibull") {
                  hessAnalytical <- eval(parse(text = paste0("calculus::jacobian(function(parm) 
                      colSums(cgradmnsfweibullnormlike_",
                  x$linkF, "(parm,
                nXvar = ncol(Xvar), nuZUvar = ncol(uHvar),
                nvZVvar = ncol(vHvar), uHvar = uHvar,
                vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
...
#' @rdname sfazisfcross
#' @exportS3Method sandwich::estfun sfazisfcross
estfun.sfazisfcross <- function(x, ...) {
  return(x$gradL_OBS)
}
