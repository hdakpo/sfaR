...
#' @noRd
cGZISFhalfnormlike4C <- function(parm, nXvar, nuZUvar, nvZVvar,
  ...
  beta4 <- parm[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 *
    nXvar + 3 * nuZUvar + 3 * nvZVvar)]
  phi4 <- parm[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 *
    nXvar + 3 * nuZUvar + 4 * nvZVvar)]
  theta1 <- parm[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 1):(4 *
    nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar)]
  theta2 <- parm[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar +
    1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)]
  theta3 <- parm[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 *
    nZHvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    3 * nZHvar)]
  ...
  ifelse(Probc1 * Pi1 + Probc2 * Pi2 + Probc3 * Pi3 + Probc4 *
    Pi4 <= 0, return(NA), return(wHvar * log(Probc1 * Pi1 +
    Probc2 * Pi2 + Probc3 * Pi3 + Probc4 * Pi4)))
}
...
#' @noRd
csGZISFfhalfnorm4C <- function(olsObj, epsiRes, nXvar, nuZUvar,
  ...
  StartVal <- c(Esti[1:(nXvar + 1)], if (nuZUvar > 1) rep(0,
    ...
    0.98 * Esti[1:nXvar], Esti[nXvar + 2], if (nvZVvar >
...
#' @param nZHvar number of separating variables
#' @noRd
cgradGZISFhalfnormlike4C <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx3 <- (prC * depsi4/ewv4_h + (2 * sigx2_1 + 2 * sigx2_2 +
    2 * sigx2_3)/wzdeno)
  ...
  sigx13 <- (0.5 * (S^2 * depsi4 * (epsilon4)^2/ewv4_h^2) -
    0.5 * depsi4)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = 2 * (S *
    ...
    sweep(vHvar, MARGIN = 1, STATS = 2 * (ewv1 * ewz1 * sigx11_1/sigx9_1),
      ...
      prC * depsi4 * (epsilon4)/(sigx3 * ewv4_h^3), FUN = "*"),
    sweep(vHvar, MARGIN = 1, STATS = sigx13 * prC/(sigx3 *
      ewv4_h), FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx12_1 *
      ewz1/(sigx3 * wzdeno), FUN = "*"), sweep(Zvar, MARGIN = 1,
      STATS = sigx12_2 * ewz2/(sigx3 * wzdeno), FUN = "*"),
    sweep(Zvar, MARGIN = 1, STATS = sigx12_3 * ewz3/(sigx3 *
      wzdeno), FUN = "*"))
...
#' @param nZHvar number of separating variables
#' @noRd
chessGZISFhalfnormlike4C <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx18_3 <- (2 * (sigx12_3/(sigx3 * wzdeno)^2) + 2/(sigx3 *
    wzdeno^2))
  ...
  hessll <- matrix(nrow = 4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    3 * nZHvar, ncol = 4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    3 * nZHvar)
  ...
  hessll[1:nXvar, (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    ...
    MARGIN = 1, STATS = -wHvar * (4 * (S^2 * sigx1_1 * sigx1_3 *
  ...
  hessll[1:nXvar, (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    ...
    MARGIN = 1, STATS = -wHvar * (4 * (S * sigx1_1 * ewu3 *
      ewz1 * ewz3 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
  ...
  hessll[1:nXvar, (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar +
    ...
    MARGIN = 1, STATS = -wHvar * (4 * (S * sigx1_1 * ewv3 *
      ewz1 * ewz3 * sigx11_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
  ...
  hessll[1:nXvar, (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    ...
    MARGIN = 1, STATS = -wHvar * (2 * (S^3 * prC * sigx1_1 *
      depsi4 * ewv4_h^3 * ewz1 * (epsilon4)/((sigx3 * ewv4_h^3)^2 *
      wzdeno * (sigma_sq1) * sqrt(sigma_sq1)))), FUN = "*"),
    Xvar)
  hessll[1:nXvar, (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S * sigx13 * prC *
      sigx1_1 * ewv4_h * ewz1/((sigx3 * ewv4_h)^2 * wzdeno *
      (sigma_sq1) * sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[1:nXvar, (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * sigx18_1 * sigx1_1 *
      ewz1/sigx21_1, FUN = "*"), Zvar)
  hessll[1:nXvar, (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    nZHvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar *
    (S * sigx27_2 * sigx1_1 * ewz1 * ewz2/sigx21_1), FUN = "*"),
    Zvar)
  hessll[1:nXvar, (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar *
    (S * sigx18_3 * sigx1_1 * ewz1 * ewz3/sigx21_1), FUN = "*"),
    Zvar)
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = -wHvar * (4 * (S * sigx1_3 * ewu1 *
      (sigma_sq3) * ewz1 * ewz3 * sigx8_1 * sqrt(sigma_sq3)/(sigx4_3^2 *
      sqrt(sigma_sq1)))), FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (4 * (ewu1 * ewu3 * ewz1 *
      ewz3 * sigx8_1 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq1)))), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (4 * (ewu1 * ewv3 * ewz1 *
      ewz3 * sigx11_3 * sigx8_1 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S^2 * prC * depsi4 *
      ewu1 * ewv4_h^3 * ewz1 * sigx8_1 * (epsilon4)/sigx26_1)),
    FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (sigx13 * prC * ewu1 *
      ewv4_h * ewz1 * sigx8_1/sigx19_1)), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    sigx18_1 * ewu1 * ewz1 * sigx8_1/sqrt(sigma_sq1), FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx27_2 * ewu1 * ewz1 *
      ewz2 * sigx8_1/sqrt(sigma_sq1)), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx18_3 * ewu1 * ewz1 *
      ewz3 * sigx8_1/sqrt(sigma_sq1)), FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
      2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (4 * (S * sigx1_3 * (sigma_sq3) *
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = -wHvar * (4 * (ewu3 * ewv1 * ewz1 *
      ewz3 * sigx11_1 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq1)))), FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
      3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (4 * (ewv1 * ewv3 * ewz1 *
      ewz3 * sigx11_1 * sigx11_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
      3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S^2 * prC * depsi4 *
      ewv1 * ewv4_h^3 * ewz1 * sigx11_1 * (epsilon4)/sigx26_1)),
    FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
      3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (sigx13 * prC * ewv1 *
      ewv4_h * ewz1 * sigx11_1/sigx19_1)), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar +
      3 * nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * sigx18_1 * ewv1 * ewz1 *
      sigx11_1/sqrt(sigma_sq1), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 *
      nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx27_2 * ewv1 * ewz1 *
      ewz2 * sigx11_1/sqrt(sigma_sq1)), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar +
      1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx18_3 * ewv1 * ewz1 *
      ewz3 * sigx11_1/sqrt(sigma_sq1)), FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    ...
    nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (4 * (S^2 * sigx1_2 * sigx1_3 *
      (sigma_sq3) * ewz2 * ewz3 * sqrt(sigma_sq3)/(sigx4_3^2 *
      (sigma_sq2) * sqrt(sigma_sq2)))), FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 *
    nXvar + 3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (4 * (S * sigx1_2 * ewu3 *
      ewz2 * ewz3 * sigx8_3 * sqrt(sigma_sq3)/sigx28_2)),
    FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 *
    nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (4 * (S * sigx1_2 * ewv3 *
      ewz2 * ewz3 * sigx11_3 * sqrt(sigma_sq3)/sigx28_2)),
    FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 *
    nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S^3 * prC * sigx1_2 *
      depsi4 * ewv4_h^3 * ewz2 * (epsilon4)/((sigx3 * ewv4_h^3)^2 *
      wzdeno * (sigma_sq2) * sqrt(sigma_sq2)))), FUN = "*"),
    Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 *
    nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S * sigx13 * prC *
      sigx1_2 * ewv4_h * ewz2/((sigx3 * ewv4_h)^2 * wzdeno *
      (sigma_sq2) * sqrt(sigma_sq2)))), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 1):(4 *
    nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (S * sigx27_1 * sigx1_2 *
      ewz1 * ewz2/sigx21_2), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar +
    1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * sigx18_2 * sigx1_2 *
      ewz2/sigx21_2, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 *
    nZHvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar *
    (S * sigx18_3 * sigx1_2 * ewz2 * ewz3/sigx21_2), FUN = "*"),
    Zvar)
  ...
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    sigx18_2 * ewu2 * ewz2 * sigx8_2/sqrt(sigma_sq2), FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    3 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar *
    (sigx18_3 * ewu2 * ewz2 * ewz3 * sigx8_2/sqrt(sigma_sq2)),
    FUN = "*"), Zvar)
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = -wHvar * (4 * (ewu3 * ewv2 * ewz2 *
      ewz3 * sigx11_2 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq2)))), FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (4 * (ewv2 * ewv3 * ewz2 *
      ewz3 * sigx11_2 * sigx11_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq2)))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S^2 * prC * depsi4 *
      ewv2 * ewv4_h^3 * ewz2 * sigx11_2 * (epsilon4)/sigx26_2)),
    FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = -wHvar * (2 * (sigx13 * prC * ewv2 *
      ewv4_h * ewz2 * sigx11_2/sigx19_2)), FUN = "*"),
    vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = wHvar * sigx18_2 * ewv2 * ewz2 *
      sigx11_2/sqrt(sigma_sq2), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx18_3 * ewv2 * ewz2 *
      ewz3 * sigx11_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (2 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S^2 * ((depsi3 * sigx14_3 +
      S * dmusig3 * sigx15_3 * ewu3 * (epsilon3)/ssq3)/sigx4_3 -
      2 * (sigx1_3^2 * ewz3/sigx4_3^2)) * ewz3), FUN = "*"),
    Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * ((sigx6_3 * dmusig3 *
      depsi3 + (S * ((0.5 * sigx14_3 - 0.5 * pmusig3) *
      depsi3/(sigma_sq3) - S * sigx6_3 * dmusig3 * sigx15_3 *
      (epsilon3)) * (epsilon3) - 0.5 * (sigx1_3/(sigma_sq3)))/(sigma_sq3))/sigx9_3 -
      2 * (sigx1_3 * ewz3 * sigx8_3/(sigx9_3^2 * (sigma_sq3)))) *
      ewu3 * ewz3), FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((S * ((0.5 * sigx14_3 -
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    ...
    MARGIN = 1, STATS = -wHvar * (2 * (S^3 * prC * sigx1_3 *
      depsi4 * ewv4_h^3 * ewz3 * (epsilon4)/((sigx3 * ewv4_h^3)^2 *
      wzdeno * sigx21_3))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S * sigx13 * prC *
      sigx1_3 * ewv4_h * ewz3/((sigx3 * ewv4_h)^2 * wzdeno *
      sigx21_3))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -wHvar *
    (S * sigx27_1 * sigx1_3 * ewz1 * ewz3/(sigx21_3)), FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -wHvar * (S * sigx27_2 * sigx1_3 * ewz2 * ewz3/(sigx21_3)),
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * ((2 - 2 * (ewz3/wzdeno))/(sigx3 *
      wzdeno) - 2 * (sigx12_3 * ewz3/(sigx3 * wzdeno)^2)) *
      sigx1_3 * ewz3/(sigx21_3), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((ewu3 * (S * (0.5 *
      sigx24_3 - (0.5 * (S^2 * sigx6_3 * depsi3 * (epsilon3)^2/(sigma_sq3)^2) -
      (((0.5 * (ewu3/(sigma_sq3)) + 1 - 0.5 * (0.5 * prU3 +
        ewu3/(sigma_sq3))) * prU3 * ewv3/sigmastar3 +
  ...
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    ...
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((S * (((((0.5 * (prU3 *
      ewv3) - S^2 * (0.5 * pvu3 + sigmastar3) * sigx6_3 *
      ewu3 * (epsilon3)^2)/(sigma_sq3) + 0.5 * ((ewu3/(sigma_sq3) -
  ...
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    ...
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S^2 * prC * depsi4 *
      ewu3 * ewv4_h^3 * ewz3 * sigx8_3 * (epsilon4)/sigx25_3)),
    FUN = "*"), Xvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (sigx13 * prC * ewu3 *
      ewv4_h * ewz3 * sigx8_3/sigx19_3)), FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -wHvar *
    (sigx27_1 * ewu3 * ewz1 * ewz3 * sigx8_3/sqrt(sigma_sq3)),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (sigx27_2 * ewu3 * ewz2 *
      ewz3 * sigx8_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((2 - 2 * (ewz3/wzdeno))/(sigx3 *
      wzdeno) - 2 * (sigx12_3 * ewz3/(sigx3 * wzdeno)^2)) *
      ewu3 * ewz3 * sigx8_3/sqrt(sigma_sq3), FUN = "*"),
    Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((S * ((((0.5 * (ewv3/(sigma_sq3)) -
      ...
      (0.5 * sigx22_3 + 2 * (ewz3 * sigx11_3)) * ewv3 *
        sigx11_3/sigx9_3^2) * ewv3 * ewz3), FUN = "*"),
    vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (S^2 * prC * depsi4 *
      ewv3 * ewv4_h^3 * ewz3 * sigx11_3 * (epsilon4)/sigx25_3)),
    FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (2 * (sigx13 * prC * ewv3 *
      ewv4_h * ewz3 * sigx11_3/sigx19_3)), FUN = "*"),
    vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (sigx27_1 * ewv3 * ewz1 * ewz3 * sigx11_3/sqrt(sigma_sq3)),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx27_2 * ewv3 * ewz2 *
      ewz3 * sigx11_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((2 - 2 * (ewz3/wzdeno))/(sigx3 *
      wzdeno) - 2 * (sigx12_3 * ewz3/(sigx3 * wzdeno)^2)) *
      ewv3 * ewz3 * sigx11_3/sqrt(sigma_sq3), FUN = "*"),
    Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S^2 * ((S^2 * (epsilon4)^2/ewv4_h^2 -
      1)/(sigx3 * ewv4_h^3) - S^2 * prC * depsi4 * (epsilon4)^2/(sigx3 *
      ewv4_h^3)^2) * prC * depsi4, FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S^2 * ((0.5 * (S^2 * (epsilon4)^2/ewv4_h^2 -
      2) - 0.5)/(sigx3 * ewv4_h) - sigx13 * prC/(sigx3 *
      ewv4_h)^2) * prC * depsi4 * (epsilon4)/ewv4_h^2,
    FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    ...
    (S^2 * (wzdeno * sigx12_1/(sigx3 * wzdeno)^2 + 1/(sigx3 *
      wzdeno)) * prC * depsi4 * ewz1 * (epsilon4)/ewv4_h^3),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    ...
    STATS = -wHvar * (S^2 * (wzdeno * sigx12_2/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * prC * depsi4 * ewz2 * (epsilon4)/ewv4_h^3),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -wHvar * (S^2 * (wzdeno * sigx12_3/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * prC * depsi4 * ewz3 * (epsilon4)/ewv4_h^3),
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 4 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * prC * (S^2 * (0.5 * (0.5 *
      (S^2 * (epsilon4)^2/ewv4_h^2) - 1) - 0.25) * depsi4 *
      (epsilon4)^2/(sigx3 * ewv4_h^3) - (sigx13 * prC +
      0.5 * (sigx3 * ewv4_h)) * sigx13/(sigx3 * ewv4_h)^2),
    FUN = "*"), vHvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 4 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar +
    ...
    ((wzdeno * sigx12_1/(sigx3 * wzdeno)^2 + 1/(sigx3 * wzdeno)) *
      sigx13 * prC * ewz1/ewv4_h), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 4 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * ((wzdeno * sigx12_2/(sigx3 *
      ...
      ewz2/ewv4_h), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 4 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 2 * nZHvar + 1):(4 * nXvar + 3 * nuZUvar +
    4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * ((wzdeno * sigx12_3/(sigx3 *
      wzdeno)^2 + 1/(sigx3 * wzdeno)) * sigx13 * prC *
      ewz3/ewv4_h), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 4 * nvZVvar + nZHvar), (4 * nXvar + 3 *
    nuZUvar + 4 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar +
  ...
  hessll[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 4 * nvZVvar + nZHvar), (4 * nXvar + 3 *
    nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 * nXvar + 3 *
  ...
  hessll[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 1):(4 * nXvar +
    ...
    3 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * ((sigx12_1/(sigx3 * wzdeno^2) +
      2 * (sigx12_3 * depsi1 * pmusig1/sigx25_1)) * ewz1 *
      ewz3), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar +
    1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar),
    (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar + 1):(4 *
      nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = wHvar * ((1 - ewz2/wzdeno)/(sigx3 *
      wzdeno) - 2 * (depsi2 * ewz2 * pmusig2/sigx25_2)) *
      sigx12_2 * ewz2, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + nZHvar +
    1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar),
    (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar +
      1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * ((sigx12_2/(sigx3 * wzdeno^2) +
      2 * (sigx12_3 * depsi2 * pmusig2/sigx25_2)) * ewz2 *
      ewz3), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar +
    1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 3 * nZHvar),
    (4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 2 * nZHvar +
      1):(4 * nXvar + 3 * nuZUvar + 4 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = wHvar * ((1 - ewz3/wzdeno)/(sigx3 *
      wzdeno) - 2 * (depsi3 * ewz3 * pmusig3/((sigx3 *
      wzdeno)^2 * sqrt(sigma_sq3)))) * sigx12_3 * ewz3,
    FUN = "*"), Zvar)
...
#' @noRd
GZISF4ChnormAlgOpt <- function(start, olsParam, dataTable, S,
  ...
  startLoglik <- sum(cGZISFhalfnormlike4C(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cGZISFhalfnormlike4C(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradGZISFhalfnormlike4C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      maxeval = itermax, stepmax = stepmax, xtol = tol,
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cGZISFhalfnormlike4C,
    grad = cgradGZISFhalfnormlike4C, hess = chessGZISFhalfnormlike4C,
    ...
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cGZISFhalfnormlike4C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradGZISFhalfnormlike4C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      ...
      fn = function(parm) -sum(cGZISFhalfnormlike4C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradGZISFhalfnormlike4C(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessGZISFhalfnormlike4C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      ...
      gr = function(parm) -colSums(cgradGZISFhalfnormlike4C(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessGZISFhalfnormlike4C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cGZISFhalfnormlike4C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradGZISFhalfnormlike4C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessGZISFhalfnormlike4C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradGZISFhalfnormlike4C(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessGZISFhalfnormlike4C(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
      mleObj$hessian <- chessGZISFhalfnormlike4C(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  }
  mleObj$logL_OBS <- cGZISFhalfnormlike4C(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradGZISFhalfnormlike4C(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
#' post. prob. and efficiencies for gzisf 4 classes halfnormal-normal distribution
...
cGZISF4Chalfnormeff <- function(object, level) {
  ...
  beta4 <- object$mlParam[(3 * object$nXvar + 3 * object$nuZUvar +
    ...
    3 * object$nvZVvar)]
  phi4 <- object$mlParam[(4 * object$nXvar + 3 * object$nuZUvar +
    3 * object$nvZVvar + 1):(4 * object$nXvar + 3 * object$nuZUvar +
    4 * object$nvZVvar)]
  theta1 <- object$mlParam[(4 * object$nXvar + 3 * object$nuZUvar +
    4 * object$nvZVvar + 1):(4 * object$nXvar + 3 * object$nuZUvar +
    4 * object$nvZVvar + object$nZHvar)]
  theta2 <- object$mlParam[(4 * object$nXvar + 3 * object$nuZUvar +
    4 * object$nvZVvar + object$nZHvar + 1):(4 * object$nXvar +
    3 * object$nuZUvar + 4 * object$nvZVvar + 2 * object$nZHvar)]
  theta3 <- object$mlParam[(4 * object$nXvar + 3 * object$nuZUvar +
    4 * object$nvZVvar + 2 * object$nZHvar + 1):(4 * object$nXvar +
    3 * object$nuZUvar + 4 * object$nvZVvar + 3 * object$nZHvar)]
...
cmargGZISF4Chalfnorm_Eu <- function(object) {
  ...
  colnames(margEff_c3) <- paste0("Eu_", colnames(uHvar)[-1],
    "_c3")
  margEff_c4 <- matrix(0, nrow = object$Nobs, ncol = object$nuZUvar -
    1)
  ...
  colnames(margEff_c) <- paste0("Eu_", colnames(uHvar)[-1],
    "_c")
  margEff <- margEff <- data.frame(margEff_c, margEff_c1, margEff_c2,
