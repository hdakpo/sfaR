...
czisfgenexponormlike_logit <- function(parm, nXvar, nuZUvar,
...
czisfgenexponormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
czisfgenexponormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
czisfgenexponormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## logit specification class membership
cmnsfgenexponormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
...
## cauchit specification class membership
cmnsfgenexponormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## probit specification class membership
cmnsfgenexponormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## cloglog specification class membership
cmnsfgenexponormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## logit specification class membership
cgradzisfgenexponormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## cauchit specification class membership
cgradzisfgenexponormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx3 <- (ewz2 * dwsr/ewvsr + 2 * (ewz1 * (eA * pa - eB *
    pb)/ewusr))
  sigx4 <- (0.5 * (S * (epsilon)/ewusr) + 2 * (ewu * ewv/(2 *
    ewu)^2))
  sigx5 <- (0.5 * (da * ewvsr/ewusr) - sigx4 * pa)
  sigx7 <- (sigx5 * eA - ((db * ewvsr/ewusr - eC * pb) * eB +
    0.5 * (eA * pa - eB * pb)))
  sigx9 <- (0.5 * (ewvsr/ewusr) - 0.5 * (S * (epsilon)/ewvsr))
  sigx10 <- (ewv * pa/(2 * ewu) - sigx9 * da)
  sigx11 <- (2 * (ewv * pb/ewu) - db * (ewvsr/ewusr - 0.5 *
  ...
  depsisr <- (S^2 * dwsr * (epsilon)^2/ewvsr^2)
  sigx12 <- (ewz2 * (0.5 * depsisr - 0.5 * dwsr)/ewvsr + 2 *
    sigx8)
  sigx13 <- (2 * ((eA * pa - eB * pb)/ewusr) - dwsr/ewvsr)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx2/sigx3,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx7 *
    ewz1/(sigx3 * ewusr)), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = sigx12/sigx3, FUN = "*"), sweep(Zvar, MARGIN = 1,
    STATS = sigx13/(pi * sigx3 * ((Wz)^2 + 1)), FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

## probit specification class membership
cgradzisfgenexponormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx1 <- ((da/ewvsr - pa/ewusr) * eA - (db/ewvsr - 2 * (pb/ewusr)) *
    eB)
  sigx2 <- (2 * (sigx1 * pwZ/ewusr) + S * (1 - pwZ) * dwsr *
    (epsilon)/ewvsr^3)
  sigx3 <- ((1 - pwZ) * dwsr/ewvsr + 2 * ((eA * pa - eB * pb) *
    pwZ/ewusr))
  sigx4 <- (0.5 * (S * (epsilon)/ewusr) + 2 * (ewu * ewv/(2 *
    ewu)^2))
  sigx5 <- (0.5 * (da * ewvsr/ewusr) - sigx4 * pa)
  ...
  depsisr <- (S^2 * dwsr * (epsilon)^2/ewvsr^2)
  sigx12 <- ((0.5 * depsisr - 0.5 * dwsr) * (1 - pwZ)/ewvsr +
    2 * sigx8)
  sigx13 <- (2 * ((eA * pa - eB * pb)/ewusr) - dwsr/ewvsr)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx2/sigx3,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx7 *
    pwZ/(sigx3 * ewusr)), FUN = "*"), sweep(vHvar, MARGIN = 1,
    STATS = sigx12/sigx3, FUN = "*"), sweep(Zvar, MARGIN = 1,
...
## cloglog specification class membership
cgradzisfgenexponormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx1 <- ((da/ewvsr - pa/ewusr) * eA - (db/ewvsr - 2 * (pb/ewusr)) *
    eB)
  sigx2 <- (2 * (sigx1 * (1 - prZ)/ewusr) + S * dwsr * prZ *
    (epsilon)/ewvsr^3)
  sigx3 <- (2 * ((1 - prZ) * (eA * pa - eB * pb)/ewusr) + dwsr *
    prZ/ewvsr)
  sigx4 <- (0.5 * (S * (epsilon)/ewusr) + 2 * (ewu * ewv/(2 *
    ewu)^2))
  sigx5 <- (0.5 * (da * ewvsr/ewusr) - sigx4 * pa)
  ...
  depsisr <- (S^2 * dwsr * (epsilon)^2/ewvsr^2)
  sigx12 <- ((0.5 * depsisr - 0.5 * dwsr) * prZ/ewvsr + 2 *
    sigx8)
  sigx13 <- (2 * ((eA * pa - eB * pb)/ewusr) - dwsr/ewvsr)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx2/sigx3,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * (sigx7 *
    (1 - prZ)/(sigx3 * ewusr)), FUN = "*"), sweep(vHvar,
    MARGIN = 1, STATS = sigx12/sigx3, FUN = "*"), sweep(Zvar,
    MARGIN = 1, STATS = sigx13 * prZ * ewz/sigx3, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}
...
## logit specification class membership
cgradmnsfgenexponormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## cauchit specification class membership
cgradmnsfgenexponormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx4 <- (0.5 * (S * (epsilon)/ewusr) + 2 * (ewu * ewv1/(2 *
    ewu)^2))
  sigx5 <- (0.5 * (dmustar1 * ewv1_h/ewusr) - sigx4 * pmustar1)
  sigx6 <- (dmustar2 * ewv1_h/ewusr - (2 * (ewv1/ewu) + S *
    (epsilon)/ewusr) * pmustar2)
  sigx7 <- (sigx6 * eusq2 + 0.5 * (eusq1 * pmustar1 - eusq2 *
    pmustar2))
  sigx8 <- (ewz2 * depsi/ewv2_h + 2 * (ewz1 * (eusq1 * pmustar1 -
    eusq2 * pmustar2)/ewusr))
  ...
  sigx13 <- (eusq1 * sigx11 - (2 * (ewv1 * pmustar2/ewu) -
    dmustar2 * sigx12) * eusq2)
  sigx14 <- (2 * ((eusq1 * pmustar1 - eusq2 * pmustar2)/ewusr) -
    depsi/ewv2_h)
  sigx24 <- (0.5 * (S^2 * depsi * (epsilon)^2/ewv2_h^2) - 0.5 *
    depsi)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx9/sigx8,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * ((sigx5 *
    eusq1 - sigx7) * ewz1/(sigx8 * ewusr)), FUN = "*"), sweep(vHvar,
    MARGIN = 1, STATS = 2 * (ewz1 * sigx13/(sigx8 * ewusr)),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = ewz2 * sigx24/(sigx8 *
    ewv2_h), FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx14/(pi *
    sigx8 * ((Wz)^2 + 1)), FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

## probit specification class membership
cgradmnsfgenexponormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx14 <- (2 * ((eusq1 * pmustar1 - eusq2 * pmustar2)/ewusr) -
    depsi/ewv2_h)
  sigx15 <- ((1 - pwZ) * depsi/ewv2_h + 2 * ((eusq1 * pmustar1 -
    eusq2 * pmustar2) * pwZ/ewusr))
  sigx24 <- (0.5 * (S^2 * depsi * (epsilon)^2/ewv2_h^2) - 0.5 *
    depsi)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx9/sigx15,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * ((sigx5 *
    eusq1 - sigx7) * pwZ/(sigx15 * ewusr)), FUN = "*"), sweep(vHvar,
    MARGIN = 1, STATS = 2 * (sigx13 * pwZ/(sigx15 * ewusr)),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx24 *
    (1 - pwZ)/(sigx15 * ewv2_h), FUN = "*"), sweep(Zvar,
    MARGIN = 1, STATS = sigx14 * dwZ/sigx15, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

## cloglog specification class membership
cgradmnsfgenexponormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx7 <- (sigx6 * eusq2 + 0.5 * (eusq1 * pmustar1 - eusq2 *
    pmustar2))
  sigx8 <- (2 * ((1 - prZ) * (eusq1 * pmustar1 - eusq2 * pmustar2)/ewusr) +
    depsi * prZ/ewv2_h)
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx9/sigx8,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 * ((sigx5 *
    eusq1 - sigx7) * (1 - prZ)/(sigx8 * ewusr)), FUN = "*"),
    sweep(vHvar, MARGIN = 1, STATS = 2 * ((1 - prZ) * sigx13/(sigx8 *
      ewusr)), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx24 *
      prZ/(sigx8 * ewv2_h), FUN = "*"), sweep(Zvar, MARGIN = 1,
      STATS = sigx14 * prZ * ewz/sigx8, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}
...
## logit specification class membership
chesszisfgenexponormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## cauchit specification class membership
chesszisfgenexponormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * (ewz2 * dwsr * (S^2 * (epsilon)^2/ewvsr^2 -
      ...
      eA - (((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr)/ewvsr -
      2/ewusr) * db/ewvsr - 2 * ((db/ewvsr - 2 * (pb/ewusr))/ewusr)) *
      eB) * ewz1/ewusr) - sigx2^2/sigx3)/sigx3, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((((0.5 + 0.5 *
      (S * (epsilon)/ewusr) + 2 * (ewu * ewv/(2 * ewu)^2)) *
      ...
      eA - ((((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr)/ewusr -
      eC/ewvsr) * db + (pb - 2 * (db * ewvsr/ewusr - eC *
      pb))/ewusr) * eB + 0.5 * sigx1))/(sigx3 * ewusr) -
      sigx7 * sigx2 * ewusr/(sigx3 * ewusr)^2) * ewz1),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (2 * (((da * (ewv/(2 * ewu) - (sigx9 * (ewvsr/ewusr +
    ...
    eA - ((2 * (ewv/ewu) - ((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr) *
    (ewvsr/ewusr - 0.5 * (S * (epsilon)/ewvsr)) + 0.5)) *
    db/ewvsr - 2 * (sigx11/ewusr)) * eB) * ewz1/ewusr) +
    S * ewz2 * (0.5 * (S^2 * (epsilon)^2/ewvsr^2 - 2) - 0.5) *
      dwsr * (epsilon)/ewvsr^3 - sigx12 * sigx2/sigx3)/sigx3,
    FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * ((2 * (sigx1/ewusr) -
      S * dwsr * (epsilon)/ewvsr^3)/(pi * sigx3 * ((Wz)^2 +
      1)) - pi * ((Wz)^2 + 1) * sigx2 * sigx13/(pi * sigx3 *
      ((Wz)^2 + 1))^2), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    2 * (((((0.5 * (0.5 * (ewvsr * (ewvsr/ewusr + S * (epsilon)/ewvsr)/ewusr) -
    ...
    (2 * ((1 - 8 * (ewu^2/(2 * ewu)^2)) * ewu * ewv/(2 *
      ewu)^2) - 0.25 * (S * (epsilon)/ewusr)) * pa)) *
    eA - (((((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr) *
    ewvsr - S * (epsilon))/ewusr - (0.5 + 2 * (ewv/ewu))) *
    db * ewvsr/ewusr + (0.5 * (S * (epsilon)/ewusr) + 2 *
    (ewv/ewu)) * pb - eC * (db * ewvsr/ewusr - eC * pb)) *
    eB + 0.5 * sigx6))/(sigx3 * ewusr) - sigx7 * (0.5 * (sigx3 *
    ewusr) + 2 * (sigx7 * ewz1))/(sigx3 * ewusr)^2) * ewz1),
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ewz1 * (2 * (((da * ewvsr/(4 *
      (ewu * ewusr)) - 2 * (ewu * pa/(2 * ewu)^2)) * ewv -
      ((0.5 * (sigx9 * (ewvsr/ewusr + S * (epsilon)/ewvsr)) -
        0.25) * da * ewvsr/ewusr + sigx4 * sigx10)) *
      eA - ((2 * ((db * ewvsr/ewusr - pb) * ewv/ewu) -
      (((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr) * (ewvsr/ewusr -
        0.5 * (S * (epsilon)/ewvsr)) - 0.5) * db * ewvsr/ewusr +
        sigx11 * eC)) * eB + 0.5 * (eA * sigx10 - sigx11 *
      eB))) - 2 * (sigx12 * sigx7/sigx3))/(sigx3 * ewusr),
    FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * sigx7 * (2/(pi * sigx3 *
      ((Wz)^2 + 1)) - 2 * (pi * ((Wz)^2 + 1) * ewz1 * sigx13/(pi *
      sigx3 * ((Wz)^2 + 1))^2))/ewusr, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (ewz2 * (S^2 * (0.5 * (0.5 *
      ...
      (S * (epsilon)/ewvsr) + 0.5 * (ewvsr/ewusr) - (2 *
      (ewvsr/ewusr) + S * (epsilon)/ewvsr) * (ewvsr/ewusr -
      0.5 * (S * (epsilon)/ewvsr))^2) * db) * eB) * ewz1/ewusr) -
      sigx12^2/sigx3)/sigx3, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + nvZVvar +
      nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    ((2 * ((eA * sigx10 - sigx11 * eB)/ewusr) - (0.5 * depsisr -
      0.5 * dwsr)/ewvsr)/(pi * sigx3 * ((Wz)^2 + 1)) -
      pi * sigx12 * ((Wz)^2 + 1) * sigx13/(pi * sigx3 *
        ((Wz)^2 + 1))^2), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    nvZVvar + nZHvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * (sigx13 * (2 * ((eA * pa -
      eB * pb)/ewusr) + 2 * (pi * Wz * sigx3) - dwsr/ewvsr)/(pi *
...
## probit specification class membership
chesszisfgenexponormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((1 - pwZ) * dwsr * (S^2 * (epsilon)^2/ewvsr^2 -
      ...
      eA - (((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr)/ewvsr -
      2/ewusr) * db/ewvsr - 2 * ((db/ewvsr - 2 * (pb/ewusr))/ewusr)) *
      eB) * pwZ/ewusr) - sigx2^2/sigx3)/sigx3, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((((0.5 + 0.5 *
      (S * (epsilon)/ewusr) + 2 * (ewu * ewv/(2 * ewu)^2)) *
      ...
      eC/ewvsr) * db + (pb - 2 * (db * ewvsr/ewusr - eC *
      pb))/ewusr) * eB + 0.5 * sigx1))/(sigx3 * ewusr) -
      sigx7 * sigx2 * ewusr/(sigx3 * ewusr)^2) * pwZ),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (2 * (((da * (ewv/(2 * ewu) - (sigx9 * (ewvsr/ewusr +
    ...
    eA - ((2 * (ewv/ewu) - ((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr) *
    (ewvsr/ewusr - 0.5 * (S * (epsilon)/ewvsr)) + 0.5)) *
    db/ewvsr - 2 * (sigx11/ewusr)) * eB) * pwZ/ewusr) + S *
    (0.5 * (S^2 * (epsilon)^2/ewvsr^2 - 2) - 0.5) * (1 -
    pwZ) * dwsr * (epsilon)/ewvsr^3 - sigx12 * sigx2/sigx3)/sigx3,
    FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (2 * (sigx1/ewusr) -
      (sigx2 * sigx13/sigx3 + S * dwsr * (epsilon)/ewvsr^3)) *
      dwZ/sigx3, FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    2 * (((((0.5 * (0.5 * (ewvsr * (ewvsr/ewusr + S * (epsilon)/ewvsr)/ewusr) -
    ...
    (ewv/ewu)) * pb - eC * (db * ewvsr/ewusr - eC * pb)) *
    eB + 0.5 * sigx6))/(sigx3 * ewusr) - sigx7 * (0.5 * (sigx3 *
    ewusr) + 2 * (sigx7 * pwZ))/(sigx3 * ewusr)^2) * pwZ),
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (2 * (((da * ewvsr/(4 * (ewu *
      ewusr)) - 2 * (ewu * pa/(2 * ewu)^2)) * ewv - ((0.5 *
      (sigx9 * (ewvsr/ewusr + S * (epsilon)/ewvsr)) - 0.25) *
      da * ewvsr/ewusr + sigx4 * sigx10)) * eA - ((2 *
      ((db * ewvsr/ewusr - pb) * ewv/ewu) - (((2 * (ewvsr/ewusr) +
      S * (epsilon)/ewvsr) * (ewvsr/ewusr - 0.5 * (S *
      (epsilon)/ewvsr)) - 0.5) * db * ewvsr/ewusr + sigx11 *
      eC)) * eB + 0.5 * (eA * sigx10 - sigx11 * eB))) -
      2 * (sigx7 * sigx12/sigx3)) * pwZ/(sigx3 * ewusr),
    FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * sigx7 * (2 - 2 * (sigx13 *
      pwZ/sigx3)) * dwZ/(sigx3 * ewusr), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((1 - pwZ) * (S^2 * (0.5 *
      (0.5 * (S^2 * (epsilon)^2/ewvsr^2) - 1) - 0.25) *
      dwsr * (epsilon)^2/ewvsr^2 - 0.5 * (0.5 * depsisr -
      0.5 * dwsr))/ewvsr + 2 * ((((sigx10/2 + (pa - sigx9 *
      da)/2) * ewv/ewu - (0.25 * (ewvsr/ewusr) + 0.25 *
      (S * (epsilon)/ewvsr) - sigx9^2 * (ewvsr/ewusr +
      ...
      pwZ/ewusr) - sigx12^2/sigx3)/sigx3, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + nvZVvar +
      nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
    (2 * ((eA * sigx10 - sigx11 * eB)/ewusr) - (sigx12 *
      sigx13/sigx3 + (0.5 * depsisr - 0.5 * dwsr)/ewvsr)) *
    dwZ/sigx3, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    nvZVvar + nZHvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * ((sigx13 * dwZ/sigx3 + Wz) *
      sigx13 * dwZ/sigx3), FUN = "*"), Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
}
...
## cloglog specification class membership
chesszisfgenexponormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll <- matrix(nrow = nXvar + nuZUvar + nvZVvar + nZHvar,
    ncol = nXvar + nuZUvar + nvZVvar + nZHvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * (2 * (((((ewvsr/ewusr + S * (epsilon)/ewvsr)/ewvsr -
      1/ewusr) * da/ewvsr - (da/ewvsr - pa/ewusr)/ewusr) *
      eA - (((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr)/ewvsr -
      2/ewusr) * db/ewvsr - 2 * ((db/ewvsr - 2 * (pb/ewusr))/ewusr)) *
      eB) * (1 - prZ)/ewusr) + dwsr * prZ * (S^2 * (epsilon)^2/ewvsr^2 -
  ...
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((((0.5 + 0.5 *
      ...
      eC/ewvsr) * db + (pb - 2 * (db * ewvsr/ewusr - eC *
      pb))/ewusr) * eB + 0.5 * sigx1))/(sigx3 * ewusr) -
      sigx7 * sigx2 * ewusr/(sigx3 * ewusr)^2) * (1 - prZ)),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (2 * (((da * (ewv/(2 * ewu) - (sigx9 * (ewvsr/ewusr +
    ...
    eA - ((2 * (ewv/ewu) - ((2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr) *
    (ewvsr/ewusr - 0.5 * (S * (epsilon)/ewvsr)) + 0.5)) *
    db/ewvsr - 2 * (sigx11/ewusr)) * eB) * (1 - prZ)/ewusr) +
    S * (0.5 * (S^2 * (epsilon)^2/ewvsr^2 - 2) - 0.5) * dwsr *
      prZ * (epsilon)/ewvsr^3 - sigx12 * sigx2/sigx3)/sigx3,
    FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (2 * (sigx1/ewusr) -
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    ...
    (ewv/ewu)) * pb - eC * (db * ewvsr/ewusr - eC * pb)) *
    eB + 0.5 * sigx6))/(sigx3 * ewusr) - sigx7 * (0.5 * (sigx3 *
    ewusr) + 2 * (sigx7 * (1 - prZ)))/(sigx3 * ewusr)^2) *
    (1 - prZ)), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (1 - prZ) * (2 * (((da *
      ewvsr/(4 * (ewu * ewusr)) - 2 * (ewu * pa/(2 * ewu)^2)) *
      ewv - ((0.5 * (sigx9 * (ewvsr/ewusr + S * (epsilon)/ewvsr)) -
      0.25) * da * ewvsr/ewusr + sigx4 * sigx10)) * eA -
      ((2 * ((db * ewvsr/ewusr - pb) * ewv/ewu) - (((2 *
        (ewvsr/ewusr) + S * (epsilon)/ewvsr) * (ewvsr/ewusr -
        0.5 * (S * (epsilon)/ewvsr)) - 0.5) * db * ewvsr/ewusr +
        sigx11 * eC)) * eB + 0.5 * (eA * sigx10 - sigx11 *
        eB))) - 2 * (sigx7 * sigx12/sigx3))/(sigx3 *
      ewusr), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * sigx7 * (2 - 2 * ((1 - prZ) *
      sigx13/sigx3)) * prZ * ewz/(sigx3 * ewusr), FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (2 * ((((sigx10/2 + (pa -
      sigx9 * da)/2) * ewv/ewu - (0.25 * (ewvsr/ewusr) +
      ...
      (2 * (ewvsr/ewusr) + S * (epsilon)/ewvsr) * (ewvsr/ewusr -
        0.5 * (S * (epsilon)/ewvsr))^2) * db) * eB) *
      (1 - prZ)/ewusr) + prZ * (S^2 * (0.5 * (0.5 * (S^2 *
      (epsilon)^2/ewvsr^2) - 1) - 0.25) * dwsr * (epsilon)^2/ewvsr^2 -
      0.5 * (0.5 * depsisr - 0.5 * dwsr))/ewvsr - sigx12^2/sigx3)/sigx3,
    FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + nvZVvar +
      nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar *
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    nvZVvar + nZHvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = wHvar * (1 - (sigx13 * prZ/sigx3 +
      1) * ewz) * sigx13 * prZ * ewz/sigx3, FUN = "*"),
    Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
...
## logit specification class membership
chessmnsfgenexponormlike_logit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
...
## cauchit specification class membership
chessmnsfgenexponormlike_cauchit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll <- matrix(nrow = nXvar + nuZUvar + 2 * nvZVvar + nZHvar,
    ncol = nXvar + nuZUvar + 2 * nvZVvar + nZHvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * (ewz2 * depsi * (S^2 * (epsilon)^2/ewv2_h^2 -
      1)/ewv2_h^3 + 2 * ((((mustar1/ewv1_h - 1/ewusr) *
      dmustar1/ewv1_h - sigx1/ewusr) * eusq1 - ((mustar2/ewv1_h -
      2/ewusr) * dmustar2/ewv1_h - 2 * (sigx2/ewusr)) *
      eusq2) * ewz1/ewusr) - sigx9^2/sigx8)/sigx8, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((((0.5 + 0.5 *
      (S * (epsilon)/ewusr) + 2 * (ewu * ewv1/(2 * ewu)^2)) *
      pmustar1 - 0.5 * (dmustar1 * ewv1_h/ewusr))/ewusr +
      (0.5 * (mustar1/ewusr) - sigx4/ewv1_h) * dmustar1) *
      eusq1 - (((mustar2/ewusr - (2 * (ewv1/ewu) + S *
      (epsilon)/ewusr)/ewv1_h) * dmustar2 + (pmustar2 -
      2 * sigx6)/ewusr) * eusq2 + 0.5 * (sigx1 * eusq1 -
      sigx2 * eusq2)))/(sigx8 * ewusr) - (sigx5 * eusq1 -
      sigx7) * sigx9 * ewusr/(sigx8 * ewusr)^2) * ewz1),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    2 * (S * (((dmustar1 * (ewv1/(2 * ewu) - (sigx10 * mustar1 +
    0.5))/ewv1_h - sigx11/ewusr) * eusq1 - ((2 * (ewv1/ewu) -
    (mustar2 * sigx12 + 0.5)) * dmustar2/ewv1_h - 2 * ((2 *
    (ewv1 * pmustar2/ewu) - dmustar2 * sigx12)/ewusr)) *
    eusq2)/(sigx8 * ewusr) - sigx9 * ewusr * sigx13/(sigx8 *
    ewusr)^2) * ewz1), FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * ewz2 * (S * (0.5 * (S^2 * (epsilon)^2/ewv2_h^2 -
      2) - 0.5) * depsi * (epsilon)/(sigx8 * ewv2_h^3) -
      sigx24 * sigx9 * ewv2_h/(sigx8 * ewv2_h)^2), FUN = "*"),
    vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * ((2 * ((sigx1 * eusq1 -
      sigx2 * eusq2)/ewusr) - S * depsi * (epsilon)/ewv2_h^3)/(pi *
      sigx8 * ((Wz)^2 + 1)) - pi * ((Wz)^2 + 1) * sigx9 *
      sigx14/(pi * sigx8 * ((Wz)^2 + 1))^2), FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    2 * (((((0.5 * (0.5 * (ewv1_h * mustar1/ewusr) - 0.5) -
    0.5 * sigx4) * dmustar1 * ewv1_h/ewusr - (sigx5 * sigx4 +
    (2 * ((1 - 8 * (ewu^2/(2 * ewu)^2)) * ewu * ewv1/(2 *
      ewu)^2) - 0.25 * (S * (epsilon)/ewusr)) * pmustar1)) *
    eusq1 - ((((mustar2 * ewv1_h - S * (epsilon))/ewusr -
    (0.5 + 2 * (ewv1/ewu))) * dmustar2 * ewv1_h/ewusr + (0.5 *
    (S * (epsilon)/ewusr) + 2 * (ewv1/ewu)) * pmustar2 -
    (2 * (ewv1/ewu) + S * (epsilon)/ewusr) * sigx6) * eusq2 +
    0.5 * (sigx5 * eusq1 - sigx6 * eusq2)))/(sigx8 * ewusr) -
    (sigx5 * eusq1 - sigx7) * (0.5 * (sigx8 * ewusr) + 2 *
      ((sigx5 * eusq1 - sigx7) * ewz1))/(sigx8 * ewusr)^2) *
    ewz1), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((((dmustar1 * ewv1_h/(4 *
      (ewu * ewusr)) - 2 * (ewu * pmustar1/(2 * ewu)^2)) *
      ewv1 - ((0.5 * (sigx10 * mustar1) - 0.25) * dmustar1 *
      ewv1_h/ewusr + sigx4 * sigx11)) * eusq1 - (2 * ((dmustar2 *
      ewv1_h/ewusr - pmustar2) * ewv1/ewu) - ((mustar2 *
      sigx12 - 0.5) * dmustar2 * ewv1_h/ewusr + (2 * (ewv1 *
      pmustar2/ewu) - dmustar2 * sigx12) * (2 * (ewv1/ewu) +
      S * (epsilon)/ewusr))) * eusq2)/(sigx8 * ewusr) -
      (0.5 * (sigx8 * ewusr) + 2 * ((sigx5 * eusq1 - sigx7) *
        ewz1)) * sigx13/(sigx8 * ewusr)^2) * ewz1), FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (2 * ((sigx5 * eusq1 - sigx7) *
      ewz2 * sigx24 * ewz1 * ewv2_h/((sigx8 * ewv2_h)^2 *
      ewusr))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    2 * nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (sigx5 * eusq1 - sigx7) *
      (2/(pi * sigx8 * ((Wz)^2 + 1)) - 2 * (pi * ((Wz)^2 +
        1) * ewz1 * sigx14/(pi * sigx8 * ((Wz)^2 + 1))^2))/ewusr,
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((((sigx11/2 + (pmustar1 -
      sigx10 * dmustar1)/2) * ewv1/ewu - (0.25 * (ewv1_h/ewusr) +
      0.25 * (S * (epsilon)/ewv1_h) - sigx10^2 * mustar1) *
      dmustar1) * eusq1 - ((2 * (2 * (ewv1 * pmustar2/ewu) -
      dmustar2 * sigx12) + 2 * (pmustar2 - dmustar2 * sigx12)) *
      ewv1/ewu - (0.25 * (S * (epsilon)/ewv1_h) + 0.5 *
      (ewv1_h/ewusr) - mustar2 * sigx12^2) * dmustar2) *
      eusq2)/(sigx8 * ewusr) - 2 * (ewz1 * sigx13^2/(sigx8 *
      ewusr)^2)) * ewz1), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (2 * (ewz2 * sigx24 * ewz1 * sigx13 * ewv2_h/((sigx8 *
      ewv2_h)^2 * ewusr))), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
      2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (2/(pi * sigx8 * ((Wz)^2 +
      1)) - 2 * (pi * ((Wz)^2 + 1) * ewz1 * sigx14/(pi *
      sigx8 * ((Wz)^2 + 1))^2)) * sigx13/ewusr, FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * ewz2 * (S^2 * (0.5 * (0.5 * (S^2 * (epsilon)^2/ewv2_h^2) -
      1) - 0.25) * depsi * (epsilon)^2/(sigx8 * ewv2_h^3) -
      (ewz2 * sigx24 + 0.5 * (sigx8 * ewv2_h)) * sigx24/(sigx8 *
        ewv2_h)^2), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (sigx24 * (1/(pi * sigx8 *
      ((Wz)^2 + 1)) + pi * ((Wz)^2 + 1) * ewz2 * sigx14/(pi *
      sigx8 * ((Wz)^2 + 1))^2)/ewv2_h), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar + nZHvar), (nXvar + nuZUvar + 2 * nvZVvar +
    1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * (sigx14 * (2 * ((eusq1 *
      pmustar1 - eusq2 * pmustar2)/ewusr) + 2 * (pi * Wz *
      sigx8) - depsi/ewv2_h)/(pi * sigx8 * ((Wz)^2 + 1))^2),
    FUN = "*"), Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
...
## probit specification class membership
chessmnsfgenexponormlike_probit <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll <- matrix(nrow = nXvar + nuZUvar + 2 * nvZVvar + nZHvar,
    ncol = nXvar + nuZUvar + 2 * nvZVvar + nZHvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * ((1 - pwZ) * depsi * (S^2 * (epsilon)^2/ewv2_h^2 -
      1)/ewv2_h^3 + 2 * ((((mustar1/ewv1_h - 1/ewusr) *
      dmustar1/ewv1_h - sigx1/ewusr) * eusq1 - ((mustar2/ewv1_h -
      2/ewusr) * dmustar2/ewv1_h - 2 * (sigx2/ewusr)) *
      eusq2) * pwZ/ewusr) - sigx9^2/sigx15)/sigx15, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((((0.5 + 0.5 *
      (S * (epsilon)/ewusr) + 2 * (ewu * ewv1/(2 * ewu)^2)) *
      ...
      (epsilon)/ewusr)/ewv1_h) * dmustar2 + (pmustar2 -
      2 * sigx6)/ewusr) * eusq2 + 0.5 * (sigx1 * eusq1 -
      sigx2 * eusq2)))/(sigx15 * ewusr) - (sigx5 * eusq1 -
      sigx7) * sigx9 * ewusr/(sigx15 * ewusr)^2) * pwZ),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    2 * (S * (((dmustar1 * (ewv1/(2 * ewu) - (sigx10 * mustar1 +
    ...
    (mustar2 * sigx12 + 0.5)) * dmustar2/ewv1_h - 2 * ((2 *
    (ewv1 * pmustar2/ewu) - dmustar2 * sigx12)/ewusr)) *
    eusq2)/(sigx15 * ewusr) - sigx9 * ewusr * sigx13/(sigx15 *
    ewusr)^2) * pwZ), FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * (1 - pwZ) * (S * (0.5 * (S^2 * (epsilon)^2/ewv2_h^2 -
      2) - 0.5) * depsi * (epsilon)/(sigx15 * ewv2_h^3) -
      sigx24 * sigx9 * ewv2_h/(sigx15 * ewv2_h)^2), FUN = "*"),
    vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (2 * ((sigx1 * eusq1 -
      sigx2 * eusq2)/ewusr) - (sigx9 * sigx14/sigx15 +
      S * depsi * (epsilon)/ewv2_h^3)) * dwZ/sigx15, FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    2 * (((((0.5 * (0.5 * (ewv1_h * mustar1/ewusr) - 0.5) -
    ...
    (S * (epsilon)/ewusr) + 2 * (ewv1/ewu)) * pmustar2 -
    (2 * (ewv1/ewu) + S * (epsilon)/ewusr) * sigx6) * eusq2 +
    0.5 * (sigx5 * eusq1 - sigx6 * eusq2)))/(sigx15 * ewusr) -
    (sigx5 * eusq1 - sigx7) * (0.5 * (sigx15 * ewusr) + 2 *
      ((sigx5 * eusq1 - sigx7) * pwZ))/(sigx15 * ewusr)^2) *
    pwZ), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((((dmustar1 * ewv1_h/(4 *
      ...
      sigx12 - 0.5) * dmustar2 * ewv1_h/ewusr + (2 * (ewv1 *
      pmustar2/ewu) - dmustar2 * sigx12) * (2 * (ewv1/ewu) +
      S * (epsilon)/ewusr))) * eusq2)/(sigx15 * ewusr) -
      (0.5 * (sigx15 * ewusr) + 2 * ((sigx5 * eusq1 - sigx7) *
        pwZ)) * sigx13/(sigx15 * ewusr)^2) * pwZ), FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (2 * ((sigx5 * eusq1 - sigx7) *
      sigx24 * (1 - pwZ) * ewv2_h * pwZ/((sigx15 * ewv2_h)^2 *
      ewusr))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    2 * nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (sigx5 * eusq1 - sigx7) *
      (2 - 2 * (sigx14 * pwZ/sigx15)) * dwZ/(sigx15 * ewusr),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((((sigx11/2 + (pmustar1 -
      ...
      ewv1/ewu - (0.25 * (S * (epsilon)/ewv1_h) + 0.5 *
      (ewv1_h/ewusr) - mustar2 * sigx12^2) * dmustar2) *
      eusq2)/(sigx15 * ewusr) - 2 * (sigx13^2 * pwZ/(sigx15 *
      ewusr)^2)) * pwZ), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (2 * (sigx24 * (1 - pwZ) * sigx13 * ewv2_h * pwZ/((sigx15 *
      ewv2_h)^2 * ewusr))), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
      2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (2 - 2 * (sigx14 * pwZ/sigx15)) *
      dwZ * sigx13/(sigx15 * ewusr), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * (1 - pwZ) * (S^2 * (0.5 * (0.5 * (S^2 *
      (epsilon)^2/ewv2_h^2) - 1) - 0.25) * depsi * (epsilon)^2/(sigx15 *
      ewv2_h^3) - (sigx24 * (1 - pwZ) + 0.5 * (sigx15 *
      ewv2_h)) * sigx24/(sigx15 * ewv2_h)^2), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * (((1 - pwZ) * sigx14/sigx15 +
      1) * sigx24 * dwZ/(sigx15 * ewv2_h)), FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar + nZHvar), (nXvar + nuZUvar + 2 * nvZVvar +
    1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -wHvar * ((sigx14 * dwZ/sigx15 +
      Wz) * sigx14 * dwZ/sigx15), FUN = "*"), Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
}
...
## cloglog specification class membership
chessmnsfgenexponormlike_cloglog <- function(parm, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll <- matrix(nrow = nXvar + nuZUvar + 2 * nvZVvar + nZHvar,
    ncol = nXvar + nuZUvar + 2 * nvZVvar + nZHvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * (2 * ((((mustar1/ewv1_h - 1/ewusr) *
      dmustar1/ewv1_h - sigx1/ewusr) * eusq1 - ((mustar2/ewv1_h -
      2/ewusr) * dmustar2/ewv1_h - 2 * (sigx2/ewusr)) *
      eusq2) * (1 - prZ)/ewusr) + depsi * prZ * (S^2 *
      (epsilon)^2/ewv2_h^2 - 1)/ewv2_h^3 - sigx9^2/sigx8)/sigx8,
    FUN = "*"), Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * 2 * (S * (((((0.5 + 0.5 *
      (S * (epsilon)/ewusr) + 2 * (ewu * ewv1/(2 * ewu)^2)) *
      ...
      2 * sigx6)/ewusr) * eusq2 + 0.5 * (sigx1 * eusq1 -
      sigx2 * eusq2)))/(sigx8 * ewusr) - (sigx5 * eusq1 -
      sigx7) * sigx9 * ewusr/(sigx8 * ewusr)^2) * (1 -
      prZ)), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    2 * (S * (((dmustar1 * (ewv1/(2 * ewu) - (sigx10 * mustar1 +
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (2 * ((sigx1 * eusq1 -
      sigx2 * eusq2)/ewusr) - (sigx9 * sigx14/sigx8 + S *
      depsi * (epsilon)/ewv2_h^3)) * prZ * ewz/sigx8, FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    2 * (((((0.5 * (0.5 * (ewv1_h * mustar1/ewusr) - 0.5) -
    ...
    0.5 * (sigx5 * eusq1 - sigx6 * eusq2)))/(sigx8 * ewusr) -
    (sigx5 * eusq1 - sigx7) * (0.5 * (sigx8 * ewusr) + 2 *
      ((sigx5 * eusq1 - sigx7) * (1 - prZ)))/(sigx8 * ewusr)^2) *
    (1 - prZ)), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((((dmustar1 * ewv1_h/(4 *
      ...
      S * (epsilon)/ewusr))) * eusq2)/(sigx8 * ewusr) -
      (0.5 * (sigx8 * ewusr) + 2 * ((sigx5 * eusq1 - sigx7) *
        (1 - prZ))) * sigx13/(sigx8 * ewusr)^2) * (1 -
      prZ)), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -wHvar * (2 * ((sigx5 * eusq1 - sigx7) *
      sigx24 * (1 - prZ) * prZ * ewv2_h/((sigx8 * ewv2_h)^2 *
      ewusr))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    2 * nvZVvar + 1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (sigx5 * eusq1 - sigx7) *
      (2 - 2 * ((1 - prZ) * sigx14/sigx8)) * prZ * ewz/(sigx8 *
      ewusr), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * 2 * (((((sigx11/2 + (pmustar1 -
      ...
      ewv1/ewu - (0.25 * (S * (epsilon)/ewv1_h) + 0.5 *
      (ewv1_h/ewusr) - mustar2 * sigx12^2) * dmustar2) *
      eusq2)/(sigx8 * ewusr) - 2 * ((1 - prZ) * sigx13^2/(sigx8 *
      ewusr)^2)) * (1 - prZ)), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 2 *
      nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -wHvar *
    (2 * (sigx24 * (1 - prZ) * prZ * sigx13 * ewv2_h/((sigx8 *
      ewv2_h)^2 * ewusr))), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
      2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * (2 - 2 * ((1 - prZ) * sigx14/sigx8)) *
      prZ * sigx13 * ewz/(sigx8 * ewusr), FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * prZ * (S^2 * (0.5 * (0.5 * (S^2 * (epsilon)^2/ewv2_h^2) -
      1) - 0.25) * depsi * (epsilon)^2/(sigx8 * ewv2_h^3) -
      (sigx24 * prZ + 0.5 * (sigx8 * ewv2_h)) * sigx24/(sigx8 *
        ewv2_h)^2), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar), (nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar +
    nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -wHvar * ((sigx14 * prZ/sigx8 + 1) *
      sigx24 * prZ * ewz/(sigx8 * ewv2_h)), FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + 2 * nvZVvar + 1):(nXvar + nuZUvar +
    2 * nvZVvar + nZHvar), (nXvar + nuZUvar + 2 * nvZVvar +
    1):(nXvar + nuZUvar + 2 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = wHvar * (1 - (sigx14 * prZ/sigx8 +
      1) * ewz) * sigx14 * prZ * ewz/sigx8, FUN = "*"),
    Zvar)
  hessll[lower.tri(hessll)] <- t(hessll)[lower.tri(hessll)]
  # hessll<-(hessll+(hessll))/2
  return(hessll)
...
## logit specification class membership
zisfgenexponormAlgOpt_logit <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(czisfgenexponormlike_logit(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(czisfgenexponormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradzisfgenexponormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = czisfgenexponormlike_logit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(czisfgenexponormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradzisfgenexponormlike_logit(parm,
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesszisfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesszisfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(czisfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradzisfgenexponormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesszisfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradzisfgenexponormlike_logit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chesszisfgenexponormlike_logit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- czisfgenexponormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradzisfgenexponormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
## cauchit specification class membership
zisfgenexponormAlgOpt_cauchit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initGenExpo <- start_st$initGenExpo
  startVal <- start_st$StartVal
  startLoglik <- sum(czisfgenexponormlike_cauchit(startVal,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("ZISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(czisfgenexponormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradzisfgenexponormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = czisfgenexponormlike_cauchit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(czisfgenexponormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradzisfgenexponormlike_cauchit(parm,
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesszisfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesszisfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(czisfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradzisfgenexponormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesszisfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradzisfgenexponormlike_cauchit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- czisfgenexponormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradzisfgenexponormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## probit specification class membership
zisfgenexponormAlgOpt_probit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initGenExpo <- start_st$initGenExpo
  startVal <- start_st$StartVal
  startLoglik <- sum(czisfgenexponormlike_probit(startVal,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("ZISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(czisfgenexponormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradzisfgenexponormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = czisfgenexponormlike_probit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(czisfgenexponormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradzisfgenexponormlike_probit(parm,
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesszisfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesszisfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(czisfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradzisfgenexponormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesszisfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradzisfgenexponormlike_probit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- czisfgenexponormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradzisfgenexponormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## cloglog specification class membership
zisfgenexponormAlgOpt_cloglog <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initGenExpo <- start_st$initGenExpo
  startVal <- start_st$StartVal
  startLoglik <- sum(czisfgenexponormlike_cloglog(startVal,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("ZISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(czisfgenexponormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradzisfgenexponormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = czisfgenexponormlike_cloglog,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(czisfgenexponormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradzisfgenexponormlike_cloglog(parm,
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chesszisfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradzisfgenexponormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chesszisfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(czisfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradzisfgenexponormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chesszisfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradzisfgenexponormlike_cloglog(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- czisfgenexponormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradzisfgenexponormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## logit specification class membership
mnsfgenexponormAlgOpt_logit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initGenExpo <- start_st$initGenExpo
  startVal <- start_st$StartVal
  startLoglik <- sum(cmnsfgenexponormlike_logit(startVal, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("ZISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmnsfgenexponormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmnsfgenexponormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmnsfgenexponormlike_logit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmnsfgenexponormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_logit(parm,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmnsfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmnsfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmnsfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmnsfgenexponormlike_logit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmnsfgenexponormlike_logit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmnsfgenexponormlike_logit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmnsfgenexponormlike_logit(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmnsfgenexponormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmnsfgenexponormlike_logit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## cauchit specification class membership
mnsfgenexponormAlgOpt_cauchit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initGenExpo <- start_st$initGenExpo
  startVal <- start_st$StartVal
  startLoglik <- sum(cmnsfgenexponormlike_cauchit(startVal,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("ZISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmnsfgenexponormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmnsfgenexponormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmnsfgenexponormlike_cauchit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmnsfgenexponormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_cauchit(parm,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmnsfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmnsfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmnsfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmnsfgenexponormlike_cauchit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmnsfgenexponormlike_cauchit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmnsfgenexponormlike_cauchit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- cmnsfgenexponormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmnsfgenexponormlike_cauchit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## probit specification class membership
mnsfgenexponormAlgOpt_probit <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initGenExpo <- start_st$initGenExpo
  startVal <- start_st$StartVal
  startLoglik <- sum(cmnsfgenexponormlike_probit(startVal,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("ZISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmnsfgenexponormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmnsfgenexponormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmnsfgenexponormlike_probit,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmnsfgenexponormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_probit(parm,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmnsfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmnsfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmnsfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmnsfgenexponormlike_probit(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmnsfgenexponormlike_probit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmnsfgenexponormlike_probit(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- cmnsfgenexponormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmnsfgenexponormlike_probit(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
...
## cloglog specification class membership
mnsfgenexponormAlgOpt_cloglog <- function(start, olsParam, dataTable,
  S, wHvar, nXvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, tol, gradtol,
  whichStart, initIter, initAlg, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  initGenExpo <- start_st$initGenExpo
  startVal <- start_st$StartVal
  startLoglik <- sum(cmnsfgenexponormlike_cloglog(startVal,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("ZISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cmnsfgenexponormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    gr = function(parm) -colSums(cgradmnsfgenexponormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = printInfo, maxeval = itermax,
      stepmax = stepmax, xtol = tol, grtol = gradtol)),
    maxLikAlgo = maxRoutine(fn = cmnsfgenexponormlike_cloglog,
      ...
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cmnsfgenexponormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_cloglog(parm,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessmnsfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
        "dgCMatrix"), method = "Sparse", control = list(maxit = itermax,
      ...
      gr = function(parm) -colSums(cgradmnsfgenexponormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessmnsfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cmnsfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradmnsfgenexponormlike_cloglog(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessmnsfgenexponormlike_cloglog(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      control = list(iter.max = itermax, trace = printInfo,
        eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmnsfgenexponormlike_cloglog(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  mleObj$logL_OBS <- cmnsfgenexponormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmnsfgenexponormlike_cloglog(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik,
