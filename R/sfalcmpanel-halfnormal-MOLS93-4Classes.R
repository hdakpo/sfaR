...
#' @noRd
pLCMhalfnormlike4C_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  eta4[ngZGvar] <- 1
  theta1 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar - 3):(4 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + nZHvar - 4)]
  theta2 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + nZHvar -
    3):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + 2 * nZHvar - 4)]
  theta3 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + 2 * nZHvar -
    3):((4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 4 * ngZGvar + 3 * nZHvar - 4))]
...
#' @noRd
psLCMfhalfnorm4C_mols93 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
  ...
  StartVal <- c(Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar -
    ...
    Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar - 1), Esti[nXvar + 2], if (nvZVvar >
      1) rep(0, nvZVvar - 1), Esti[(nXvar + 3):(nXvar + ngZGvar + 1)], rep(0,
      3 * nZHvar))
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike4C_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  return(sweep(gradll[, -c((nXvar + nuZUvar + nvZVvar + ngZGvar), 2 * (nXvar +
    ...
    4 * (nXvar + nuZUvar + nvZVvar + ngZGvar))], MARGIN = 1, STATS = wHvar, FUN = "*"))
}
...
#' @noRd
LCM4ChnormAlgOpt_mols93 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm4C_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike4C_mols93(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike4C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    -colSums(pgradLCMhalfnormlike4C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike4C_mols93,
    grad = pgradLCMhalfnormlike4C_mols93, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      -sum(pLCMhalfnormlike4C_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
    }, gr = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike4C_mols93(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike4C_mols93(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike4C_mols93(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
        pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)),
        unname(mleObj$par))
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike4C_mols93(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike4C_mols93(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
...
pLCM4Chalfnormeff_gzit <- function(object, level) {
  ...
  theta1 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) - 3):(4 * object$nXvar + 4 * object$nuZUvar + 4 *
    object$nvZVvar + 4 * ncol(object$gHvar) + object$nZHvar - 4)]
  theta2 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) + object$nZHvar - 3):(4 * object$nXvar + 4 * object$nuZUvar +
    4 * object$nvZVvar + 4 * ncol(object$gHvar) + 2 * object$nZHvar - 4)]
  theta3 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar + 4 * object$nvZVvar +
    4 * ncol(object$gHvar) + 2 * object$nZHvar - 3):(4 * object$nXvar + 4 * object$nuZUvar +
    4 * object$nvZVvar + 4 * ncol(object$gHvar) + 3 * object$nZHvar - 4)]
