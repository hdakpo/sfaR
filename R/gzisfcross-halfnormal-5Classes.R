...
#' @noRd
cGZISFhalfnormlike5C <- function(parm, nXvar, nuZUvar, nvZVvar,
  ...
  beta5 <- parm[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar)]
  phi5 <- parm[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 *
    nXvar + 4 * nuZUvar + 5 * nvZVvar)]
  theta1 <- parm[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 1):(5 *
    nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar)]
  theta2 <- parm[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar)]
  theta3 <- parm[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 *
    nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    3 * nZHvar)]
  theta4 <- parm[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 *
    nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    4 * nZHvar)]
  ...
  ifelse(Probc1 * Pi1 + Probc2 * Pi2 + Probc3 * Pi3 + Probc4 *
    Pi4 + Probc5 * Pi5 <= 0, return(NA), return(wHvar * log(Probc1 *
    Pi1 + Probc2 * Pi2 + Probc3 * Pi3 + Probc4 * Pi4 + Probc5 *
    Pi5)))
}
...
#' @noRd
csGZISFfhalfnorm5C <- function(olsObj, epsiRes, nXvar, nuZUvar,
...
#' @param nZHvar number of separating variables
#' @noRd
cgradGZISFhalfnormlike5C <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx1_2 <- (dmusig2 * depsi2 * ewu2/sigmastar2 + S * depsi2 *
    pmusig2 * (epsilon2))
  sigx1_3 <- (dmusig3 * depsi3 * ewu3/sigmastar3 + S * depsi3 *
    pmusig3 * (epsilon3))
  sigx1_4 <- (dmusig4 * depsi4 * ewu4/sigmastar4 + S * depsi4 *
    pmusig4 * (epsilon4))
  ...
  sigx3 <- (prC * depsi5/ewv5_h + (2 * sigx2_1 + 2 * sigx2_2 +
    2 * sigx2_3 + 2 * sigx2_4)/wzdeno)
  ...
  sigx10_3 <- ((0.5 * pvu3 + sigmastar3) * dmusig3 * depsi3 *
    ewu3/ssq3^2 + 0.5 * sigx5_3)
  sigx10_4 <- ((0.5 * pvu4 + sigmastar4) * dmusig4 * depsi4 *
    ewu4/ssq4^2 + 0.5 * sigx5_4)
  ...
  sigx13 <- (0.5 * (S^2 * depsi5 * (epsilon5)^2/ewv5_h^2) -
    0.5 * depsi5)
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = 2 * (S *
    ...
    sweep(vHvar, MARGIN = 1, STATS = 2 * (ewv1 * ewz1 * sigx11_1/sigx9_1),
      ...
      MARGIN = 1, STATS = 2 * (S * sigx1_3 * ewz3/sigx4_3),
      FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = 2 *
      (ewu3 * ewz3 * sigx8_3/sigx9_3), FUN = "*"), sweep(vHvar,
      MARGIN = 1, STATS = 2 * (ewv3 * ewz3 * sigx11_3/sigx9_3),
      FUN = "*"), sweep(Xvar, MARGIN = 1, STATS = 2 * (S *
      sigx1_4 * ewz4/sigx4_4), FUN = "*"), sweep(uHvar,
      MARGIN = 1, STATS = 2 * (ewu4 * ewz4 * sigx8_4/sigx9_4),
      FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = 2 *
      (ewv4 * ewz4 * sigx11_4/sigx9_4), FUN = "*"), sweep(Xvar,
      MARGIN = 1, STATS = S^2 * prC * depsi5 * (epsilon5)/(sigx3 *
        ewv5_h^3), FUN = "*"), sweep(vHvar, MARGIN = 1,
      STATS = sigx13 * prC/(sigx3 * ewv5_h), FUN = "*"),
    sweep(Zvar, MARGIN = 1, STATS = sigx12_1 * ewz1/(sigx3 *
      wzdeno), FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx12_2 *
      ewz2/(sigx3 * wzdeno), FUN = "*"), sweep(Zvar, MARGIN = 1,
      STATS = sigx12_3 * ewz3/(sigx3 * wzdeno), FUN = "*"),
    sweep(Zvar, MARGIN = 1, STATS = sigx12_4 * ewz4/(sigx3 *
      wzdeno), FUN = "*"))
...
#' @param nZHvar number of separating variables
#' @noRd
chessGZISFhalfnormlike5C <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi1 <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx14_3 <- (S * (dmusig3 * ewu3/sigmastar3 + S * pmusig3 *
    (epsilon3)) * (epsilon3)/(sigma_sq3) - pmusig3)
  sigx14_4 <- (S * (dmusig4 * ewu4/sigmastar4 + S * pmusig4 *
    (epsilon4)) * (epsilon4)/(sigma_sq4) - pmusig4)
  ...
  sigx17_2 <- (S * (0.5 * sigx16_2 - sigx6_2 * dmusig2) * (epsilon2) -
    2 * (pmusig2/(sigma_sq2)))
  sigx17_3 <- (S * (0.5 * sigx16_3 - sigx6_3 * dmusig3) * (epsilon3) -
    2 * (pmusig3/(sigma_sq3)))
  sigx17_4 <- (S * (0.5 * sigx16_4 - sigx6_4 * dmusig4) * (epsilon4) -
    2 * (pmusig4/(sigma_sq4)))
  sigx18_1 <- ((2 - 2 * (ewz1/wzdeno))/(sigx3 * wzdeno) - 2 *
    (sigx12_1 * ewz1/(sigx3 * wzdeno)^2))
  sigx18_2 <- ((2 - 2 * (ewz2/wzdeno))/(sigx3 * wzdeno) - 2 *
    (sigx12_2 * ewz2/(sigx3 * wzdeno)^2))
  sigx18_3 <- ((2 - 2 * (ewz3/wzdeno))/(sigx3 * wzdeno) - 2 *
    (sigx12_3 * ewz3/(sigx3 * wzdeno)^2))
  sigx18_4 <- (2 * (sigx12_4/(sigx3 * wzdeno)^2) + 2/(sigx3 *
    wzdeno^2))
  ...
  sigx27_1 <- (2 * (sigx12_1/(sigx3 * wzdeno)^2) + 2/(sigx3 *
    wzdeno^2))
  sigx27_2 <- (2 * (sigx12_2/(sigx3 * wzdeno)^2) + 2/(sigx3 *
    wzdeno^2))
  sigx27_3 <- (2 * (sigx12_3/(sigx3 * wzdeno)^2) + 2/(sigx3 *
    wzdeno^2))
  sigx27_4 <- ((2 - 2 * (ewz4/wzdeno))/(sigx3 * wzdeno) - 2 *
    (sigx12_4 * ewz4/(sigx3 * wzdeno)^2))
  hessll <- matrix(nrow = 5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    4 * nZHvar, ncol = 5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    4 * nZHvar)
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = 2 * (S^2 * ((depsi1 * sigx14_1 + S * dmusig1 *
      sigx15_1 * ewu1 * (epsilon1)/ssq1)/sigx4_1 - 2 *
      (sigx1_1^2 * ewz1/sigx4_1^2)) * ewz1) * wHvar, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S * ((sigx6_1 * dmusig1 * depsi1 +
      (S * ((0.5 * sigx14_1 - 0.5 * pmusig1) * depsi1/(sigma_sq1) -
        S * sigx6_1 * dmusig1 * sigx15_1 * (epsilon1)) *
        (epsilon1) - 0.5 * (sigx1_1/(sigma_sq1)))/(sigma_sq1))/sigx9_1 -
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    ...
    (S * (((S * ((0.5 * sigx14_1 - 0.5 * pmusig1) * depsi1/(sigma_sq1) +
      S * (0.5 * pvu1 + sigmastar1) * dmusig1 * sigx15_1 *
        ewu1 * (epsilon1)/ssq1^2) * (epsilon1) - 0.5 *
      (sigx1_1/(sigma_sq1)))/(sigma_sq1) - (0.5 * pvu1 +
      sigmastar1) * dmusig1 * depsi1 * ewu1/ssq1^2)/sigx9_1 -
  ...
  hessll[1:nXvar, (2 * nXvar + nuZUvar + nvZVvar + 1):(2 *
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewu2 * ewz1 *
      ewz2 * sigx8_2 * sqrt(sigma_sq2)/sigx20)) * wHvar,
    FUN = "*"), uHvar)
  hessll[1:nXvar, (2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 *
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewv2 * ewz1 *
      ewz2 * sigx11_2 * sqrt(sigma_sq2)/sigx20)) * wHvar,
    FUN = "*"), vHvar)
  hessll[1:nXvar, (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    ...
    MARGIN = 1, STATS = -(4 * (S^2 * sigx1_1 * sigx1_3 *
      (sigma_sq3) * ewz1 * ewz3 * sqrt(sigma_sq3)/(sigx4_3^2 *
  ...
  hessll[1:nXvar, (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewu3 * ewz1 *
      ewz3 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 * (sigma_sq1) *
  ...
  hessll[1:nXvar, (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewv3 * ewz1 *
      ewz3 * sigx11_3 * sqrt(sigma_sq3)/(sigx9_3^2 * (sigma_sq1) *
  ...
  hessll[1:nXvar, (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    ...
    MARGIN = 1, STATS = -(4 * (S^2 * sigx1_1 * sigx1_4 *
      (sigma_sq4) * ewz1 * ewz4 * sqrt(sigma_sq4)/(sigx4_4^2 *
      (sigma_sq1) * sqrt(sigma_sq1)))) * wHvar, FUN = "*"),
    Xvar)
  hessll[1:nXvar, (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewu4 * ewz1 *
      ewz4 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 * (sigma_sq1) *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuZUvar + 3 * nvZVvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewv4 * ewz1 *
      ewz4 * sigx11_4 * sqrt(sigma_sq4)/(sigx9_4^2 * (sigma_sq1) *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), vHvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S^3 * prC * sigx1_1 * depsi5 *
      ewv5_h^3 * ewz1 * (epsilon5)/((sigx3 * ewv5_h^3)^2 *
      wzdeno * (sigma_sq1) * sqrt(sigma_sq1)))) * wHvar,
    FUN = "*"), Xvar)
  hessll[1:nXvar, (5 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S * sigx13 * prC * sigx1_1 *
      ewv5_h * ewz1/((sigx3 * ewv5_h)^2 * wzdeno * (sigma_sq1) *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), vHvar)
  hessll[1:nXvar, (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * sigx18_1 * sigx1_1 * ewz1/sigx21_1 *
      wHvar, FUN = "*"), Zvar)
  hessll[1:nXvar, (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx27_2 * sigx1_1 * ewz1 * ewz2/sigx21_1) * wHvar, FUN = "*"),
    Zvar)
  hessll[1:nXvar, (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx27_3 * sigx1_1 * ewz1 * ewz3/sigx21_1) * wHvar, FUN = "*"),
    Zvar)
  hessll[1:nXvar, (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    4 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx18_4 * sigx1_1 * ewz1 * ewz4/sigx21_1) * wHvar, FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = 2 *
    (((ewu1 * (S * (0.5 * sigx24_1 - (0.5 * (S^2 * sigx6_1 *
      depsi1 * (epsilon1)^2/(sigma_sq1)^2) - (((0.5 * (ewu1/(sigma_sq1)) +
      1 - 0.5 * (0.5 * prU1 + ewu1/(sigma_sq1))) * prU1 *
      ewv1/sigmastar1 + (2 - 2 * ((0.5 * puv1 + sigmastar1)^2 *
      ewu1 * (sigma_sq1)/ssq1^2)) * sigmastar1)/ssq1^2 +
      S^2 * sigx6_1^2 * ewu1 * (epsilon1)^2/ssq1) * depsi1) *
      dmusig1) * (epsilon1) - 0.5 * ((S * sigx7_1 * (epsilon1) -
      depsi1 * pmusig1/(sigma_sq1))/(sigma_sq1))) + S *
      sigx7_1 * (epsilon1) - 0.5 * dpesq1)/sigx9_1 - sigx23_1 *
      ewu1 * sigx8_1/sigx9_1^2) * ewu1 * ewz1) * wHvar,
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((S * (((((0.5 * (prU1 * ewv1) -
      S^2 * (0.5 * pvu1 + sigmastar1) * sigx6_1 * ewu1 *
        (epsilon1)^2)/(sigma_sq1) + 0.5 * ((ewu1/(sigma_sq1) -
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_2 * ewu1 * (sigma_sq2) *
      ewz1 * ewz2 * sigx8_1 * sqrt(sigma_sq2)/(sigx4_2^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (2 * nXvar + nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewu1 * ewu2 * ewz1 * ewz2 *
      sigx8_1 * sigx8_2 * sqrt(sigma_sq2)/(sigx9_2^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (3 * nXvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewu1 * ewu3 * ewz1 * ewz3 *
      sigx8_1 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  ...
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 3 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewu1 * ewu4 * ewz1 * ewz4 *
      sigx8_1 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 4 * nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewu1 * ewv4 * ewz1 * ewz4 *
      sigx11_4 * sigx8_1 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewu1 *
      ewv5_h^3 * ewz1 * sigx8_1 * (epsilon5)/sigx26_1)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewu1 * ewv5_h *
      ewz1 * sigx8_1/sigx19_1)) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = sigx18_1 *
    ewu1 * ewz1 * sigx8_1/sqrt(sigma_sq1) * wHvar, FUN = "*"),
    Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx27_2 * ewu1 * ewz1 * ewz2 *
      sigx8_1/sqrt(sigma_sq1)) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx27_3 * ewu1 * ewz1 * ewz3 *
      sigx8_1/sqrt(sigma_sq1)) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx18_4 * ewu1 * ewz1 * ewz4 *
      sigx8_1/sqrt(sigma_sq1)) * wHvar, FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    ...
    MARGIN = 1, STATS = -(4 * (ewv1 * ewv2 * ewz1 * ewz2 *
      sigx11_1 * sigx11_2 * sqrt(sigma_sq2)/(sigx9_2^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
      2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * (sigma_sq3) *
      ewv1 * ewz1 * ewz3 * sigx11_1 * sqrt(sigma_sq3)/(sigx4_3^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
      3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewu3 * ewv1 * ewz1 * ewz3 *
      sigx11_1 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
      3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewv1 * ewv3 * ewz1 * ewz3 *
      sigx11_1 * sigx11_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
      3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * (sigma_sq4) *
      ewv1 * ewz1 * ewz4 * sigx11_1 * sqrt(sigma_sq4)/(sigx4_4^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
      4 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewu4 * ewv1 * ewz1 * ewz4 *
      sigx11_1 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
      4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewv1 * ewv4 * ewz1 * ewz4 *
      sigx11_1 * sigx11_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq1)))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
      4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewv1 *
      ewv5_h^3 * ewz1 * sigx11_1 * (epsilon5)/sigx26_1)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
      4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewv1 * ewv5_h *
      ewz1 * sigx11_1/sigx19_1)) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 1):(5 * nXvar +
      4 * nuZUvar + 5 * nvZVvar + nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx18_1 * ewv1 * ewz1 * sigx11_1/sqrt(sigma_sq1) *
      wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar + 1):(5 *
      nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx27_2 * ewv1 * ewz1 * ewz2 *
      sigx11_1/sqrt(sigma_sq1)) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar +
      1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx27_3 * ewv1 * ewz1 * ewz3 *
      sigx11_1/sqrt(sigma_sq1)) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar +
      1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx18_4 * ewv1 * ewz1 * ewz4 *
      sigx11_1/sqrt(sigma_sq1)) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar +
    nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = 2 * (S^2 * ((depsi2 * sigx14_2 + S * dmusig2 *
      sigx15_2 * ewu2 * (epsilon2)/ssq2)/sigx4_2 - 2 *
      (sigx1_2^2 * ewz2/sigx4_2^2)) * ewz2) * wHvar, FUN = "*"),
    Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = 2 * (S * ((sigx6_2 * dmusig2 * depsi2 + (S *
      ((0.5 * sigx14_2 - 0.5 * pmusig2) * depsi2/(sigma_sq2) -
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    ...
    nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S * (((S * ((0.5 * sigx14_2 -
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (S^2 * sigx1_2 * sigx1_3 *
      (sigma_sq3) * ewz2 * ewz3 * sqrt(sigma_sq3)/(sigx4_3^2 *
      (sigma_sq2) * sqrt(sigma_sq2)))) * wHvar, FUN = "*"),
    Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_2 * ewu3 * ewz2 *
      ewz3 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 * (sigma_sq2) *
  ...
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_2 * ewu4 * ewz2 *
      ewz4 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 * (sigma_sq2) *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), uHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_2 * ewv4 * ewz2 *
      ewz4 * sigx11_4 * sqrt(sigma_sq4)/(sigx9_4^2 * (sigma_sq2) *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 *
    nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S^3 * prC * sigx1_2 * depsi5 *
      ewv5_h^3 * ewz2 * (epsilon5)/((sigx3 * ewv5_h^3)^2 *
      wzdeno * (sigma_sq2) * sqrt(sigma_sq2)))) * wHvar,
    FUN = "*"), Xvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 *
    nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S * sigx13 * prC * sigx1_2 *
      ewv5_h * ewz2/((sigx3 * ewv5_h)^2 * wzdeno * (sigma_sq2) *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 1):(5 *
    nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx27_1 * sigx1_2 * ewz1 *
      ewz2/sigx21_2) * wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * sigx18_2 * sigx1_2 * ewz2/sigx21_2 *
      wHvar, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 *
    nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx27_3 * sigx1_2 * ewz2 * ewz3/sigx21_2) * wHvar, FUN = "*"),
    Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + nuZUvar +
    nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 *
    nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    4 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx18_4 * sigx1_2 * ewz2 * ewz4/sigx21_2) * wHvar, FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    MARGIN = 1, STATS = 2 * (((ewu2 * (S * (0.5 * sigx24_2 -
      (0.5 * (S^2 * sigx6_2 * depsi2 * (epsilon2)^2/(sigma_sq2)^2) -
        (((0.5 * (ewu2/(sigma_sq2)) + 1 - 0.5 * (0.5 *
          prU2 + ewu2/(sigma_sq2))) * prU2 * ewv2/sigmastar2 +
      ...
      sigx23_2 * ewu2 * sigx8_2/sigx9_2^2) * ewu2 * ewz2) *
      wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (2 * nXvar + 2 * nuZUvar + nvZVvar +
    1):(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((S * (((((0.5 * (prU2 * ewv2) -
      S^2 * (0.5 * pvu2 + sigmastar2) * sigx6_2 * ewu2 *
        (epsilon2)^2)/(sigma_sq2) + 0.5 * ((ewu2/(sigma_sq2) -
      ...
      sigx23_2 * sigx11_2/sigx9_2^2) * ewu2 * ewv2 * ewz2) *
      wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * ewu2 * (sigma_sq3) *
      ewz2 * ewz3 * sigx8_2 * sqrt(sigma_sq3)/(sigx4_3^2 *
  ...
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    MARGIN = 1, STATS = -(4 * (ewu2 * ewu3 * ewz2 * ewz3 *
      sigx8_2 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    MARGIN = 1, STATS = -(4 * (ewu2 * ewv3 * ewz2 * ewz3 *
      sigx11_3 * sigx8_2 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * ewu2 * (sigma_sq4) *
      ewz2 * ewz4 * sigx8_2 * sqrt(sigma_sq4)/(sigx4_4^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    MARGIN = 1, STATS = -(4 * (ewu2 * ewu4 * ewz2 * ewz4 *
      sigx8_2 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    ...
    MARGIN = 1, STATS = -(4 * (ewu2 * ewv4 * ewz2 * ewz4 *
      sigx11_4 * sigx8_2 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (4 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewu2 *
      ewv5_h^3 * ewz2 * sigx8_2 * (epsilon5)/sigx26_2)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (5 * nXvar + 4 * nuZUvar + 4 * nvZVvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewu2 * ewv5_h *
      ewz2 * sigx8_2/sigx19_2)) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx27_1 * ewu2 * ewz1 * ewz2 *
      sigx8_2/sqrt(sigma_sq2)) * wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    2 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = sigx18_2 *
    ewu2 * ewz2 * sigx8_2/sqrt(sigma_sq2) * wHvar, FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    3 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx27_3 *
    ewu2 * ewz2 * ewz3 * sigx8_2/sqrt(sigma_sq2)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuZUvar + nvZVvar + 1):(2 * nXvar + 2 *
    nuZUvar + nvZVvar), (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    4 * nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx18_4 *
    ewu2 * ewz2 * ewz4 * sigx8_2/sqrt(sigma_sq2)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = 2 * (((S * ((((0.5 * (ewv2/(sigma_sq2)) -
      ...
      (0.5 * sigx22_2 + 2 * (ewz2 * sigx11_2)) * ewv2 *
        sigx11_2/sigx9_2^2) * ewv2 * ewz2) * wHvar, FUN = "*"),
    vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (2 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * (sigma_sq3) *
      ewv2 * ewz2 * ewz3 * sigx11_2 * sqrt(sigma_sq3)/(sigx4_3^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewu3 * ewv2 * ewz2 * ewz3 *
      sigx11_2 * sigx8_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewv2 * ewv3 * ewz2 * ewz3 *
      sigx11_2 * sigx11_3 * sqrt(sigma_sq3)/(sigx9_3^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * (sigma_sq4) *
      ewv2 * ewz2 * ewz4 * sigx11_2 * sqrt(sigma_sq4)/(sigx4_4^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewu4 * ewv2 * ewz2 * ewz4 *
      sigx11_2 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    ...
    MARGIN = 1, STATS = -(4 * (ewv2 * ewv4 * ewz2 * ewz4 *
      sigx11_2 * sigx11_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq2)))) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewv2 *
      ewv5_h^3 * ewz2 * sigx11_2 * (epsilon5)/sigx26_2)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewv2 * ewv5_h *
      ewz2 * sigx11_2/sigx19_2)) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx27_1 *
    ewv2 * ewz1 * ewz2 * sigx11_2/sqrt(sigma_sq2)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx18_2 * ewv2 * ewz2 * sigx11_2/sqrt(sigma_sq2) *
      wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx27_3 * ewv2 * ewz2 * ewz3 *
      sigx11_2/sqrt(sigma_sq2)) * wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + nvZVvar + 1):(2 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx18_4 * ewv2 * ewz2 * ewz4 *
      sigx11_2/sqrt(sigma_sq2)) * wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (2 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S^2 * ((depsi3 * sigx14_3 +
      S * dmusig3 * sigx15_3 * ewu3 * (epsilon3)/ssq3)/sigx4_3 -
      2 * (sigx1_3^2 * ewz3/sigx4_3^2)) * ewz3) * wHvar,
    FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S * ((sigx6_3 * dmusig3 * depsi3 +
      (S * ((0.5 * sigx14_3 - 0.5 * pmusig3) * depsi3/(sigma_sq3) -
        S * sigx6_3 * dmusig3 * sigx15_3 * (epsilon3)) *
        (epsilon3) - 0.5 * (sigx1_3/(sigma_sq3)))/(sigma_sq3))/sigx9_3 -
      2 * (sigx1_3 * ewz3 * sigx8_3/(sigx9_3^2 * (sigma_sq3)))) *
      ewu3 * ewz3) * wHvar, FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S * (((S * ((0.5 * sigx14_3 -
      0.5 * pmusig3) * depsi3/(sigma_sq3) + S * (0.5 *
      pvu3 + sigmastar3) * dmusig3 * sigx15_3 * ewu3 *
      (epsilon3)/ssq3^2) * (epsilon3) - 0.5 * (sigx1_3/(sigma_sq3)))/(sigma_sq3) -
      (0.5 * pvu3 + sigmastar3) * dmusig3 * depsi3 * ewu3/ssq3^2)/sigx9_3 -
      2 * (sigx1_3 * ewz3 * sigx11_3/(sigx9_3^2 * (sigma_sq3)))) *
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    ...
    MARGIN = 1, STATS = -(4 * (S^2 * sigx1_3 * sigx1_4 *
      (sigma_sq4) * ewz3 * ewz4 * sqrt(sigma_sq4)/(sigx4_4^2 *
      (sigma_sq3) * sqrt(sigma_sq3)))) * wHvar, FUN = "*"),
    Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * ewu4 * ewz3 *
      ewz4 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 * (sigma_sq3) *
  ...
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * ewv4 * ewz3 *
      ewz4 * sigx11_4 * sqrt(sigma_sq4)/(sigx9_4^2 * (sigma_sq3) *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S^3 * prC * sigx1_3 * depsi5 *
      ewv5_h^3 * ewz3 * (epsilon5)/((sigx3 * ewv5_h^3)^2 *
      wzdeno * (sigma_sq3) * sqrt(sigma_sq3)))) * wHvar,
    FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S * sigx13 * prC * sigx1_3 *
      ewv5_h * ewz3/((sigx3 * ewv5_h)^2 * wzdeno * (sigma_sq3) *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx27_1 * sigx1_3 * ewz1 * ewz3/sigx21_3) * wHvar, FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * sigx27_2 * sigx1_3 * ewz2 * ewz3/sigx21_3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = S * sigx18_3 * sigx1_3 * ewz3/sigx21_3 * wHvar,
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    2 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * sigx18_4 * sigx1_3 * ewz3 * ewz4/sigx21_3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 2 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((ewu3 * (S * (0.5 * sigx24_3 -
      (0.5 * (S^2 * sigx6_3 * depsi3 * (epsilon3)^2/(sigma_sq3)^2) -
        (((0.5 * (ewu3/(sigma_sq3)) + 1 - 0.5 * (0.5 *
          prU3 + ewu3/(sigma_sq3))) * prU3 * ewv3/sigmastar3 +
          (2 - 2 * ((0.5 * puv3 + sigmastar3)^2 * ewu3 *
          (sigma_sq3)/ssq3^2)) * sigmastar3)/ssq3^2 +
          S^2 * sigx6_3^2 * ewu3 * (epsilon3)^2/ssq3) *
          depsi3) * dmusig3) * (epsilon3) - 0.5 * ((S *
      sigx7_3 * (epsilon3) - depsi3 * pmusig3/(sigma_sq3))/(sigma_sq3))) +
      S * sigx7_3 * (epsilon3) - 0.5 * dpesq3)/sigx9_3 -
      sigx23_3 * ewu3 * sigx8_3/sigx9_3^2) * ewu3 * ewz3) *
      wHvar, FUN = "*"), uHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((S * (((((0.5 * (prU3 * ewv3) -
      S^2 * (0.5 * pvu3 + sigmastar3) * sigx6_3 * ewu3 *
        (epsilon3)^2)/(sigma_sq3) + 0.5 * ((ewu3/(sigma_sq3) -
      1) * ewv3/(sigma_sq3) + 1 - 0.5 * (prU3 * prV3))) *
      depsi3/sigmastar3 + 0.5 * (S^2 * (0.5 * pvu3 + sigmastar3) *
      depsi3 * (epsilon3)^2/(sigma_sq3)^2)) * ewu3 + (0.5 *
      pvu3 + sigmastar3) * (1 - 2 * ((0.5 * puv3 + sigmastar3) *
      ewu3 * (sigma_sq3) * sigmastar3/ssq3^2)) * depsi3) *
      dmusig3/ssq3^2 + 0.5 * sigx24_3) * (epsilon3) - 0.5 *
      ((S * sigx7_3 * (epsilon3) - depsi3 * pmusig3/(sigma_sq3))/(sigma_sq3)))/sigx9_3 -
      sigx23_3 * sigx11_3/sigx9_3^2) * ewu3 * ewv3 * ewz3) *
      wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * ewu3 * (sigma_sq4) *
      ewz3 * ewz4 * sigx8_3 * sqrt(sigma_sq4)/(sigx4_4^2 *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), Xvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu3 * ewu4 * ewz3 * ewz4 *
      sigx8_3 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), uHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu3 * ewv4 * ewz3 * ewz4 *
      sigx11_4 * sigx8_3 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewu3 *
      ewv5_h^3 * ewz3 * sigx8_3 * (epsilon5)/sigx25_3)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewu3 * ewv5_h *
      ewz3 * sigx8_3/sigx19_3)) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx27_1 *
    ewu3 * ewz1 * ewz3 * sigx8_3/sqrt(sigma_sq3)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx27_2 * ewu3 * ewz2 * ewz3 *
      sigx8_3/sqrt(sigma_sq3)) * wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = sigx18_3 * ewu3 * ewz3 * sigx8_3/sqrt(sigma_sq3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 2 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx18_4 * ewu3 * ewz3 * ewz4 *
      sigx8_3/sqrt(sigma_sq3)) * wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    2 * nvZVvar + 1):(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = 2 * (((S * ((((0.5 * (ewv3/(sigma_sq3)) -
      0.5 * (0.5 * prV3 + ewv3/(sigma_sq3))) * prV3 + S^2 *
      (0.5 * pvu3 + sigmastar3)^2 * ewu3 * ewv3 * (epsilon3)^2/(ssq3^2 *
      (sigma_sq3))) * depsi3 * ewu3/sigmastar3 + ((0.5 *
      (S^2 * depsi3 * (epsilon3)^2/(sigma_sq3)^2) - 2 *
      ((0.5 * pvu3 + sigmastar3) * depsi3 * (sigma_sq3) *
        sigmastar3/ssq3^2)) * ewv3 + depsi3) * (0.5 *
      pvu3 + sigmastar3)) * dmusig3 * ewu3/ssq3^2 + S *
      (0.5 * (ewv3 * (S * ((0.5 * pvu3 + sigmastar3) *
        dmusig3 * ewu3/ssq3^2 + 0.5 * sigx16_3) * (epsilon3) -
        2 * (pmusig3/(sigma_sq3)))) + 0.5 * pmusig3) *
      depsi3 * (epsilon3)/(sigma_sq3)^2) * (epsilon3) -
      (0.5 * (depsi3 * pmusig3) + 0.5 * (ewv3 * (S * sigx10_3 *
        (epsilon3) - depsi3 * pmusig3/(sigma_sq3))))/(sigma_sq3))/sigx9_3 -
      (0.5 * sigx22_3 + 2 * (ewz3 * sigx11_3)) * ewv3 *
        sigx11_3/sigx9_3^2) * ewv3 * ewz3) * wHvar, FUN = "*"),
    vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * (sigma_sq4) *
      ewv3 * ewz3 * ewz4 * sigx11_3 * sqrt(sigma_sq4)/(sigx4_4^2 *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewu4 * ewv3 * ewz3 * ewz4 *
      sigx11_3 * sigx8_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), uHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewv3 * ewv4 * ewz3 * ewz4 *
      sigx11_3 * sigx11_4 * sqrt(sigma_sq4)/(sigx9_4^2 *
      sqrt(sigma_sq3)))) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewv3 *
      ewv5_h^3 * ewz3 * sigx11_3 * (epsilon5)/sigx25_3)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewv3 * ewv5_h *
      ewz3 * sigx11_3/sigx19_3)) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx27_1 *
    ewv3 * ewz1 * ewz3 * sigx11_3/sqrt(sigma_sq3)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx27_2 * ewv3 * ewz2 * ewz3 *
      sigx11_3/sqrt(sigma_sq3)) * wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx18_3 * ewv3 * ewz3 * sigx11_3/sqrt(sigma_sq3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 2 * nvZVvar + 1):(3 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx18_4 * ewv3 * ewz3 * ewz4 *
      sigx11_3/sqrt(sigma_sq3)) * wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (3 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S^2 * ((depsi4 * sigx14_4 +
      S * dmusig4 * sigx15_4 * ewu4 * (epsilon4)/ssq4)/sigx4_4 -
      2 * (sigx1_4^2 * ewz4/sigx4_4^2)) * ewz4) * wHvar,
    FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S * ((sigx6_4 * dmusig4 * depsi4 +
      (S * ((0.5 * sigx14_4 - 0.5 * pmusig4) * depsi4/(sigma_sq4) -
        S * sigx6_4 * dmusig4 * sigx15_4 * (epsilon4)) *
        (epsilon4) - 0.5 * (sigx1_4/(sigma_sq4)))/(sigma_sq4))/sigx9_4 -
      2 * (sigx1_4 * ewz4 * sigx8_4/(sigx9_4^2 * (sigma_sq4)))) *
      ewu4 * ewz4) * wHvar, FUN = "*"), uHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S * (((S * ((0.5 * sigx14_4 -
      0.5 * pmusig4) * depsi4/(sigma_sq4) + S * (0.5 *
      pvu4 + sigmastar4) * dmusig4 * sigx15_4 * ewu4 *
      (epsilon4)/ssq4^2) * (epsilon4) - 0.5 * (sigx1_4/(sigma_sq4)))/(sigma_sq4) -
      (0.5 * pvu4 + sigmastar4) * dmusig4 * depsi4 * ewu4/ssq4^2)/sigx9_4 -
      2 * (sigx1_4 * ewz4 * sigx11_4/(sigx9_4^2 * (sigma_sq4)))) *
      ewv4 * ewz4) * wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S^3 * prC * sigx1_4 * depsi5 *
      ewv5_h^3 * ewz4 * (epsilon5)/((sigx3 * ewv5_h^3)^2 *
      wzdeno * sigx21_4))) * wHvar, FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(2 * (S * sigx13 * prC * sigx1_4 *
      ewv5_h * ewz4/((sigx3 * ewv5_h)^2 * wzdeno * sigx21_4))) *
      wHvar, FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx27_1 * sigx1_4 * ewz1 * ewz4/(sigx21_4)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * sigx27_2 * sigx1_4 * ewz2 * ewz4/(sigx21_4)) *
      wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * sigx27_3 * sigx1_4 * ewz3 * ewz4/(sigx21_4)) *
      wHvar, FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    3 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = S * sigx27_4 * sigx1_4 * ewz4/(sigx21_4) * wHvar,
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 3 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((ewu4 * (S * (0.5 * sigx24_4 -
      (0.5 * (S^2 * sigx6_4 * depsi4 * (epsilon4)^2/(sigma_sq4)^2) -
        (((0.5 * (ewu4/(sigma_sq4)) + 1 - 0.5 * (0.5 *
          prU4 + ewu4/(sigma_sq4))) * prU4 * ewv4/sigmastar4 +
          (2 - 2 * ((0.5 * puv4 + sigmastar4)^2 * ewu4 *
          (sigma_sq4)/ssq4^2)) * sigmastar4)/ssq4^2 +
          S^2 * sigx6_4^2 * ewu4 * (epsilon4)^2/ssq4) *
          depsi4) * dmusig4) * (epsilon4) - 0.5 * ((S *
      sigx7_4 * (epsilon4) - depsi4 * pmusig4/(sigma_sq4))/(sigma_sq4))) +
      S * sigx7_4 * (epsilon4) - 0.5 * dpesq4)/sigx9_4 -
      sigx23_4 * ewu4 * sigx8_4/sigx9_4^2) * ewu4 * ewz4) *
      wHvar, FUN = "*"), uHvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((S * (((((0.5 * (prU4 * ewv4) -
      S^2 * (0.5 * pvu4 + sigmastar4) * sigx6_4 * ewu4 *
        (epsilon4)^2)/(sigma_sq4) + 0.5 * ((ewu4/(sigma_sq4) -
      1) * ewv4/(sigma_sq4) + 1 - 0.5 * (prU4 * prV4))) *
      depsi4/sigmastar4 + 0.5 * (S^2 * (0.5 * pvu4 + sigmastar4) *
      depsi4 * (epsilon4)^2/(sigma_sq4)^2)) * ewu4 + (0.5 *
      pvu4 + sigmastar4) * (1 - 2 * ((0.5 * puv4 + sigmastar4) *
      ewu4 * (sigma_sq4) * sigmastar4/ssq4^2)) * depsi4) *
      dmusig4/ssq4^2 + 0.5 * sigx24_4) * (epsilon4) - 0.5 *
      ((S * sigx7_4 * (epsilon4) - depsi4 * pmusig4/(sigma_sq4))/(sigma_sq4)))/sigx9_4 -
      sigx23_4 * sigx11_4/sigx9_4^2) * ewu4 * ewv4 * ewz4) *
      wHvar, FUN = "*"), vHvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewu4 *
      ewv5_h^3 * ewz4 * sigx8_4 * (epsilon5)/sigx25_4)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewu4 * ewv5_h *
      ewz4 * sigx8_4/sigx19_4)) * wHvar, FUN = "*"), vHvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx27_1 *
    ewu4 * ewz1 * ewz4 * sigx8_4/sqrt(sigma_sq4)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx27_2 * ewu4 * ewz2 * ewz4 *
      sigx8_4/sqrt(sigma_sq4)) * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx27_3 * ewu4 * ewz3 * ewz4 *
      sigx8_4/sqrt(sigma_sq4)) * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 3 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = sigx27_4 * ewu4 * ewz4 * sigx8_4/sqrt(sigma_sq4) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    3 * nvZVvar + 1):(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = 2 * (((S * ((((0.5 * (ewv4/(sigma_sq4)) -
      0.5 * (0.5 * prV4 + ewv4/(sigma_sq4))) * prV4 + S^2 *
      (0.5 * pvu4 + sigmastar4)^2 * ewu4 * ewv4 * (epsilon4)^2/(ssq4^2 *
      (sigma_sq4))) * depsi4 * ewu4/sigmastar4 + ((0.5 *
      (S^2 * depsi4 * (epsilon4)^2/(sigma_sq4)^2) - 2 *
      ((0.5 * pvu4 + sigmastar4) * depsi4 * (sigma_sq4) *
        sigmastar4/ssq4^2)) * ewv4 + depsi4) * (0.5 *
      pvu4 + sigmastar4)) * dmusig4 * ewu4/ssq4^2 + S *
      (0.5 * (ewv4 * (S * ((0.5 * pvu4 + sigmastar4) *
        dmusig4 * ewu4/ssq4^2 + 0.5 * sigx16_4) * (epsilon4) -
        2 * (pmusig4/(sigma_sq4)))) + 0.5 * pmusig4) *
      depsi4 * (epsilon4)/(sigma_sq4)^2) * (epsilon4) -
      (0.5 * (depsi4 * pmusig4) + 0.5 * (ewv4 * (S * sigx10_4 *
        (epsilon4) - depsi4 * pmusig4/(sigma_sq4))))/(sigma_sq4))/sigx9_4 -
      (0.5 * sigx22_4 + 2 * (ewz4 * sigx11_4)) * ewv4 *
        sigx11_4/sigx9_4^2) * ewv4 * ewz4) * wHvar, FUN = "*"),
    vHvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (S^2 * prC * depsi5 * ewv4 *
      ewv5_h^3 * ewz4 * sigx11_4 * (epsilon5)/sigx25_4)) *
      wHvar, FUN = "*"), Xvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(2 * (sigx13 * prC * ewv4 * ewv5_h *
      ewz4 * sigx11_4/sigx19_4)) * wHvar, FUN = "*"), vHvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx27_1 *
    ewv4 * ewz1 * ewz4 * sigx11_4/sqrt(sigma_sq4)) * wHvar,
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx27_2 * ewv4 * ewz2 * ewz4 *
      sigx11_4/sqrt(sigma_sq4)) * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx27_3 * ewv4 * ewz3 * ewz4 *
      sigx11_4/sqrt(sigma_sq4)) * wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 3 * nvZVvar + 1):(4 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx27_4 * ewv4 * ewz4 * sigx11_4/sqrt(sigma_sq4) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (4 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S^2 * ((S^2 * (epsilon5)^2/ewv5_h^2 -
      1)/(sigx3 * ewv5_h^3) - S^2 * prC * depsi5 * (epsilon5)^2/(sigx3 *
      ewv5_h^3)^2) * prC * depsi5 * wHvar, FUN = "*"),
    Xvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S^2 * ((0.5 * (S^2 * (epsilon5)^2/ewv5_h^2 -
      2) - 0.5)/(sigx3 * ewv5_h) - sigx13 * prC/(sigx3 *
      ewv5_h)^2) * prC * depsi5 * (epsilon5)/ewv5_h^2 *
      wHvar, FUN = "*"), vHvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S^2 *
    (wzdeno * sigx12_1/(sigx3 * wzdeno)^2 + 1/(sigx3 * wzdeno)) *
    prC * depsi5 * ewz1 * (epsilon5)/ewv5_h^3) * wHvar, FUN = "*"),
    Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S^2 * (wzdeno * sigx12_2/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * prC * depsi5 * ewz2 * (epsilon5)/ewv5_h^3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S^2 * (wzdeno * sigx12_3/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * prC * depsi5 * ewz3 * (epsilon5)/ewv5_h^3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 4 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S^2 * (wzdeno * sigx12_4/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * prC * depsi5 * ewz4 * (epsilon5)/ewv5_h^3) *
      wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    4 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = prC * (S^2 * (0.5 * (0.5 * (S^2 *
      (epsilon5)^2/ewv5_h^2) - 1) - 0.25) * depsi5 * (epsilon5)^2/(sigx3 *
      ewv5_h^3) - (sigx13 * prC + 0.5 * (sigx3 * ewv5_h)) *
      sigx13/(sigx3 * ewv5_h)^2) * wHvar, FUN = "*"), vHvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar +
    nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -((wzdeno *
    sigx12_1/(sigx3 * wzdeno)^2 + 1/(sigx3 * wzdeno)) * sigx13 *
    prC * ewz1/ewv5_h) * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -((wzdeno * sigx12_2/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * sigx13 * prC * ewz2/ewv5_h) *
      wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -((wzdeno * sigx12_3/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * sigx13 * prC * ewz3/ewv5_h) *
      wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 4 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar), (5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -((wzdeno * sigx12_4/(sigx3 * wzdeno)^2 +
      1/(sigx3 * wzdeno)) * sigx13 * prC * ewz4/ewv5_h) *
      wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar + nZHvar), (5 * nXvar + 4 *
    nuZUvar + 5 * nvZVvar + 1):(5 * nXvar + 4 * nuZUvar +
    5 * nvZVvar + nZHvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = ((1 - ewz1/wzdeno)/(sigx3 * wzdeno) - 2 * (depsi1 *
      ewz1 * pmusig1/sigx25_1)) * sigx12_1 * ewz1 * wHvar,
    FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar + nZHvar), (5 * nXvar + 4 *
    nuZUvar + 5 * nvZVvar + nZHvar + 1):(5 * nXvar + 4 *
    nuZUvar + 5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx12_1/(sigx3 * wzdeno^2) +
      2 * (sigx12_2 * depsi1 * pmusig1/sigx25_1)) * ewz1 *
      ewz2) * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar + nZHvar), (5 * nXvar + 4 *
    nuZUvar + 5 * nvZVvar + 2 * nZHvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx12_1/(sigx3 * wzdeno^2) +
      2 * (sigx12_3 * depsi1 * pmusig1/sigx25_1)) * ewz1 *
      ewz3) * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar + nZHvar), (5 * nXvar + 4 *
    nuZUvar + 5 * nvZVvar + 3 * nZHvar + 1):(5 * nXvar +
    4 * nuZUvar + 5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx12_1/(sigx3 * wzdeno^2) +
      2 * (sigx12_4 * depsi1 * pmusig1/sigx25_1)) * ewz1 *
      ewz4) * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar + 1):(5 *
      nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz2/wzdeno)/(sigx3 * wzdeno) -
      2 * (depsi2 * ewz2 * pmusig2/sigx25_2)) * sigx12_2 *
      ewz2 * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar +
      1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx12_2/(sigx3 * wzdeno^2) +
      2 * (sigx12_3 * depsi2 * pmusig2/sigx25_2)) * ewz2 *
      ewz3) * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar +
    ...
    MARGIN = 1, STATS = -((sigx12_2/(sigx3 * wzdeno^2) +
      2 * (sigx12_4 * depsi2 * pmusig2/sigx25_2)) * ewz2 *
      ewz4) * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar +
      1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz3/wzdeno)/(sigx3 * wzdeno) -
      2 * (depsi3 * ewz3 * pmusig3/sigx26_3)) * sigx12_3 *
      ewz3 * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 2 * nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar +
      1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx12_3/(sigx3 * wzdeno^2) +
      2 * (sigx12_4 * depsi3 * pmusig3/sigx26_3)) * ewz3 *
      ewz4) * wHvar, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar +
    1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 4 * nZHvar),
    (5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 3 * nZHvar +
      1):(5 * nXvar + 4 * nuZUvar + 5 * nvZVvar + 4 * nZHvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz4/wzdeno)/(sigx3 * wzdeno) -
      2 * (depsi4 * ewz4 * pmusig4/sigx26_4)) * sigx12_4 *
      ewz4 * wHvar, FUN = "*"), Zvar)
...
#' @noRd
GZISF5ChnormAlgOpt <- function(start, olsParam, dataTable, S,
  ...
  startLoglik <- sum(cGZISFhalfnormlike5C(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(cGZISFhalfnormlike5C(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
      Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradGZISFhalfnormlike5C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
    hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      maxeval = itermax, stepmax = stepmax, xtol = tol,
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cGZISFhalfnormlike5C,
    grad = cgradGZISFhalfnormlike5C, hess = chessGZISFhalfnormlike5C,
    ...
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(cGZISFhalfnormlike5C(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradGZISFhalfnormlike5C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      ...
      fn = function(parm) -sum(cGZISFhalfnormlike5C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gr = function(parm) -colSums(cgradGZISFhalfnormlike5C(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hs = function(parm) as(-chessGZISFhalfnormlike5C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      ...
      gr = function(parm) -colSums(cgradGZISFhalfnormlike5C(parm,
        ...
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hess = function(parm) -chessGZISFhalfnormlike5C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
      print.info = printInfo, maxiter = itermax, epsa = gradtol,
      epsb = gradtol), nlminb = nlminb(start = startVal,
      objective = function(parm) -sum(cGZISFhalfnormlike5C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      gradient = function(parm) -colSums(cgradGZISFhalfnormlike5C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)),
      hessian = function(parm) -chessGZISFhalfnormlike5C(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar),
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradGZISFhalfnormlike5C(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessGZISFhalfnormlike5C(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
      mleObj$hessian <- chessGZISFhalfnormlike5C(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
  }
  mleObj$logL_OBS <- cGZISFhalfnormlike5C(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradGZISFhalfnormlike5C(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)
...
#' post. prob. and efficiencies for gzisf 5 classes halfnormal-normal distribution
...
cGZISF5Chalfnormeff <- function(object, level) {
  ...
  beta5 <- object$mlParam[(4 * object$nXvar + 4 * object$nuZUvar +
    ...
    4 * object$nvZVvar)]
  phi5 <- object$mlParam[(5 * object$nXvar + 4 * object$nuZUvar +
    4 * object$nvZVvar + 1):(5 * object$nXvar + 4 * object$nuZUvar +
    5 * object$nvZVvar)]
  theta1 <- object$mlParam[(5 * object$nXvar + 4 * object$nuZUvar +
    5 * object$nvZVvar + 1):(5 * object$nXvar + 4 * object$nuZUvar +
    5 * object$nvZVvar + object$nZHvar)]
  theta2 <- object$mlParam[(5 * object$nXvar + 4 * object$nuZUvar +
    5 * object$nvZVvar + object$nZHvar + 1):(5 * object$nXvar +
    4 * object$nuZUvar + 5 * object$nvZVvar + 2 * object$nZHvar)]
  theta3 <- object$mlParam[(5 * object$nXvar + 4 * object$nuZUvar +
    5 * object$nvZVvar + 2 * object$nZHvar + 1):(5 * object$nXvar +
    4 * object$nuZUvar + 5 * object$nvZVvar + 3 * object$nZHvar)]
  theta4 <- object$mlParam[(5 * object$nXvar + 4 * object$nuZUvar +
    5 * object$nvZVvar + 3 * object$nZHvar + 1):(5 * object$nXvar +
    4 * object$nuZUvar + 5 * object$nvZVvar + 4 * object$nZHvar)]
...
#' marginal effects for for gzisf 5 classes halfnormal-normal distribution
#' @param object object of class sfacross
#' @noRd
cmargGZISF5Chalfnorm_Eu <- function(object) {
  ...
  colnames(margEff_c4) <- paste0("Eu_", colnames(uHvar)[-1],
    "_c4")
  margEff_c5 <- matrix(0, nrow = object$Nobs, ncol = object$nuZUvar -
    1)
