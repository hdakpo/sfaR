...
#' @noRd
praynormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
...
#' @noRd
pstraynorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradraynormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  sigx7 <- (1/(ssqx2) - (0.5 * sigx6 + sigmastar * gisq) * ewu/ssq)
  ...
  sigx14 <- (gid1_epsi_gisq/(ssqx2) - ewu * sigx13 * gid2_gicub * giepsi/ssq)
  sigx15 <- (0.5 * (dmusig * ewv * gid2_gicub/(ssqx2)) + S^2 * dmusig * sigx14 *
    giepsi)
  sigx16 <- (pmusig * gid1_epsi_gisq - ewu * (pmusig * gid2_gicub/sigmasq + S *
    dmusig * sigx14) * giepsi)
  sigx17 <- ((sigx15 * ewu + S * sigx16)/sigx3 + 0.5 * gid2_gicub)
  sigx18 <- (gid1sq_epsi_gisq/(ssqx2) - ewu * sigx13 * gid2sq_gicub * giepsi/ssq)
  sigx19 <- (0.5 * (dmusig * ewv * gid2sq_gicub/(ssqx2)) + S^2 * dmusig * sigx18 *
    giepsi)
  sigx20 <- (sigx19 * ewu + S * (pmusig * gid1sq_epsi_gisq - ewu * (pmusig * gid2sq_gicub/sigmasq +
    S * dmusig * sigx18) * giepsi))
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * ewu * sigx4/sigmasq, FUN = "*") -
    ...
    MARGIN = 1, STATS = ((sigx12/sigmasq + 2 * (epsilon_isq/(2 * ewv)^2)) * ewv -
      0.5 * (TT - 1)), FUN = "*"), ewu * (S^2 * ewu * sigx14 * giepsi/sigmastar -
    sigx17)/sigmasq, ewu * (S^2 * ewu * sigx18 * giepsi/sigmastar - sigx21)/sigmasq)
...
#' @param TT vector of time of presence
#' @noRd
phessraynormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx26 <- (ssg6 * dmusig * ewv/ssq + S^2 * sigx7 * dmusig * giepsi^2/sigmasq)
  sigx27 <- ((((0.5 * sigx22 - 0.5)/(ssqx2) - 0.5 * sigx7) * ewv - 2 * (ssg6 *
    ssqx2 * sigx13/ssq)) * ewu + sigmastar)
  sigx28 <- (S^2 * ewu * sigx18 * giepsi/sigmastar - sigx21)
  sigx29 <- (S^2 * ewu * sigx14 * giepsi/sigmastar - sigx17)
  sigx30 <- ((dmusig + S^2 * sigx11 * dmusig * ewu^2 * ewv * giepsi^2/(ssq * ssqx2))/(ssqx2) -
    sigx11 * dmusig * ewv/ssq)
  sigx31 <- (S * sigx11 * dmusig * ewu * giepsi/ssq - pmusig/sigmasq)
  sigx32 <- ((0.5 * ((1 - ewv/sigmasq)/(ssqx2)) - 0.5 * (1/(ssqx2) - sigx11 * ewv/ssq)) *
    ewu - 2 * (sigx11 * ssqx2 * sigx13/ssq))
  sigx33 <- (0.5 * (sigx13/ssq) - 0.5/(sigmasq^2 * sigmastar))
  sigx34 <- (sigx33 * ewu^2 * ewv * gid2_gicub^2 + sigx13 * gd2Zsq_gd1_gicub)
  sigx35 <- (sigx34 * giepsi + (2 * gid1_epsi_gisq - 2 * (ewu * ssqx2 * sigx13 *
    gid2_gicub * giepsi/ssq)) * sigx13 * gid2_gicub)
  sigx36 <- (sigx35/ssq + S^2 * ewu * sigx14^2 * giepsi/(ssqx2))
  sigx37 <- (sigx33 * ewu^2 * ewv * gid2sq_gicub^2 + sigx13 * gd2Zfour_gd1_gicub)
  sigx38 <- (sigx37 * giepsi + (2 * gid1sq_epsi_gisq - 2 * (ewu * ssqx2 * sigx13 *
    gid2sq_gicub * giepsi/ssq)) * sigx13 * gid2sq_gicub)
  sigx39 <- (gd1Zfour_gd1_epsi_gisq/(ssqx2) - (sigx38/ssq + S^2 * ewu * sigx18^2 *
    giepsi/(ssqx2)) * ewu)
  sigx40 <- (sigx33 * ewu^2 * ewv * gid2_gicub * gid2sq_gicub + sigx13 * gd2Zcub_gd1_gicub)
  sigx41 <- (sigx40 * giepsi + sigx13 * (gid2_gicub * gid1sq_epsi_gisq + gid2sq_gicub *
    (gid1_epsi_gisq - 2 * (ewu * ssqx2 * sigx13 * gid2_gicub * giepsi/ssq))))
  sigx42 <- (gd1Zcub_gd1_epsi_gisq/(ssqx2) - (sigx41/ssq + S^2 * ewu * sigx14 *
    sigx18 * giepsi/(ssqx2)) * ewu)
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    S^2 * (1/ewv - (((1 - S^2 * ewu * giepsi^2/(sigmasq * ewv)) * sigx1 - ewu/(ssqx2)) *
    dmusig + ewu * sigx2^2/(sigx3 * sigmasq))/sigx3) * ewu/sigmasq, FUN = "*"),
    Xgi) - sapply(1:nXvar, function(x) crossprod(Xsq[[x]], as.matrix(wHvar/(2 *
    ewv))))
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * S * (((sigx5 * sigx2/sigx3 + 0.5 * (S * dmusig * giepsi/sigmastar)) *
      ewu/sigmasq - pmusig) * sigx22/sigx3 + 2 * (S * sigx7 * ewu * giepsi/sigmastar)) *
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xepsi_i,
    ...
    MARGIN = 1, STATS = wHvar * S * (((sigx10 * sigx2/sigx3 - S * (0.5 * ((1 -
      ewv/sigmasq)/ewv) + 1/sigmasq) * dmusig * giepsi/sigmastar) * ewu + pmusig)/(sigx3 *
      sigmasq) - 2 * (S * sigx11 * ewu * giepsi/(ssq * sigmastar))) * ewu/sigmasq *
      ewv, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * sigx14 * S * ewu/sigmastar * S * ewu/sigmasq, FUN = "*") +
    sweep(Xi_gid1_gisq, MARGIN = 1, STATS = wHvar * giepsi/(ssqx2) * S * ewu/sigmastar *
      S * ewu/sigmasq, FUN = "*") - sweep(Xgi, MARGIN = 1, STATS = wHvar *
    ewu * sigx13 * gid2_gicub/ssq * giepsi * S * ewu/sigmastar * S * ewu/sigmasq,
    FUN = "*") - sweep(Xgi, MARGIN = 1, STATS = wHvar * ((sigx15 * ewu + S *
    sigx16) * sigx2/sigx3 + S * (0.5 * (ewu * gid2_gicub * giepsi/sigmasq) -
    gid1_epsi_gisq) * dmusig/sigmastar - pmusig * gid2_gicub) * ewu/sigmasq/sigx3 *
    S * ewu/sigmasq, FUN = "*") - sweep(Xi_gid1_gisq, MARGIN = 1, STATS = wHvar *
    pmusig/sigx3 * S * ewu/sigmasq, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * sigx18 * S * ewu/sigmastar * S * ewu/sigmasq, FUN = "*") +
    sweep(Xi_gid1sq_gisq, MARGIN = 1, STATS = wHvar * giepsi/(ssqx2) * S * ewu/sigmastar *
      S * ewu/sigmasq, FUN = "*") - sweep(Xgi, MARGIN = 1, STATS = wHvar *
    ewu * sigx13 * gid2sq_gicub/ssq * giepsi * S * ewu/sigmastar * S * ewu/sigmasq,
    FUN = "*") - sweep(Xgi, MARGIN = 1, STATS = wHvar * (sigx20 * sigx2/sigx3 +
    S * (0.5 * (ewu * gid2sq_gicub * giepsi/sigmasq) - gid1sq_epsi_gisq) * dmusig/sigmastar -
    pmusig * gid2sq_gicub) * ewu/sigmasq/sigx3 * S * ewu/sigmasq, FUN = "*") -
    sweep(Xi_gid1sq_gisq, MARGIN = 1, STATS = wHvar * pmusig/sigx3 * S * ewu/sigmasq,
      FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((0.5 * (dmusig * ewv/sigmastar) + ewu * (S^2 *
      sigx7 * dmusig * giepsi^2 - (sigx5 * sigx22/sigx3 + gisq) * sigx5/sigmasq) -
      (0.5 * (0.5 * (sigx22 * dmusig * ewv/sigmastar) + S^2 * sigx7 * dmusig *
        ewu * giepsi^2) + S * pmusig * giepsi)) * sigx22/sigx3 + ewu * (S^2 *
      (2/(ssqx2) - (((0.5 * (ewu * gisq/sigmasq) + 2 - 0.5 * (0.5 * sigx22 +
        ewu * gisq/sigmasq)) * sigx22 * ewv/sigmastar + (4 * gisq - 2 * (ssg6^2 *
        ewu * sigmasq/ssq)) * sigmastar) * ewu/ssq + 0.5 * (sigx22 * sigx7))) *
      giepsi^2/sigmastar - sigx8 * gisq/sigmasq) - 0.5 * gisq) * ewu/sigmasq,
    FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (((0.5 *
    ((1 - ewv/sigmasq) * dmusig) + 0.5 * ((dmusig * ewv * gisq/sigmasq - S^2 *
    (1 - ewv/sigmasq) * sigx7 * dmusig * ewu * giepsi^2/sigmastar) * ewu/sigmasq -
    0.5 * (sigx22 * (1 - ewv/sigmasq) * dmusig)))/sigmastar + (S * pmusig * giepsi -
    (sigx10 * sigx5 * sigx22/sigx3 + S * (pmusig * gisq/sigmasq + S * sigx7 *
      dmusig * giepsi) * giepsi) * ewu)/sigmasq)/sigx3 - (sigx12 * gisq/sigmasq +
    S^2 * (((0.5 * (sigx22 * ewv/sigmasq) + 0.5 * ((ewu * gisq/sigmasq - 1) *
      ewv/sigmasq + 1 - 0.5 * (sigx22 * (1 - ewv/sigmasq))) + 0.5 * (1 - ewv/sigmasq)) *
      ewu/sigmastar + sigmastar)/(ssq * sigmastar) + sigx11 * (1/(ssq * sigmastar) -
      (0.5 * (ssq * sigx22/(ssqx2)) + 2 * (ssg6 * ewu)) * ewu * ewv/(ssq *
        sigmastar)^2)) * ewu * giepsi^2)) * ewu * ewv/sigmasq, FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ewu * (ewu * (S^2 * (2 * (gid1_epsi_gisq/(ssqx2)) -
      ((sigx25 * gid2_gicub * giepsi + ssg6 * gid1_epsi_gisq) * ewu/ssq + 0.5 *
        (sigx22 * sigx14))) * giepsi/sigmastar - (((0.5 * (dmusig * ewv *
      gid2_gicub/sigmastar) - (sigx15 * ewu + S * sigx16) * sigx5 * sigx22/sigx3)/sigmasq +
      S * (((pmusig * gisq/sigmasq + S * sigx7 * dmusig * giepsi) * gid2_gicub/sigmasq +
        S * ((sigx27 * gid2_gicub * giepsi + ssg6 * gid1_epsi_gisq)/ssq +
          S^2 * sigx7 * ewu * sigx14 * giepsi^2/(ssqx2)) * dmusig) * ewu +
        S * (ssg6 * ewu/ssq - 1/(ssqx2)) * dmusig * gid1_epsi_gisq - pmusig *
        gid2_gicub/sigmasq) * giepsi - (0.5 * (sigx26 * gid2_gicub) + S^2 *
      ((sigx27 * gid2_gicub * giepsi + ssg6 * gid1_epsi_gisq)/ssq + S^2 * sigx7 *
        ewu * sigx14 * giepsi^2/(ssqx2)) * dmusig * giepsi) * ewu)/sigx3 +
      sigx29 * gisq/sigmasq)) - sigx17)/sigmasq, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 2)] <- colSums(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ewu * (ewu * (S^2 * (2 * (gid1sq_epsi_gisq/(ssqx2)) -
      ((sigx25 * gid2sq_gicub * giepsi + ssg6 * gid1sq_epsi_gisq) * ewu/ssq +
        0.5 * (sigx22 * sigx18))) * giepsi/sigmastar - (((0.5 * (dmusig *
      ewv * gid2sq_gicub/sigmastar) - sigx20 * sigx5 * sigx22/sigx3)/sigmasq +
      S * (((pmusig * gisq/sigmasq + S * sigx7 * dmusig * giepsi) * gid2sq_gicub/sigmasq +
        S * ((sigx27 * gid2sq_gicub * giepsi + ssg6 * gid1sq_epsi_gisq)/ssq +
          S^2 * sigx7 * ewu * sigx18 * giepsi^2/(ssqx2)) * dmusig) * ewu +
        S * (ssg6 * ewu/ssq - 1/(ssqx2)) * dmusig * gid1sq_epsi_gisq - pmusig *
        gid2sq_gicub/sigmasq) * giepsi - (0.5 * (sigx26 * gid2sq_gicub) +
      S^2 * ((sigx27 * gid2sq_gicub * giepsi + ssg6 * gid1sq_epsi_gisq)/ssq +
        S^2 * sigx7 * ewu * sigx18 * giepsi^2/(ssqx2)) * dmusig * giepsi) *
      ewu)/sigx3 + sigx28 * gisq/sigmasq)) - sigx21)/sigmasq, FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    (((((0.5 * (ewv * (S^2 * sigx11 * dmusig * ewu^2 * giepsi^2/(ssq * sigmastar) -
      dmusig)/sigmasq - 0.5 * ((1 - ewv/sigmasq) * dmusig)) + 0.5 * dmusig) *
      (1 - ewv/sigmasq)/sigmastar + (ewv * (S * sigx31 * giepsi - sigx10^2 *
      ewu/sigx3) + S * pmusig * giepsi)/sigmasq)/sigx3 - S^2 * (sigx11 * (1/(ssq *
      sigmastar) - (0.5 * (ssq * (1 - ewv/sigmasq)/(ssqx2)) + 2 * (sigx11 *
      ewv)) * ewu * ewv/(ssq * sigmastar)^2) + (0.5 * (ewv/sigmasq) - 0.5 *
      (0.5 * (1 - ewv/sigmasq) + ewv/sigmasq)) * (1 - ewv/sigmasq) * sigmasq/(ssq *
      ewv)) * ewu * giepsi^2) * ewu - (sigx12 * ewv/sigmasq + 0.5))/sigmasq +
      (2 - 16 * (ewv^2/(2 * ewv)^2)) * epsilon_isq/(2 * ewv)^2) * ewv, FUN = "*"),
    vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 1)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = -wHvar * ((((0.5 *
    (sigx30 * gid2_gicub) + S * (S * sigx11 * dmusig * gid1_epsi_gisq/ssq - sigx31 *
    gid2_gicub/sigmasq) * giepsi - (sigx15 * ewu + S * sigx16) * sigx10/(sigx3 *
    sigmasq)) * ewv/sigx3 + S^2 * ((sigx32 * ewu * gid2_gicub * giepsi + sigx11 *
    gid1_epsi_gisq) * ewv/ssq + 0.5 * ((1 - ewv/sigmasq) * sigx14)) * giepsi/sigmastar) *
    ewu + ewv * sigx29/sigmasq) * ewu/sigmasq), FUN = "*"))
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 2)] <- colSums(sweep(vHvar, MARGIN = 1, STATS = -wHvar * ((((0.5 *
    (sigx30 * gid2sq_gicub) + S * (S * sigx11 * dmusig * gid1sq_epsi_gisq/ssq -
    sigx31 * gid2sq_gicub/sigmasq) * giepsi - sigx20 * sigx10/(sigx3 * sigmasq)) *
    ewv/sigx3 + S^2 * ((sigx32 * ewu * gid2sq_gicub * giepsi + sigx11 * gid1sq_epsi_gisq) *
    ewv/ssq + 0.5 * ((1 - ewv/sigmasq) * sigx18)) * giepsi/sigmastar) * ewu +
    ewv * sigx28/sigmasq) * ewu/sigmasq), FUN = "*"))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 1)] <- sum(wHvar *
    (ewu * (ewu * (S^2 * ((0.5 * (ewu * gid2_gicub * giepsi/sigmasq) + gid1_epsi_gisq) *
      sigx14 + (gd1Zsq_gd1_epsi_gisq/(ssqx2) - sigx35 * ewu/ssq) * giepsi)/sigmastar -
      sigx29 * gid2_gicub/sigmasq) - ((((sigx15 * ewu + S * sigx16)^2/(sigx3 *
      sigmasq) + 0.5 * (((dmusig * gd2Zsq_gd1_gicub - S^2 * dmusig * ewu^2 *
      gid2_gicub * sigx14 * giepsi/(ssqx2))/(ssqx2) - dmusig * ewu * sigx13 *
      gid2_gicub^2/ssq) * ewv) + S^2 * (sigx43 * giepsi + gid1_epsi_gisq *
      sigx14) * dmusig) * ewu + S * (pmusig * gd1Zsq_gd1_epsi_gisq - (((pmusig *
      gd2Zsq_gd1_gicub - ewu * (pmusig * gid2_gicub/sigmasq + S * dmusig *
      sigx14) * gid2_gicub)/sigmasq + S * dmusig * sigx43) * giepsi + (2 *
      (S * dmusig * sigx14) + pmusig * gid2_gicub/sigmasq) * gid1_epsi_gisq) *
      ewu))/sigx3 + 0.5 * gd2Zsq_gd1_gicub))/sigmasq))
  hessll[(nXvar + nuZUvar + nvZVvar + 1), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    (ewu * (ewu * (S^2 * ((0.5 * (ewu * gid2_gicub * giepsi/sigmasq) + gid1_epsi_gisq) *
      sigx18 + (gd1Zcub_gd1_epsi_gisq/(ssqx2) - sigx41 * ewu/ssq) * giepsi)/sigmastar -
      sigx28 * gid2_gicub/sigmasq) - ((((sigx15 * ewu + S * sigx16) * sigx20/(sigx3 *
      sigmasq) + 0.5 * (((dmusig * gd2Zcub_gd1_gicub - S^2 * dmusig * ewu^2 *
      gid2sq_gicub * sigx14 * giepsi/(ssqx2))/(ssqx2) - dmusig * ewu * sigx13 *
      gid2_gicub * gid2sq_gicub/ssq) * ewv) + S^2 * (sigx42 * giepsi + gid1_epsi_gisq *
      sigx18) * dmusig) * ewu + S * (pmusig * gd1Zcub_gd1_epsi_gisq - (((pmusig *
      gd2Zcub_gd1_gicub - ewu * (pmusig * gid2_gicub/sigmasq + S * dmusig *
      sigx14) * gid2sq_gicub)/sigmasq + S * dmusig * sigx42) * giepsi + (pmusig *
      gid2sq_gicub/sigmasq + S * dmusig * sigx18) * gid1_epsi_gisq + S * dmusig *
      sigx14 * gid1sq_epsi_gisq) * ewu))/sigx3 + 0.5 * gd2Zcub_gd1_gicub))/sigmasq))
  hessll[(nXvar + nuZUvar + nvZVvar + 2), (nXvar + nuZUvar + nvZVvar + 2)] <- sum(wHvar *
    (ewu * (ewu * (S^2 * ((0.5 * (ewu * gid2sq_gicub * giepsi/sigmasq) + gid1sq_epsi_gisq) *
      sigx18 + (gd1Zfour_gd1_epsi_gisq/(ssqx2) - sigx38 * ewu/ssq) * giepsi)/sigmastar -
      sigx28 * gid2sq_gicub/sigmasq) - (((sigx20^2/(sigx3 * sigmasq) + 0.5 *
      (((dmusig * gd2Zfour_gd1_gicub - S^2 * dmusig * ewu^2 * gid2sq_gicub *
        sigx18 * giepsi/(ssqx2))/(ssqx2) - dmusig * ewu * sigx13 * gid2sq_gicub^2/ssq) *
        ewv) + S^2 * (sigx39 * giepsi + gid1sq_epsi_gisq * sigx18) * dmusig) *
      ewu + S * (pmusig * gd1Zfour_gd1_epsi_gisq - (((pmusig * gd2Zfour_gd1_gicub -
      ewu * (pmusig * gid2sq_gicub/sigmasq + S * dmusig * sigx18) * gid2sq_gicub)/sigmasq +
      S * dmusig * sigx39) * giepsi + (2 * (S * dmusig * sigx18) + pmusig *
      gid2sq_gicub/sigmasq) * gid1sq_epsi_gisq) * ewu))/sigx3 + 0.5 * gd2Zfour_gd1_gicub))/sigmasq))
...
#' @noRd
raynormAlgOpt_k90 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(praynormlike_k90(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(praynormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradraynormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = praynormlike_k90,
    grad = pgradraynormlike_k90, hess = phessraynormlike_k90, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phessraynormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      ...
      -phessraynormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradraynormlike_k90(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessraynormlike_k90(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phessraynormlike_k90(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- praynormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradraynormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
