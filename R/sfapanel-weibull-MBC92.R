...
#' @noRd
pweibullnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
pstweibullnorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar,
  vHvar, Yvar, Xvar, S, wHvar, printInfo, tol, N, FiMat, whichStart, initIter,
  initAlg) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradweibullnormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  lFi1 <- sweep(lFimat, MARGIN = 1, STATS = (S * ewu_h * Zi_epsi), FUN = "*")
  lFi2 <- sweep(lFimat, MARGIN = 1, STATS = (S * ewu_h * Zisq_epsi), FUN = "*")
  ...
  lFi9 <- sweep(((lFi4 + lFi5) * lFiuv), MARGIN = 1, STATS = (ewu_h/(2 * ewv)),
    FUN = "*")
  lFi10 <- sweep((lFimat)^2, MARGIN = 1, STATS = ewu_h^2 * giZi, FUN = "*")
  lFi11 <- sweep((lFimat)^2, MARGIN = 1, STATS = ewu_h^2 * giZisq, FUN = "*")
...
#' @noRd
weibullnormAlgOpt_mbc92 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  startLoglik <- sum(pweibullnormlike_mbc92(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pweibullnormlike_mbc92(parm,
    ...
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pweibullnormlike_mbc92,
    grad = pgradweibullnormlike_mbc92, start = startVal, finalHessian = if (hessianType ==
    ...
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pweibullnormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      fn = function(parm) -sum(pweibullnormlike_mbc92(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
        N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_mbc92(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradweibullnormlike_mbc92(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      objective = function(parm) -sum(pweibullnormlike_mbc92(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradweibullnormlike_mbc92(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
      Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  }
  ...
  mleObj$logL_OBS <- pweibullnormlike_mbc92(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradweibullnormlike_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffWeibull_mbc92 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffWeibull_mbc92 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq, giepsi,
  gisq, git) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffWeibull_mbc92 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_isq,
  giepsi, gisq, git) {
  exp(git * u) * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) *
    exp(-(u/sigmaU)^k) * exp(-(epsilon_isq + 2 * S * u * giepsi + u^2 * gisq)/(2 *
    sigmaV^2))
...
pweibullnormeff_mbc92 <- function(object, level) {
  ...
  for (i in seq_along(1:object$Nid)) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * giepsi[i] + ur^2 * gisq[i])/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffWeibull_mbc92, lowerLimit = 0, upperLimit = Inf,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in seq_along(1:object$Nid)) {
      res$teBC[i] <- hcubature(f = fnCondBCEffWeibull_mbc92, lowerLimit = 0,
