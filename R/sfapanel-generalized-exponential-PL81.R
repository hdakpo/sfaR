...
#' @noRd
pgenexponormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  ...
  ll <- log(2) - 1/2 * Wu - (TT - 1)/2 * Wv - 1/2 * log(TT) -
...
#' @param TT vector of time of presence
#' @noRd
pgradgenexponormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  Xsig1 <- sweep(2 * Xepsi_i, MARGIN = 1, STATS = pmusig2/ewv,
    FUN = "*") - sweep(X_iM, MARGIN = 1, STATS = 2 * (S *
    epsi_vu2/TT) * pmusig2/ewv, FUN = "*")
  Xsig2 <- sweep(X_iM, MARGIN = 1, STATS = S * dmusig2/ssq_v,
    FUN = "*")
  Xsig3 <- sweep(2 * Xepsi_i, MARGIN = 1, STATS = pmusig/ewv,
    FUN = "*") - sweep(X_iM, MARGIN = 1, STATS = 2 * (S *
    epsi_vu/TT) * pmusig/ewv, FUN = "*")
  Xsig4 <- sweep(X_iM, MARGIN = 1, STATS = S * dmusig/ssq_v,
    FUN = "*")
  Xsig5 <- sweep((0.5 * Xsig1 + Xsig2), MARGIN = 1, STATS = sigx3_2,
    FUN = "*") - sweep((0.5 * Xsig3 + Xsig4), MARGIN = 1,
    STATS = sigx3_1, FUN = "*")
  gradll <- cbind(sweep(Xsig5, MARGIN = 1, STATS = 1/sigx4,
    FUN = "*"), sweep(uHvar, MARGIN = 1, STATS = (sigx7/ttsq -
    0.5), FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = (sigx6/sigx4 -
    0.5 * (TT - 1)), FUN = "*"))
...
#' @param TT vector of time of presence
#' @noRd
phessgenexponormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  wutsq <- (wutt/(wutt)^2)
  Xsig6 <- sweep(X_iM, MARGIN = 1, STATS = (S * (1/(wutt) -
    epsi_vu/ssq_v^2)), FUN = "*")
  Xsig7 <- sweep(X_iM, MARGIN = 1, STATS = (S * (2/(wutt) -
    epsi_vu2/ssq_v^2)), FUN = "*")
  Xsig8 <- sweep(X_iM, MARGIN = 1, STATS = S * pmusig2, FUN = "*")
  Xsig9 <- sweep(X_iM, MARGIN = 1, STATS = S * (0.5 * (sigx11) +
    0.5 * (pmusig - sigx11)), FUN = "*")
  Xsig10 <- sweep(X_iM, MARGIN = 1, STATS = S * epsivusq2 *
    dmusig2/ssq_v, FUN = "*")
  Xsig11 <- sweep(X_iM, MARGIN = 1, STATS = S * epsivusq1 *
    dmusig/ssq_v, FUN = "*")
  Xsig12 <- sweep(X_iM, MARGIN = 1, STATS = S * (sigx2_1 *
    epsi_vu/(TT * ewv) + 0.5/ssq_v^2) * dmusig * ewv/sqrt(wvtt),
    FUN = "*")
  Xsig13 <- sweep(X_iM, MARGIN = 1, STATS = S * (epsi_vu2 *
    wusig/(TT * ewv) + 0.5/ssq_v^2) * dmusig2 * ewv/sqrt(wvtt),
    FUN = "*")
  Xsig14 <- sweep(X_iM, MARGIN = 1, STATS = (S * epsi_vu2/TT),
    FUN = "*")
  Xsig15 <- sweep(X_iM, MARGIN = 1, STATS = (S * epsi_vu/TT),
    FUN = "*")
  Xsig16 <- sweep(X_iM, MARGIN = 1, STATS = 1/ssq_v, FUN = "*")
  Xsig17 <- sweep(X_iM, MARGIN = 1, STATS = S * pmusig, FUN = "*")
  Xsig18 <- sweep((2 * Xepsi_i - 2 * Xsig14), MARGIN = 1, STATS = S *
    dmusig2, FUN = "*")
  Xsig19 <- sweep((2 * Xepsi_i - 2 * Xsig14), MARGIN = 1, STATS = (dw2pmu/ewv),
    FUN = "*")
  Xsig20 <- sweep((2 * Xepsi_i - 2 * Xsig15), MARGIN = 1, STATS = (dw1pmu/ewv),
    FUN = "*")
  Xsig21 <- sweep(Xsig5, MARGIN = 1, STATS = TT * sigx7 * ewu_h/ttsq^2,
    FUN = "*")
  Xsig22 <- sweep(Xepsi_i, MARGIN = 1, STATS = 1/ewv, FUN = "*")
  Xsig23 <- sweep((0.5 * Xsig19 + Xsig8), MARGIN = 1, STATS = sigx3_2,
    FUN = "*")
  Xsig24 <- sweep((0.5 * Xsig20 + Xsig9), MARGIN = 1, STATS = sigx3_1,
    FUN = "*")
  Xsig25 <- sweep((2 * Xepsi_i - 2 * Xsig15), MARGIN = 1, STATS = (sigx5_1/ewv),
    FUN = "*")
  Xsig26 <- sweep((2 * Xsig7 + 2 * Xsig22), MARGIN = 1, STATS = pmusig2,
    FUN = "*")
  Xsig27 <- sweep((2 * Xepsi_i - 2 * Xsig14), MARGIN = 1, STATS = (sigx5_2/ewv),
    FUN = "*")
  Xsig28 <- sweep((2 * Xsig6 + 2 * Xsig22), MARGIN = 1, STATS = pmusig,
    FUN = "*")
  Xsig29 <- sweep(Xsig5, MARGIN = 1, STATS = sigx6/sigx4, FUN = "*")
  Xsig30 <- sweep((0.5 * (Xsig28 - Xsig11) + Xsig12 - 0.5 *
    Xsig25), MARGIN = 1, STATS = sigx3_1, FUN = "*")
  Xsig31 <- sweep((0.5 * (Xsig26 - Xsig10) + Xsig13 - 0.5 *
    Xsig27), MARGIN = 1, STATS = sigx3_2, FUN = "*")
  Xsig32 <- sweep((Xsig30 - (Xsig29 + Xsig31)), MARGIN = 1,
    STATS = 1/sigx4, FUN = "*")
  Xsig33 <- sweep((Xsig23 - Xsig24), MARGIN = 1, STATS = 1/ttsq,
    FUN = "*")
  Xsig34 <- sweep((2 * Xepsi_i - 2 * Xsig15), MARGIN = 1, STATS = S *
    dmusig, FUN = "*")
  hessll <- matrix(nrow = nXvar + nuZUvar + nvZVvar, ncol = nXvar +
    nuZUvar + nvZVvar)
  hessll[1:nXvar, 1:nXvar] <- 0.5 * (sapply(1:nXvar, function(x) crossprod(2 *
    Xsq[[x]], as.matrix(wHvar * pmusig2 * sigx3_2/ewv/sigx4))) -
    crossprod(sweep(X_iM, MARGIN = 1, STATS = wHvar * 2 *
      (S^2/TT) * pmusig2 * sigx3_2/ewv/sigx4, FUN = "*"),
      X_iM) - crossprod(sweep(Xsig18, MARGIN = 1, STATS = wHvar *
    sigx3_2/ewv/sigx4, FUN = "*"), Xsig16)) - (0.5 * crossprod(sweep((0.5 *
    Xsig1 + Xsig2), MARGIN = 1, STATS = wHvar * sigx3_2/ewv/sigx4,
    FUN = "*"), (2 * Xepsi_i - 2 * Xsig14)) + crossprod(sweep(X_iM,
    MARGIN = 1, STATS = wHvar * S^2 * epsi_vu2 * dmusig2/(TT^2 *
      sqrt(wvtt)) * sigx3_2/ewv/sigx4, FUN = "*"), X_iM)) -
    (0.5 * (sapply(1:nXvar, function(x) crossprod(2 * Xsq[[x]],
      as.matrix(wHvar * pmusig * sigx3_1/ewv/sigx4))) -
      crossprod(sweep(X_iM, MARGIN = 1, STATS = wHvar *
        2 * (S^2/TT) * pmusig * sigx3_1/ewv/sigx4, FUN = "*"),
        X_iM) - crossprod(sweep(Xsig34, MARGIN = 1, STATS = wHvar *
      sigx3_1/ewv/sigx4, FUN = "*"), Xsig16)) - (0.5 *
      crossprod(sweep((0.5 * Xsig3 + Xsig4), MARGIN = 1,
        STATS = wHvar * sigx3_1/ewv/sigx4, FUN = "*"),
        (2 * Xepsi_i - 2 * Xsig15)) + crossprod(sweep(X_iM,
      MARGIN = 1, STATS = wHvar * S^2 * epsi_vu * dmusig/(TT^2 *
        sqrt(wvtt)) * sigx3_1/ewv/sigx4, FUN = "*"),
      X_iM))) - crossprod(sweep(Xsig5, MARGIN = 1, STATS = wHvar/sigx4^2,
    FUN = "*"), Xsig5)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod((Xsig33 -
    Xsig21), sweep(uHvar, MARGIN = 1, STATS = wHvar, FUN = "*"))
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(Xsig32, sweep(vHvar, MARGIN = 1,
    STATS = wHvar, FUN = "*"))
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar +
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ((((0.25 * (sigx11) - 0.5 * (0.5 * (sigx11) - 0.5 * pmusig)) *
      ewv - 0.5 * (dw1pmu * epsi_vu/TT)) * sigx3_1 - sigx3_2 *
      (ewv * pmusig2 - epsi_vu2 * dw2pmu/TT))/(TT * sigx4 *
      ewu_h^2) - sigx7 * (dw1pmu * sigx3_1 + 0.5 * ttsq -
      dw2pmu * sigx3_2)/ttsq^2), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar +
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((0.5 * (0.5 * (epsivusq1 *
      dmusig * ewv/(wutt * sqrt(wvtt))) + 2 * (((0.25 *
      sigx9 - 0.5 * wutsq) * epsi_vu - 0.5 * (sigx2_1 *
      ewv/ewu_h)) * pmusig)) - (((0.25 * (ewv/ssq_v^2) +
      0.5 * (sigx10))/ewu_h - 0.5 * wutsq) * dwv1 + 0.5 *
      (sigx5_1 * epsi_vu/(wutt)))) * sigx3_1 - (sigx6 *
      sigx7/ttsq + (0.5 * (epsivusq2 * dmusig2 * ewv/(wutt *
      sqrt(wvtt)) + 2 * (((0.5 * sigx9 - wutt/(wutt)^2) *
      epsi_vu2 - wusig * ewv/ewu_h) * pmusig2)) - (((epsi_vu2 *
      wusig/TT + 0.5 * (ewv/ssq_v^2))/ewu_h - wutt/(wutt)^2) *
      dwv2 + sigx5_2 * epsi_vu2/(wutt))) * sigx3_2))/sigx4,
    FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar),
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * ((0.5 * (sigx5_1 * epsivusq1) +
      0.5 * ((2 * ((sigx2_1/ewu_h - 0.5 * (sigx8 * epsi_vu/ssq_v^2)) *
        ewv) - epsilon_isq/ewv) * pmusig - sigx2_1 *
        epsivusq1 * dwv1) - (1/(wutt) - ((sigx2_1^2 +
      0.5/ssq_v^2) * epsi_vu + 0.5 * (sigx8 * ewv/ssq_v^2) +
      0.5 * sigx2_1)) * dwv1) * sigx3_1 - (sigx6^2/sigx4 +
      (0.5 * (sigx5_2 * epsivusq2) + 0.5 * ((2 * ((2 *
        (wusig/ewu_h) - 0.5 * (epsi_vu2 * wusq/ssq_v^2)) *
        ewv) - epsilon_isq/ewv) * pmusig2 - epsivusq2 *
        wusig * dwv2) - (2/(wutt) - ((wusig^2 + 0.5/ssq_v^2) *
        epsi_vu2 + 0.5 * (wusq * ewv/ssq_v^2) + 0.5 *
        wusig)) * dwv2) * sigx3_2))/sigx4, FUN = "*"),
    vHvar)
...
genexponormAlgOpt_pl81 <- function(start, olsParam, dataTable,
  ...
  startLoglik <- sum(pgenexponormlike_pl81(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) {
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgenexponormlike_pl81,
    grad = pgradgenexponormlike_pl81, hess = phessgenexponormlike_pl81,
    ...
    fn = function(parm) {
      ...
      as(-phessgenexponormlike_pl81(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgenexponormlike_pl81(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phessgenexponormlike_pl81(parm = mleObj$par,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (method == "sr1") {
      mleObj$hessian <- phessgenexponormlike_pl81(parm = mleObj$solution,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p)
  ...
  mleObj$logL_OBS <- pgenexponormlike_pl81(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradgenexponormlike_pl81(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
