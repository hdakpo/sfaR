...
#' @noRd
plognormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar,
  uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S, wHvar, N,
  FiMat) {
  beta <- parm[1:(nXvar)]
...
#' @noRd
pstlognorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, muHvar,
  nmuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, printInfo,
  tol, N, FiMat, whichStart, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradlognormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
  ...
  sigFi5 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 *
    ewv), FUN = "*") * (sweep(sweep(EqFi, MARGIN = 1, STATS = gid2sq_gicub,
    FUN = "*"), MARGIN = 1, STATS = 2 * (S * gid1sq_epsi_gisq),
    FUN = "+"))
  sigFi6 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 *
    ewv), FUN = "*") * sweep(sweep(EqFi, MARGIN = 1, STATS = gid2_gicub,
    FUN = "*"), MARGIN = 1, STATS = 2 * (S * gid1_epsi_gisq),
    FUN = "+")
  ...
  gradll <- cbind(gx, gmu, gu, gv - sweep(vHvar, MARGIN = 1,
    STATS = 0.5 * TT, FUN = "*"), apply(-sigFi6, 1, sum)/sigDiv,
    apply(-sigFi5, 1, sum)/sigDiv)
...
#' @noRd
lognormAlgOpt_k90 <- function(start, olsParam, dataTable, S,
  ...
  startLoglik <- sum(plognormlike_k90(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(plognormlike_k90(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
    ...
    hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = plognormlike_k90,
    ...
    fn = function(parm) -sum(plognormlike_k90(parm, nXvar = nXvar,
      ...
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)),
    hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradlognormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)),
    ...
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradlognormlike_k90(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradlognormlike_k90(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N))
  ...
  mleObj$logL_OBS <- plognormlike_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradlognormlike_k90(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, N = N, FiMat = FiMat_N)
...
#' @noRd
fnCondEffLogNorm_k90 <- function(u, sigmaU, sigmaV, mu, S, TT,
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffLogNorm_k90 <- function(u, sigmaU, sigmaV, mu, S,
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffLogNorm_k90 <- function(u, sigmaU, sigmaV,
...
plognormeff_k90 <- function(object, level) {
  ...
  for (i in 1:object$Nid) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) *
      ...
      exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffLogNorm_k90, lowerLimit = 0,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in 1:object$Nobs) {
      res$teBC[i] <- hcubature(f = fnCondBCEffLogNorm_k90,
