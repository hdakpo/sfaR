...
#' @noRd
pweibullnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  for (i in 1:N) {
    ...
    ll[i] <- -TT[i]/2 * log(2 * pi) - TT[i] * Wv[i]/2 + log(mean(exp(-1/(2 *
      exp(Wv[i])) * (epsilon_isq[i] + 2 * S * ur * epsilon_i[i] + TT[i] * ur^2))))
  }
  return(wHvar * ll)
}
...
#' @noRd
pstweibullnorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar,
  vHvar, Yvar, Xvar, S, wHvar, whichStart, initIter, initAlg, printInfo, tol, N,
  FiMat) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradweibullnormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  lFisq <- sweep(lFimat, MARGIN = 1, STATS = 2 * (S * ewu_h * epsilon_i), FUN = "*")
  lFiu <- sweep(lFimat^2, MARGIN = 1, STATS = TT * (ewu_h)^2, FUN = "*")
  lFiepsi <- sweep(lFisq + lFiu, MARGIN = 1, STATS = epsilon_isq, FUN = "+")
  lFiuv <- exp(-sweep(lFiepsi, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*"))
  sdFiv <- apply(lFiuv, 1, sum)
  lFi1k <- log(-log(1 - FiMat))
  lFi2k <- (-log(1 - FiMat))^(2/k)
  lFiuepsi <- sweep(lFimat, MARGIN = 1, STATS = S * epsilon_i, FUN = "*")
  ...
  lFi6 <- sweep((lFiuepsi + lFi3) * lFiuv, MARGIN = 1, STATS = ewu_h/(2 * ewv),
    FUN = "*")
  lFi7 <- sweep(lFi1k * (2 * (lFiuepsi) + 2 * (lFi3)) * lFiuv, MARGIN = 1, STATS = ewu_h/(2 *
    (k^2 * ewv)), FUN = "*")
  lFi8 <- sweep((lFiepsi) * lFiuv, MARGIN = 1, STATS = ewv/(2 * ewv)^2, FUN = "*")
  gx <- matrix(nrow = N, ncol = nXvar)
  for (k in 1:nXvar) {
    gx[, k] <- -apply(sweep(2 * lFi4 * lFi5, MARGIN = 1, STATS = X_iM[, k], FUN = "*") +
      sweep(lFi5, MARGIN = 1, STATS = 2 * Xepsi_i[, k], FUN = "*"), 1, sum)/sdFiv
  }
  gu <- matrix(nrow = N, ncol = nuZUvar)
  for (k in 1:nuZUvar) {
    gu[, k] <- apply(sweep(lFi6, MARGIN = 1, STATS = -uHvar[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gv <- matrix(nrow = N, ncol = nvZVvar)
  for (k in 1:nvZVvar) {
    gv[, k] <- apply(2 * sweep(lFi8, MARGIN = 1, STATS = vHvar[, k], FUN = "*"),
      1, sum)/sdFiv
  }
  gradll <- cbind(gx, gu, gv - sweep(vHvar, MARGIN = 1, STATS = 0.5 * TT, FUN = "*"),
    apply(lFi7, 1, sum)/sdFiv)
...
#' @noRd
weibullnormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  stepmax, tol, whichStart, initIter, initAlg, gradtol, hessianType, qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
    start_st <- pstweibullnorm_pl81(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      ...
      tol = tol, printInfo = printInfo, whichStart = whichStart, initIter = initIter,
      initAlg = initAlg)
  ...
  startLoglik <- sum(pweibullnormlike_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pweibullnormlike_pl81(parm,
    ...
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pweibullnormlike_pl81,
    ...
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pweibullnormlike_pl81(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradweibullnormlike_pl81(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      fn = function(parm) -sum(pweibullnormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        ...
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradweibullnormlike_pl81(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      objective = function(parm) -sum(pweibullnormlike_pl81(parm, nXvar = nXvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradweibullnormlike_pl81(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
      Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  }
  ...
  mleObj$logL_OBS <- pweibullnormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradweibullnormlike_pl81(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffWeibull_pl81 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_i, epsilon_isq) {
  u * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) * exp(-(u/sigmaU)^k) *
    exp(-(epsilon_isq + 2 * S * u * epsilon_i + u^2)/(2 * sigmaV^2))
}
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffWeibull_pl81 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_i, epsilon_isq) {
  exp(-u) * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) * exp(-(u/sigmaU)^k) *
    exp(-(epsilon_isq + 2 * S * u * epsilon_i + u^2)/(2 * sigmaV^2))
}
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffWeibull_pl81 <- function(u, sigmaU, sigmaV, k, S, TT, epsilon_i,
  epsilon_isq) {
  exp(u) * k/(sigmaU * sigmaV^TT * (2 * pi)^(TT/2)) * (u/sigmaU)^(k - 1) * exp(-(u/sigmaU)^k) *
    exp(-(epsilon_isq + 2 * S * u * epsilon_i + u^2)/(2 * sigmaV^2))
}
...
pweibullnormeff_pl81 <- function(object, level) {
  ...
  for (i in seq_along(1:object$Nid)) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * epsilon_i[i] + ur^2)/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffWeibull_pl81, lowerLimit = 0, upperLimit = Inf,
      maxEval = 100, fDim = 1, sigmaU = exp(Wu[i]/2), sigmaV = exp(Wv[i]/2),
      k = k, epsilon_i = epsilon_i[i], epsilon_isq = epsilon_isq[i], TT = TT[i],
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in seq_along(1:object$Nid)) {
      teBC[i] <- hcubature(f = fnCondBCEffWeibull_pl81, lowerLimit = 0, upperLimit = Inf,
        maxEval = 100, fDim = 1, sigmaU = exp(Wu[i]/2), sigmaV = exp(Wv[i]/2),
        k = k, epsilon_i = epsilon_i[i], epsilon_isq = epsilon_isq[i], TT = TT[i],
        S = object$S, vectorInterface = FALSE, tol = 1e-15)$integral/density_epsilon_vec[i]
      teBC_reciprocal[i] <- hcubature(f = fnCondBCreciprocalEffWeibull_pl81,
        ...
        sigmaV = exp(Wv[i]/2), k = k, epsilon_i = epsilon_i[i], epsilon_isq = epsilon_isq[i],
