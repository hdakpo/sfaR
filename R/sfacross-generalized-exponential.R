...
#' @noRd
cgenexponormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar,
  wHvar, S) {
  beta <- parm[1:(nXvar)]
  ...
  ll <- (log(2) - 1/2 * Wu + log(exp(A) * pnorm(a) - exp(B) * pnorm(b)))
  return(ll * wHvar)
}
...
#' @param vHvar matrix of Zv variables
#' @noRd
cstgenexponorm <- function(olsObj, epsiRes, S, nuZUvar, uHvar, nvZVvar, vHvar) {
...
#' @param wHvar vector of weights (weighted likelihood)
#' @noRd
cgradgenexponormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigx1 <- (pda) * eA - (db/exp(Wv/2) - 2 * (pb/exp(Wu/2))) * eB
  pab <- eA * pa - eB * pb
  sigx2 <- 0.5 * (S * (epsilon)/exp(Wu/2)) + 2 * (exp(Wu) * exp(Wv)/(2 * exp(Wu))^2)
  sigx3 <- (0.5 * (da * wvwu) - (sigx2) * pa) * eA - (db * wvwu - (eC) * pb) *
    eB
  sigx5 <- 2 * (exp(Wv) * pb/exp(Wu)) - db * (wvwu - epsiv)
  sigx4 <- eA * (exp(Wv) * pa/(2 * exp(Wu)) - (epsiuv) * da) - (sigx5) * eB
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * (sigx1)/(pab), FUN = "*"),
    sweep(uHvar, MARGIN = 1, STATS = ((sigx3)/(pab) - 0.5), FUN = "*"), sweep(vHvar,
      MARGIN = 1, STATS = (sigx4)/(pab), FUN = "*"))
...
#' @param wHvar vector of weights (weighted likelihood)
#' @noRd
chessgenexponormlike <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, S, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  hessll[1:nXvar, 1:nXvar] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = S^2 * wHvar *
    ((((wvwuepsi)/exp(Wv/2) - 1/exp(Wu/2)) * da/exp(Wv/2) - (pda)/exp(Wu/2)) *
      eA - ((((wvwuepsix2)/exp(Wv/2) - 2/exp(Wu/2)) * db/exp(Wv/2) - 2 * ((db/exp(Wv/2) -
      2 * (pb/exp(Wu/2)))/exp(Wu/2))) * eB + (sigx1)^2/(pab)))/(pab), FUN = "*"),
    Xvar)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = S * wHvar * ((((0.5 + sigx2) * pa - 0.5 * (da * wvwu))/exp(Wu/2) +
      (0.5 * ((wvwuepsi)/exp(Wu/2)) - (sigx2)/exp(Wv/2)) * da) * eA - ((((wvwuepsix2)/exp(Wu/2) -
      (eC)/exp(Wv/2)) * db + (pb - 2 * (db * wvwu - (eC) * pb))/exp(Wu/2)) *
      eB + (sigx3) * (sigx1)/(pab)))/(pab), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * wHvar * ((da * (exp(Wv)/(2 * exp(Wu)) - ((epsiuv) *
      (wvwuepsi) + 0.5))/exp(Wv/2) - (exp(Wv) * pa/(2 * exp(Wu)) - (epsiuv) *
      da)/exp(Wu/2)) * eA - (((2 * (exp(Wv)/exp(Wu)) - ((wvwuepsix2) * (wvwu -
      epsiv) + 0.5)) * db/exp(Wv/2) - 2 * ((sigx5)/exp(Wu/2))) * eB + (sigx1) *
      (sigx4)/(pab)))/(pab), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * (((0.5 * (0.5 * (exp(Wv/2) * (wvwuepsi)/exp(Wu/2)) -
      0.5) - 0.5 * (sigx2)) * da * wvwu - ((0.5 * (da * wvwu) - (sigx2) * pa) *
      (sigx2) + (2 * ((1 - 8 * (exp(Wu)^2/(2 * exp(Wu))^2)) * exp(Wu) * exp(Wv)/(2 *
      exp(Wu))^2) - 0.25 * (S * (epsilon)/exp(Wu/2))) * pa)) * eA - (((((wvwuepsix2) *
      exp(Wv/2) - S * (epsilon))/exp(Wu/2) - (0.5 + 2 * (exp(Wv)/exp(Wu)))) *
      db * wvwu + (0.5 * (S * (epsilon)/exp(Wu/2)) + 2 * (exp(Wv)/exp(Wu))) *
  ...
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    ((((exp(Wv) * pa/(2 * exp(Wu)) - (epsiuv) * da)/2 + (pa - (epsiuv) * da)/2) *
      exp(Wv)/exp(Wu) - (0.25 * (wvwu) + 0.25 * (S * (epsilon)/exp(Wv/2)) -
      (epsiuv)^2 * (wvwuepsi)) * da) * eA - (((2 * (sigx5) + 2 * (pb - db *
      (wvwu - epsiv))) * exp(Wv)/exp(Wu) - (0.25 * (S * (epsilon)/exp(Wv/2)) +
      0.5 * (wvwu) + b * (wvwu - epsiv)^2) * db) * eB + (sigx4)^2/(pab)))/(pab),
    FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * (((da * exp(Wv/2)/(4 *
    (exp(Wu) * exp(Wu/2))) - 2 * (exp(Wu) * pa/(2 * exp(Wu))^2)) * exp(Wv) -
    ((0.5 * ((epsiuv) * (wvwuepsi)) - 0.25) * da * wvwu + (sigx2) * (exp(Wv) *
      pa/(2 * exp(Wu)) - (epsiuv) * da))) * eA - ((sigx3) * (sigx4)/(pab) +
    (2 * ((db * wvwu - pb) * exp(Wv)/exp(Wu)) - (((wvwuepsix2) * (wvwu - epsiv) -
      0.5) * db * wvwu + (sigx5) * (eC))) * eB))/(pab), FUN = "*"), vHvar)
...
genexponormAlgOpt <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cgenexponormlike(parm,
    ...
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradgenexponormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cgenexponormlike,
    ...
    fn = function(parm) -sum(cgenexponormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradgenexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hs = function(parm) as(-chessgenexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), gr = function(parm) -colSums(cgradgenexponormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hess = function(parm) -chessgenexponormlike(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    objective = function(parm) -sum(cgenexponormlike(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      wHvar = wHvar, S = S)), gradient = function(parm) -colSums(cgradgenexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, wHvar = wHvar, S = S)), hessian = function(parm) -chessgenexponormlike(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
  ...
  mleObj$logL_OBS <- cgenexponormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    wHvar = wHvar, S = S)
  mleObj$gradL_OBS <- cgradgenexponormlike(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    wHvar = wHvar, S = S)
...
cgenexponormeff <- function(object, level) {
  ...
  u <- exp(Wv/2) * (exp(A) * (dnorm(a) + a * pnorm(a)) - exp(B) * (dnorm(b) + b *
    pnorm(b)))/(exp(A) * pnorm(a) - exp(B) * pnorm(b))
  if (object$logDepVar == TRUE) {
    teJLMS <- exp(-u)
    teBC <- (exp(A) * exp(-a * exp(Wv/2) + exp(Wv)/2) * pnorm(a - exp(Wv/2)) -
      exp(B) * exp(-b * exp(Wv/2) + exp(Wv)/2) * pnorm(b - exp(Wv/2)))/(exp(A) *
      pnorm(a) - exp(B) * pnorm(b))
    teBC_reciprocal <- (exp(A) * exp(a * exp(Wv/2) + exp(Wv)/2) * pnorm(a + exp(Wv/2)) -
      exp(B) * exp(b * exp(Wv/2) + exp(Wv)/2) * pnorm(b + exp(Wv/2)))/(exp(A) *
      pnorm(a) - exp(B) * pnorm(b))
...
cmarggenexponorm_Eu <- function(object) {
  ...
  uHvar <- model.matrix(object$formula, data = object$dataTable, rhs = 2)
  Wu <- as.numeric(crossprod(matrix(delta), t(uHvar)))
  margEff <- kronecker(matrix(delta[2:object$nuZUvar] * 3/4, nrow = 1), matrix(exp(Wu/2),
    ncol = 1))
...
cmarggenexponorm_Vu <- function(object) {
  ...
  uHvar <- model.matrix(object$formula, data = object$dataTable, rhs = 2)
  Wu <- as.numeric(crossprod(matrix(delta), t(uHvar)))
  margEff <- kronecker(matrix(delta[2:object$nuZUvar] * 5/4, nrow = 1), matrix(exp(Wu),
    ncol = 1))
