...
#' @noRd
plognormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar, vHvar,
...
#' @noRd
pstlognorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar, muHvar, nmuZUvar, nvZVvar,
...
#' @param FiMat matrix of random draws
#' @noRd
pgradlognormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar,
  ...
  sigFi5 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*") *
    (sweep(sweep(EqFi, MARGIN = 1, STATS = giZi, FUN = "*"), MARGIN = 1, STATS = 2 *
      (S * Zi_epsi), FUN = "+"))
  sigFi6 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 * ewv), FUN = "*") *
    sweep(sweep(EqFi, MARGIN = 1, STATS = giZisq, FUN = "*"), MARGIN = 1, STATS = 2 *
      (S * Zisq_epsi), FUN = "+")
  ...
  gradll <- cbind(gx, gmu, gu, gv - sweep(vHvar, MARGIN = 1, STATS = 0.5 * TT,
    FUN = "*"), apply(-sigFi5, 1, sum)/sigDiv, apply(-sigFi6, 1, sum)/sigDiv)
...
#' @noRd
lognormAlgOpt_mbc92 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  ...
  startLoglik <- sum(plognormlike_mbc92(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(plognormlike_mbc92(parm,
    ...
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_mbc92(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
    ...
    control = list(trace = if (printInfo) 1 else 0, maxeval = itermax, stepmax = stepmax,
      xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = plognormlike_mbc92,
    ...
    fn = function(parm) -sum(plognormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    ...
    fn = function(parm) -sum(plognormlike_mbc92(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N,
      FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradlognormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N,
    ...
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
    nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradlognormlike_mbc92(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradlognormlike_mbc92(mleObj$par, nXvar = nXvar,
  ...
  mleObj$logL_OBS <- plognormlike_mbc92(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradlognormlike_mbc92(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffLogNorm_mbc92 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq, giepsi,
  gisq) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffLogNorm_mbc92 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq, giepsi,
  gisq, git) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffLogNorm_mbc92 <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq,
  giepsi, gisq, git) {
...
plognormeff_mbc92 <- function(object, level) {
  ...
  for (i in 1:object$Nid) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * giepsi[i] + ur^2 * gisq[i])/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffLogNorm_mbc92, lowerLimit = 0, upperLimit = Inf,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in 1:object$Nobs) {
      res$teBC[i] <- hcubature(f = fnCondBCEffLogNorm_mbc92, lowerLimit = 0,
