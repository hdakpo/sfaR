...
#' @noRd
plognormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstlognorm_mbc92 <- function(olsObj, epsiRes, nXvar, nuZUvar,
  muHvar, nmuZUvar, nvZVvar, uHvar, vHvar, Yvar, Xvar, S, wHvar,
  printInfo, tol, N, FiMat, whichStart, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradlognormlike_mbc92 <- function(parm, nXvar, nuZUvar, nvZVvar,
  nmuZUvar, uHvar, vHvar, muHvar, Yvar, Xvar, pindex, TT, S,
  wHvar, N, FiMat) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  ...
  sigFi5 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 *
    ewv), FUN = "*") * (sweep(sweep(EqFi, MARGIN = 1, STATS = giZi,
    FUN = "*"), MARGIN = 1, STATS = 2 * (S * Zi_epsi), FUN = "+"))
  sigFi6 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 *
    ewv), FUN = "*") * sweep(sweep(EqFi, MARGIN = 1, STATS = giZisq,
    FUN = "*"), MARGIN = 1, STATS = 2 * (S * Zisq_epsi),
    FUN = "+")
  sigFi7 <- sweep(sigFi2, MARGIN = 1, STATS = 1/(2 * ewv),
    FUN = "*")
  sigFi8 <- sweep(sigFi2 * EqFi, MARGIN = 1, STATS = 1/(2 *
  ...
  for (k in 1:nXvar) {
    gx[, k] <- -apply(sweep(2 * (S * EqFi) * sigFi7, MARGIN = 1,
      STATS = Xgi[, k], FUN = "*") + sweep(sigFi7, MARGIN = 1,
      STATS = Xepsi_i[, k], FUN = "*"), 1, sum)/sigDiv
  }
  gmu <- matrix(nrow = N, ncol = nmuZUvar)
  for (k in 1:nmuZUvar) {
    gmu[, k] <- apply(sweep(sigFi8, MARGIN = 1, STATS = -muHvar[,
      k], FUN = "*"), 1, sum)/sigDiv
  }
  ...
  gradll <- cbind(gx, gmu, gu, gv - sweep(vHvar, MARGIN = 1,
    STATS = 0.5 * TT, FUN = "*"), apply(-sigFi5, 1, sum)/sigDiv,
    apply(-sigFi6, 1, sum)/sigDiv)
...
#' @noRd
lognormAlgOpt_mbc92 <- function(start, olsParam, dataTable, S,
  ...
  if (!is.null(start)) {
    ...
    start_st <- pstlognorm_mbc92(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(plognormlike_mbc92(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(plognormlike_mbc92(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
      muHvar = muHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    hessian = 0, control = list(trace = if (printInfo) 1 else 0,
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = plognormlike_mbc92,
    grad = pgradlognormlike_mbc92, start = startVal, finalHessian = if (hessianType ==
    ...
    fn = function(parm) -sum(plognormlike_mbc92(parm, nXvar = nXvar,
      ...
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)),
    hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradlognormlike_mbc92(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)),
    ...
    epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(plognormlike_mbc92(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradlognormlike_mbc92(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradlognormlike_mbc92(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N))
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradlognormlike_mbc92(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    if (method == "sr1")
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradlognormlike_mbc92(parm,
        ...
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
        N = N, FiMat = FiMat_N)), unname(mleObj$solution))
  }
  mleObj$logL_OBS <- plognormlike_mbc92(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradlognormlike_mbc92(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
    muHvar = muHvar_p, N = N, FiMat = FiMat_N)
...
#' @noRd
fnCondEffLogNorm_mbc92 <- function(u, sigmaU, sigmaV, mu, S,
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffLogNorm_mbc92 <- function(u, sigmaU, sigmaV, mu, S,
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffLogNorm_mbc92 <- function(u, sigmaU, sigmaV,
...
#' @noRd
plognormeff_mbc92 <- function(object, level) {
  ...
  for (i in 1:object$Nid) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) *
      ...
      exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffLogNorm_mbc92, lowerLimit = 0,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in 1:object$Nobs) {
      res$teBC[i] <- hcubature(f = fnCondBCEffLogNorm_mbc92,
      ...
      res$teBC_reciprocal[i] <- hcubature(f = fnCondBCreciprocalEffLogNorm_mbc92,
        lowerLimit = 0, upperLimit = Inf, maxEval = 100,
        fDim = 1, sigmaU = exp(res$Wu[i]/2), sigmaV = exp(res$Wv[i]/2),
        mu = res$mu[i], TT = res$TT[i], epsilon_isq = res$epsilon_isq[i],
        giepsi = res$giepsi[i], gisq = res$gisq[i], S = object$S,
