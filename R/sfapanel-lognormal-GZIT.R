...
#' @noRd
plognormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar, vHvar,
...
#' @noRd
pstlognorm_gzit <- function(olsObj, epsiRes, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, ngZGvar, gHvar, Yvar, Xvar, S, wHvar, modelType, whichStart, printInfo,
...
#' @param FiMat matrix of random draws
#' @noRd
pgradlognormlike_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, nmuZUvar, uHvar,
...
#' @noRd
lognormAlgOpt_gzit <- function(start, randStart, sdStart, olsParam, dataTable, S,
  gHvar, ngZGvar, nXvar, muHvar_c, muHvar_p, nmuZUvar, N, NT, FiMat_N, FiMat_NT,
  uHvar_c, uHvar_p, nuZUvar, vHvar_c, vHvar_p, nvZVvar, pindex, TT, Yvar, Xvar,
  wHvar_c, wHvar_p, modelType, method, printInfo, itermax, stepmax, tol, gradtol,
  hessianType, whichStart, initIter, initAlg, qac) {
  if (!is.null(start)) {
    startVal <- start
  } else {
    start_st <- pstlognorm_gzit(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
      ...
      uHvar = uHvar_c, modelType = modelType, vHvar = vHvar_c, Yvar = Yvar,
      Xvar = Xvar, S = S, initIter = initIter, initAlg = initAlg, wHvar = wHvar_c,
  ...
  startLoglik <- sum(plognormlike_gzit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(plognormlike_gzit(parm,
    ...
    N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_gzit(parm,
    nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
    N = N, FiMat = FiMat_N)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = plognormlike_gzit,
    grad = pgradlognormlike_gzit, start = startVal, finalHessian = if (hessianType ==
    ...
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(plognormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_gzit(parm,
      ...
      fn = function(parm) -sum(plognormlike_gzit(parm, nXvar = nXvar, gHvar = gHvar,
        ...
        FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_gzit(parm,
        ...
        muHvar = muHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradlognormlike_gzit(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
      ...
      method = "Sparse", control = list(maxit = itermax, cgtol = gradtol, stop.trust.radius = tol,
        ...
        preconditioner = 1L)), mla = marqLevAlg::mla(b = startVal, fn = function(parm) -sum(plognormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradlognormlike_gzit(parm,
      ...
      epsa = gradtol, epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(plognormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
      N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradlognormlike_gzit(parm,
      nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradlognormlike_gzit(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradlognormlike_gzit(parm,
        nXvar = nXvar, gHvar = gHvar, ngZGvar = ngZGvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, nmuZUvar = nmuZUvar,
  ...
  mleObj$logL_OBS <- plognormlike_gzit(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradlognormlike_gzit(parm = mlParam, nXvar = nXvar, gHvar = gHvar,
    ngZGvar = ngZGvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
    wHvar = wHvar_p, nmuZUvar = nmuZUvar, muHvar = muHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
fnCondEffLogNorm_gzit <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq, giepsi,
  gisq) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCEffLogNorm_gzit <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq, giepsi,
  gisq, git) {
...
#' @param TT time presence vector
#' @noRd
fnCondBCreciprocalEffLogNorm_gzit <- function(u, sigmaU, sigmaV, mu, S, TT, epsilon_isq,
  giepsi, gisq, git) {
...
#' @noRd
plognormeff_gzit <- function(object, level) {
  ...
  for (i in 1:object$Nid) {
    ...
    density_epsilon_vec[i] <- mean(1/((2 * pi)^(TT[i]/2) * exp(Wv[i]/2 * TT[i])) *
      exp(-(epsilon_isq[i] + 2 * object$S * ur * giepsi[i] + ur^2 * gisq[i])/(2 *
        exp(Wv[i]))))
    u[i] <- hcubature(f = fnCondEffLogNorm_gzit, lowerLimit = 0, upperLimit = Inf,
  ...
  if (object$logDepVar == TRUE) {
    ...
    for (i in 1:object$Nobs) {
      res$teBC[i] <- hcubature(f = fnCondBCEffLogNorm_gzit, lowerLimit = 0,
      ...
      res$teBC_reciprocal[i] <- hcubature(f = fnCondBCreciprocalEffLogNorm_gzit,
