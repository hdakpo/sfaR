...
#' @noRd
pLCMhalfnormlike5C_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  git5 <- 1/unlist(lapply(TT, FUN = function(x) 1 + exp(eta5_1 * seq(1:x) + eta5_2 *
    (seq(1:x))^2)))
...
#' @noRd
psLCMfhalfnorm5C_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike5C_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  gitd1_5 <- Zit * exp(Zit * (eta5_1 + eta5_2 * Zit))
  gitd1_epsit_gitsq5 <- -gitd1_5 * epsilon_it5 * git5^2
  gid1_epsi_gisq5 <- as.numeric(tapply(gitd1_epsit_gitsq5, pindex[, 1], sum))
  ...
  gitd2_5 <- 2 * Zit * exp(Zit * (eta5_1 + eta5_2 * Zit))
  gitd2_gitcub5 <- -gitd1_5 * git5^3
  gid2_gicub5 <- as.numeric(tapply(gitd2_gitcub5, pindex[, 1], sum))
  ...
  gitd1sq_epsit_gitsq5 <- -Zit^2 * exp(Zit * (eta5_1 + eta5_2 * Zit)) * epsilon_it5 *
    git5^2
  gid1sq_epsi_gisq5 <- as.numeric(tapply(gitd1sq_epsit_gitsq5, pindex[, 1], sum))
  ...
  gitd2sq_gitcub5 <- -2 * Zit^2 * exp(Zit * (eta5_1 + eta5_2 * Zit)) * git5^3
  gid2sq_gicub5 <- as.numeric(tapply(gitd2sq_gitcub5, pindex[, 1], sum))
  ewu1 <- exp(Wu1)
  ewu2 <- exp(Wu2)
  ewu3 <- exp(Wu3)
  ewu4 <- exp(Wu4)
  ...
  sigx19_5 <- (gid1_epsi_gisq5/ssq5 - ewu5 * sigx18_5 * gid2_gicub5 * giepsi5/ssq5^2)
  ...
  sigx20_5 <- (ewu5 * ewv5 * pmusig5 * gid2_gicub5/(sigmasq5^2 * sigmastar5))
  ...
  sigx22_5 <- (gid1sq_epsi_gisq5/ssq5 - ewu5 * sigx18_5 * gid2sq_gicub5 * giepsi5/ssq5^2)
  ...
  sigx23_5 <- (ewu5 * ewv5 * pmusig5 * gid2sq_gicub5/(sigmasq5^2 * sigmastar5))
  ...
  gradll <- cbind(sweep((0.5 * Xsig5 + Xsig3), MARGIN = 1, STATS = -(2 * (sigx5_1 *
    ...
    FUN = "*"), 2 * (sigx5_2 * ewu2 * ewz2 * sigx21_2/sigx30), 2 * (sigx5_2 *
    ewu2 * ewz2 * sigx24_2/sigx30), sweep((0.5 * Xsig9 + Xsig8), MARGIN = 1,
    ...
    FUN = "*"), 2 * (sigx5_3 * ewu3 * ewz3 * sigx21_3/sigx25), 2 * (sigx5_3 *
    ewu3 * ewz3 * sigx24_3/sigx25), sweep((0.5 * Xsig12 + Xsig11), MARGIN = 1,
    ...
    FUN = "*"), 2 * (sigx5_4 * ewu4 * ewz4 * sigx21_4/sigx31), 2 * (sigx5_4 *
    ewu4 * ewz4 * sigx24_4/sigx31), sweep((0.5 * Xsig15 + Xsig14), MARGIN = 1,
    ...
    sweep(vHvar, MARGIN = 1, STATS = 2 * (sigx17_5 * wzdeno * sigx5_5/sigx7bis),
      FUN = "*"), 2 * (wzdeno * sigx5_5 * ewu5 * sigx21_5/sigx26), 2 * (wzdeno *
      sigx5_5 * ewu5 * sigx24_5/sigx26), sweep(Zvar, MARGIN = 1, STATS = (2 *
...
#' @noRd
LCM5ChnormAlgOpt_k90 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm5C_k90(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike5C_k90(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike5C_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, gr = function(parm) {
    -colSums(pgradLCMhalfnormlike5C_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
  }, hessian = 0, control = list(trace = if (printInfo) 1 else 0, maxeval = itermax,
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike5C_k90,
    grad = pgradLCMhalfnormlike5C_k90, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike5C_k90(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike5C_k90(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike5C_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike5C_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike5C_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
    Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar,
    nZHvar = nZHvar)
  if (is.null(start)) {
