...
#' @noRd
phalfnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  ll <- log(2) - TT/2 * log(2 * pi) + log(pnorm(mustar/sigmastar)) + log(sigmastar) -
...
#' @noRd
psthalfnorm_mols93 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ngZGvar, gHvar, Yvar, Xvar, S, wHvar, printInfo, tol, whichStart, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param TT vector of time of presence
#' @noRd
pgradhalfnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, ngZGvar, gHvar, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  eta <- parm[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + ngZGvar -
  ...
  sqspmu <- (ssq * pmusig)
  sigx6 <- (ssq * ssqx2)
  sigZ1 <- sweep(Zigiepsi, MARGIN = 1, STATS = 1/(ssqx2), FUN = "*") - sweep(Zigisq,
    MARGIN = 1, STATS = ewu * sigx3 * giepsi/ssq, FUN = "*")
  sigx8 <- (0.5 * sigx2 + sigmastar)
  gradll <- cbind(-(0.5 * (sweep(Xepsi_i, MARGIN = 1, STATS = 1/ewv, FUN = "*") -
    sweep(Xgi, MARGIN = 1, STATS = 2 * (S^2 * ewu * giepsi/sigmasq)/ewv, FUN = "*")) +
    sweep(Xgi, MARGIN = 1, STATS = S * dmusig * ewu/sps, FUN = "*")), sweep(uHvar,
    MARGIN = 1, STATS = (0.5 * wu2sq + S * sigx5 * ewu * sigx1 * giepsi - 0.5),
    FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = (0.5 * wvsq + S * sigx8 * dmusig *
    ewu * ewv * giepsi/sqspmu - (0.5 * (2 * (S^2 * sigx8 * ewu^2 * ewv * giepsi^2/sigx6) -
    epsilon_isq/ewv) + 0.5 * TT)), FUN = "*"), sweep(sigZ1, MARGIN = 1, STATS = S *
...
#' @param TT vector of time of presence
#' @noRd
phesshalfnormlike_mols93 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, pindex, TT, S, ngZGvar, gHvar, wHvar) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  ...
  sigZ1 <- sweep(Zigiepsi, MARGIN = 1, STATS = 1/(ssqx2), FUN = "*") - sweep(Zigisq,
    MARGIN = 1, STATS = ewu * sigx3 * giepsi/ssq, FUN = "*")
  sigx9 <- (0.5 + 0.5 * (ewv/sigmasq) - 0.5 * (0.5 * wvsq + ewv/sigmasq))
  sigx10 <- (ewu * gisq/sigmasq)
  sigx11 <- ((ewu * gisq/sigmasq - 1) * ewv/sigmasq + 1 - 0.5 * (wu2sq * wvsq))
  ...
  }) + crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar * (S^2 * ewu/sigmasq)/ewv,
    FUN = "*"), Xgi) + crossprod(sweep(Xgi, MARGIN = 1, STATS = -wHvar * (S^2 *
    dmusig * (dmusig/sps^2 - S * giepsi/(sigmasq^2 * ewv * pmustar)) * ewu^2),
    FUN = "*"), Xgi)
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xgi, MARGIN = 1,
    STATS = wHvar * S * sigx5 * ewu * (S * (2/sigmastar - dmusig * (dmusig/(pmustar) -
      S * giepsi/ewv)/pmusig) * ewu * giepsi/sigmasq - dpmu), FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(Xgi,
    MARGIN = 1, STATS = wHvar * S * ((ewv - S^2 * ewu * giepsi^2/sigmasq)/sqspmu +
      S * ssq * dmusig * ewu * ewv * giepsi/(sqspmu^2 * ssqx2)) * sigx8 * dmusig *
      ewu, FUN = "*"), vHvar) - crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    2 * (S^2 * sigx8 * ewu^2 * ewv * giepsi/sigx6), FUN = "*"), vHvar) + crossprod(sweep(Xepsi_i,
  ...
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + nvZVvar +
    ...
    crossprod(Xzigi[[x]], as.matrix(wHvar * S * ewu * (sigx1 * 1/(ssqx2))))
  }) - crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar * S * ewu * (sigx1 * (ewu *
    sigx3 * 1/ssq)), FUN = "*"), Zigisq) + crossprod(sweep(Xgi, MARGIN = 1, STATS = wHvar *
    S * ewu * (S * (1/sigmastar - dmusig * (dmusig/(pmustar) - S * giepsi/ewv)/pmusig) *
    ewu/sigmasq), FUN = "*"), sigZ1)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ewu * (S * ((1/(ssqx2) - ((0.5 * sigx10 + 1.5 -
      0.5 * (0.5 * wu2sq + ewu * gisq/sigmasq)) * wu2sq * ewv/sigmastar + (3 *
      gisq - 2 * (sigx4^2 * ewu * sigmasq/ssq)) * sigmastar) * ewu/ssq) * sigx1 +
      S * (1 - dmusig * (dpmu - S * ewu * giepsi/(ssqx2))/pmusig) * sigx5^2 *
        ewu * giepsi) * giepsi - 0.5 * (wu2sq * gisq/sigmasq)), FUN = "*"),
    uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 1):(nXvar + nuZUvar +
    ...
    S * (((((0.5 * (wu2sq * ewv) - S^2 * sigx8 * sigx5 * ewu * giepsi^2)/sigmasq +
      0.5 * sigx11 + 0.5 * wvsq) * ewu/sigmastar + sigmastar)/sqspmu - sigx8 *
      (2 * (sigx4 * sigmasq * pmustar) - S * ssq * sigx5 * dmusig * giepsi) *
      ewu/sqspmu^2) * dmusig - S * (((0.5 * (wu2sq * ewv/sigmasq) + 0.5 * sigx11) *
      ewu/sigmastar + 2 * sigx8)/sigx6 - ((ssq * gisq + 2 * (sigx4 * ssqx3)) *
      sigmastar + 0.5 * (ssq * wu2sq * ewv/sigmastar)) * sigx8 * ewu/sigx6^2) *
      ewu * giepsi) * giepsi) * ewu * ewv, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + nvZVvar + 1):(nXvar +
    ...
    ewu * S * sigx5 * sigx1, FUN = "*"), Zigiepsi) + crossprod(sweep(uHvar, MARGIN = 1,
    STATS = wHvar * ewu * S * ewu * S * (1 - dmusig * (dpmu - S * ewu * giepsi/(ssqx2))/pmusig) *
      sigx5 * giepsi, FUN = "*"), sigZ1) - crossprod(sweep(uHvar, MARGIN = 1,
    STATS = wHvar * ewu * S * ewu * (2 * sigmastar - ((0.25 * wu2sq + 0.5 + 0.5 *
      sigx10) * ewv/(ssqx2) + 2 * (sigx4 * ssqx2 * sigx3/ssq)) * ewu) * sigx1/ssq *
      giepsi, FUN = "*"), Zigisq) - crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar *
    ewu * 0.5 * (wu2sq/sigmasq), FUN = "*"), Zigisq)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    ...
    (ewv * (S * (((sigx9 * wvsq + S^2 * sigx8^2 * ewu * ewv * giepsi^2/(ssq *
      sigmasq)) * ewu/sigmastar + sigmastar)/sqspmu - sigx8^2 * (2 * sps +
      S * dmusig * ewu * giepsi) * ewv/sqspmu^2) * dmusig * ewu * giepsi -
      0.5 * (wvsq/sigmasq)) - 0.5 * (2 * (S^2 * ((sigx9 * wvsq * ewu/sigmastar +
      sigmastar)/sigx6 - ((ssq + 2 * (sigx8 * ssqx3)) * sigmastar + 0.5 * (ssq *
      wvsq * ewu/sigmastar)) * sigx8 * ewv/sigx6^2) * ewu^2 * ewv * giepsi^2) +
      epsilon_isq/ewv)), FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), (nXvar + nuZUvar +
    nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + ngZGvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = wHvar * 0.5/sigmasq^2 * ewu * ewv, FUN = "*"), Zigisq) -
    S * (crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar * ((0.5 * (wvsq/(ssqx2)) -
      0.5 * (1/(ssqx2) - sigx8 * ewv/ssq)) * ewu - 2 * (sigx8 * ssqx2 * sigx3/ssq)) *
      ewu * giepsi * sigx1/ssq * ewu * ewv, FUN = "*"), Zigisq) + crossprod(sweep(vHvar,
      MARGIN = 1, STATS = wHvar * sigx8 * sigx1/ssq * ewu * ewv, FUN = "*"),
      Zigiepsi) + crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar * S * sigx8 *
      (1 + dmusig * sigx1/pmusig) * ewu * giepsi/ssq * ewu * ewv, FUN = "*"),
      sigZ1))
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + ngZGvar),
    ...
    (sapply(1:ngZGvar, function(x) {
      crossprod(Zisqgiepsi[[x]], as.matrix(wHvar * ewu * sigx1/(ssqx2)))
    }) - (crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar * ewu * sigx1 * (0.5 *
      (sigx3/ssq) - 0.5/(ssqx3)) * ewu^2 * ewv * giepsi * ewu/ssq, FUN = "*"),
      Zigisq) + sapply(1:ngZGvar, function(x) {
      crossprod(Zisqgisq[[x]], as.matrix(wHvar * ewu * sigx1 * sigx3 * giepsi *
        ewu/ssq))
    }) + (crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar * ewu * sigx1 * sigx3 *
      ewu/ssq, FUN = "*"), Zigiepsi) + crossprod(sweep(Zigiepsi, MARGIN = 1,
      STATS = wHvar * ewu * sigx1 * sigx3 * ewu/ssq, FUN = "*"), Zigisq) -
      crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar * ewu * sigx1 * 2 *
        (ewu * ssqx2 * sigx3 * giepsi/ssq) * sigx3 * ewu/ssq, FUN = "*"),
        Zigisq))) + crossprod(sweep(sigZ1, MARGIN = 1, STATS = wHvar * ewu *
      S * (1 - dmusig * (dpmu - S * ewu * giepsi/(ssqx2))/pmusig) * ewu, FUN = "*"),
      sigZ1)) - 0.5 * (sapply(1:ngZGvar, function(x) {
    crossprod(Zisqgisq[[x]], as.matrix(wHvar * ewu/sigmasq))
  }) - crossprod(sweep(Zigisq, MARGIN = 1, STATS = wHvar * ewu * ewu/sigmasq/sigmasq,
    FUN = "*"), Zigisq))
...
#' @noRd
halfnormAlgOpt_mols93 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psthalfnorm_mols93(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(phalfnormlike_mols93(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(phalfnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar,
      Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p))
  }, gr = function(parm) {
    -colSums(pgradhalfnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = phalfnormlike_mols93,
    grad = pgradhalfnormlike_mols93, hess = phesshalfnormlike_mols93, start = startVal,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      ...
      as(-phesshalfnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      -phesshalfnormlike_mols93(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradhalfnormlike_mols93(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- phesshalfnormlike_mols93(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar,
        ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    }
    if (method == "sr1") {
      mleObj$hessian <- phesshalfnormlike_mols93(parm = mleObj$solution, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar,
        ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
        TT = TT, S = S, wHvar = wHvar_p)
    }
  }
  mleObj$logL_OBS <- phalfnormlike_mols93(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  mleObj$gradL_OBS <- pgradhalfnormlike_mols93(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p)
  if (is.null(start)) {
...
#' @noRd
phalfnormeff_mols93 <- function(object, level) {
