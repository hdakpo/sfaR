...
#' @noRd
pLCMhalfnormlike2C_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  theta <- parm[(2 * nXvar + 2 * nuZUvar + 2 * nvZVvar + 2 * ngZGvar + 1):(2 *
    nXvar + 2 * nuZUvar + 2 * nvZVvar + nZHvar + 2 * ngZGvar)]
...
#' @noRd
psLCMfhalfnorm2C_gzit <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar_c,
  ...
  StartVal <- c(Esti[1:(nXvar)], Esti[nXvar + 1], if (nuZUvar > 1) rep(0, nuZUvar -
    ...
    1), Esti[(nXvar + 3):(nXvar + ngZGvar + 2)], rep(0, nZHvar))
...
#' @param nZHvar number of separating variables
#' @noRd
pgradLCMhalfnormlike2C_gzit <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @noRd
LCM2ChnormAlgOpt_gzit <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  if (!is.null(start)) {
    ...
    start_st <- psLCMfhalfnorm2C_gzit(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pLCMhalfnormlike2C_gzit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) {
    -sum(pLCMhalfnormlike2C_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    ...
    -colSums(pgradLCMhalfnormlike2C_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pLCMhalfnormlike2C_gzit,
    grad = pgradLCMhalfnormlike2C_gzit, start = startVal, finalHessian = if (hessianType ==
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) {
      -sum(pLCMhalfnormlike2C_gzit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar))
    }, gr = function(parm) {
      ...
      as(calculus::jacobian(function(parm) -colSums(pgradLCMhalfnormlike2C_gzit(parm,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradLCMhalfnormlike2C_gzit(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb")) {
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradLCMhalfnormlike2C_gzit(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        gHvar = gHvar, ngZGvar = ngZGvar, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
        pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, Zvar = Zvar, nZHvar = nZHvar)),
        unname(mleObj$par))
  ...
  mleObj$logL_OBS <- pLCMhalfnormlike2C_gzit(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    ...
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- pgradLCMhalfnormlike2C_gzit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, gHvar = gHvar, ngZGvar = ngZGvar,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S,
...
#' @noRd
pLCM2Chalfnormeff_gzit <- function(object, level) {
  ...
  theta <- object$mlParam[(2 * object$nXvar + 2 * object$nuZUvar + 2 * object$nvZVvar +
    ...
    object$nvZVvar + 2 * ncol(object$gHvar) + object$nZHvar)]
