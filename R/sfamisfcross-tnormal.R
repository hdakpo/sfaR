...
cmisftruncnormlike_logit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  ...
  delta2 <- parm[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 *
    nuZUvar)]
  phi <- parm[(nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar +
    2 * nuZUvar + nvZVvar)]
  theta <- parm[(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 *
    nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)]
  ...
  mustar1 <- (mu1 * exp(Wv) - exp(Wu1) * S * epsilon)/(exp(Wu1) + exp(Wv))
  sigmastar1 <- sqrt(exp(Wu1) * exp(Wv)/(exp(Wu1) + exp(Wv)))
  mustar2 <- (mu2 * exp(Wv) - exp(Wu2) * S * epsilon)/(exp(Wu2) + exp(Wv))
...
cmisftruncnormlike_cauchit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
cmisftruncnormlike_probit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
cmisftruncnormlike_cloglog <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar, muHvar,
  uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
#' @noRd
cstmisftruncnorm <- function(olsObj, epsiRes, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  ...
  StartVal <- c(Esti[1:nXvar], 0.95 * Esti[nXvar + 1], if (nmuZUvar > 1) rep(0,
    ...
    Esti[nXvar + 2], if (nuZUvar > 1) rep(0, nuZUvar - 1), Esti[nXvar + 3], if (nvZVvar >
...
#' @noRd
# logit specification class membership
cgradmisftruncnormlike_logit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx2/sigx3, FUN = "*"),
    sweep(muHvar, MARGIN = 1, STATS = sigx5_1 * ewz/sigx3, FUN = "*"), sweep(muHvar,
      ...
      prC/sigx3, FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx16_1 * ewv *
      ewz/s3sq1 + sigx16_2 * prC * ewv/s3sq2, FUN = "*"), sweep(Zvar, MARGIN = 1,
      STATS = sigx19 * ewz/sigx3, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

# cauchit specification class membership
cgradmisftruncnormlike_cauchit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx3/sigx2, FUN = "*"),
    sweep(muHvar, MARGIN = 1, STATS = sigx5_1 * ewz1/sigx2, FUN = "*"), sweep(muHvar,
      ...
      ewz2/sigx2, FUN = "*"), sweep(vHvar, MARGIN = 1, STATS = sigx19 * ewv/sigx2,
      FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx9/sigx18, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

# probit specification class membership
cgradmisftruncnormlike_probit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  gradll <- cbind(sweep(Xvar, MARGIN = 1, STATS = S * sigx3/sigx2, FUN = "*"),
    sweep(muHvar, MARGIN = 1, STATS = sigx5_1 * pwZ/sigx2, FUN = "*"), sweep(muHvar,
      ...
      STATS = sigx19 * ewv/sigx2, FUN = "*"), sweep(Zvar, MARGIN = 1, STATS = sigx9 *
      dwZ/sigx2, FUN = "*"))
  return(sweep(gradll, MARGIN = 1, STATS = wHvar, FUN = "*"))
}

# cloglog specification class membership
cgradmisftruncnormlike_cloglog <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
...
#' @noRd
# logit specification class membership
chessmisftruncnormlike_logit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  hessll <- matrix(nrow = nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar,
    ncol = nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)
  ...
  hessll[1:nXvar, (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * (((dmusig1 * sigx21_1 * ewv/ssq1 - sigx20_1 * depsi1)/pmusq1 -
  ...
  hessll[1:nXvar, (nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (((dmusig2 * sigx21_2 * ewv/ssq2 - sigx20_2 *
  ...
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar +
    ...
    sigx22_2 - (sigx23_2 + (pvs2 * ewu2/ssq2^2 - (sigx7_2 * muvu2/ewv + 1/sigmastar2)/(sigma_sq2)) *
  ...
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar +
    ...
    (sigma_sq1)))) * ewz/sqrt(sigma_sq1) + (((sigx21_2 * sigx13_2/(sigma_sq2) -
    sigx12_2 * depsi2 * ewu2/ssq2^2) * dmusig2 + 0.5 * sigx22_2)/pmu2 - 0.5 *
    (sigx1_2 * pmu2/((sigma_sq2) * pmusq2^2))) * prC/sqrt(sigma_sq2) - (sigx16_1 *
    ewz/sqrt(sigma_sq1) + sigx16_2 * prC/sqrt(sigma_sq2)) * sigx2/sigx3) * ewv/sigx3,
    FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 *
    nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * (sigx17 * sigx1_1/(sigma_sq1) - (sigx2 * sigx19/sigx3 +
      prC * sigx1_2/(wzdeno * (sigma_sq2) * pmu2 * sqrt(sigma_sq2)))) * ewz/sigx3,
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = -wHvar * (((((1 - musi1^2/(sigma_sq1)) * pmusig1 + dmusig1 *
      ewv * musi1/ssq1) * depsi1 + dmusig1 * sigx24_1 * ewv/ssq1)/spmu1 + ((sigx4_1/(spmu1^2 *
      ewu1_h) - 2 * (wzdeno^2 * depsi1 * dmu1 * pmusig1 * pmu1/wup1^2)) * (sigma_sq1) +
      (dmusig1 * depsi1 * ewv/ssq1 - (depsi1 * musi1/(sigma_sq1) + mu1 * depsi1/ewu1_h^2) *
        pmusig1)/wup1) * wzdeno * dmu1 * sqrt(sigma_sq1) + sigx5_1^2 * ewz/sigx3) *
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 1):(nXvar + 2 *
    ...
    (((0.5 * sigx27_1 - ((pvs1 * ewv/ssq1^2 - sigx7_1 * muvu1/(ewu1 * (sigma_sq1))) *
      ...
      (dmusig1 * ewv/ssq1 - 2 * (wzdeno^2 * dmu1 * (sigma_sq1) * pmusig1 *
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((((1/ssq1 - sigx12_1 * ewv/ssq1^2) * depsi1 -
      ...
      wzdeno)/sqrt(sigma_sq1) - (sigx16_1 * ewz/sqrt(sigma_sq1) + sigx16_2 *
      prC/sqrt(sigma_sq2)) * sigx5_1/sigx3) * ewv * ewz/sigx3, FUN = "*"),
    vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * (sigx17 * dmusig1 * depsi1 * ewv/ssq1 - ((((2 -
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + nmuZUvar + 1):(nXvar +
    ...
    musi2^2/(sigma_sq2)) * pmusig2 + dmusig2 * ewv * musi2/ssq2) * depsi2 + dmusig2 *
    sigx24_2 * ewv/ssq2)/spmu2 + ((sigx4_2/(spmu2^2 * ewu2_h) - 2 * (depsi2 *
    dmu2 * pmusig2 * pmu2/wup2^2)) * (sigma_sq2) + (dmusig2 * depsi2 * ewv/ssq2 -
    (depsi2 * musi2/(sigma_sq2) + mu2 * depsi2/ewu2_h^2) * pmusig2)/wup2) * dmu2 *
    sqrt(sigma_sq2) + sigx5_2^2 * prC/sigx3) * prC/sigx3), FUN = "*"), muHvar)
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((0.5 * sigx27_2 - ((pvs2 * ewv/ssq2^2 - sigx7_2 *
      ...
      (0.5 * sigx9_2 - 0.5 * sigx10_2) * (dmusig2 * ewv/ssq2 - 2 * (dmu2 *
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((((1/ssq2 - sigx12_2 * ewv/ssq2^2) * depsi2 -
      ...
      (sigx16_1 * ewz/sqrt(sigma_sq1) + sigx16_2 * prC/sqrt(sigma_sq2)) * sigx5_2/sigx3) *
      prC * ewv/sigx3, FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar +
    nZHvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = -wHvar * ((sigx19 *
    sigx5_2/sigx3 + sigx4_2/(wzdeno * (sigma_sq2) * pmu2 * sqrt(sigma_sq2)) -
  ...
  hessll[(nXvar + 2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar), (nXvar +
    ...
    MARGIN = 1, STATS = wHvar * ((((((sigx7_1 * depsi1 * muvu1/sigmastar1 + 0.5 *
      ...
      ewv/(sigma_sq1)) + 0.5 * ((ewu1/(sigma_sq1) - 1) * ewv/(sigma_sq1) +
      ...
      pmusig1/(sigma_sq1) - sigx7_1 * dmusig1) * depsi1 * musi1^2/(sigma_sq1)^2))/(wzdeno *
  ...
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    ...
    MARGIN = 1, STATS = wHvar * ((((0.5 * (((0.5 * (musi2^2/(sigma_sq2)) - 2) *
      ...
      0.5 * (0.5 * prU2 + ewu2/(sigma_sq2))) * prU2 * ewv * muvu2/sigmastar2 -
  ...
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    (nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar +
      nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * ((((((sigx7_2 *
    depsi2 * muvu2/sigmastar2 + 0.5 * (depsi2 * musi2^2/(sigma_sq2))) * sigx13_2/(sigma_sq2) -
    ((0.5 * (prU2 * ewv/(sigma_sq2)) + 0.5 * ((ewu2/(sigma_sq2) - 1) * ewv/(sigma_sq2) +
      1 - 0.5 * (prU2 * prV2))) * muvu2/sigmastar2 + mu2 * pvs2 - sigx12_2 *
      (2 * (pvs2 * (sigma_sq2) * muvu2 * sigmastar2/ssq2^2) + S * (epsilon))) *
      depsi2/ssq2^2) * dmusig2 + 0.5 * (((0.5 * (musi2^2/(sigma_sq2)) - 2) *
    pmusig2/(sigma_sq2) - sigx7_2 * dmusig2) * depsi2 * musi2^2/(sigma_sq2)^2)) *
    ewu2 + 0.5 * (mu2 * sigx14_2 * dmu2/(ewu2_h * pmu2)))/pmu2 - (0.5 * ((sigx8_2 *
    ewu2 * pmu2 - (0.5 * (mu2 * dmu2/ewu2_h) + 2 * ((0.5 * sigx9_2 - 0.5 * sigx10_2) *
    pmu2^2 * sqrt(sigma_sq2)/pmusq2^2)) * depsi2 * pmusig2)/pmusq2^2) + 0.5 *
    (sigx16_2 * ewu2/(sigma_sq2))))/sqrt(sigma_sq2) - (sigx16_1 * ewz/sqrt(sigma_sq1) +
    sigx16_2 * prC/sqrt(sigma_sq2)) * sigx11_2/sigx3) * prC * ewv/sigx3, FUN = "*"),
    vHvar)
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar +
      2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
  ...
  hessll[(nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    ...
    (((((0.5 * (depsi1 * musi1^2/(sigma_sq1)) - depsi1 * muvu1 * sigx13_1/sigmastar1) *
      ewv * sigx13_1/(sigma_sq1) + depsi1 * (mu1/ssq1 - (((3 * (mu1) - 2 *
      (sigx12_1 * (sigma_sq1) * muvu1 * sigmastar1/ssq1^2)) * ewv - S * ewu1 *
      (epsilon)) * sigx12_1 + (0.5 * (ewv/(sigma_sq1)) - 0.5 * (0.5 * prV1 +
      ewv/(sigma_sq1))) * prV1 * ewu1 * muvu1/sigmastar1)/ssq1^2)) * dmusig1 +
      (0.5 * (((0.5 * (musi1^2/(sigma_sq1)) - 2) * pmusig1/(sigma_sq1) + dmusig1 *
        sigx13_1) * ewv) + 0.5 * pmusig1) * depsi1 * musi1^2/(sigma_sq1)^2)/(wzdeno *
  ...
  hessll[(nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    ...
    nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * ((0.5 * (sigx17 * depsi1 * musi1^2/(sigma_sq1)^2) - ((0.5/sqrt(sigma_sq1) -
      sigx26_1) * ewz + 0.5 * (wzdeno/sqrt(sigma_sq1))) * depsi1 * pmu1/pmusq1^2) *
      pmusig1 + sigx17 * dmusig1 * depsi1 * sigx13_1 - ((sigx16_1 * ewz/sqrt(sigma_sq1) +
      sigx16_2 * prC/sqrt(sigma_sq2)) * sigx19/sigx3 + (sigx14_2/(wzdeno *
      pmu2) - 0.5 * (wzdeno * depsi2 * pmusig2 * pmu2/sigx18^2)) * prC/sqrt(sigma_sq2))) *
      ewv * ewz/sigx3, FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar +
    2 * nuZUvar + nvZVvar + nZHvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar +
    1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Zvar,
...
# cauchit specification class membership
chessmisftruncnormlike_cauchit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  hessll[1:nXvar, (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * (((dmusig1 * (depsi1 * mupsi1 - depsi1 * musi1/ewv) *
      ewv/starsq1 - ((mupsi1^2/sigma_sq1 - 1) * pmusig1 + dmusig1 * ewu1 *
  ...
  hessll[1:nXvar, (nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * (((dmusig2 * (depsi2 * mupsi2 - depsi2 *
      musi2/ewv) * ewv/starsq2 - ((mupsi2^2/sigma_sq2 - 1) * pmusig2 + dmusig2 *
  ...
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = wHvar * S * ((0.5 * (((mupsi1^2/sigma_sq1 - 2) * pmusig1 +
      ...
      depsi1 * mupsi1/sigma_sq1 + (sigx7_1 * ewu1/starsq1^2 - (sigx8_1 * musi1/ewv +
  ...
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar +
    ...
    (sigx7_2 * ewu2/starsq2^2 - (sigx8_2 * musi2/ewv + 1/sigmastar2)/sigma_sq2) *
      depsi2) * dmusig2) * ewu2/psq2 - (sigx3 * sigx11_2/sigx2 + (0.5 * wup2 -
    0.5 * mud2) * sigx1_2/(sigma_sq2 * psq2^2))) * ewz2/sigx2, FUN = "*"), uHvar)
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar +
    2 * nuZUvar + nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar *
    S * (((((depsi1 * mupsi1 - depsi1 * musi1/ewv) * sigx13_1/sigma_sq1 - sigx12_1 *
    ...
    (sigx1_1 * pmu1/(sigma_sq1 * psq1^2))) * ewz1/sqrt(sigma_sq1) + ((((depsi2 *
    mupsi2 - depsi2 * musi2/ewv) * sigx13_2/sigma_sq2 - sigx12_2 * depsi2 * ewu2/starsq2^2) *
    dmusig2 + 0.5 * (((mupsi2^2/sigma_sq2 - 2) * pmusig2 + dmusig2 * ewu2 * mupsi2/starsq2) *
    depsi2 * mupsi2/sigma_sq2^2))/pmu2 - 0.5 * (sigx1_2 * pmu2/(sigma_sq2 * psq2^2))) *
    ewz2/sqrt(sigma_sq2) - sigx19 * sigx3/sigx2) * ewv/sigx2, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 *
    nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * ((sigx1_1/ssqq1 - sigx1_2/ssqq2)/sigx18 - pi * sigx3 *
      ((Wz)^2 + 1) * sigx9/sigx18^2), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = -wHvar * (((((1 - mupsi1^2/sigma_sq1) * pmusig1 + dmusig1 *
      ewv * mupsi1/starsq1) * depsi1 + dmusig1 * (depsi1 * musi1/ewu1 + depsi1 *
      mupsi1) * ewv/starsq1)/ssqq1 + ((sigx4_1/(ssqq1^2 * ewusr1) - 2 * (depsi1 *
      dmu1 * pmusig1 * pmu1/(ewusr1 * psq1^2)^2)) * sigma_sq1 + (dmusig1 *
      depsi1 * ewv/starsq1 - (depsi1 * mupsi1/sigma_sq1 + mu1 * depsi1/ewusr1^2) *
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 1):(nXvar + 2 *
    nmuZUvar + nuZUvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = wHvar *
    (((0.5 * (((2 - mupsi1^2/sigma_sq1) * pmusig1 + dmusig1 * ewv * mupsi1/starsq1) *
      depsi1 * mupsi1/sigma_sq1^2) - ((sigx7_1 * ewv/starsq1^2 - sigx8_1 *
      ...
      ewv/starsq1 - 2 * (dmu1 * sigma_sq1 * pmusig1 * pmu1/(ewusr1 * psq1^2))) *
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * (((((1/starsq1 - sigx12_1 * ewv/starsq1^2) *
      depsi1 - (depsi1 * musi1/ewu1 + depsi1 * mupsi1) * sigx13_1/sigma_sq1) *
      dmusig1 + 0.5 * (((2 - mupsi1^2/sigma_sq1) * pmusig1 + dmusig1 * ewv *
      mupsi1/starsq1) * depsi1 * mupsi1/sigma_sq1^2) - sigx15_1 * dmu1/(ewusr1 *
      pmu1))/pmu1 - 0.5 * (((1 - 2 * (sigma_sq1 * pmu1^2/psq1^2)) * depsi1 *
      dmu1 * pmusig1/ewusr1 + sigx4_1 * pmu1/sigma_sq1)/psq1^2))/sqrt(sigma_sq1) -
      sigx19 * sigx5_1/sigx2) * ewz1 * ewv/sigx2, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(muHvar,
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + nmuZUvar + 1):(nXvar +
    2 * nmuZUvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = -wHvar * (((((1 -
    mupsi2^2/sigma_sq2) * pmusig2 + dmusig2 * ewv * mupsi2/starsq2) * depsi2 +
    dmusig2 * (depsi2 * musi2/ewu2 + depsi2 * mupsi2) * ewv/starsq2)/ssqq2 +
    ((sigx4_2/(ssqq2^2 * ewusr2) - 2 * (depsi2 * dmu2 * pmusig2 * pmu2/(ewusr2 *
      psq2^2)^2)) * sigma_sq2 + (dmusig2 * depsi2 * ewv/starsq2 - (depsi2 *
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((0.5 * (((2 - mupsi2^2/sigma_sq2) * pmusig2 +
      dmusig2 * ewv * mupsi2/starsq2) * depsi2 * mupsi2/sigma_sq2^2) - ((sigx7_2 *
      ewv/starsq2^2 - sigx8_2 * musi2/(ewu2 * sigma_sq2)) * depsi2 - sigx8_2 *
      ...
      ewv/starsq2 - 2 * (dmu2 * sigma_sq2 * pmusig2 * pmu2/(ewusr2 * psq2^2))) *
      depsi2)/psq2^2 + sigx11_2 * sigx5_2 * ewz2/sigx2)) * ewz2/sigx2, FUN = "*"),
    uHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = wHvar * (((((1/starsq2 - sigx12_2 * ewv/starsq2^2) *
      depsi2 - (depsi2 * musi2/ewu2 + depsi2 * mupsi2) * sigx13_2/sigma_sq2) *
      dmusig2 + 0.5 * (((2 - mupsi2^2/sigma_sq2) * pmusig2 + dmusig2 * ewv *
      ...
      sigx19 * sigx5_2/sigx2) * ewz2 * ewv/sigx2, FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar +
    nZHvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = -wHvar * (sigx5_2 *
    (1/sigx18 + pi * ((Wz)^2 + 1) * ewz2 * sigx9/sigx18^2)), FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar), (nXvar +
    2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar)] <- crossprod(sweep(uHvar,
  ...
  hessll[(nXvar + 2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar), (nXvar +
    ...
    MARGIN = 1, STATS = wHvar * ((((((sigx8_1 * depsi1 * musi1/sigmastar1 + 0.5 *
      ...
      ewv/sigma_sq1) + 0.5 * ((ewu1/sigma_sq1 - 1) * ewv/sigma_sq1 + 1 - 0.5 *
      (usq1 * vsq1))) * musi1/sigmastar1 + mu1 * sigx7_1 - sigx12_1 * (2 *
      (sigx7_1 * sigma_sq1 * musi1 * sigmastar1/starsq1^2) + S * (epsilon))) *
  ...
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    ...
    MARGIN = 1, STATS = wHvar * ((((0.5 * (((0.5 * (mupsi2^2/sigma_sq2) - 2) *
      ...
      0.5 * (0.5 * usq2 + ewu2/sigma_sq2)) * usq2 * ewv * musi2/sigmastar2 -
  ...
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    (nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar +
      nvZVvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar * ((((((sigx8_2 *
    depsi2 * musi2/sigmastar2 + 0.5 * (depsi2 * mupsi2^2/sigma_sq2)) * sigx13_2/sigma_sq2 -
    ((0.5 * (usq2 * ewv/sigma_sq2) + 0.5 * ((ewu2/sigma_sq2 - 1) * ewv/sigma_sq2 +
      1 - 0.5 * (usq2 * vsq2))) * musi2/sigmastar2 + mu2 * sigx7_2 - sigx12_2 *
      (2 * (sigx7_2 * sigma_sq2 * musi2 * sigmastar2/starsq2^2) + S * (epsilon))) *
      depsi2/starsq2^2) * dmusig2 + 0.5 * (((0.5 * (mupsi2^2/sigma_sq2) - 2) *
    pmusig2/sigma_sq2 - sigx8_2 * dmusig2) * depsi2 * mupsi2^2/sigma_sq2^2)) *
    ewu2 + 0.5 * (mu2 * sigx15_2 * dmu2/(ewusr2 * pmu2)))/pmu2 - (0.5 * ((sigx10_2 *
    ewu2 * pmu2 - (0.5 * (mu2 * dmu2/ewusr2) + 2 * ((0.5 * wup2 - 0.5 * mud2) *
    pmu2^2 * sqrt(sigma_sq2)/psq2^2)) * depsi2 * pmusig2)/psq2^2) + 0.5 * (sigx16_2 *
    ewu2/sigma_sq2)))/sqrt(sigma_sq2) - sigx19 * sigx11_2/sigx2) * ewz2 * ewv/sigx2,
    FUN = "*"), vHvar)
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar +
      2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
  ...
  hessll[(nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    ...
    (((((0.5 * (depsi1 * mupsi1^2/sigma_sq1) - depsi1 * musi1 * sigx13_1/sigmastar1) *
      ewv * sigx13_1/sigma_sq1 + depsi1 * (mu1/starsq1 - (((3 * (mu1) - 2 *
      (sigx12_1 * sigma_sq1 * musi1 * sigmastar1/starsq1^2)) * ewv - S * ewu1 *
      (epsilon)) * sigx12_1 + (0.5 * (ewv/sigma_sq1) - 0.5 * (0.5 * vsq1 +
      ewv/sigma_sq1)) * vsq1 * ewu1 * musi1/sigmastar1)/starsq1^2)) * dmusig1 +
      (0.5 * (((0.5 * (mupsi1^2/sigma_sq1) - 2) * pmusig1/sigma_sq1 + dmusig1 *
        sigx13_1) * ewv) + 0.5 * pmusig1) * depsi1 * mupsi1^2/sigma_sq1^2)/pmu1 -
...
# probit specification class membership
chessmisftruncnormlike_probit <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar +
    ...
    depsi1 * ewu1/starsq1^2) * dmusig1 + 0.5 * (((mupsi1^2/sigma_sq1 - 2) * pmusig1 +
    dmusig1 * ewu1 * mupsi1/starsq1) * depsi1 * mupsi1/sigma_sq1^2))/pmu1 - 0.5 *
    (sigx1_1 * pmu1/(sigma_sq1 * psq1^2))) * pwZ/sqrt(sigma_sq1) + ((((depsi2 *
    mupsi2 - depsi2 * musi2/ewv) * sigx13_2/sigma_sq2 - sigx12_2 * depsi2 * ewu2/starsq2^2) *
    dmusig2 + 0.5 * (((mupsi2^2/sigma_sq2 - 2) * pmusig2 + dmusig2 * ewu2 * mupsi2/starsq2) *
    depsi2 * mupsi2/sigma_sq2^2))/pmu2 - 0.5 * (sigx1_2 * pmu2/(sigma_sq2 * psq2^2))) *
    (1 - pwZ)/sqrt(sigma_sq2) - sigx19 * sigx3/sigx2) * ewv/sigx2, FUN = "*"),
    vHvar)
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 *
    nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * (sigx1_1/ssqq1 - (sigx3 * sigx9/sigx2 + sigx1_2/ssqq2)) *
      dwZ/sigx2, FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 1):(nXvar + nmuZUvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = -wHvar * (((((1 - mupsi1^2/sigma_sq1) * pmusig1 + dmusig1 *
      ewv * mupsi1/starsq1) * depsi1 + dmusig1 * (depsi1 * musi1/ewu1 + depsi1 *
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((((1/starsq1 - sigx12_1 * ewv/starsq1^2) *
      ...
      pmu1))/pmu1 - 0.5 * (((1 - 2 * (sigma_sq1 * pmu1^2/psq1^2)) * depsi1 *
      dmu1 * pmusig1/ewusr1 + sigx4_1 * pmu1/sigma_sq1)/psq1^2))/sqrt(sigma_sq1) -
      sigx19 * sigx5_1/sigx2) * pwZ * ewv/sigx2, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(muHvar,
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((((1/starsq2 - sigx12_2 * ewv/starsq2^2) *
      ...
      pmu2))/pmu2 - 0.5 * (((1 - 2 * (sigma_sq2 * pmu2^2/psq2^2)) * depsi2 *
      dmu2 * pmusig2/ewusr2 + sigx4_2 * pmu2/sigma_sq2)/psq2^2))/sqrt(sigma_sq2) -
      sigx19 * sigx5_2/sigx2) * (1 - pwZ) * ewv/sigx2, FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar +
    nZHvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = -wHvar * (((1 - pwZ) *
    sigx9/sigx2 + 1) * sigx5_2 * dwZ/sigx2), FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar), (nXvar +
    2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((0.5 * (((0.5 * (mupsi1^2/sigma_sq1) - 2) *
      pmusig1/sigma_sq1 - sigx8_1 * dmusig1) * ewu1) + 0.5 * pmusig1) * depsi1 *
  ...
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    ...
    ewu2 * pmu2 - (0.5 * (mu2 * dmu2/ewusr2) + 2 * ((0.5 * wup2 - 0.5 * mud2) *
    pmu2^2 * sqrt(sigma_sq2)/psq2^2)) * depsi2 * pmusig2)/psq2^2) + 0.5 * (sigx16_2 *
    ewu2/sigma_sq2)))/sqrt(sigma_sq2) - sigx19 * sigx11_2/sigx2) * (1 - pwZ) *
    ewv/sigx2, FUN = "*"), vHvar)
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar +
      2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
  ...
  hessll[(nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    nvZVvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 *
    nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = wHvar * (sigx16_1/sqrt(sigma_sq1) - (sigx19 * sigx9/sigx2 + sigx16_2/sqrt(sigma_sq2))) *
...
# cloglog specification class membership
chessmisftruncnormlike_cloglog <- function(parm, nXvar, nmuZUvar, nuZUvar, nvZVvar,
  muHvar, uHvar, vHvar, Yvar, Xvar, S, wHvar, Zvar, nZHvar) {
  beta <- parm[1:(nXvar)]
  omega1 <- parm[(nXvar + 1):(nXvar + nmuZUvar)]
  omega2 <- parm[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar)]
  ...
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + 1):(nXvar + 2 * nmuZUvar +
    ...
    depsi1 * ewu1/starsq1^2) * dmusig1 + 0.5 * (((mupsi1^2/sigma_sq1 - 2) * pmusig1 +
    dmusig1 * ewu1 * mupsi1/starsq1) * depsi1 * mupsi1/sigma_sq1^2))/pmu1 - 0.5 *
    (sigx1_1 * pmu1/(sigma_sq1 * psq1^2))) * (1 - prZ)/sqrt(sigma_sq1) + ((((depsi2 *
    mupsi2 - depsi2 * musi2/ewv) * sigx13_2/sigma_sq2 - sigx12_2 * depsi2 * ewu2/starsq2^2) *
    dmusig2 + 0.5 * (((mupsi2^2/sigma_sq2 - 2) * pmusig2 + dmusig2 * ewu2 * mupsi2/starsq2) *
    depsi2 * mupsi2/sigma_sq2^2))/pmu2 - 0.5 * (sigx1_2 * pmu2/(sigma_sq2 * psq2^2))) *
    prZ/sqrt(sigma_sq2) - sigx19 * sigx3/sigx2) * ewv/sigx2, FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 *
    nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = wHvar * S * (sigx1_1/ssqq1 - (sigx3 * sigx9/sigx2 + sigx1_2/ssqq2)) *
  ...
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((((1/starsq1 - sigx12_1 * ewv/starsq1^2) *
      ...
      pmu1))/pmu1 - 0.5 * (((1 - 2 * (sigma_sq1 * pmu1^2/psq1^2)) * depsi1 *
      dmu1 * pmusig1/ewusr1 + sigx4_1 * pmu1/sigma_sq1)/psq1^2))/sqrt(sigma_sq1) -
      sigx19 * sigx5_1/sigx2) * (1 - prZ) * ewv/sigx2, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuZUvar), (nXvar + 2 * nmuZUvar + 2 * nuZUvar +
    nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(muHvar,
  ...
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    ...
    MARGIN = 1, STATS = wHvar * (((((1/starsq2 - sigx12_2 * ewv/starsq2^2) *
      ...
      pmu2))/pmu2 - 0.5 * (((1 - 2 * (sigma_sq2 * pmu2^2/psq2^2)) * depsi2 *
      dmu2 * pmusig2/ewusr2 + sigx4_2 * pmu2/sigma_sq2)/psq2^2))/sqrt(sigma_sq2) -
      sigx19 * sigx5_2/sigx2) * prZ * ewv/sigx2, FUN = "*"), vHvar)
  hessll[(nXvar + nmuZUvar + 1):(nXvar + 2 * nmuZUvar), (nXvar + 2 * nmuZUvar +
    2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar +
    nZHvar)] <- crossprod(sweep(muHvar, MARGIN = 1, STATS = -wHvar * ((sigx9 *
    prZ/sigx2 + 1) * sigx5_2 * prZ * ewz/sigx2), FUN = "*"), Zvar)
  hessll[(nXvar + 2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar), (nXvar +
    2 * nmuZUvar + 1):(nXvar + 2 * nmuZUvar + nuZUvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = wHvar * ((((0.5 * (((0.5 * (mupsi1^2/sigma_sq1) - 2) *
      pmusig1/sigma_sq1 - sigx8_1 * dmusig1) * ewu1) + 0.5 * pmusig1) * depsi1 *
  ...
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    ...
    ewu2 * pmu2 - (0.5 * (mu2 * dmu2/ewusr2) + 2 * ((0.5 * wup2 - 0.5 * mud2) *
    pmu2^2 * sqrt(sigma_sq2)/psq2^2)) * depsi2 * pmusig2)/psq2^2) + 0.5 * (sigx16_2 *
    ewu2/sigma_sq2)))/sqrt(sigma_sq2) - sigx19 * sigx11_2/sigx2) * prZ * ewv/sigx2,
    FUN = "*"), vHvar)
  hessll[(nXvar + 2 * nmuZUvar + nuZUvar + 1):(nXvar + 2 * nmuZUvar + 2 * nuZUvar),
    (nXvar + 2 * nmuZUvar + 2 * nuZUvar + nvZVvar + 1):(nXvar + 2 * nmuZUvar +
      2 * nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
...
#' @noRd
# logit specification class membership
misftruncnormAlgOpt_logit <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  startLoglik <- sum(cmisftruncnormlike_logit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisftruncnormlike_logit(parm,
    ...
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisftruncnormlike_logit,
    ...
    fn = function(parm) -sum(cmisftruncnormlike_logit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar,
      nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisftruncnormlike_logit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    ...
    epsa = gradtol, epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisftruncnormlike_logit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisftruncnormlike_logit(mleObj$par, nXvar = nXvar,
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmisftruncnormlike_logit(parm = mleObj$par, nXvar = nXvar,
  ...
  mleObj$logL_OBS <- cmisftruncnormlike_logit(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar,
    nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisftruncnormlike_logit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)
...
# cauchit specification class membership
misftruncnormAlgOpt_cauchit <- function(start, randStart, sdStart, olsParam, dataTable,
  S, wHvar, nXvar, muHvar, nmuZUvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, whichStart, initIter, initAlg,
  tol, gradtol, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(cmisftruncnormlike_cauchit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar,
    nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisftruncnormlike_cauchit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    ...
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)), hessian = 0, control = list(trace = printInfo,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisftruncnormlike_cauchit,
    ...
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar, nZHvar = nZHvar), sr1 = trustOptim::trust.optim(x = startVal,
    fn = function(parm) -sum(cmisftruncnormlike_cauchit(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
      Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisftruncnormlike_cauchit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    fn = function(parm) -sum(cmisftruncnormlike_cauchit(parm, nXvar = nXvar,
      ...
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar), print.info = printInfo, maxiter = itermax,
    epsa = gradtol, epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisftruncnormlike_cauchit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    ...
    eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisftruncnormlike_cauchit(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
      Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmisftruncnormlike_cauchit(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar,
        nmuZUvar = nmuZUvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmisftruncnormlike_cauchit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisftruncnormlike_cauchit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik, mleObj = mleObj,
...
# probit specification class membership
misftruncnormAlgOpt_probit <- function(start, randStart, sdStart, olsParam, dataTable,
  S, wHvar, nXvar, muHvar, nmuZUvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, whichStart, initIter, initAlg,
  tol, gradtol, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(cmisftruncnormlike_probit(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar,
    nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisftruncnormlike_probit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    ...
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)), hessian = 0, control = list(trace = printInfo,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisftruncnormlike_probit,
    ...
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar, nZHvar = nZHvar), sr1 = trustOptim::trust.optim(x = startVal,
    fn = function(parm) -sum(cmisftruncnormlike_probit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar,
      nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisftruncnormlike_probit(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar)), method = "SR1", control = list(maxit = itermax,
      cgtol = gradtol, stop.trust.radius = tol, prec = tol, report.level = if (printInfo) 4L else 0,
    ...
    fn = function(parm) -sum(cmisftruncnormlike_probit(parm, nXvar = nXvar, nuZUvar = nuZUvar,
      ...
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar), print.info = printInfo, maxiter = itermax,
    epsa = gradtol, epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisftruncnormlike_probit(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    ...
    eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisftruncnormlike_probit(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
      Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmisftruncnormlike_probit(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar,
        nmuZUvar = nmuZUvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmisftruncnormlike_probit(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar,
    nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisftruncnormlike_probit(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik, mleObj = mleObj,
...
# cloglog specification class membership
misftruncnormAlgOpt_cloglog <- function(start, randStart, sdStart, olsParam, dataTable,
  S, wHvar, nXvar, muHvar, nmuZUvar, uHvar, nuZUvar, vHvar, nvZVvar, Zvar, nZHvar,
  Yvar, Xvar, method, printInfo, itermax, stepmax, whichStart, initIter, initAlg,
  tol, gradtol, hessianType, qac) {
  start_st <- if (!is.null(start))
  ...
  if (randStart)
    startVal <- startVal + rnorm(length(startVal), sd = sdStart)
  startLoglik <- sum(cmisftruncnormlike_cloglog(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar,
    nZHvar = nZHvar))
  if (method %in% c("bfgs", "bhhh", "nr", "nm", "cg", "sann")) {
  ...
  cat("MISF Estimation...\n")
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(cmisftruncnormlike_cloglog(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    ...
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)), hessian = 0, control = list(trace = printInfo,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = cmisftruncnormlike_cloglog,
    ...
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar,
    muHvar = muHvar, nmuZUvar = nmuZUvar, Zvar = Zvar, nZHvar = nZHvar), sr1 = trustOptim::trust.optim(x = startVal,
    fn = function(parm) -sum(cmisftruncnormlike_cloglog(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
      Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradmisftruncnormlike_cloglog(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    ...
    fn = function(parm) -sum(cmisftruncnormlike_cloglog(parm, nXvar = nXvar,
      ...
      Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar), print.info = printInfo, maxiter = itermax,
    epsa = gradtol, epsb = gradtol), nlminb = nlminb(start = startVal, objective = function(parm) -sum(cmisftruncnormlike_cloglog(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    ...
    eval.max = itermax, rel.tol = tol, x.tol = tol)))
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(cgradmisftruncnormlike_cloglog(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
      Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
      Zvar = Zvar, nZHvar = nZHvar))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- chessmisftruncnormlike_cloglog(parm = mleObj$par, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar,
        nmuZUvar = nmuZUvar, Zvar = Zvar, nZHvar = nZHvar)
    if (method == "sr1")
  ...
  mleObj$logL_OBS <- cmisftruncnormlike_cloglog(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)
  mleObj$gradL_OBS <- cgradmisftruncnormlike_cloglog(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S, wHvar = wHvar, muHvar = muHvar, nmuZUvar = nmuZUvar,
    Zvar = Zvar, nZHvar = nZHvar)
  return(list(startVal = startVal, startLoglik = startLoglik, mleObj = mleObj,
...
cmisftruncnormeff_logit <- function(object, level) {
  ...
  phi <- object$mlParam[(object$nXvar + 2 * object$nmuZUvar + 2 * object$nuZUvar +
  ...
  mustar1 <- (mu1 * exp(Wv) - exp(Wu1) * object$S * epsilon)/(exp(Wu1) + exp(Wv))
  mustar2 <- (mu2 * exp(Wv) - exp(Wu2) * object$S * epsilon)/(exp(Wu2) + exp(Wv))
  sigmastar1 <- sqrt(exp(Wu1) * exp(Wv)/(exp(Wu1) + exp(Wv)))
  sigmastar2 <- sqrt(exp(Wu2) * exp(Wv)/(exp(Wu2) + exp(Wv)))
  Pi1 <- 1/sqrt(exp(Wu1) + exp(Wv)) * dnorm((mu1 + object$S * epsilon)/sqrt(exp(Wu1) +
    exp(Wv))) * pnorm(mustar1/sigmastar1)/pnorm(mu1/sqrt(exp(Wu1)))
  Pi2 <- 1/sqrt(exp(Wu2) + exp(Wv)) * dnorm((mu2 + object$S * epsilon)/sqrt(exp(Wu2) +
    exp(Wv))) * pnorm(mustar2/sigmastar2)/pnorm(mu2/sqrt(exp(Wu2)))
  Probc1 <- exp(Wz)/(1 + exp(Wz))
  Probc2 <- 1 - Probc1
