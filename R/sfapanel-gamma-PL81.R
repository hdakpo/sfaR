...
#' @noRd
pgammanormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  ...
  ll <- -1/2 * P * Wu - (TT - 1) * Wv/2 - 1/2 * log(TT) - (TT - 1)/2 * log(2 *
...
#' @noRd
pstgammanorm_pl81 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  Yvar, Xvar, S, wHvar, whichStart, initIter, initAlg, printInfo, tol, N, FiMat) {
  if (whichStart == 1L) {
...
#' @param FiMat matrix of random draws
#' @noRd
pgradgammanormlike_pl81 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
  ...
  sigFi1 <- qnorm(sweep((1 - FiMat), MARGIN = 1, STATS = pmusig2, FUN = "*") +
    FiMat)
  sigFi2 <- dnorm(sigFi1)
  sigFi3 <- sweep(sigFi1, MARGIN = 1, STATS = ewv_h/sqrt(TT), FUN = "*")
  ...
  sigFi6 <- sweep((1 - FiMat)/sigFi2, MARGIN = 1, STATS = (dmusig2 * sigx3) * ewv_h,
    FUN = "*") + sweep(0.5 * sigFi1, MARGIN = 1, STATS = ewv_h/sqrt(TT), FUN = "*")
  sigFi7 <- sweep(sigFi6, MARGIN = 1, STATS = (ewv/(TT * ewu_h)), FUN = "-")
  sigFi8 <- sweep((0.5 - 0.5 * (sigFi5)) * (sigFi4)^(P - 2), MARGIN = 1, STATS = ewv *
    (P - 1)/(TT * ewu_h), FUN = "*")
  gx <- matrix(nrow = N, ncol = nXvar)
  for (k in 1:nXvar) {
    gx[, k] <- apply(sweep(S * (sigFi5 - 1) * (sigFi4)^(P - 2) * (P - 1), MARGIN = 1,
      STATS = X_iM[, k]/TT, FUN = "*"), 1, sum)/sdDiv
  }
  gu <- matrix(nrow = N, ncol = nuZUvar)
  for (k in 1:nuZUvar) {
    gu[, k] <- apply(sweep(sigFi8, MARGIN = 1, STATS = uHvar[, k], FUN = "*"),
      1, sum)/sdDiv
  }
  gv <- matrix(nrow = N, ncol = nvZVvar)
  for (k in 1:nvZVvar) {
    gv[, k] <- apply(sweep(sigFi7 * (sigFi4)^(P - 2) * (P - 1), MARGIN = 1, STATS = vHvar[,
      k], FUN = "*"), 1, sum)/sdDiv
  }
  gradll <- cbind(sweep(X_iM, MARGIN = 1, STATS = S * sigx5/(TT * ewv_h), FUN = "*") +
    gx - sweep(Xepsi_i, MARGIN = 1, STATS = 1/ewv, FUN = "*"), gu + sweep(uHvar,
    MARGIN = 1, STATS = ((0.5 * (dmusig * sqrt(TT)/pmusig) - 0.5 * (epsi_uv/ewv_h)) *
      ttsq - 0.5 * P), FUN = "*"), gv + sweep(vHvar, MARGIN = 1, STATS = (sigx5 *
    sigx3 + 2 * sigx2 - 0.5 * (TT - 1)), FUN = "*"), apply((sigFi4)^(P - 1) *
    log(sigFi4), 1, sum)/sdDiv - (0.5 * (Wu) + digamma(P)))
...
#' @noRd
gammanormAlgOpt_pl81 <- function(start, randStart, sdStart, olsParam, dataTable,
  ...
  startLoglik <- sum(pgammanormlike_pl81(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pgammanormlike_pl81(parm,
    ...
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgammanormlike_pl81,
    ...
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pgammanormlike_pl81(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_pl81(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      fn = function(parm) -sum(pgammanormlike_pl81(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        ...
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradgammanormlike_pl81(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      objective = function(parm) -sum(pgammanormlike_pl81(parm, nXvar = nXvar,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgammanormlike_pl81(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
      Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  }
  ...
  mleObj$logL_OBS <- pgammanormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradgammanormlike_pl81(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
pgammanormeff_pl81 <- function(object, level) {
  ...
  if (object$logDepVar == TRUE) {
    ...
    teBC <- exp(-mui_Gi + sigmastar/2) * pnorm(mui_Gi/sigmastar) * Gi/(pnorm(mui/sigmastar) *
      Hi2)
