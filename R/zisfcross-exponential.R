...
cstzisfexponorm <- function(olsObj, epsiRes, nXvar, nuZUvar, 
  ...
  names(StartVal) <- c(names(Esti)[1:nXvar], paste0("Zu_", 
    ...
    colnames(Zvar)))
names(initExpo$estimate) <- c(names(Esti)[1:nXvar], paste0(
    "Zu_",
    colnames(uHvar)[1]
...
chesszisfexponormlike <- function(parm, nXvar, nuZUvar, nvZVvar, 
  ...
  hessll[1:nXvar, (nXvar + 1):(nXvar + nuZUvar)] <- crossprod(sweep(Xvar, 
    MARGIN = 1, STATS = wHvar*S * (((((0.5 + 0.5 * (S * (epsilon)/ewusr) + 
      2 * (ewu * ewv/(2 * ewu)^2)) * pmustar - 0.5 * deemu)/ewusr + 
      (0.5 * (sigx12/ewusr) - sigx3/ewvsr) * dmustar)/(1 + 
      ewz) + 0.5 * sigx4)/ewusr - (sigx5 * sigx2/sigx1 + 
      0.5 * ((1 + ewz) * dmustar * ewusr/(((1 + ewz) * 
        ewusr)^2 * ewvsr)))) * epsi * ewz/sigx1, FUN = "*"), 
    uHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + 1):(nXvar + nuZUvar + 
    nvZVvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = wHvar*S * 
    ((dmustar * (ewv/(2 * ewu) - (sigx6 * sigx12 + 0.5))/ewvsr - 
      sigx7/ewusr) * epsi * ewz/((1 + ewz) * ewusr) + S * 
      (0.5 * (S^2 * (epsilon)^2/ewvsr^2 - 2) - 0.5) * wzdeno * 
      dwsr * (epsilon)/ewvsr^3 - sigx9 * sigx2/sigx1)/sigx1, 
    FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuZUvar + nvZVvar + 1):(nXvar + 
    nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(Xvar, 
    MARGIN = 1, STATS = wHvar*S * (sigx10 * dpmu * epsi - (sigx11 * 
      sigx2/sigx1 + S * wzdeno * dwsr * (epsilon)/((1 + 
      ewz) * ewvsr^3))) * ewz/sigx1, FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + 1):(nXvar + 
    nuZUvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = wHvar*(((0.5 * 
    (0.5 * (ewvsr * sigx12/ewusr) - 0.5) - 0.5 * sigx3) * 
    dmustar * ewvsr/ewusr - (2 * ((1 - 8 * (ewu^2/(2 * ewu)^2)) * 
    ewu * ewv/(2 * ewu)^2) - 0.25 * (S * (epsilon)/ewusr)) * 
    pmustar)/((1 + ewz) * ewusr) - ((sigx5 * epsi * ewz/sigx1 + 
    0.5 * (S * (epsilon)/ewusr) + 2 * (ewu * ewv/(2 * ewu)^2)) * 
    sigx5 + (0.5 * ((0.5 - (1 + ewz)^2 * ewusr^2/((1 + ewz) * 
    ewusr)^2) * ewusr * pmustar + 0.5 * (dmustar * ewvsr)) + 
    0.5 * ((0.5 * deemu - sigx3 * pmustar) * ewusr)) * (1 + 
    ewz)/((1 + ewz) * ewusr)^2)) * epsi * ewz/sigx1, FUN = "*"), 
    uHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 
    1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(uHvar, 
    MARGIN = 1, STATS = wHvar*(((dmustar * ewvsr/(4 * (ewu * ewusr)) - 
      2 * (ewu * pmustar/(2 * ewu)^2)) * ewv - ((0.5 * 
      (sigx6 * sigx12) - 0.25) * dmustar * ewvsr/ewusr + 
      sigx3 * sigx7))/((1 + ewz) * ewusr) - (sigx5 * sigx9/sigx1 + 
      0.5 * ((1 + ewz) * ewusr * sigx7/((1 + ewz) * ewusr)^2))) * 
      epsi * ewz/sigx1, FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuZUvar), (nXvar + nuZUvar + 
    nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + nZHvar)] <- crossprod(sweep(uHvar, 
    MARGIN = 1, STATS = wHvar*(0.5 * (sigx10 * dmustar * ewvsr/ewusr) - 
      ((((0.5 - (1 + ewz)^2 * ewusr^2/((1 + ewz) * ewusr)^2) * 
        ewz + 0.5 * (1 + ewz)) * ewusr/((1 + ewz) * ewusr)^2 + 
        sigx3 * sigx10) * pmustar + sigx5 * sigx11 * 
        ewz/sigx1)) * epsi * ewz/sigx1, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), 
    (nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)] <- crossprod(sweep(vHvar, 
    MARGIN = 1, STATS = wHvar*(((sigx7/2 + (pmustar - sigx6 * dmustar)/2) * 
      ewv/ewu - (0.25 * (ewvsr/ewusr) + 0.25 * (S * (epsilon)/ewvsr) - 
      sigx6^2 * sigx12) * dmustar) * epsi * ewz/((1 + ewz) * 
      ewusr) + wzdeno * (S^2 * (0.5 * (0.5 * (S^2 * (epsilon)^2/ewvsr^2) - 
      1) - 0.25) * dwsr * (epsilon)^2/ewvsr^2 - 0.5 * sigx8)/ewvsr - 
      sigx9^2/sigx1)/sigx1, FUN = "*"), vHvar)
  hessll[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar), 
    (nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + nvZVvar + 
      nZHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = wHvar*(sigx10 * 
    epsi * sigx7 - (sigx9 * sigx11/sigx1 + (0.5 * (S^2 * 
    dwsr * (epsilon)^2/((1 + ewz) * ewvsr^3)) - 0.5 * ((1 + 
    ewz) * dwsr * ewvsr/((1 + ewz) * ewvsr)^2)) * wzdeno)) * 
    ewz/sigx1, FUN = "*"), Zvar)
  hessll[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 
    ...
    MARGIN = 1, STATS = wHvar*((wzdeno * (1/((1 + ewz)^2 * ewvsr) + 
      ewvsr/((1 + ewz) * ewvsr)^2) * dwsr - (sigx11^2/sigx1 + 
...
#' @noRd
zisfexponormAlgOpt <- function(start, olsParam, dataTable, S, wHvar, 
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal, 
    fn = function(parm) -sum(czisfexponormlike(parm, nXvar = nXvar, 
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar, 
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, wHvar = wHvar, Zvar = Zvar, 
      nZHvar = nZHvar)), gr = function(parm) -colSums(cgradzisfexponormlike(parm, 
    ...
    maxLikAlgo = maxRoutine(fn = czisfexponormlike, grad = cgradzisfexponormlike, 
      ...
      fn = function(parm) -sum(czisfexponormlike(parm, 
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gr = function(parm) -colSums(cgradzisfexponormlike(parm, 
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
      ...
      fn = function(parm) -sum(czisfexponormlike(parm, 
        ...
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), hs = function(parm) as(-chesszisfexponormlike(parm, 
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
      ...
      fn = function(parm) -sum(czisfexponormlike(parm, 
        ...
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), hess = function(parm) -chesszisfexponormlike(parm, 
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
        S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar), print.info = printInfo, 
      maxiter = itermax, epsa = gradtol, epsb = gradtol), 
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(czisfexponormlike(parm, 
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), gradient = function(parm) -colSums(cgradzisfexponormlike(parm, 
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
      S = S, wHvar = wHvar, Zvar = Zvar, nZHvar = nZHvar)), hessian = function(parm) -chesszisfexponormlike(parm, 
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, 
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, 
...
czisfexponormeff <- function(parm) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  theta <- parm[(nXvar + nuZUvar + nvZVvar + 1):(nXvar + nuZUvar + 
  ...
  Wz <- as.numeric(crossprod(matrix(theta), t(Zvar)))
  epsilon <- Yvar - as.numeric(crossprod(matrix(beta), t(Xvar)))
  mustar <- -S * epsilon - exp(Wv)/sqrt(exp(Wu))
  Pi1 <- 1/exp(Wu/2) * exp(S * epsilon/exp(Wu/2) + exp(Wv)/(2 * 
    exp(Wu))) * pnorm(-S * epsilon/exp(Wv/2) - exp(Wv/2)/exp(Wu/2))
  Pi2 <- 1/exp(Wv/2) * dnorm(S * epsilon/exp(Wv/2))
  Probc1 <- exp(Wz)/(1 + exp(Wz))
  ...
  teBC_Wc <- Pcond_c1 * teBC_c1 + Pcond_c2
  res <- bind_cols(Group_c = Group_c, u_c1 = u_c1, u_c2 = u_c2, 
    u_c = u_c, u_Wc = u_Wc, teJLMS_c1 = teJLMS_c1, teJLMS_c2 = teJLMS_c2, 
    teJLMS_c = teJLMS_c, teJLMS_Wc = teJLMS_Wc, teBC_c1 = teBC_c1, 
    teBC_c2 = teBC_c2, teBC_c = teBC_c, teBC_Wc = teBC_Wc, 
