...
#' @noRd
pgammanormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
...
#' @noRd
pstgammanorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar, nvZVvar, uHvar, vHvar,
...
#' @param TT vector of time of presence
#' @noRd
pgradgammanormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar, uHvar, vHvar, Yvar,
  Xvar, pindex, TT, S, N, wHvar, FiMat) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  P <- parm[nXvar + nuZUvar + nvZVvar + 1]
  ...
  sigx9 <- (0.5 * (epsi_uv * gid2_gicub/sqrt(gisq)) + S * sqrt(gisq) * gid1_epsi_gisq)
  sigx10 <- (sigx9/sigsta - epsi_uv * ewv_h * sqrt(gisq) * gid2_gicub/sigsta^2)
  sigx11 <- (S * gid1_epsi_gisq - epsi_uv * gid2_gicub/gisq)
  sigx12 <- (0.5 * (epsi_uv * gid2sq_gicub/sqrt(gisq)) + S * sqrt(gisq) * gid1sq_epsi_gisq)
  sigx13 <- (sigx12/sigsta - epsi_uv * ewv_h * sqrt(gisq) * gid2sq_gicub/sigsta^2)
  sigx14 <- (S * gid1sq_epsi_gisq - epsi_uv * gid2sq_gicub/gisq)
  ...
  sigFi10 <- sweep((0.5 - 0.5 * (sigFi5)) * (sigFi4)^(P - 2), MARGIN = 1, STATS = ewv *
    (P - 1)/(ewu_h * gisq), FUN = "*")
  sigFi11 <- sweep(sigFi1, MARGIN = 1, STATS = (gid2_gicub/gisq), FUN = "*")
  ...
  sigFi15 <- sweep(sigFi1, MARGIN = 1, STATS = (gid2sq_gicub/gisq), FUN = "*")
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * sigx4/sigsta, FUN = "*") +
    ...
    (P - 1), 1, sum)/sdDiv - 0.5 * (gid2_gicub/gisq), sigx13 * sigx8 + apply((sigFi18) *
    (sigFi4)^(P - 2) * (P - 1), 1, sum)/sdDiv - 0.5 * (gid2sq_gicub/gisq))
...
#' @noRd
gammanormAlgOpt_k90 <- function(start, randStart, sdStart, olsParam, dataTable, S,
  nXvar, N, NT, FiMat_N, FiMat_NT, uHvar_c, uHvar_p, nuZUvar, vHvar_c, vHvar_p,
  ...
  if (!is.null(start)) {
    ...
    start_st <- pstgammanorm_k90(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  startLoglik <- sum(pgammanormlike_k90(startVal, nXvar = nXvar, nuZUvar = nuZUvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal, fn = function(parm) -sum(pgammanormlike_k90(parm,
    ...
    N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_k90(parm,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
    Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
    N = N, FiMat = FiMat_N)), hessian = 0, control = list(trace = if (printInfo) 1 else 0,
    maxeval = itermax, stepmax = stepmax, xtol = tol, grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgammanormlike_k90,
    grad = pgradgammanormlike_k90, start = startVal, finalHessian = if (hessianType ==
    ...
    sr1 = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pgammanormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      fn = function(parm) -sum(pgammanormlike_k90(parm, nXvar = nXvar, nuZUvar = nuZUvar,
        nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
        FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
        S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradgammanormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
      ...
      objective = function(parm) -sum(pgammanormlike_k90(parm, nXvar = nXvar,
        nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
        Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
        N = N, FiMat = FiMat_N)), gradient = function(parm) -colSums(pgradgammanormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgammanormlike_k90(mleObj$par, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p,
      Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  }
  ...
  if (hessianType != 2) {
    if (method %in% c("ucminf", "nlminb"))
      mleObj$hessian <- calculus::jacobian(function(parm) colSums(pgradgammanormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
        vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex, TT = TT,
  ...
  mleObj$logL_OBS <- pgammanormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradgammanormlike_k90(parm = mlParam, nXvar = nXvar, nuZUvar = nuZUvar,
    nvZVvar = nvZVvar, uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  if (is.null(start)) {
...
#' @noRd
pgammanormeff_k90 <- function(object, level) {
