...
#' @noRd
pgammanormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
...
#' @noRd
pstgammanorm_k90 <- function(olsObj, epsiRes, nXvar, nuZUvar,
  nvZVvar, uHvar, vHvar, Yvar, Xvar, S, printInfo, tol, N,
  wHvar, FiMat, whichStart, initIter, initAlg) {
  if (whichStart == 1L) {
...
#' @param TT vector of time of presence
#' @noRd
pgradgammanormlike_k90 <- function(parm, nXvar, nuZUvar, nvZVvar,
  uHvar, vHvar, Yvar, Xvar, pindex, TT, S, N, wHvar, FiMat) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuZUvar)]
  phi <- parm[(nXvar + nuZUvar + 1):(nXvar + nuZUvar + nvZVvar)]
  P <- parm[nXvar + nuZUvar + nvZVvar + 1]
  ...
  sigx9 <- (0.5 * (epsi_uv * gid2_gicub/sqrt(gisq)) + S * sqrt(gisq) *
    gid1_epsi_gisq)
  sigx10 <- (sigx9/sigsta - epsi_uv * ewv_h * sqrt(gisq) *
    gid2_gicub/sigsta^2)
  sigx11 <- (S * gid1_epsi_gisq - epsi_uv * gid2_gicub/gisq)
  sigx12 <- (0.5 * (epsi_uv * gid2sq_gicub/sqrt(gisq)) + S *
    sqrt(gisq) * gid1sq_epsi_gisq)
  sigx13 <- (sigx12/sigsta - epsi_uv * ewv_h * sqrt(gisq) *
    gid2sq_gicub/sigsta^2)
  ...
  gradll <- cbind(sweep(Xgi, MARGIN = 1, STATS = S * sigx4/sigsta,
    ...
    sum)/sdDiv - (0.5 * (Wu) + digamma(P)), sigx10 * sigx8 +
    apply((sigFi14) * (sigFi4)^(P - 2) * (P - 1), 1, sum)/sdDiv -
    0.5 * (gid2_gicub/gisq), sigx13 * sigx8 + apply((sigFi18) *
...
#' @noRd
gammanormAlgOpt_k90 <- function(start, olsParam, dataTable, S,
  nXvar, N, NT, FiMat_N, FiMat_NT, uHvar_c, uHvar_p, nuZUvar,
  ...
  startLoglik <- sum(pgammanormlike_k90(startVal, nXvar = nXvar,
  ...
  mleObj <- switch(method, ucminf = ucminf::ucminf(par = startVal,
    fn = function(parm) -sum(pgammanormlike_k90(parm, nXvar = nXvar,
      nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
      vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
      TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)),
    gr = function(parm) -colSums(pgradgammanormlike_k90(parm,
      ...
      grtol = gradtol)), maxLikAlgo = maxRoutine(fn = pgammanormlike_k90,
    ...
    sparse = trustOptim::trust.optim(x = startVal, fn = function(parm) -sum(pgammanormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      ...
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N)), hs = function(parm) as(calculus::jacobian(function(parm) -colSums(pgradgammanormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      ...
      fn = function(parm) -sum(pgammanormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p, N = N, FiMat = FiMat_N)), gr = function(parm) -colSums(pgradgammanormlike_k90(parm,
        nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
        uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar,
        Xvar = Xvar, pindex = pindex, TT = TT, S = S,
        wHvar = wHvar_p, N = N, FiMat = FiMat_N)), print.info = printInfo,
      maxiter = itermax, epsa = gradtol, epsb = gradtol),
    nlminb = nlminb(start = startVal, objective = function(parm) -sum(pgammanormlike_k90(parm,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
  ...
  if (method %in% c("ucminf", "nlminb")) {
    mleObj$gradient <- colSums(pgradgammanormlike_k90(mleObj$par,
      nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
      uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
      pindex = pindex, TT = TT, S = S, wHvar = wHvar_p,
      N = N, FiMat = FiMat_N))
  ...
  mleObj$logL_OBS <- pgammanormlike_k90(parm = mlParam, nXvar = nXvar,
    nuZUvar = nuZUvar, nvZVvar = nvZVvar, uHvar = uHvar_p,
    vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar, pindex = pindex,
    TT = TT, S = S, wHvar = wHvar_p, N = N, FiMat = FiMat_N)
  mleObj$gradL_OBS <- pgradgammanormlike_k90(parm = mlParam,
    nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar_p, vHvar = vHvar_p, Yvar = Yvar, Xvar = Xvar,
    pindex = pindex, TT = TT, S = S, wHvar = wHvar_p, N = N,
    FiMat = FiMat_N)
