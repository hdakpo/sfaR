...
#' Generalized zero inefficiency stochastic frontier using cross-sectional data
#'
#' @description
...
#'
#' @aliases sfagzisfcross bread.sfagzisfcross estfun.sfagzisfcross 
#' print.sfagzisfcross
#'
...
#' ML estimation. New starting values are obtained as old ones + draws from
...
#' @param x an object of class sfagzisfcross (returned by the function 
#' \code{\link{sfagzisfcross}}).
#' @param ... additional arguments of frontier are passed to sfagzisfcross; 
...
#'
#' @return \code{\link{sfagzisfcross}} returns a list of class 
...
#'
#' @note In the case of panel data, \code{\link{sfagzisfcross}} estimates a 
#' pooled cross-section where the probability of belonging to a class a priori 
#' is not permanent (not fixed over time).
#'
# @author K Hervé Dakpo
#'
#' @seealso \code{\link[=print.sfagzisfcross]{print}} for printing 
...
sfagzisfcross <- function(formula, uhet, vhet, thet, logDepVar = TRUE, data, subset,
  ...
  typeSfa <- if (S == 1L) {
    "Generalized Zero Inefficiency Production/Profit Frontier, e = v - u"
  } else {
    "Generalized Zero Inefficiency Cost Frontier, e = v + u"
  }
  if (length(logDepVar) != 1 || !is.logical(logDepVar[1])) {
    stop("argument 'logDepVar' must be a single logical value", call. = FALSE)
  }
  if (!(gzisfClasses %in% 2:5)) {
    stop("argument 'gzisfClasses' must be comprised between 2 and 5", call. = FALSE)
  }
  # Number of parameters -------
  nParm <- (gzisfClasses - 1) * (nXvar + nuZUvar + nvZVvar) + nXvar + nvZVvar +
    (gzisfClasses - 1) * nZHvar
  ...
  # arguments -------
  FunArgs <- list(start = start, randStart = randStart, sdStart = sdStart, olsParam = olsParam,
    dataTable = dataTable, nXvar = nXvar, nuZUvar = nuZUvar, nvZVvar = nvZVvar,
    uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Zvar = Zvar, nZHvar = nZHvar,
    Xvar = Xvar, S = S, wHvar = wHvar, method = method, initAlg = initAlg, initIter = initIter,
    whichStart = whichStart, printInfo = printInfo, itermax = itermax, stepmax = stepmax,
  ...
  mleList <- tryCatch(switch(as.character(gzisfClasses), `2` = do.call(GZISF2ChnormAlgOpt,
    FunArgs), `3` = do.call(GZISF3ChnormAlgOpt, FunArgs), `4` = do.call(GZISF4ChnormAlgOpt,
    FunArgs), `5` = do.call(GZISF5ChnormAlgOpt, FunArgs)), error = function(e) print(e))
  ...
  # quick renaming -------
  names(mleList$startVal) <- fName_sfagzisfcross(Xvar = Xvar, uHvar = uHvar, vHvar = vHvar,
    Zvar = Zvar, nZHvar = nZHvar, gzisfClasses = gzisfClasses)
  ...
  returnObj$nClasses <- gzisfClasses
  returnObj$mlLoglik <- mleList$mleLoglik
  returnObj$mlParam <- mleList$mlParam
  returnObj$mlParamMatrix <- rbind(matrix(mleList$mlParam[1:(gzisfClasses * (nXvar +
    nuZUvar + nvZVvar))], ncol = gzisfClasses), cbind(matrix(mleList$mlParam[(gzisfClasses *
    (nXvar + nuZUvar + nvZVvar) + 1):(gzisfClasses * (nXvar + nuZUvar + nvZVvar) +
    (gzisfClasses - 1) * nZHvar)], ncol = gzisfClasses - 1), NA))
  colnames(returnObj$mlParamMatrix) <- paste0("Class", 1:gzisfClasses)
  d1names <- names(mleList$mlParam)[c(1:(nXvar + nuZUvar + nvZVvar), (gzisfClasses *
    (nXvar + nuZUvar + nvZVvar) + 1):(gzisfClasses * (nXvar + nuZUvar + nvZVvar) +
    nZHvar))]
  ...
  rm(mleList)
  class(returnObj) <- "sfagzisfcross"
  return(returnObj)
}

# print for sfagzisfcross ----------
#' @rdname sfagzisfcross
#' @exportS3Method print sfagzisfcross
print.sfagzisfcross <- function(x, ...) {
  cat("Call:\n")
  cat(deparse(x$call, width.cutoff = 500))
  cat("\n\n")
  cat("Likelihood estimates using", x$optType, "\n")
  cat("Normal-Half Normal Generalized Zero Inefficiency Stochastic Frontier Model",
    "\n")
...
#' @rdname sfagzisfcross
#' @exportS3Method sandwich::bread sfagzisfcross
bread.sfagzisfcross <- function(x, ...) {
  if (x$hessianType == "Analytic Hessian") {
    ...
    if (x$nClasses == 2) {
      hessAnalytical <- chessGZISFhalfnormlike2C(parm = x$mlParam, nXvar = ncol(Xvar),
        nuZUvar = ncol(uHvar), nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
        Yvar = Yvar, Xvar = Xvar, wHvar = x$dataTable$weights, S = x$S, Zvar = Zvar,
        nZHvar = ncol(Zvar))
    } else {
      if (x$nClasses == 3) {
        hessAnalytical <- chessGZISFhalfnormlike3C(parm = x$mlParam, nXvar = ncol(Xvar),
          nuZUvar = ncol(uHvar), nvZVvar = ncol(vHvar), uHvar = uHvar, vHvar = vHvar,
          Yvar = Yvar, Xvar = Xvar, wHvar = x$dataTable$weights, S = x$S,
          Zvar = Zvar, nZHvar = ncol(Zvar))
      } else {
        if (x$nClasses == 4) {
          hessAnalytical <- chessGZISFhalfnormlike4C(parm = x$mlParam, nXvar = ncol(Xvar),
...
#' @rdname sfagzisfcross
#' @exportS3Method sandwich::estfun sfagzisfcross
estfun.sfagzisfcross <- function(x, ...) {
  return(x$gradL_OBS)
}
